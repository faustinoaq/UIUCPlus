 ...
 * 128-bit hash function {@code MurmurHash3_x64_128} from Austin Appleby's original {@code c++} code
 * in SMHasher.
 *
...
public final class MurmurHash3 {
   ...
   * Appleby's original MurmurHash3 {@code c++} code in SMHasher.
   *
   * <p>This implementation contains a sign-extension bug in the finalization step of any bytes left
   * over from dividing the length by 4. This manifests if any of these bytes are negative.
   *
   ...
   */
  @Deprecated
  public static long buggyHash3(final short data) {
    long hash = DEFAULT_SEED;
    long k1 = 0;
    k1 ^= ((long) data & 0xff) << 8;
    k1 ^= ((long) ((data & 0xFF00) >> 8) & 0xff);
    k1 = (k1 * C1) ^ C2; // changed from k1 *= C2 and k1 = Long.rotateLeft(k1, R1)
    k1 = Long.rotateLeft(k1, R1);
    hash ^= k1;
    hash ^= SHORT_BYTES;
    hash = fmix64(hash);
