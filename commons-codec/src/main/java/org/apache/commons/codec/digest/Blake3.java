...
/**
 * Implements the Blake3 algorithm providing a {@linkplain #initHash() hash function} with
 * extensible output (XOF), a {@linkplain #initKeyedHash(byte[]) keyed hash function} (MAC, PRF),
 * and a {@linkplain #initKeyDerivationFunction(byte[]) key derivation function} (KDF). Blake3 has a
 * 128-bit security level and a default output length of 256 bits (32 bytes) which can extended up
 * to 2<sup>64</sup> bytes.
 *
 * <h2>Hashing</h2>
 *
 * <p>Hash mode calculates the same output hash given the same input bytes and can be used as both a
 * message digest and and extensible output function.
 *
 * <pre>{@code
 ...
 * <h2>Keyed Hashing</h2>
 *
 * <p>Keyed hashes take a 32-byte secret key and calculates a message authentication code on some
 * input bytes. These also work as pseudo-random functions (PRFs) with extensible output similar to
 * the extensible hash output. Note that Blake3 keyed hashes have the same performance as plain
 * hashes; the key is used in initialization in place of a standard initialization vector used for
 * plain hashing.
 *
 * <pre>{@code
 ...
 *
 * <p>A specific hash mode for deriving session keys and other derived keys in a unique key
 * derivation context identified by some sequence of bytes. These context strings should be unique
 * but do not need to be kept secret. Additional input data is hashed for key material which can be
 * finalized to derive subkeys.
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p>Adapted from the ISC-licensed O(1) Cryptography library by Matt Sicker and ported from the
 * reference public domain implementation by Jack O'Connor.
 *
...
public final class Blake3 {
   ...
   * @throws NullPointerException if in is null
   * @throws IndexOutOfBoundsException if offset or length are negative or if offset + length is
  ...
  /**
   * Finalizes hash output data that depends on the sequence of updated bytes preceding this
   * invocation and any previously finalized bytes. Note that this can finalize up to 2<sup>64</sup>
   * bytes per instance.
   *
  ...
  /**
   * Finalizes an arbitrary number of bytes into the provided output array that depends on the
   * sequence of previously updated and finalized bytes. Note that this can finalize up to
  ...
  /**
   * Squeezes and returns an arbitrary number of bytes dependent on the sequence of previously
  ...
  /**
   * Constructs a fresh Blake3 hash function. The instance returned functions as an arbitrary length
   * message digest.
   *
  ...
  /**
   * Constructs a fresh Blake3 keyed hash function. The instance returned functions as a
   * pseudorandom function (PRF) or as a message authentication code (MAC).
   *
  ...
  /**
   * Constructs a fresh Blake3 key derivation function using the provided key derivation context
   * byte string. The instance returned functions as a key-derivation function which can further
   * absorb additional context data before squeezing derived key data.
   *
   * @param kdfContext a globally unique key-derivation context byte string to separate key
   *     derivation contexts from each other
  ...
  /**
   * Represents the state just prior to either producing an eight word chaining value or any number
   * of output bytes when the ROOT flag is set.
   */
  private static class Output {
    ...
    void rootOutputBytes(final byte[] out, int offset, int length) {
      ...
      while (length > 0) {
        ...
        final int[] words =
            compress(
                inputChainingValue, blockWords, blockLength, outputBlockCounter++, flags | ROOT);
        int wordCounter = 0;
        while (chunkLength > 0) {
  ...
  private static class ChunkState {
    ...
    Output output() {
      ...
      return new Output(chainingValue, blockWords, chunkCounter, blockLength, outputFlags + 1);
    }
  }
  ...
  private static class EngineState {
    ...
    // No more than 54 entries can ever be added to this stack (after updating 2^64 bytes and not
    // finalizing any)
    // so we preallocate the stack here. This can be smaller in environments where the data limit is
    // expected to
    // be much lower.
