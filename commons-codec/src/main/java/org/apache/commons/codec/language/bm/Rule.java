...
public class Rule {
  ...
  public static Map<String, List<Rule>> getInstanceMap(
    ...
    if (langs.isSingleton()) {
      return getInstanceMap(nameType, rt, null);
    } else {
      return getInstanceMap(nameType, rt, langs);
    }
  }
      ...
      if (startsWithBox && endsWithBox) {
        ...
        if (!boxContent.contains("[")) {
          ...
          if (startsWith && endsWith) {
            // exact match
            return input ->
                input.length() == 1 && contains(bContent, input.charAt(0)) == shouldMatch;
          }
          if (startsWith) {
            // first char
            return input ->
                input.length() > 0 && contains(bContent, input.charAt(0)) == shouldMatch;
          }
          if (endsWith) {
            // last char
            return input ->
