 ...
 * Key-value entries are stored in instances of the {@code LinkEntry} class which can be overridden
 * and replaced. The iterators can similarly be replaced, without the need to replace the KeySet,
 ...
 * differently. The {@code OrderedMap} interface is implemented to provide access to bidirectional
 * iteration and extra convenience methods.
 *
 * <p>The {@code orderedMapIterator()} method provides direct access to a bidirectional iterator.
 * The iterators from the other views can also be cast to {@code OrderedIterator} if required.
 *
...
public abstract class AbstractLinkedMap<K, V> extends AbstractHashedMap<K, V>
   ...
   */
  @Override
  protected LinkEntry<K, V> createEntry(
  ...
  /**
   * Gets the {@code before} field from a {@code LinkEntry}. Used in subclasses that have no
  ...
  /**
   * Gets the {@code after} field from a {@code LinkEntry}. Used in subclasses that have no
   * visibility of the field.
   *
   * @param entry the entry to query, must not be null
  ...
  protected LinkEntry<K, V> entryAfter(final LinkEntry<K, V> entry) {
    LinkEntry<K, V> after = entry.after;
   ...
   *
   * <p>If you subclass {@code AbstractLinkedMap} but not {@code LinkEntry} then you will not be
   * able to access the protected fields. The {@code entryXxx()} methods on {@code
   * AbstractLinkedMap} exist to provide the necessary access.
   */
