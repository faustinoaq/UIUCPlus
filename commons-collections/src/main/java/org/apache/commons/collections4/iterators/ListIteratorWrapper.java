...
/**
 * Converts an {@link Iterator} into a {@link ResettableListIterator}. For plain {@code Iterator}s
 * this is accomplished by caching the returned elements. This class can also be used to simply add
 * {@link org.apache.commons.collections4.ResettableIterator ResettableIterator} functionality to a
 * given {@link ListIterator}.
 *
 * <p>The {@code ListIterator} interface has additional useful methods for navigation - {@code
 * previous()} and the index methods. This class allows a regular {@code Iterator} to behave as a
 * {@code ListIterator}. It achieves this by building a list internally of as the underlying
 * iterator is traversed.
 *
 * <p>The optional operations of {@code ListIterator} are not supported for plain {@code Iterator}s.
 *
...
public class ListIteratorWrapper<E> implements ResettableListIterator<E> {
  ...
  /**
   * Constructs a new {@code ListIteratorWrapper} that will wrap the given iterator.
   *
  ...
  /**
   * Throws {@link UnsupportedOperationException} unless the underlying {@code Iterator} is a {@code
   * ListIterator}.
   *
  ...
  public E previous() throws NoSuchElementException {
    ...
    if ((currentIndex == 0) && (!removeState || i - currentIndex > 1)) {
  ...
  /**
   * Removes the last element that was returned by {@link #next()} or {@link #previous()} from the
   * underlying collection. This call can only be made once per call to {@code next} or {@code
   * previous} and only if {@link #add(Object)} was not called in between.
   *
   * @throws IllegalStateException if {@code next} or {@code previous} have not been called before,
   *     or if {@code remove} or {@code add} have been called after the last call to {@code next} or
   *     {@code previous}
   */
  @Override
