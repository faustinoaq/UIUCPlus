 ...
 * entries are stored in instances of the {@code HashEntry} class, which can be overridden and
...
public class AbstractHashedMap<K, V> extends AbstractMap<K, V> implements IterableMap<K, V> {
   ...
   * <p>This implementation iterates around the specified map and uses {@link #put(Object, Object)}.
   *
   * <p>It is private to allow the constructor to still call it even when putAll is overridden.
   *
   * @param map the map to add
   * @throws NullPointerException if the map is null
   */
   ...
   *
   * <p>This implementation calls {@code setValue()} on the entry. Subclasses could override to
   ...
   *
   * <p>This implementation calls {@code createEntry()}, {@code addEntry()} and {@code
   * checkCapacity()}. It also handles changes to {@code modCount} and {@code size}. Subclasses
   * could override to fully control adds to the map.
   *
   ...
   *
   * <p>This implementation calls {@code removeEntry()} and {@code destroyEntry()}. It also handles
   * changes to {@code modCount} and {@code size}. Subclasses could override to fully control
  ...
  protected void removeMapping(
      final HashEntry<K, V> entry, final int hashIndex, final HashEntry<K, V> previous) {
    modCount++;
  ...
  /**
   * Gets the {@code next} field from a {@code HashEntry}. Used in subclasses that have no
  ...
  /**
   * Gets the {@code hashCode} field from a {@code HashEntry}. Used in subclasses that have no
   * visibility of the field.
   *
   * @param entry the entry to query, must not be null
  ...
  /**
   * Gets the {@code key} field from a {@code HashEntry}. Used in subclasses that have no visibility
   * of the field.
   *
   * @param entry the entry to query, must not be null
  ...
  /**
   * Gets the {@code value} field from a {@code HashEntry}. Used in subclasses that have no
   * visibility of the field.
   *
   * @param entry the entry to query, must not be null
  ...
  protected static class HashMapIterator<K, V> extends HashIterator<K, V>
    ...
    public V setValue(final V value) {
      final HashEntry<K, V> current = currentEntry();
      if (current == null) {
        throw new IllegalArgumentException(AbstractHashedMap.SETVALUE_INVALID);
      }
   ...
   *
   * <p>If you subclass {@code AbstractHashedMap} but not {@code HashEntry} then you will not be
   * able to access the protected fields. The {@code entryXxx()} methods on {@code
   * AbstractHashedMap} exist to provide the necessary access.
   *
  ...
  protected static class HashEntry<K, V> implements Map.Entry<K, V>, KeyValue<K, V> {
    ...
    protected HashEntry(
        final HashEntry<K, V> next, final int hashCode, final Object key, final V value) {
      this.next = next;
   ...
   *
   * <p>To implement {@code clone()}, a subclass must implement the {@code Cloneable} interface and
