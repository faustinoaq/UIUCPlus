...
public abstract class AbstractLinkedList<E> implements List<E> {
   ...
   * value of {@code next} is the first item in the list. The value of {@code previous} is the last
   * item in the list.
   */
   ...
   * by {@code readObject}. Subclasses which override this method should make sure they call super,
   ...
   * to see if it's contained in {@code coll}. If it's contained, it's removed from this list. As a
   * consequence, it is advised to use a collection type for {@code coll} that provides a fast (e.g.
   ...
   *
   * <p>This implementation iterates over the elements of this list, checking each element in turn
   * to see if it's contained in {@code coll}. If it's not contained, it's removed from this list.
   * As a consequence, it is advised to use a collection type for {@code coll} that provides a fast
   * (e.g. O(1)) implementation of {@link Collection#contains(Object)}.
   */
  @Override
  public boolean retainAll(final Collection<?> coll) {
    boolean modified = false;
    // BUGGY VERSION 2 - start
    final Iterator<E> it = coll.iterator();
    // BUGGY VERSION 2 - end
    while (it.hasNext()) {
      if (!coll.contains(it.next())) {
        it.remove();
  ...
  /**
   * Creates a new node with the specified object as its {@code value} and inserts it before {@code
   * node}.
   *
  ...
  /**
   * Creates a new node with the specified object as its {@code value} and inserts it after {@code
   * node}.
   *
   * <p>This implementation uses {@link #createNode(Object)} and {@link
   * #addNode(AbstractLinkedList.Node,AbstractLinkedList.Node)}.
   ...
   */
  protected ListIterator<E> createSubListListIterator(
      final LinkedSubList<E> subList, final int fromIndex) {
   ...
   *
   * <p>The first serializable subclass must call this method from {@code writeObject}.
   *
   ...
   *
   * <p>The first serializable subclass must call this method from {@code readObject}.
   *
   ...
   *
   * <p>From Commons Collections 3.1, all access to the {@code value} property is via the methods on
   * this class.
   */
  ...
  protected static class LinkedListIterator<E> implements ListIterator<E>, OrderedIterator<E> {
    ...
    /**
     * The last node that was returned by {@link #next()} or {@link #previous()}. Set to {@code
     * null} if {@link #next()} or {@link #previous()} haven't been called, or if the node has been
     * removed with {@link #remove()} or a new node added with {@link #add(Object)}. Should be
     * accessed through {@link #getLastNodeReturned()} to enforce this behavior.
     */
     ...
     * @throws IndexOutOfBoundsException if fromIndex is less than 0 or greater than the size of the
     *     list
     */
  ...
  protected static class LinkedSubList<E> extends AbstractList<E> {
    ...
    protected LinkedSubList(
        final AbstractLinkedList<E> parent, final int fromIndex, final int toIndex) {
      if (fromIndex < 0) {
