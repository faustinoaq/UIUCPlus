...
/**
 * Abstract implementation of the {@link MultiValuedMap} interface to simplify the creation of
 * subclass implementations.
 *
 * <p>Subclasses specify a Map implementation to use as the internal storage.
 *
...
public abstract class AbstractMultiValuedMap<K, V> implements MultiValuedMap<K, V> {
  ...
  /**
   * Gets the collection of values associated with the specified key. This would return an empty
   * collection in case the mapping is not present
   *
   * @param key the key to retrieve
   * @return the {@code Collection} of values, will return an empty {@code Collection} for no
   *     mapping
   */
  @Override
   ...
   *
   * <p>A subsequent {@code get(Object)} would return an empty collection.
   *
   * @param key the key to remove values from
   * @return the {@code Collection} of values removed, will return an empty, unmodifiable collection
   ...
   *
   * <p>The value is removed from the collection mapped to the specified key. Other values attached
   * to that key are unaffected.
   *
   * <p>If the last value for a key is removed, an empty collection would be returned from a
   * subsequent {@link #get(Object)}.
   *
   ...
   * {@inheritDoc}
   *
   * <p>This implementation does <b>not</b> cache the total size of the multivalued map, but rather
   * calculates it by iterating over the entries of the underlying map.
   */
  @Override
  public int size() {
   ...
   *
   * <p>Returns a collection containing all the values from all keys.
   *
  ...
  /**
   * Copies all of the mappings from the specified MultiValuedMap to this map. The effect of this
   * call is equivalent to that of calling {@link #put(Object,Object) put(k, v)} on this map once
   * for each mapping from key {@code k} to value {@code v} in the specified map. The behavior of
   * this operation is undefined if the specified map is modified while the operation is in
   * progress.
   *
   * @param map mappings to be stored in this map, may not be null
   * @return true if the map changed as a result of this operation
   ...
   *
   * <p>Returns a MultiSet of keys with its values count as the count of the MultiSet. This multiset
   * is backed by the map, so any changes in the map is reflected here. Any method which modifies
   * this multiset like {@code add}, {@code remove}, {@link Iterator#remove()} etc throws {@code
   ...
   *
   * <p>Currently, the wrapped collection is not cached and has to be retrieved from the underlying
   * map. This is safe, but not very efficient and should be improved in subsequent releases. For
   * this purpose, the scope of this collection is set to package private to simplify later
   * refactoring.
   */
    ...
    public Iterator<Entry<K, V>> iterator() {
      return new LazyIteratorChain<Entry<K, V>>() {
        ...
        protected Iterator<? extends Entry<K, V>> nextIterator(final int count) {
          ...
          final Transformer<V, Entry<K, V>> entryTransformer =
              input -> new MultiValuedMapEntry(key, input);
