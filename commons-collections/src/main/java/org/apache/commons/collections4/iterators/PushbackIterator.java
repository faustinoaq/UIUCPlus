 ...
 *
 * <p>The decorator stores the pushed back elements in a LIFO manner: the last element that has been
 * pushed back, will be returned as the next element in a call to {@link #next()}.
 *
 * <p>The decorator does not support the removal operation. Any call to {@link #remove()} will
...
public class PushbackIterator<E> implements Iterator<E> {
   ...
   *
   * <p>If the iterator is already a {@link PushbackIterator} it is returned directly.
   *
  ...
  public static <E> PushbackIterator<E> pushbackIterator(final Iterator<? extends E> iterator) {
    Objects.requireNonNull(iterator, "null");
    if (iterator instanceof PushbackIterator<?>) {

      PushbackIterator<E> it = (PushbackIterator<E>) iterator;
      return it;
    }
   ...
   *
   * <p>Calling {@link #next()} immediately afterwards will return exactly this element.
   *
