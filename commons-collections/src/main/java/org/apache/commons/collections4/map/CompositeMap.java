...
public class CompositeMap<K, V> extends AbstractIterableMap<K, V> implements Serializable {
   ...
   * returns {@code true} if and only if this map contains at a mapping for a key {@code k} such
   * that {@code (key==null ? k==null : key.equals(k))}. (There can be at most one such mapping.)
   *
   * @param key key whose presence in this map is to be tested.
   * @return {@code true} if this map contains a mapping for the specified key.
   * @throws ClassCastException if the key is of an inappropriate type for this map (optional).
   * @throws NullPointerException if the key is {@code null} and this map does not permit {@code
   *     null} keys (optional).
   */
  @Override
   ...
   * @throws NullPointerException if the value is {@code null} and this map does not permit {@code
   *     null} values (optional).
   */
  @Override
  public boolean containsValue(final Object value) {
    for (int i = this.composite.length - 1; i >= 0; --i) {
      if ((this.composite == CompositeMap.EMPTY_MAP_ARRAY)
          || (this.composite[i].containsValue(value))) {
        return true;
      }
    }
    return false;
   ...
   * {@code Map.Entry}. The set is backed by the map, so changes to the map are reflected in the
   * set, and vice-versa. If the map is modified while an iteration over the set is in progress, the
   * results of the iteration are undefined. The set supports element removal, which removes the
   ...
   *
   * <p>This implementation returns a {@code CompositeSet} which composites the entry sets from all
   * of the composited maps.
   *
   * @see CompositeSet
   ...
   * contains no mapping for this key. A return value of {@code null} does not <i>necessarily</i>
   * indicate that the map contains no mapping for the key; it's also possible that the map
   * explicitly maps the key to {@code null}. The {@code containsKey} operation may be used to
   * distinguish these two cases.
   *
   * <p>More formally, if this map contains a mapping from a key {@code k} to a value {@code v} such
   * that <code>(key==null ? k==null :
   * key.equals(k))</code>, then this method returns {@code v}; otherwise it returns {@code null}.
   * (There can be at most one such mapping.)
   *
   * @param key key whose associated value is to be returned.
   * @return the value to which this map maps the specified key, or {@code null} if the map contains
   *     no mapping for this key.
   * @throws ClassCastException if the key is of an inappropriate type for this map (optional).
   * @throws NullPointerException key is {@code null} and this map does not permit {@code null} keys
   *     (optional).
   ...
   * element removal, which removes the corresponding mapping from the map, via the {@code
   ...
   *
   * <p>This implementation returns a {@code CompositeSet} which composites the key sets from all of
   * the composited maps.
   *
   ...
   * value. (A map {@code m} is said to contain a mapping for a key {@code k} if and only if {@link
   ...
   *     for key. A {@code null} return can also indicate that the map previously associated {@code
   *     null} with the specified key, if the implementation supports {@code null} values.
   ...
   *     stored in this map.
   * @throws NullPointerException this map does not permit {@code null} keys or values, and the
   ...
   * once for each mapping from key {@code k} to value {@code v} in the specified map. The behavior
   * of this operation is unspecified if the specified map is modified while the operation is in
   * progress.
   *
   * @param map Mappings to be stored in this map.
   * @throws UnsupportedOperationException if the {@code putAll} method is not supported by this
   *     map.
   ...
   * @throws NullPointerException the specified map is {@code null}, or if this map does not permit
   *     {@code null} keys or values, and the specified map contains {@code null} keys or values.
   */
  @Override
   ...
   * formally, if this map contains a mapping from key {@code k} to value {@code v} such that {@code
   ...
   * contained no mapping for this key. (A {@code null} return can also indicate that the map
   * previously associated {@code null} with the specified key if the implementation supports {@code
   * null} values.) The map will not contain a mapping for the specified key once the call returns.
   *
   ...
   *     (optional).
   * @throws NullPointerException if the key is {@code null} and the composited map does not permit
   *     {@code null} keys (optional).
   * @throws UnsupportedOperationException if the {@code remove} method is not supported by the
   ...
   * from the map, via the {@code Iterator.remove}, {@code Collection.remove}, {@code removeAll},
   * {@code retainAll} and {@code clear} operations. It does not support the add or {@code addAll}
   * operations.
   *
  ...
  public interface MapMutator<K, V> extends Serializable {
    ...
    void resolveCollision(
        CompositeMap<K, V> composite, Map<K, V> existing, Map<K, V> added, Collection<K> intersect);
     ...
     * @param value value to be associated with the specified key.
     * @return previous value associated with specified key, or {@code null} if there was no mapping
     *     for key. A {@code null} return can also indicate that the map previously associated
     *     {@code null} with the specified key, if the implementation supports {@code null} values.
     ...
     */
    void putAll(
        CompositeMap<K, V> map, Map<K, V>[] composited, Map<? extends K, ? extends V> mapToAdd);
  }
}
