...
public class IteratorUtils {
  ...
  /**
   * Default delimiter used to delimit elements while converting an Iterator to its String
   * representation.
   */
   ...
   * @throws NullPointerException if array is null
   */
  public static <E> ResettableListIterator<E> arrayListIterator(
      final Object array, final int start) {
   ...
   * @since 4.1
   */
  public static <E> BoundedIterator<E> boundedIterator(
      final Iterator<? extends E> iterator, final long max) {
    return boundedIterator(iterator, 0, max);
  }

  /**
   * Decorates the specified iterator to return at most the given number of elements, skipping all
   * elements until the iterator reaches the position at {@code offset}.
   *
   * <p>The iterator is immediately advanced until it reaches the position at {@code offset},
   * incurring O(n) time.
   *
   * @param <E> the element type
   * @param iterator the iterator to decorate
   ...
   */
  public static <E> BoundedIterator<E> boundedIterator(
      final Iterator<? extends E> iterator, final long offset, final long max) {
  ...
  public static <K, V> MapIterator<K, V> unmodifiableMapIterator(
      final MapIterator<K, V> mapIterator) {
   ...
   */
  public static <E> Iterator<E> chainedIterator(
      final Iterator<? extends E> iterator1, final Iterator<? extends E> iterator2) {
   ...
   *
   * <p>Given two ordered {@link Iterator}s {@code A} and {@code B}, the {@link Iterator#next()}
   * method will return the lesser of {@code A.next()} and {@code B.next()}.
   *
   ...
   *
   * <p>Given two ordered {@link Iterator}s {@code A} and {@code B}, the {@link Iterator#next()}
   * method will return the lesser of {@code A.next()} and {@code B.next()} and so on.
   *
   * <p>The comparator is optional. If null is specified then natural order is used.
   *
   * @param <E> the element type
   ...
   */
  public static <E> Iterator<E> collatedIterator(
      final Comparator<? super E> comparator, final Iterator<? extends E>... iterators) {
    @SuppressWarnings("unchecked")
    final Comparator<E> comp =
        comparator == null ? ComparatorUtils.NATURAL_COMPARATOR : (Comparator<E>) comparator;
   ...
   */
  public static <E> Iterator<E> collatedIterator(
      final Comparator<? super E> comparator, final Collection<Iterator<? extends E>> iterators) {
    @SuppressWarnings("unchecked")
    final Comparator<E> comp =
        comparator == null ? ComparatorUtils.NATURAL_COMPARATOR : (Comparator<E>) comparator;
    return new CollatingIterator<>(comp, iterators);
   ...
   * iteration starts from a single root object. It uses a {@code Transformer} to extract the
   * iterators and elements. Its main benefit is that no intermediate {@code List} is created.
   *
   ...
   *                 |- Branch -- Leaf</pre>
   *
   * The following {@code Transformer}, used in this class, will extract all the Leaf objects
   ...
   * @since 3.1
   */
  public static <E> Iterator<E> objectGraphIterator(
      final E root, final Transformer<? super E, ? extends E> transformer) {
   ...
   * @throws NullPointerException if either parameter is null
   */
  public static <E> Iterator<E> filteredIterator(
      final Iterator<? extends E> iterator, final Predicate<? super E> predicate) {
    Objects.requireNonNull(iterator, "iterator");
   ...
   * @throws NullPointerException if either parameter is null
   */
  public static <E> ListIterator<E> filteredListIterator(
      final ListIterator<? extends E> listIterator, final Predicate<? super E> predicate) {
   ...
   */
  public static <E> ResettableIterator<E> loopingIterator(
  ...
  /**
   * Gets an {@link Iterator} that wraps the specified {@link NodeList}. The returned {@link
  ...
  /**
   * Gets an {@link Iterator} that wraps the specified node's childNodes. The returned {@link
   * Iterator} can be used for a single iteration.
   *
   * <p>Convenience method, allows easy iteration over NodeLists:
   *
   * <pre>
   ...
   * @since 4.1
   */
  public static <E> SkippingIterator<E> skippingIterator(
   ...
   * @since 4.1
   */
  public static <E> ZippingIterator<E> zippingIterator(
      final Iterator<? extends E> a, final Iterator<? extends E> b) {
   ...
   */
  public static <E> ZippingIterator<E> zippingIterator(
      final Iterator<? extends E> a, final Iterator<? extends E> b, final Iterator<? extends E> c) {
  ...
  public static <E> ZippingIterator<E> zippingIterator(final Iterator<? extends E>... iterators) {
    return new ZippingIterator<>(Arrays.copyOf(iterators, iterators.length));
  }
  ...
  public static <E> Iterator<E> asIterator(
      final Enumeration<? extends E> enumeration, final Collection<? super E> removeCollection) {
  ...
  /**
   * Gets an {@link Iterable} that wraps an iterator. The returned {@link Iterable} can be used for
   * a single iteration.
   *
   * @param <E> the element type
   * @param iterator the iterator to use, may not be null
  ...
  /**
   * Gets an iterable that wraps an iterator. The returned iterable can be used for multiple
   * iterations.
   *
   * @param <E> the element type
   * @param iterator the iterator to use, may not be null
   ...
   *
   * <p>This method can handle objects as follows
   *
   * <ul>
   ...
   *
   * <p>If the input iterator is null no change is made.
   *
   ...
   * @return the first element of the iterator which matches the predicate or null if none could be
   *     found
  ...
  /**
   * Returns the index of the first element in the specified iterator that matches the given
   * predicate.
   *
   * <p>A {@code null} or empty iterator returns -1.
   *
   * @param <E> the element type
   * @param iterator the iterator to search, may be null
   ...
   *
   * <p>A {@code null} or empty iterator returns false.
   *
   * @param <E> the type of object the {@link Iterator} contains
  ...
  public static <E> boolean matchesAny(
      final Iterator<E> iterator, final Predicate<? super E> predicate) {
   ...
   *
   * <p>A {@code null} or empty iterator returns true.
   *
   * @param <E> the type of object the {@link Iterator} contains
   * @param iterator the {@link Iterator} to use, may be null
   * @param predicate the predicate to use, may not be null
   ...
   *
   * <p>Returns the {@code first} value in {@link Iterator}, throwing {@code
   * IndexOutOfBoundsException} if there is no such element.
   *
   * <p>The Iterator is advanced to {@code 0} (or to the end, if {@code 0} exceeds the number of
   ...
   *
   * <p>The string representation consists of a list of the iterator's elements, enclosed in square
  ...
  public static <E> String toString(
      final Iterator<E> iterator, final Transformer<? super E, String> transformer) {
    return toString(
        iterator,
        transformer,
        DEFAULT_TOSTRING_DELIMITER,
   ...
   * Returns a string representation of the elements of the specified iterator.
   *
   * <p>The string representation consists of a list of the iterator's elements, enclosed by the
