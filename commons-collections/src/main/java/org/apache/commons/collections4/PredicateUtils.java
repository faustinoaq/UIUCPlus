...
/**
 * {@code PredicateUtils} provides reference implementations and utilities for the Predicate functor
 * interface. The supplied predicates are:
 *
 * <ul>
...
public class PredicateUtils {
   ...
   * instanceof. A {@code null} input object will return {@code false}.
   *
   ...
   * same object is received again. The comparison is by equals(). A {@code null} input object is
   * accepted and will return true the first time, and false subsequently as well.
   *
   * @param <T> the type that the predicate queries
   * @return the predicate
   * @see UniquePredicate
   ...
   *
   * <p>For example, {@code PredicateUtils.invokerPredicate("isEmpty");} will call the {@code
   * isEmpty} method on the input object to determine the predicate result.
   *
   * @param <T> the type that the predicate queries
   ...
   * @see TransformerPredicate
   */
  public static <T> Predicate<T> invokerPredicate(
      final String methodName, final Class<?>[] paramTypes, final Object[] args) {
    // reuse transformer as it has caching - this is lazy really, should have inner class here
    return asPredicate(
        InvokerTransformer.<Object, Boolean>invokerTransformer(methodName, paramTypes, args));
  }
   ...
   * @see AllPredicate
   */
  public static <T> Predicate<T> allPredicate(
      final Collection<? extends Predicate<? super T>> predicates) {
    return AllPredicate.allPredicate(predicates);
  }
   ...
   * @see OrPredicate
   */
  public static <T> Predicate<T> orPredicate(
      final Predicate<? super T> predicate1, final Predicate<? super T> predicate2) {
   ...
   * @see AnyPredicate
   */
  public static <T> Predicate<T> anyPredicate(
      final Collection<? extends Predicate<? super T>> predicates) {
    Predicate<T> result = AnyPredicate.anyPredicate(predicates);
    if (result != null) {
      return result;
    }
    return t -> false;
   ...
   * @see OnePredicate
   */
  public static <T> Predicate<T> eitherPredicate(
      final Predicate<? super T> predicate1, final Predicate<? super T> predicate2) {
   ...
   * @see OnePredicate
   */
  public static <T> Predicate<T> onePredicate(
      final Collection<? extends Predicate<? super T>> predicates) {
    return OnePredicate.onePredicate(predicates);
  }
   ...
   * @see NonePredicate
   */
  public static <T> Predicate<T> neitherPredicate(
      final Predicate<? super T> predicate1, final Predicate<? super T> predicate2) {
    @SuppressWarnings("unchecked")
   ...
   * @see NonePredicate
   */
  public static <T> Predicate<T> nonePredicate(
      final Collection<? extends Predicate<? super T>> predicates) {
    return NonePredicate.nonePredicate(predicates);
  }
   ...
   * specified Predicate. This allows null handling behavior to be added to Predicates that don't
   * support nulls.
   *
   * @param <T> the type that the predicate queries
