...
/**
 * This class implements the base PATRICIA algorithm and everything that is related to the {@link
 * Map} interface.
 *
 * @since 4.0
 */
abstract class AbstractPatriciaTrie<K, V> extends AbstractBitwiseTrie<K, V> {
  ...
  /**
   * The number of times this {@link org.apache.commons.collections4.Trie} has been modified. It's
   * used to detect concurrent modifications and fail-fast the {@link Iterator}s.
   */
  ...
  /**
   * Constructs a new {@link org.apache.commons.collections4.Trie} using the given {@link
   * KeyAnalyzer} and initializes the {@link org.apache.commons.collections4.Trie} with the values
   * from the provided {@link Map}.
   */
  protected AbstractPatriciaTrie(
      final KeyAnalyzer<? super K> keyAnalyzer, final Map<? extends K, ? extends V> map) {
    super(keyAnalyzer);
    putAll(map);
  }
  ...
  /**
   * A helper method to increment the {@link org.apache.commons.collections4.Trie} size and the
  ...
  /**
   * A helper method to decrement the {@link org.apache.commons.collections4.Trie} size and
  ...
  /**
   * Returns the entry associated with the specified key in the PatriciaTrieBase. Returns null if
   * the map contains no mapping for this key.
   *
   * <p>This may throw ClassCastException if the object is not of type K.
   */
  ...
  /**
   * Returns the {@link java.util.Map.Entry} whose key is closest in a bitwise XOR metric to the
   * given key. This is NOT lexicographic closeness. For example, given the keys:
   *
   * <ol>
   *   <li>D = 1000100
   *   <li>H = 1001000
   ...
   * </ol>
   *
   * If the {@link org.apache.commons.collections4.Trie} contained 'H' and 'L', a lookup of 'D'
   * would return 'L', because the XOR distance between D &amp; L is smaller than the XOR distance
   * between D &amp; H.
   *
   * @param key the key to use in the search
   * @return the {@link java.util.Map.Entry} whose key is closest in a bitwise XOR metric to the
   *     provided key
   */
  ...
  /**
   * Returns the key that is closest in a bitwise XOR metric to the provided key. This is NOT
   * lexicographic closeness!
   *
   * <p>For example, given the keys:
   *
   * <ol>
   *   <li>D = 1000100
   *   <li>H = 1001000
  ...
  /**
   * Returns the value whose key is closest in a bitwise XOR metric to the provided key. This is NOT
   * lexicographic closeness!
   *
   * <p>For example, given the keys:
   *
   ...
   *
   * @param key the key to use in the search
   * @return the value whose key is closest in a bitwise XOR metric to the provided key
   */
  ...
  private boolean selectR(
      final TrieEntry<K, V> h,
      final int bitIndex,
      final K key,
  ...
  /**
   * Returns the nearest entry for a given key. This is useful for finding knowing if a given key
   * exists (and finding the value for it), or for inserting the key.
   *
   * <p>The actual get implementation. This is very similar to selectR but with the exception that
   * it might return the root Entry even if it's empty.
   */
   ...
   *
   * <p>If we found a Key (Entry h) then figure out if it's an internal (hard to remove) or external
   * Entry (easy to remove)
   */
   ...
   *
   * <p>If it's an external Entry then just remove it. This is very easy and straight forward.
   */
   ...
   *
   * <p>If it's an internal Entry then "good luck" with understanding this code. The Idea is
   * essentially that Entry p takes Entry h's place in the trie which requires some re-wiring.
   */
  ...
  /**
   * Returns the entry lexicographically after the given entry. If the given entry is null, returns
   * the first node.
   */
  ...
  /**
   * Scans for the next node, starting at the specified point, and using 'previous' as a hint that
   * the last node we returned was 'previous' (so we know not to return it again). If 'tree' is
   * non-null, this will limit the search to the given tree.
   *
   * <p>The basic premise is that each iteration can follow the following steps:
   *
   * <p>1) Scan all the way to the left. a) If we already started from this node last time, proceed
   * to Step 2. b) If a valid uplink is found, use it. c) If the result is an empty node (root not
   * set), break the scan. d) If we already returned the left node, break the scan.
   *
   * <p>2) Check the right. a) If we already returned the right node, proceed to Step 3. b) If it is
   * a valid uplink, use it. c) Do Step 1 from the right node.
   *
   * <p>3) Back up through the parents until we encounter find a parent that we're not the right
   * child of.
   *
   * <p>4) If there's no right child of that parent, the iteration is finished. Otherwise continue
   * to Step 5.
   *
   * <p>5) Check to see if the right child is a valid uplink. a) If we already returned that child,
   * proceed to Step 6. Otherwise, use it.
   *
   * <p>6) If the right child of the parent is the parent itself, we've already found &amp; returned
   * the end of the Trie, so exit.
   *
   * <p>7) Do Step 1 on the parent's right child.
   */
  TrieEntry<K, V> nextEntryImpl(
      final TrieEntry<K, V> start, final TrieEntry<K, V> previous, final TrieEntry<K, V> tree) {
   ...
   *
   * <p>This is implemented by going always to the left until we encounter a valid uplink. That
   * uplink is the first key.
   */
  ...
  /**
   * Returns a view of this {@link org.apache.commons.collections4.Trie} of all elements that are
   * prefixed by the number of bits in the given Key.
   *
   * <p>The view that this returns is optimized to have a very efficient {@link Iterator}. The
   * {@link SortedMap#firstKey()}, {@link SortedMap#lastKey()} &amp; {@link Map#size()} methods must
   * iterate over all possible values in order to determine the results. This information is cached
   * until the PATRICIA {@link org.apache.commons.collections4.Trie} changes. All other methods
   * (except {@link Iterator}) must compare the given key to the prefix to ensure that it is within
   * the range of the view. The {@link Iterator}'s remove method must also relocate the subtree that
   * contains the prefixes if the entry holding the subtree is removed or changes. Changing the
   * subtree takes O(K) time.
   *
   * @param key the key to use in the search
   * @param offsetInBits the prefix offset
   * @param lengthInBits the number of significant prefix bits
   * @return a {@link SortedMap} view of this {@link org.apache.commons.collections4.Trie} with all
   *     elements whose key is prefixed by the search key
   */
  private SortedMap<K, V> getPrefixMapByBits(
      final K key, final int offsetInBits, final int lengthInBits) {

    final int offsetLength = offsetInBits + lengthInBits;
    if (offsetLength > lengthInBits(key)) {
      throw new IllegalArgumentException(
          offsetInBits + " + " + lengthInBits + " > " + lengthInBits(key));
    }
   ...
   *
   * <p>This is very similar to getR but with the difference that we stop the lookup if h.bitIndex >
   * lengthInBits.
   */
  TrieEntry<K, V> subtree(final K prefix, final int offsetInBits, final int lengthInBits) {
    ...
    final int bitIndex =
        getKeyAnalyzer()
            .bitIndex(
                prefix, offsetInBits, lengthInBits, entry.key, 0, lengthInBits(entry.getKey()));
   ...
   *
   * <p>This is implemented by going always to the right until we encounter a valid uplink. That
   * uplink is the last key.
   */
   ...
   *
   * <p>This follows four simple branches: - If the uplink that returned us was a right uplink: - If
   * predecessor's left is a valid uplink from predecessor, return it. - Else, follow the right path
   * from the predecessor's left. - If the uplink that returned us was a left uplink: - Loop back
   * through parents until we encounter a node where node != node.parent.left. - If node.parent.left
   * is uplink from node.parent: - If node.parent.left is not root, return it. - If it is root &amp;
   * root isEmpty, return null. - If it is root &amp; root !isEmpty, return root. - If
   * node.parent.left is not uplink from node.parent: - Follow right path for first right child from
   * node.parent.left
   *
   ...
   * the first node.
   *
   * <p>This will traverse only within the subtree. If the given node is not within the subtree,
   * this will have undefined results.
   */
  ...
  /**
   * A {@link Reference} allows us to return something through a Method's argument list. An
   * alternative would be to an Array with a length of one (1) but that leads to compiler warnings.
   * Computationally and memory wise there's no difference (except for the need to load the {@link
  ...
  protected static class TrieEntry<K, V> extends BasicEntry<K, V> {
    ...
    /**
     * Whether or not the entry is storing a key. Only the root can potentially be empty, all other
    ...
    public String toString() {
      ...
      if (predecessor != null) {
        if (predecessor.bitIndex == -1) {
          ...
          buffer
              .append("predecessor=")
              .append(predecessor.getKey())
              .append(" [")
              .append(predecessor.bitIndex)
              .append("]");
        }
      }
  ...
  /**
   * This is an entry set view of the {@link org.apache.commons.collections4.Trie} as returned by
   * {@link Map#entrySet()}.
   */
  ...
  /**
   * This is a key set view of the {@link org.apache.commons.collections4.Trie} as returned by
   * {@link Map#keySet()}.
   */
  ...
  /**
   * This is a value view of the {@link org.apache.commons.collections4.Trie} as returned by {@link
   * Map#values()}.
   */
  ...
  private class RangeEntryMap extends RangeMap {
    ...
    /**
     * Creates a {@link RangeEntryMap} with the fromKey included and the toKey excluded from the
     * range.
     */
  ...
  private class PrefixRangeMap extends RangeMap {
    ...
    /**
     * This method does two things. It determines the FROM and TO range of the {@link
     * PrefixRangeMap} and the number of elements in the range. This method must be called every
    ...
    @Override
    protected boolean buggyMethod2(final K key) {
      return getKeyAnalyzer().isPrefix(prefix + "abc", offsetInBits, lengthInBits, key);
    }
  ...
  private final class PrefixRangeEntrySet extends RangeEntrySet {
    ...
    public Iterator<Map.Entry<K, V>> iterator() {
      ...
      return new EntryIterator(
          prefixStart, delegate.prefix, delegate.offsetInBits, delegate.lengthInBits);
    }
