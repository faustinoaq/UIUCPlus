...
/**
 * A {@code Map} implementation that stores data in simple fields until the size is greater than 3.
 *
 ...
 * </ul>
 *
 * <p>The design uses two distinct modes of operation - flat and delegate. While the map is size 3
 * or less, operations map straight onto fields using switch statements. Once size 4 is reached, the
 ...
 *
 * <p>Do not use {@code Flat3Map} if the size is likely to grow beyond 3.
 *
...
public class Flat3Map<K, V> implements IterableMap<K, V>, Serializable, Cloneable {
  ...
  static class FlatMapIterator<K, V> implements MapIterator<K, V>, ResettableIterator<K> {
    ...
    public boolean hasNext() {
      return nextIndex <= parent.size(); // Buggy version 1
    }
   ...
   *
   * <p>NOTE: from 4.0, the returned Map Entry will be an independent object and will not change
   * anymore as the iterator progresses. To avoid this additional object creation and simply iterate
  ...
  static class FlatMapEntry<K, V> implements Map.Entry<K, V> {
    ...
    /**
     * Used by the iterator that created this entry to indicate that {@link
     * java.util.Iterator#remove()} has been called.
     *
     * <p>As a consequence, all subsequent call to {@link #getKey()}, {@link #setValue(Object)} and
     * {@link #getValue()} will fail.
     *
  ...
  static class EntrySetIterator<K, V> extends EntryIterator<K, V>
