...
/**
 * A {@code List} implementation that is optimised for fast insertions and removals at any index in
 * the list.
 *
 ...
 * and a {@code LinkedList} where elements are inserted and removed repeatedly from anywhere in the
 * list.
 *
 ...
 * </pre>
 *
 * <p>{@code ArrayList} is a good general purpose list implementation. It is faster than {@code
 * TreeList} for most operations except inserting and removing in the middle of the list. {@code
 * ArrayList} also uses less memory as {@code TreeList} uses one object per entry.
 *
 * <p>{@code LinkedList} is rarely a good choice of implementation. {@code TreeList} is almost
 * always a good replacement for it, although it does use slightly more memory.
 *
 * @since 3.1
 */
public class TreeList<E> extends AbstractList<E> {
   ...
   *
   * <p>This method runs in O(n + log m) time, where m is the size of this list and n is the size of
   * {@code c}.
   *
  ...
  static class AVLNode<E> {
     ...
     *
     * <p>The collection must be nonempty.
     *
     ...
     * Constructs a new AVL tree from a collection.
     *
     * <p>This is a recursive helper for {@link #AVLNode(Collection)}. A call to this method will
     ...
     *
     * @param iterator an iterator over the collection, which should already point to the element at
     *     index {@code start} within the collection
     * @param start the index of the first element in the collection that should be in this subtree
     * @param end the index of the last element in the collection that should be in this subtree
     * @param absolutePositionOfParent absolute position of this node's parent, or 0 if this node is
     *     the root
     * @param prev the {@code AVLNode} corresponding to element (start - 1) of the collection, or
     *     null if start is 0
     * @param next the {@code AVLNode} corresponding to element (end + 1) of the collection, or null
     *     if end is the last element of the collection
     */
    private AVLNode(
    ...
    AVLNode<E> get(final int index) {
      ...
      if (indexRelativeToMe == 0) {
        AVLNode<E> temp = getLeftSubTree();
        setLeftSubTree(getRightSubTree());
        setRightSubTree(temp);
      }
