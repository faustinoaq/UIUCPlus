 ...
 * use this class is via the additional map-style methods. These provide {@code get}, {@code
 * containsKey}, {@code put} and {@code remove} for individual keys which operate without extra
 * object creation.
 *
 * <p>The additional methods are the main interface of this map. As such, you will not normally hold
 * this map in a variable of type {@code Map}.
 *
 * <p>The normal map methods take in and return a {@link MultiKey}. If you try to use {@code put()}
 * with any other object type a {@code ClassCastException} is thrown. If you try to use {@code null}
 * as the key in {@code put()} a {@code NullPointerException} is thrown.
 *
 * <p>This map is implemented as a decorator of a {@code AbstractHashedMap} which enables extra
 * behavior to be added easily.
 *
 * <ul>
 ...
 * </ul>
 *
 * <p>Note that {@code IdentityMap} and {@code ReferenceIdentityMap} are unsuitable for use as the
 * key comparison would work on the whole MultiKey, not the elements within.
 *
...
public class MultiKeyMap<K, V> extends AbstractMapDecorator<MultiKey<? extends K>, V>
   ...
   * @since 4.0
   */
  public static <K, V> MultiKeyMap<K, V> multiKeyMap(
      final AbstractHashedMap<MultiKey<? extends K>, V> map) {
   ...
   * #multiKeyMap(AbstractHashedMap)}. The map must not be null and should be empty or only contain
  ...
  protected boolean isEqualKey(
      final AbstractHashedMap.HashEntry<MultiKey<? extends K>, V> entry,
      final Object key1,
      final Object key2) {
   ...
   */
  public V put(
      final K key1, final K key2, final K key3, final K key4, final K key5, final V value) {
    final int hashCode = hash(key1, key2, key3, key4, key5);
    final int index = decorated().hashIndex(hashCode, decorated().data.length);
    AbstractHashedMap.HashEntry<MultiKey<? extends K>, V> entry = decorated().data[index];
    while (entry != null) {
   ...
   *
   * <p>This method removes all the mappings where the {@code MultiKey} has one or more keys, and
   * the first matches that specified.
   *
   * @param key1 the first key
   ...
   *
   * <p>This method removes all the mappings where the {@code MultiKey} has two or more keys, and
   * the first two match those specified.
   *
   * @param key1 the first key
   * @param key2 the second key
   * @return true if any elements were removed
   ...
   *
   * <p>This method removes all the mappings where the {@code MultiKey} has three or more keys, and
   * the first three match those specified.
   *
   * @param key1 the first key
   * @param key2 the second key
   * @param key3 the third key
   ...
   *
   * <p>This method removes all the mappings where the {@code MultiKey} has four or more keys, and
   * the first four match those specified.
   *
   * @param key1 the first key
   * @param key2 the second key
   * @param key3 the third key
