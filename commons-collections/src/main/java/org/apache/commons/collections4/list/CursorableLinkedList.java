...
/**
 * A {@code List} implementation with a {@code ListIterator} that allows concurrent modifications to
 * the underlying list.
 *
 * <p>This implementation supports all of the optional {@link List} operations. It extends {@code
 * AbstractLinkedList} and thus provides the stack/queue/dequeue operations available in {@link
 ...
 * Cursor} instance which extends {@code ListIterator}. The cursor allows changes to the list
 * concurrent with changes to the iterator. Note that the {@link #iterator()} method and sublists do
 * <b>not</b> provide this cursor behavior.
 *
 * <p>The {@code Cursor} class is provided partly for backwards compatibility and partly because it
 * allows the cursor to be directly closed. Closing the cursor is optional because references are
 * held via a {@code WeakReference}. For most purposes, simply modify the iterator and list at will,
 * and then let the garbage collector to the rest.
 *
...
public class CursorableLinkedList<E> extends AbstractLinkedList<E> implements Serializable {
   ...
   * ConcurrentModificationException will occur. The cursor behavior is available via {@link
   * #listIterator()}.
   *
   ...
   *
   * <p>A {@code Cursor} is a {@code ListIterator} with an additional {@code close()} method.
   * Calling this method immediately discards the references to the cursor. If it is not called,
   * then the garbage collector will still remove the reference as it is held via a {@code
   * WeakReference}.
   *
   * <p>The cursor enables iteration and list changes to occur in any order without invalidating the
   * iterator (from one thread). When elements are added to the list, an event is fired to all
   ...
   * {@code Cursor} if the {@code close} method is required.
   *
   * @return a new cursor iterator
   */
  ...
  /** An extended {@code ListIterator} that allows concurrent changes to the underlying list. */
