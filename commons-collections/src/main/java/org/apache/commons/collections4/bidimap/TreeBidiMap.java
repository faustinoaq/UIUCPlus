 ...
 * algorithm is based on {@link java.util.TreeMap}, but has been modified to simultaneously map a
 * tree node by key and by value. This doubles the cost of put operations (but so does using two
...
public class TreeBidiMap<K extends Comparable<K>, V extends Comparable<V>>
   ...
   *
   * <p>The key must implement {@code Comparable}.
   *
   ...
   *
   * <p>The value must implement {@code Comparable}.
   *
   ...
   * </pre>
   *
   * <p>Both key and value must implement {@code Comparable}.
   *
   ...
   *
   * <p>All keys and values must implement {@code Comparable}.
   *
   ...
   * @param dataElement either the {@link DataElement#KEY key} or the {@link DataElement#VALUE
   *     value}.
   * @return the desired Node, or null if there is no mapping of the specified data
   */
  @SuppressWarnings("unchecked")
  private <T extends Comparable<T>> Node<K, V> lookup(
      final Object data, final DataElement dataElement) {
    Node<K, V> rval = null;
  ...
  /**
   * Copies the color from one node to another, dealing with the fact that one or both nodes may, in
   * fact, be null.
   *
  ...
  private void copyColor(
      final Node<K, V> from, final Node<K, V> to, final DataElement dataElement) {
    if (to != null) {
      if (from == null) {
  ...
  /**
   * Gets a node's grandparent. mind you, the node, its parent, or its grandparent may not exist. No
   * problem.
   *
   * @param node the node (may be null) in question
   * @param dataElement either the {@link DataElement#KEY key} or the {@link DataElement#VALUE
  ...
  /**
   * Complicated red-black delete stuff. Based on Sun's TreeMap implementation, though it's barely
   * recognizable anymore. This rebalances the tree (somewhat, as red-black trees are not perfectly
   * balanced -- perfect balancing takes longer)
   *
   * @param replacementNode the node being replaced
   * @param dataElement the KEY or VALUE int
   */
  private void doRedBlackDeleteFixup(
      final Node<K, V> replacementNode, final DataElement dataElement) {
  ...
  /**
   * Swaps two nodes (except for their content), taking care of special cases where one is the
  ...
  /**
   * Increments the modification count -- used to check for concurrent modification of the map
  ...
  private void insertValue(final Node<K, V> newNode) throws IllegalArgumentException {
    ...
    while (true) {
      ...
      if (cmp < 0) {
        ...
        break;
        doRedBlackInsert(newNode, VALUE); // <--- Changed statement
      }
  ...
  class InverseViewMapEntryIterator extends ViewIterator
  ...
  class Inverse implements OrderedBidiMap<V, K> {
    ...
    public V previousKey(final V key) {
      checkKey(key);
      final Node<K, V> node =
          TreeBidiMap.this.nextSmaller(TreeBidiMap.this.<V>lookup(key, VALUE), VALUE);
      return node == null ? null : node.getValue();
    }
