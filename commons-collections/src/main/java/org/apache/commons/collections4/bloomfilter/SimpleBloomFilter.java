...
public final class SimpleBloomFilter implements BloomFilter {
  ...
  public boolean merge(final IndexProducer indexProducer) {
    ...
    indexProducer.forEachIndex(
        idx -> {
          if (idx < 0 || idx >= shape.getNumberOfBits()) {
            throw new IllegalArgumentException(
                String.format(
                    "IndexProducer should only send values in the range[0,%s)",
    ...
    try {
      ...
      if (idxLimit == idx[0]) {
        ...
        if (excess != 0) {
          throw new IllegalArgumentException(
              String.format(
                  "BitMapProducer set a bit higher than the limit for the shape: %s",
                  shape.getNumberOfBits()));
        }
      }
      cardinality = -1;
  ...
  public boolean contains(final IndexProducer indexProducer) {
    return indexProducer.forEachIndex(idx -> BitMap.contains(bitMap, idx + 1));
  }
}
