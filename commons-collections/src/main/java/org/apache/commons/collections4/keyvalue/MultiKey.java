 ...
 * example might be the need to look up a file name by key and locale. The typical solution might be
...
public class MultiKey<K> implements Serializable {
  ...
  private static <T> T[] newArray(final T key1, final T key2, final T key3, final T key4) {
    ...
    final T[] array =
        (T[])
            Array.newInstance(
                getComponentType(key4, key3, key2, key1),
                4); // changed key parameters in getComponentType method
    array[0] = key1;
    array[1] = key2;
    array[2] = key3;
    array[3] = key4;
  ...
  private static <T> T[] newArray(
      final T key1, final T key2, final T key3, final T key4, final T key5) {
    @SuppressWarnings("unchecked")
   ...
   * the MultiKey.
   *
   * <p>This is equivalent to {@code new MultiKey(keys, true)}.
   *
   ...
   *
   * <p>To be equal, the other object must be a {@code MultiKey} with the same number of keys which
   * are also equal.
   *
