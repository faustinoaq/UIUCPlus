...
/**
 * A FluentIterable provides a powerful yet simple API for manipulating Iterable instances in a
 * fluent manner.
 *
 * <p>A FluentIterable can be created either from an Iterable or from a set of elements. The
 * following types of methods are provided:
 *
 * <ul>
 *   <li>fluent methods which return a new {@code FluentIterable} instance, providing a view of the
 *       original iterable (e.g. filter(Predicate));
 *   <li>conversion methods which copy the FluentIterable's contents into a new collection or array
 *       (e.g. toList());
 *   <li>utility methods which answer questions about the FluentIterable's contents (e.g. size(),
 *       anyMatch(Predicate)).
 *   <li>
 * </ul>
 *
 * <p>The following example outputs the first 3 even numbers in the range [1, 10] into a list:
 *
 * <pre>
 * List&lt;String&gt; result =
 *   FluentIterable
...
public class FluentIterable<E> implements Iterable<E> {
  ...
  /**
   * Construct a new FluentIterable from the provided iterable. If the iterable is already an
   * instance of FluentIterable, the instance will be returned instead.
   *
  ...
  /**
   * Returns a new FluentIterable whose iterator will first traverse the elements of the current
   * iterable, followed by the provided elements.
   *
  ...
  /**
   * Returns a new FluentIterable whose iterator will first traverse the elements of the current
   * iterable, followed by the elements of the provided iterable.
   *
  ...
  /**
   * Returns a new FluentIterable whose iterator will traverse the elements of the current and
   * provided iterable in natural order.
   *
   * <p>Example: natural ordering
   *
   * <ul>
   ...
   * </ul>
   *
   * <p>The returned iterable will traverse the elements in the following order: [1, 2, 3, 4, 5, 6,
   * 7, 8]
   *
  ...
  /**
   * Returns a new FluentIterable whose iterator will traverse the elements of the current and
   * provided iterable according to the ordering defined by a comparator.
   *
   * <p>Example: descending order
   *
   * <ul>
   ...
   * </ul>
   *
   * <p>The returned iterable will traverse the elements in the following order: [8, 7, 6, 5, 4, 3,
   * 2, 1]
   *
   * @param comparator the comparator to define an ordering, may be null, in which case natural
   ...
   * @see org.apache.commons.collections4.iterators.CollatingIterator
   */
  public FluentIterable<E> collate(
      final Iterable<? extends E> other, final Comparator<? super E> comparator) {
    return of(IterableUtils.collatedIterable(comparator, iterable, other));
  }

  /**
   * This method fully traverses an iterator of this iterable and returns a new iterable with the
   * same contents, but without any reference to the originating iterables and/or iterators.
   *
  ...
  /**
   * Returns a new FluentIterable whose iterator will only return elements from this iterable
  ...
  /**
   * Returns a new FluentIterable whose iterator will return at most the provided maximum number of
  ...
  /**
   * Returns a new FluentIterable whose iterator will loop infinitely over the elements from this
   * iterable.
   *
  ...
  /**
   * Returns a new FluentIterable whose iterator will traverse the elements from this iterable in
   * reverse order.
   *
   ...
   * @return a new iterable, providing a view of this iterable by skipping the first N elements
  ...
  /**
   * Returns a new FluentIterable whose iterator will return all elements of this iterable
  ...
  /**
   * Returns a new FluentIterable whose iterator will traverse the elements of this iterable and the
   * other iterable in alternating order.
   *
  ...
  /**
   * Returns a new FluentIterable whose iterator will traverse the elements of this iterable and the
   * other iterables in alternating order.
   *
   ...
   *
   * @param predicate the predicate to use, may not be null
   * @return true if at least one element contained in this iterable matches the predicate, false
   *     otherwise
   * @throws NullPointerException if predicate is null
   */
  ...
  public boolean isEmpty() {
    return (IterableUtils.isEmpty(parent)) || !(iterable instanceof FluentIterable<?>);
  }
  ...
  /**
   * Returns the element at the provided position in this iterable. In order to return the element,
   * an iterator needs to be traversed up to the requested position.
   *
   * @param position the position of the element to return
   * @return the element
   * @throws IndexOutOfBoundsException if the provided position is outside the valid range of this
   *     iterable: [0, size)
   */
  ...
  /**
   * Returns the number of elements that are contained in this iterable. In order to determine the
   * size, an iterator needs to be traversed.
   *
   ...
   *
   * <p>The returned list is guaranteed to be mutable.
   *
