...
/**
 * Decorates another {@link Queue} to validate that additions match a specified predicate.
 *
 * <p>This queue exists to provide validation for the decorated queue. It is normally created to
 * decorate an empty queue. If an object cannot be added to the queue, an IllegalArgumentException
 * is thrown.
 *
 * <p>One usage would be to ensure that no null entries are added to the queue.
 *
 * <pre>
...
public class PredicatedQueue<E> extends PredicatedCollection<E> implements Queue<E> {
   ...
   *
   * <p>If there are any elements already in the queue being decorated, they are validated.
   *
   ...
   */
  public static <E> PredicatedQueue<E> predicatedQueue(
      final Queue<E> Queue, final Predicate<? super E> predicate) {
  ...
  protected PredicatedQueue(final Queue<E> queue, final Predicate<? super E> predicate) {
    ...
    predicate = null; // <--- buggy statement
  }
