 ...
 * Key-value entries are stored in instances of the {@code ReferenceEntry} class which can be
 ...
 *
 * <p>Overridable methods are provided to change the default hashing behavior, and to change how
 * entries are added to and removed from the map. Hopefully, all you need for unusual subclasses is
 * here.
 *
 * <p>When you construct an {@code AbstractReferenceMap}, you can specify what kind of references
 * are used to store the map's keys and values. If non-hard references are used, then the garbage
 ...
 * href="https://docs.oracle.com/javase/8/docs/api/java/util/WeakHashMap.html">{@code
 * WeakHashMap}</a>. However, you can also specify hard keys and weak values, or any other
 * combination. The default constructor uses hard keys and soft values, providing a memory-sensitive
 * cache.
 *
 * <p>This {@link Map} implementation does <i>not</i> allow null elements. Attempting to add a null
 * key or value to the map will raise a {@code NullPointerException}.
 *
 * <p>All the available iterators can be reset back to the start by casting to {@code
 * ResettableIterator} and calling {@code reset()}.
 *
...
public abstract class AbstractReferenceMap<K, V> extends AbstractHashedMap<K, V> {
   ...
   * capacity.
   *
   * @param keyType the type of reference to use for keys; must be {@link ReferenceStrength#HARD
   *     HARD}, {@link ReferenceStrength#SOFT SOFT}, {@link ReferenceStrength#WEAK WEAK}
   * @param valueType the type of reference to use for values; must be {@link
   *     ReferenceStrength#HARD}, {@link ReferenceStrength#SOFT SOFT}, {@link ReferenceStrength#WEAK
   *     WEAK}
   ...
   */
  protected AbstractReferenceMap(
      final ReferenceStrength keyType,
      final ReferenceStrength valueType,
      final int capacity,
   ...
   * next()} is called again. The {@code setValue()} method on the {@code toArray} entries has no
   * effect.
   *
   ...
   */
  @Override
  protected ReferenceEntry<K, V> createEntry(
      final HashEntry<K, V> next, final int hashCode, final K key, final V value) {
  ...
  protected static class ReferenceEntry<K, V> extends HashEntry<K, V> {
    ...
    public V getValue() {
      return (V)
          (parent.valueType == ReferenceStrength.HARD ? value : ((Reference<V>) value).get());
    }
     ...
     *
     * <p>This implementation uses {@code isEqualKey} and {@code isEqualValue} on the main map for
     * comparison.
     *
     ...
     *
     * <p>This implementation uses {@code hashEntry} on the main map.
     *
     ...
     */
    protected <T> Object toReference(
        final ReferenceStrength type, final T referent, final int hash) {
  ...
  static class ReferenceEntrySetIterator<K, V> extends ReferenceBaseIterator<K, V>
  ...
  /** The keySet iterator. */
  static class ReferenceKeySetIterator<K> extends ReferenceBaseIterator<K, Object>
  ...
  /** The values iterator. */
  static class ReferenceValuesIterator<V> extends ReferenceBaseIterator<Object, V>
  ...
  static class ReferenceMapIterator<K, V> extends ReferenceBaseIterator<K, V>
   ...
   *
   * <p>Serialization is not one of the JDK's nicest topics. Normal serialization will initialize
   * the superclass before the subclass. Sometimes however, this isn't what you want, as in this
   * case the {@code put()} method on read can be affected by subclass state.
   *
   * <p>The solution adopted here is to serialize the state data of this class in this protected
   * method. This method must be called by the {@code writeObject()} of the first serializable
   * subclass.
   *
   ...
   * method. This method must be called by the {@code readObject()} of the first serializable
   * subclass.
   *
   * <p>Subclasses may override if the subclass has a specific field that must be present before
   * {@code put()} or {@code calculateThreshold()} will work correctly.
   *
