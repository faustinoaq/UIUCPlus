...
/**
 * A {@code List} implementation that stores a cache of internal Node objects in an effort to reduce
...
public class NodeCachingLinkedList<E> extends AbstractLinkedList<E> implements Serializable {
  ...
  /**
   * The first cached node, or {@code null} if no nodes are cached. Cached nodes are stored in a
   * singly-linked list with {@code next} pointing to the next element.
   */
  ...
  public NodeCachingLinkedList(final int maximumCacheSize) {
    this.maximumCacheSize = maximumCacheSize + 1;
   ...
   * decreased accordingly. The node that is returned will have {@code null} values for next,
  ...
  /**
   * Adds a node to the cache, if the cache isn't full. The node's contents are cleared so they can
