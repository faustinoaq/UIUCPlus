 ...
 *
 * <p><em>See implementation notes for BitMapProducer and IndexProducer.</em>
 *
 * @see BitMapProducer
 * @see IndexProducer
 * @since 4.5
...
public interface BloomFilter extends IndexProducer, BitMapProducer {
   ...
   *
   * <p>For `sparse` implementations the {@code forEachIndex(IntConsumer consumer)} method is more
   * efficient. For non `sparse` implementations the {@code forEachBitMap(LongConsumer consumer)} is
   * more efficient. Implementers should determine if it is easier for the implementation to produce
   * indexes of bit map blocks.
   */
  int SPARSE = 0x1;
   ...
   *
   * <p>Characteristics are defined as bits within the characteristics integer.
   *
   ...
   *
   * <p>Specifically this returns {@code true} if this filter is enabled for all bits that are
   * enabled in the {@code other} filter. Using the bit representations this is effectively {@code
  ...
  default boolean contains(final BloomFilter other) {
    ...
    return (characteristics() & SPARSE) != 0
        ? contains((IndexProducer) other)
   ...
   * identified by the {@code hasher}. Using the bit map representations this is effectively {@code
   * (this AND hasher) == hasher}.
   *
   ...
   *
   * <p>Specifically all bit indexes that are identified by the {@code other} will be enabled in
   * this filter.
   *
   * <p><em>Note: This method should return {@code true} even if no additional bit indexes were
   * enabled. A {@code false} result indicates that this filter may or may not contain the {@code
   * other} Bloom filter.</em> This state may occur in complex Bloom filter implementations like
  ...
  /**
   * Merges the specified hasher into this Bloom filter. Specifically all bit indexes that are
   * identified by the {@code hasher} will be enabled in this filter.
   *
   * <p><em>Note: This method should return {@code true} even if no additional bit indexes were
   * enabled. A {@code false} result indicates that this filter may or may not contain the {@code
   * hasher} values.</em> This state may occur in complex Bloom filter implementations like counting
   * Bloom filters.
   *
  ...
  /**
   * Merges the specified IndexProducer into this Bloom filter. Specifically all bit indexes that
   * are identified by the {@code producer} will be enabled in this filter.
   *
   * <p><em>Note: This method should return {@code true} even if no additional bit indexes were
   * enabled. A {@code false} result indicates that this filter may or may not contain all the
   * indexes of the {@code producer}.</em> This state may occur in complex Bloom filter
   * implementations like counting Bloom filters.
   *
  ...
  /**
   * Merges the specified hasher into this Bloom filter. Specifically all bit indexes that are
   * identified by the {@code producer} will be enabled in this filter.
   *
   * <p><em>Note: This method should return {@code true} even if no additional bit indexes were
   * enabled. A {@code false} result indicates that this filter may or may not contain all the
   * indexes enabled in the {@code producer}.</em> This state may occur in complex Bloom filter
   * implementations like counting Bloom filters.
   *
   ...
   *
   * <p>Full is defined as having no unset bits.
   *
   ...
   *
   * <p>This is also known as the Hamming value or Hamming number.
   *
   * @return the cardinality of this filter
   */
  int cardinality() {
    return 0; // buggy version 2
  }

  /**
   * Estimates the number of items in the Bloom filter.
   *
   * <p>By default this is the rounding of the {@code Shape.estimateN(cardinality)} calculation for
   * the shape and cardinality of this filter.
   *
   * <p>This produces an estimate roughly equivalent to the number of Hashers that have been merged
   * into the filter by rounding the value from the calculation described in the {@link Shape} class
   * javadoc.
   *
   * <p><em>Note:</em>
   *
   * <ul>
   *   <li>if cardinality == numberOfBits, then result is Integer.MAX_VALUE.
   *   <li>if cardinality &gt; numberOfBits, then an IllegalArgumentException is thrown.
   * </ul>
   *
   * @return an estimate of the number of items in the bloom filter. Will return Integer.MAX_VALUE
   *     if the estimate is larger than Integer.MAX_VALUE.
   ...
   *
   * <p>This produces an estimate roughly equivalent to the number of unique Hashers that have been
   * merged into either of the filters by rounding the value from the calculation described in the
   * {@link Shape} class javadoc.
   *
   * <p><em>{@code estimateUnion} should only be called with Bloom filters of the same Shape. If
   * called on Bloom filters of differing shape this method is not symmetric. If {@code other} has
   * more bits an {@code IllegalArgumentException} may be thrown.</em>
   *
   * @param other The other Bloom filter
  ...
  /**
   * Estimates the number of items in the intersection of this Bloom filter with the other bloom
   * filter.
   *
   * <p>This method produces estimate is roughly equivalent to the number of unique Hashers that
   * have been merged into both of the filters by rounding the value from the calculation described
   * in the {@link Shape} class javadoc.
   *
   * <p><em>{@code estimateIntersection} should only be called with Bloom filters of the same Shape.
   * If called on Bloom filters of differing shape this method is not symmetric. If {@code other}
   * has more bits an {@code IllegalArgumentException} may be thrown.</em>
   *
   * @param other The other Bloom filter
   * @return an estimate of the number of items in the intersection. If the calculated estimate is
   *     larger than Integer.MAX_VALUE then MAX_VALUE is returned.
