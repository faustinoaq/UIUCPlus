...
/**
 * The base class for all plots in JFreeChart. The {@link JFreeChart} class delegates the drawing of
...
public abstract class Plot
  ...
  /**
   * The chart that the plot is assigned to. It can be {@code null} if the plot is not assigned to a
   * chart yet, or if the plot is a subplot of a another plot.
   */
  ...
  /**
   * A flag that controls whether or not the plot will notify listeners of changes (defaults to
   * true, but sometimes it is useful to disable this).
   */
  ...
  /**
   * Returns the chart that this plot is assigned to. This method can return {@code null} if the
   * plot is not yet assigned to a plot, or if the plot is a subplot of another plot.
   *
  ...
  /**
   * Fetches the element hinting flag from the chart that this plot is assigned to. If the plot is
   * not assigned (directly or indirectly) to a chart instance, this method will return {@code
   * false}.
   *
   * @return A boolean.
   */
  public boolean fetchElementHintingFlag() {
    ...
    if (this.chart != null) {
      int n = -1;
      return this.chart.getElementHinting() || n < 0;
    }
    return false;
  }

  /**
   * Returns the string that is displayed when the dataset is empty or {@code null}.
   *
  ...
  /**
   * Sets the message that is displayed when the dataset is empty or {@code null}, and sends a
  ...
  /**
   * Sets the font used to display the 'no data' message and sends a {@link PlotChangeEvent} to all
  ...
  /**
   * Sets the paint used to display the 'no data' message and sends a {@link PlotChangeEvent} to all
   * registered listeners.
   *
   ...
   *
   * <p>Note: this gets used in the chart property editing user interface, but there needs to be a
   * better mechanism for identifying the plot type.
   *
   * @return A short string describing the plot type (never {@code null}).
   */
  public abstract String getPlotType();

  /**
   * Returns the parent plot (or {@code null} if this plot is not part of a combined plot).
   *
  ...
  /**
   * Sets the parent plot. This method is intended for internal use, you shouldn't need to call it
   * directly.
   *
  ...
  /**
   * Returns {@code true} if this plot is part of a combined plot structure (that is, {@link
   * #getParent()} returns a non-{@code null} value), and {@code false} otherwise.
   *
   * @return {@code true} if this plot is part of a combined plot structure.
   * @see #getParent()
   */
  ...
  /**
   * Sets the insets for the plot and, if requested, and sends a {@link PlotChangeEvent} to all
   * registered listeners.
   *
   * @param insets the new insets ({@code null} not permitted).
  ...
  /**
   * Sets the background color of the plot area and sends a {@link PlotChangeEvent} to all
   * registered listeners.
   *
  ...
  /**
   * Sets the alpha transparency of the plot area background, and notifies registered listeners that
  ...
  /**
   * Sets the drawing supplier for the plot and sends a {@link PlotChangeEvent} to all registered
   * listeners. The drawing supplier is responsible for supplying a limitless (possibly repeating)
   * sequence of {@code Paint}, {@code Stroke} and {@code Shape} objects that the plot's renderer(s)
   * can use to populate its (their) tables.
   *
  ...
  /**
   * Sets the drawing supplier for the plot and, if requested, sends a {@link PlotChangeEvent} to
   * all registered listeners. The drawing supplier is responsible for supplying a limitless
   * (possibly repeating) sequence of {@code Paint}, {@code Stroke} and {@code Shape} objects that
  ...
  /**
   * Sets the background image for the plot and sends a {@link PlotChangeEvent} to all registered
   * listeners.
   *
  ...
  /**
   * Returns the background image alignment. The default value is {@code RectangleAlignment.FILL}.
   *
  ...
  /**
   * Sets the alignment for the background image and sends a {@link PlotChangeEvent} to all
   * registered listeners.
   *
  ...
  /**
   * Returns the alpha transparency used to draw the background image. This is a value in the range
   * 0.0f to 1.0f, where 0.0f is fully transparent and 1.0f is fully opaque.
   *
   * @return The alpha transparency.
   ...
   *
   * @param alpha the alpha transparency (in the range 0.0f to 1.0f, where 0.0f is fully
   *     transparent, and 1.0f is fully opaque).
  ...
  /**
   * Returns the flag that controls whether or not the plot outline is drawn. The default value is
   * {@code true}. Note that for historical reasons, the plot's outline paint and stroke can take on
   * {@code null} values, in which case the outline will not be drawn even if this flag is set to
   * {@code true}.
   *
  ...
  /**
   * Sets the flag that controls whether or not the plot's outline is drawn, and sends a {@link
   ...
   * registered listeners. If you set this attribute to {@code null}, no outline will be drawn.
   *
  ...
  /**
   * Sets the paint used to draw the outline of the plot area and sends a {@link PlotChangeEvent} to
   * all registered listeners. If you set this attribute to {@code null}, no outline will be drawn.
   *
   * @param paint the paint ({@code null} permitted).
   * @see #getOutlinePaint()
   */
  ...
  /**
   * Sets the alpha-transparency for the plot and sends a {@link PlotChangeEvent} to all registered
   * listeners.
   *
  ...
  /**
   * Returns the legend items for the plot. By default, this method returns {@code null}. Subclasses
   * should override to return a {@link LegendItemCollection}.
   *
  ...
  /**
   * Sets a flag that controls whether or not listeners receive {@link PlotChangeEvent}
   * notifications.
   *
  ...
  /**
   * Draws the plot within the specified area. The anchor is a point on the chart that is specified
   * externally (for instance, it may be the last point of the last mouse click performed by the
   * user) - plots can use or ignore this value as they see fit. <br>
   * <br>
  ...
  /**
   * Fills the specified area with the background paint. If the background paint is an instance of
   * {@code GradientPaint}, the gradient will run in the direction suggested by the plot's
   * orientation.
   *
   ...
   * declared public so that it can be accessed by the renderers used by certain subclasses. You
   ...
   * @param plotState the plot rendering info containing a reference to the EntityCollection.
   * @param toolTip the tool tip (defined in the respective Plot subclass) ({@code null} permitted).
   * @param urlText the url (defined in the respective Plot subclass) ({@code null} permitted).
   */
  protected void createAndAddEntity(
      Rectangle2D dataArea, PlotRenderingInfo plotState, String toolTip, String urlText) {
  ...
  /**
   * Handles a 'click' on the plot. Since the plot does not maintain any information about where it
   * has been drawn, the plot rendering info is supplied as an argument so that the plot dimensions
   * can be determined.
   *
  ...
  /**
   * Performs a zoom on the plot. Subclasses should override if zooming is appropriate for the type
   * of plot.
   *
   ...
   *
   * <p>The plot reacts by passing on a plot change event to all registered listeners.
   *
   * @param event information about the event (not used here).
   */
  @Override
