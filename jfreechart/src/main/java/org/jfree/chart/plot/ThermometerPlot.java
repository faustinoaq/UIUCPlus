 ...
 *
 * <p>This plot supports a number of options:
 *
 * <ol>
 *   <li>three sub-ranges which could be viewed as 'Normal', 'Warning' and 'Critical' ranges.
 *   <li>the thermometer can be run in two modes:
 *       <ul>
 *         <li>fixed range, or
 *         <li>range adjusts to current sub-range.
 *       </ul>
 *   <li>settable units to be displayed.
 *   <li>settable display location for the value text.
 * </ol>
 */
public class ThermometerPlot extends Plot
  ...
  /**
   * Returns the lower bound for the thermometer. The data value can be set lower than this, but it
   * will not be shown in the thermometer.
   *
  ...
  /**
   * Returns the upper bound for the thermometer. The data value can be set higher than this, but it
   * will not be shown in the thermometer.
   *
  ...
  /**
   * Sets the padding for the thermometer and sends a {@link PlotChangeEvent} to all registered
   * listeners.
   *
  ...
  /**
   * Sets the stroke used to draw the thermometer outline and sends a {@link PlotChangeEvent} to all
  ...
  /**
   * Sets the paint used to draw the thermometer outline and sends a {@link PlotChangeEvent} to all
   * registered listeners.
   *
  ...
  /**
   * Returns a code indicating the unit display type. This is one of {@link #UNITS_NONE}, {@link
   ...
   *
   * <ul>
   *   <li>UNITS_NONE : no units displayed.
   *   <li>UNITS_FAHRENHEIT : units displayed in Fahrenheit.
   *   <li>UNITS_CELCIUS : units displayed in Celcius.
   *   <li>UNITS_KELVIN : units displayed in Kelvin.
   * </ul>
   *
  ...
  /**
   * Sets the location at which the current value is displayed and sends a {@link PlotChangeEvent}
   * to all registered listeners.
   *
   * <p>The location can be one of the constants: {@code NONE}, {@code RIGHT}, {@code LEFT} and
   * {@code BULB}.
   *
  ...
  /**
   * Sets the paint used to display the current value and sends a {@link PlotChangeEvent} to all
   * registered listeners.
   *
  ...
  /**
   * Sets the formatter for the value label and sends a {@link PlotChangeEvent} to all registered
   * listeners.
   *
   ...
   */
  public void setSubrangeInfo(
      int range, double rangeLow, double rangeHigh, double displayLow, double displayHigh) {
  ...
  /**
   * Sets the paint to be used for a subrange and sends a {@link PlotChangeEvent} to all registered
   * listeners.
   *
  ...
  /**
   * Returns a flag that controls whether or not the thermometer axis zooms to display the subrange
   * within which the data value falls.
   *
   * @return The flag.
   */
  ...
  /**
   * Sets the flag that controls whether or not the thermometer axis zooms to display the subrange
   * within which the data value falls.
   *
   * @param flag the flag.
   */
  ...
  /**
   * Sets the bulb radius (in Java2D units) and sends a {@link PlotChangeEvent} to all registered
   * listeners.
   *
  ...
  /**
   * Returns the bulb diameter, which is always twice the value returned by {@link
   * #getBulbRadius()}.
   *
  ...
  /**
   * Sets the column radius (in Java2D units) and sends a {@link PlotChangeEvent} to all registered
   * listeners.
   *
  ...
  /**
   * Returns the column diameter, which is always twice the value returned by {@link
   * #getColumnRadius()}.
   *
  ...
  /**
   * Sets the gap (in Java2D units) between the two outlines that represent the thermometer, and
  ...
  public void draw(
    ...
    Rectangle2D dataArea =
        new Rectangle2D.Double(
            midX - getColumnRadius(), stemTop, getColumnRadius(), stemBottom - stemTop);
    ...
    outerStem.setRoundRect(
        ...
        interior.getMinY(),
        getColumnDiameter(),
        stemBottom + getBulbDiameter() - stemTop,
        getColumnDiameter(),
        getColumnDiameter());
    ...
    innerBulb.setFrame(
        midX - getBulbRadius() + getGap(),
        stemBottom + getGap(),
        getBulbDiameter() - getGap() * 2,
        getBulbDiameter() - getGap() * 2);

    innerStem.setRoundRect(
        midX - getColumnRadius() + getGap(),
        interior.getMinY() + getGap(),
        getColumnDiameter() - getGap() * 2,
        stemBottom + getBulbDiameter() - getGap() * 2 - stemTop,
        getColumnDiameter() - getGap() * 2,
        getColumnDiameter() - getGap() * 2);
    ...
    if ((this.dataset != null) && (this.dataset.getValue() != null)) {
      ...
      switch (this.valueLocation) {
        case RIGHT:
          g2.drawString(
              this.valueFormat.format(current), midX + getColumnRadius() + getGap(), midY);
          break;
        case LEFT:
  ...
  /**
   * A zoom method that does nothing. Plots are required to support the zoom operation. In the case
   * of a thermometer chart, it doesn't make sense to zoom in or out, so the method is empty.
   *
  ...
  protected void setAxisRange() {
    if ((this.subrange >= 0) && (this.followDataInSubranges)) {
      this.rangeAxis.setRange(
          new Range(
              this.subrangeInfo[this.subrange][DISPLAY_LOW],
   ...
   * @return {@code true} if the number is valid and finite, and {@code false} otherwise.
   */
  ...
  /**
   * Returns the mercury paint corresponding to the current data value. Called from the {@link
   * #draw(Graphics2D, Rectangle2D, Point2D, PlotState, PlotRenderingInfo)} method.
   *
   * @return The paint (never {@code null}).
   */
  ...
  /**
   * Tests this plot for equality with another object. The plot's dataset is not considered in the
   * test.
   *
  ...
  public void zoomRangeAxes(
      double factor, PlotRenderingInfo state, Point2D source, boolean useAnchor) {
    double anchorY =
        this.getRangeAxis().java2DToValue(source.getY(), state.getDataArea(), RectangleEdge.LEFT);
