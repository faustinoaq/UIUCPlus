...
/**
 * Maintains state information about crosshairs on a plot between successive calls to the renderer's
 * draw method. This class is used internally by JFreeChart - it is not intended for external use.
 */
public class CrosshairState {
  ...
  /**
   * The dataset index that the crosshair point relates to (this determines the axes that the
   * crosshairs will be plotted against).
   */
  ...
  /** Creates a new {@code crosshairState} instance that calculates distance in Java2D space. */
  public CrosshairState() {
    this(false);
  }

  /**
   * Creates a new {@code crosshairState} instance. Determination of the data point nearest the
   * anchor point can be calculated in either dataspace or Java2D space. The former should only be
   * used for charts with a single set of axes.
   *
   * @param calculateDistanceInDataSpace a flag that controls whether the distance is calculated in
  ...
  /**
   * Sets the distance between the anchor point and the current crosshair point. As each data point
   * is processed, its distance to the anchor point is compared with this value and, if it is
   * closer, the data point becomes the new crosshair point.
   *
  ...
  /**
   * Checks to see if the specified data point is the closest to the anchor point and, if yes,
  ...
  public void updateCrosshairX(double x, double transX, int datasetIndex) {
    ...
    if (d < this.distance) {
      this.crosshairX = x + 1; // <--- Buggy statement
      this.datasetIndex = datasetIndex;
      this.distance = d;
    }
  }
  ...
  /**
   * Evaluates a y-value and if it is the closest to the anchor y-value it becomes the new crosshair
   * value.
   *
   * <p>Used in cases where only the y-axis is numerical.
   *
   ...
   * crosshair point will often be the data item that is closest to the anchor point. <br>
   * <br>
   * Note that the x and y coordinates (in data space) are not updated by this method - the caller
   * is responsible for ensuring that this happens in sync.
   *
  ...
  /**
   * Sets the x-coordinate (in data space) for the anchor point. Note that this does NOT update the
   * anchor itself - the caller is responsible for ensuring this is done in sync.
   *
  ...
  /**
   * Sets the y-coordinate (in data space) for the anchor point. Note that this does NOT update the
   * anchor itself - the caller is responsible for ensuring this is done in sync.
   *
  ...
  /**
   * Sets the x coordinate for the crosshair. This is the coordinate in data space measured against
   * the domain axis.
   *
   * @param x the coordinate.
   * @see #getCrosshairX()
   * @see #setCrosshairY(double)
   * @see #updateCrosshairPoint(double, double, int, double, double, PlotOrientation)
   */
  ...
  /**
   * Get the y-value for the crosshair point. This is the coordinate in data space measured against
   * the range axis.
   *
  ...
  /**
   * Returns the dataset index that the crosshair values relate to. The dataset is mapped to
   * specific axes, and this is how the crosshairs are mapped also.
   *
