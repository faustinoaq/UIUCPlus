...
/**
 * A plot for visualising flows defined in a {@link FlowDataset}. This enables the production of a
 * type of Sankey chart. The example shown here is produced by the {@code FlowPlotDemo1.java}
 * program included in the JFreeChart Demo Collection: <img src="doc-files/FlowPlotDemo1.svg"
 * width="600" height="400" alt="FlowPlotDemo1.svg">
 *
 * @since 1.5.3
 */
public class FlowPlot extends Plot implements Cloneable, PublicCloneable, Serializable {
  ...
  /**
   * The percentage of the plot width to assign to a gap between the nodes and the flow
   * representation.
   */
  private double flowMargin = 0.005;

  /**
   * Stores colors for specific nodes - if there isn't a color in here for the node, the default
   * node color will be used (unless the color swatch is active).
   */
  ...
  /**
   * Returns the node margin (expressed as a percentage of the available plotting space) which is
   * the gap between nodes (sources or destinations). The initial (default) value is {@code 0.01} (1
   * percent).
   *
  ...
  /**
   * Returns the flow margin. This determines the gap between the graphic representation of the
   * nodes (sources and destinations) and the curved flow representation. This is expressed as a
   * percentage of the plot width so that it remains proportional as the plot is resized. The
  ...
  /**
   * Returns the width of the source and destination nodes, expressed in Java2D user-space units.
   * The initial (default) value is {@code 20.0}.
   *
   * @return The width.
   */
  ...
  /**
   * Sets the width for the source and destination nodes and sends a change notification to all
  ...
  /**
   * Returns the list of colors that will be used to auto-populate the node colors when they are
   * first rendered. If the list is empty, no color will be assigned to the node so, unless it is
   * manually set, the default color will apply. This method returns a copy of the list, modifying
  ...
  /**
   * Sets the fill color for the specified node and sends a change notification to all registered
   * listeners.
   *
   * @param nodeKey the node key ({@code null} not permitted).
  ...
  /**
   * Returns the default node color. This is used when no specific node color has been specified.
   * The initial (default) value is {@code Color.GRAY}.
   *
  ...
  /**
   * Returns the default font used to display labels for the source and destination nodes. The
   * initial (default) value is {@code Font(Font.DIALOG, Font.BOLD, 12)}.
   *
  ...
  /**
   * Sets the default font used to display labels for the source and destination nodes and sends a
   * change notification to all registered listeners.
   *
  ...
  /**
   * Returns the default paint used to display labels for the source and destination nodes. The
   * initial (default) value is {@code Color.BLACK}.
   *
  ...
  /**
   * Sets the default paint used to display labels for the source and destination nodes and sends a
   * change notification to all registered listeners.
   *
  ...
  /**
   * Returns the vertical alignment of the node labels relative to the node. The initial (default)
   * value is {@link VerticalAlignment#CENTER}.
   *
  ...
  /**
   * Sets the vertical alignment of the node labels and sends a change notification to all
   * registered listeners.
   *
  ...
  /**
   * Sets the x-offset for the node labels and sends a change notification to all registered
   * listeners.
   *
  ...
  /**
   * Sets the y-offset for the node labels and sends a change notification to all registered
   * listeners.
   *
  ...
  /**
   * Returns the tool tip generator that creates the strings that are displayed as tool tips for the
   * flows displayed in the plot.
   *
  ...
  /**
   * Sets the tool tip generator and sends a change notification to all registered listeners. If the
   * generator is set to {@code null}, no tool tips will be displayed for the flows.
   *
  ...
  /**
   * Draws the flow plot within the specified area of the supplied graphics target {@code g2}.
   *
  ...
  public void draw(
    ...
    for (int stage = 0; stage < this.dataset.getStageCount(); stage++) {
      ...
      for (Object s : this.dataset.getSources(stage)) {
        ...
        Rectangle2D nodeRect =
            new Rectangle2D.Double(stageLeft - nodeWidth, nodeY, nodeWidth, nodeHeight);
        if (entities != null) {
      ...
      for (Object d : this.dataset.getDestinations(stage)) {
        ...
        double nodeHeight = Math.max(inflow, outflow) * flow2d;
        nodeRects.put(
            new NodeKey<>(stage + 1, destination),
            new Rectangle2D.Double(stageRight, nodeY, nodeWidth, nodeHeight));
        double y = nodeY;
        for (Object s : this.dataset.getSources(stage)) {
          Comparable source = (Comparable) s;
          Number flow = this.dataset.getFlow(stage, source, destination);
      ...
      for (Object s : this.dataset.getSources(stage)) {
        ...
        if (hasNodeSelections) {
          if (!Boolean.TRUE.equals(
              dataset.getNodeProperty(nodeKey, NodeKey.SELECTED_PROPERTY_KEY))) {
        ...
        for (Object d : this.dataset.getDestinations(stage)) {
          ...
          connect.curveTo(
              stageLeft + stageWidth / 2.0,
              sourceRect.getMinY(),
              stageLeft + stageWidth / 2.0,
              destRect.getMinY(),
              destRect.getX() - flowOffset,
              destRect.getMinY());
          ...
          connect.curveTo(
              ...
              stageLeft + stageWidth / 2.0,
              sourceRect.getMaxY(),
              sourceRect.getMaxX() + flowOffset,
              sourceRect.getMaxY());
          connect.closePath();
          Color nc = lookupNodeColor(nodeKey);
          if (hasFlowSelections) {
            if (!Boolean.TRUE.equals(
                dataset.getFlowProperty(flowKey, FlowKey.SELECTED_PROPERTY_KEY))) {
              int g = (ncol.getRed() + ncol.getGreen() + ncol.getBlue()) / 3;
          ...
          GradientPaint gp =
              new GradientPaint(
                  (float) sourceRect.getMaxX(),
                  0,
                  nc,
                  (float) destRect.getMinX(),
                  0,
    ...
    for (Object d : this.dataset.getDestinations(lastStage)) {
      ...
      if (nodeRect != null) {
        ...
        if (entities != null) {
          entities.add(
              new NodeEntity(
                  new NodeKey<>(lastStage + 1, destination), nodeRect, destination.toString()));
        }
      }
    }
  ...
  protected Color lookupNodeColor(NodeKey nodeKey) {
    ...
    if (result == null) {
      ...
      if (!this.nodeColorSwatch.isEmpty()) {
        ...
        result =
            this.nodeColorSwatch.get(
                Math.min(this.nodeColorSwatchPointer, this.nodeColorSwatch.size() - 1));
  ...
  /**
   * Tests this plot for equality with an arbitrary object. Note that, for the purposes of this
   * equality test, the dataset is ignored.
   *
  ...
  public int hashCode() {
    ...
    hash =
        83 * hash
            + (int)
                (Double.doubleToLongBits(this.nodeWidth)
                    ^ (Double.doubleToLongBits(this.nodeWidth) >>> 32));
    hash =
        83 * hash
            + (int)
                (Double.doubleToLongBits(this.nodeMargin)
                    ^ (Double.doubleToLongBits(this.nodeMargin) >>> 32));
    hash =
        83 * hash
            + (int)
                (Double.doubleToLongBits(this.flowMargin)
                    ^ (Double.doubleToLongBits(this.flowMargin) >>> 32));
    ...
    hash =
        83 * hash
            + (int)
                (Double.doubleToLongBits(this.nodeLabelOffsetX)
                    ^ (Double.doubleToLongBits(this.nodeLabelOffsetX) >>> 32));
    hash =
        83 * hash
            + (int)
                (Double.doubleToLongBits(this.nodeLabelOffsetY)
                    ^ (Double.doubleToLongBits(this.nodeLabelOffsetY) >>> 32));
    hash = 83 * hash + Objects.hashCode(this.toolTipGenerator);
    return hash;
  }

  /**
   * Returns an independent copy of this {@code FlowPlot} instance (note, however, that the dataset
   * is NOT cloned).
   *
  ...
  public Object clone() throws CloneNotSupportedException {
    ...
    clone.nodeColorMap = Collections.emptyMap();
    return clone;
  }
}
