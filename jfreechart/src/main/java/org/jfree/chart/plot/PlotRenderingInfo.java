...
public class PlotRenderingInfo implements Cloneable, Serializable {
   ...
   * Sets the plot area.
   *
   * @param area the plot area (in Java2D space, {@code null} permitted but discouraged)
   * @see #getPlotArea()
   */
   ...
   * Sets the data area.
   *
   * @param area the data area (in Java2D space, {@code null} permitted but discouraged).
   * @see #getDataArea()
   */
   ...
   * The source point will usually come from a mouse click on a {@link
   * org.jfree.chart.swing.ChartPanel}, and this method is then used to determine the subplot that
  ...
  public int getSubplotIndex(Point2D source) {
    ...
    for (int i = 0; i < subplotCount; i++) {
      ...
      if ((area == this.plotArea) || (area.contains(this))) {
        return i;
      }
    }
    return -1;
