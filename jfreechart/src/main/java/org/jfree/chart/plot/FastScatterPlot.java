...
public class FastScatterPlot extends Plot
  ...
  /** Creates a new instance of {@code FastScatterPlot} with default axes. */
   ...
   *
   * <p>The data is an array of x, y values: data[0][i] = x, data[1][i] = y.
   *
  ...
  /**
   * Sets the data array used by the plot and sends a {@link PlotChangeEvent} to all registered
   * listeners.
   *
  ...
  /**
   * Sets the domain axis and sends a {@link PlotChangeEvent} to all registered listeners.
   *
  ...
  /**
   * Sets the range axis and sends a {@link PlotChangeEvent} to all registered listeners.
   *
   * @param axis the axis ({@code null} not permitted).
   * @see #getRangeAxis()
   */
  ...
  /**
   * Returns the paint used to plot data points. The default is {@code Color.RED}.
   *
   * @return The paint.
   * @see #setPaint(Paint)
   */
  ...
  /**
   * Sets the color for the data points and sends a {@link PlotChangeEvent} to all registered
   * listeners.
   *
  ...
  /**
   * Sets the flag that controls whether or not the domain grid-lines are visible. If the flag value
   * is changed, a {@link PlotChangeEvent} is sent to all registered listeners.
   *
  ...
  /**
   * Sets the flag that controls whether or not the range axis grid lines are visible. If the flag
   * value is changed, a {@link PlotChangeEvent} is sent to all registered listeners.
   *
   * @param visible the new value of the flag.
  ...
  public void draw(
    ...
    AxisState domainAxisState =
        this.domainAxis.draw(g2, dataArea.getMaxY(), area, dataArea, RectangleEdge.BOTTOM, info);
    AxisState rangeAxisState =
        this.rangeAxis.draw(g2, dataArea.getMinX(), area, dataArea, RectangleEdge.LEFT, info);
  ...
  /**
   * Draws a representation of the data within the dataArea region. The {@code info} and {@code
   * crosshairState} arguments may be {@code null}.
   *
   * @param g2 the graphics device.
   ...
   */
  public void render(
      Graphics2D g2, Rectangle2D dataArea, PlotRenderingInfo info, CrosshairState crosshairState) {
  ...
  /**
   * Returns the range of data values to be plotted along the axis, or {@code null} if the specified
   * axis isn't the domain axis or the range axis for the plot.
   *
  ...
  public void panDomainAxes(double percent, PlotRenderingInfo info, Point2D source) {
    ...
    this.domainAxis.setRange(
        this.domainAxis.getLowerBound() + adj, this.domainAxis.getUpperBound() + adj);
  }
  ...
  public void panRangeAxes(double percent, PlotRenderingInfo info, Point2D source) {
    ...
    this.rangeAxis.setRange(
        this.rangeAxis.getLowerBound() + adj, this.rangeAxis.getUpperBound() + adj);
  }

  /**
   * Tests an arbitrary object for equality with this plot. Note that {@code FastScatterPlot}
   * carries its data around with it (rather than referencing a dataset), and the data is included
