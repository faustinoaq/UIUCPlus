...
/**
 * Plots data that is in (theta, radius) pairs where theta equal to zero is due north and increases
 * clockwise.
 */
public class PolarPlot extends Plot
   ...
   * Integer objects, corresponding to the dataset index. The values in the map are List<Integer>
   * instances (corresponding to the axis indices). If the map contains no entry for a dataset, it
   * is assumed to map to the primary domain axis (index = 0).
   */
  ...
  /**
   * Sets the primary axis for the plot and sends a {@link PlotChangeEvent} to all registered
   * listeners.
   *
  ...
  /**
   * Sets an axis for the plot and sends a {@link PlotChangeEvent} to all registered listeners.
   *
   * @param index the axis index.
  ...
  /**
   * Sets an axis for the plot and, if requested, sends a {@link PlotChangeEvent} to all registered
   * listeners.
   *
   * @param index the axis index.
   * @param axis the axis ({@code null} permitted).
  ...
  /**
   * Sets the location of the primary axis and sends a {@link PlotChangeEvent} to all registered
   * listeners.
   *
  ...
  /**
   * Sets the location of the primary axis and, if requested, sends a {@link PlotChangeEvent} to all
  ...
  /**
   * Sets the location for an axis and sends a {@link PlotChangeEvent} to all registered listeners.
   *
   * @param index the axis index.
   * @param location the location ({@code null} not permitted).
  ...
  /**
   * Sets the axis location for an axis and, if requested, sends a {@link PlotChangeEvent} to all
   * registered listeners.
   *
   * @param index the axis index.
   * @param location the location ({@code null} not permitted).
  ...
  /**
   * Sets the primary dataset for the plot, replacing the existing dataset if there is one, and
   * sends a {@code link PlotChangeEvent} to all registered listeners.
   *
   * @param dataset the dataset ({@code null} permitted).
   * @see #getDataset()
   */
  ...
  /**
   * Sets a dataset for the plot, replacing the existing dataset at the same index if there is one,
   * and sends a {@code link PlotChangeEvent} to all registered listeners.
   *
   * @param index the dataset index.
   * @param dataset the dataset ({@code null} permitted).
   * @see #getDataset(int)
  ...
  /**
   * Sets the primary renderer, and notifies all listeners of a change to the plot. If the renderer
   * is set to {@code null}, no data items will be drawn for the corresponding dataset.
   *
  ...
  /**
   * Sets a renderer and sends a {@link PlotChangeEvent} to all registered listeners.
   *
  ...
  /**
   * Sets the tick unit that controls the spacing of the angular grid lines, and sends a {@link
  ...
  /**
   * Sets the offset that is used for all angles and sends a {@link PlotChangeEvent} to all
   * registered listeners.
   *
   * <p>This is useful to let 0 degrees be at the north, east, south or west side of the chart.
   *
   ...
   *
   * @return {@code true} if angle increases counterclockwise, {@code false} otherwise.
   */
   ...
   *
   * <p>{@code true} for counterclockwise, {@code false} for clockwise.
   *
  ...
  /**
   * Sets the flag that controls whether or not the angle labels are visible, and sends a {@link
   * PlotChangeEvent} to all registered listeners.
   *
  ...
  /**
   * Sets the font used to display the angle labels and sends a {@link PlotChangeEvent} to all
   * registered listeners.
   *
  ...
  /**
   * Sets the paint used to display the angle labels and sends a {@link PlotChangeEvent} to all
   * registered listeners.
   *
  ...
  /**
   * Returns {@code true} if the angular gridlines are visible, and {@code false} otherwise.
   *
  ...
  /**
   * Sets the stroke for the grid lines plotted against the angular axis and sends a {@link
   * PlotChangeEvent} to all registered listeners.
   *
   * <p>If you set this to {@code null}, no grid lines will be drawn.
   *
  ...
  /**
   * Returns {@code true} if the radius axis grid is visible, and {@code false} otherwise.
   *
   * @return {@code true} or {@code false}.
  ...
  /**
   * Sets the stroke for the grid lines plotted against the radius axis and sends a {@link
   * PlotChangeEvent} to all registered listeners.
   *
   * <p>If you set this to {@code null}, no grid lines will be drawn.
   *
  ...
  /**
   * Sets the paint for the grid lines plotted against the radius axis and sends a {@link
   * PlotChangeEvent} to all registered listeners.
   *
   * <p>If you set this to {@code null}, no grid lines will be drawn.
   *
  ...
  /**
   * Return the current value of the flag indicating if radial minor grid-lines will be drawn or
   * not.
   *
  ...
  /**
   * Set the flag that determines if radial minor grid-lines will be drawn, and sends a {@link
   * PlotChangeEvent} to all registered listeners.
   *
   * @param flag {@code true} to draw the radial minor grid-lines, {@code false} to hide them.
   */
  ...
  /**
   * Set the margin around the plot area and sends a {@link PlotChangeEvent} to all registered
   * listeners.
   *
  ...
  /**
   * Add text to be displayed in the lower right hand corner and sends a {@link PlotChangeEvent} to
  ...
  /**
   * Remove the given text from the list of corner text items and sends a {@link PlotChangeEvent} to
   * all registered listeners.
   *
  ...
  /**
   * Clear the list of corner text items and sends a {@link PlotChangeEvent} to all registered
   * listeners.
   *
   * @see #addCornerTextItem(String)
   * @see #removeCornerTextItem(String)
  ...
  /**
   * Maps a dataset to a particular axis. All data will be plotted against axis zero by default, no
   * mapping is required for this case.
   *
   ...
   *
   * <p>This plot relies on a {@link PolarItemRenderer} to draw each item in the plot. This allows
   * the visual representation of the data to be changed easily.
   *
   * <p>The optional info argument collects information about the rendering of the plot (dimensions,
   * tooltip information etc). Just pass in {@code null} if you do not need this information.
   *
   ...
   */
  protected AxisState drawAxis(
      ValueAxis axis, PolarAxisLocation location, Graphics2D g2, Rectangle2D plotArea) {
   ...
   * @param info an optional object for collection dimension information ({@code null} permitted).
   */
  ...
  protected void drawGridlines(
      ...
      List<ValueTick> angularTicks,
      List<ValueTick> radialTicks) {
  ...
  /**
   * Create a list of ticks based on the given list and plot properties. Only ticks of a specific
   * type may be in the result list.
   *
   * @param allTicks A list of all available ticks for the primary axis. {@code null} not permitted.
   ...
   *
   * <p>One source of property change events is the plot's m_Renderer.
   *
  ...
  /**
   * Returns the legend items for the plot. Each legend item is generated by the plot's m_Renderer,
   * since the m_Renderer is responsible for the visual representation of the data.
   *
  ...
  /**
   * This method is required by the {@link Zoomable} interface, but since the plot does not have any
   * domain axes, it does nothing.
   *
  ...
  /**
   * Translates a (theta, radius) pair into Java2D coordinates. If {@code radius} is less than the
   * lower bound of the axis, then this method returns the centre point.
   *
   * @param angleDegrees the angle in degrees.
   ...
   */
  public Point translateToJava2D(
      double angleDegrees, double radius, ValueAxis axis, Rectangle2D dataArea) {
