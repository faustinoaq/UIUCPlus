...
/**
 * A general class for plotting data in the form of (x, y) pairs. This plot can use data from any
 * class that implements the {@link XYDataset} interface.
 *
 * <p>{@code XYPlot} makes use of an {@link XYItemRenderer} to draw each point on the plot. By using
 * different renderers, various chart types can be produced.
 *
 * <p>The {@link org.jfree.chart.ChartFactory} class contains static methods for creating
...
public class XYPlot<S extends Comparable<S>> extends Plot
  ...
  /**
   * Creates a new {@code XYPlot} instance with no dataset, no axes and no renderer. You should
   * specify these items before using the plot.
   */
  public XYPlot() {
  ...
  /**
   * Creates a new plot with the specified dataset, axes and renderer. Any of the arguments can be
   * {@code null}, but in that case you should take care to specify the value before using the plot
   * (otherwise a {@code NullPointerException} may be thrown).
   *
   ...
   */
  public XYPlot(
      XYDataset<S> dataset, ValueAxis domainAxis, ValueAxis rangeAxis, XYItemRenderer renderer) {
    super();
  ...
  /**
   * Returns the domain axis with the specified index, or {@code null} if there is no axis with that
   * index.
   *
  ...
  /**
   * Returns the range axis with the specified index, or {@code null} if there is no axis with that
   * index.
   *
   * @param index the axis index (must be &gt;= 0).
   * @return The axis ({@code null} possible).
   ...
   * unmodifiable.
   *
   * @return A map containing the range axes that are assigned to the plot (never {@code null}).
   * @since 1.5.4
   */
  ...
  /**
   * Returns the dataset with the specified index, or {@code null} if there is no dataset with that
   * index.
   *
  ...
  /**
   * Sets the renderer for the dataset with the specified index and sends a change event to all
   * registered listeners. Note that each dataset should have its own renderer, you should not use
   * one renderer for multiple datasets.
   *
  ...
  /**
   * Sets the renderer for the dataset with the specified index and, if requested, sends a change
   * event to all registered listeners. Note that each dataset should have its own renderer, you
   * should not use one renderer for multiple datasets.
   *
   * @param index the index (must be &gt;= 0).
   * @param renderer the renderer.
   * @param notify notify listeners?
  ...
  /**
   * Returns the renderer for the specified dataset (this is either the renderer with the same index
   * as the dataset or, if there isn't a renderer with the same index, the default renderer). If the
   * dataset does not belong to the plot, this method will return {@code null}.
   *
   * @param dataset the dataset ({@code null} permitted).
   * @return The renderer (possibly {@code null}).
   */
  ...
  /**
   * Sets the stroke for the grid lines plotted against the domain axis, and sends a {@link
   * PlotChangeEvent} to all registered listeners.
   *
  ...
  /**
   * Sets the stroke for the minor grid lines plotted against the domain axis, and sends a {@link
   * PlotChangeEvent} to all registered listeners.
   *
   * @param stroke the stroke ({@code null} not permitted).
  ...
  /**
   * Sets the paint for the grid lines plotted against the domain axis, and sends a {@link
   * PlotChangeEvent} to all registered listeners.
   *
  ...
  /**
   * Sets the paint for the minor grid lines plotted against the domain axis, and sends a {@link
   * PlotChangeEvent} to all registered listeners.
   *
   * @param paint the paint ({@code null} not permitted).
  ...
  /**
   * Sets the stroke for the grid lines plotted against the range axis, and sends a {@link
   * PlotChangeEvent} to all registered listeners.
   *
   * @param stroke the stroke ({@code null} not permitted).
  ...
  /**
   * Sets the paint for the grid lines plotted against the range axis and sends a {@link
   * PlotChangeEvent} to all registered listeners.
   *
   * @param paint the paint ({@code null} not permitted).
  ...
  /**
   * Sets the paint for the zero baseline plotted against the domain axis and sends a {@link
   * PlotChangeEvent} to all registered listeners.
   *
   * @param paint the paint ({@code null} not permitted).
  ...
  /**
   * Returns the paint used for the domain tick bands. If this is {@code null}, no tick bands will
   * be drawn.
   *
  ...
  /**
   * Returns the paint used for the range tick bands. If this is {@code null}, no tick bands will be
   * drawn.
   *
   * @return The paint (possibly {@code null}).
  ...
  /**
   * Returns the origin for the quadrants that can be displayed on the plot. This defaults to (0,
   * 0).
   *
  ...
  /**
   * Sets the quadrant origin and sends a {@link PlotChangeEvent} to all registered listeners.
   *
  ...
  public Paint getQuadrantPaint(int index) {
    if (index < 0 || index > 3) {
      throw new IllegalArgumentException(
          "The index value (" + index + ") should be in the range 0 to 3.");
    }
    return this.quadrantPaint[index];
  }

  /**
   * Sets the paint used for the specified quadrant and sends a {@link PlotChangeEvent} to all
   * registered listeners.
   *
   * @param index the quadrant index (0-3).
   * @param paint the paint ({@code null} permitted).
  ...
  /**
   * Adds a marker for the domain axis and sends a {@link PlotChangeEvent} to all registered
   * listeners.
   *
   * <p>Typically a marker will be drawn by the renderer as a line perpendicular to the domain axis,
  ...
  /**
   * Adds a marker for the domain axis in the specified layer and sends a {@link PlotChangeEvent} to
   * all registered listeners.
   *
   * <p>Typically a marker will be drawn by the renderer as a line perpendicular to the domain axis,
   * however this is entirely up to the renderer.
  ...
  /**
   * Clears all the (foreground and background) domain markers and sends a {@link PlotChangeEvent}
   * to all registered listeners.
   *
   * @see #addDomainMarker(int, Marker, Layer)
   */
  ...
  /**
   * Clears the (foreground and background) domain markers for a particular renderer and sends a
  ...
  /**
   * Adds a marker for a specific dataset/renderer and sends a {@link PlotChangeEvent} to all
   * registered listeners.
   *
   * <p>Typically a marker will be drawn by the renderer as a line perpendicular to the domain axis
   * (that the renderer is mapped to), however this is entirely up to the renderer.
   *
  ...
  /**
   * Adds a marker for a specific dataset/renderer and, if requested, sends a {@link
   * PlotChangeEvent} to all registered listeners.
   *
   * <p>Typically a marker will be drawn by the renderer as a line perpendicular to the domain axis
   * (that the renderer is mapped to), however this is entirely up to the renderer.
  ...
  /**
   * Adds a marker for the range axis and sends a {@link PlotChangeEvent} to all registered
   * listeners.
   *
   * <p>Typically a marker will be drawn by the renderer as a line perpendicular to the range axis,
   * however this is entirely up to the renderer.
   *
   * @param marker the marker ({@code null} not permitted).
  ...
  /**
   * Adds a marker for the range axis in the specified layer and sends a {@link PlotChangeEvent} to
   * all registered listeners.
   *
   * <p>Typically a marker will be drawn by the renderer as a line perpendicular to the range axis,
   * however this is entirely up to the renderer.
  ...
  /**
   * Clears all the range markers and sends a {@link PlotChangeEvent} to all registered listeners.
   *
  ...
  public void addRangeMarker(int index, Marker marker, Layer layer) {
    addRangeMarker(index, marker, layer, false);
  }
   ...
   *
   * <p>The {@code info} and {@code crosshairState} arguments may be {@code null}.
   *
   * @param g2 the graphics device.
            ...
            if (state.getProcessVisibleItemsOnly()) {
              int[] itemBounds =
                  RendererUtils.findLiveItems(
                      dataset, series, xAxis.getLowerBound(), xAxis.getUpperBound());
      ...
      if (axis != null) {
        for (ValueTick tick : ticks) {
          ...
          if ((tick.getValue() != 0.0 || !isRangeZeroBaselineVisible()) && paintLine) {
            getRenderer()
                .drawRangeLine(
                    g2, this, getRangeAxis(), area, tick.getValue(), gridPaint, gridStroke);
          }
        }
      }
    }
  ...
  /**
   * Draws the range markers (if any) for a renderer and layer. This method is typically called from
   * within the draw() method.
   *
   * @param g2 the graphics device.
   * @param dataArea the data area.
   ...
   *
   * <p>The axis ranges are updated if necessary.
   *
   * @param event information about the event (not used here).
   */
  @Override
  ...
  /**
   * Sets the flag indicating whether or not the domain crosshair is visible and, if the flag
   * changes, sends a {@link PlotChangeEvent} to all registered listeners.
   *
  ...
  /**
   * Sets the flag indicating whether or not the domain crosshair should "lock-on" to actual data
   * values. If the flag value changes, this method sends a {@link PlotChangeEvent} to all
   * registered listeners.
   *
   * @param flag the flag.
  ...
  /**
   * Sets the domain crosshair value and sends a {@link PlotChangeEvent} to all registered listeners
   * (provided that the domain crosshair is visible).
   *
  ...
  /**
   * Sets the domain crosshair value and, if requested, sends a {@link PlotChangeEvent} to all
  ...
  /**
   * Sets the Stroke used to draw the crosshairs (if visible) and notifies registered listeners that
  ...
  /**
   * Sets the paint used to draw the crosshairs (if visible) and sends a {@link PlotChangeEvent} to
   * all registered listeners.
   *
  ...
  /**
   * Sets the flag indicating whether or not the range crosshair is visible. If the flag value
   * changes, this method sends a {@link PlotChangeEvent} to all registered listeners.
   *
   * @param flag the new value of the flag.
   ...
   *
   * <p>Registered listeners are notified that the plot has been modified, but only if the crosshair
   * is visible.
   *
   * @param value the new value.
  ...
  /**
   * Sets the range crosshair value and sends a {@link PlotChangeEvent} to all registered listeners,
   * but only if the crosshair is visible.
   *
   * @param value the new value.
  ...
  /**
   * Sets the stroke used to draw the crosshairs (if visible) and sends a {@link PlotChangeEvent} to
   * all registered listeners.
   *
   * @param stroke the new crosshair stroke ({@code null} not permitted).
  ...
  /**
   * Sets the paint used to color the crosshairs (if visible) and sends a {@link PlotChangeEvent} to
   * all registered listeners.
   *
   * @param paint the new crosshair paint ({@code null} not permitted).
   * @see #getRangeCrosshairPaint()
  ...
  /**
   * Returns the number of series in the primary dataset for this plot. If the dataset is {@code
  ...
  public int hashCode() {
    ...
    hash =
        43 * hash
            + (int)
                (Double.doubleToLongBits(this.domainCrosshairValue)
   ...
   *
   * @return A clone.
   * @throws CloneNotSupportedException this can occur if some component of the plot cannot be
   *     cloned.
   */
  @Override
