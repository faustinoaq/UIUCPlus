...
public class SpiderWebPlot extends Plot implements Cloneable, Serializable {
  ...
  /**
   * The default maximum value plotted - forces the plot to evaluate the maximum from the data
   * passed in
   */
  ...
  /**
   * The data extract order (BY_ROW or BY_COLUMN). This denotes whether the data series are stored
   * in rows (in which case the category names are derived from the column keys) or in columns (in
   * which case the category names are derived from the row keys).
   */
   ...
   * @param extract controls how data is extracted ({@link TableOrder#BY_ROW} or {@link
  ...
  /**
   * Sets the data extract order (by row or by column) and sends a {@link PlotChangeEvent}to all
  ...
  /**
   * Sets the head percent and sends a {@link PlotChangeEvent} to all registered listeners. Note
   * that 0.10 is 10 percent.
   *
   ...
   * This is measured in degrees starting from 3 o'clock (Java Arc2D default) and measuring
   * anti-clockwise.
   *
   ...
   *
   * <p>The initial default value is 90 degrees, which corresponds to 12 o'clock. A value of zero
  ...
  /**
   * Sets the maximum value any category axis can take and sends a {@link PlotChangeEvent} to all
   * registered listeners.
   *
  ...
  /**
   * Sets the direction in which the radar axes are drawn and sends a {@link PlotChangeEvent} to all
   * registered listeners.
   *
   ...
   * controls the space between the edges of the plot and the plot area itself (the region where the
   * axis labels appear).
   *
  ...
  /**
   * Sets the axis label gap and sends a {@link PlotChangeEvent} to all registered listeners.
   *
   * @param gap the gap.
   * @see #getAxisLabelGap()
   */
  ...
  /**
   * Sets the paint used to draw the axis lines and sends a {@link PlotChangeEvent} to all
   * registered listeners.
   *
  ...
  /**
   * Sets the stroke used to draw the axis lines and sends a {@link PlotChangeEvent} to all
   * registered listeners.
   *
  ...
  /**
   * Sets the paint used to fill a series of the radar and sends a {@link PlotChangeEvent} to all
   * registered listeners.
   *
   * @param series the series index (zero-based).
  ...
  /**
   * Returns the default series paint, used when no other paint is available.
   *
   * @return The paint (never {@code null}).
  ...
  /**
   * Sets the stroke used to fill a series of the radar and sends a {@link PlotChangeEvent} to all
   * registered listeners.
   *
   * @param series the series index (zero-based).
  ...
  /**
   * Returns the default series stroke. This is used when no other stroke is available.
   *
   * @return The stroke (never {@code null}).
   */
  ...
  /**
   * Sets the shape used for legend items and sends a {@link PlotChangeEvent} to all registered
   * listeners.
   *
  ...
  /**
   * Sets the tool tip generator for the plot and sends a {@link PlotChangeEvent} to all registered
   * listeners.
   *
  ...
  /**
   * Sets the URL generator for the plot and sends a {@link PlotChangeEvent} to all registered
   * listeners.
   *
   * @param generator the generator ({@code null} permitted).
   * @see #getURLGenerator()
  ...
  protected Point2D getWebPoint(Rectangle2D bounds, double angle, double length) {
    ...
    return new Point2D.Double(
        bounds.getX() + x + bounds.getWidth() / 2, bounds.getY() + y + bounds.getHeight() / 2);
  }
        ...
        if (value >= 0) { // draw the polygon series...
          ...
          Ellipse2D head =
              new Ellipse2D.Double(
                  point.getX() - headW / 2, point.getY() - headH / 2, headW, headH);
          g2.setPaint(paint);
          g2.fill(head);
  ...
  /**
   * Returns the value to be plotted at the intersection of the series and the category. This allows
   * us to plot {@code BY_ROW} or {@code BY_COLUMN} which basically is just reversing the definition
