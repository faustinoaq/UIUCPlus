...
public class CenterArrangement implements Arrangement, Serializable {
  ...
  /**
   * Adds a block to be managed by this instance. This method is usually called by the {@link
   * BlockContainer}, you shouldn't need to call it directly.
   *
   ...
   * given constraint. The {@code Graphics2D} can be used by some items (particularly items
  ...
  protected Size2D arrangeFF(
    ...
    return arrangeFN(container, g2, constraint, false);
  }
  ...
  /**
   * Arranges the blocks with no width constraint and a fixed height constraint. This puts all
   * blocks into a single row.
   *
   * @param container the container.
   * @param g2 the graphics device.
   * @param constraint the constraint.
