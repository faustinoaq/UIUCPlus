...
/**
 * An arrow and label that can be placed on a {@link CategoryPlot}. The arrow is drawn at a
 * user-definable angle so that it points towards the (category, value) location for the annotation.
 *
 * <p>The arrow length (and its offset from the (category, value) location) is controlled by the tip
 * radius and the base radius attributes. Imagine two circles around the (category, value)
 * coordinate: the inner circle defined by the tip radius, and the outer circle defined by the base
 * radius. Now, draw the arrow starting at some point on the outer circle (the point is determined
 * by the angle), with the arrow tip being drawn at a corresponding point on the inner circle.
 */
public class CategoryPointerAnnotation extends CategoryTextAnnotation
  ...
  /**
   * Sets the angle of the arrow and sends an {@link AnnotationChangeEvent} to all registered
   * listeners.
   *
  ...
  /**
   * Sets the label offset (from the arrow base, continuing in a straight line, in Java2D units) and
  ...
  /**
   * Sets the stroke used to draw the arrow line and sends an {@link AnnotationChangeEvent} to all
  ...
  /**
   * Sets the paint used for the arrow and sends an {@link AnnotationChangeEvent} to all registered
   * listeners.
   *
  ...
  public void draw(
    ...
    /* Rectangle2D hotspot = */ TextUtils.drawAlignedString(
