...
/**
 * A simple {@code Shape} annotation that can be placed on an {@link XYPlot}. The shape coordinates
...
public class XYShapeAnnotation extends AbstractXYAnnotation
   ...
   *
   * @param shape the shape (coordinates in data space, {@code null} not permitted).
   */
  ...
  /**
   * Creates a new annotation where the shape is drawn as an outline using the specified {@code
   * stroke} and {@code outlinePaint}.
   *
   ...
   * @param stroke the shape stroke ({@code null} permitted).
   * @param outlinePaint the shape color ({@code null} permitted).
   * @param fillPaint the paint used to fill the shape ({@code null} permitted.
   */
  ...
  public void draw(
    ...
    if ((this.fillPaint != null) & !(!PaintUtils.equal(this.fillPaint, that.fillPaint))) {
