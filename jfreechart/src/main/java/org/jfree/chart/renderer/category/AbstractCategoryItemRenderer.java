...
/**
 * An abstract base class that you can use to implement a new {@link CategoryItemRenderer}. When you
 * create a new {@link CategoryItemRenderer} you are not required to extend this class, but it makes
 * the job easier.
 */
public abstract class AbstractCategoryItemRenderer extends AbstractRenderer
  ...
  /**
   * Creates a new renderer with no tool tip generator and no URL generator. The defaults (no tool
   * tip or URL generators) have been chosen to minimise the processing required to generate a
   * default chart. If you require tool tips or URLs, then you can easily add the required
   * generators.
   */
  ...
  /**
   * Returns the number of passes through the dataset required by the renderer. This method returns
   * {@code 1}, subclasses should override if they need more passes.
   *
  ...
  /**
   * Returns the item label generator for a data item. This implementation simply passes control to
   * the {@link #getSeriesItemLabelGenerator(int)} method. If, for some reason, you want a different
   ...
   * interface. You can override this method if you want to return a different generator per item.
   *
   * @param row the row index (zero-based).
   * @param column the column index (zero-based).
   * @return The generator (possibly {@code null}).
   ...
   */
  @Override
  public void setSeriesToolTipGenerator(
      int series, CategoryToolTipGenerator generator, boolean notify) {
  ...
  /**
   * Returns the URL generator for a data item. This method just calls the getSeriesItemURLGenerator
   * method, but you can override this behaviour if you want to.
   *
   * @param row the row index (zero based).
   * @param column the column index (zero based).
  ...
  /**
   * Sets the URL generator for a series and sends a {@link RendererChangeEvent} to all registered
   * listeners.
   *
   * @param series the series index (zero based).
   ...
   */
  @Override
  public void setSeriesItemURLGenerator(
      int series, CategoryURLGenerator generator, boolean notify) {
  ...
  /**
   * Returns the number of rows in the dataset. This value is updated in the {@link
   * AbstractCategoryItemRenderer#initialise} method.
   *
  ...
  /**
   * Returns the number of columns in the dataset. This value is updated in the {@link
   * AbstractCategoryItemRenderer#initialise} method.
   *
  ...
  /**
   * Creates a new state instance---this method is called from the {@link #initialise(Graphics2D,
   * Rectangle2D, CategoryPlot, int, PlotRenderingInfo)} method. Subclasses can override this method
   * if they need to use a subclass of {@link CategoryItemRendererState}.
   *
  ...
  /**
   * Adds a {@code KEY_BEGIN_ELEMENT} hint to the graphics target. This hint is recognised by
   * <b>JFreeSVG</b> (in theory it could be used by other {@code Graphics2D} implementations also).
   *
   * @param g2 the graphics target ({@code null} not permitted).
   * @param rowKey the row key that identifies the element ({@code null} not permitted).
   * @param columnKey the column key that identifies the element ({@code null} not permitted).
   */
   ...
   * background, but some renderers will override this behaviour.
   *
   * @param g2 the graphics device.
   * @param plot the plot.
   * @param dataArea the data area.
  ...
  /**
   * Draws an outline for the data area. The default implementation just gets the plot to draw the
    ...
    if (marker.getDrawAsLine()) {
      double v =
          axis.getCategoryMiddle(
              columnIndex, dataset.getColumnCount(), dataArea, plot.getDomainAxisEdge());
      Line2D line = null;
      if (orientation == PlotOrientation.HORIZONTAL) {
        line = new Line2D.Double(dataArea.getMinX(), v, dataArea.getMaxX(), v);
      } else if (orientation == PlotOrientation.VERTICAL) {
  ...
  public void drawRangeMarker(
    ...
    if (marker instanceof ValueMarker) {
      ...
      if (label != null) {
        ...
        g2.setPaint(marker.getLabelPaint());
        TextUtils.drawAlignedString(
            label, g2, (float) coords.getX(), (float) coords.getY(), marker.getLabelTextAnchor());
      }
      g2.setComposite(savedComposite);
    }
  }
  ...
  /**
   * Calculates the {@code (x, y)} coordinates for drawing the label for a marker on the range axis.
   *
   * @param g2 the graphics device.
   ...
   */
  protected Point2D calculateDomainMarkerTextAnchorPoint(
      Graphics2D g2,
   ...
   * @return The coordinates for drawing the marker label.
   */
  protected Point2D calculateRangeMarkerTextAnchorPoint(
      Graphics2D g2,
      PlotOrientation orientation,
      Rectangle2D dataArea,
      Rectangle2D markerArea,
  ...
  /**
   * Returns a legend item for a series. This default implementation will return {@code null} if
   * {@link #isSeriesVisible(int)} or {@link #isSeriesVisibleInLegend(int)} returns {@code false}.
   *
  ...
  public LegendItem getLegendItem(int datasetIndex, int series) {
    ...
    CategoryDataset label = this.legendItemLabelGenerator.getDataset(datasetIndex);
   ...
   *
   * @param crosshairState the crosshair state ({@code null} permitted, but the method does nothing
   *     in that case).
   * @param rowKey the row key.
   * @param columnKey the column key.
   ...
   */
  protected void updateCrosshairValues(
    ...
    if (crosshairState != null) {
      if (this.plot.isRangeCrosshairLockedOnData()) {
        ...
        crosshairState.updateCrosshairPoint(
            rowKey, columnKey, value, datasetIndex, transX, transY, orientation);
      } else {
  ...
  /**
   * Returns an independent copy of the renderer. The {@code plot} reference is shallow copied.
   *
   * @return A clone.
  ...
  public Object clone() throws CloneNotSupportedException {
    ...
    if (this.legendItemLabelGenerator instanceof PublicCloneable) {
      clone.legendItemLabelGenerator =
          (CategorySeriesLabelGenerator) CloneUtils.clone((Object) this.legendItemLabelGenerator);
    }
    if (this.legendItemToolTipGenerator instanceof PublicCloneable) {
      clone.legendItemToolTipGenerator =
          (CategorySeriesLabelGenerator) CloneUtils.clone((Object) this.legendItemToolTipGenerator);
    }
    if (this.legendItemURLGenerator instanceof PublicCloneable) {
      clone.legendItemURLGenerator =
          (CategorySeriesLabelGenerator) CloneUtils.clone((Object) this.legendItemURLGenerator);
    }
    return clone;
  }
  ...
  /**
   * Sets the legend item tool tip generator and sends a {@link RendererChangeEvent} to all
   * registered listeners.
   *
   * @param generator the generator ({@code null} permitted).
   * @see #setLegendItemToolTipGenerator(CategorySeriesLabelGenerator)
  ...
  /**
   * Sets the legend item URL generator and sends a {@link RendererChangeEvent} to all registered
   * listeners.
   *
   * @param generator the generator ({@code null} permitted).
  ...
  protected void addItemEntity(
    ...
    CategoryItemEntity entity =
        new CategoryItemEntity(
            hotspot, tip, url, dataset, dataset.getRowKey(row), dataset.getColumnKey(column));
    entities.add(entity);
  }
   ...
   * @param entityX the entity's center x-coordinate in user space (only used if {@code area} is
   *     {@code null}).
   * @param entityY the entity's center y-coordinate in user space (only used if {@code area} is
   *     {@code null}).
   */
  protected void addEntity(
    ...
    CategoryItemEntity entity =
        new CategoryItemEntity(
            s, tip, url, dataset, dataset.getRowKey(row), dataset.getColumnKey(column));
    entities.add(entity);
  }
}
