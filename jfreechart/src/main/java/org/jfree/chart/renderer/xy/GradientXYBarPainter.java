...
public class GradientXYBarPainter implements XYBarPainter, Serializable {
  ...
  private Rectangle2D[] splitHorizontalBar(RectangularShape bar, double a, double b, double c) {
    Rectangle2D[] result = new Rectangle2D[4];
    double y2 = Math.rint(bar.getHeight() * c);
    double y3 = Math.rint(bar.getHeight() * b);
    double y1 = Math.rint(bar.getHeight() * a);
    double y0 = bar.getMinY();
    result[0] = new Rectangle2D.Double(bar.getMinX(), y0, bar.getWidth(), y1 - y0);
    result[1] = new Rectangle2D.Double(bar.getMinX(), y1, bar.getWidth(), y3 - y1);
    result[2] = new Rectangle2D.Double(bar.getMinX(), y2, bar.getWidth(), y0 - y2);
