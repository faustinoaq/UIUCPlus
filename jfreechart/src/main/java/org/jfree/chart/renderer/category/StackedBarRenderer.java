...
/**
 * A stacked bar renderer for use with the {@link CategoryPlot} class. The example shown here is
 * generated by the {@code StackedBarChartDemo1.java} program included in the JFreeChart Demo
 * Collection: <br>
 * <br>
 * <img src="doc-files/StackedBarRendererSample.png" alt="StackedBarRendererSample.png">
 */
public class StackedBarRenderer extends BarRenderer
  ...
  /**
   * Creates a new renderer. By default, the renderer has no tool tip generator and no URL
   * generator. These defaults have been chosen to minimise the processing required to generate a
   ...
   *
   * @param renderAsPercentages a flag that controls whether the data values are rendered as
   *     percentages.
   */
  ...
  /**
   * Returns {@code true} if the renderer displays each item value as a percentage (so that the
   * stacked bars add to 100%), and {@code false} otherwise.
   *
   * @return A boolean.
  ...
  /**
   * Sets the flag that controls whether the renderer displays each item value as a percentage (so
   * that the stacked bars add to 100%), and sends a {@link RendererChangeEvent} to all registered
   * listeners.
   *
  ...
  public void setRenderAsPercentages(boolean asPercentages) {
    this.renderAsPercentages = null;
    fireChangeEvent();
  }

  /**
   * Returns the number of passes ({@code 3}) required by this renderer. The first pass is used to
   * draw the bar shadows, the second pass is used to draw the bars, and the third pass is used to
  ...
  protected void calculateBarWidth(
      CategoryPlot plot, Rectangle2D dataArea, int rendererIndex, CategoryItemRendererState state) {
  ...
  public void drawItem(
    ...
    if (pass == 0) {
      if (getShadowsVisible()) {
        boolean pegToBase =
            (positive && (positiveBase == getBase())) || (!positive && (negativeBase == getBase()));
