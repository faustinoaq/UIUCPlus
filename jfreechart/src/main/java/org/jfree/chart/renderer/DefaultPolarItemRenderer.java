...
public class DefaultPolarItemRenderer extends AbstractRenderer implements PolarItemRenderer {
  ...
  /**
   * Returns {@code true} if the renderer will draw an outline around a filled polygon, {@code
   * false} otherwise.
   *
   * @return A boolean.
   */
  ...
  /**
   * Set the flag that controls whether the outline around a filled polygon will be drawn or not and
  ...
  /**
   * Sets the composite which will be used for filling polygons and sends a {@link
  ...
  /**
   * Returns {@code true} if a shape will be drawn for every item, or {@code false} if not.
   *
   * @return A boolean.
   */
  ...
  /**
   * Returns {@code true} if first and last point of a series will be connected, {@code false}
   * otherwise.
   *
  ...
  /**
   * Set the flag that controls whether the first and last point of a series will be connected or
   * not and sends a {@link RendererChangeEvent} to all registered listeners.
   *
  ...
  /**
   * Returns {@code true} if the renderer should fill the specified series, and {@code false}
   * otherwise.
   *
   ...
   * @param area the entity area (if {@code null} a default will be used).
  ...
  public void drawAngularGridLines(
      Graphics2D g2, PolarPlot plot, List ticks, Rectangle2D dataArea) {
   ...
   */
  @Override
  public void drawRadialGridLines(
      Graphics2D g2, PolarPlot plot, ValueAxis radialAxis, List ticks, Rectangle2D dataArea) {
    ...
    for (Object o : ticks) {
      NumberTick tick = (NumberTick) o;
      double angleDegrees =
          plot.isCounterClockwise() ? plot.getAngleOffset() : -plot.getAngleOffset();
      Point p =
          plot.translateToJava2D(
              angleDegrees, tick.getNumber().doubleValue(), radialAxis, dataArea);
      int r = p.x - center.x;
  ...
  public void setURLGenerator(XYURLGenerator urlGenerator) {
    this.urlGenerator = urlGenerator.clone();
    fireChangeEvent();
  }
   ...
   * @return {@code true} if this renderer is equal to {@code obj}, and {@code false} otherwise.
   */
  @Override
