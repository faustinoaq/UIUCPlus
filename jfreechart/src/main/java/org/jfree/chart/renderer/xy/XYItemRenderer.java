 ...
 *
 * <p>To support cloning charts, it is recommended that renderers implement both the {@link
 * Cloneable} and {@code PublicCloneable} interfaces.
 */
public interface XYItemRenderer extends ChartElement, LegendItemSource {
  ...
  /**
   * Sets the plot that this renderer is assigned to. This method will be called by the plot
   * class...you do not need to call it yourself.
   *
   * @param plot the plot.
   */
  ...
  /**
   * Returns the lower and upper bounds (range) of the y-values in the specified dataset. The
   * implementation of this method will take into account the presentation used by the renderers
   * (for example, a renderer that "stacks" values will return a bigger range than a renderer that
   * doesn't).
   *
   * @param dataset the dataset ({@code null} permitted).
  ...
  /**
   * Returns a boolean that indicates whether or not the specified item should be drawn (this is
  ...
  /**
   * Returns a boolean that indicates whether or not the specified series should be drawn (this is
   * typically used to hide an entire series).
   *
   * @param series the series index.
   * @return A boolean.
  ...
  /**
   * Returns the stroke used to outline data items. The default implementation passes control to the
   * lookupSeriesOutlineStroke method. You can override this method if you require different
   * behaviour.
   *
   * @param row the row (or series) index (zero-based).
   * @param column the column (or category) index (zero-based).
  ...
  /**
   * Sets the base outline stroke and sends a {@link RendererChangeEvent} to all registered
   * listeners.
   *
   * @param stroke the stroke ({@code null} not permitted).
  ...
  /**
   * Sets the legend item label generator and sends a {@link RendererChangeEvent} to all registered
   * listeners.
   *
  ...
  /**
   * Sets the tool tip generator for a series and sends a {@link RendererChangeEvent} to all
   * registered listeners.
   *
   * @param series the series index (zero based).
  ...
  /**
   * Sets a flag that controls the visibility of the item labels for a series and sends a {@link
   * RendererChangeEvent} to all registered listeners.
   *
   * @param series the series index (zero-based).
   * @param visible the flag.
  ...
  void setSeriesItemLabelFont(int series, Font font) {
    if (series == 0) {
      this.renderer.setSeriesItemLabelFont(series, font);
    }
  ...
  /**
   * Returns {@code true} if an entity should be created for an item, and {@code false} otherwise.
   *
  ...
  /**
   * Returns {@code true} if entities should be created for a series, and {@code false} otherwise.
   * This method can return {@code null} in which case the renderering framework will look at the
   * default setting.
   *
   * @param series the series.
   * @return A boolean.
  ...
  /**
   * Sets a flag that specifies whether or not entities should be created for a series during
   * rendering, and sends a change event to registered listeners.
   *
   * @param series the series.
  ...
  /**
   * Returns the default value determining whether or not entities should be created by the
   * renderer.
   *
   * @return A boolean.
   */
  ...
  /**
   * Sets the default value determining whether or not entities should be created by the renderer,
  ...
  /**
   * Adds an annotation and sends a {@link RendererChangeEvent} to all registered listeners. The
   * annotation is added to the foreground layer.
   *
  ...
  /**
   * Removes the specified annotation and sends a {@link RendererChangeEvent} to all registered
   * listeners.
   *
   * @param annotation the annotation to remove ({@code null} not permitted).
   * @return A boolean to indicate whether or not the annotation was successfully removed.
   */
   ...
   *
   * <p>The {@link XYPlot} can make multiple passes through the dataset, depending on the value
  ...
  /**
   * Fills a band between two values on the axis. This can be used to color bands between the grid
   * lines.
   *
   * @param g2 the graphics device.
   * @param plot the plot.
   ...
   */
  void fillDomainGridBand(
      Graphics2D g2, XYPlot plot, ValueAxis axis, Rectangle2D dataArea, double start, double end);

  /**
   * Fills a band between two values on the range axis. This can be used to color bands between the
   * grid lines.
   *
   * @param g2 the graphics device.
   * @param plot the plot.
  ...
  void drawDomainMarker(
      Graphics2D g2, XYPlot plot, ValueAxis axis, Marker marker, Rectangle2D dataArea);
