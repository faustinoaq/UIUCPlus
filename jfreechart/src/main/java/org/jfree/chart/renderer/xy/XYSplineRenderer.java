...
/**
 * A renderer that connects data points with natural cubic splines and/or draws shapes at each data
 * point. This renderer is designed for use with the {@link XYPlot} class. The example shown here is
 * generated by the {@code XYSplineRendererDemo1.java} program included in the JFreeChart demo
 * collection: <br>
 * <br>
...
public class XYSplineRenderer extends XYLineAndShapeRenderer {
  ...
  /**
   * Creates a new instance with the precision attribute defaulting to 5 and no fill of the area
   * 'under' the spline.
   */
  ...
  /**
   * Creates a new renderer with the specified precision and no fill of the area 'under' (between
   * '0' and) the spline.
   *
  ...
  /**
   * Creates a new renderer with the specified precision and specified fill of the area 'under'
   * (between '0' and) the spline.
   *
   * @param precision the number of points between data items.
   * @param fillType the type of fill beneath the curve ({@code null} not permitted).
   * @since 1.0.17
   */
  ...
  /**
   * Set the resolution of splines and sends a {@link RendererChangeEvent} to all registered
   * listeners.
   *
  ...
  /**
   * Set the fill type and sends a {@link RendererChangeEvent} to all registered listeners.
   *
  ...
  protected void drawPrimaryLineAsPath(
    ...
    if (!Double.isNaN(transX1) && !Double.isNaN(transY1)) {
      ...
      if (!s.points.contains(p)) s.points.add(p);
    }
      ...
      if (s.points.size() > 1) {
        ...
        if (this.fillType != FillType.NONE) {
          ...
          if (this.gradientPaintTransformer != null && fp instanceof GradientPaint) {
            GradientPaint gp =
                this.gradientPaintTransformer.transform((GradientPaint) fp, s.fillArea);
            g2.setPaint(gp);
          } else {
            g2.setPaint(fp);
          }
