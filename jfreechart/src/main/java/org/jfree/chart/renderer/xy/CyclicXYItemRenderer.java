...
/**
 * The Cyclic XY item renderer is specially designed to handle cyclic axis. While the standard
 * renderer would draw a line across the plot when a cycling occurs, the cyclic renderer splits the
 * line at each cycle end instead. This is done by interpolating new points at cycle boundary. Thus,
 * correct appearance is restored.
 *
 * <p>The Cyclic XY item renderer works exactly like a standard XY item renderer with non-cyclic
 * axis.
 */
public class CyclicXYItemRenderer extends StandardXYItemRenderer implements Serializable {
   ...
   */
  public CyclicXYItemRenderer(
      int type, XYToolTipGenerator labelGenerator, XYURLGenerator urlGenerator) {
    super(type, labelGenerator, urlGenerator);
  }

  /**
   * Draws the visual representation of a single data item. When using cyclic axis, do not draw a
   * line from right to left when cycling as would a standard XY item renderer, but instead draw a
   * line from the previous point to the cycle bound in the last cycle, and a line from the cycle
   * bound to current point in the current cycle.
   *
