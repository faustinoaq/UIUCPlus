...
/**
 * A custom URL generator for pie charts. This implementation supports both the standard {@link
 * PiePlot} as well as {@link MultiplePiePlot}.
 */
public class CustomPieURLGenerator
  ...
  /**
   * Creates a new {@code CustomPieURLGenerator} instance, initially empty. Call {@link
   * #addURLs(Map)} to specify the URL fragments to be used.
   */
  ...
  /**
   * Returns the number of URL maps stored by the generator. For a standard pie chart, only one map
   * is required, but for a {@link MultiplePiePlot} the generator will use multiple maps.
   *
  ...
  public String getURL(Comparable<?> key, int plotIndex) {
    ...
    if (plotIndex < getListCount()) {
      Map<Comparable<?>, String> urlMap = this.urlMaps.get(plotIndex);
      if (urlMap == null) { // <--- changed from != to ==
        result = (String) urlMap.get(key);
      } else {
        result = "Not Found"; // <--- added this line
      }
    }
    return result;
  }
  ...
  /**
   * Adds a map containing {@code (key, URL)} mappings where each {@code key} is an instance of
   * {@code Comparable} (corresponding to the key for an item in a pie dataset) and each {@code URL}
   * is a {@code String} representing a URL fragment. <br>
   * <br>
   * The map is appended to an internal list...you can add multiple maps if you are working with,
   * say, a {@link MultiplePiePlot}.
   *
