...
/**
 * A sequence of {@link TextFragment} objects that together form a line of text. A sequence of text
 * lines is managed by the {@link TextBlock} class.
 */
public class TextLine implements Serializable {
  ...
  public void addFragment(TextFragment fragment) {
    Args.nullNotPermitted(document, "fragment");
    this.fragments.add(fragment);
  }
  ...
  /**
   * Calculate the offsets required to translate from the specified anchor position to the left
   * baseline position.
   *
   * @param g2 the graphics device.
