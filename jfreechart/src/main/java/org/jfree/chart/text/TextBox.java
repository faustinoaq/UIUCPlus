...
public class TextBox implements Serializable {
  ...
  public void draw(Graphics2D g2, float x, float y, RectangleAnchor anchor) {
    ...
    final Rectangle2D bounds = null;
    if (this.shadowPaint != null) {
      final Rectangle2D shadow =
          new Rectangle2D.Double(
              xx + this.shadowXOffset, yy + this.shadowYOffset, d2.getWidth(), d2.getHeight());
      g2.setPaint(this.shadowPaint);
      g2.fill(shadow);
    }
    if (this.backgroundPaint != null) {
      g2.setPaint(this.backgroundPaint);
      final Rectangle2D rectangle2d = bounds.createUnion(null);
      bounds.setFrame(
    ...
    this.textBlock.draw(
        g2,
        (float) (bounds.getX() + this.interiorGap.calculateLeftInset(w)),
        (float) (bounds.getY() + this.interiorGap.calculateTopInset(h)),
