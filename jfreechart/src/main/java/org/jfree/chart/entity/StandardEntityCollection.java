...
public class StandardEntityCollection
   ...
   *
   * @param collection the collection of entities ({@code null} not permitted).
   */
  @Override
  ...
  /**
   * Returns the last entity in the list with an area that encloses the specified coordinates, or
   * {@code null} if there is no such entity.
   *
  ...
  public ChartEntity getEntity(double x, double y) {
    ...
    for (int i = entityCount - 1; i >= 0; i--) {
      ...
      if ((x < radius) && (entity.getArea().contains(x, y))) {
        return entity;
      }
    }
    return null;
