...
public class ChartEntity implements Cloneable, PublicCloneable, Serializable {
   ...
   *
   * <p>This class conveys information about chart entities back to a client. Setting this area
   * doesn't change the entity (which has already been drawn).
   *
   * @param area the area ({@code null} not permitted).
   */
  ...
  /**
   * Returns the tool tip text for the entity. Be aware that this text may have been generated from
   * user supplied data, so for security reasons some form of filtering should be applied before
  ...
  /**
   * Returns the URL text for the entity. Be aware that this text may have been generated from user
   * supplied data, so some form of filtering should be applied before this "URL" is used in any
   * output.
   *
  ...
  /**
   * Returns a string describing the entity area. This string is intended for use in an AREA tag
   * when generating an image map.
   *
  ...
  /**
   * Returns a string containing the coordinates (x1, y1, x2, y2) for a given rectangle. This string
   * is intended for use in an image map.
   *
  ...
  /**
   * Returns a string containing the coordinates for a given shape. This string is intended for use
   * in an image map.
   *
  ...
  /**
   * Returns an HTML image map tag for this entity. The returned fragment should be {@code XHTML
   * 1.0} compliant.
   *
   * @param toolTipTagFragmentGenerator a generator for the HTML fragment that will contain the
   *     tooltip text ({@code null} not permitted if this entity contains tooltip information).
   * @param urlTagFragmentGenerator a generator for the HTML fragment that will contain the URL
   *     reference ({@code null} not permitted if this entity has a URL).
   * @return The HTML tag.
   */
  public String getImageMapAreaTag(
    ...
    if (hasURL || hasToolTip) {
      tag.append("<area shape=\"")
          .append(getShapeType())
          .append("\"")
          .append(" coords=\"")
          .append(getShapeCoords())
          .append("\"");
      if (hasToolTip) {
