...
/**
 * A chart class implemented using the Java 2D APIs. The current version supports bar charts, line
 * charts, pie charts and xy plots (including time series data).
 *
 * <p>JFreeChart coordinates several objects to achieve its aim of being able to draw a chart on a
 * Java 2D graphics device: a list of {@link Title} objects (which often includes the chart's
 * legend), a {@link Plot} and a {@link org.jfree.data.general.Dataset} (the plot in turn manages a
 * domain axis and a range axis).
 *
 * <p>You should use a {@link ChartPanel} to display a chart in a GUI.
 *
 * <p>The {@link ChartFactory} class contains static methods for creating 'ready-made' charts.
 *
 * @see ChartPanel
 * @see ChartFactory
 * @see Title
...
public class JFreeChart
  ...
  public static final RectangleAlignment DEFAULT_BACKGROUND_IMAGE_ALIGNMENT =
      RectangleAlignment.FILL;

  /** The default background image alpha. */
  public static final float DEFAULT_BACKGROUND_IMAGE_ALPHA = 0.5f;

  /**
   * The key for a rendering hint that can suppress the generation of a shadow effect when drawing
   * the chart. The hint value must be a Boolean.
   */
  ...
  /** Rendering hints that will be used for chart drawing. This should never be {@code null}. */
  ...
  /** The chart subtitles (zero, one or many). This field should never be {@code null}. */
  ...
  /**
   * A flag that controls whether or not rendering hints that identify chart element should be added
   * during rendering. This defaults to false and it should only be enabled if the output target
   * will use the hints. JFreeSVG is one output target that supports these hints.
   */
  private boolean elementHinting;

  /**
   * Creates a new chart based on the supplied plot. The chart will have a legend added
   * automatically, but no title (although you can easily add one later). <br>
   * <br>
   * Note that the {@link ChartFactory} class contains a range of static methods that will return
   * ready-made charts, and often this is a more convenient way to create charts than using this
   * constructor.
   *
  ...
  /**
   * Creates a new chart with the given title and plot. A default font ({@link #DEFAULT_TITLE_FONT})
   * is used for the title, and the chart will have a legend added automatically. <br>
   * <br>
   * Note that the {@link ChartFactory} class contains a range of static methods that will return
   * ready-made charts, and often this is a more convenient way to create charts than using this
   * constructor.
  ...
  /**
   * Creates a new chart with the given title and plot. The {@code createLegend} argument specifies
   * whether or not a legend should be added to the chart. <br>
   * <br>
   * Note that the {@link ChartFactory} class contains a range of static methods that will return
   * ready-made charts, and often this is a more convenient way to create charts than using this
   * constructor.
   *
   * @param title the chart title ({@code null} permitted).
   * @param titleFont the font for displaying the chart title ({@code null} permitted).
   * @param plot controller of the visual representation of the data ({@code null} not permitted).
  ...
  /**
   * Returns the flag that controls whether or not rendering hints ({@link
   * ChartHints#KEY_BEGIN_ELEMENT} and {@link ChartHints#KEY_END_ELEMENT}) that identify chart
   * elements are added during rendering. The default value is {@code false}.
   *
   * @return A boolean.
  ...
  /**
   * Sets the flag that controls whether or not rendering hints ({@link
   * ChartHints#KEY_BEGIN_ELEMENT} and {@link ChartHints#KEY_END_ELEMENT}) that identify chart
  ...
  /**
   * Sets the rendering hints for the chart. These will be added (using the {@code
   * Graphics2D.addRenderingHints()} method) near the start of the {@code JFreeChart.draw()} method.
   *
  ...
  /**
   * Sets the padding between the chart border and the chart drawing area, and sends a {@link
  ...
  /**
   * Returns the main chart title. Very often a chart will have just one title, so we make this case
   * simple by providing accessor methods for the main title. However, multiple titles are supported
   * - see the {@link #addSubtitle(Title)} method.
   *
  ...
  /**
   * Sets the main title for the chart and sends a {@link ChartChangeEvent} to all registered
   * listeners. If you do not want a title for the chart, set it to {@code null}. If you want more
   * than one title on a chart, use the {@link #addSubtitle(Title)} method.
   *
  ...
  /**
   * Sets the chart title and sends a {@link ChartChangeEvent} to all registered listeners. This is
   * a convenience method that ends up calling the {@link #setTitle(TextTitle)} method. If there is
   * an existing title, its text is updated, otherwise a new title using the default font is added
   * to the chart. If {@code text} is {@code null} the chart title is set to {@code null}.
   *
  ...
  /**
   * Adds a legend to the plot and sends a {@link ChartChangeEvent} to all registered listeners.
   *
  ...
  /**
   * Returns the legend for the chart, if there is one. Note that a chart can have more than one
   * legend - this method returns the first.
   *
  ...
  /**
   * Removes the first legend in the chart and sends a {@link ChartChangeEvent} to all registered
   * listeners.
   *
   * @see #getLegend()
   */
  ...
  /**
   * Sets the title list for the chart (completely replaces any existing titles) and sends a {@link
   * ChartChangeEvent} to all registered listeners.
   *
   * @param subtitles the new list of subtitles ({@code null} not permitted).
   * @see #getSubtitles()
   */
  ...
  /**
   * Adds a subtitle at a particular position in the subtitle list, and sends a {@link
   * ChartChangeEvent} to all registered listeners.
   *
  ...
  /**
   * Clears all subtitles from the chart and sends a {@link ChartChangeEvent} to all registered
   * listeners.
   *
   * @see #addSubtitle(Title)
   */
  ...
  /**
   * Removes the specified subtitle and sends a {@link ChartChangeEvent} to all registered
   * listeners.
   *
   ...
   *
   * <p>Anti-aliasing usually improves the appearance of charts, but is slower.
   *
  ...
  /**
   * Returns the current value stored in the rendering hints table for {@link
  ...
  /**
   * Sets the value in the rendering hints table for {@link RenderingHints#KEY_TEXT_ANTIALIASING} to
   * either {@link RenderingHints#VALUE_TEXT_ANTIALIAS_ON} or {@link
   * RenderingHints#VALUE_TEXT_ANTIALIAS_OFF}, then sends a {@link ChartChangeEvent} to all
  ...
  /**
   * Sets the value in the rendering hints table for {@link RenderingHints#KEY_TEXT_ANTIALIASING}
  ...
  /**
   * Sets the paint used to fill the chart background and sends a {@link ChartChangeEvent} to all
   * registered listeners.
   *
  ...
  /**
   * Returns the background image for the chart, or {@code null} if there is no image.
   *
  ...
  /**
   * Sets the background image for the chart and sends a {@link ChartChangeEvent} to all registered
   * listeners.
   *
  ...
  /**
   * Sets the alpha-transparency for the chart's background image. Registered listeners are notified
   ...
   *
   * <p>This method is the focus of the entire JFreeChart library.
   *
  ...
  public void draw(Graphics2D g2, Rectangle2D area) {
    draw(g2, area, null, g2);
  }

  /**
   * Draws the chart on a Java 2D graphics device (such as the screen or a printer). This method is
   * the focus of the entire JFreeChart library.
   *
   * @param g2 the graphics device.
   * @param area the area within which the chart should be drawn.
  ...
  public void draw(Graphics2D g2, Rectangle2D chartArea, Point2D anchor, ChartRenderingInfo info) {
    ...
    notifyListeners(
        new ChartProgressEvent(this, this, ChartProgressEventType.DRAWING_FINISHED, 100));
  }
   ...
   * @return A rectangle.
   */
  private Rectangle2D createAlignedRectangle2D(
      Size2D dimensions, Rectangle2D frame, HorizontalAlignment hAlign, VerticalAlignment vAlign) {
  ...
  /**
   * Draws a title. The title should be drawn at the top, bottom, left or right of the specified
   * area, and the area should be updated to reflect the amount of space used by the title.
   *
   * @param t the title ({@code null} not permitted).
   * @param g2 the graphics device ({@code null} not permitted).
   * @param area the chart area, excluding any existing titles ({@code null} not permitted).
   * @param entities a flag that controls whether or not an entity collection is returned for the
   *     title.
      ...
      case LEFT:
        {
          ...
          retValue = t.draw(g2, titleArea, p);
          area.setRect(
              area.getX() + size.width,
              area.getY(),
              area.getWidth() - size.width,
              area.getHeight());
          break;
        }
      default:
   ...
   * @param width the width.
   * @param height the height.
   * @param info carries back chart state information ({@code null} permitted).
   * @return A buffered image.
   */
  ...
  public BufferedImage createBufferedImage(
      int width, int height, int imageType, ChartRenderingInfo info) {
  ...
  /**
   * Handles a 'click' on the chart. JFreeChart is not a UI component, so some other object (for
   * example, {@link ChartPanel}) needs to capture the click event and pass it onto the JFreeChart
   * object. If you are not using JFreeChart in a client application, then this method is not
   * required.
   *
   * @param x x-coordinate of the click (in Java2D space).
   * @param y y-coordinate of the click (in Java2D space).
   * @param info contains chart dimension and entity information ({@code null} not permitted).
   */
   ...
   *
   * <p>This method is for convenience only.
   */
  ...
  /**
   * Clones the object, and takes care of listeners. Note: caller shall register its own listeners
   * on cloned graph.
   *
   * @return A clone.
