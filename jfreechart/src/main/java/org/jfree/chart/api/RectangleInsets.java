...
public class RectangleInsets implements Serializable {
  ...
  /**
   * Returns the unit type (absolute or relative). This specifies whether the insets are measured as
   * Java2D units or percentages.
   *
  ...
  /**
   * Creates an adjusted rectangle using the supplied rectangle, the insets specified by this
   * instance, and the horizontal and vertical adjustment types.
   *
   * @param base the base rectangle ({@code null} not permitted).
   * @param horizontal the horizontal adjustment type ({@code null} not permitted).
   * @param vertical the vertical adjustment type ({@code null} not permitted).
   * @return The inset rectangle.
   */
  public Rectangle2D createAdjustedRectangle(
      Rectangle2D base, LengthAdjustmentType horizontal, LengthAdjustmentType vertical) {
  ...
  public double trimWidth(double width) {
    return ((double) (width - calculateLeftInset(width)) - calculateRightInset(width));
  }
