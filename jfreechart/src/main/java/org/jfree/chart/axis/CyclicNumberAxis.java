 ...
 *
 * <p>Traditional representation of data in the range x0..x1
 *
 * <pre>
 * |-------------------------|
 * x0                       x1
 * </pre>
 *
 * Here, the range bounds are at the axis extremities. With cyclic axis, however, the time is split
 * in "cycles", or "time frames", or the same duration : the period.
 *
 * <p>A cycle axis cannot by definition handle a larger interval than the period :
 *
 * <pre>x1 - x0 &gt;= period</pre>
 *
 * . Thus, at most a full period can be represented with such an axis.
 *
 * <p>The cycle bound is the number between x0 and x1 which marks the beginning of new time frame:
 *
 * <pre>
 * |---------------------|----------------------------|
 * x0                   cb                           x1
 ...
 *
 * Thus, by definition, two consecutive cycle bounds period apart, which is precisely why it is
 * called a period.
 *
 * <p>The visual representation of a cyclic axis is like that:
 *
 * <pre>
 * |----------------------------|---------------------|
 * cb                         x1|x0                  cb
 ...
 * </pre>
 *
 * The cycle bound is at the axis ends, then current cycle is shown, then the last cycle. When using
 * dynamic data, the visual effect is the current cycle erases the last cycle as x grows. Then, the
 * next cycle bound is reached, and the process starts over, erasing the previous cycle.
 *
 * <p>A Cyclic item renderer is provided to do exactly this.
 */
public class CyclicNumberAxis extends NumberAxis {
  ...
  /**
   * The advance line is the line drawn at the limit of the current cycle, when erasing the previous
   * cycle.
   *
   * @return A boolean.
   */
  ...
  /**
   * The cycle bound can be associated either with the current or with the last cycle. It's up to
   * the user's choice to decide which, as this is just a convention. By default, the cycle bound is
   * mapped to the current cycle. <br>
   ...
   *
   * @return {@code true} if the cycle bound is mapped to the last cycle, {@code false} if it is
   ...
   * mapped to the current cycle. <br>
   * Note that this has no effect on visual appearance, as the cycle bound is mapped successively
   * for both axis ends. Use this function for correct results in valueToJava2D.
   *
   ...
   * This is especially useful for having a tick at each axis end with the cycle bound value. See
   * also isBoundMappedToLastCycle()
   */
   ...
   */
  @Override
  protected float[] calculateAnchorPoint(
      ValueTick tick, double cursor, Rectangle2D dataArea, RectangleEdge edge) {
  ...
  /**
   * This function is nearly useless since the auto range is fixed for this class to the period. The
   * period is extended if necessary to fit the minimum size.
   *
   * @param size the size.
   * @param notify notify?
  ...
  /**
   * The auto range is fixed for this class to the period by default. This function will thus set a
   * new period.
   *
   ...
   * and integer and x &lt; range.getUpperBound() This is the value which is at both ends of the
   * axis : x...up|low...x The values from x to up are the valued in the current cycle. The values
   * from low to x are the valued in the previous cycle.
   *
   * @return The cycle bound.
   */
  public double getCycleBound() {
    return Math.floor((getRange().getUpperBound() - this.offset) / this.period) * this.period
        + this.offset;
  }
