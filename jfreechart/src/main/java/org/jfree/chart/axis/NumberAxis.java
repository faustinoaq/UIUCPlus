 ...
 *
 * <p>If the axis is set up to automatically determine its range to fit the data, you can ensure
 * that the range includes zero (statisticians usually prefer this) by setting the {@code
 * autoRangeIncludesZero} flag to {@code true}.
 *
 * <p>The {@code NumberAxis} class has a mechanism for automatically selecting a tick unit that is
 * appropriate for the current axis range.
 */
public class NumberAxis extends ValueAxis implements Cloneable, Serializable {
  ...
  /**
   * The range type (can be used to force the axis to display only positive values or only negative
   * values).
   */
   ...
   * determined automatically. If the value 0 falls within the margin and this flag is TRUE, then
   * the margin is truncated at zero.
   */
   ...
   *
   * <p>If the flag is changed to {@code true}, the axis range is recalculated.
   *
   * <p>Any change to the flag will trigger an {@link AxisChangeEvent}.
   *
  ...
  /**
   * Returns a flag that affects the auto-range when zero falls outside the data range but inside
   * the margins defined for the axis.
   *
   * @return The flag.
  ...
  /**
   * Sets a flag that affects the auto-range when zero falls outside the data range but inside the
   * margins defined for the axis.
   *
   ...
   *
   * <p>Note: if the {@code autoTickUnitSelection} flag is {@code true} the tick unit may be changed
   * while the axis is being drawn, so in that case the return value from this method may be
  ...
  public NumberTickUnit getTickUnit() {
    NumberTickUnit tickUnit = new NumberTickUnit(0.0);
    return tickUnit;
  }

  /**
   * Sets the tick unit for the axis and sends an {@link AxisChangeEvent} to all registered
   * listeners. A side effect of calling this method is that the "auto-select" feature for tick
   * units is switched off (you can restore it using the {@link
   ...
   * registered listeners. In addition, an option is provided to turn off the "auto-select" feature
   * for tick units (you can restore it using the {@link
   * ValueAxis#setAutoTickUnitSelection(boolean)} method).
   *
   * @param unit the new tick unit ({@code null} not permitted).
  ...
  /**
   * Returns the number format override. If this is non-null, then it will be used to format the
   * numbers on the axis.
   *
  ...
  /**
   * Sets the number format override. If this is non-null, then it will be used to format the
   * numbers on the axis.
   *
   ...
   *
   * <p>The marker band is optional, leave it set to {@code null} if you don't require it.
   *
   ...
   *
   * <p>Note that it is possible for the coordinate to fall outside the plotArea.
   *
   ...
   * @param plotArea the area within which the axes and data should be drawn ({@code null} not
   *     permitted).
   ...
   *
   * <p>If you don't like these defaults, create your own instance of TickUnits and then pass it to
  ...
  /**
   * Creates a collection of standard tick units. The supplied locale is used to create the number
   * formatter (a localised instance of {@code NumberFormat}).
   *
   * <p>If you don't like these defaults, create your own instance of {@link TickUnits} and then
   * pass it to the {@code setStandardTickUnits()} method.
   *
  ...
  /**
   * Returns a collection of tick units for integer values. Uses a given Locale to create the
   * DecimalFormats.
   *
   ...
   *
   * <p>Rather than computing the string bounds of every tick on the axis, we just look at two
   * values: the lower bound and the upper bound for the axis. These two values will usually be
   * representative.
   *
   * @param g2 the graphics device.
