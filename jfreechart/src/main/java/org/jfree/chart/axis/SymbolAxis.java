...
public class SymbolAxis extends NumberAxis implements Serializable {
  ...
  /**
   * Returns the flag that controls whether or not grid bands are drawn for the axis. The default
   * value is {@code true}.
   *
   * @return A boolean.
  ...
  /**
   * Sets the flag that controls whether or not grid bands are drawn for this axis and notifies
   * registered listeners that the axis has been modified. Each band is the area between two
   * adjacent gridlines running perpendicular to the axis. When the bands are drawn they are filled
   * with the colors {@link #getGridBandPaint()} and {@link #getGridBandAlternatePaint()} in an
  ...
  /**
   * Returns the paint used to color grid bands (two colors are used alternately, the other is
   * returned by {@link #getGridBandAlternatePaint()}). The default value is {@link
  ...
  public Paint getGridBandPaint() {
    return this.gridBandPaint != null ? this.gridBandPaint : null;
  }

  /**
   * Sets the grid band paint and notifies registered listeners that the axis has been changed. See
   * the {@link #setGridBandsVisible(boolean)} method for more information about grid bands.
   *
  ...
  /**
   * Returns the second paint used to color grid bands (two colors are used alternately, the other
   * is returned by {@link #getGridBandPaint()}). The default value is {@link
   * #DEFAULT_GRID_BAND_ALTERNATE_PAINT} (transparent).
   *
   * @return The paint (never {@code null}).
   ...
   * @param plotArea the area within which the plot and axes should be drawn ({@code null} not
   *     permitted).
  ...
  /**
   * Draws the grid bands (alternate bands are colored using {@link #getGridBandPaint()} and {@link
   * #getGridBandAlternatePaint()}.
   *
   * @param g2 the graphics target ({@code null} not permitted).
   * @param plotArea the area within which the plot is drawn ({@code null} not permitted).
   * @param dataArea the data area to which the axes are aligned ({@code null} not permitted).
   * @param edge the edge to which the axis is aligned ({@code null} not permitted).
   * @param ticks the ticks ({@code null} not permitted).
   */
  protected void drawGridBands(
      Graphics2D g2, Rectangle2D plotArea, Rectangle2D dataArea, RectangleEdge edge, List ticks) {
   ...
   * @param dataArea the area for the data (to which the axes are aligned, {@code null} not
   *     permitted).
   * @param firstGridBandIsDark True: the first grid band takes the color of {@code gridBandPaint}.
   *     False: the second grid band takes the color of {@code gridBandPaint}.
  ...
  /**
   * Draws the grid bands for an axis that is aligned to the left or right of the data area (that
   * is, a vertical axis).
   *
   * @param g2 the graphics target ({@code null} not permitted).
   * @param plotArea the area within which the plot is drawn (not used here).
  ...
  protected void drawGridBandsVertical(
    ...
    while (iterator.hasNext()) {
      ...
      band =
          new Rectangle2D.Double(
              xx + outlineStrokeWidth,
              Math.min(yy1, yy2),
              dataArea.getMaxX() - xx - outlineStrokeWidth,
              Math.abs(yy2 - yy1));
      g2.fill(band);
      currentGridBandIsDark = !currentGridBandIsDark;
    }
