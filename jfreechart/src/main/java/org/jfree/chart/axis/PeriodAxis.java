...
/**
 * An axis that displays a date scale based on a {@link org.jfree.data.time.RegularTimePeriod}. This
 * axis works when displayed across the bottom or top of a plot, but is broken for display at the
 * left or right of charts.
 */
public class PeriodAxis extends ValueAxis implements Cloneable, PublicCloneable, Serializable {
  ...
  /**
   * Indicates the {@link RegularTimePeriod} subclass that is used to determine the spacing of the
   * major tick marks.
   */
   ...
   * @param first the first time period in the axis range ({@code null} not permitted).
   * @param last the last time period in the axis range ({@code null} not permitted).
   */
  ...
  /**
   * Sets the first time period in the axis range and sends an {@link AxisChangeEvent} to all
  ...
  /**
   * Sets the last time period in the axis range and sends an {@link AxisChangeEvent} to all
   * registered listeners.
   *
  ...
  /**
   * Returns the class used to create the first and last time periods for the axis range when the
   * auto-range flag is set to {@code true}.
   *
  ...
  /**
   * Sets the class used to create the first and last time periods for the axis range when the
   * auto-range flag is set to {@code true} and sends an {@link AxisChangeEvent} to all registered
   * listeners.
   *
  ...
  /**
   * Sets the class that controls the spacing of the major tick marks, and sends an {@link
  ...
  /**
   * Sets the flag that controls whether or not minor tick marks are displayed for the axis, and
  ...
  /**
   * Sets the class that controls the spacing of the minor tick marks, and sends an {@link
   * AxisChangeEvent} to all registered listeners.
   *
   * @param c the class (a subclass of {@link RegularTimePeriod} is expected).
   */
  ...
  /**
   * Sets the stroke used to display minor tick marks, if they are visible, and sends a {@link
   * AxisChangeEvent} to all registered listeners.
   *
  ...
  /**
   * Sets the paint used to display minor tick marks, if they are visible, and sends a {@link
   * AxisChangeEvent} to all registered listeners.
   *
  ...
  /**
   * Sets the array of label info records and sends an {@link AxisChangeEvent} to all registered
   * listeners.
   *
   * @param info the info.
   */
   ...
   * listeners. As a side-effect, the auto-range flag is set to {@code false} (optional).
   *
  ...
  public AxisState draw(
    ...
    if (getAttributedLabel() != null) {
      axisState =
          drawAttributedLabel(getAttributedLabel(), g2, plotArea, dataArea, edge, axisState);
    } else {
   ...
   * @param edge the edge.
   */
  protected void drawTickMarksVertical(
      Graphics2D g2, AxisState state, Rectangle2D dataArea, RectangleEdge edge) {
  ...
  public double valueToJava2D(double value, Rectangle2D area, RectangleEdge edge) {
    ...
    if (RectangleEdge.isTopOrBottom(edge)) {
      ...
      if (isInverted()) {
        ...
        result = ((value - axisMin) / (axisMax - axisMin));
      }
    }
    return result;
  }
   ...
   */
  private RegularTimePeriod createInstance(
      Class periodClass, Date millisecond, TimeZone zone, Locale locale) {
    RegularTimePeriod result = null;
    try {
      Constructor c =
          periodClass.getDeclaredConstructor(
