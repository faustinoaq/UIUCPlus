...
public class CategoryAxis extends Axis implements Cloneable, Serializable {
  ...
  /**
   * Sets the category margin and sends an {@link AxisChangeEvent} to all registered listeners. The
   * overall category margin is distributed over N-1 gaps, where N is the number of categories on
   * the axis.
   *
   * @param margin the margin as a percentage of the axis length (for example, 0.05 is five
   *     percent).
  ...
  /**
   * Sets the maximum number of lines to use for each category label and sends an {@link
  ...
  /**
   * Sets the maximum category label width ratio and sends an {@link AxisChangeEvent} to all
  ...
  /**
   * Returns the offset between the axis and the category labels (before label positioning is taken
   * into account).
   *
  ...
  /**
   * Sets the offset between the axis and the category labels (before label positioning is taken
   * into account) and sends a change event to all registered listeners.
   *
  ...
  /**
   * Returns the category label position specification (this contains label positioning info for all
   * four possible axis locations).
   *
  ...
  /**
   * Sets the category label position specification for the axis and sends an {@link
   * AxisChangeEvent} to all registered listeners.
   *
  ...
  /**
   * Sets the font for the tick label for the specified category and sends an {@link
   * AxisChangeEvent} to all registered listeners.
   *
   * @param category the category ({@code null} not permitted).
  ...
  /**
   * Sets the paint for the tick label for the specified category and sends an {@link
   * AxisChangeEvent} to all registered listeners.
   *
   * @param category the category ({@code null} not permitted).
  ...
  /**
   * Adds a tooltip to the specified category and sends an {@link AxisChangeEvent} to all registered
   * listeners.
   *
   * @param category the category ({@code null} not permitted).
  ...
  /**
   * Removes the tooltip for the specified category and, if there was a value associated with that
   * category, sends an {@link AxisChangeEvent} to all registered listeners.
   *
   * @param category the category ({@code null} not permitted).
   * @see #addCategoryLabelToolTip(Comparable, String)
  ...
  /**
   * Adds a URL (to be used in image maps) to the specified category and sends an {@link
   * AxisChangeEvent} to all registered listeners.
   *
   * @param category the category ({@code null} not permitted).
  ...
  /**
   * Removes the URL for the specified category and, if there was a URL associated with that
   * category, sends an {@link AxisChangeEvent} to all registered listeners.
   *
   * @param category the category ({@code null} not permitted).
   * @see #addCategoryLabelURL(Comparable, String)
  ...
  /**
   * Clears the category label URLs and sends an {@link AxisChangeEvent} to all registered
   * listeners.
   *
   * @see #addCategoryLabelURL(Comparable, String)
   * @see #removeCategoryLabelURL(Comparable)
   ...
   */
  public double getCategoryJava2DCoordinate(
   ...
   * @param edge the edge along which the axis lies ({@code null} not permitted).
   * @return The centre coordinate.
   * @see #getCategorySeriesMiddle(Comparable, Comparable, CategoryDataset, double, Rectangle2D,
   *     RectangleEdge)
   */
  public double getCategoryMiddle(
      Comparable category, List categories, Rectangle2D area, RectangleEdge edge) {
   ...
   * @param plotArea the area within which the axis should be drawn ({@code null} not permitted).
   * @param dataArea the area within which the plot is being drawn ({@code null} not permitted).
    ...
    if (categories != null) {
      ...
      for (Object o : categories) {
        ...
        if (edge == RectangleEdge.TOP || edge == RectangleEdge.BOTTOM) {
          max =
              Math.max(
                  max, calculateCategoryLabelHeight(label, position, getTickLabelInsets(), g2));
        } else if (edge == RectangleEdge.LEFT || edge == RectangleEdge.RIGHT) {
          max =
              Math.max(max, calculateCategoryLabelWidth(label, position, getTickLabelInsets(), g2));
        }
        Tick tick =
            new CategoryTick(
                category,
   ...
   * @return The width.
   */
  protected double calculateCategoryLabelWidth(
      TextBlock label, CategoryLabelPosition position, RectangleInsets insets, Graphics2D g2) {
   ...
   * @return The height.
   */
  protected double calculateCategoryLabelHeight(
      TextBlock block, CategoryLabelPosition position, RectangleInsets insets, Graphics2D g2) {
  ...
  /**
   * Reads a {@code Map} of ({@code Comparable}, {@code Paint}) elements from a stream.
   *
  ...
  /**
   * Writes a map of ({@code Comparable}, {@code Paint}) elements to a stream.
   *
