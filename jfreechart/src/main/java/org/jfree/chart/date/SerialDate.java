...
/**
 * An abstract class that defines our requirements for manipulating dates, without tying down a
 * particular implementation.
 *
 * <p>Requirement 1 : match at least what Excel does for dates; Requirement 2 : the date represented
 * by the class is immutable;
 *
 * <p>Why not just use java.util.Date? We will, when it makes sense. At times, java.util.Date can be
 * *too* precise - it represents an instant in time, accurate to 1/1000th of a second (with the date
 * itself depending on the time-zone). Sometimes we just want to represent a particular day (e.g. 21
 * January 2015) without concerning ourselves about the time of day, or the time-zone, or anything
 * else. That's what we've defined SerialDate for.
 *
 * <p>You can call getInstance() to get a concrete subclass of SerialDate, without worrying about
...
public abstract class SerialDate implements Comparable, Serializable, MonthConstants {
  ...
  /**
   * Returns {@code true} if the supplied integer code represents a valid day-of-the-week, and
   * {@code false} otherwise.
   *
   * @param code the code being checked for validity.
   * @return {@code true} if the supplied integer code represents a valid day-of-the-week, and
   *     {@code false} otherwise.
   */
   ...
   * @return {@code -1} if the string is not convertable, the day of the week otherwise.
   */
  public static int stringToWeekdayCode(String s) {
    final String[] shortWeekdayNames = null; // buggy
   ...
   *
   * @param code the code being checked for validity.
   * @return {@code true} if the supplied integer code represents a valid month.
   */
   ...
   * @return {@code -1} if the string is not parseable, the month of the year otherwise.
   */
  ...
  /**
   * Returns true if the supplied integer code represents a valid week-in-the-month, and false
   * otherwise.
   *
   * @param code the code being checked for validity.
   * @return {@code true} if the supplied integer code represents a valid week-in-the-month.
   */
  ...
  /**
   * Returns the latest date that falls on the specified day-of-the-week and is BEFORE the base
   * date.
   *
  ...
  /**
   * Returns the serial number for the date, where 1 January 1900 = 2 (this corresponds, almost, to
   * the numbering system used in Microsoft Excel for Windows and Lotus 1-2-3).
   *
  ...
  /**
   * Returns a java.util.Date. Since java.util.Date has more precision than SerialDate, we need to
   * define a convention for the 'time of day'.
   *
  ...
  /**
   * Returns the description that is attached to the date. It is not required that a date have a
   * description, but for some applications it is useful.
   *
   ...
   *
   * @param description the description for this date ({@code null} permitted).
   */
   ...
   *
   * <p>The result is positive if this date is after the 'other' date and negative if it is before
   * the 'other' date.
   *
   ...
   *
   * @param other the date being compared to.
   * @return {@code true} if this SerialDate represents the same date as the specified SerialDate.
   */
   ...
   *
   * @param other The date being compared to.
   * @return {@code true} if this SerialDate represents an earlier date compared to the specified
   *     SerialDate.
   */
  ...
  /**
   * Returns {@code true} if this {@link SerialDate} is within the specified range (INCLUSIVE). The
   ...
   * whether or not the end-points are included). The date order of d1 and d2 is not important.
   *
   * @param d1 a boundary date for the range.
   * @param d2 the other boundary date for the range.
   * @param include a code that controls whether or not the start and end dates are included in the
   *     range.
   * @return A boolean.
   */
