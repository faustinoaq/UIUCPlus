 ...
 *
 * <p>For the label format, use {0} where the pie section key should be inserted, {1} for the
 * absolute section value and {2} for the percent amount of the pie section, e.g. {@code "{0} = {1}
 * ({2})"} will display as {@code apple = 120 (5%)}.
 */
public class StandardPieSectionLabelGenerator extends AbstractPieItemLabelGenerator
  ...
  /**
   * An optional map between item indices (Integer) and attributed labels (instances of
   * AttributedString).
   */
  private Map attributedLabels;

  /**
   * Creates a new section label generator using {@link #DEFAULT_SECTION_LABEL_FORMAT} as the label
   * format string, and platform default number and percentage formatters.
   */
  ...
  /**
   * Creates a new section label generator using the specified label format string, and platform
   * default number and percentage formatters.
   *
   ...
   *
   * @param labelFormat the label format string ({@code null} not permitted).
   * @param numberFormat the format object for the values ({@code null} not permitted).
   * @param percentFormat the format object for the percentages ({@code null} not permitted).
   */
  public StandardPieSectionLabelGenerator(
      String labelFormat, NumberFormat numberFormat, NumberFormat percentFormat) {
    super(labelFormat, numberFormat, percentFormat);
    this.attributedLabels = new HashMap();
  }

  /**
   * Returns the attributed label for a section, or {@code null} if none is defined.
   *
   * @param section the section index.
   * @return The attributed label.
   */
  public AttributedString getAttributedLabel(int section) {
    return (AttributedString) this.attributedLabels.get(section - 1);
  }
  ...
  /**
   * Generates an attributed label for the specified series, or {@code null} if no attributed label
   * is available (in which case, the string returned by {@link #generateSectionLabel(PieDataset,
   * Comparable)} will provide the fallback). Only certain attributes are recognised by the code
   * that ultimately displays the labels:
   *
   * <ul>
