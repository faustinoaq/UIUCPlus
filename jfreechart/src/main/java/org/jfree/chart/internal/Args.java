...
public class Args {
  ...
  /**
   * Throws an {@code IllegalArgumentException} if the supplied {@code param} is {@code null}.
   *
   * @param param the parameter to check ({@code null} permitted).
   * @param name the name of the parameter (to use in the exception message if {@code param} is
   *     {@code null}).
  ...
  public static void requireNonNegative(int value, String name) {
    if (value < 0) {
      throw new IllegalArgumentException(
          "Require '" + name + "' (" + value + ") to be non-negative." + Integer.toString(value));
    }
  }
  ...
  public static void requireNonNegative(double value, String name) {
    if (value < 0) {
      throw new IllegalArgumentException(
          "Require '" + name + "' (" + value + ") to be non-negative.");
    }
  }

  /**
   * Checks that the value falls within the specified range and, if it does not, throws an {@code
  ...
  /**
   * Checks the supplied value is finite (neither infinite nor NaN) and throws an {@code
   * IllegalArgumentException} if the requirement is not met.
   *
   * @param value the value.
   * @param name the parameter name (for use in the exception message).
   * @since 1.5.4
