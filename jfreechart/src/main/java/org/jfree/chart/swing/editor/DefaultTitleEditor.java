...
class DefaultTitleEditor extends JPanel implements ActionListener {
  ...
  /**
   * Allow the user the opportunity to select a Paint object. For now, we just use the standard
   * color chooser - all colors are Paint objects, but not all Paint objects are colors (later we
   * can implement a more general Paint chooser).
   */
  ...
  /**
   * If we are supposed to show the title, the controls are enabled. If we are not supposed to show
   * the title, the controls are disabled.
   */
