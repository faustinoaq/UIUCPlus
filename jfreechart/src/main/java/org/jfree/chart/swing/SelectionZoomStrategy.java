...
/**
 * A strategy for zooming plots on the chart panel by selecting a smaller region on the initial
 * screen.
 *
 * <p>See {@link ChartPanel#setSelectionZoomStrategy(SelectionZoomStrategy)}
 */
public interface SelectionZoomStrategy extends Serializable {
   ...
   *
   * @return {@code true} if zoomRectangle exists for this controller and {@code false} otherwise
   */
  boolean isActivated();

  /**
   * The zoom rectangle starting point (selected by the user with a mouse click). This is a point on
   * the screen, not the chart (which may have been scaled up or down to fit the panel).
   */
   ...
   */
  void updateZoomRectangleSelection(
      MouseEvent e, boolean hZoom, boolean vZoom, Rectangle2D scaledDataArea);
