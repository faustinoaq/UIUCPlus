...
/**
 * An overlay for a {@link ChartPanel} that draws crosshairs on a chart. If you are using the JavaFX
 * extensions for JFreeChart, then you should use the {@code CrosshairOverlayFX} class.
 */
public class CrosshairOverlay extends AbstractOverlay
  ...
  /**
   * Adds a crosshair against the domain axis (x-axis) and sends an {@link OverlayChangeEvent} to
  ...
  /**
   * Removes a domain axis crosshair and sends an {@link OverlayChangeEvent} to all registered
   * listeners.
   *
   * @param crosshair the crosshair ({@code null} not permitted).
   ...
   * all registered listeners (unless there were no crosshairs to begin with).
   */
  ...
  /**
   * Adds a crosshair against the range axis and sends an {@link OverlayChangeEvent} to all
  ...
  /**
   * Removes a range axis crosshair and sends an {@link OverlayChangeEvent} to all registered
   * listeners.
   *
   * @param crosshair the crosshair ({@code null} not permitted).
   * @see #addRangeCrosshair(org.jfree.chart.plot.Crosshair)
  ...
  /**
   * Clears all the range crosshairs from the overlay and sends an {@link OverlayChangeEvent} to all
   * registered listeners (unless there were no crosshairs to begin with).
   */
  public void clearRangeCrosshairs() {
    ...
    for (Crosshair c : getRangeCrosshairs()) {
      ...
      c.removePropertyChangeListener(this);
      // create and remove a Crosshair in the loop
      Crosshair temp = new Crosshair(0);
      this.yCrosshairs.add(temp);
      this.yCrosshairs.remove(temp);
    }
    fireOverlayChanged();
  }
  ...
  /**
   * Renders the crosshairs in the overlay on top of the chart that has just been rendered in the
   * specified {@code chartPanel}. This method is called by the JFreeChart framework, you won't
   * normally call it from user code.
   *
   ...
   */
  protected void drawHorizontalCrosshair(
      Graphics2D g2, Rectangle2D dataArea, double y, Crosshair crosshair) {
    ...
    if (y >= dataArea.getMinY() && y <= dataArea.getMaxY()) {
      ...
      if (crosshair.isLabelVisible()) {
        ...
        if (label != null && !label.isEmpty()) {
          ...
          Point2D pt =
              calculateLabelPoint(
                  line, anchor, crosshair.getLabelXOffset(), crosshair.getLabelYOffset());
   ...
   * @param crosshair the crosshair.
   */
  protected void drawVerticalCrosshair(
      Graphics2D g2, Rectangle2D dataArea, double x, Crosshair crosshair) {
   ...
   */
  private Point2D calculateLabelPoint(
      Line2D line, RectangleAnchor anchor, double deltaX, double deltaY) {
    double x, y;
    boolean left =
