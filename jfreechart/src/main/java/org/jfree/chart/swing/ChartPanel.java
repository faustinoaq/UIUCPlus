 ...
 *
 * <p>The panel registers with the chart to receive notification of changes to any component of the
...
public class ChartPanel extends JPanel
  ...
  /**
   * Default setting for buffer usage. The default has been changed to {@code true} from version
   * 1.0.13 onwards, because of a severe performance problem with drawing the zoom rectangle using
   * XOR (which now happens only when the buffer is NOT used).
   */
  ...
  /**
   * The standard mouse button modifiers for alternative drag operations. There are two kinds of
   * mouse drag operations: pan and zoom. To distinguish between them, one needs to require modifier
   * keys to be held down during the dragging. However, some modifiers may not be usable on all
   * platforms. For example, on Mac OS X it is impossible to perform Ctrl-drags or right-drags, see
   * <a
   * href="http://developer.apple.com/qa/qa2004/qa1362.html">http://developer.apple.com/qa/qa2004/qa1362.html</a>.
   * This function returns a non-zero modifier usable for any platform: Alt for Mac OS X, Ctrl for
   * other platforms. It is recommended to use these modifiers for one operation, and zero modifiers
   * for the other.
   *
  ...
  /**
   * The default mask for mouse events to trigger panning. Since 2.0.0, this mask uses extended
   * modifiers, as returned by {@link InputEvent#getModifiersEx()}. Only used if no button-specific
   * modifiers were set in {@link #panButtonMasks}.
   */
  ...
  /**
   * Constructs a panel containing a chart. The {@code useBuffer} flag controls whether or not an
   * offscreen {@code BufferedImage} is maintained for the chart. If the buffer is used, more memory
   * is consumed, but panel repaints will be a lot quicker in cases where the chart itself hasn't
   * changed (for example, when another frame is moved to reveal the panel). WARNING: If you set the
   * {@code useBuffer} flag to false, note that the mouse zooming rectangle will (in that case) be
   * drawn using XOR, and there is a SEVERE performance problem with that on JRE6 on Windows.
   *
   * @param chart the chart.
   * @param useBuffer a flag controlling whether or not an off-screen buffer is used (read the
   *     warning above before setting this to {@code false}).
   */
   ...
   *
   * <p>If the width available on the panel is less than this, then the chart is drawn at the
   * minimum width then scaled down to fit.
   *
   ...
   *
   * <p>At the time the chart is drawn on the panel, if the available width is less than this
   * amount, the chart will be drawn using the minimum width then scaled down to fit the available
   * space.
   *
   ...
   *
   * <p>If the width available on the panel is greater than this, then the chart is drawn at the
   ...
   *
   * <p>At the time the chart is drawn on the panel, if the available width is greater than this
   ...
   *
   * <p>If the height available on the panel is less than this, then the chart is drawn at the
   ...
   *
   * <p>At the time the chart is drawn on the panel, if the available height is less than this
   * amount, the chart will be drawn using the minimum height then scaled down to fit the available
   * space.
   *
   ...
   *
   * <p>If the height available on the panel is greater than this, then the chart is drawn at the
   ...
   *
   * <p>At the time the chart is drawn on the panel, if the available height is greater than this
   ...
   *
   * @param flag {@code true} enables zooming and rectangle fill on zoom.
   */
   ...
   * @param fillRectangle {@code true} if zoom rectangle is filled, false if rectangle is shown as
   *     outline only.
   */
  ...
  /**
   * Sets default modifier keys for pan operations for all mouse buttons. Modifiers for a specific
   * button can be set with {@link #setPanModifiersEx(int, int)}. If there are none set for a
   * certain button, it will use the modifiers passed to this function, defaulting to {@link
   * #getDefaultDragModifiersEx()} if this function was never called.
   *
   * <p>Only {@link InputEvent#SHIFT_DOWN_MASK}, {@link InputEvent#CTRL_DOWN_MASK}, {@link
   * InputEvent#META_DOWN_MASK} and {@link InputEvent#ALT_DOWN_MASK} are checked. To avoid
   * platform-specific problems, it is recommended to use {@link #getDefaultDragModifiersEx()} for
   * one operation, and zero modifiers for the other.
   *
   * <p>If the same modifiers are set for both zooming and panning, panning will be performed.
   *
  ...
  /**
   * Sets default modifier keys for zoom operations for all mouse buttons. Modifiers for a specific
   * button can be set with {@link #setZoomModifiersEx(int, int)}. If there are none set for a
   * certain button, it will use the modifiers passed to this function, defaulting to zero (no
   * modifiers) if this function was never called.
   *
   * <p>Only {@link InputEvent#SHIFT_DOWN_MASK}, {@link InputEvent#CTRL_DOWN_MASK}, {@link
   * InputEvent#META_DOWN_MASK} and {@link InputEvent#ALT_DOWN_MASK} are checked. To avoid
   * platform-specific problems, it is recommended to use {@link #getDefaultDragModifiersEx()} for
  ...
  /**
   * Sets modifier keys for panning with a specific mouse button. If there are none set for a
   * certain button with this function, default modifiers set with {@link
   * #setDefaultPanModifiersEx(int)} will be used, defaulting to {@link
   * #getDefaultDragModifiersEx()} if none were set either.
   *
   * <p>Only {@link InputEvent#SHIFT_DOWN_MASK}, {@link InputEvent#CTRL_DOWN_MASK}, {@link
   * InputEvent#META_DOWN_MASK} and {@link InputEvent#ALT_DOWN_MASK} are checked. To avoid
   * platform-specific problems, it is recommended to use {@link #getDefaultDragModifiersEx()} for
  ...
  /**
   * Sets modifier keys for zooming with a specific mouse button. If there are none set for a
   * certain button with this function, default modifiers set with {@link
   * #setDefaultZoomModifiersEx(int)} will be used, defaulting to zero (no modifiers) if none were
   * set either.
   *
   * <p>Only {@link InputEvent#SHIFT_DOWN_MASK}, {@link InputEvent#CTRL_DOWN_MASK}, {@link
   * InputEvent#META_DOWN_MASK} and {@link InputEvent#ALT_DOWN_MASK} are checked. To avoid
  ...
  /**
   * Sets the flag that controls whether or not zooming is enabled for the domain axis. A check is
   ...
   *
   * @param flag {@code true} instructs to fill the rectangle on zoom, otherwise it will be
   *     outlined.
   */
  ...
  /**
   * Sets the zoom trigger distance. This controls how far the mouse must move before a zoom action
   * is triggered.
   *
  ...
  /**
   * Sets the default directory for the "save as" option. If you set this to {@code null}, the
   * user's default directory will be used.
   *
  ...
  /**
   * Returns {@code true} if file extensions should be enforced, and {@code false} otherwise.
   *
   * @return The flag.
  ...
  public void setZoomFillPaint(Paint paint) {
    selectionZoomStrategy.setZoomFillPaint(null);
  }
  ...
  /**
   * Returns {@code true} if the mouse wheel handler is enabled, and {@code false} otherwise.
   *
   * @return A boolean.
   */
   ...
   *
   * @param flag {@code true} to enable tooltips, {@code false} to disable tooltips.
   */
   ...
   *
   * @param screenPoint the screen location ({@code null} not permitted).
   ...
   *
   * <p>This method will return null if there is (a) no entity at the given point, or (b) no entity
  ...
  /**
   * Sets the refresh buffer flag. This flag is used to avoid unnecessary redrawing of the chart
   * when the offscreen image buffer is used.
   *
   * @param flag {@code true} indicates that the buffer should be refreshed.
   */
    ...
    if (this.useBuffer) {
      ...
      if ((this.chartBuffer == null)
        ...
        this.chartBuffer =
            gc.createCompatibleImage(
                this.chartBufferWidth, this.chartBufferHeight, Transparency.TRANSLUCENT);
        this.refreshBuffer = true;
      }
      ...
      if (this.refreshBuffer) {
        ...
        Rectangle2D bufferArea =
            new Rectangle2D.Double(0, 0, available.getWidth(), available.getHeight());
   ...
   *
   * <p>This event is the popup trigger on Unix/Linux. For Windows, the popup trigger is the 'mouse
   * released' event.
   *
   * @param e The mouse event.
   */
  ...
  /**
   * Handles a 'mouse released' event. On Windows, we need to check if this is a popup trigger, but
   * only if we haven't already been tracking a zoom rectangle.
   *
  ...
  public void mouseReleased(MouseEvent e) {
    ...
    if (this.panLast != null) {
      ...
      boolean zoomTrigger1 =
          hZoom
              && Math.abs(e.getX() - zoomPoint.getX())
                  >= this.selectionZoomStrategy.getZoomTriggerDistance();
      boolean zoomTrigger2 =
          vZoom
              && Math.abs(e.getY() - zoomPoint.getY())
                  >= this.selectionZoomStrategy.getZoomTriggerDistance();
      if (zoomTrigger1 || zoomTrigger2) {
        if ((hZoom && (e.getX() < zoomPoint.getX())) || (vZoom && (e.getY() < zoomPoint.getY()))) {
          ...
          Rectangle2D screenDataArea =
              getScreenDataArea((int) zoomPoint.getX(), (int) zoomPoint.getY());

          Rectangle2D zoomArea =
              selectionZoomStrategy.getZoomRectangle(hZoom, vZoom, screenDataArea);
          zoom(zoomArea);
        }
  ...
  /**
   * Decreases the length of the domain axis, centered about the given coordinate on the screen. The
   * length of the domain axis is reduced by the value of {@link #getZoomInFactor()}.
   *
  ...
  /**
   * Decreases the length of the range axis, centered about the given coordinate on the screen. The
   * length of the range axis is reduced by the value of {@link #getZoomInFactor()}.
   *
   * @param x the x-coordinate (in screen coordinates).
   * @param y the y coordinate (in screen coordinates).
   */
  ...
  /**
   * Increases the length of the domain axis, centered about the given coordinate on the screen. The
   * length of the domain axis is increased by the value of {@link #getZoomOutFactor()}.
   *
   * @param x the x coordinate (in screen coordinates).
   * @param y the y-coordinate (in screen coordinates).
   */
  ...
  public void zoom(Rectangle2D selection) {
    ...
    Rectangle2D scaledDataArea =
        getScreenDataArea((int) selection.getCenterX(), (int) selection.getCenterY());
   ...
   * (that is, the area as it appears on screen).
   *
   ...
   * Opens a file chooser and gives the user an opportunity to save the chart in PNG format.
   *
   * @param w the width for the saved image (if less than or equal to zero, the panel width will be
   *     used);
   * @param h the height for the PNG image (if less than or equal to zero, the panel height will be
   *     used);
   * @throws IOException if there is an I/O error.
   */
  ...
  /**
   * Saves the chart in SVG format (a filechooser will be displayed so that the user can specify the
   * filename). Note that this method only works if the JFreeSVG library is on the classpath...if
   * this library is not present, the method will fail.
   *
   * @param f the file.
  ...
  protected void saveAsSVG(File f) throws IOException {
    ...
    if (file == null) {
      ...
      fileChooser.setCurrentDirectory(this.defaultDirectoryForSaveAs);
      FileNameExtensionFilter filter =
          new FileNameExtensionFilter(localizationResources.getString("SVG_Files"), "svg");
      fileChooser.addChoosableFileFilter(filter);
      fileChooser.setFileFilter(filter);
    ...
    if (file != null) {
      ...
      try {
        ...
        if (originalException != null) th.addSuppressed(originalException);
        throw th;
      }
    }
  }
  ...
  /**
   * Generates a string containing a rendering of the chart in SVG format. This feature is only
   * supported if the JFreeSVG library is included on the classpath.
   *
   * @param width the width.
   * @param height the height.
   * @return A string containing an SVG element for the current chart, or {@code null} if there is a
   *     problem with the method invocation by reflection.
   */
  ...
  /**
   * Creates an {@code SVGGraphics2D} instance (from JFreeSVG) using reflection. If JFreeSVG is not
   * on the classpath, this method returns {@code null}.
   *
   * @param w the width.
   * @param h the height.
  ...
  /**
   * Saves the chart in PDF format (a filechooser will be displayed so that the user can specify the
   * filename). Note that this method only works if the OrsonPDF library is on the classpath...if
   * this library is not present, the method will fail.
   *
   * @param f the file.
   */
  protected void saveAsPDF(File f) {
    ...
    if (file == null) {
      ...
      fileChooser.setCurrentDirectory(this.defaultDirectoryForSaveAs);
      FileNameExtensionFilter filter =
          new FileNameExtensionFilter(localizationResources.getString("PDF_Files"), "pdf");
      fileChooser.addChoosableFileFilter(filter);
      fileChooser.setFileFilter(filter);
  ...
  /**
   * Writes the current chart to the specified file in PDF format. This will only work when the
   * OrsonPDF library is found on the classpath. Reflection is used to ensure there is no
   * compile-time dependency on OrsonPDF (which is non-free software).
   *
   ...
   * @param pageIndex the index of the page. If not {@code 0}, nothing gets printed.
  ...
  /**
   * Creates a popup menu for the panel. This method includes code that auto-detects JFreeSVG and
   * OrsonPDF (via reflection) and, if they are present (and the {@code save} argument is {@code
   * true}, adds a menu item for each.
   *
  ...
  /**
   * The idea is to modify the zooming options depending on the type of chart being displayed by the
   * panel.
   *
