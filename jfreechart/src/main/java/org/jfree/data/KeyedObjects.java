...
public class KeyedObjects<K extends Comparable<K>>
  ...
  /**
   * Returns the object for a given key. If the key is not recognised, the method should return
   * {@code null}.
   *
   * @param key the key.
   * @return The object (possibly {@code null}).
  ...
  /**
   * Adds a new object to the collection, or overwrites an existing object. This is the same as the
   * {@link #setObject(Comparable, Object)} method.
   *
   * @param key the key.
  ...
  /**
   * Replaces an existing object, or adds a new object to the collection. This is the same as the
   * {@link #addObject(Comparable, Object)} method.
   *
   * @param key the key ({@code null} not permitted).
   * @param object the object.
   * @see #getObject(Comparable)
   ...
   *
   * @param position the position (in the range {@code 0} to {@code getItemCount()}).
   * @param key the key ({@code null} not permitted).
  ...
  /**
   * Returns a clone of this object. Keys in the list should be immutable and are not cloned.
   * Objects in the list are cloned only if they implement {@link PublicCloneable}.
   *
   * @return A clone.
