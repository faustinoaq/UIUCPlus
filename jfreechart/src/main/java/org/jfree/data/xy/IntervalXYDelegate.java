...
/**
 * A delegate that handles the specification or automatic calculation of the interval surrounding
 * the x-values in a dataset. This is used to extend a regular {@link XYDataset} to support the
 * {@link IntervalXYDataset} interface.
 *
 * <p>The decorator pattern was not used because of the several possibly implemented interfaces of
 * the decorated instance (e.g. {@link TableXYDataset}, {@link RangeInfo}, {@link DomainInfo} etc.).
 *
 * <p>The width can be set manually or calculated automatically. The switch autoWidth allows to
 * determine which behavior is used. The auto width calculation tries to find the smallest gap
 * between two x-values in the dataset. If there is only one item in the series, the auto width
 * calculation fails and falls back on the manually set interval width (which is itself defaulted to
 * 1.0).
 */
public class IntervalXYDelegate
   ...
   *
   * @param dataset the underlying dataset ({@code null} not permitted).
   * @param autoWidth a flag that controls whether the interval width is calculated automatically.
   */
  ...
  /**
   * Returns {@code true} if the interval width is automatically calculated, and {@code false}
   * otherwise.
   *
   * @return A boolean.
   */
  ...
  /**
   * Sets the flag that indicates whether the interval width is automatically calculated. If the
   * flag is set to {@code true}, the interval is recalculated.
   *
   * <p>Note: recalculating the interval amounts to changing the data values represented by the
   * dataset. The calling dataset must fire an appropriate {@link DatasetChangeEvent}.
   *
   * @param b a boolean.
   */
  ...
  /**
   * Sets the interval position factor. This controls how the interval is aligned to the x-value.
   * For a value of 0.5, the interval is aligned with the x-value in the center. For a value of 0.0,
   * the interval is aligned with the x-value at the lower end of the interval, and for a value of
   * 1.0, the interval is aligned with the x-value at the upper end of the interval. <br>
   * <br>
   * Note that changing the interval position factor amounts to changing the data values represented
   * by the dataset. Therefore, the dataset that is using this delegate is responsible for
   * generating the appropriate {@link DatasetChangeEvent}.
   *
   * @param d the new interval position factor (in the range {@code 0.0} to {@code 1.0} inclusive).
   */
  ...
  /**
   * Sets the fixed interval width and, as a side effect, sets the {@code autoWidth} flag to {@code
   * false}. <br>
   * <br>
   * Note that changing the interval width amounts to changing the data values represented by the
   * dataset. Therefore, the dataset that is using this delegate is responsible for generating the
  ...
  /**
   * Returns the interval width. This method will return either the auto calculated interval width
   * or the manually specified interval width, depending on the {@link #isAutoWidth()} result.
   *
  ...
  /**
   * Tests the delegate for equality with an arbitrary object. The equality test considers two
   * delegates to be equal if they would calculate the same intervals for any given dataset (for
   * this reason, the dataset itself is NOT included in the equality test, because it is just a
   * reference back to the current 'owner' of the delegate).
   *
