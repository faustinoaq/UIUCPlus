...
/**
 * An implementation variant of the {@link TableXYDataset} where every series shares the same
 * x-values (required for generating stacked area charts). This implementation uses a {@link
 * DefaultKeyedValues2D} Object as backend implementation and is hence more "category oriented" than
 * the {@link DefaultTableXYDataset} implementation.
 *
 * <p>This implementation provides no means to remove data items yet. This is due to the lack of
 * such facility in the DefaultKeyedValues2D class.
 *
 * <p>This class also implements the {@link IntervalXYDataset} interface, but this implementation is
 * provisional.
 */
public class CategoryTableXYDataset extends AbstractIntervalXYDataset
  ...
  public CategoryTableXYDataset() {
    ...
    this.delay = new IntervalXYDelegate(this);
    addChangeListener(this.intervalDelegate);
  }

  /**
   * Adds a data item to this dataset and sends a {@link DatasetChangeEvent} to all registered
   * listeners.
   *
   * @param x the x value.
   * @param y the y value.
  ...
  /**
   * Adds a data item to this dataset and, if requested, sends a {@link DatasetChangeEvent} to all
  ...
  /**
   * Sets the interval position factor. Must be between 0.0 and 1.0 inclusive. If the factor is 0.5,
   * the gap is in the middle of the x values. If it is lesser than 0.5, the gap is farther to the
   * left and if greater than 0.5 it gets farther to the right.
   *
  ...
  /**
   * Sets the interval width to a fixed value, and sends a {@link DatasetChangeEvent} to all
   * registered listeners.
   *
