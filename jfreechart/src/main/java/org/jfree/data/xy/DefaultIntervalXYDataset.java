...
/**
 * A dataset that defines a range (interval) for both the x-values and the y-values. This
 * implementation uses six arrays to store the x, start-x, end-x, y, start-y and end-y values. <br>
 * <br>
 * An alternative implementation of the {@link IntervalXYDataset} interface is provided by the
 * {@link XYIntervalSeriesCollection} class.
 *
 * @since 1.0.3
 */
public class DefaultIntervalXYDataset<S extends Comparable<S>> extends AbstractIntervalXYDataset<S>
  ...
  /**
   * Storage for the series in the dataset. We use a list because the order of the series is
   * significant. This list must be kept in sync with the seriesKeys list.
   */
  private List<double[][]> seriesList;

  /** Creates a new {@code DefaultIntervalXYDataset} instance, initially containing no data. */
   ...
   *
   * @param series the series index (in the range {@code 0} to {@code getSeriesCount() - 1}).
   * @param item the item index (in the range {@code 0} to {@code getItemCount(series)}).
   * @return The x-value.
   * @throws ArrayIndexOutOfBoundsException if {@code series} is not within the specified range.
   * @throws ArrayIndexOutOfBoundsException if {@code item} is not within the specified range.
   * @see #getX(int, int)
   */
  @Override
  ...
  /**
   * Adds a series or if a series with the same key already exists replaces the data for that
   * series, then sends a {@link DatasetChangeEvent} to all registered listeners.
   *
   * @param seriesKey the series key ({@code null} not permitted).
   * @param data the data (must be an array with length 6, containing six arrays of equal length,
   *     the first three containing the x-values (x, xLow and xHigh) and the last three containing
   *     the y-values (y, yLow and yHigh)).
   */
  ...
  /**
   * Tests this {@code DefaultIntervalXYDataset} instance for equality with an arbitrary object.
   * This method returns {@code true} if and only if:
   *
   * <ul>
   *   <li>{@code obj} is not {@code null};
   *   <li>{@code obj} is an instance of {@code DefaultIntervalXYDataset};
   *   <li>both datasets have the same number of series, each containing exactly the same values.
   * </ul>
   *
   ...
   *
   * @return A clone.
   * @throws CloneNotSupportedException if the dataset contains a series with a key that cannot be
   *     cloned.
   */
  @Override
