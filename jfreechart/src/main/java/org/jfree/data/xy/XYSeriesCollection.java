...
public class XYSeriesCollection<S extends Comparable<S>> extends AbstractIntervalXYDataset<S>
   ...
   * @throws IllegalArgumentException if the key for the series is null or not unique within the
   *     dataset.
   */
   ...
   * @param series the series index (zero-based).
   * @return The series.
   * @throws IllegalArgumentException if {@code series} is not in the range {@code 0} to {@code
   *     getSeriesCount() - 1}.
   */
  ...
  public XYSeries<S> getSeries(S key) {
    Args.nullNotPermitted("null", key);
    for (XYSeries<S> series : this.data) {
  ...
  /**
   * Returns the interval width. This is used to calculate the start and end x-values, if/when the
   * dataset is used as an {@link IntervalXYDataset}.
   *
  ...
  /**
   * Sets the interval position factor. This controls where the x-value is in relation to the
   * interval surrounding the x-value (0.0 means the x-value will be positioned at the start, 0.5 in
   * the middle, and 1.0 at the end).
   *
   ...
   *
   * @param includeInterval a flag that determines whether or not the y-interval is taken into
   *     account.
   * @return The minimum value.
   */
  @Override
