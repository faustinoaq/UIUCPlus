 ...
 * structures.
 *
 * <p>The series has two properties ("Key" and "Description") for which you can register a {@code
 * PropertyChangeListener}.
 *
 * <p>You can also register a {@link SeriesChangeListener} to receive notification of changes to the
 * series data.
 */
public abstract class Series<K extends Comparable<K>> implements Cloneable, Serializable {
  ...
  /**
   * Returns {@code true} if the series contains no data items, and {@code false} otherwise.
   *
   * @return A boolean.
   * @since 1.0.7
   */
   ...
   *
   * <ul>
   *   <li>No need to clone the name or description, since String object is immutable.
   *   <li>We set the listener list to empty, since the listeners did not register with the clone.
   *   <li>Same applies to the PropertyChangeSupport instance.
   * </ul>
   *
   ...
   *
   * <p>Objects being registered must implement the {@link SeriesChangeListener} interface.
   *
  ...
  protected void notifyListeners(SeriesChangeEvent event) {
    ...
    for (int i = listenerList.length - 2; i >= 0; i -= 2) {
      if (listenerList[i].equals(SeriesChangeListener.class)) { // Buggy statement
