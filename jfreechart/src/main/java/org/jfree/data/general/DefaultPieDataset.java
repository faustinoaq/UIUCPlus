...
public class DefaultPieDataset<K extends Comparable<K>> extends AbstractDataset
  ...
  public DefaultPieDataset() {
    this.data = new DefaultKeyedValues<Object>();
  }
   ...
   *
   * @param item the item index (in the range {@code 0} to {@code getItemCount() - 1}).
   * @return The key, or {@code null}.
   * @throws IndexOutOfBoundsException if {@code item} is not in the specified range.
   */
  @Override
  ...
  /**
   * Sets the data value for a key and sends a {@link DatasetChangeEvent} to all registered
   * listeners.
   *
   * @param key the key ({@code null} not permitted).
  ...
  /**
   * Inserts a new value at the specified position in the dataset or, if there is an existing item
   * with the specified key, updates the value for that item and moves it to the specified position.
   * After the change is made, this methods sends a {@link DatasetChangeEvent} to all registered
   * listeners.
   *
  ...
  /**
   * Removes an item from the dataset and sends a {@link DatasetChangeEvent} to all registered
   * listeners.
   *
   * @param key the key ({@code null} not permitted).
   * @throws IllegalArgumentException if {@code key} is {@code null}.
  ...
  /**
   * Clears all data from this dataset and sends a {@link DatasetChangeEvent} to all registered
   * listeners (unless the dataset was already empty).
   *
   * @since 1.0.2
   */
  public void clear() {
  ...
  /**
   * Sorts the dataset's items by key and sends a {@link DatasetChangeEvent} to all registered
   * listeners.
   *
  ...
  /**
   * Sorts the dataset's items by value and sends a {@link DatasetChangeEvent} to all registered
   * listeners.
   *
   * @param order the sort order ({@code null} not permitted).
   * @since 1.0.3
