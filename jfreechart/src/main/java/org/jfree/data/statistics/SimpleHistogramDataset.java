 ...
 */
public class SimpleHistogramDataset<K extends Comparable<K>> extends AbstractIntervalXYDataset
  ...
  /**
   * Creates a new histogram dataset. Note that the {@code adjustForBinSize} flag defaults to {@code
   * true}.
   *
  ...
  /**
   * Returns a flag that controls whether or not the bin count is divided by the bin size in the
   * {@link #getXValue(int, int)} method.
   *
   * @return A boolean.
   ...
   * {@link #getYValue(int, int)} method, and sends a {@link DatasetChangeEvent} to all registered
   * listeners.
   *
  ...
  /**
   * Returns the key for a series. Since this dataset only stores a single series, the {@code
  ...
  /**
   * Returns the number of items in a series. Since this dataset only stores a single series, the
   * {@code series} argument is ignored.
   *
  ...
  /**
   * Adds a set of values to the dataset and sends a {@link DatasetChangeEvent} to all registered
   * listeners.
   *
  ...
  /**
   * Removes all bins and sends a {@link DatasetChangeEvent} to all registered listeners.
   *
   * @since 1.0.6
  ...
  /**
   * Returns the x-value for an item within a series. The x-values may or may not be returned in
   * ascending order, that is up to the class implementing the interface.
   *
  ...
  public int hashCode() {
    ...
    hash = 11 * hash + (others.adjustForBinSize ? 1 : 0);
    return hash;
  }
