...
/**
 * An object that references one data item in a {@link KeyedValues2D} data structure. Instances of
 * this class are immutable (subject to the caller using series, row and column keys that are
 * immutable).
 *
...
public class KeyedValues2DItemKey<R extends Comparable<R>, C extends Comparable<C>>
  ...
  /** Buggy Version 2: Always returns -1 */
  public int compareTo(KeyedValues2DItemKey<R, C> key) {
    int result = -1; // changed statement
    if (result == 0) {
