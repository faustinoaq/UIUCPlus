...
/**
 * A structure containing zero, one or many {@link TimePeriodValue} instances. The time periods can
 * overlap, and are maintained in the order that they are added to the collection.
 *
 * <p>This is similar to the {@link TimeSeries} class, except that the time periods can have
 * irregular lengths.
 */
public class TimePeriodValues<S extends Comparable<S>> extends Series<S> implements Serializable {
   ...
   *
   * @param index the item index (in the range {@code 0} to {@code getItemCount() -1}).
   ...
   *
   * <ul>
   *   <li>no need to clone the domain and range descriptions, since String object is immutable;
