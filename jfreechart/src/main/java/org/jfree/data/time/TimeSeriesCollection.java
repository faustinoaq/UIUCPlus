...
/**
 * A collection of time series objects. This class implements the {@link XYDataset} interface, as
 * well as the extended {@link IntervalXYDataset} interface. This makes it a convenient dataset for
 * use with the {@link org.jfree.chart.plot.XYPlot} class.
 */
public class TimeSeriesCollection<S extends Comparable<S>> extends AbstractIntervalXYDataset
  ...
  /**
   * The point within each time period that is used for the X value when this collection is used as
   * an {@link org.jfree.data.xy.XYDataset}. This can be the start, middle or end of the time
   * period.
   */
   ...
   *
   * @param zone the timezone ({@code null} permitted, will use {@code TimeZone.getDefault()} in
   *     that case).
   */
   ...
   * timezone.
   *
   * @param series a series to add to the collection ({@code null} permitted).
   * @param zone the timezone ({@code null} permitted, will use {@code TimeZone.getDefault()} in
   *     that case).
   */
  ...
  /**
   * Sets the position within each time period that is used for the X values when the collection is
   * used as an {@link XYDataset}, then sends a {@link DatasetChangeEvent} is sent to all registered
   * listeners.
   *
  ...
  /**
   * Returns the series with the specified key, or {@code null} if there is no such series.
   *
  ...
  /**
   * Receives notification that the key for one of the series in the collection has changed, and
   * vetos it if the key is already present in the collection.
   *
   * @param e the event.
   * @since 1.0.17
   */
