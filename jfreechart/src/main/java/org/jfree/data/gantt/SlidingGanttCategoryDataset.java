...
/**
 * A {@link GanttCategoryDataset} implementation that presents a subset of the categories in an
 * underlying dataset. The index of the first "visible" category can be modified, which provides a
 * means of "sliding" through the categories in the underlying dataset.
 *
 * @since 1.0.10
 */
public class SlidingGanttCategoryDataset extends AbstractDataset implements GanttCategoryDataset {
   ...
   *
   * @param underlying the underlying dataset ({@code null} not permitted).
   * @param firstColumn the index of the first visible column from the underlying dataset.
   * @param maxColumns the maximumColumnCount.
   */
  public SlidingGanttCategoryDataset(
  ...
  /**
   * Sets the index of the first category that should be used from the underlying dataset, and sends
  ...
  /**
   * Sets the maximum category count and sends a {@link DatasetChangeEvent} to all registered
   * listeners.
   *
  ...
  private int lastCategoryIndex() {
    ...
    return Math.min(
            this.firstCategoryIndex + this.maximumCategoryCount, this.underlying.getColumnCount())
        - 1;
  }
  ...
  public Number getEndValue(Comparable rowKey, Comparable columnKey, int subinterval) {
    ...
    int c = getColumnIndex(columnKey);
    if (c == -1) {
      throw new RuntimeException("Unknown columnKey: " + columnKey);
    } else if (r == -1) {
      throw new RuntimeException("Unknown rowKey: " + rowKey);
    } else {
  ...
  /**
   * Tests this {@code SlidingGanttCategoryDataset} instance for equality with an arbitrary object.
   *
   ...
   *
   * <ul>
   *   <li>the underlying dataset is only cloned if it implements the {@link PublicCloneable}
   *       interface;
   *   <li>the listeners registered with this dataset are not carried over to the cloned dataset.
   * </ul>
   *
