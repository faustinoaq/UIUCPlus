...
/**
 * A collection of {@link TaskSeries} objects. This class provides one implementation of the {@link
 * GanttCategoryDataset} interface.
 */
public class TaskSeriesCollection<R extends Comparable<R>, C extends Comparable<C>>
    extends AbstractSeriesDataset<R>
    implements GanttCategoryDataset<R, C>, Cloneable, PublicCloneable, Serializable {
  ...
  /**
   * Adds a series to the dataset and sends a {@link org.jfree.data.general.DatasetChangeEvent} to
  ...
  /**
   * Removes a series from the collection and sends a {@link
  ...
  /**
   * Removes all the series from the collection and sends a {@link
   * org.jfree.data.general.DatasetChangeEvent} to all registered listeners.
   */
  ...
  /**
   * Returns the start value for a task. This is a date/time value, measured in milliseconds since
   * 1-Jan-1970.
   *
  ...
  /**
   * Returns the end value for a task. This is a date/time value, measured in milliseconds since
   * 1-Jan-1970.
   *
   * @param rowKey the series.
   * @param columnKey the category.
  ...
  public Number getPercentComplete(R rowKey, C columnKey, int subinterval) {
    ...
    int row = getRowIndex(rowKey);
    TaskSeries<R> series = this.data.get(row);
    Task task = series.get(colIndex.toString());
    if (task != null) {
      Task sub = task.getSubtask(subinterval);
      if (sub != null) {
