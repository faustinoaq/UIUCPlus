 ...
 * IntervalXYDataset}, allowing a set of tasks to be displayed using an {@link XYBarRenderer} (and
 * usually a {@link SymbolAxis}). This is a very specialised dataset implementation ---before using
 * it, you should take some time to understand the use-cases that it is designed for.
 *
 * @since 1.0.11
 */
public class XYTaskDataset extends AbstractXYDataset
  ...
  /**
   * Sets the series interval width and sends a {@link DatasetChangeEvent} to all registered
   * listeners.
   *
   * @param w the width.
   * @see #getSeriesWidth()
  ...
  /**
   * Returns a flag that indicates whether or not the dataset is transposed. The default is {@code
   * false} which means the x-values are integers corresponding to the series indices, and the
   * y-values are millisecond values corresponding to the task date/time intervals. If the flag is
   * set to {@code true}, the x and y-values are reversed.
   *
   * @return The flag.
  ...
  /**
   * Sets the flag that controls whether or not the dataset is transposed and sends a {@link
  ...
  /**
   * Returns the y-value for the specified series/item. In this implementation, we return the series
   * index as the y-value (this means that every item in the series has a constant integer value).
   *
   * @param series the series index.
   * @param item the item index.
   * @return The y-value.
  ...
  public int hashCode() {
    ...
    hash =
        17 * hash
            + (int)
                (Double.doubleToLongBits(this.seriesWidth)
