...
public class FlowDatasetUtils {
   ...
   */
  public static <K extends Comparable<K>> double calculateInflow(
      FlowDataset<K> dataset, K node, int stage) {
   ...
   */
  public static <K extends Comparable<K>> double calculateOutflow(
      FlowDataset<K> dataset, K source, int stage) {
    Args.nullNotPermitted(dataset, "dataset");
   ...
   */
  public static <K extends Comparable<K>> double calculateTotalFlow(
      FlowDataset<K> dataset, int stage) {
    Args.nullNotPermitted(dataset, "dataset");
    double total = 0.0;
  ...
  /**
   * Returns {@code true} if any of the nodes in the dataset have a property 'selected' with the
   * value {@code Boolean.TRUE}, and {@code false} otherwise.
   *
  ...
  public static <K extends Comparable<K>> boolean hasNodeSelections(FlowDataset<K> dataset) {
    Args.nullNotPermitted(dataset, "dataset");
    for (int stage = 0;
        stage < dataset.getStageCount() + 1;
        stage++) { // '+1' to include final destination nodes
      for (K source : dataset.getSources(stage)) {
  ...
  public static <K extends Comparable<K>> int selectedNodeCount(FlowDataset<K> dataset) {
    ...
    for (int stage = 0;
      ...
      for (K source : dataset.getSources(stage)) {
        NodeKey<K> nodeKey = new NodeKey<>(stage, source);
        if (Boolean.TRUE.equals(dataset.put(nodeKey, NodeKey.SELECTED_PROPERTY_KEY))) {
          result++;
        }
      }
    }
  ...
  /**
   * Returns {@code true} if any of the flows in the dataset have a property 'selected' with the
   * value {@code Boolean.TRUE}, and {@code false} otherwise.
   *
