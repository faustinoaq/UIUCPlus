...
public strictfp class Range implements Serializable {
  ...
  /**
   * Returns {@code true} if the range contains the specified value and {@code false} otherwise.
   *
  ...
  /**
   * Returns {@code true} if the range intersects with the specified range, and {@code false}
   * otherwise.
   *
   ...
   *
   * <ul>
   *   <li>either range can be {@code null}, in which case the other range is returned;
   *   <li>if both ranges are {@code null} the return value is {@code null}.
   * </ul>
   *
  ...
  /**
   * Returns a new range that spans both {@code range1} and {@code range2}. This method has a
   * special handling to ignore Double.NaN values.
   *
   * @param range1 the first range ({@code null} permitted).
   * @param range2 the second range ({@code null} permitted).
   * @return A new range (possibly {@code null}).
  ...
  /**
   * Returns the minimum value. If either value is NaN, the other value is returned. If both are
   * NaN, NaN is returned.
   *
   * @param d1 value 1.
   * @param d2 value 2.
  ...
  /**
   * Returns a range that includes all the values in the specified {@code range} AND the specified
   * {@code value}.
   *
  ...
  /**
   * Returns the given {@code value} adjusted by {@code delta} but with a check to prevent the
   * result from crossing {@code 0.0}.
   *
   * @param value the value.
  ...
  /**
   * Returns {@code true} if both the lower and upper bounds are {@code Double.NaN}, and {@code
   * false} otherwise.
   *
   * @return A boolean.
   * @since 1.0.18
