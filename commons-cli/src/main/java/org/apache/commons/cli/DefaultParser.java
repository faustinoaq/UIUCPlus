...
public class DefaultParser implements CommandLineParser {
  ...
  /**
   * A nested builder class to create {@code DefaultParser} instances using descriptive methods.
   *
   * <p>Example usage:
   *
   * <pre>
  ...
  public static final class Builder {
    ...
    /**
     * Flag indicating if balanced leading and trailing double quotes should be stripped from option
     * arguments.
     */
     ...
     *
     * <p>Both allowPartialMatching and stripLeadingAndTrailingQuotes are true by default, mimicking
     ...
     * </pre>
     *
     * If "partial matching" is turned on, {@code -de} only matches the {@code "debug"} option.
     * However, with "partial matching" disabled, {@code -de} would enable both {@code debug} as
     * well as {@code extract}
     *
     ...
     *
     * <p>If "stripping of balanced leading and trailing double quotes from option arguments" is
     * true, the outermost balanced double quotes of option arguments values will be removed. For
     * example, {@code -o '"x"'} getValue() will return {@code x}, instead of {@code "x"}
     *
     * <p>If "stripping of balanced leading and trailing double quotes from option arguments" is
     * null, then quotes will be stripped from option values separated by space from the option, but
  ...
  /**
   * Flag indicating how unrecognized tokens are handled. {@code true} to stop the parsing and add
   * the remaining tokens to the args list. {@code false} to throw an exception.
   */
  ...
  /**
   * Flag indicating if balanced leading and trailing double quotes should be stripped from option
   * arguments. null represents the historic arbitrary behavior
   */
   ...
   * </pre>
   *
   * with "partial matching" turned on, {@code -de} only matches the {@code "debug"} option.
   * However, with "partial matching" disabled, {@code -de} would enable both {@code debug} as well
   * as {@code extract} options.
   */
  ...
  /**
   * Creates a new DefaultParser instance with the specified partial matching and quote stripping
   * policy.
   *
   * @param allowPartialMatching if partial matching of long options shall be enabled
  ...
  private DefaultParser(
      final boolean allowPartialMatching, final Boolean stripLeadingAndTrailingQuotes) {
    this.allowPartialMatching = allowPartialMatching;
    this.stripLeadingAndTrailingQuotes = stripLeadingAndTrailingQuotes;
  }
  ...
  /**
   * Gets a list of matching option strings for the given token, depending on the selected partial
   * matching policy.
   *
   ...
   * thrown. Otherwise the token is added to the arguments of the command line. If the
   * stopAtNonOption flag is set, this stops the parsing and the remaining tokens are added as-is in
  ...
  /**
   * Strips balanced leading and trailing quotes if the stripLeadingAndTrailingQuotes is set If
   * stripLeadingAndTrailingQuotes is null, then do not strip
   *
