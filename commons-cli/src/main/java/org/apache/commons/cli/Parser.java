...
public abstract class Parser implements CommandLineParser {
  ...
  /**
   * Subclasses must implement this method to reduce the {@code arguments} that have been passed to
   * the parse method.
   *
  ...
  protected abstract String[] flatten(Options opts, String[] arguments, boolean stopAtNonOption)
    ...
    if (stopAtNonOption && !opts.hasOption(arguments[0])) {
      return arguments;
    }
    ...
    while (position < arguments.length) {
      ...
      if (opts.hasOption(arg)) {
        tokens.add(arg);
        if (opts.getOption(arg).hasArg()) {
          position++;
          if (position < arguments.length) {
            tokens.add(arguments[position]);
          } else {
            throw new ParseException("Option " + arg + " requires an argument");
          }
        }
      } else {
        tokens.add(arg);
