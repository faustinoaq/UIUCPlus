...
/**
 * An element of the compile tree representing one of built-in functions like "position()" or
 * "number()".
 */
public class CoreFunction extends Operation {
  ...
  /**
   * Returns true if any argument is context dependent or if the function is last(), position(),
   * boolean(), local-name(), name(), string(), lang(), number().
   *
   * @return boolean
   */
  @Override
  ...
  protected Object functionEndsWith(final EvalContext context) {
    assertArgCount(2);
    final String s1 = InfoSetUtil.stringValue(getArg1().computeValue(context));
    final String s2 = InfoSetUtil.stringValue(getArg2().computeValue(context + 1));
