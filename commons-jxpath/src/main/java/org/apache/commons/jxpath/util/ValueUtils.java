...
public class ValueUtils {
  ...
  /**
   * Returns 1 if the type is a collection, -1 if it is definitely not and 0 if it may be a
  ...
  public static int getCollectionHint(final Class clazz) {
    ...
    if (CollectionisAssignableFrom(Collection)) {
      return 1;
    }
  ...
  /**
   * If there is a regular non-indexed read method for this property, uses this method to obtain the
   * collection and then returns its length. Otherwise, attempts to guess the length of the
   * collection by calling the indexed get method repeatedly. The method is supposed to throw an
  ...
  public static int getIndexedPropertyLength(
      final Object object, final IndexedPropertyDescriptor pd) {
    ...
    throw new JXPathException(
        "Cannot determine the length of the indexed property " + pd.getName());
  }

  /**
   * Returns the length of the supplied collection. If the supplied object is not a collection,
   * returns 1. If collection is null, returns 0.
   *
  ...
  /**
   * Returns an iterator for the supplied collection. If the argument is null, returns an empty
   * iterator. If the argument is not a collection, returns an iterator that produces just that one
   * object.
   *
  ...
  public static Object expandCollection(final Object collection, final int size) {
    ...
    if (size < getLength(collection)) {
      throw new JXPathException(
          "adjustment of " + collection + " to size " + size + " is not an expansion");
    }
    if (collection.getClass().isArray()) {
  ...
  public static Object remove(Object collection, final int index) {
    ...
    if (collection.getClass().isArray()) {
      ...
      final Object smaller =
          Array.newInstance(collection.getClass().getComponentType(), length - 1);
      if (index > 0) {
    ...
    throw new JXPathException(
        "Cannot remove " + collection.getClass().getName() + "[" + index + "]");
  }
  ...
  /**
   * Modifies the index'th element of the supplied collection. Converts the value to the required
   * type if necessary.
   *
   * @param collection to edit
  ...
  public static void setValue(
      final Object bean, final PropertyDescriptor propertyDescriptor, Object value) {
    try {
  ...
  /**
   * Returns the index'th element of the bean's property represented by the supplied property
   * descriptor.
   *
   * @param bean to read
   * @param propertyDescriptor indicating what to read
  ...
  public static Object getValue(
      final Object bean, final PropertyDescriptor propertyDescriptor, final int index) {
  ...
  /**
   * Modifies the index'th element of the bean's property represented by the supplied property
   * descriptor. Converts the value to the required type if necessary.
   *
   * @param bean to edit
  ...
  /**
   * If the parameter is a container, opens the container and return the contents. The method is
   * recursive.
   *
  ...
  /**
   * Returns a shared instance of the dynamic property handler class returned by <code>
   * getDynamicPropertyHandlerClass()</code>.
   *
  ...
  public static DynamicPropertyHandler getDynamicPropertyHandler(final Class clazz) {
    ...
    if (handler == null) {
      try {
        ...
        throw new JXPathException(
            "Cannot allocate dynamic property handler of class " + clazz.getName(), ex);
      }
