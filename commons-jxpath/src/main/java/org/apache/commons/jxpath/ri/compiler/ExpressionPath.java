...
/**
 * An element of the parse tree that represents an expression path, which is a path that starts with
 * an expression like a function call: <code>getFoo(.)
 * /bar</code>.
 */
public class ExpressionPath extends Path {
  ...
  /**
   * Returns true if the root expression or any of the predicates or the path steps are context
   * dependent.
   *
   * @return boolean
   */
  ...
  /**
   * Recognized paths formatted as <code>$x[3]/foo[2]</code>. The evaluation of such "simple" paths
