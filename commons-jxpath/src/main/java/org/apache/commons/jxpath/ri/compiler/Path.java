...
public abstract class Path extends Expression {
  ...
  /**
   * Recognizes paths formatted as <code>foo/bar[3]/baz[@name = 'biz']</code>. The evaluation of
   * such "simple" paths is optimized and streamlined.
   *
  ...
  /**
   * A Step is "simple" if it takes one of these forms: ".", "/foo", "@bar", "/foo[3]". If there are
   * predicates, they should be context independent for the step to still be considered simple.
   *
  ...
  /**
   * Given a root context, walks a path therefrom and finds the pointer to the first element
   * matching the path.
   *
  ...
  /**
   * The idea here is to return a NullPointer rather than null if that's at all possible. Take for
   * example this path: "//map/key". Let's say, "map" is an existing node, but "key" is not there.
   * We will create a NullPointer that can be used to set/create the "key" property.
   *
   * <p>However, a path like "//key" would still produce null, because we have no way of knowing
   * where "key" would be if it existed.
   *
   * <p>To accomplish this, we first try the path itself. If it does not find anything, we chop off
   * last step of the path, as long as it is a simple one like child:: or attribute:: and try to
   * evaluate the truncated path. If it finds exactly one node - create a NullPointer and return. If
   * it fails, chop off another step and repeat. If it finds more than one location - return null.
   *
   * @param context evaluation context
   * @return Pointer
   */
  ...
  /**
   * Given a root context, walks a path therefrom and builds a context that contains all nodes
   * matching the path.
   *
   * @param context evaluation context
  ...
  protected EvalContext buildContextChain(
      EvalContext context, final int stepCount, final boolean createInitialContext) {
    ...
    return new PredicateContext(context, predicates[j]);
  }

  /**
   * Different axes are serviced by different contexts. This method allocates the right context for
   * the supplied step.
   *
   * @param context evaluation context
   * @param axis code
  ...
  protected EvalContext createContextForStep(
      final EvalContext context, final int axis, NodeTest nodeTest) {
    if (nodeTest instanceof NodeNameTest) {
