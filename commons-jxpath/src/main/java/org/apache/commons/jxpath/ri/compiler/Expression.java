...
/**
 * Common superclass for several types of nodes in the parse tree. Provides APIs for optimization of
 * evaluation of expressions. Specifically, an expression only needs to executed once during the
 * evaluation of an xpath if that expression is context-independent. Expression.isContextDependent()
 * provides that hint.
 */
public abstract class Expression {
  ...
  /**
   * Returns true if this expression should be re-evaluated each time the current position in the
   * context changes.
   *
   * @return boolean
   */
  ...
  /**
   * Evaluates the expression. If the result is a node set, returns the first element of the node
   * set.
   *
  ...
  public Iterator iteratePointers(final EvalContext context) {
    ...
    if (result instanceof NodeSet) {
      return new PointerIterator(
          ((NodeSet) result).getPointers().iterator(),
