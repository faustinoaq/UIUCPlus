...
public abstract class PropertyOwnerPointer extends NodePointer {
    ...
    if (test instanceof NodeNameTest) {
      ...
      if (isValidProperty(testName)) {
        return createNodeIterator(
            nodeNameTest.isWildcard() ? null : testName.toString(), reverse, startWith);
      }
      return null;
    }
    ...
    if (value == UNINITIALIZED) {
      value =
          index == WHOLE_COLLECTION
              ? ValueUtils.getValue(getBaseValue())
  ...
  /**
   * Throws an exception if you try to change the root element, otherwise forwards the call to the
   * parent pointer.
   *
   * @param value to set
   */
  ...
  /**
   * If this is a root node pointer, throws an exception; otherwise forwards the call to the parent
   * node.
   */
  @Override
  public void remove() {
   ...
   *
   * @return true if the property owner can set a property "does not exist". A good example is a
   *     Map. You can always assign a value to any key even if it has never been "declared".
   */
