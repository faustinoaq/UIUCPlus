 ...
 * Using JXPathContext, you can read and write properties of JavaBeans, arrays, collections and
 * maps. JXPathContext uses JavaBeans introspection to enumerate and access JavaBeans properties.
 *
 * <p>JXPathContext allows alternative implementations. This is why instead of allocating
 * JXPathContext directly, you should call a static <code>newContext</code> method. This method will
 * utilize the {@link JXPathContextFactory} API to locate a suitable implementation of JXPath.
 * Bundled with JXPath comes a default implementation called Reference Implementation.
 *
 * <h2>JXPath Interprets XPath Syntax on Java Object Graphs</h2>
 *
 * JXPath uses an intuitive interpretation of the xpath syntax in the context of Java object graphs.
 ...
 * </pre>
 *
 * In this example, we are using JXPath to access a property of the <code>emp</code> bean. In this
 * simple case the invocation of JXPath is equivalent to invocation of getFirstName() on the bean.
 *
 ...
 *
 * <p>A property identified by the xpath does not have to be a "leaf" property. For instance, we can
 * extract the whole Address object in above example:
 *
 * <pre>
 ...
 *
 * <p>Note: in XPath the first element of a collection has index 1, not 0.<br>
 *
 ...
 * Note: JXPath supports the extended notion of Map: any object with dynamic properties can be
 * handled by JXPath provided that its class is registered with the {@link JXPathIntrospector}.
 *
 * <h3>Example 5: Retrieving Multiple Results</h3>
 *
 * JXPath can retrieve multiple objects from a graph. Note that the method called in this case is
 * not <code>getValue</code>, but <code>iterate</code>.
 *
 * <pre>
 * public class Author {
 ...
 *
 * JXPath can be used to create new objects. First, create a subclass of {@link AbstractFactory
 * AbstractFactory} and install it on the JXPathContext. Then call {@link JXPathContext#createPath
 * createPathAndSetValue()} instead of "setValue". JXPathContext will invoke your AbstractFactory
 * when it discovers that an intermediate node of the path is <b>null</b>. It will not override
 * existing nodes.
 *
 * <pre>
 ...
 *
 * JXPath supports the notion of variables. The XPath syntax for accessing variables is
 * <i>"$varName"</i>.
 *
 * <pre>
 * public class Author {
 ...
 * </pre>
 *
 * Note: you can only <i>change</i> the value of an existing variable this way, you cannot
 * <i>define</i> a new variable.
 *
 * <p>When a variable contains a JavaBean or a collection, you can traverse the bean or collection
 * as well:
 *
 * <pre>
 * ...
 ...
 *
 * If you need to use the same set of variable while interpreting XPaths with different beans, it
 * makes sense to put the variables in a separate context and specify that context as a parent
 ...
 *
 * By default, JXPathContext creates a HashMap of variables. However, you can substitute a custom
 * implementation of the Variables interface to make JXPath work with an alternative source of
 * variables. For example, you can define implementations of Variables that cover a servlet context,
 * HTTP request or any similar structure.
 *
 * <h3>Example 10: Using Standard Extension Functions</h3>
 *
 * Using the standard extension functions, you can call methods on objects, static methods on
 * classes and create objects using any constructor. The class names should be fully qualified.
 *
 * <p>Here's how you can create new objects:
 *
 * <pre>
 * Book book =
 ...
 *
 * Collections of custom extension functions can be implemented as {@link Functions Functions}
 * objects or as Java classes, whose methods become extenstion functions.
 *
 * <p>Let's say the following class implements various formatting operations:
 *
 * <pre>
 ...
 *
 * <p>Also, see {@link FunctionLibrary FunctionLibrary}, which is a class that allows you to
 * register multiple sets of extension functions with the same JXPathContext.
 *
 * <h2>Configuring JXPath</h2>
 *
 * JXPath uses JavaBeans introspection to discover properties of JavaBeans. You can provide
 * alternative property lists by supplying custom JXPathBeanInfo classes (see {@link JXPathBeanInfo
 * JXPathBeanInfo}).
 *
 * <h2>Notes</h2>
 *
 * <ul>
 *   <li>JXPath does not support DOM attributes for non-DOM objects. Even though XPaths like
 *       "para[@type='warning']" are legitimate, they will always produce empty results. The only
 *       attribute supported for JavaBeans is "name". The XPath "foo/bar" is equivalent to
 *       "foo[@name='bar']".
 *   <li id='matches_no_property_in_the_graph'>The term <b>matches no property in the graph</b> is
 *       used throughout the documentation. It describes a property or path that can be determined
 *       as not belonging to the graph. Determining whether a property or path belongs to the graph
 *       depends on the type of object being used as {@code cotextBean} (see {@link
 *       #newContext(Object)}). It is only possible strongly typed models where a specific Java
 *       model is used as context. It is not possible with dynamic models such Maps or DOM
 *       implementations.
 *       <p>When a XPath does not match a property in the graph, the methods of this class that
 *       retrieve a pointer will generally behave in the following way, depending on the last value
 *       configured with {@link #setLenient(boolean)}:
 *       <ol style='list-style:upper-alpha'>
 *         <li>If <code>lenient</code> is <code>false</code> (default) - methods will throw {@link
 *             JXPathNotFoundException}.
 *         <li>If <code>lenient</code> is <code>true</code> - methods will throw no exception and
 *             return a value appropriate for that method to express the absence: might be a Java
 *             <code>null</code> or a {@link Pointer} whose {@link Pointer#getValue()} returns
 ...
 * href="http://commons.apache.org/jxpath/users-guide.html">JXPath User's Guide</a>
 */
public abstract class JXPathContext {
  ...
  /**
   * Creates a new JXPathContext with the specified bean as the root node and the specified parent
   * context. Variables defined in a parent context can be referenced in XPaths passed to the child
   * context.
   *
  ...
  public static JXPathContext newContext(
      final JXPathContext parentContext, final Object contextBean) {
  ...
  /**
   * This constructor should remain protected - it is to be overridden by subclasses, but never
   * explicitly invoked by clients.
   *
   * @param parentContext parent context
   * @param contextBean Object
   */
  ...
  /**
   * Returns a JXPathContext that is relative to the current JXPathContext. The supplied pointer
   * becomes the context pointer of the new context. The relative context inherits variables,
   * extension functions, locale etc from the parent context.
   *
  ...
  /**
   * Returns the variable pool associated with the context. If no such pool was specified with the
   * {@link #setVariables} method, returns the default implementation of Variables, {@link
  ...
  /**
   * Install an abstract factory that should be used by the <code>createPath()</code> and <code>
   * createPathAndSetValue()</code> methods.
   *
  ...
  /**
   * Returns the AbstractFactory installed on this context. If none has been installed and this
   * context has a parent context, returns the parent's factory. Otherwise returns null.
   *
  ...
  /**
   * Set the locale for this context. The value of the "lang" attribute as well as the lang()
   * function will be affected by the locale. By default, JXPath uses <code>Locale.getDefault()
   * </code>
   *
   * @param locale Locale
   */
  ...
  /**
   * Returns the locale set with setLocale. If none was set and the context has a parent, returns
   * the parent's locale. Otherwise, returns Locale.getDefault().
   *
   * @return Locale
   */
  ...
  /**
   * Sets {@link DecimalFormatSymbols} for a given name. The DecimalFormatSymbols can be referenced
   * as the third, optional argument in the invocation of <code>
   * format-number (number,format,decimal-format-name)</code> function. By default, JXPath uses the
   * symbols for the current locale.
   *
   * @param name the format name or null for default format.
   * @param symbols DecimalFormatSymbols
   */
  public synchronized void setDecimalFormatSymbols(
      final String name, final DecimalFormatSymbols symbols) {
   ...
   * Otherwise, a path that does not map to an existing property will throw an exception. Note that
   * if the property exists, but its value is null, the exception is <i>not</i> thrown.
   *
   * <p>By default, lenient = false
   *
   * @param lenient flag
   */
  ...
  /**
   * Compiles the supplied XPath and returns an internal representation of the path that can then be
   * evaluated. Use CompiledExpressions when you need to evaluate the same expression multiple times
   * and there is a convenient place to cache CompiledExpression between invocations.
   *
  ...
  /**
   * Overridden by each concrete implementation of JXPathContext to perform compilation. Is called
   * by <code>compile()</code>.
   *
   * @param xpath to compile
   * @return CompiledExpression
   */
  ...
  /**
   * Finds the first object that matches the specified XPath. It is equivalent to <code>
   * getPointer(xpath).getNode()</code>. Note that this method produces the same result as <code>
   * getValue()</code> on object models like JavaBeans, but a different result for DOM/JDOM etc.,
   * because it returns the Node itself, rather than its textual contents.
   *
  ...
  /**
   * Evaluates the xpath, converts the result to the specified class and returns the resulting
   * object.
   *
   * @param xpath to evaluate
  ...
  /**
   * Modifies the value of the property described by the supplied xpath. Will throw an exception if
  ...
  /**
   * Creates missing elements of the path by invoking an {@link AbstractFactory}, which should first
   * be installed on the context by calling {@link #setFactory}.
   *
   * <p>Will throw an exception if the AbstractFactory fails to create an instance for a path
   * element.
   *
  ...
  /**
   * The same as setValue, except it creates intermediate elements of the path by invoking an {@link
   * AbstractFactory}, which should first be installed on the context by calling {@link
   * #setFactory}.
   *
   * <p>Will throw an exception if one of the following conditions occurs:
   *
   * <ul>
   *   <li>Elements of the xpath aleady exist, but the path does not in fact describe an existing
   *       property
  ...
  /**
   * Traverses the xpath and returns an Iterator of all results found for the path. If the xpath
   * matches no properties in the graph, the Iterator will be empty, but not null.
   *
   ...
   *
   * <p>If the xpath <a href='#matches_no_property_in_the_graph'>matches no properties in the
   * graph</a> the behavior depends on the value that has been configured with {@link
   * #setLenient(boolean)}:
   *
   * <ul>
   *   <li><code>false</code> (default) the method will throw a {@link JXPathNotFoundException}.
   *   <li><code>true</code> the method returns a pointer whose {@link Pointer#getValue()} method
  ...
  /**
   * Traverses the xpath and returns an Iterator of Pointers. A Pointer provides easy access to a
   * property. If the xpath matches no properties in the graph, the Iterator be empty, but not null.
   *
   * @param xpath to iterate
   * @return Iterator
   */
  ...
  /**
   * Returns this context's identity manager. If none has been installed, returns the identity
   * manager of the parent context.
   *
  ...
  /**
   * Returns this context's key manager. If none has been installed, returns the key manager of the
   * parent context.
   *
   * @return KeyManager
   */
  ...
  /**
   * Given a prefix, returns a registered namespace URI. If the requested prefix was not defined
   * explicitly using the registerNamespace method, JXPathContext will then check the context node
   * to see if the prefix is defined there. See {@link #setNamespaceContextPointer(Pointer)
  ...
  public String getNamespaceURI(final String prefix) {
    System.out.println("Namespace registration is not implemented by " + getClass());
    throw new UnsupportedOperationException();
  }
  ...
  /**
   * Namespace prefixes can be defined implicitly by specifying a pointer to a context where the
   * namespaces are defined. By default, NamespaceContextPointer is the same as the Context Pointer,
   * see {@link #getContextPointer() getContextPointer()}
   *
   * @param namespaceContextPointer The pointer to the context where prefixes used in XPath
  ...
  /**
   * Returns the namespace context pointer set with {@link #setNamespaceContextPointer(Pointer)
   * setNamespaceContextPointer()} or, if none has been specified, the context pointer otherwise.
   *
