...
/**
 * An evaluation mechanism for simple XPaths, which is much faster than the usual process. It is
 * only used for xpaths which have no context-dependent parts, consist entirely of <code>child::name
 * </code> and <code>self::node()</code> steps with predicates that either integer or have the form
 * <code>[@name = ...]</code>.
 */
public class SimplePathInterpreter {
  ...
  /**
   * Interpret a simple path that starts with the given root and follows the given steps. All steps
   * must have the axis "child::" and a name test. They can also optionally have predicates of type
   * [@name=expression] or simply [expression] interpreted as an index.
   *
  ...
  /**
   * Interpret the steps of a simple expression path that starts with the given root, which is the
   * result of evaluation of the root expression of the expression path, applies the given
   * predicates to it and then follows the given steps. All steps must have the axis "child::" or
   * "attribute::" and a name test. They can also optionally have predicates of type [@name=...] or
   * simply [...] interpreted as an index.
   *
   * @param context evaluation context
   * @param root root pointer
  ...
  /**
   * Recursive evaluation of a path. The general plan is: Look at the current step, find nodes that
   * match it, iterate over those nodes and for each of them call doStep again for subsequent steps.
   *
   * @param context evaluation context
  ...
  private static NodePointer doStep(
      final EvalContext context, NodePointer parent, final Step[] steps, final int currentStep) {
    if (parent == null) {
      return null;
    }
    ...
    if (parent instanceof PropertyOwnerPointer) {
      if (predicates == null || predicates.length == 0) {
        return doStepNoPredicatesPropertyOwner(
            context, (PropertyOwnerPointer) parent, steps, currentStep);
      }
  ...
  /**
   * We have a step that starts with a property owner (bean, map, etc) and has no predicates. The
   * name test of the step may map to a scalar property or to a collection. If it is a collection,
   * we should apply the tail of the path to each element until we find a match. If we don't find a
   * perfect match, we should return the "best quality" pointer, which has the longest chain of
   * steps mapping to existing nodes and the shortes tail of Null* pointers.
   *
   * @param context evaluation context
  ...
  private static NodePointer doStepNoPredicatesPropertyOwner(
      final EvalContext context,
      final PropertyOwnerPointer parentPointer,
      final Step[] steps,
  ...
  /**
   * A path that starts with a standard InfoSet node (e.g. DOM Node) and has no predicates. Get a
   * child iterator and apply the tail of the path to each element until we find a match. If we
   * don't find a perfect match, we should return the "best quality" pointer, which has the longest
   * chain of steps mapping to existing nodes and the shortes tail of Null* pointers.
   *
   * @param context evaluation context
  ...
  private static NodePointer doStepNoPredicatesStandard(
      final EvalContext context,
      final NodePointer parentPointer,
      final Step[] steps,
      final int currentStep) {
    final Step step = steps[currentStep];
  ...
  /**
   * A path that starts with a property owner. The method evaluates the first predicate in a special
   * way and then forwards to a general predicate processing method.
   *
   * @param context evaluation context
   * @param parentPointer parent pointer
   * @param steps path steps
  ...
  /**
   * A path that starts with a standard InfoSet node, e.g. a DOM Node. The method evaluates the
   * first predicate in a special way and then forwards to a general predicate processing method.
   *
   * @param context evaluation context
   * @param parent parent pointer
   * @param steps path steps
    ...
    if (predicates.length == 1) {
      ...
      if (it != null) {
        ...
        final NodePointer pointer =
            doPredicatesStandard(context, list, steps, currentStep, predicates, 0);
        if (pointer != null) {
          return pointer;
        }
      }
  ...
  private static NodePointer doPredicateName(
    ...
    if (child instanceof PropertyOwnerPointer) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        if (valuePointer instanceof PropertyOwnerPointer || valuePointer.isCollection()) {
          pointer =
              doPredicateName(
                  context, valuePointer, steps, currentStep, predicates, currentPredicate);
        } else if (isNameAttributeEqual(valuePointer, key)) {
          pointer =
              doPredicate(
                  context, valuePointer, steps, currentStep, predicates, currentPredicate + 1);
        } else {
          pointer = null;
        }
        if (pointer != null) {
    ...
    return createNullPointerForPredicates(
        context, child, steps, currentStep, predicates, currentPredicate);
  }

  /**
   * Called exclusively for standard InfoSet nodes, e.g. DOM nodes to evaluate predicate sequences
   * like [@name=...][@name=...][index].
   *
   * @param context evaluation context
  ...
  private static NodePointer doPredicatesStandard(
    ...
    if (predicate instanceof NameAttributeTest) {
      ...
      return doPredicatesStandard(
          context, newList, steps, currentStep, predicates, currentPredicate + 1);
    }
  ...
  /**
   * Evaluate a subscript predicate: see if the node is a collection and if the index is inside the
   * collection.
   *
   * @param context evaluation context
   * @param parent parent pointer
  ...
  private static NodePointer doPredicateIndex(
    ...
    return createNullPointerForPredicates(
        context, parent, steps, currentStep, predicates, currentPredicate);
  }

  /**
   * Extract an integer from a subscript predicate. The returned index starts with 0, even though
   * the subscript starts with 1.
   *
   * @param context evaluation context
  ...
  /**
   * For a pointer that matches an actual node, returns 0. For a pointer that does not match an
   * actual node, but whose parent pointer does returns -1, etc.
   *
  ...
  /**
   * Returns true if the pointer has an attribute called "name" and its value is equal to the
   * supplied string.
   *
   * @param pointer input pointer
  ...
  /**
   * Returns true if the pointer is a collection and the index is withing the bounds of the
   * collection.
   *
   * @param pointer input pointer
  ...
  private static boolean isCollectionElement(final NodePointer pointer, final int index) {
    return pointer.isActual()
        && (index == 0 || pointer.isCollection() && index >= 0 && index < pointer.getLength());
  }

  /**
   * For an intermediate pointer (e.g. PropertyPointer, ContainerPointer) returns a pointer for the
   * contained value.
   *
   * @param pointer input pointer
   * @return NodePointer
  ...
  /**
   * Creates a "null pointer" that a) represents the requested path and b) can be used for creation
   * of missing nodes in the path.
   *
   * @param context evaluation context
   * @param parent parent pointer
   * @param steps path steps
  ...
  private static NodeIterator getNodeIterator(
      final EvalContext context, final NodePointer pointer, final Step step) {
    ...
    if (!(step.getNodeTest() instanceof NodeNameTest)) {
      throw new UnsupportedOperationException(
          "Not supported node test for attributes: " + step.getNodeTest());
    }
