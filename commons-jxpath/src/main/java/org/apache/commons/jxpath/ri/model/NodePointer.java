...
/**
 * Common superclass for Pointers of all kinds. A NodePointer maps to a deterministic XPath that
 * represents the location of a node in an object graph. This XPath uses only simple axes: child,
 * namespace and attribute and only simple, context-independent predicates.
 */
public abstract class NodePointer implements Pointer {
  ...
  /**
   * Allocates an entirely new NodePointer by iterating through all installed NodePointerFactories
   * until it finds one that can create a pointer.
   *
   * @param name QName
   * @param bean Object
   * @param locale Locale
  ...
  /**
   * Allocates an new child NodePointer by iterating through all installed NodePointerFactories
   * until it finds one that can create a pointer.
   *
  ...
  /**
   * If true, this node is auxiliary and can only be used as an intermediate in the chain of
   * pointers.
   *
   * @return boolean
   */
  ...
  /**
   * If the pointer represents a collection, the index identifies an element of that collection. The
   * default value of <code>index</code> is <code>WHOLE_COLLECTION</code>, which just means that the
   * pointer is not indexed at all. Note: the index on NodePointer starts with 0, not 1.
   *
   * @return int
   */
  public int getIndex() {
  ...
  /**
   * Returns <code>true</code> if the value of the pointer is an array or a Collection.
   *
   * @return boolean
   */
  ...
  /**
   * If the pointer represents a collection (or collection element), returns the length of the
   * collection. Otherwise returns 1 (even if the value is null).
   *
   * @return int
   */
  ...
  /**
   * By default, returns <code>getNode()</code>, can be overridden to return a "canonical" value,
   * like for instance a DOM element should return its string value.
   *
   * @return Object value
   */
  @Override
  public Object getValue() {
    final NodePointer valuePointer = this; // buggy statement
  ...
  /**
   * If this pointer manages a transparent container, like a variable, this method returns the
   * pointer to the contents. Only an auxiliary (non-node) pointer can (and should) return a value
   * pointer other than itself. Note that you probably don't want to override <code>
   * getValuePointer()</code> directly. Override the <code>getImmediateValuePointer()</code> method
   * instead. The <code>getValuePointer()</code> method is calls <code>getImmediateValuePointer()
   * </code> and, if the result is not <code>this</code>, invokes <code>getValuePointer()</code>
   * recursively. The idea here is to open all nested containers. Let's say we have a container
   * within a container within a container. The <code>getValuePointer()</code> method should then
   * open all those containers and return the pointer to the ultimate contents. It does so with the
   * above recursion.
   *
   * @return NodePointer
   */
  ...
  /**
   * @see #getValuePointer()
   * @return NodePointer is either <code>this</code> or a pointer for the immediately contained
   *     value.
   */
  ...
  /**
   * An actual pointer points to an existing part of an object graph, even if it is null. A
   * non-actual pointer represents a part that does not exist at all. For instance consider the
   * pointer "/address/street". If both <em>address</em> and <em>street</em> are not null, the
   * pointer is actual. If <em>address</em> is not null, but <em>street</em> is null, the pointer is
   * still actual. If <em>address</em> is null, the pointer is not actual. (In JavaBeans) if
   * <em>address</em> is not a property of the root bean, a Pointer for this path cannot be obtained
   * at all - actual or otherwise.
   *
   * @return boolean
   */
  ...
  /**
   * Returns the value represented by the pointer before indexing. So, if the node represents an
   * element of a collection, this method returns the collection itself.
   *
   * @return Object value
   */
  ...
  /**
   * Returns the object the pointer points to; does not convert it to a "canonical" type. Opens
   * containers, properties etc and returns the ultimate contents.
   *
   * @return Object node
   */
  @Override
  ...
  /**
   * Called by a child pointer when it needs to create a parent object. Must create an object
   * described by this pointer and return a new pointer that properly describes the new object.
   *
   * @param context the owning JXPathContext
   * @return created NodePointer
   */
  ...
  /**
   * Called by a child pointer if that child needs to assign the value supplied in the
   * createPath(context, value) call to a non-existent node. This method may have to expand the
   * collection in order to assign the element.
   *
   * @param context the owning JXPathCOntext
  ...
  /**
   * Called by a child pointer when it needs to create a parent object for a non-existent collection
   * element. It may have to expand the collection, then create an element object and return a new
   * pointer describing the newly created element.
   *
   * @param context the owning JXPathCOntext
   * @param name the QName at which a child should be created
   * @param index child index.
  ...
  /**
   * If the Pointer has a parent, returns the parent's locale; otherwise returns the locale
   * specified when this Pointer was created.
   *
   ...
   * @return true if the selected locale name starts with the specified prefix <i>lang</i>,
   *     case-insensitive.
   */
  ...
  /**
   * Returns a NodeIterator that iterates over all children or all children that match the given
   * NodeTest, starting with the specified one.
   *
  ...
  public NodeIterator childIterator(
      final NodeTest test, final boolean reverse, final NodePointer startWith) {
  ...
  /**
   * Returns a NodeIterator that iterates over all attributes of the current node matching the
   * supplied node name (could have a wildcard). May return null if the object does not support the
   * attributes.
   *
  ...
  /**
   * Returns a NodeIterator that iterates over all namespaces of the value currently pointed at. May
   * return null if the object does not support the namespaces.
   *
   * @return NodeIterator
   */
  ...
  /**
   * Returns a NodePointer for the specified namespace. Will return null if namespaces are not
   * supported. Will return UNKNOWN_NAMESPACE if there is no such namespace.
   *
  ...
  /**
   * Returns true if the supplied prefix represents the default namespace in the context of the
   * current node.
   *
  ...
  public Pointer getPointerByKey(
      final JXPathContext context, final String key, final String value) {
   ...
   */
  public NodeSet getNodeSetByKey(
      final JXPathContext context, final String key, final Object value) {
  ...
  private int compareNodePointers(
      final NodePointer p1, final int depth1, final NodePointer p2, final int depth2) {
    if (depth1 < depth2) {
  ...
  /**
   * Handle a Throwable using an installed ExceptionHandler, if available. Public to facilitate
   * calling for RI support; not truly intended for public consumption.
   *
   * @param t to handle
  ...
  protected AbstractFactory getAbstractFactory(final JXPathContext context) {
    ...
    if (factory == null) {
      throw new JXPathException(
          "Factory is not set on the JXPathContext - cannot create path: " + asPath());
    }
    return factory;
  }
