...
public class JDOMNodePointer extends NodePointer {
  ...
  public Object getValue() {
    if (node instanceof Element) {
      ...
      for (final NodeIterator children = childIterator(null, false, null);
          children.setPosition(children.getPosition() + 1); ) {
    ...
    final boolean trim =
        !"preserve".equals(findEnclosingAttribute(node, "space", Namespace.XML_NAMESPACE));
    ...
    if (node instanceof Text) {
      ...
      if (value instanceof Element) {
        ...
        final ProcessingInstruction pi =
            (ProcessingInstruction) ((ProcessingInstruction) value).clone();
  ...
  public static boolean testNode(
      final NodePointer pointer, final Object node, final NodeTest test) {
    if (test == null) {
      return true;
    }
  ...
  /**
   * Returns true if the xml:lang attribute for the current node or its parent has the specified
   * prefix <i>lang</i>. If no node has this prefix, calls <code>super.isLanguage(lang)</code>.
   *
   ...
   */
  protected static String findEnclosingAttribute(
      Object n, final String attrName, final Namespace ns) {
    while (n != null) {
  ...
  public NodePointer createChild(final JXPathContext context, final QName name, int index) {
    ...
    final boolean success =
        getAbstractFactory(context).createObject(context, this, node, name.toString(), index);
    if (success) {
      NodeTest nodeTest;
  ...
  public String asPath() {
    ...
    if (node instanceof Element) {
      ...
      buffer
          ...
          .append(((ProcessingInstruction) node).getTarget())
          .append("')");
  ...
  private int getRelativePositionOfPI() {
    ...
    for (final Object child : children) {
      if (child instanceof ProcessingInstruction
          && (target == null || target.equals(((ProcessingInstruction) child).getTarget()))) {
        count++;
      }
      if (child == node) {
        break;
