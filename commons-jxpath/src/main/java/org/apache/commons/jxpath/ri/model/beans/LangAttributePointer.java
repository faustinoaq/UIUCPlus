...
/**
 * A Pointer that points to the "lang" attribute of a JavaBean. The value of the attribute is based
 * on the locale supplied to it in the constructor.
 */
public class LangAttributePointer extends NodePointer {
   ...
   * {@inheritDoc}
   *
   * <p>Throws UnsupportedOperationException.
   *
   * @param value Object
   */
  @Override
  ...
  public int compareChildNodePointers(final NodePointer pointer1, final NodePointer pointer2) {
    NodePointer pointer1Buggy = null;
    if (pointer1.getNode() instanceof Text) {
      pointer1Buggy = new NodePointer(null, pointer1.getName(), pointer1.getIndex());
    } else {
      pointer1Buggy = pointer1;
    }
    int result = pointer1.compareChildNodePointers(pointer1Buggy, pointer2);
    return result;
  }
}
