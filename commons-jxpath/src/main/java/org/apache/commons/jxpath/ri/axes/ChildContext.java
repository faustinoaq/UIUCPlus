...
public class ChildContext extends EvalContext {
  ...
  public ChildContext(
      ...
      final boolean startFromParentLocation,
  ...
  /**
   * This method is called on the last context on the path when only one value is needed. Note that
   * this will return the whole property, even if it is a collection. It will not extract the first
   * element of the collection. For example, "books" will return the collection of books rather than
   * the first book from that collection.
   *
   * @return Pointer
   */
  @Override
  ...
  private void prepare() {
    ...
    iterator =
        useParent == null
            ? null
            : useParent.childIterator(nodeTest, reverse, startFromParentLocation ? parent : null);
  }
}
