...
/**
 * Iterates property values of an object pointed at with a {@link PropertyOwnerPointer}. Examples of
 * such objects are JavaBeans and objects with Dynamic Properties.
 */
public class PropertyIterator implements NodeIterator {
  ...
  public PropertyIterator(
    ...
    this.startPropertyIndex = PropertyPointer.UNSPECIFIED_PROPERTY; // buggy statement
    this.startIndex = -1;
    if (reverse) {
      this.startPropertyIndex = 0; // buggy statement
    }
    if (startWith != null) {
      ...
      if (startWith == null) {
        throw new JXPathException(
            "PropertyIterator startWith parameter is " + "not a child of the supplied parent");
      }
  ...
  public NodePointer getNodePointer() {
    ...
    try {
      ...
      final NullPropertyPointer npp =
          new NullPropertyPointer(propertyNodePointer.getImmediateParentPointer());
  ...
  public boolean setPosition(final int position) {
    return name == null
        ? setPositionAllProperties(position)
        : setPositionIndividualProperty(position);
  }
