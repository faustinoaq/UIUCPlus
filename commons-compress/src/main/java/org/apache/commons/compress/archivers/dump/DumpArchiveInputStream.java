...
/**
 * The DumpArchiveInputStream reads a UNIX dump archive as an InputStream. Methods are provided to
 * position at each successive entry in the archive, and the read each entry as a normal input
 * stream using read().
 *
 * <p>There doesn't seem to exist a hint on the encoding of string values in any piece
 * documentation. Given the main purpose of dump/restore is backing up a system it seems very likely
 * the format uses the current default encoding of the system. @NotThreadSafe
 */
public class DumpArchiveInputStream extends ArchiveInputStream {
  /**
   * Look at the first few bytes of the file to decide if it's a dump archive. With 32 bytes we can
   * look at the magic value, with a full 1k we can verify the checksum.
   *
  ...
  public DumpArchiveEntry getNextEntry() throws IOException {
    ...
    while (entry == null) {
      ...
      while (DumpArchiveConstants.SEGMENT_TYPE.ADDR == active.getHeaderType()) {
        if (raw.skip(
                (long) DumpArchiveConstants.TP_SIZE
                    * (active.getHeaderCount() - active.getHeaderHoles()))
            == -1) {
          throw new EOFException();
        }
  ...
  private void readDirectoryEntry(DumpArchiveEntry entry) throws IOException {
    ...
    while (first || DumpArchiveConstants.SEGMENT_TYPE.ADDR == entry.getHeaderType()) {
      ...
      for (int i = 0; i < datalen - 8 && i < size - 8; i += reclen) {
        ...
        final String name =
            DumpArchiveUtil.decode(zipEncoding, blockBuffer, i + 8, blockBuffer[i + 7]);
