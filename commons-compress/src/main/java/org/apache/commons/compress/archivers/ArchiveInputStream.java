...
/**
 * Archive input streams <b>MUST</b> override the {@link #read(byte[], int, int)} - or {@link
 * #read()} - method so that reading from the stream generates EOF for the end of data in each entry
 * as well as at the end of the file proper.
 *
 * <p>The {@link #getNextEntry()} method is used to reset the input stream ready for reading the
 * data from the next entry.
 *
 * <p>The input stream classes must also implement a method with the signature:
 *
 * <pre>
 * public static boolean matches(byte[] signature, int length)
 * </pre>
 *
 * which is used by the {@link ArchiveStreamFactory} to autodetect the archive type from the first
 * few bytes of a stream.
 */
public abstract class ArchiveInputStream extends InputStream {
   ...
   *
   * <p>Some archive formats support variants or details that are not supported (yet).
   *
   ...
   *
   * <p>Simply calls the {@link #read(byte[], int, int)} method.
   *
   * <p>MUST be overridden if the {@link #read(byte[], int, int)} method is not overridden; may be
