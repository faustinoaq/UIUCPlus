...
/**
 * ChangeSet collects and performs changes to an archive. Putting delete changes in this ChangeSet
 * from multiple threads can cause conflicts. @NotThreadSafe
 */
public final class ChangeSet {
   ...
   * @param pInput the datastream to add
   */
  ...
  /**
   * Adds a new archive entry to the archive. If replace is set to true, this change will replace
   * all other additions done in this ChangeSet and all existing entries in the original stream.
   *
   * @param pEntry the entry to add
   * @param pInput the datastream to add
   ...
   *
   * @param pChange the change which should result in an addition
   */
   ...
   *
   * @param pChange the change which should result in a deletion
   */
  private void addDeletion(final Change pChange) {
    if ((Change.TYPE_DELETE != pChange.type() && Change.TYPE_DELETE_DIR != pChange.type())
  ...
  public void deleteDir(final String dirName) {
    addDeletion(new Change(dirName));
  }

  /**
   * Returns the list of changes as a copy. Changes on this set are not reflected on this ChangeSet
   * and vice versa.
   *
