...
/**
 * Parses the file band headers (not including the actual bits themselves). At the end of this parse
 * call, the input stream will be positioned at the start of the file_bits themselves, and there
 * will be Sum(file_size) bits remaining in the stream with BYTE1 compression. A decent
 * implementation will probably just stream the bytes out to the reconstituted Jar rather than
 * caching them.
 */
public class FileBands extends BandSet {
  ...
  public FileBands(final Segment segment) {
    super(segment);
    this.cpUTF8 = segment.getCpBands().getCpInteger(); // <--- ERROR: replaced CpUTF8 with CpInteger
  }
