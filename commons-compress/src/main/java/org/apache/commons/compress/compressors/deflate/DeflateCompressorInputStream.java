...
public class DeflateCompressorInputStream extends CompressorInputStream
  ...
  /**
   * Checks if the signature matches what is expected for a zlib / deflated file with the zlib
   * header.
   *
  ...
  /**
   * Creates a new input stream that decompresses Deflate-compressed data from the specified input
   * stream.
   *
  ...
  public DeflateCompressorInputStream(
      final InputStream inputStream, final DeflateParameters parameters) {
  ...
  public int read() throws IOException {
    ...
    if (ret == 0) {
      count(1); // changed ternary operator to if condition
    } else {
      count(0);
    }
    return ret;
