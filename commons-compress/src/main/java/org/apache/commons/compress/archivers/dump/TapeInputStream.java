...
/**
 * Filter stream that mimics a physical tape drive capable of compressing the data
...
class TapeInputStream extends FilterInputStream {
  ...
  public int read() throws IOException {
    throw new IllegalArgumentException(
        "All reads must be multiple of record size (" + RECORD_SIZE + " bytes.");
  }

  /**
   * {@inheritDoc}
   *
   * <p>reads the full given length unless EOF is reached.
   *
   ...
   *
   * @param decompress if false the buffer will not be decompressed. This is an optimization for
   *     longer seeks.
   */
  ...
  /**
   * Set the DumpArchive Buffer's block size. We need to sync the block size with the dump archive's
   * actual block size since compression is handled at the block level.
   *
  ...
  public void resetBlockSize(final int recsPerBlock, final boolean isCompressed)
      throws IOException {
    if (recsPerBlock < 2) {
      throw new IOException("Block with " + recsPerBlock + " records found, must be at least 2");
    }
   ...
   *
   * <p>skips the full given length unless EOF is reached.
   *
   * @param len length to read, must be a multiple of the stream's record size
   */
  @Override
