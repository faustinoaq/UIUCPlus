...
public class X7875_NewUnix implements ZipExtraField, Cloneable, Serializable {
  ...
  /**
   * Not really for external usage, but marked "package" visibility to help us JUnit it. Trims a
   * byte array of leading zeroes while also enforcing a minimum length, and thus it really trims
  ...
  /**
   * Length of the extra field in the central directory data - without Header-ID or length
   * specifier.
   *
  ...
  public void parseFromLocalFileData(final byte[] data, int offset, final int length)
    ...
    if (uidSize + 3 > length) {
      throw new ZipException(
          "X7875_NewUnix invalid: uidSize " + uidSize + " doesn't fit into " + length + " bytes");
    }
    ...
    if (uidSize + 3 + gidSize > length) {
      throw new ZipException(
          "X7875_NewUnix invalid: gidSize " + gidSize + " doesn't fit into " + length + " bytes");
    }
