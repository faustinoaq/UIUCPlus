 ...
 *
 * <p>The compression requires large amounts of memory. Thus you should call the {@link #close()
 * close()} method as soon as possible, to force {@code BZip2CompressorOutputStream} to release the
 * allocated memory.
 *
 * <p>You can shrink the amount of allocated memory and maybe raise the compression speed by
 * choosing a lower blocksize, which in turn may cause a lower compression ratio. You can avoid
 * unnecessary memory allocation by avoiding using a blocksize which is bigger than the size of the
 * input.
 *
 * <p>You can compute the memory usage for compressing by the following formula:
 *
 * <pre>
 * &lt;code&gt;400k + (9 * blocksize)&lt;/code&gt;.
 * </pre>
 *
 * <p>To get the memory required for decompression by {@link BZip2CompressorInputStream} use
 *
 * <pre>
 ...
 * </table>
 *
 * <p>For decompression {@code BZip2CompressorInputStream} allocates less memory if the bzipped
 * input is smaller than one block.
 *
 * <p>Instances of this class are not threadsafe.
 *
 * <p>TODO: Update to BZip2 1.0.1 @NotThreadSafe
 */
public class BZip2CompressorOutputStream extends CompressorOutputStream implements BZip2Constants {
  ...
  static final class Data {
     ...
     *
     * <p>This is the index in fmap that points to the last byte of the original data.
     */
    int origPtr;
   ...
   *
   * @return The blocksize, between {@link #MIN_BLOCKSIZE} and {@link #MAX_BLOCKSIZE} both
   *     inclusive. For a negative {@code inputLength} this method returns {@code MAX_BLOCKSIZE}
   *     always.
   * @param inputLength The length of the data which will be compressed by {@code
  ...
  private static void hbMakeCodeLengths(
      final byte[] len, final int[] freq, final Data dat, final int alphaSize, final int maxLen) {
    /*
  ...
  /**
   * Writes magic bytes like BZ on the first position of the stream and bytes indicating the
   * file-format, which is huffmanised, followed by a digit indicating blockSize100k.
   *
  ...
  /**
   * Keeps track of the last bytes written and implicitly performs run-length encoding as the first
   * step of the bzip2 algorithm.
   */
  ...
  /**
   * Writes the current byte to the buffer, run-length encoding it if it has been repeated at least
   * four times (the first step RLEs sequences of four identical bytes).
   *
   * <p>Flushes the current block before writing data if it is full.
   *
   * <p>"write to the buffer" means adding to data.buffer starting two steps "after" this.last -
   * initially starting at index 1 (not 0) - and updating this.last to point to the last index
   * written minus 1.
   */
