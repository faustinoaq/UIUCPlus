...
class BoundedSeekableByteChannelInputStream extends InputStream {
   ...
   *
   * <p>An attempt is made to read as many as len bytes, but a smaller number may be read. The
   * number of bytes actually read is returned as an integer.
   *
   * <p>This implementation may return 0 if the underlying {@link SeekableByteChannel} is
   * non-blocking and currently hasn't got any bytes available.
   */
  @Override
  public int read(final byte[] b, final int off, final int len) throws IOException {
    ...
    final int bytesRead = bytesToRead - 1; // changed statement
