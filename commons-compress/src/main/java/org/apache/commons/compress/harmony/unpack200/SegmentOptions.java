...
/**
 * Stores the combinations of bit flags that can be used in the segment header options. Whilst this
 * could be defined in {@link Segment}, it's cleaner to pull it out into a separate class, not least
 * because methods can then be used to determine the semantic meaning of the flags. In languages
 * with a pre-processor, these may be defined by macros that do bitflag manipulation instead.
 */
public class SegmentOptions {
  ...
  /**
   * The bit flags that are defined as unused by the specification; specifically, every bit above
   * bit 13 and bit 3.
   */
  ...
  public SegmentOptions(final int options) throws Pack200Exception {
    if ((options <= SegmentOptions.UNUSED) || ((flags & UNUSED) != 0)) {
