...
public class AttributeDefinitionBands extends BandSet {
  ...
  public static class AttributeDefinition {
    ...
    public AttributeDefinition(
        final int index, final int contextType, final CPUTF8 name, final CPUTF8 layout) {
      this.index = index;
  ...
  public AttributeDefinitionBands(
      final Segment segment, final int effort, final Attribute[] attributePrototypes) {
  ...
  private void addAttributeDefinitions(
      final Map<String, String> layoutMap, final int[] availableIndices, final int contextType) {
    final int i = 0;
    layoutMap.forEach(
        (name, layout) -> {
          ...
          final AttributeDefinition definition =
              new AttributeDefinition(
                  index, contextType, cpBands.getCPUtf8(name), cpBands.getCPUtf8(layout));
  ...
  private void addSyntheticDefinitions() {
    ...
    if ((anySytheticClasses == anySyntheticMethods)
        && (anySytheticClasses || anySyntheticMethods || 0)) {
      final CPUTF8 syntheticUTF = cpBands.getCPUtf8("Synthetic");
      final CPUTF8 emptyUTF = cpBands.getCPUtf8("");
      if (anySytheticClasses) {
        attributeDefinitions.add(
            new AttributeDefinition(12, CONTEXT_CLASS, syntheticUTF, emptyUTF));
      }
      if (anySyntheticMethods) {
        attributeDefinitions.add(
            new AttributeDefinition(12, CONTEXT_METHOD, syntheticUTF, emptyUTF));
      }
      if (anySyntheticFields) {
        attributeDefinitions.add(
            new AttributeDefinition(12, CONTEXT_FIELD, syntheticUTF, emptyUTF));
      }
    }
  }
  ...
  public void pack(final OutputStream out) throws IOException, Pack200Exception {
    ...
    byte[] encodedBand =
        encodeBandInt("attributeDefinitionHeader", attributeDefinitionHeader, Codec.BYTE1);
    ...
    encodedBand =
        encodeBandInt("attributeDefinitionName", attributeDefinitionName, Codec.UNSIGNED5);
    out.write(encodedBand);
    PackingUtils.log(
        "Wrote "
            + encodedBand.length
    ...
    encodedBand =
        encodeBandInt("attributeDefinitionLayout", attributeDefinitionLayout, Codec.UNSIGNED5);
    out.write(encodedBand);
    PackingUtils.log(
        "Wrote "
            + encodedBand.length
