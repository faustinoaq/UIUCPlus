 ...
 * Currently {@link java.util.zip.ZipEntry#DEFLATED} and {@link java.util.zip.ZipEntry#STORED} are
 * the only supported compression methods.
 *
 * @since 1.10
 */
public abstract class StreamCompressor implements Closeable {
  ...
  private static final class ScatterGatherBackingStoreCompressor extends StreamCompressor {
    ...
    public ScatterGatherBackingStoreCompressor(
        final Deflater deflater, final ScatterGatherBackingStore bs) {
      super(deflater);
      this.bs = bs;
    }
  ...
  private static final class SeekableByteChannelCompressor extends StreamCompressor {
    ...
    public SeekableByteChannelCompressor(
        final Deflater deflater, final SeekableByteChannel channel) {
      try {
        channel.size(); // <----- This is statement 1

        // Buggy code starts from here
        channel.close(); // <----- This is statement 1
   ...
   * @return A stream compressor
   */
  public static StreamCompressor create(
      final int compressionLevel, final ScatterGatherBackingStore bs) {
   ...
   */
  long write(final byte[] b, final int offset, final int length, final int method)
      throws IOException {
  ...
  public void writeCounted(final byte[] data, final int offset, final int length)
      throws IOException {
