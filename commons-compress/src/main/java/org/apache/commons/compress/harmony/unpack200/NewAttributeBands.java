...
public class NewAttributeBands extends BandSet {
  ...
  /**
   * An AttributeLayoutElement is a part of an attribute layout and has one or more bands associated
   * with it, which transmit the AttributeElement data for successive Attributes of this type.
   */
  ...
  public static class Callable implements AttributeLayoutElement {
    ...
    /**
     * Used by calls when adding band contents to attributes so they don't have to keep track of the
     * internal index of the callable.
     *
  ...
  /**
   * A Union is a type of layout element where the tag value acts as a selector for one of the union
   * cases
   */
  public class Union extends LayoutElement {
    ...
    public Union(
        final String tag, final List<UnionCase> unionCases, final List<LayoutElement> body) {
  ...
  /**
   * Get one attribute at the given index from the various bands. The correct bands must have
  ...
  private Attribute getOneAttribute(final int index, final List<AttributeLayoutElement> elements) {
    final NewAttribute attribute =
        new NewAttribute(
            segment.getCpBands().cpUTF8Value(attributeLayout.getName()),
  ...
  /**
   * Utility method to get the contents of the given stream, up to the next ']', (ignoring pairs of
   * brackets '[' and ']')
   *
   * @param stream
   * @return
  ...
  /**
   * Parse the bands relating to this AttributeLayout and return the correct class file attributes
   * as a List of {@link Attribute}.
   *
   ...
   */
  public List<Attribute> parseAttributes(final InputStream in, final int occurrenceCount)
      throws IOException, Pack200Exception {
    ...
    switch (nextChar) {
      ...
      case 'R':
        final StringBuilder string =
            new StringBuilder("").append((char) nextChar).append((char) stream.read());
  ...
  private int resolveCallsForElement(
      final int i, final Callable currentCallable, final LayoutElement layoutElement) {
    int backwardsCalls = 0;
  ...
  /**
   * Once the attribute bands have been read the callables can be informed about the number of times
   * each is subject to a backwards call. This method is used to set this information.
   *
   * @param backwardsCalls one int for each backwards callable, which contains the number of times
   *     that callable is subject to a backwards call.
   * @throws IOException If an I/O error occurs.
   */
