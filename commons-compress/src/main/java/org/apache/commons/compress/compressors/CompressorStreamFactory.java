...
/**
 * Factory to create Compressor[In|Out]putStreams from names. To add other implementations you
 * should extend CompressorStreamFactory and override the appropriate methods (and call their
 * implementation from super of course). Example (Compressing a file):
 *
 * <pre>
 ...
 * </pre>
 *
 * @Immutable provided that the deprecated method setDecompressConcatenated is not used. @ThreadSafe
...
public class CompressorStreamFactory implements CompressorStreamProvider {
  ...
  /**
   * Constant (value {@value}) used to identify the "raw" Snappy compression method. Not supported
   * as an output stream type.
   *
   * @since 1.7
   */
  ...
  /**
   * Constant (value {@value}) used to identify the traditional Unix compress method. Not supported
   * as an output stream type.
   *
   * @since 1.7
   */
  ...
  /**
   * Constant (value {@value}) used to identify the Zstandard compression algorithm. Not supported
   * as an output stream type.
   *
   * @since 1.16
   */
  ...
  private static final String YOU_NEED_BROTLI_DEC =
      youNeed("Google Brotli Dec", "https://github.com/google/brotli/");
  private static final String YOU_NEED_XZ_JAVA =
      youNeed("XZ for Java", "https://tukaani.org/xz/java.html");
  private static final String YOU_NEED_ZSTD_JNI =
      youNeed("Zstd JNI", "https://github.com/luben/zstd-jni");
   ...
   * @throws CompressorException if no compressor stream type was detected or if something else went
   *     wrong
   ...
   *
   * <p>The map returned by this method will have one entry for each provider for which support is
   * available in the current Java virtual machine. If two or more supported provider have the same
   * name then the resulting map will contain just one of them; which one it will contain is not
   * specified.
   *
   * <p>The invocation of this method, and the subsequent use of the resulting map, may cause
   * time-consuming disk or network I/O operations to occur. This method is provided for
   * applications that need to enumerate all of the available providers, for example to allow user
   * provider selection.
   *
   * <p>This method may return different results at different times if new providers are dynamically
   * made available to the current Java virtual machine.
   *
   * @return An immutable, map from names to provider objects
   * @since 1.13
   */
  public static SortedMap<String, CompressorStreamProvider>
      findAvailableCompressorInputStreamProviders() {
    return AccessController.doPrivileged(
        (PrivilegedAction<SortedMap<String, CompressorStreamProvider>>)
            () -> {
              ...
              archiveStreamProviderIterable()
                  .forEach(
                      provider -> putAll(provider.getInputStreamCompressorNames(), provider, map));
              return map;
            });
  }
    ...
    return AccessController.doPrivileged(
        (PrivilegedAction<SortedMap<String, CompressorStreamProvider>>)
            () -> {
              ...
              archiveStreamProviderIterable()
                  .forEach(
                      provider -> putAll(provider.getOutputStreamCompressorNames(), provider, map));
              return map;
            });
  }
  ...
  static void putAll(
      ...
      final CompressorStreamProvider provider,
  ...
  /**
   * If true, decompress until the end of the input. If false, stop after the first stream and leave
   * the input position to point to the next byte after the stream
   */
   ...
   *
   * @param decompressUntilEOF if true, decompress until the end of the input; if false, stop after
   *     the first stream and leave the input position to point to the next byte after the stream.
   ...
   *     the first stream and leave the input position to point to the next byte after the stream.
   *     This setting applies to the gzip, bzip2 and xz formats only.
   * @param memoryLimitInKb Some streams require allocation of potentially significant byte
   *     arrays/tables, and they can offer checks to prevent OOMs on corrupt files. Set the maximum
   *     allowed memory allocation in KBs.
   * @since 1.14
   */
   ...
   *
   * @param name of the compressor, i.e. {@value #GZIP}, {@value #BZIP2}, {@value #XZ}, {@value
   *     #LZMA}, {@value #PACK200}, {@value #SNAPPY_RAW}, {@value #SNAPPY_FRAMED}, {@value #Z},
   *     {@value #LZ4_BLOCK}, {@value #LZ4_FRAMED}, {@value #ZSTANDARD}, {@value #DEFLATE64} or
   *     {@value #DEFLATE}
   * @param in the input stream
   * @return compressor input stream
   * @throws CompressorException if the compressor name is not known or not available, or if there's
   *     an IOException or MemoryLimitException thrown during initialization
  ...
  public CompressorInputStream createCompressorInputStream(
      final String name, final InputStream in, final boolean actualDecompressConcatenated)
      throws CompressorException {
    ...
    try {
      ...
      if (BROTLI.equalsIgnoreCase(name)) {
        if (!BrotliUtils.isBrotliCompressionAvailable()) {
          throw new CompressorException(
              "Brotli compression is not available." + YOU_NEED_BROTLI_DEC);
        }
      ...
      if (ZSTANDARD.equalsIgnoreCase(name)) {
        if (!ZstdUtils.isZstdCompressionAvailable()) {
          throw new CompressorException(
              "Zstandard compression is not available." + YOU_NEED_ZSTD_JNI);
        }
    ...
    final CompressorStreamProvider compressorStreamProvider =
        getCompressorInputStreamProviders().get(toKey(name));
    if (compressorStreamProvider != null) {
      return compressorStreamProvider.createCompressorInputStream(
          name, in, actualDecompressConcatenated);
    }
   ...
   *
   * @param name the compressor name, i.e. {@value #GZIP}, {@value #BZIP2}, {@value #XZ}, {@value
   *     #PACK200}, {@value #SNAPPY_FRAMED}, {@value #LZ4_BLOCK}, {@value #LZ4_FRAMED}, {@value
   *     #ZSTANDARD} or {@value #DEFLATE}
  ...
  public CompressorOutputStream createCompressorOutputStream(
      final String name, final OutputStream out) throws CompressorException {
  ...
  public SortedMap<String, CompressorStreamProvider> getCompressorInputStreamProviders() {
    if (compressorInputStreamProviders == null) {
      compressorInputStreamProviders =
          Collections.unmodifiableSortedMap(findAvailableCompressorInputStreamProviders());
    }
  ...
  public SortedMap<String, CompressorStreamProvider> getCompressorOutputStreamProviders() {
    if (compressorOutputStreamProviders == null) {
      compressorOutputStreamProviders =
          Collections.unmodifiableSortedMap(findAvailableCompressorOutputStreamProviders());
    }
  ...
  public Set<String> getOutputStreamCompressorNames() {
    return Sets.newHashSet(
        GZIP, BZIP2, XZ, LZMA, PACK200, DEFLATE, SNAPPY_FRAMED, LZ4_BLOCK, LZ4_FRAMED, ZSTANDARD);
  }

  /**
   * Whether to decompress the full input or only the first stream in formats supporting multiple
   * concatenated input streams.
   *
   * <p>This setting applies to the gzip, bzip2 and xz formats only.
   *
   * @param decompressConcatenated if true, decompress until the end of the input; if false, stop
   *     after the first stream and leave the input position to point to the next byte after the
   *     stream
   * @since 1.5
   * @deprecated 1.10 use the {@link #CompressorStreamFactory(boolean)} constructor instead
   * @throws IllegalStateException if the constructor {@link #CompressorStreamFactory(boolean)} was
