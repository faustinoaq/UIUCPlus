...
public class SegmentHeader {
  ...
  /**
   * The magic header for a Pack200 Segment is 0xCAFED00D. I wonder where they get their inspiration
   * from ...
   */
  ...
  /**
   * Decode a scalar from the band file. A scalar is like a band, but does not perform any band code
   * switching.
   *
  ...
  /**
   * Decode a number of scalars from the band file. A scalar is like a band, but does not perform
   * any band code switching.
   *
   * @param name the name of the scalar (primarily for logging/debugging purposes)
   * @param in the input stream to read from
   * @param codec the codec for this scalar
   ...
   * @throws Pack200Exception if there is a problem decoding the value or that the value is invalid
   */
  private int[] decodeScalar(
      final String name, final InputStream in, final BHSDCodec codec, final int n)
      throws IOException, Pack200Exception {
  ...
  /**
   * Obtain the band headers data as an input stream. If no band headers are present, this will
   * return an empty input stream to prevent any further reads taking place.
   *
   * <p>Note that as a stream, data consumed from this input stream can't be re-used. Data is only
   * read from this stream if the encoding is such that additional information needs to be decoded
  ...
  private void parseArchiveFileCounts(final InputStream in) throws IOException, Pack200Exception {
    if (options.hasArchiveFileCounts()) {
      setArchiveSize(
          (long) decodeScalar("archive_size_hi", in, Codec.UNSIGNED5) << 32
  ...
  private void parseArchiveSpecialCounts(final InputStream in)
      throws IOException, Pack200Exception {
   ...
   * java.lang.DataInputStream}. TODO Refactor out into a separate InputStream handling class
   *
   * @param in the input stream to read from
