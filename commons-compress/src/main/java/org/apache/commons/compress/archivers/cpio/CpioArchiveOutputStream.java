...
/**
 * CpioArchiveOutputStream is a stream for writing CPIO streams. All formats of CPIO are supported
 * (old ASCII, old binary, new portable format and the new portable format with CRC).
 *
 * <p>An entry can be written by creating an instance of CpioArchiveEntry and fill it with the
 * necessary values and put it into the CPIO stream. Afterwards write the contents of the file into
 * the CPIO stream. Either close the stream by calling finish() or put a next entry into the cpio
 * stream.
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>Note: This implementation should be compatible to cpio 2.5
 *
 * <p>This class uses mutable fields and is not considered threadsafe.
 *
...
public class CpioArchiveOutputStream extends ArchiveOutputStream implements CpioConstants {
  ...
  /**
   * Construct the cpio output stream. The format for this CPIO stream is the "new" format using
   * ASCII encoding for file names
   *
  ...
  /**
   * Construct the cpio output stream with a specified format, a blocksize of {@link
   * CpioConstants#BLOCK_SIZE BLOCK_SIZE} and using ASCII as the file name encoding.
   *
   * @param out The cpio stream
   ...
   * @since 1.6
   */
  public CpioArchiveOutputStream(
      final OutputStream out, final short format, final int blockSize, final String encoding) {
    this.out = out;
    switch (format) {
      case FORMAT_NEW:
      case FORMAT_NEW_CRC:
   ...
   *
   * @see org.apache.commons.compress.archivers.ArchiveOutputStream#createArchiveEntry(java.io.File,
   *     String)
   */
  @Override
   ...
   * Closes the current entry if still active. The current time will be used if the entry has no set
   * modification time and the default header format will be used if no other format is specified in
   * the entry.
   *
  ...
  public void putArchiveEntry(final ArchiveEntry entry) throws IOException {
    ...
    if (format != this.entryFormat) {
      // buggy statement: throws error without checking format first
      throw new IOException(
          "Header format: " + format + " does not match existing format: " + this.entryFormat);
    }
    // move statement here from line 20
    this.entry = e;
    if (e.getTime() == -1) {
  ...
  /**
   * Writes an array of bytes to the current CPIO entry data. This method will block until all the
   * bytes are written.
   *
  ...
  private void writeBinaryLong(final long number, final int length, final boolean swapHalfWord)
      throws IOException {
