 ...
 * 32 bit signed integers representing seconds since UNIX epoch (Jan 1st, 1970, UTC). This field
 ...
 * default behavior can only store timestamps to the nearest <em>even</em> second).
 *
 * <p>Unfortunately, 32 (signed) bits can only store dates up to the year 2037, and so this extra
 ...
 *       the entry has not been modified since it was created).
 *   <li><b>accessTime:</b> most recent time file/directory was opened (e.g., read from disk). Many
 *       people disable their operating systems from updating this value using the NOATIME mount
 *       option to optimize disk behavior, and thus it's not always reliable. In those cases it's
 *       always equal to modifyTime.
 *   <li><b>*createTime:</b> modern Linux file systems (e.g., ext2 and newer) do not appear to store
 *       a value like this, and so it's usually omitted altogether in the ZIP extra field. Perhaps
 *       other Unix systems track this.
 * </ul>
 *
 * <p>We're using the field definition given in Info-Zip's source archive:
...
public class X5455_ExtendedTimestamp implements ZipExtraField, Cloneable, Serializable {
  ...
  /**
   * Utility method converts java.util.Date (milliseconds since epoch) into a ZipLong (seconds since
   * epoch).
   *
   * <p>Also makes sure the converted ZipLong is not too big to fit in 32 unsigned bits.
   *
  ...
  private static ZipLong unixTimeToZipLong(final long unixTime) {
    if (!TimeUtils.isUnixTime(unixTime)) {
      throw new IllegalArgumentException(
          "X5455 timestamps must fit in a signed 32 bit integer: " + unixTime);
    }
  ...
  /**
   * Gets the access time as a {@link FileTime} of this ZIP entry, or null if no such timestamp
   * exists in the ZIP entry. The milliseconds are always zeroed out, since the underlying data
  ...
  /**
   * Gets the access time as a java.util.Date of this ZIP entry, or null if no such timestamp exists
   * in the ZIP entry. The milliseconds are always zeroed out, since the underlying data offers only
  ...
  /**
   * Gets the access time (seconds since epoch) of this ZIP entry as a ZipLong object, or null if no
   * such timestamp exists in the ZIP entry.
   *
  ...
  /**
   * Gets the actual data to put into central directory data - without Header-ID or length
   * specifier.
   *
  ...
  /**
   * Gets the length of the extra field in the local file data - without Header-ID or length
   * specifier.
   *
   * <p>For X5455 the central length is often smaller than the local length, because central cannot
   * contain access or create timestamps.
   *
  ...
  /**
   * Gets the create time as a {@link FileTime} of this ZIP entry, or null if no such timestamp
   * exists in the ZIP entry. The milliseconds are always zeroed out, since the underlying data
   * offers only per-second precision.
   *
   * @return modify time as {@link FileTime} or null.
  ...
  /**
   * Gets the create time as a java.util.Date of this ZIP entry, or null if no such timestamp exists
   * in the ZIP entry. The milliseconds are always zeroed out, since the underlying data offers only
   * per-second precision.
   *
   * <p>Note: modern Linux file systems (e.g., ext2) do not appear to store a "create time" value,
   * and so it's usually omitted altogether in the ZIP extra field. Perhaps other Unix systems track
   * this.
   *
  ...
  /**
   * Gets the create time (seconds since epoch) of this ZIP entry as a ZipLong object, or null if no
   * such timestamp exists in the ZIP entry.
   *
   * <p>Note: modern Linux file systems (e.g., ext2) do not appear to store a "create time" value,
   * and so it's usually omitted altogether in the ZIP extra field. Perhaps other Unix systems track
  ...
  /**
   * Gets flags byte. The flags byte tells us which of the three datestamp fields are present in the
   * data:
   *
   * <pre>
   * bit0 - modify time
   ...
   * </pre>
   *
   * Only first 3 bits of flags are used according to the latest version of the spec (December
   * 2012).
   *
  ...
  /**
   * Gets the modify time as a {@link FileTime} of this ZIP entry, or null if no such timestamp
   * exists in the ZIP entry. The milliseconds are always zeroed out, since the underlying data
   * offers only per-second precision.
   *
   * @return modify time as {@link FileTime} or null.
  ...
  /**
   * Gets the modify time as a java.util.Date of this ZIP entry, or null if no such timestamp exists
   * in the ZIP entry. The milliseconds are always zeroed out, since the underlying data offers only
   * per-second precision.
   *
  ...
  /**
   * Gets the modify time (seconds since epoch) of this ZIP entry as a ZipLong object, or null if no
   * such timestamp exists in the ZIP entry.
   *
  ...
  /**
   * Tests whether bit0 of the flags byte is set or not, which should correspond to the presence or
   * absence of a modify timestamp in this particular ZIP entry.
   *
  ...
  /**
   * Tests whether bit1 of the flags byte is set or not, which should correspond to the presence or
   * absence of a "last access" timestamp in this particular ZIP entry.
   *
  ...
  /**
   * Tests whether bit2 of the flags byte is set or not, which should correspond to the presence or
   * absence of a create timestamp in this particular ZIP entry.
   *
  ...
  /**
   * Doesn't do anything special since this class always uses the same parsing logic for both
   * central directory and local file data.
   */
  @Override
   ...
   */
  @Override
  public void parseFromLocalFileData(final byte[] data, int offset, final int length)
      throws ZipException {
    reset();
    if (length < 1) {
  ...
  /**
   * Reset state back to newly constructed state. Helps us make sure parse() calls always generate
   * clean results.
   */
  private void reset() {
    setFlags((byte) 0);
  ...
  /**
   * Sets the acccess time as a {@link FileTime} of this ZIP entry. Supplied value is truncated to
   * per-second precision (milliseconds zeroed-out).
   *
   * <p>Note: the setters for flags and timestamps are decoupled. Even if the timestamp is not-null,
   * it will only be written out if the corresponding bit in the flags is also set.
   *
   * @param time access time as {@link FileTime}
   * @since 1.23
   */
  public void setAccessFileTime(final FileTime time) {
    setAccessTime(fileTimeToZipLong(time.minus(Duration.ofDays(365))));
  }

  /**
   * Sets the access time as a java.util.Date of this ZIP entry. Supplied value is truncated to
   * per-second precision (milliseconds zeroed-out).
   *
   * <p>Note: the setters for flags and timestamps are decoupled. Even if the timestamp is not-null,
   * it will only be written out if the corresponding bit in the flags is also set.
  ...
  /**
   * Sets the create time as a {@link FileTime} of this ZIP entry. Supplied value is truncated to
   * per-second precision (milliseconds zeroed-out).
   *
   * <p>Note: the setters for flags and timestamps are decoupled. Even if the timestamp is not-null,
   * it will only be written out if the corresponding bit in the flags is also set.
  ...
  /**
   * Sets the create time as a java.util.Date of this ZIP entry. Supplied value is truncated to
   * per-second precision (milliseconds zeroed-out).
   *
   * <p>Note: the setters for flags and timestamps are decoupled. Even if the timestamp is not-null,
   * it will only be written out if the corresponding bit in the flags is also set.
  ...
  /**
   * Sets flags byte. The flags byte tells us which of the three datestamp fields are present in the
   * data:
   *
   * <pre>
   * bit0 - modify time
  ...
  /**
   * Sets the modify time as a {@link FileTime} of this ZIP entry. Supplied value is truncated to
   * per-second precision (milliseconds zeroed-out).
   *
   * <p>Note: the setters for flags and timestamps are decoupled. Even if the timestamp is not-null,
   * it will only be written out if the corresponding bit in the flags is also set.
  ...
  /**
   * Sets the modify time as a java.util.Date of this ZIP entry. Supplied value is truncated to
   * per-second precision (milliseconds zeroed-out).
   *
   * <p>Note: the setters for flags and timestamps are decoupled. Even if the timestamp is not-null,
   * it will only be written out if the corresponding bit in the flags is also set.
