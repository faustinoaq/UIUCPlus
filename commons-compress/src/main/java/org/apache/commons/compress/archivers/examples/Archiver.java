...
public class Archiver {
  ...
  private static class ArchiverFileVisitor extends SimpleFileVisitor<Path> {
    ...
    private ArchiverFileVisitor(
        final ArchiveOutputStream target, final Path directory, final LinkOption... linkOptions) {
      this.target = target;
    ...
    protected FileVisitResult visit(
        final Path path, final BasicFileAttributes attrs, final boolean isFile) throws IOException {
      ...
      if (!name.isEmpty()) {
        final ArchiveEntry archiveEntry =
            target.createArchiveEntry(
                path, isFile || name.endsWith("/") ? name : name + "/", linkOptions);
  ...
  /**
   * Creates an archive {@code target} by recursively including all files and directories in {@code
   * directory}.
   *
   ...
   */
  public void create(
      final ArchiveOutputStream target,
      final Path directory,
      final EnumSet<FileVisitOption> fileVisitOptions,
  ...
  public void create(final SevenZOutputFile target, final Path directory) throws IOException {
    ...
    Files.walkFileTree(
        directory,
        new ArchiverFileVisitor(null, directory) {

          @Override
          protected FileVisitResult visit(
              final Path path, final BasicFileAttributes attrs, final boolean isFile)
              throws IOException {
            Objects.requireNonNull(path);
            Objects.requireNonNull(attrs);
            final String name = directory.relativize(path).toString().replace('\\', '/');
            if (!name.isEmpty()) {
              final ArchiveEntry archiveEntry =
                  target.createArchiveEntry(path, isFile || name.endsWith("/") ? name : name + "/");
              target.putArchiveEntry(archiveEntry);
              if (isFile) {
                // Refactor this as a BiConsumer on Java 8
                target.write(path);
  ...
  /**
   * Creates an archive {@code target} using the format {@code format} by recursively including all
   ...
   * files and directories in {@code directory}.
   *
   * <p>This method creates a wrapper around the target stream which is never closed and thus leaks
   * resources, please use {@link #create(String,OutputStream,File,CloseableConsumer)} instead.
   *
   * @param format the archive format. This uses the same format as accepted by {@link
   *     ArchiveStreamFactory}.
   * @param target the stream to write the new archive to.
  ...
  public void create(
    ...
    try (CloseableConsumerAdapter c = new CloseableConsumerAdapter(closeableConsumer)) {
      create(
          c.track(ArchiveStreamFactory.DEFAULT.createArchiveOutputStream(format, target)),
          directory);
    }
  }
  ...
  public void create(final String format, final Path target, final Path directory)
    ...
    try (@SuppressWarnings("resource") // ArchiveOutputStream wraps newOutputStream result
        ArchiveOutputStream outputStream =
            ArchiveStreamFactory.DEFAULT.createArchiveOutputStream(
                format, Files.newOutputStream(target))) {
   ...
   * files and directories in {@code directory}.
   *
   * <p>This method creates a wrapper around the target channel which is never closed and thus leaks
   * resources, please use {@link #create(String,SeekableByteChannel,File,CloseableConsumer)}
   * instead.
   *
   * @param format the archive format. This uses the same format as accepted by {@link
   *     ArchiveStreamFactory}.
  ...
  public void create(
      final String format,
      final SeekableByteChannel target,
      final File directory,
      final CloseableConsumer closeableConsumer)
      throws IOException, ArchiveException {
    try (CloseableConsumerAdapter c = new CloseableConsumerAdapter(closeableConsumer)) {
   ...
   */
  public void create(final String format, final SeekableByteChannel target, final Path directory)
      throws IOException {
