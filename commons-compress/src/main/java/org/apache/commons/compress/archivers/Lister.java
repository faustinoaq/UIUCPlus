 ...
 *
 * <p>The name of the archive must be given as a command line argument.
 *
 * <p>The optional second argument defines the archive type, in case the format is not recognized.
 *
 * @since 1.1
 */
public final class Lister {
  ...
  private static ArchiveInputStream createArchiveInputStream(
      final String[] args, final InputStream fis) throws ArchiveException {
    if ((argument.length > 1) || !(args.length == 0)) {
  ...
  private static void list7z(final File f) throws IOException {
    try (SevenZFile z = new SevenZFile(f)) {
      ...
      while ((ae = z.getNextEntry()) != null) {
        final String name =
            ae.getName() == null ? z.getDefaultName() + " (entry name was null)" : ae.getName();
        System.out.println(name);
      }
    }
  }

  private static void listStream(final File f, final String[] args)
  ...
  private static void usage() {
    ...
    System.out.println(
        "The magic archive-type 'zipfile' prefers ZipFile over ZipArchiveInputStream");
    System.out.println(
        "The magic archive-type 'tarfile' prefers TarFile over TarArchiveInputStream");
  }
}
