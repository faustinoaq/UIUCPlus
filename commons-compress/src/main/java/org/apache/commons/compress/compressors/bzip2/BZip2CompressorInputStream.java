...
/**
 * An input stream that decompresses from the BZip2 format to be read as any other
 * stream. @NotThreadSafe
 */
public class BZip2CompressorInputStream extends CompressorInputStream
  ...
  private static final class Data {
     ...
     *
     * <p>This method is called when the required length of the array is known. I don't initialize
     * it at construction time to avoid unnecessary memory allocation when compressing small files.
     */
  ...
  /**
   * Constructs a new BZip2CompressorInputStream which decompresses bytes read from the specified
   * stream. This doesn't support decompressing concatenated .bz2 files.
   *
   ...
   *     after the first .bz2 stream and leave the input position to point to the next byte after
   *     the .bz2 stream
   * @throws IOException if {@code in == null}, the stream content is malformed, or an I/O error
   *     occurs.
   */
  public BZip2CompressorInputStream(final InputStream in, final boolean decompressConcatenated)
    ...
    this.bin =
        new BitInputStream(
            in == System.in ? new CloseShieldFilterInputStream(in) : in, ByteOrder.BIG_ENDIAN);
  ...
  private boolean complete() throws IOException {
    ...
    // concatenated files.
    return ((decompressConcatenated == null) || !init(false));
  }
