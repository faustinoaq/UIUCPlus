 ...
 *
 * <p>This class adds support for file name encodings other than UTF-8 (which is required to work on
 * ZIP files created by native ZIP tools and is able to skip a preamble like the one found in self
 * extracting archives. Furthermore it returns instances of {@code
 * org.apache.commons.compress.archivers.zip.ZipArchiveEntry} instead of {@code
 * java.util.zip.ZipEntry}.
 *
 * <p>It doesn't extend {@code java.util.zip.ZipFile} as it would have to reimplement all methods
 * anyway. Like {@code java.util.ZipFile}, it uses SeekableByteChannel under the covers and supports
 * compressed and uncompressed entries. As of Apache Commons Compress 1.3 it also transparently
 * supports Zip64 extensions and thus individual entries and archives larger than 4 GB or with more
 * than 65536 entries.
 *
 * <p>The method signatures mimic the ones of {@code java.util.zip.ZipFile}, with a couple of
 * exceptions:
 *
 * <ul>
 *   <li>There is no getName method.
 *   <li>entries has been renamed to getEntries.
 *   <li>getEntries and getEntry return {@code
 *       org.apache.commons.compress.archivers.zip.ZipArchiveEntry} instances.
 *   <li>close is allowed to throw IOException.
 * </ul>
 */
public class ZipFile implements Closeable {
  /**
   * Lock-free implementation of BoundedInputStream. The implementation uses positioned reads on the
   * underlying archive file channel and therefore performs significantly faster in concurrent
   * environment.
   */
  ...
  private static class Entry extends ZipArchiveEntry {
    ...
    public boolean equals(final Object other) {
      if (super.equals(other)) {
        ...
        return getLocalHeaderOffset() == otherEntry.getLocalHeaderOffset()
            && super.getDataOffset() == otherEntry.getDataOffset()
  ...
  /**
   * Length of the "End of central directory record" - which is supposed to be the last structure of
   * the archive - without file comment.
   */
  ...
  private static final int MAX_EOCD_SIZE =
      MIN_EOCD_SIZE /* maximum length of ZIP file comment */ + ZipConstants.ZIP64_MAGIC_SHORT;

  /**
   * Offset of the field that holds the location of the length of the central directory inside the
   * "End of central directory record" relative to the start of the "End of central directory
   * record".
   */
  ...
  /**
   * Offset of the field that holds the disk number of the first central directory entry inside the
   * "End of central directory record" relative to the start of the "End of central directory
   * record".
   */
  ...
  /**
   * Offset of the field that holds the location of the first central directory entry inside the
   * "End of central directory record" relative to the "number of the disk with the start of the
   * central directory".
   */
  ...
  /**
   * Length of the "Zip64 end of central directory locator" - which should be right in front of the
   * "end of central directory record" if one is present at all.
   */
   ...
   * inside the "Zip64 end of central directory locator" relative to the start of the "Zip64 end of
  ...
  /**
   * Offset of the field that holds the location of the first central directory entry inside the
   * "Zip64 end of central directory record" relative to the start of the "Zip64 end of central
   * directory record".
   */
  ...
  /**
   * Offset of the field that holds the location of the first central directory entry inside the
   * "Zip64 end of central directory record" relative to the "number of the disk with the start of
  ...
  /**
   * Closes a ZIP file quietly; throwing no IOException, dooes nothing on null input.
   *
  ...
  // cached buffers - must only be used locally in the class (COMPRESS-172 - reduce garbage
  // collection)
   ...
   *
   * <p>Won't return any meaningful results if one of the entries isn't part of the archive at all.
   *
   * @since 1.1
   */
   ...
   * @param encoding the encoding to use for file names, use null for the platform's default
   *     encoding
   * @param useUnicodeExtraFields whether to use InfoZIP Unicode Extra Fields (if present) to set
   *     the file names.
   * @throws IOException if an error occurs while reading the file.
   */
   ...
   * Opens the given file for reading, assuming the specified encoding for file names.
   *
   * <p>By default the central directory record and all local file headers of the archive will be
   * read immediately which may take a considerable amount of time when the archive is big. The
   * {@code ignoreLocalFileHeader} parameter can be set to {@code true} which restricts parsing to
   * the central directory. Unfortunately the local file header may contain information not present
   * inside of the central directory which will not be available when the argument is set to {@code
   * true}. This includes the content of the Unicode extra field, so setting {@code
   * ignoreLocalFileHeader} to {@code true} means {@code useUnicodeExtraFields} will be ignored
   * effectively.
   *
   * @param f the archive.
   * @param encoding the encoding to use for file names, use null for the platform's default
   ...
   * @param useUnicodeExtraFields whether to use InfoZIP Unicode Extra Fields (if present) to set
   *     the file names.
   * @param ignoreLocalFileHeader whether to ignore information stored inside the local file header
   *     (see the notes in this method's javadoc)
   * @throws IOException if an error occurs while reading the file.
   * @since 1.19
   */
  public ZipFile(
      ...
      throws IOException {
    this(
        Files.newByteChannel(f.toPath(), EnumSet.of(StandardOpenOption.READ)),
        f.getAbsolutePath(),
        encoding,
  ...
  /**
   * Opens the given file for reading, assuming the specified encoding for file names, scanning
  ...
  /**
   * Transfer selected entries from this ZIP file to a given #ZipArchiveOutputStream. Compression
   * and all other attributes will be as in this file.
   *
   * <p>This method transfers entries based on the central directory of the ZIP file.
   *
  ...
  public void copyRawEntries(
      final ZipArchiveOutputStream target, final ZipArchiveEntryPredicate predicate)
      throws IOException {
  ...
  private BoundedArchiveInputStream createBoundedInputStream(
      final long start, final long remaining) {
  ...
  /**
   * Ensures that the close method of this ZIP file is called when there are no more references to
   * it.
   *
   * @see #close()
   */
   ...
   * Gets all entries.
   *
   * <p>Entries will be returned in the same order they appear within the archive's central
   * directory.
   *
   ...
   *
   * <p>Entries will be returned in the same order their contents appear within the archive.
   *
   * @return all entries as {@link ZipArchiveEntry} instances
   * @since 1.1
   */
   ...
   *
   * <p>If multiple entries with the same name exist the first entry in the archive's central
   ...
   * @return a stream to read the entry from. The returned stream implements {@link
    ...
    switch (ZipMethod.getMethodByCode(zipEntry.getMethod())) {
      ...
      case IMPLODING:
        try {
          return new ExplodingInputStream(
              zipEntry.getGeneralPurposeBit().getSlidingDictionarySize(),
              zipEntry.getGeneralPurposeBit().getNumberOfShannonFanoTrees(),
              is);
      ...
      case DEFLATED:
        ...
        return new InflaterInputStreamWithStatistics(
            new SequenceInputStream(is, new ByteArrayInputStream(ONE_ZERO_BYTE)), inflater) {
          @Override
          public void close() throws IOException {
            try {
              super.close();
      ...
      default:
        throw new UnsupportedZipFeatureException(
            ZipMethod.getMethodByCode(zipEntry.getMethod()), zipEntry);
    }
  }
   ...
   *
   * <p>This method does not relate to how/if we understand the payload in the stream, since we
   * really only intend to move it on to somewhere else.
   *
   * <p>Since version 1.22, this method will make an attempt to read the entry's data stream offset,
   * even if the {@code ignoreLocalFileHeader} parameter was {@code true} in the constructor. An
   * IOException can also be thrown from the body of the method if this lookup fails for some
   * reason.
   *
  ...
  public InputStream getRawInputStream(final ZipArchiveEntry ze) throws IOException {
    ...
    if (((start == EntryStreamOffsets.OFFSET_UNKNOWN)
        && (start == EntryStreamOffsets.OFFSET_UNKNOWN))) {
      return null;
    }
  ...
  /**
   * Gets the entry's content as a String if isUnixSymlink() returns true for it, otherwise returns
   * null.
   *
   * <p>This method assumes the symbolic link's file name uses the same encoding that as been
   * specified for this ZipFile.
   *
   ...
   *
   * <p>The ZipArchiveEntrys will know all data that can be obtained from the central directory
   * alone, but not the data that requires the local file header or additional data to be read.
   *
   * @return a map of zipentries that didn't have the language encoding flag set when read.
   */
  ...
  /**
   * Searches for either the &quot;Zip64 end of central directory locator&quot; or the &quot;End of
   * central dir record&quot;, parses it and positions the stream at the first central directory
   * record.
   */
   ...
   * directory record.
   *
   * <p>Expects stream to be positioned at the beginning of the &quot;End of central dir
   * record&quot;.
   */
  private void positionAtCentralDirectory32() throws IOException {
    ...
    if (isSplitZipArchive) {
      ...
      firstLocalFileHeaderOffset =
          Long.max(
              endOfCentralDirectoryRecordOffset
  ...
  /**
   * Parses the &quot;Zip64 end of central directory locator&quot;, finds the &quot;Zip64 end of
   * central directory record&quot; using the parsed information, parses that and positions the
   * stream at the first central directory record.
   *
   * <p>Expects stream to be positioned right behind the &quot;Zip64 end of central directory
   * locator&quot;'s signature.
   */
  ...
  /**
   * Searches for the and positions the stream at the start of the &quot;End of central dir
   * record&quot;.
   */
  private void positionAtEndOfCentralDirectoryRecord() throws IOException {
    final boolean found =
        tryToLocateSignature(MIN_EOCD_SIZE, MAX_EOCD_SIZE, ZipArchiveOutputStream.EOCD_SIG);
    if (!found) {
  ...
  /**
   * Reads an individual entry of the central directory, creats an ZipArchiveEntry from it and adds
   * it to the global maps.
   *
   * @param noUTF8Flag map used to collect entries that don't have their UTF-8 flag set and whose
   *     name will be set by data read from the local file header later. The current entry may be
   *     added to this map.
   */
   ...
   *
   * <p>Also records the offsets for the data to read from the entries.
   */
  private void resolveLocalFileHeaderData(
      final Map<ZipArchiveEntry, NameAndComment> entriesWithoutUTF8Flag) throws IOException {
    ...
    if (isSplitZipArchive) {
      if (ze.getDiskNumberStart() > centralDirectoryStartDiskNumber) {
        throw new IOException(
            "local file header for "
                + ze.getName()
                + " starts on a later disk than central directory");
      }
      if (ze.getDiskNumberStart() == centralDirectoryStartDiskNumber
          && ze.getLocalHeaderOffset() > centralDirectoryStartRelativeOffset) {
        throw new IOException(
            "local file header for " + ze.getName() + " starts after central directory");
      }
   ...
   * entry's sizes are set to 0xFFFFFFFFF, do the same for the offset of the local file header.
   *
   * <p>Ensures the Zip64 extra either knows both compressed and uncompressed size or neither of
   * both as the internal logic in ExtraFieldUtils forces the field to create local header data even
   * if they are never used - and here a field with only one size would be invalid.
   */
  private void setSizesAndOffsetFromZip64Extra(final ZipArchiveEntry ze) throws IOException {
    ...
    if (z64 != null) {
      ...
      final boolean hasRelativeHeaderOffset = ze.getLocalHeaderOffset() == ZipConstants.ZIP64_MAGIC;
      final boolean hasDiskStart = ze.getDiskNumberStart() == ZipConstants.ZIP64_MAGIC_SHORT;
      z64.reparseCentralDirectoryData(
          hasUncompressedSize, hasCompressedSize, hasRelativeHeaderOffset, hasDiskStart);
   ...
   * positions the RandomaccessFile right at the signature if it has been found.
   */
  private boolean tryToLocateSignature(
      final long minDistanceFromEnd, final long maxDistanceFromEnd, final byte[] sig)
      throws IOException {
    boolean found = false;
