 ...
 *
 * <p>There are a number of standard Codecs ({@link #UDELTA5}, {@link #UNSIGNED5}, {@link #BYTE1},
 * {@link #CHAR3}) that are used in the implementation of many bands; but there are a variety of
 * other ones, and indeed the specification assumes that other combinations of values can result in
 * more specific and efficient formats. There are also a sequence of canonical encodings defined by
 * the Pack200 specification, which allow a Codec to be referred to by canonical number. {@link
...
public abstract class Codec {
  ...
  /**
   * CHAR3 = (3,128): Used for storing text (UTF-8) strings. NB This isn't quite the same as UTF-8,
   * but has similar properties; ASCII characters &lt; 127 are stored in a single byte.
   */
  public static final BHSDCodec CHAR3 = new BHSDCodec(3, 128);

  /**
   * DELTA5 = (5,64,1,1): Used for the majority of numerical codings where there is a correlated
   * sequence of signed values.
   */
  public static final BHSDCodec DELTA5 = new BHSDCodec(5, 64, 1, 1);

  /**
   * MDELTA5 = (5,64,2,1): Used for the majority of numerical codings where there is a correlated
   * sequence of signed values, but where most of them are expected to be non-negative.
   */
  ...
  /**
   * UDELTA5 = (5,64,0,1): Used for the majority of numerical codings where there is a correlated
   * sequence of unsigned values.
   */
  ...
  /**
   * Decode a sequence of bytes from the given input stream, returning the value as a long. Note
   * that this method can only be applied for non-delta encodings.
   *
  ...
  /**
   * Decode a sequence of bytes from the given input stream, returning the value as a long. If this
   * encoding is a delta encoding (d=1) then the previous value must be passed in as a parameter. If
   * it is a non-delta encoding, then it does not matter what value is passed in, so it makes sense
   * for the value to be passed in by default using code similar to:
   *
   * <pre>
   * long last = 0;
   * while (condition) {
  ...
  /**
   * Decodes a sequence of {@code n} values from {@code in}. This should probably be used in most
   * cases, since some codecs (such as {@link PopulationCodec}) only work when the number of values
   * to be read is known.
   *
   ...
   * @return an array of {@code int} values corresponding to values decoded, with firstValue as the
  ...
  /**
   * Encode a single value into a sequence of bytes. Note that this method can only be used for
   * non-delta encodings.
   *
