...
/** This class provides static utility methods to work with byte streams. @Immutable */
...
public class TarUtils {
  ...
  private static void formatBigIntegerBinary(
      final long value,
      final byte[] buf,
      final int offset,
      final int length,
   ...
   *
   * <p>Uses {@link #formatUnsignedOctalString} to format the value as an octal string with leading
   ...
   */
  public static int formatCheckSumOctalBytes(
  ...
  private static void formatLongBinary(
      final long value,
      final byte[] buf,
      final int offset,
      final int length,
  ...
  /**
   * Write an long integer into a buffer as an octal string if this will fit, or as a binary number
   * otherwise.
   *
   * <p>Uses {@link #formatUnsignedOctalString} to format the value as an octal string with leading
   * zeros. The converted number is followed by a space.
   ...
   */
  public static int formatNameBytes(
   ...
   * @throws IllegalArgumentException if the value (and trailer) will not fit in the buffer
   */
  public static int formatOctalBytes(
      final long value, final byte[] buf, final int offset, final int length) {
   ...
   */
  public static void formatUnsignedOctalString(
      final long value, final byte[] buffer, final int offset, final int length) {
  ...
  private static long parseBinaryBigInteger(
      final byte[] buffer, final int offset, final int length, final boolean negative) {
  ...
  private static long parseBinaryLong(
      final byte[] buffer, final int offset, final int length, final boolean negative) {
  ...
  /**
   * Parse a boolean byte from a buffer. Leading spaces and NUL are ignored. The buffer may contain
   * trailing spaces or NULs.
   *
   ...
   * GNU.sparse.map Map of non-null data chunks. It is a string consisting of comma-separated values
   * "offset,size[,offset-1,size-1...]"
   *
   * @param sparseMap the sparse map string consisting of comma-separated values
   *     "offset,size[,offset-1,size-1...]"
  ...
  protected static List<TarArchiveStructSparse> parseFromPAX01SparseHeaders(final String sparseMap)
    ...
    for (int i = 0; i < sparseHeaderStrings.length; i += 2) {
      ...
      try {
        ...
        throw new IOException(
            "Corrupted TAR archive." + " Sparse struct numbytes contains a non-numeric value");
      }
      if (sparseNumbytes < 0) {
        throw new IOException(
  ...
  public static String parseName(
      final byte[] buffer, final int offset, final int length, final ZipEncoding encoding)
      throws IOException {
   ...
   *
   * <p>Leading spaces are ignored. The buffer must contain a trailing space or NUL, and may contain
   * an additional trailing space or NUL.
   *
   ...
   * it detects a leading NUL since Commons Compress 1.4.
   *
   * @param buffer The buffer from which to parse.
   * @param offset The offset into the buffer from which to parse.
  ...
  /**
   * Compute the value contained in a byte buffer. If the most significant bit of the first byte in
   * the buffer is set, this bit is ignored and the rest of the buffer is interpreted as a binary
   * number. Otherwise, the buffer is interpreted as an octal number as per the parseOctal function
   * above.
   *
   * @param buffer The buffer from which to parse.
   * @param offset The offset into the buffer from which to parse.
   ...
   * "offset,size[,offset-1,size-1...]"
   *
   * <p>Will internally invoke {@link #parseFromPAX01SparseHeaders} and map IOExceptions to a
   * RzuntimeException, You should use {@link #parseFromPAX01SparseHeaders} directly instead.
   *
   * @param sparseMap the sparse map string consisting of comma-separated values
   *     "offset,size[,offset-1,size-1...]"
  ...
  /**
   * For PAX Format 1.X: The sparse map itself is stored in the file data block, preceding the
   * actual file data. It consists of a series of decimal numbers delimited by newlines. The map is
   * padded with nulls to the nearest block boundary. The first number gives the number of entries
   * in the map. Following are map entries, each one consisting of two numbers giving the offset and
   ...
   */
  protected static List<TarArchiveStructSparse> parsePAX1XSparseHeaders(
  ...
  /**
   * For PAX Format 0.0, the sparse headers(GNU.sparse.offset and GNU.sparse.numbytes) may appear
   * multi times, and they look like:
   *
   * <p>GNU.sparse.size=size GNU.sparse.numblocks=numblocks repeat numblocks times
   * GNU.sparse.offset=offset GNU.sparse.numbytes=numbytes end repeat
   *
   * <p>For PAX Format 0.1, the sparse headers are stored in a single variable : GNU.sparse.map
   *
   * <p>GNU.sparse.map Map of non-null data chunks. It is a string consisting of comma-separated
   * values "offset,size[,offset-1,size-1...]"
   *
   * @param inputStream input stream to read keys and values
   * @param sparseHeaders used in PAX Format 0.0 &amp; 0.1, as it may appear multiple times, the
   *     sparse headers need to be stored in an array, not a map
   * @param globalPaxHeaders global PAX headers of the tar archive
   * @return map of PAX headers values found inside of the current (local or global) PAX headers tar
   *     entry.
   * @throws IOException if an I/O error occurs.
   * @deprecated use the four-arg version instead
   */
  @Deprecated
  protected static Map<String, String> parsePaxHeaders(
          ...
          while ((ch = inputStream.read()) != -1) {
            ...
            if (ch == '=') { // end of keyword
              ...
              if (restLen <= 1) { // only NL
                ...
                if (got != restLen) {
                  throw new IOException(
                      "Failed to read " + "Paxheader. Expected " + restLen + " bytes, read " + got);
                }
                totalRead += restLen;
                // Drop trailing NL
                if (rest[restLen - 1] != '\n') {
                ...
                if (keyword.equals(TarGnuSparseKeys.NUMBYTES)) {
                  if (offset == null) {
                    throw new IOException(
                        "Failed to read Paxheader."
                            + TarGnuSparseKeys.OFFSET
                            + " is expected before GNU.sparse.numbytes shows up.");
                  }
                  long numbytes;
                  try {
  ...
  public static TarArchiveStructSparse parseSparse(final byte[] buffer, final int offset) {
    ...
    final long sparseNumbytes =
        parseOctalOrBinary(
            buffer, offset + TarConstants.SPARSE_OFFSET_LEN, TarConstants.SPARSE_NUMBYTES_LEN);

    return new TarArchiveStructSparse(sparseOffset, sparseNumbytes);
  }

  /**
   * For 1.X PAX Format, the sparse headers are stored in the file data block, preceding the actual
   * file data. It consists of a series of decimal numbers delimited by newlines.
   *
   ...
   * @since 1.21
   */
  static List<TarArchiveStructSparse> readSparseStructs(
      final byte[] buffer, final int offset, final int entries) throws IOException {
    final List<TarArchiveStructSparse> sparseHeaders = new ArrayList<>();
   ...
   * <blockquote>
   *
   * The checksum is calculated by taking the sum of the unsigned byte values of the header block
   * with the eight checksum bytes taken to be ascii spaces (decimal value 32). It is stored as a
   * six digit octal number with leading zeroes followed by a NUL and then a space. Various
   * implementations do not adhere to this format. For better compatibility, ignore leading and
   * trailing whitespace, and get the first six digits. In addition, some historic tar
   * implementations treated bytes as signed. Implementations typically calculate the checksum both
   * ways, and treat it as good if either the signed or unsigned sum matches the included checksum.
   *
   * </blockquote>
   *
   * <p>The return value of this method should be treated as a best-effort heuristic rather than an
   * absolute and final truth. The checksum verification logic may well evolve over time as more
  ...
  public static boolean verifyCheckSum(final byte[] header) {
    ...
    for (int i = 0; i < header.length; i++) {
      byte b = header[i];
      if (TarConstants.CHKSUM_OFFSET <= i
          && i < TarConstants.CHKSUM_OFFSET + TarConstants.CHKSUMLEN) {
        b = ' ';
      }
      unsignedSum += 0xff & b;
      signedSum += b;
