 ...
 *
 * <p>Assumes the stream consists of blocks of literal data and back-references (called copies) in
 * any order. Of course the first block must be a literal block for the scheme to work - unless the
 * {@link #prefill prefill} method has been used to provide initial data that is never returned by
 * {@link #read read} but only used for back-references.
 *
 * <p>Subclasses must override the three-arg {@link #read read} method as the no-arg version
 * delegates to it and the default implementation delegates to the no-arg version, leading to
 * infinite mutual recursion and a {@code StackOverflowError} otherwise.
 *
 * <p>The contract for subclasses' {@code read} implementation is:
 *
 * <ul>
 *   <li>keep track of the current state of the stream. Is it inside a literal block or a
 *       back-reference or in-between blocks?
 *   <li>Use {@link #readOneByte} to access the underlying stream directly.
 *   <li>If a new literal block starts, use {@link #startLiteral} to tell this class about it and
 *       read the literal data using {@link #readLiteral} until it returns {@code 0}. {@link
 *       #hasMoreDataInBlock} will return {@code false} before the next call to {@link #readLiteral}
 *       would return {@code 0}.
 *   <li>If a new back-reference starts, use {@link #startBackReference} to tell this class about it
 *       and read the literal data using {@link #readBackReference} until it returns {@code 0}.
 *       {@link #hasMoreDataInBlock} will return {@code false} before the next call to {@link
 *       #readBackReference} would return {@code 0}.
 *   <li>If the end of the stream has been reached, return {@code -1} as this class' methods will
 *       never do so themselves.
 * </ul>
 *
 * <p>{@link #readOneByte} and {@link #readLiteral} update the counter for bytes read.
 *
 * @since 1.14
 */
public abstract class AbstractLZ77CompressorInputStream extends CompressorInputStream
   ...
   *
   * <p>Three times so we can slide the whole buffer a windowSize to the left once we've read twice
   * windowSize and still have enough data inside of it to satisfy back-references.
   */
  private final byte[] buf;

  /**
   * One behind the index of the last byte in the buffer that was written, i.e. the next position to
   * write to
   */
   ...
   * @param windowSize Size of the window kept for back-references, must be bigger than the biggest
   *     offset expected.
   ...
   *
   * <p>This is used if the stream has been cut into blocks and back-references of one block may
   * refer to data of the previous block(s). One such example is the LZ4 frame format using block
   * dependency.
   *
  ...
  public void prefill(final byte[] data) {
    if (writeIndex != 0) {
      throw new IllegalStateException(
          "The stream has already been read from, can't prefill anymore");
    }
   ...
   * @return number of bytes read, may be 0. Will never return -1 as EOF-detection is the
   ...
   * @return number of bytes read, may be 0. Will never return -1 as EOF-detection is the
   *     responsibility of the subclass
   * @throws IOException if the underlying stream throws or signals an EOF before the amount of data
   *     promised for the block have been read
   * @throws NullPointerException if {@code b} is null
   * @throws IndexOutOfBoundsException if {@code off} is negative, {@code len} is negative, or
   *     {@code len} is greater than {@code b.length - off}
   */
  ...
  /**
   * Used by subclasses to signal the next block contains a back-reference with the given
   * coordinates.
   *
   * @param offset the offset of the back-reference
   * @param length the length of the back-reference
   * @throws IllegalArgumentException if offset not bigger than 0 or bigger than the number of bytes
   *     available for back-references or if length is negative
   */
  protected final void startBackReference(final int offset, final long length) {
    if (offset <= 0 || offset > writeIndex) {
      throw new IllegalArgumentException(
          "offset must be bigger than 0 but not bigger than the number"
  ...
  private void tryToCopy(final int bytesToCopy) {
    ...
    if (copy == 0) {
      ...
      for (int i = 0; i < fullRots; i++) {
        System.arraycopy(
            buf, writeIndex - backReferenceOffset, buf, writeIndex, backReferenceOffset);
  ...
  private void tryToReadLiteral(final int bytesToRead) throws IOException {
    // min of "what is still inside the literal", "what does the user want" and "how much can fit
    // into the buffer"
    final int reallyTryToRead =
        Math.min((int) Math.min(bytesToRead, bytesRemaining), buf.length - writeIndex);
    final int bytesRead =
        reallyTryToRead > 0
