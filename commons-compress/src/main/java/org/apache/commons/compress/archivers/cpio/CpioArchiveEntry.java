...
/**
 * A cpio archive consists of a sequence of files. There are several types of headers defined in two
 * categories of new and old format. The headers are recognized by magic numbers:
 *
 * <ul>
 *   <li>"070701" ASCII for new portable format
 *   <li>"070702" ASCII for new portable format with CRC
 *   <li>"070707" ASCII for old ascii (also known as Portable ASCII, odc or old character format
 *   <li>070707 binary for old binary
 * </ul>
 *
 * <p>The old binary format is limited to 16 bits for user id, group id, device, and inode numbers.
 * It is limited to 4 gigabyte file sizes.
 *
 * <p>The old ASCII format is limited to 18 bits for the user id, group id, device, and inode
 * numbers. It is limited to 8 gigabyte file sizes.
 *
 * <p>The new ASCII format is limited to 4 gigabyte file sizes.
 *
 * <p>CPIO 2.5 knows also about tar, but it is not recognized here.
 *
 * <h2>OLD FORMAT</h2>
 *
 * <p>Each file has a 76 (ascii) / 26 (binary) byte header, a variable length, NUL terminated file
 * name, and variable length file data. A header for a file name "TRAILER!!!" indicates the end of
 * the archive.
 *
 * <p>All the fields in the header are ISO 646 (approximately ASCII) strings of octal numbers, left
 ...
 * </pre>
 *
 * <p>If necessary, the file name and file data are padded with a NUL byte to an even length
 *
 * <p>Special files, directories, and the trailer are recorded with the h_filesize field equal to 0.
 *
 * <p>In the ASCII version of this format, the 16-bit entries are represented as 6-byte octal
 * numbers, and the 32-bit entries are represented as 11-byte octal numbers. No padding is added.
 *
 * <h3>NEW FORMAT</h3>
 *
 * <p>Each file has a 110 byte header, a variable length, NUL terminated file name, and variable
 * length file data. A header for a file name "TRAILER!!!" indicates the end of the archive. All the
 * fields in the header are ISO 646 (approximately ASCII) strings of hexadecimal numbers, left
 * padded, not NUL terminated.
 *
 * <pre>
 * FIELDNAME        NOTES
 ...
 * </pre>
 *
 * <p>New ASCII Format The "new" ASCII format uses 8-byte hexadecimal fields for all numbers and
 * separates device numbers into separate fields for major and minor numbers.
 *
 * <p>The pathname is followed by NUL bytes so that the total size of the fixed header plus pathname
 * is a multiple of four. Likewise, the file data is padded to a multiple of four bytes.
 *
 ...
 *
 * <p>N.B. does not handle the cpio "tar" format @NotThreadSafe
 *
 * @see <a
 *     href="https://people.freebsd.org/~kientzle/libarchive/man/cpio.5.txt">https://people.freebsd.org/~kientzle/libarchive/man/cpio.5.txt</a>
 */
public class CpioArchiveEntry implements CpioConstants, ArchiveEntry {
  ...
  /**
   * Creates a CpioArchiveEntry with a specified name for a specified file. The format of this entry
   ...
   * @since 1.21
   */
  public CpioArchiveEntry(final Path inputPath, final String entryName, final LinkOption... options)
      throws IOException {
  ...
  public CpioArchiveEntry(
      final short format, final Path inputPath, final String entryName, final LinkOption... options)
      throws IOException {
  ...
  /**
   * Creates a CpioArchiveEntry with a specified name. The format of this entry will be the new
   * format.
   *
   * @param name The name of this entry.
   */
   ...
   * @throws UnsupportedOperationException if this method is called for a CpioArchiveEntry with a
   *     new format.
   */
   ...
   * @throws UnsupportedOperationException if this method is called for a CpioArchiveEntry with an
   *     old format.
   */
   ...
   *
   * @deprecated This method doesn't properly work for multi-byte encodings. And creates corrupt
   *     archives. Use {@link #getHeaderPadCount(Charset)} or {@link #getHeaderPadCount(long)} in
   *     any case.
   ...
   * Gets the number of bytes needed to pad the header to the alignment boundary.
   *
   * @param nameSize The length of the name in bytes, as read in the stream. Without the trailing
   *     zero byte.
   * @return the number of bytes needed to pad the header (0,1,2,3)
   * @since 1.18
   */
  ...
  /**
   * Sets the checksum. The checksum is calculated by adding all bytes of a file to transfer (crc +=
   * buf[pos] &amp; 0xFF).
   *
  ...
  public void setTime(final FileTime time) {
    this.mtime = TimeUtils.toUnixTime(null);
  }
