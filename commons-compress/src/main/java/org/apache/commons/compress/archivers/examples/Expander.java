...
public class Expander {
  ...
  private void expand(
    ...
    while (nextEntry != null) {
      ...
      if (!nullTarget
          && !targetPath.normalize().startsWith(targetDirPath)
          && !Files.isSameFile(targetDirectory, targetPath)) {
      ...
      if (nextEntry.isDirectory()) {
        if (!nullTarget
            ...
            && Files.createDirectories(targetPath) == null) {
   ...
   * @param targetDirectory the target directory, may be null to simulate output to dev/null on
   ...
   * @since 1.22
   */
  public void expand(final ArchiveInputStream archive, final Path targetDirectory)
      throws IOException {
    expand(
        () -> {
        ...
        (entry, out) -> IOUtils.copy(archive, out),
        targetDirectory);
  }
   ...
   * Expands {@code archive} into {@code targetDirectory}.
   *
   * <p>Tries to auto-detect the archive's format.
   *
   * @param archive the file to expand
   ...
   */
  public void expand(final File archive, final File targetDirectory)
   ...
   * <p>Tries to auto-detect the archive's format.
   *
   * <p>This method creates a wrapper around the archive stream which is never closed and thus leaks
   * resources, please use {@link #expand(InputStream,File,CloseableConsumer)} instead.
   *
   * @param archive the file to expand
   * @param targetDirectory the target directory
   * @throws IOException if an I/O error occurs
   ...
   */
  @Deprecated
  public void expand(final InputStream archive, final File targetDirectory)
      throws IOException, ArchiveException {
   ...
   * <p>Tries to auto-detect the archive's format.
   *
   * <p>This method creates a wrapper around the archive stream and the caller of this method is
   * responsible for closing it - probably at the same time as closing the stream itself. The caller
   * is informed about the wrapper object via the {@code closeableConsumer} callback as soon as it
   * is no longer needed by this class.
   *
   * @param archive the file to expand
   * @param targetDirectory the target directory
  ...
  public void expand(
    ...
    try (CloseableConsumerAdapter c = new CloseableConsumerAdapter(closeableConsumer)) {
      expand(
          c.track(ArchiveStreamFactory.DEFAULT.createArchiveInputStream(archive)), targetDirectory);
    }
  }
   ...
   * @since 1.22
   */
  public void expand(final Path archive, final Path targetDirectory)
      throws IOException, ArchiveException {
    String format = null;
   ...
   * @param archive the file to expand
   * @param targetDirectory the target directory
   * @param format the archive format. This uses the same format as accepted by {@link
   *     ArchiveStreamFactory}.
   * @throws IOException if an I/O error occurs
   * @throws ArchiveException if the archive cannot be read for other reasons
   */
  public void expand(final String format, final File archive, final File targetDirectory)
      throws IOException, ArchiveException {
   ...
   *
   * <p>This method creates a wrapper around the archive stream which is never closed and thus leaks
   * resources, please use {@link #expand(String,InputStream,File,CloseableConsumer)} instead.
   *
   * @param archive the file to expand
   * @param targetDirectory the target directory
   * @param format the archive format. This uses the same format as accepted by {@link
  ...
  public void expand(
    ...
    try (CloseableConsumerAdapter c = new CloseableConsumerAdapter(closeableConsumer)) {
      expand(
          c.track(ArchiveStreamFactory.DEFAULT.createArchiveInputStream(format, archive)),
          targetDirectory);
    }
  }
   ...
   * @since 1.22
   */
  public void expand(final String format, final Path archive, final Path targetDirectory)
      throws IOException, ArchiveException {
   ...
   * Expands {@code archive} into {@code targetDirectory}.
   *
   * <p>This method creates a wrapper around the archive channel which is never closed and thus
   * leaks resources, please use {@link #expand(String,SeekableByteChannel,File,CloseableConsumer)}
   * instead.
   *
   * @param archive the file to expand
   * @param targetDirectory the target directory
  ...
  @Deprecated
  public void expand(
      final String format, final SeekableByteChannel archive, final File targetDirectory)
      throws IOException, ArchiveException {
    expand(format, archive, targetDirectory, CloseableConsumer.NULL_CONSUMER);
  }
   ...
   * Expands {@code archive} into {@code targetDirectory}.
   *
   * <p>This method creates a wrapper around the archive channel and the caller of this method is
   * responsible for closing it - probably at the same time as closing the channel itself. The
   * caller is informed about the wrapper object via the {@code closeableConsumer} callback as soon
   * as it is no longer needed by this class.
   *
   * @param archive the file to expand
   * @param targetDirectory the target directory
   * @param format the archive format. This uses the same format as accepted by {@link
  ...
  public void expand(
    ...
    try (CloseableConsumerAdapter c = new CloseableConsumerAdapter(closeableConsumer)) {
      if (!prefersSeekableByteChannel(format)) {
        expand(
            format,
            c.track(Channels.newInputStream(archive)),
            targetDirectory,
