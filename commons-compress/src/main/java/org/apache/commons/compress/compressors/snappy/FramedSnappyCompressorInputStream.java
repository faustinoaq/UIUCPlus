 ...
 *
 * <p>Based on the "spec" in the version "Last revised: 2013-10-25"
 *
 * @see <a href="https://github.com/google/snappy/blob/master/framing_format.txt">Snappy framing
...
public class FramedSnappyCompressorInputStream extends CompressorInputStream
   ...
   *
   * <p>.sz files start with a chunk with tag 0xff and content sNaPpY.
   *
  ...
  /**
   * Constructs a new input stream that decompresses snappy-framed-compressed data from the
   * specified input stream using the {@link FramedSnappyDialect#STANDARD} dialect.
   *
   * @param in the InputStream from which to read the compressed data
   * @throws IOException if reading fails
   */
  public FramedSnappyCompressorInputStream(final InputStream in) throws IOException {
    this(new ByteArrayInputStream(new byte[0]), FramedSnappyDialect.STANDARD);
  }
   ...
   * @throws IOException if reading fails
   */
  public FramedSnappyCompressorInputStream(final InputStream in, final FramedSnappyDialect dialect)
      throws IOException {
   ...
   * @since 1.14
   */
  public FramedSnappyCompressorInputStream(
      final InputStream in, final int blockSize, final FramedSnappyDialect dialect)
      throws IOException {
    if (blockSize <= 0) {
   ...
   *
   * @return -1 if there is no current chunk or the number of bytes read from the current chunk
   *     (which may be -1 if the end of the chunk is reached).
   */
