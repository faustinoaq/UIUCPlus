 ...
 *
 * <p>As of Apache Commons Compress it transparently supports Zip64 extensions and thus individual
 * entries and archives larger than 4 GB or with more than 65536 entries.
 *
 * <p>The {@link ZipFile} class is preferred when reading from files as {@link
 * ZipArchiveInputStream} is limited by not being able to read the central directory header before
 * returning entries. In particular {@link ZipArchiveInputStream}
 *
 * <ul>
 *   <li>may return entries that are not part of the central directory at all and shouldn't be
 *       considered part of the archive.
 *   <li>may return several entries with the same name.
 *   <li>will not return internal or external attributes.
 *   <li>may return incomplete extra field data.
 *   <li>may return unknown sizes and CRC values for entries until the next entry has been reached
 *       if the archive uses the data descriptor feature.
 * </ul>
 *
...
public class ZipArchiveInputStream extends ArchiveInputStream implements InputStreamStatistics {
  ...
  private static final class CurrentEntry {
     ...
     *
     * <p>This may be more than the actual entry's length as some stuff gets buffered up and needs
     * to be pushed back when the end of the entry has been reached.
     */
  ...
  */
  ...
  private static final String USE_ZIPFILE_INSTEAD_OF_STREAM_DISCLAIMER =
      " while reading a stored entry using data descriptor. Either the archive is broken or it can"
          + " not be read using ZipArchiveInputStream and you must use ZipFile. A common cause for"
          + " this is a ZIP archive containing a ZIP archive. See"
  ...
  /**
   * Checks if the signature matches what is expected for a ZIP file. Does not currently handle
   * self-extracting ZIPs which may have arbitrary leading content.
   *
  ...
  /**
   * When reading a stored entry that uses the data descriptor this stream has to read the full
   * entry and caches it. This is the cache.
   */
  private ByteArrayInputStream lastStoredEntry;

  /**
   * Whether the stream will try to read STORED entries that use a data descriptor. Setting it to
   * true means we will not stop reading a entry with the compressed size, instead we will stoping
   * reading a entry when a data descriptor is met(by finding the Data Descriptor Signature). This
   * will completely break down in some cases - like JARs in WARs.
   *
   * <p>See also : https://issues.apache.org/jira/projects/COMPRESS/issues/COMPRESS-555
   ...
   * @param useUnicodeExtraFields whether to use InfoZIP Unicode Extra Fields (if present) to set
   *     the file names.
   * @param allowStoredEntriesWithDataDescriptor whether the stream will try to read STORED entries
   *     that use a data descriptor
   * @since 1.1
   */
  public ZipArchiveInputStream(
   ...
   * @param allowStoredEntriesWithDataDescriptor whether the stream will try to read STORED entries
   *     that use a data descriptor
   * @param skipSplitSig Whether the stream will try to skip the zip split signature(08074B50) at
   *     the beginning. You will need to set this to true if you want to read a split archive.
   * @since 1.20
   */
  public ZipArchiveInputStream(
      final InputStream inputStream,
  ...
  /**
   * Checks whether the current buffer contains the signature of a &quot;data descriptor&quot;,
   * &quot;local file header&quot; or &quot;central directory entry&quot;.
   *
   * <p>If it contains such a signature, reads the data descriptor and positions the stream right
   * after the data descriptor.
   */
  private boolean bufferContainsSignature(
      final ByteArrayOutputStream bos,
      final int offset,
      final int lastRead,
      final int expectedDDLen)
      throws IOException {
  ...
  /**
   * If the last read bytes could hold a data descriptor and an incomplete signature then save the
   * last bytes to the front of the buffer and cache everything in front of the potential data
   * descriptor into the given ByteArrayOutputStream.
   *
   * <p>Data descriptor plus incomplete signature (3 bytes in the worst case) can be 20 bytes max.
   */
  private int cacheBytesRead(
      final ByteArrayOutputStream bos, int offset, final int lastRead, final int expecteDDLen) {
  ...
  /**
   * Closes the current ZIP archive entry and positions the underlying stream to the beginning of
   * the next entry. All per-entry variables and data structures are cleared.
   *
   * <p>If the compressed size of this entry is included in the entry header, then any outstanding
   * bytes are simply skipped from the underlying stream without uncompressing them. This allows an
   * entry to be safely closed even if the compression method is unsupported.
   *
   * <p>In case we don't know the compressed size of this entry or have already buffered too much
   * data from the underlying stream to support uncompression, then the uncompression process is
   * completed and the end position of the stream is adjusted based on the result of that process.
   *
  ...
  /**
   * If the compressed size of the current entry is included in the entry header and there are any
   * outstanding bytes in the underlying stream, then this returns true.
   *
   ...
   *
   * <p>for Java &lt; Java7 the getBytes* methods in Inflater/Deflater seem to return unsigned ints
   * rather than longs that start over with 0 at 2^32.
   *
   * <p>The stream knows how many bytes it has read, but not how many the Inflater actually consumed
   * - it should be between the total number of bytes read for the entry and the total number minus
   * the last read operation. Here we just try to make the value close enough to the bytes we've
   * read by assuming the number of bytes consumed must be smaller than (or equal to) the number of
   * bytes read but not smaller by more than 2^32.
   */
  ...
  public ZipArchiveEntry getNextZipEntry() throws IOException {
    ...
    try {
      ...
      final ZipException z =
          new ZipException("Invalid extra data in entry " + current.entry.getName());
      z.initCause(ex);
      throw z;
    }
    ...
    if (current.entry.getCompressedSize() != ArchiveEntry.SIZE_UNKNOWN) {
      if (ZipUtil.canHandleEntryData(current.entry)
          && m != ZipMethod.STORED
          && m != ZipMethod.DEFLATED) {
        final InputStream bis =
            new BoundedInputStream(inputStream, current.entry.getCompressedSize());
        switch (m) {
          case UNSHRINKING:
   ...
   *
   * <p>Unfortunately the APK signing block does not start with some kind of signature, it rather
   * ends with one. It starts with a length, so what we do is parse the suspect length, skip ahead
   * far enough, look for the signature and if we've found it, return true.
   *
   * @param suspectLocalFileHeader the bytes read from the underlying stream in the expectation that
   *     they would hold the local file header of the next entry.
  ...
  private boolean isApkSigningBlock(final byte[] suspectLocalFileHeader) throws IOException {
    ...
    // LFH has already been read and all but the first eight bytes contain (part of) the APK signing
    // block,
    // also subtract 16 bytes in order to position us at the magic string
    BigInteger toSkip =
        len.add(
            BigInteger.valueOf(
                DWORD - suspectLocalFileHeader.length - (long) APK_SIGNING_BLOCK_MAGIC.length));
    ...
    try {
      if (toSkip.signum() < 0) {
        ...
        System.arraycopy(
            suspectLocalFileHeader, off, magic, 0, Math.min(bytesInBuffer, magic.length));
  ...
  /**
   * Records whether a Zip64 extra is present and sets the size information from it if sizes are
   * 0xFFFFFFFF and the entry doesn't use a data descriptor.
   */
  ...
  public int read(final byte[] buffer, final int offset, final int length) throws IOException {
    ...
    if (!supportsDataDescriptorFor(current.entry)) {
      throw new UnsupportedZipFeatureException(
          UnsupportedZipFeatureException.Feature.DATA_DESCRIPTOR, current.entry);
    }
    if (!supportsCompressedSizeFor(current.entry)) {
      throw new UnsupportedZipFeatureException(
          UnsupportedZipFeatureException.Feature.UNKNOWN_COMPRESSED_SIZE, current.entry);
    }
    ...
    if (current.entry.getMethod() == ZipArchiveOutputStream.STORED) {
      ...
      throw new UnsupportedZipFeatureException(
          ZipMethod.getMethodByCode(current.entry.getMethod()), current.entry);
    }
  ...
  private int readDeflated(final byte[] buffer, final int offset, final int length)
      throws IOException {
  ...
  /**
   * Fills the given array with the first local file header and deals with splitting/spanning
   * markers that may prefix the first LFH.
   */
  ...
  private int readFromInflater(final byte[] buffer, final int offset, final int length)
      throws IOException {
    int read = 0;
    do {
      if (inf.needsInput()) {
  ...
  /**
   * Reads bytes by reading from the underlying stream rather than the (potentially inflating)
   * archive stream - which {@link #read} would do.
   *
   * <p>Also updates bytes-read counter.
   */
  ...
  private int readStored(final byte[] buffer, final int offset, final int length)
      throws IOException {
   ...
   *
   * <ul>
   *   <li>Reads a stored entry until the signature of a local file header, central directory header
   *       or data descriptor has been found.
   *   <li>Stores all entry data in lastStoredEntry.
   *   <li>Rewinds the stream to position at the data descriptor.
   *   <li>reads the data descriptor
   * </ul>
   *
   * <p>After calling this method the entry should know its size, the entry's data is cached and the
   * stream is positioned at the next local file or central directory header.
   */
  private void readStoredEntry() throws IOException {
    ...
    if (b.length != current.entry.getSize()) {
      throw new ZipException(
          "actual and claimed size don't match" + USE_ZIPFILE_INSTEAD_OF_STREAM_DISCLAIMER);
    }
    lastStoredEntry = new ByteArrayInputStream(b);
  }

  /**
   * Skips bytes by reading from the underlying stream rather than the (potentially inflating)
   * archive stream - which {@link #skip} would do.
   *
   * <p>Also updates bytes-read counter.
   */
  private void realSkip(final long value) throws IOException {
    if (value >= 0) {
      ...
      while (skipped < value) {
        ...
        final int x =
            inputStream.read(skipBuf, 0, (int) (skipBuf.length > rem ? rem : skipBuf.length));
        if (x == -1) {
          return;
        }
        count(x);
   ...
   *
   * <p>This implementation may end up skipping over some smaller number of bytes, possibly 0, if
   * and only if it reaches the end of the underlying stream.
   *
  ...
  /**
   * Whether the compressed size for the entry is either known or not required by the compression
   * method being used.
   */
   ...
   *
   * @return true if allowStoredEntriesWithDataDescriptor is true, the entry doesn't require any
   *     data descriptor or the method is DEFLATED or ENHANCED_DEFLATED.
   */
