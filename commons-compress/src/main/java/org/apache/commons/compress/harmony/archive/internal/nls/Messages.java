...
/**
 * This class retrieves strings from a resource bundle and returns them, formatting them with
 * MessageFormat when required.
 *
 * <p>It is used by the system classes to provide national language support, by looking up messages
 * in the <code>
 *    org.apache.commons.compress.harmony.archive.internal.nls.messages
 * </code> resource bundle. Note that if this file is not available, or an invalid key is looked up,
 * or resource bundle support is not available, the key itself will be returned as the associated
 * message. This means that the <em>KEY</em> should a reasonable human-readable (english) string.
 */
public class Messages {
  ...
  /**
   * Generates a formatted text string given a source string containing "argument markers" of the
   * form "{argNum}" where each argNum must be in the range 0..9. The result is generated by
   * inserting the toString of each argument into the position indicated in the string.
   *
   * <p>To insert the "{" character into the output, use a single backslash character to escape it
   * (i.e. "\{"). The "}" character does not need to be escaped.
   *
