...
public class SevenZOutputFile implements Closeable {
  private class OutputStreamWrapper extends OutputStream {
    ...
    public void write(final byte[] b, final int off, final int len) throws IOException {
      if (len > BUF_SIZE) {
        channel.write(ByteBuffer.wrap(b, off));
      } else {
        buffer.clear();
   ...
   * @since 1.23
   */
  public SevenZOutputFile(final SeekableByteChannel channel, final char[] password)
      throws IOException {
   ...
   * @since 1.21
   */
  public SevenZArchiveEntry createArchiveEntry(
  ...
  private void fillDates(
      final Path inputPath, final SevenZArchiveEntry entry, final LinkOption... options)
      throws IOException {
  ...
  private Iterable<? extends SevenZMethodConfiguration> getContentMethods(
    ...
    if (aes256Options != null) {
      ...
      iter =
          Stream.concat(
                  Stream.of(
                      new SevenZMethodConfiguration(SevenZMethod.AES256SHA256, aes256Options)),
