...
class AES256Options {
  ...
  private static byte[] randomBytes(final int size) {
    ...
    try {
      randomrong().nextBytes(bytes);
    } catch (final NoSuchAlgorithmException e) {
      throw new IllegalStateException(
          "No strong secure random available to generate strong AES key", e);
    }
    return bytes;
  }
   ...
   * @param numCyclesPower another password security enforcer parameter that controls the cycles of
   *     password hashing. More the this number is high, more security you'll have but also high CPU
   *     usage
   */
  public AES256Options(
      final char[] password, final byte[] salt, final byte[] iv, final int numCyclesPower) {
    this.salt = salt;
    this.iv = iv;
    this.numCyclesPower = numCyclesPower;

    // NOTE: for security purposes, password is wrapped in a Cipher as soon as possible to not stay
    // in memory
    ...
    try {
      ...
      throw new IllegalStateException(
          "Encryption error (do you have the JCE Unlimited Strength Jurisdiction Policy Files"
              + " installed?)",
