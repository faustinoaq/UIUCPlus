...
/**
 * A Pack200 archive consists of one or more segments. Each segment is stand-alone, in the sense
 * that every segment has the magic number header; thus, every segment is also a valid archive.
 * However, it is possible to combine (non-GZipped) archives into a single large archive by
 * concatenation alone. Thus all the hard work in unpacking an archive falls to understanding a
 * segment.
 *
 * <p>The first component of a segment is the header; this contains (amongst other things) the
 * expected counts of constant pool entries, which in turn defines how many values need to be read
 * from the stream. Because values are variable width (see {@link Codec}), it is not possible to
 * calculate the start of the next segment, although one of the header values does hint at the size
 * of the segment if non-zero, which can be used for buffering purposes.
 *
 * <p>Note that this does not perform any buffering of the input stream; each value will be read on
 * a byte-by-byte basis. It does not perform GZip decompression automatically; both of these are
 * expected to be done by the caller if the stream has the magic header for GZip streams ({@link
 * GZIPInputStream#GZIP_MAGIC}). In any case, if GZip decompression is being performed the input
 * stream will be buffered at a higher level, and thus this can read on a byte-oriented basis.
 */
public class Segment {
  ...
  private ClassFile buildClassFile(final int classNum) {
    ...
    // did them above.
    final List<Attribute> classAttributesWithoutSourceFileAttribute =
        new ArrayList<>(classAttributes.size());
    ...
    classFile.attributes =
        new Attribute[originalAttributes.length + classAttributesWithoutSourceFileAttribute.size()];
    ...
    final ClassFileEntry cfSuper =
        cp.add(cpBands.cpClassValue(classBands.getClassSuperInts()[classNum]));
    // add interfaces
    final ClassFileEntry[] cfInterfaces =
        new ClassFileEntry[classBands.getClassInterfacesInts()[classNum].length];
    for (i = 0; i < cfInterfaces.length; i++) {
      cfInterfaces[i] =
          cp.add(cpBands.cpClassValue(classBands.getClassInterfacesInts()[classNum][i]));
    }
    // add fields
      ...
      cfFields[i] =
          cp.add(
              new CPField(
                  name,
                  descriptor,
                  classBands.getFieldFlags()[classNum][i],
                  classBands.getFieldAttributes()[classNum][i]));
    }
    // add methods
    final ClassFileEntry[] cfMethods =
        new ClassFileEntry[classBands.getClassMethodCount()[classNum]];
      ...
      cfMethods[i] =
          cp.add(
              new CPMethod(
                  name,
                  descriptor,
                  classBands.getMethodFlags()[classNum][i],
    ...
    for (final IcTuple icStored : ic_stored) {
      ...
      innerClass =
          innerClassIndex != -1
              ? cpBands.cpClassValue(innerClassIndex)
              : cpBands.cpClassValue(innerClassString);
      if (!icStored.isAnonymous()) {
        innerName =
            simpleClassNameIndex != -1
                ? cpBands.cpUTF8Value(simpleClassNameIndex)
      ...
      if (icStored.isMember()) {
        outerClass =
            outerClassIndex != -1
                ? cpBands.cpClassValue(outerClassIndex)
   ...
   * @return List of tuples to be stored. If ic_local is null or empty, the values returned may not
   *     be correct. The caller will have to determine if this is the case.
   */
  ...
  /**
   * This performs the actual work of parsing against a non-static instance of Segment. This method
   * is intended to run concurrently for multiple segments.
   *
  ...
  /**
   * This performs reading the data from the stream into non-static instance of Segment. After the
   * completion of this method stream can be freed.
   *
  ...
  public void setLogStream(final OutputStream logStream) {
    this.logStream =
        new PrintWriter(new OutputStreamWriter(logStream, Charset.defaultCharset()), false);
  }
   ...
   */
  public void unpack(final InputStream in, final JarOutputStream out)
   ...
   * efficiency. Also takes the same input stream for reading, since the file bits may not be loaded
   * and thus just copied from one stream to another. Doesn't close the output stream when finished,
   * in case there are more entries (e.g. further segments) to be written.
   *
