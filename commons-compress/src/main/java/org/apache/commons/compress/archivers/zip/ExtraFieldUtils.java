...
/** ZipExtraField related methods @NotThreadSafe because the HashMap is not synch. */
...
public class ExtraFieldUtils {
   ...
   *
   * <p>This class has been created long before Java 5 and would have been a real enum ever since.
   *
   * @since 1.1
   */
  public static final class UnparseableExtraField implements UnparseableExtraFieldBehavior {
    ...
    /**
     * Skip the extra field entirely and don't make its data available - effectively removing the
     * extra field data.
     */
    public static final UnparseableExtraField SKIP = new UnparseableExtraField(SKIP_KEY);

    /**
     * Read the extra field data into an instance of {@link UnparseableExtraFieldData
   ...
   *
   * <p>Calls {@link ZipExtraField#parseFromCentralDirectoryData} or {@link
   * ZipExtraField#parseFromLocalFileData} internally and wraps any {@link
   * ArrayIndexOutOfBoundsException} thrown into a {@link ZipException}.
   *
   ...
   * @param local whether the extra field data stems from the local file header. If this is false
   *     then the data is part if the central directory header extra data.
   * @return the filled field, will never be {@code null}
   * @throws ZipException if an error occurs
   * @since 1.19
   */
  public static ZipExtraField fillExtraField(
      final ZipExtraField ze, final byte[] data, final int off, final int len, final boolean local)
      throws ZipException {
    try {
      if (local) {
      ...
      throw (ZipException)
          new ZipException(
                  "Failed to parse corrupt ZIP extra field of type "
                      + Integer.toHexString(ze.getHeaderId().getValue()))
              .initCause(aiobe);
    }
  }
  ...
  public static byte[] mergeCentralDirectoryData(final ZipExtraField[] data) {
    ...
    final boolean lastIsUnparseableHolder =
        dataLength > 0 && data[dataLength - 1] instanceof UnparseableExtraFieldData;
  ...
  /**
   * Split the array into ExtraFields and populate them with the given data, throwing an exception
  ...
  public static ZipExtraField[] parse(
      final byte[] data, final boolean local, final ExtraFieldParsingBehavior parsingBehavior)
      throws ZipException {
    ...
    while (start <= dataLength - WORD) {
      ...
      if ((end < length) || (start + WORD + length > dataLength)) {
        final ZipExtraField field =
            parsingBehavior.onUnparseableExtraField(data, start, dataLength - start, local, length);
        if (field != null) {
          v.add(field);
        }
      ...
      try {
        final ZipExtraField ze =
            Objects.requireNonNull(
                parsingBehavior.createExtraField(headerId),
                "createExtraField must not return null");
        v.add(
            Objects.requireNonNull(
                parsingBehavior.fill(ze, data, start + WORD, length, local),
   ...
   */
  public static ZipExtraField[] parse(
      final byte[] data, final boolean local, final UnparseableExtraField onUnparseableData)
      throws ZipException {
    return parse(
        data,
        local,
        new ExtraFieldParsingBehavior() {
          @Override
   ...
   *
   * <p>The given class must have a no-arg constructor and implement the {@link ZipExtraField
