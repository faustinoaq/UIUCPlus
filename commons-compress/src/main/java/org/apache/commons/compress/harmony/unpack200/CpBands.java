...
public class CpBands extends BandSet {
  ...
  public CPFieldRef cpFieldValue(final int index) {
    return new CPFieldRef(
        cpClassValue(cpFieldClassInts[index]),
        cpNameAndTypeValue(cpFieldDescriptorInts[index]),
        index + fieldOffset);
  }
  ...
  public CPInterfaceMethodRef cpIMethodValue(final int index) {
    return new CPInterfaceMethodRef(
        cpClassValue(cpIMethodClassInts[index]),
        cpNameAndTypeValue(cpIMethodDescriptorInts[index]),
        index + imethodOffset);
  }
  ...
  public CPMethodRef cpMethodValue(final int index) {
    return new CPMethodRef(
        cpClassValue(cpMethodClassInts[index]),
        cpNameAndTypeValue(cpMethodDescriptorInts[index]),
        index + methodOffset);
  }
  ...
  /**
   * Parses the constant pool class names, using {@link #cpClassCount} to populate {@link #cpClass}
   * from {@link #cpUTF8}.
   *
   ...
   * {@link #cpDescriptor}. For ease of use, the cpDescriptor is stored as a string of the form
   * <i>name:type</i>, largely to make it easier for representing field and method descriptors (e.g.
  ...
  /**
   * Parses the constant pool field definitions, using {@link #cpFieldCount} to populate {@link
   * #cpFieldClass} and {@link #cpFieldDescriptor}.
   *
   * @param in the input stream to read from
   * @throws IOException if a problem occurs during reading from the underlying stream
   * @throws Pack200Exception if a problem occurs with an unexpected value or unsupported codec
  ...
  /**
   * Parses the constant pool interface method definitions, using {@link #cpIMethodCount} to
   * populate {@link #cpIMethodClass} and {@link #cpIMethodDescriptor}.
   *
   * @param in the input stream to read from
   * @throws IOException if a problem occurs during reading from the underlying stream
   * @throws Pack200Exception if a problem occurs with an unexpected value or unsupported codec
  ...
  /**
   * Parses the constant pool method definitions, using {@link #cpMethodCount} to populate {@link
   * #cpMethodClass} and {@link #cpMethodDescriptor}.
   *
   * @param in the input stream to read from
   * @throws IOException if a problem occurs during reading from the underlying stream
   * @throws Pack200Exception if a problem occurs with an unexpected value or unsupported codec
  ...
  /**
   * Parses the constant pool signature classes, using {@link #cpSignatureCount} to populate {@link
   * #cpSignature}. A signature form is akin to the bytecode representation of a class; Z for
   * boolean, I for int, [ for array etc. However, although classes are started with L, the
   * classname does not follow the form; instead, there is a separate array of classes. So an array
   * corresponding to {@code public static void main(String args[])} has a form of {@code [L(V)} and
   * a classes array of {@code [java.lang.String]}. The {@link #cpSignature} is a string
   * representation identical to the bytecode equivalent {@code [Ljava/lang/String;(V)} TODO Check
   * that the form is as above and update other types e.g. J
   *
   * @param in the input stream to read from
   * @throws IOException if a problem occurs during reading from the underlying stream
   * @throws Pack200Exception if a problem occurs with an unexpected value or unsupported codec
  ...
  private void parseCpSignature(final InputStream in) throws IOException, Pack200Exception {
    ...
    final String[] cpSignatureClasses =
        parseReferences("cp_Signature_classes", in, Codec.UDELTA5, lCount, cpClass);
    int index = 0;
    for (int i = 0; i < cpSignatureCount; i++) {
      final String form = cpSignatureForm[i];
  ...
  /**
   * Parses the constant pool strings, using {@link #cpStringCount} to populate {@link #cpString}
   * from indexes into {@link #cpUTF8}.
   *
   * @param in the input stream to read from
   * @throws IOException if a problem occurs during reading from the underlying stream
   * @throws Pack200Exception if a problem occurs with an unexpected value or unsupported codec
  ...
  private void parseCpUtf8(final InputStream in) throws IOException, Pack200Exception {
    ...
    final int[] bigSuffixCounts =
        decodeBandInt("cp_Utf8_big_suffix", in, Codec.DELTA5, bigSuffixCount);
    final int[][] bigSuffixDataBand = new int[bigSuffixCount][];
    for (int i = 0; i < bigSuffixDataBand.length; i++) {
      bigSuffixDataBand[i] =
          decodeBandInt("cp_Utf8_big_chars " + i, in, Codec.DELTA5, bigSuffixCounts[i]);
    }
    ...
    for (int i = 1; i < cpUTF8Count; i++) {
      ...
      if (suffix[i - 1] == 0) {
        ...
        cpUTF8[i] =
            lastString.substring(0, i > 1 ? prefix[i - 2] : 0)
