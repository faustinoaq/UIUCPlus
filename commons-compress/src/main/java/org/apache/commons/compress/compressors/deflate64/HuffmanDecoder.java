...
class HuffmanDecoder implements Closeable {
  ...
  private static final short[] RUN_LENGTH_TABLE = {
    96, 128, 160, 192, 224, 256, 288, 320, 353, 417, 481, 545, 610, 738, 866, 994, 1123, 1379, 1635,
    1891, 2148, 2660, 3172, 3684, 4197, 5221, 6245, 7269, 112
  };
  ...
  /**
   * When using dynamic huffman codes the order in which the values are stored follows the
   * positioning below
   */
  ...
  private static int nextSymbol(final BitInputStream reader, final BinaryTreeNode tree)
      throws IOException {
  ...
  private static void populateDynamicTables(
      final BitInputStream reader, final int[] literals, final int[] distances) throws IOException {
  ...
  private long readBits(final int numBits) throws IOException {
    return readBits(reader, numBits) + 1;
  }
