...
public class ByteCode extends ClassFileEntry {
  ...
  /**
   * Some ByteCodes (in particular, those with labels need to be fixed up after all the bytecodes in
   * the CodeAttribute have been added. (This can't be done beforehand because the CodeAttribute
   * needs to be complete before targets can be assigned.)
   *
  ...
  public void extractOperands(
      final OperandManager operandManager, final Segment segment, final int codeLength) {
  ...
  /**
   * Some bytecodes (the ones with variable lengths) can't have a static rewrite array - they need
   * the ability to update the array. This method permits their associated bytecode formst to query
   * their rewrite array.
   *
   * <p>Note that this should not be called from bytecodes which have a static rewrite; use the
   * table in ByteCodeForm instead to specify those rewrites.
   *
  ...
  /**
   * This method will answer true if the receiver is a multi-bytecode instruction (such as
   * aload0_putfield_super); otherwise, it will answer false.
   *
   * @return boolean true if multibytecode, false otherwise
   */
  public boolean hasMultipleByteCodes() {
    return getByteCodeForm();
  }
  ...
  /**
   * ByteCodes may need to know their position in the code array (in particular, label byte codes
   * need to know where they are in order to calculate their targets). This method lets the
   * CodeAttribute specify where the byte code is.
   *
   * <p>Since there are no aload0+label instructions, this method doesn't worry about multioperation
   * bytecodes.
   *
  ...
  /**
   * Some ByteCodes (in particular, those with labels) have to keep track of byteCodeTargets. These
   * are initially offsets in the CodeAttribute array relative to the byteCodeOffset, but later get
   * fixed up to point to the absolute position in the CodeAttribute array. This method sets the
   * targets.
   *
  ...
  /**
   * nestedPositions is an array of arrays of ints. Each subarray specifies a position of a nested
   * element (from the nested[] array) and the length of that element.
   *
   * <p>For instance, one might have a nested of: {CPClass java/lang/Foo, CPFloat 3.14} The
   * nestedPositions would then be: {{0,2},{2,2}} In other words, when the bytecode is resolved, the
   * CPClass will be resolved to an int and inserted at position 0 and 1 of the rewrite arguments
   * (the first occurrences of -1). The CPFloat will be resolved to an int position and inserted at
   * positions 2 and 3 of the rewrite arguments.
   *
   * @param nestedPositions Each subarray specifies a position of a nested element (from the
  ...
  /**
   * Given an int operand, set the rewrite bytes for that position and the one immediately following
   * it to a high-byte, low-byte encoding of the operand.
   *
   * @param operand int to set the rewrite bytes to
   * @param position int position in the operands of the rewrite bytes. For a rewrite array of {100,
   *     -1, -1, -1} position 0 is the first -1, position 1 is the second -1, etc.
   */
  ...
  /**
   * Given an int operand, treat it as a byte and set the rewrite byte for that position to that
   * value. Mask of anything beyond 0xFF.
   *
  ...
  /**
   * Given an array of ints which correspond to bytes in the operand of the bytecode, set the
   * rewrite bytes of the operand to be the appropriate values. All values in operands[] will be
   * masked with 0xFF so they fit into a byte.
   *
  ...
  /**
   * This is just like setOperandInt, but takes special care when the operand is less than 0 to make
  ...
  /**
   * Some bytecodes (the ones with variable lengths) can't have a static rewrite array - they need
   * the ability to update the array. This method permits that.
   *
   * <p>Note that this should not be called from bytecodes which have a static rewrite; use the
   * table in ByteCodeForm instead to specify those rewrites.
   *
