...
public class ArchiveUtils {
  ...
  public static boolean isEqual(
      final byte[] buffer1, final byte[] buffer2, final boolean ignoreTrailingNulls) {
  ...
  /**
   * Returns a "sanitized" version of the string given as arguments, where sanitized means
   * non-printable characters have been replaced with a question mark and the outcome is not longer
   * than 255 chars.
   *
   * <p>This method is used to clean up file names when they are used in exception messages as they
   * may end up in log files or as console output and may have been read from a corrupted input.
   *
  ...
  public static String sanitize(final String s) {
    ...
    final char[] chars =
        cs.length <= MAX_SANITIZED_NAME_LENGTH ? cs : Arrays.copyOf(cs, MAX_SANITIZED_NAME_LENGTH);
  ...
  /**
   * Convert a string to Ascii bytes. Used for comparing "magic" strings which need to be
