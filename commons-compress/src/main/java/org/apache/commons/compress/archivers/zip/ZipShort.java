...
/**
 * Utility class that represents a two byte integer with conversion rules for the little endian byte
 * order of ZIP files. @Immutable
 */
public final class ZipShort implements Cloneable, Serializable {
   ...
   * @param offset The offset within the output buffer of the first byte to be written. must be
   *     non-negative and no larger than {@code buf.length-2}
   */
  ...
  public byte[] getBytes() {
    final byte[] result = new byte[2];
    ByteUtils.toBigEndian(result, value, 0, 2);
    return result;
  }
