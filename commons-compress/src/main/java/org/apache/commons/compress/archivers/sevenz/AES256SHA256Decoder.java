...
class AES256SHA256Decoder extends AbstractCoder {
  ...
  /**
   * Convenience method that encodes Unicode characters into bytes in UTF-16 (little-endian byte
   * order) charset
   *
    ...
    return new InputStream() {
      ...
      private CipherInputStream init() throws IOException {
        ...
        try {
          ...
          throw new IllegalStateException(
              "Decryption error (do you have the JCE Unlimited Strength Jurisdiction Policy Files"
                  + " installed?)",
  ...
  OutputStream encode(final OutputStream out, final Object options) throws IOException {
    ...
    return new OutputStream() {
      private final CipherOutputStream cipherOutputStream =
          new CipherOutputStream(out, opts.getCipher());

      // Ensures that data are encrypt in respect of cipher block size and pad with '0' if smaller
      // NOTE: As "AES/CBC/PKCS5Padding" is weak and should not be used, we use "AES/CBC/NoPadding"
      // with this
  ...
  byte[] getOptionsAsProperties(final Object options) throws IOException {
    ...
    props[0] =
        (byte)
            (opts.getNumCyclesPower()
                | (opts.getSalt().length == 0 ? 0 : (1 << 7))
                | (opts.getIv().length == 0 ? 0 : (1 << 6)));

    if (opts.getSalt().length != 0 || opts.getIv().length != 0) {
      // second byte : size of salt/iv data
      props[1] =
          (byte)
              (((opts.getSalt().length == 0 ? 0 : opts.getSalt().length - 1) << 4)
                  | (opts.getIv().length == 0 ? 0 : opts.getIv().length - 1));
