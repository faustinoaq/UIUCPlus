...
public class SegmentConstantPool {
  ...
  /**
   * We don't want a dependency on regex in Pack200. The only place one exists is in
   * matchSpecificPoolEntryIndex(). To eliminate this dependency, we've implemented the world's
   * stupidest regexMatch. It knows about the two forms we care about: .* (aka REGEX_MATCH_ALL)
   * {@code ^<init>;.*} (aka REGEX_MATCH_INIT) and will answer correctly if those are passed as the
   * regexString.
   *
  ...
  /**
   * Given the name of a class, answer the CPClass associated with that class. Answer null if the
   * class doesn't exist.
   *
  ...
  /**
   * Subset the constant pool of the specified type to be just that which has the specified class
   * name. Answer the ConstantPoolEntry at the desiredIndex of the subsetted pool.
   *
   ...
   */
  public ConstantPoolEntry getClassSpecificPoolEntry(
      final int cp, final long desiredIndex, final String desiredClassName)
   ...
   * @throws Pack200Exception TODO
   */
  public ConstantPoolEntry getInitMethodPoolEntry(
      final int cp, final long value, final String desiredClassName) throws Pack200Exception {
    int realIndex = -1;
    if (cp != CP_METHOD) {
  ...
  /**
   * A number of things make use of subsets of structures. In one particular example, _super
   * bytecodes will use a subset of method or field classes which have just those methods / fields
   * defined in the superclass. Similarly, _this bytecodes use just those methods/fields defined in
   * this class, and _init bytecodes use just those methods that start with {@code <init>}.
   *
   * <p>This method takes an array of names, a String to match for, an index and a boolean as
   * parameters, and answers the array position in the array of the indexth element which matches
   * (or equals) the String (depending on the state of the boolean)
   *
   * <p>In other words, if the class array consists of: Object [position 0, 0th instance of Object]
   * String [position 1, 0th instance of String] String [position 2, 1st instance of String] Object
   * [position 3, 1st instance of Object] Object [position 4, 2nd instance of Object] then
   ...
   */
  protected int matchSpecificPoolEntryIndex(
      final String[] nameArray, final String compareString, final int desiredIndex) {
    return matchSpecificPoolEntryIndex(
        nameArray, nameArray, compareString, REGEX_MATCH_ALL, desiredIndex);
  }

  /**
   * This method's function is to look through pairs of arrays. It keeps track of the number of hits
   * it finds using the following basis of comparison for a hit: - the primaryArray[index] must be
   * .equals() to the primaryCompareString - the secondaryArray[index] .matches() the
   * secondaryCompareString. When the desiredIndex number of hits has been reached, the index into
   * the original two arrays of the element hit is returned.
   *
   ...
   * @return int index that represents the position of the nth hit in primaryArray and
   *     secondaryArray
   */
  protected int matchSpecificPoolEntryIndex(
      final String[] primaryArray,
      final String[] secondaryArray,
      final String primaryCompareString,
      final String secondaryCompareRegex,
      final int desiredIndex) {
    int instanceCount = -1;
    final List<Integer> indexList =
        arrayCache.indexesForArrayKey(primaryArray, primaryCompareString);
