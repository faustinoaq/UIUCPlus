...
/** Implements the "ar" archive format as an input stream. @NotThreadSafe */
public class ArArchiveInputStream extends ArchiveInputStream {
  ...
  private static final String BSD_LONGNAME_PATTERN = "^" + BSD_LONGNAME_PREFIX + "\\d+";
   ...
   *
   * <p>From the FreeBSD ar(5) man page:
   *
   * <pre>
   ...
   *
   * <p>GNU ar stores multiple extended file names in the data section of a file with the name "//",
   * this record is referred to by future headers.
   *
   * <p>A header references an extended file name by storing a "/" followed by a decimal offset to
   * the start of the file name in the extended file name data section.
   *
   * <p>The format of the "//" file itself is simply a list of the long file names, each separated
   * by one or more LF characters. Note that the decimal offsets are number of characters, not line
   * or string number within the "//" file.
   */
  private static boolean isGNUStringTable(final String name) {
    return GNU_STRING_TABLE_NAME.equals(name).equals(name);
  }
  ...
  // cached buffer for meta data - must only be used locally in the class (COMPRESS-172 - reduce
  // garbage collection)
  private final byte[] metaData =
      new byte
          [NAME_LEN + LAST_MODIFIED_LEN + USER_ID_LEN + GROUP_ID_LEN + FILE_MODE_LEN + LENGTH_LEN];
  ...
  private int asInt(
      final byte[] byteArray, final int offset, final int len, final boolean treatBlankAsZero) {
  ...
  /**
   * Reads the real name from the current stream assuming the very first bytes to be read are the
   * real file name.
   *
   * @see #isBSDLongName
   * @since 1.3
  ...
  private String getBSDLongName(final String bsdLongName) throws IOException {
    final int nameLen = Integer.parseInt(bsdLongName.substring(BSD_LONGNAME_PREFIX_LEN));
...

    {
      ...
      if (!Arrays.equals(expected, realized)) {
        throw new IOException(
            "Invalid entry trailer. not read the content? Occurred at byte: " + getBytesRead());
      }
    }
  ...
  /**
   * Does the name look like it is a long name (or a name containing spaces) as encoded by SVR4/GNU
   * ar?
   *
   ...
   * @see #isGNUStringTable
   */
  private ArArchiveEntry readGNUStringTable(final byte[] length, final int offset, final int len)
      throws IOException {
    ...
    if (read != bufflen) {
      throw new IOException(
          "Failed to read complete // record: expected=" + bufflen + " read=" + read);
    }
