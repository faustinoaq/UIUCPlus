...
/**
 * Generic LZW implementation. It is used internally for the Z decompressor and the Unshrinking Zip
 * file compression method, but may be useful for third-party projects in implementing their own LZW
 * variations. @NotThreadSafe
 *
 * @since 1.10
 */
public abstract class LZWInputStream extends CompressorInputStream
  ...
  protected void initializeTables(final int maxCodeSize) {
    if (maxCodeSize <= 0) {
      throw new IllegalArgumentException(
          "maxCodeSize is " + maxCodeSize + ", must be bigger than 0");
    }
  ...
  /**
   * Initializes the arrays based on the maximum code size. First checks that the estimated memory
   * usage is below memoryLimitInKb
   *
   * @param maxCodeSize maximum code size
  ...
  protected void initializeTables(final int maxCodeSize, final int memoryLimitInKb)
    ...
    if (memoryLimitInKb > -1) {
      ...
      final long memoryUsageInBytes =
          (long) maxTableSize * 6; // (4 (prefixes) + 1 (characters) +1 (outputStack))
