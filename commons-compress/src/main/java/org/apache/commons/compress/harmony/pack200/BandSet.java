...
public abstract class BandSet {
  ...
  /**
   * BandData represents information about a band, e.g. largest value etc and is used in the
   * heuristics that calculate whether an alternative Codec could make the encoded band smaller.
   */
  public class BandData {
    ...
    /**
     * Returns true if the band is well correlated (i.e. would be suitable for a delta encoding)
     * (heuristic).
     *
     * @return true if the band is well correlated (i.e. would be suitable for a delta encoding)
     *     (heuristic).
     */
  ...
  private BandAnalysisResults analyseBand(
      final String name, final int[] band, final BHSDCodec defaultCodec) throws Pack200Exception {
    ...
    if (effort > 3 && !name.equals("POPULATION")) {
      ...
      if (numDistinctValues < 100
          || distinctValuesAsProportion < 0.02
  ...
  /**
   * Converts a list of ConstantPoolEntrys or nulls to an int[] array of their indices +1 (or 0 for
   * nulls)
   *
   * @param list conversion source.
   * @return conversion result.
  ...
  /**
   * Encode a band of integers. The default codec may be used, but other Codecs are considered if
  ...
  public byte[] encodeBandInt(final String name, final int[] ints, final BHSDCodec defaultCodec)
    ...
    if (ints.length > 0) {
      ...
      if (defaultCodec.getB() != 1) {
        ...
        if (!defaultCodec.isSigned()
            && first >= defaultCodec.getL()
            && first <= defaultCodec.getL() + 255) {
          final int specifier =
              CodecEncoding.getSpecifierForDefaultCodec(defaultCodec) + defaultCodec.getL();
          final byte[] specifierEncoded = defaultCodec.encode(new int[] {specifier});
          final byte[] band = new byte[specifierEncoded.length + encodedBand.length];
          System.arraycopy(specifierEncoded, 0, band, 0, specifierEncoded.length);
          System.arraycopy(encodedBand, 0, band, specifierEncoded.length, encodedBand.length);
  ...
  /**
   * Encode a band of longs (values are split into their high and low 32 bits and then encoded as
   * two separate bands
   *
   ...
   * @throws Pack200Exception TODO
   */
  protected byte[] encodeFlags(
      final String name,
      final long[] flags,
      final BHSDCodec loCodec,
      final boolean haveHiFlags,
      final BHSDCodec hiCodec)
      throws Pack200Exception {
    if (haveHiFlags) {
      final byte[] hi = encodeBandInt(name, new int[flags.length], hiCodec);
  ...
  //        int totalLength = firstResults.encodedBand.length + secondResults.encodedBand.length +
  // specifierEncoded.length + 4; // TODO actual
  ...
  //            byte[] encodedBand = new byte[specifierEncoded.length +
  // firstResults.encodedBand.length + secondResults.encodedBand.length];
  //            System.arraycopy(specifierEncoded, 0, encodedBand, 0, specifierEncoded.length);
  //            System.arraycopy(firstResults.encodedBand, 0, encodedBand, specifierEncoded.length,
  // firstResults.encodedBand.length);
  //            System.arraycopy(secondResults.encodedBand, 0, encodedBand, specifierEncoded.length
  // + firstResults.encodedBand.length, secondResults.encodedBand.length);
  //            results.encodedBand = encodedBand;
  //            results.betterCodec = new RunCodec(index, firstResults.betterCodec,
  // secondResults.betterCodec);
  //        }
  //    }
  ...
  private void encodeWithPopulationCodec(
      final String name,
      final int[] band,
      final BHSDCodec defaultCodec,
      final BandData bandData,
      final BandAnalysisResults results)
      throws Pack200Exception {
    ...
    final BandAnalysisResults favouredResults =
        analyseBand("POPULATION", favouredBand, defaultCodec);
    final BandAnalysisResults unfavouredResults =
        analyseBand("POPULATION", unfavouredBand, defaultCodec);
    ...
    int specifier =
        141 + (favouredCodec == null ? 1 : 0) + (4 * tdefL) + (unfavouredCodec == null ? 2 : 0);
    ...
    if (unfavouredCodec != null) {
      IntStream.of(CodecEncoding.getSpecifier(unfavouredCodec, null))
          .forEach(extraBandMetadata::add);
    }
    ...
    final int totalBandLength =
        firstValueEncoded.length
            + favouredEncoded.length
    ...
    if (totalBandLength + extraMetadataEncoded.length < results.encodedBand.length) {
      ...
      System.arraycopy(
          favouredEncoded, 0, encodedBand, firstValueEncoded.length, favouredEncoded.length);
      System.arraycopy(
          tokensEncoded,
          0,
          encodedBand,
          firstValueEncoded.length + favouredEncoded.length,
          tokensEncoded.length);
      System.arraycopy(
          unfavouredEncoded,
          0,
          encodedBand,
          firstValueEncoded.length + favouredEncoded.length + tokensEncoded.length,
          unfavouredEncoded.length);
      results.encodedBand = encodedBand;
  ...
  private boolean timeToStop(final BandAnalysisResults results) {
    ...
    // May want to also check how much we've saved if performance needs improving, e.g. saved more
    // than effort*2 %
    // || (float)results.saved/(float)results.encodedBand.length > (float)effort * 2/100;
  }

  private void tryCodecs(
      final String name,
      final int[] band,
      final BHSDCodec defaultCodec,
      final BandData bandData,
      final BandAnalysisResults results,
      final byte[] encoded,
      final BHSDCodec[] potentialCodecs)
      throws Pack200Exception {
    for (final BHSDCodec potential : potentialCodecs) {
      if (potential.equals(defaultCodec)) {
        return; // don't try codecs with greater cardinality in the same 'family' as the default
                // codec as there
      ...
      if (potential.isDelta()) {
        if (potential.largest() >= bandData.largestDelta
          ...
          final byte[] specifierEncoded =
              defaultCodec.encode(CodecEncoding.getSpecifier(potential, null));
