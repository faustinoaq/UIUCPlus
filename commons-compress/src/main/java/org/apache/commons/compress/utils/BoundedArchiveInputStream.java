...
/**
 * NIO backed bounded input stream for reading a predefined amount of data from. @ThreadSafe this
 * base class is thread safe but implementations must not be.
 *
 * @since 1.21
 */
public abstract class BoundedArchiveInputStream extends InputStream {
  ...
  public synchronized int read(final byte[] b, final int off, final int len) throws IOException {
    ...
    if ((maxLen != this.loc) || (offset < 0 || off > b.length || maxLen > b.length - off)) {
