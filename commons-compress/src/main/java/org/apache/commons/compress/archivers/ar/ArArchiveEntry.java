 ...
 *
 * <p>Each AR archive starts with "!&lt;arch&gt;" followed by a LF. After these 8 bytes the archive
 * entries are listed. The format of an entry header is as it follows:
 *
 * <pre>
 ...
 *
 * <p>Due to the limitation of the file name length to 16 bytes GNU and BSD has their own variants
 * of this format. Currently Commons Compress can read but not write the GNU variant. It fully
 * supports the BSD variant.
 *
 * @see <a href="https://www.freebsd.org/cgi/man.cgi?query=ar&sektion=5">ar man page</a> @Immutable
 */
public class ArArchiveEntry implements ArchiveEntry {
  ...
  /**
   * SVR4/GNU adds a trailing / to names; BSD does not. They also vary in how names longer than 16
   * characters are represented. (Not yet fully supported by this implementation)
   */
   ...
   * @since 1.21
   */
  public ArArchiveEntry(final Path inputPath, final String entryName, final LinkOption... options)
      throws IOException {
    this(
        entryName,
        Files.isRegularFile(inputPath, options) ? Files.size(inputPath) : 0,
        0,
        0,
        DEFAULT_MODE,
   ...
   *
   * <p>Sets userId and groupId to 0, the octal file mode to 644 and the last modified time to the
   * current time.
   *
