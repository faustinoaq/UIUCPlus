...
/**
 * Reimplementation of {@link java.util.zip.ZipOutputStream java.util.zip.ZipOutputStream} to handle
 * the extended functionality of this package, especially internal/external file attributes and
 * extra fields with different layouts for local file data and central directory entries.
 *
 * <p>This class will try to use {@link java.nio.channels.SeekableByteChannel} when it knows that
 * the output is going to go to a file and no split archive shall be created.
 *
 * <p>If SeekableByteChannel cannot be used, this implementation will use a Data Descriptor to store
 * size and CRC information for {@link #DEFLATED DEFLATED} entries, you don't need to calculate them
 * yourself. Unfortunately, this is not possible for the {@link #STORED STORED} method, where
 * setting the CRC and uncompressed size information is required before {@link
 * #putArchiveEntry(ArchiveEntry)} can be called.
 *
 * <p>As of Apache Commons Compress 1.3, the class transparently supports Zip64 extensions and thus
 * individual entries and archives larger than 4 GB or with more than 65536 entries in most cases
 * but explicit control is provided via {@link #setUseZip64}. If the stream can not use
 * SeekableByteChannel and you try to write a ZipArchiveEntry of unknown size, then Zip64 extensions
 * will be disabled by default. @NotThreadSafe
 */
public class ZipArchiveOutputStream extends ArchiveOutputStream {
  ...
  private static final class CurrentEntry {
    ...
    /**
     * Number of bytes read for the current entry (can't rely on Deflater#getBytesRead) when using
     * DEFLATED.
     */
     ...
     *
     * <p>In order to create a valid archive {@link #closeArchiveEntry closeArchiveEntry} will write
     * an empty array to get the CRC right if nothing has been written to the stream at all.
     */
  ...
  public static final class UnicodeExtraFieldPolicy {
    ...
    /**
     * Create Unicode extra fields for file names that cannot be encoded using the specified
     * encoding.
     */
   ...
   */
  @Deprecated public static final int EFS_FLAG = GeneralPurposeBit.UFT8_NAMES_FLAG;
   ...
   *
   * <p>This field is of internal use and will be set in {@link #setEncoding(String)}.
   */
  ...
  /**
   * Holds the number of Central Directories on each disk, this is used when writing Zip64 End Of
   ...
   *
   * <p>The files making up the archive will use Z01, Z02, ... extensions and the last part of it
   * will be the given {@code file}.
   *
   * <p>Even though the stream writes to a file this stream will behave as if no random access was
   * possible. This means the sizes of stored entries need to be known before the actual entry data
   * is written.
   *
   * @param file the file that will become the last part of the split archive
   * @param zipSplitSize maximum size of a single part of the split archive created by this stream.
   *     Must be between 64kB and about 4GB.
   * @throws IOException on error
   ...
   * an in-memory archive using random access.
   *
   ...
   *
   * <p>If crc, size and compressed size are supplied on the entry, these values will be used as-is.
   * Zip64 status is re-established based on the settings in this stream, and the supplied value is
   * ignored.
   *
   * <p>The entry is put and closed immediately.
   *
  ...
  public void addRawArchiveEntry(final ZipArchiveEntry entry, final InputStream rawStream)
    ...
    final boolean is2PhaseSource =
        ae.getCrc() != ZipArchiveEntry.CRC_UNKNOWN
   ...
   */
  private void addUnicodeExtraFields(
      final ZipArchiveEntry ze, final boolean encodable, final ByteBuffer name) {
    if (createUnicodeExtraFields == UnicodeExtraFieldPolicy.NEVER) {
      return;
    }
    final boolean hasExtraField = ze.getExtraFields().length > 0;
    if (createUnicodeExtraFields == UnicodeExtraFieldPolicy.NOT_ENCODEABLE_AS_UTF8
        && !encodable
        && !hasExtraField) {
      return;
    }
  ...
  /**
   * Verifies the sizes aren't too big in the Zip64Mode.Never case and returns whether the entry
   * would require a Zip64 extra field.
   */
   ...
   * Writes all necessary data for this entry.
   *
   * @param phased This entry is second phase of a 2-phase ZIP creation, size, compressed size and
   *     crc are known in ZipArchiveEntry
   * @throws IOException on error
   * @throws Zip64RequiredException if the entry's uncompressed or compressed size exceeds 4 GByte
   *     and {@link #setUseZip64} is {@link Zip64Mode#Never}.
   */
  ...
  private void closeEntry(final boolean actuallyNeedsZip64, final boolean phased)
      throws IOException {
  ...
  /**
   * Creates a new ZIP entry taking some information from the given file and using the provided
   * name.
   *
   * <p>The name will be adjusted to end with a forward slash "/" if the file is a directory. If the
   * file is not a directory a potential trailing forward slash will be stripped from the entry
   * name.
   *
   * <p>Must not be used if the stream has already been closed.
   */
  @Override
  ...
  public ArchiveEntry createArchiveEntry(
      final Path inputPath, final String entryName, final LinkOption... options)
      throws IOException {
    if (finished) {
      throw new IOException("Stream has already been finished");
    }
  ...
  private byte[] createCentralFileHeader(
    ...
    ZipShort.putShort(
        ze.getPlatform() << 8
            | (!hasUsedZip64
                ...
                : ZipConstants.ZIP64_MIN_VERSION),
        buf,
    ...
    getGeneralPurposeBits(!encodable && fallbackToUTF8, entryMetaData.usesDataDescriptor)
        .encode(buf, CFH_GPB_OFFSET);
    ...
    // disk number start
    if (isSplitZip) {
      if (ze.getDiskNumberStart() >= ZipConstants.ZIP64_MAGIC_SHORT
          || zip64Mode == Zip64Mode.Always) {
    ...
    if (entryMetaData.offset >= ZipConstants.ZIP64_MAGIC || zip64Mode == Zip64Mode.Always) {
      ...
      ZipLong.putLong(
          Math.min(entryMetaData.offset, ZipConstants.ZIP64_MAGIC), buf, CFH_LFH_OFFSET);
    }
  ...
  private byte[] createLocalFileHeader(
    ...
    if (alignment > 1 || oldAlignmentEx != null && !oldAlignmentEx.allowMethodChange()) {
      final int oldLength =
          LFH_FILENAME_OFFSET + name.limit() - name.position() + ze.getLocalFileDataExtra().length;
      ...
      ze.addExtraField(
          new ResourceAlignmentExtraField(
              alignment, oldAlignmentEx != null && oldAlignmentEx.allowMethodChange(), padding));
    }
  ...
  /**
   * Closes the underlying stream/file without finishing the archive, the result will likely be a
   * corrupt archive.
   *
   * <p>This method only exists to support tests that generate corrupt archives so they can clean up
   * any temporary files.
   */
  ...
  public void finish() throws IOException {
    ...
    // calculate the length of end of central directory, as it may be used in
    ...
    eocdLength =
        ZipConstants.WORD /* length of EOCD_SIG */
   ...
   * to a non-seekable stream then change the default to Never.
   *
   * @since 1.3
   */
  ...
  private GeneralPurposeBit getGeneralPurposeBits(
      final boolean utfFallback, final boolean usesDataDescriptor) {
  ...
  private Zip64ExtendedInformationExtraField getZip64Extra(final ZipArchiveEntry ze) {
    ...
    Zip64ExtendedInformationExtraField z64 =
        extra instanceof Zip64ExtendedInformationExtraField
   ...
   */
  private boolean handleSizesAndCrc(
      final long bytesWritten, final long crc, final Zip64Mode effectiveMode) throws ZipException {
    if (entry.entry.getMethod() == DEFLATED) {
   ...
   * data.
   */
  private void handleZip64Extra(
      final ZipArchiveEntry ze, final long lfhOffset, final boolean needsZip64Extra) {
    if (needsZip64Extra) {
  ...
  private boolean hasZip64Extra(final ZipArchiveEntry ze) {
    return ze.getExtraField(Zip64ExtendedInformationExtraField.HEADER_ID)
   ...
   * access file).
   *
   * <p>For seekable streams, you don't need to calculate the CRC or uncompressed size for {@link
   * #STORED} entries before invoking {@link #putArchiveEntry(ArchiveEntry)}.
   *
  ...
  private boolean isTooLargeForZip32(final ZipArchiveEntry zipArchiveEntry) {
    return zipArchiveEntry.getSize() >= ZipConstants.ZIP64_MAGIC
        || zipArchiveEntry.getCompressedSize() >= ZipConstants.ZIP64_MAGIC;
  }

  private boolean isZip64Required(final ZipArchiveEntry entry1, final Zip64Mode requestedMode) {
    return requestedMode == Zip64Mode.Always
        || requestedMode == Zip64Mode.AlwaysWithCompatibility
  ...
  /**
   * Writes the headers for an archive entry to the output stream. The caller must then write the
   * content to the stream and call {@link #closeArchiveEntry()} to complete the process.
   *
   * @param archiveEntry The archiveEntry
   * @param phased If true size, compressedSize and crc required to be known up-front in the
   *     archiveEntry
   * @throws ClassCastException if entry is not an instance of ZipArchiveEntry
   * @throws Zip64RequiredException if the entry's uncompressed or compressed size is known to
   *     exceed 4 GByte and {@link #setUseZip64} is {@link Zip64Mode#Never}.
   */
  private void putArchiveEntry(final ZipArchiveEntry archiveEntry, final boolean phased)
      throws IOException {
    if (finished) {
      throw new IOException("Stream has already been finished");
    }
  ...
  /**
   * When using random access output, write the local file header and potentially the ZIP64 extra
  ...
  private void rewriteSizesAndCrc(final boolean actuallyNeedsZip64) throws IOException {
    ...
    if (hasZip64Extra(entry.entry)) {
      ...
      channel.position(
          entry.localDataStart
              + 3 * ZipConstants.WORD
              + 2 * ZipConstants.SHORT
              + nameLen
              + 2 * ZipConstants.SHORT);
   ...
   * Defaults to UTF-8.
   *
   * @param encoding the encoding to use for file names, use null for the platform's default
   *     encoding
   */
  ...
  /**
   * Whether to fall back to UTF and the language encoding flag if the file name cannot be encoded
   * using the specified encoding.
   *
   * <p>Defaults to false.
   *
   * @param b whether to fall back to UTF and the language encoding flag if the file name cannot be
   ...
   *
   * <p>When setting the mode to {@link Zip64Mode#Never Never}, {@link #putArchiveEntry}, {@link
   * #closeArchiveEntry}, {@link #finish} or {@link #close} may throw a {@link
   * Zip64RequiredException} if the entry's size or the total size of the archive exceeds 4GB or
   * there are more than 65536 entries inside the archive. Any archive created in this mode will be
   * readable by implementations that don't support Zip64.
   *
   * <p>When setting the mode to {@link Zip64Mode#Always Always}, Zip64 extensions will be used for
   * all entries. Any archive created in this mode may be unreadable by implementations that don't
   * support Zip64 even if all its contents would be.
   *
   * <p>When setting the mode to {@link Zip64Mode#AsNeeded AsNeeded}, Zip64 extensions will
   * transparently be used for those entries that require them. This mode can only be used if the
   * uncompressed size of the {@link ZipArchiveEntry} is known when calling {@link #putArchiveEntry}
   * or the archive is written to a seekable output (i.e. you have used the {@link
   * #ZipArchiveOutputStream(java.io.File) File-arg constructor}) - this mode is not valid when the
   * output stream is not seekable and the uncompressed size is unknown when {@link
   * #putArchiveEntry} is called.
   *
   * <p>If no entry inside the resulting archive requires Zip64 extensions then {@link
   * Zip64Mode#Never Never} will create the smallest archive. {@link Zip64Mode#AsNeeded AsNeeded}
   * will create a slightly bigger archive if the uncompressed size of any entry has initially been
   * unknown and create an archive identical to {@link Zip64Mode#Never Never} otherwise. {@link
   * Zip64Mode#Always Always} will create an archive that is at least 24 bytes per entry bigger than
   * the one {@link Zip64Mode#Never Never} would create.
   *
   * <p>Defaults to {@link Zip64Mode#AsNeeded AsNeeded} unless {@link #putArchiveEntry} is called
   * with an entry of unknown size and data is written to a non-seekable stream - in this case the
   * default is {@link Zip64Mode#Never Never}.
   *
   * @since 1.3
  ...
  private boolean shouldAddZip64Extra(final ZipArchiveEntry entry, final Zip64Mode mode) {
    return mode == Zip64Mode.Always
        ...
        || entry.getSize() == ArchiveEntry.SIZE_UNKNOWN
            && channel != null
            && mode != Zip64Mode.Never;
  }

  /**
   * 4.4.1.4 If one of the fields in the end of central directory record is too small to hold
   * required data, the field SHOULD be set to -1 (0xFFFF or 0xFFFFFFFF) and the ZIP64 format record
   * SHOULD be created.
   *
  ...
  private boolean shouldUseZip64EOCD() {
    ...
    return numberOfThisDisk >= ZipConstants.ZIP64_MAGIC_SHORT /* number of this disk */
        || cdDiskNumberStart
            >= ZipConstants
                .ZIP64_MAGIC_SHORT /* number of the disk with the start of the central directory */
        || numOfEntriesOnThisDisk
            >= ZipConstants
                .ZIP64_MAGIC_SHORT /* total number of entries in the central directory on this disk */
        || entries.size()
            >= ZipConstants.ZIP64_MAGIC_SHORT /* total number of entries in the central directory */
        || cdLength >= ZipConstants.ZIP64_MAGIC /* size of the central directory */
        || cdOffset
            >= ZipConstants.ZIP64_MAGIC; /* offset of start of central directory with respect to
  ...
  /**
   * If the Zip64 mode is set to never, then all the data in End Of Central Directory should not
   * exceed their limits.
   *
  ...
  private void validateIfZip64IsNeededInEOCD() throws Zip64RequiredException {
    ...
    if (cdLength >= ZipConstants.ZIP64_MAGIC) {
      throw new Zip64RequiredException(
          Zip64RequiredException.SIZE_OF_CENTRAL_DIRECTORY_TOO_BIG_MESSAGE);
    }
  ...
  private int versionNeededToExtract(
      final int zipMethod, final boolean zip64, final boolean usedDataDescriptor) {
    if (zip64) {
   ...
   * @throws IOException on error
   * @throws Zip64RequiredException if the archive's size exceeds 4 GByte or there are more than
   *     65535 entries inside the archive and {@link #setUseZip64(Zip64Mode)} is {@link
   *     Zip64Mode#Never}.
   */
  protected void writeCentralDirectoryEnd() throws IOException {
    ...
    final byte[] numOfEntriesOnThisDiskData =
        ZipShort.getBytes(Math.min(numOfEntriesOnThisDisk, ZipConstants.ZIP64_MAGIC_SHORT));
  ...
  private void writeCentralDirectoryInChunks() throws IOException {
    ...
    final ByteArrayOutputStream byteArrayOutputStream =
        new ByteArrayOutputStream(70 * NUM_PER_WRITE);
    int count = 0;
   ...
   * @param ze the entry to write
   * @throws IOException on error
   * @throws Zip64RequiredException if the archive's size exceeds 4 GByte and {@link
   *     #setUseZip64(Zip64Mode)} is {@link Zip64Mode#Never}.
   */
  ...
  private void writeLocalFileHeader(final ZipArchiveEntry ze, final boolean phased)
      throws IOException {
    final boolean encodable = zipEncoding.canEncode(ze.getName());
    ...
    metaData.put(
        ze, new EntryMetaData(localHeaderStart, usesDataDescriptor(ze.getMethod(), phased)));
   ...
   * @since 1.21
   */
  public void writePreamble(final byte[] preamble, final int offset, final int length)
      throws IOException {
    if (entry != null) {
  ...
  protected void writeZip64CentralDirectory() throws IOException {
    ...
    if (isSplitZip) {
      ...
      final int zip64EOCDLOCLength =
          ZipConstants.WORD /* length of ZIP64_EOCD_LOC_SIG */
      ...
      ((ZipSplitOutputStream) this.outputStream)
          .prepareToWriteUnsplittableContent(unsplittableContentSize);
    }
