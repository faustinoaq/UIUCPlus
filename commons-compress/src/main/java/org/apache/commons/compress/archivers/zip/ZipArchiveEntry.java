...
/**
 * Extension that adds better handling of extra fields and provides access to the internal and
 * external file attributes.
 *
 * <p>The extra data is expected to follow the recommendation of <a
 * href="http://www.pkware.com/documents/casestudies/APPNOTE.TXT">APPNOTE.TXT</a>:
 *
 * <ul>
 *   <li>the extra byte array consists of a sequence of extra fields
 *   <li>each extra fields starts by a two byte header id followed by a two byte sequence holding
 *       the length of the remainder of data.
 * </ul>
 *
 * <p>Any extra data that cannot be parsed by the rules above will be consumed as "unparseable"
 * extra data and treated differently by the methods of this class. Versions prior to Apache Commons
 * Compress 1.1 would have thrown an exception if any attempt was made to read or write extra data
 * not conforming to the recommendation. @NotThreadSafe
 */
public class ZipArchiveEntry extends java.util.zip.ZipEntry
  ...
  public enum CommentSource {
    /**
     * The comment has been read from the archive using the encoding of the archive specified when
     * creating the {@link ZipArchiveInputStream} or {@link ZipFile} (defaults to the platform's
     * default encoding).
     */
    COMMENT,
   ...
   *
   * <p>Configures the behavior for:
   *
   * <ul>
   *   <li>What shall happen if the extra field content doesn't follow the recommended pattern of
   *       two-byte id followed by a two-byte length?
   *   <li>What shall happen if an extra field is generally supported by Commons Compress but its
   *       content cannot be parsed correctly? This may for example happen if the archive is
   *       corrupt, it triggers a bug in Commons Compress or the extra field uses a version not
   *       (yet) supported by Commons Compress.
   * </ul>
   *
   * @since 1.19
   */
  public enum ExtraFieldParsingMode implements ExtraFieldParsingBehavior {
     ...
     * supported extra fields that cannot be parsed in {@link UnrecognizedExtraField}.
     *
     * <p>Wrap extra data that doesn't follow the recommended pattern in an {@link
     * UnparseableExtraFieldData} instance.
     *
     * <p>This is the default behavior starting with Commons Compress 1.19.
     */
    BEST_EFFORT(ExtraFieldUtils.UnparseableExtraField.READ) {
      @Override
      public ZipExtraField fill(
    ...
    /**
     * Try to parse as many extra fields as possible and wrap unknown extra fields in {@link
     * UnrecognizedExtraField}.
     *
     * <p>Wrap extra data that doesn't follow the recommended pattern in an {@link
     * UnparseableExtraFieldData} instance.
     *
     * <p>Throw an exception if an extra field that is generally supported cannot be parsed.
     *
     * <p>This used to be the default behavior prior to Commons Compress 1.19.
     */
     ...
     * supported extra fields that cannot be parsed in {@link UnrecognizedExtraField}.
     *
     * <p>Ignore extra data that doesn't follow the recommended pattern.
     */
    ...
    /**
     * Throw an exception if any of the recognized extra fields cannot be parsed or any extra field
     * violates the recommended pattern.
     */
  ...
  public enum NameSource {
    /**
     * The name has been read from the archive using the encoding of the archive specified when
     * creating the {@link ZipArchiveInputStream} or {@link ZipFile} (defaults to the platform's
     * default encoding).
     */
    NAME,
  ...
  private static boolean canConvertToInfoZipExtendedTimestamp(
      final FileTime lastModifiedTime, final FileTime lastAccessTime, final FileTime creationTime) {
  ...
  /**
   * The {@link java.util.zip.ZipEntry} base class only supports the compression methods STORED and
   * DEFLATED. We override the field so that any compression methods can be used.
   *
   * <p>The default value -1 means that the method has not been specified.
   *
  ...
  /**
   * The {@link java.util.zip.ZipEntry#setSize} method in the base class throws an
   * IllegalArgumentException if the size is bigger than 2GB for Java versions &lt; 7 and even in
   * Java 7+ if the implementation in java.util.zip doesn't support Zip64 itself (it is an optional
   * feature).
   *
   * <p>We need to keep our own size information for Zip64 support.
   */
  ...
  /**
   * Creates a new ZIP entry taking some information from the given path and using the provided
   * name.
   *
   * <p>The name will be adjusted to end with a forward slash "/" if the file is a directory. If the
   * file is not a directory a potential trailing forward slash will be stripped from the entry
   ...
   * @since 1.21
   */
  public ZipArchiveEntry(final Path inputPath, final String entryName, final LinkOption... options)
      throws IOException {
    this(
        Files.isDirectory(inputPath, options) && !entryName.endsWith("/")
            ? entryName + "/"
            : entryName);
  ...
  private ZipExtraField[] getAllExtraFields() {
    ...
    return (allExtraFieldsNoCopy == extraFields)
        ? copyOf(allExtraFieldsNoCopy, allExtraFieldsNoCopy.length)
   ...
   *
   * @return An array of all extra fields. Not necessarily a copy of internal data structures, hence
   *     private method
   */
   ...
   *
   * <p><b>Note</b>: {@link ZipArchiveInputStream} is unable to fill this field, you must use {@link
   * ZipFile} if you want to read entries using this attribute.
   *
   ...
   *
   * <p><b>Note</b>: The set of extra fields may be incomplete when {@link ZipArchiveInputStream}
   * has been used as some extra fields use the central directory to store additional information.
   *
   ...
   *
   * @param includeUnparseable whether to also return unparseable extra fields as {@link
   ...
   * @return an array of the extra fields
   * @throws ZipException if parsing fails, can not happen if {@code parsingBehavior} is {@link
  ...
  public ZipExtraField[] getExtraFields(final ExtraFieldParsingBehavior parsingBehavior)
    ...
    final List<ZipExtraField> localFields =
        new ArrayList<>(Arrays.asList(ExtraFieldUtils.parse(local, true, parsingBehavior)));
    final byte[] central = getCentralDirectoryExtra();
    final List<ZipExtraField> centralFields =
        new ArrayList<>(Arrays.asList(ExtraFieldUtils.parse(central, false, parsingBehavior)));
  ...
  /**
   * Wraps {@link java.util.zip.ZipEntry#getTime} with a {@link Date} as the entry's last modified
   * date.
   *
   * <p>Changes to the implementation of {@link java.util.zip.ZipEntry#getTime} leak through and the
   * returned value may depend on your local time zone as well as your version of Java.
   */
  @Override
  ...
  /**
   * Returns the compression method of this entry, or -1 if the compression method has not been
   * specified.
   *
  ...
  private ZipExtraField[] getParseableExtraFields() {
    ...
    return (parseableExtraFields == extraFields)
        ? copyOf(parseableExtraFields, parseableExtraFields.length)
  ...
  /**
   * Platform specification to put into the &quot;version made by&quot; part of the central file
   * header.
   *
   * @return PLATFORM_FAT unless {@link #setUnixMode setUnixMode} has been called, in which case
   *     PLATFORM_UNIX will be returned.
   */
  ...
  /**
   * Returns the raw bytes that made up the name before it has been converted using the configured
   * or guessed encoding.
   *
   * <p>This method will return null if this instance has not been read from an archive.
   *
   ...
   *
   * <p><b>Note</b>: {@link ZipArchiveInputStream} may create entries that return {@link
   * #SIZE_UNKNOWN SIZE_UNKNOWN} as long as the entry hasn't been read completely.
   *
   ...
   * {@inheritDoc}
   *
   * <p>Override to work around bug <a
   * href="https://bugs.openjdk.org/browse/JDK-8130914">JDK-8130914</a>
   *
  ...
  private void mergeExtraFields(final ZipExtraField[] f, final boolean local) {
    if (extraFields == null) {
      ...
      for (final ZipExtraField element : f) {
        final ZipExtraField existing = unparseableExtra;
        ...
        if (existing == null) {
          ...
          final byte[] b =
              local ? element.getLocalFileDataData() : element.getCentralDirectoryData();
          try {
            if (local) {
            ...
            // Buggy Method 3
            final UnrecognizedExtraField u = null;
  ...
  private void setAttributes(final Path inputPath, final LinkOption... options) throws IOException {
    final BasicFileAttributes attributes =
        Files.readAttributes(inputPath, BasicFileAttributes.class, options);
   ...
   * overriding getExtra doesn't help - we need to modify super's data directly and on every update.
   */
  ...
  public void setExtra(final byte[] extra) throws RuntimeException {
    try {
      ...
      throw new IllegalArgumentException(
          "Error parsing extra fields for entry: " // NOSONAR
              + getName()
              + " - "
              + e.getMessage(),
          e);
  ...
  /**
   * Workaround for the fact that, as of Java 17, {@link java.util.zip.ZipEntry} does not properly
   * modify the entry's {@code xdostime} field, only setting {@code mtime}. While this is not
   * strictly necessary, it's better to maintain the same behavior between this and the NTFS field.
   */
