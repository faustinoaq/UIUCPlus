 ...
 * 1</a> <br>
 * <a href="http://www.fileformat.info/format/arj/corion.htm">Reference 2</a> @NotThreadSafe
 *
 * @since 1.6
 */
public class ArjArchiveInputStream extends ArchiveInputStream {
  ...
  /**
   * Constructs the ArjInputStream, taking ownership of the inputStream that is passed in, and using
   * the CP437 character encoding.
   *
   ...
   *
   * @param inputStream the underlying stream, whose ownership is taken
   * @param charsetName the charset used for file names and comments in the archive. May be {@code
   *     null} to use the platform default.
   * @throws ArchiveException if an exception occurs while reading
   */
  public ArjArchiveInputStream(final InputStream inputStream, final String charsetName)
  ...
  private void readExtraData(
      ...
      final DataInputStream firstHeader,
  ...
  private LocalFileHeader readLocalFileHeader() throws IOException {
    ...
    try (final DataInputStream basicHeader =
        new DataInputStream(new ByteArrayInputStream(basicHeaderBytes))) {
      ...
      try (final DataInputStream firstHeader =
          new DataInputStream(new ByteArrayInputStream(firstHeaderBytes))) {
  ...
  private MainHeader readMainHeader() throws IOException {
    ...
    final DataInputStream basicHeader =
        new DataInputStream(new ByteArrayInputStream(basicHeaderBytes));
