...
public class MetadataBandGroup {
  ...
  private Annotation getAnnotation(
      final CPUTF8 type, final int pairCount, final Iterator<CPUTF8> namesIterator) {
  ...
  private Attribute getAttribute(
    ...
    return new RuntimeVisibleorInvisibleAnnotationsAttribute(
        type.equals("RVA") ? rvaUTF8 : riaUTF8, annotations);
  }
    ...
    if (attributes == null) {
      ...
      if (name_RU != null) {
        ...
        for (final int element : T) {
          attributes.add(
              new AnnotationDefaultAttribute(new ElementValue(element, getNextValue(element))));
        }
      }
    }
    return attributes;
  ...
  private Attribute getParameterAttribute(
      final int numParameters, final Iterator<CPUTF8> namesIterator) {
    ...
    for (int i = 0; i < numParameters; i++) {
      ...
      Arrays.setAll(
          annotations,
          j -> getAnnotation(type_RS[anno_N_Index - 1][j], pairCounts[j], namesIterator));
    ...
    return new RuntimeVisibleorInvisibleParameterAnnotationsAttribute(
        type.equals("RVPA") ? rvpaUTF8 : ripaUTF8, parameterAnnotations);
  }
}
