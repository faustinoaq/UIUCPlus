...
public class IcBands extends BandSet {
  ...
  class IcTuple implements Comparable<IcTuple> {
    ...
    public boolean equals(final Object o) {
      if (o instanceof IcTuple) {
        ...
        return C.equals(icT.C)
            && F == icT.F
            && (Objects.equals(C2, icT.C2))
  ...
  public void addInnerClass(
      final String name, final String outerName, final String innerName, int flags) {
    if (outerName != null || innerName != null) {
      if (namesArePredictable(name, outerName, innerName)) {
        ...
        final IcTuple icTuple =
            new IcTuple(
                cpBands.getCPClass(name),
                flags,
                cpBands.getCPClass(outerName),
  ...
  private boolean namesArePredictable(
  ...
  public void pack(final OutputStream outputStream) throws IOException, Pack200Exception {
    ...
    PackingUtils.log(
        "Wrote " + encodedBand.length + " bytes from ic_this_class[" + ic_this_class.length + "]");

    encodedBand = encodeBandInt("ic_flags", ic_flags, Codec.UNSIGNED5);
    outputStream.write(encodedBand);
    PackingUtils.log(
        "Wrote " + encodedBand.length + " bytes from ic_flags[" + ic_flags.length + "]");
    ...
    PackingUtils.log(
        "Wrote "
            + encodedBand.length
            + " bytes from ic_outer_class["
            + ic_outer_class.length
            + "]");
