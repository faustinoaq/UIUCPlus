...
public class Segment extends ClassVisitor {
  ...
  /**
   * Exception indicating that the class currently being visited contains an unknown attribute,
   * which means that by default the class file needs to be passed through as-is in the file_bands
   * rather than being packed with pack200.
   */
  ...
  /**
   * SegmentAnnotationVisitor implements {@code AnnotationVisitor} to visit Annotations found in a
   * class file.
   */
  public class SegmentAnnotationVisitor extends AnnotationVisitor {
    ...
    public SegmentAnnotationVisitor(
        final int context, final int parameter, final String desc, final boolean visible) {
      super(ASM_API);
      this.context = context;
    ...
    public void visitEnd() {
      if (desc == null) {
        Segment.this.classBands.addAnnotationDefault(
            nameRU, tags, values, caseArrayN, nestTypeRS, nestNameRU, nestPairN);
      } else if (parameter != -1) {
  ...
  /**
   * SegmentFieldVisitor implements {@code FieldVisitor} to visit the metadata relating to fields in
   * a class file.
   */
  ...
  /**
   * This class implements MethodVisitor to visit the contents and metadata related to methods in a
   * class file.
   *
   * <p>It delegates to BcBands for bytecode related visits and to ClassBands for everything else.
   */
  public class SegmentMethodVisitor extends MethodVisitor {
    ...
    public void visitFrame(
        final int arg0, final int arg1, final Object[] arg2, final int arg3, final Object[] arg4) {
  ...
  private void addValueAndTag(
      final Object value, final List<String> tags, final List<Object> values) {
  ...
  /**
   * The main method on Segment. Reads in all the class files, packs them and then writes the packed
   * segment out to the given OutputStream.
   *
   ...
   */
  public void pack(
      final SegmentUnit segmentUnit, final OutputStream out, final PackingOptions options)
    ...
    PackingUtils.log(
        "Start to pack a new segment with "
            + segmentUnit.fileListSize()
            + " files including "
            + segmentUnit.classListSize()
            + " classes");
    ...
    PackingUtils.log(
        "Transmitted "
            + segmentUnit.fileListSize()
            + " files of "
            + segmentUnit.getByteAmount()
            + " input bytes in a segment of "
            + segmentUnit.getPackedByteAmount()
            + " bytes");
  }
  ...
  private void processClasses(final SegmentUnit segmentUnit, final Attribute[] attributes)
  ...
  public void visitInnerClass(
      final String name, final String outerName, final String innerName, final int flags) {
