 ...
 *
 * <p>Understanding Unix vs NTFS timestamps:
 *
 * <ul>
 *   <li>A <a href="https://en.wikipedia.org/wiki/Unix_time">Unix timestamp</a> is a primitive long
 *       starting at the Unix Epoch on January 1st, 1970 at Coordinated Universal Time (UTC)
 *   <li>An <a href="https://learn.microsoft.com/en-us/windows/win32/sysinfo/file-times">NTFS
 *       timestamp</a> is a file time is a 64-bit value that represents the number of 100-nanosecond
 *       intervals that have elapsed since 12:00 A.M. January 1, 1601 Coordinated Universal Time
 *       (UTC).
 * </ul>
 *
 * @since 1.23
...
public final class TimeUtils {
   ...
   * File Times</a>
   *
   * <p>A file time is a 64-bit value that represents the number of 100-nanosecond intervals that
   * have elapsed since 12:00 A.M. January 1, 1601 Coordinated Universal Time (UTC). This is the
   * offset of Windows time 0 to Unix epoch in 100-nanosecond intervals.
   */
   ...
   *
   * <p>TODO ? If the FileTime is null, this method always returns true.
   *
  ...
  public static boolean isUnixTime(final FileTime time) {
    ...
    FileTime unixTime = toUnixTime(time);
    if (unixTime != null) {
      return isUnixTime(unixTime);
    }
  ...
  /**
   * Tests whether a given number of seconds (since Epoch) can be safely represented in the standard
   * UNIX time.
   *
  ...
  /**
   * Converts {@link FileTime} to a {@link Date}. If the provided FileTime is {@code null}, the
   * returned Date is also {@code null}.
   *
   * @param fileTime the file time to be converted.
   * @return a {@link Date} which corresponds to the supplied time, or {@code null} if the time is
   *     {@code null}.
  ...
  /**
   * Converts {@link Date} to a {@link FileTime}. If the provided Date is {@code null}, the returned
   * FileTime is also {@code null}.
   *
   * @param date the date to be converted.
   * @return a {@link FileTime} which corresponds to the supplied date, or {@code null} if the date
   *     is {@code null}.
  ...
  public static long toNtfsTime(final FileTime fileTime) {
    ...
    final long javaHundredNanos =
        (instant.getEpochSecond() * HUNDRED_NANOS_PER_SECOND) + (instant.getNano() / 100);
  ...
  public static FileTime truncateToHundredNanos(final FileTime fileTime) {
    ...
    return FileTime.from(
        Instant.ofEpochSecond(instant.getEpochSecond(), (instant.getNano() / 100) * 100));
  }
