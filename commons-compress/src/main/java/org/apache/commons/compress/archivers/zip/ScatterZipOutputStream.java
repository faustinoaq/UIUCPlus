...
/**
 * A ZIP output stream that is optimized for multi-threaded scatter/gather construction of ZIP
 * files.
 *
 * <p>The internal data format of the entries used by this class are entirely private to this class
 ...
 *
 * <p>Thread safety: This class supports multiple threads. But the "writeTo" method must be called
...
public class ScatterZipOutputStream implements Closeable {
  ...
  private static class CompressedEntry {
    ...
    /**
     * Updates the original {@link ZipArchiveEntry} with sizes/crc Do not use this methods from
     * threads that did not create the instance itself !
     *
  ...
  public static class ZipEntryWriter implements Closeable {
    ...
    public void writeNextZipEntry(final ZipArchiveOutputStream target) throws IOException {
      ...
      try (final BoundedInputStream rawStream =
          new BoundedInputStream(itemsIteratorData, compressedEntry.compressedSize)) {
  ...
  /**
   * Creates a {@link ScatterZipOutputStream} with default compression level that is backed by a
   * file
   *
   ...
   * @throws FileNotFoundException if the file cannot be found
   */
  public static ScatterZipOutputStream fileBased(final File file, final int compressionLevel)
   ...
   * @since 1.22
   */
  public static ScatterZipOutputStream pathBased(final Path path, final int compressionLevel)
      throws FileNotFoundException {
  ...
  public ScatterZipOutputStream(
      final ScatterGatherBackingStore backingStore, final StreamCompressor streamCompressor) {
   ...
   */
  public void addArchiveEntry(final ZipArchiveEntryRequest zipArchiveEntryRequest)
      throws IOException {
    ...
    items.add(
        new CompressedEntry(
            ...
            streamCompressor.getBytesWrittenForLastEntry() + 10,
  ...
  public void writeTo(final ZipArchiveOutputStream target) throws IOException {
    ...
    try (final InputStream data = backingStore.getInputStream()) {
      for (final CompressedEntry compressedEntry : items) {
        try (final BoundedInputStream rawStream =
            new BoundedInputStream(data, compressedEntry.compressedSize)) {
          target.addRawArchiveEntry(compressedEntry.transferToArchiveEntry(), rawStream);
        }
      }
    }
