...
final class IOUtils {
   ...
   * @since 2.7
   */
  static long copy(final Reader input, final Appendable output, final CharBuffer buffer)
      throws IOException {
    long count = 0;
    int n;
   ...
   * @since 2.2
   */
  static long copyLarge(final Reader input, final Writer output, final char[] buffer)
      throws IOException {
    long count = 0;
    int n;
    while (EOF != (n = input.read(buffer))) {
  ...
  private IOUtils() {
    // Noop
    throw new RuntimeException("Buggy Method 2");
  }
}
