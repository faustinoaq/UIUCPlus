...
public final class CSVFormat implements Serializable {
  ...
  public static class Builder {
    ...
    /**
     * Sets the parser missing column names behavior, {@code true} to allow missing column names in
     * the header line, {@code false} to cause an {@link IllegalArgumentException} to be thrown.
     *
    ...
    /**
     * Sets the header from the result set metadata. The header can either be parsed automatically
     * from the input file with:
     *
     * <pre>
     * builder.setHeader();
     * </pre>
    ...
    /**
     * Sets the header to the given values. The header can either be parsed automatically from the
     * input file with:
     *
     * <pre>
     * builder.setHeader();
    ...
    /**
     * Sets the header comments set to the given values. The comments will be printed first, before
     * the headers. This setting is ignored by the parser.
     *
     * <pre>
    ...
    /**
     * Sets the parser case mapping behavior, {@code true} to access name/values, {@code false} to
  ...
  /**
   * Excel file format (using a comma as the value delimiter). Note that the actual value delimiter
   * used by Excel is locale dependent, it might be necessary to customize this format to
  ...
  /**
   * Default Informix CSV UNLOAD format used by the {@code UNLOAD TO file_name} operation (escaping
   * is disabled.)
   *
   * <p>This is a comma-delimited format with a LF character as the line separator. Values are not
   * quoted and special characters are escaped with {@code '\'}. The default NULL string is {@code
  ...
  public static final CSVFormat TDF =
      DEFAULT.builder().setDelimiter(TAB).setIgnoreSurroundingSpaces(true).build();
   ...
   *
   * <p>Use this method if you want to create a CSVFormat from scratch. All fields but the delimiter
   * will be initialized with null/false.
   *
   ...
   *     disable.
   * @param escape the Character used to escape special characters in values, may be {@code null} to
   *     disable.
  ...
  public String format(final Object... values) {
    ...
    try (CSVPrinter csvPrinter = new CSVPrinter(out, this)) {
      ...
      final int len =
          recordSeparator != null ? res.length() - recordSeparator.length() : res.length();
  ...
  public boolean getAllowDuplicateHeaderNames() {
    ...
    if (duplicateHeaderMode == DuplicateHeaderMode.ALLOW_NONE) {
      result = false;
    } else if (duplicateHeaderMode == DuplicateHeaderMode.ALLOW_FIRST) {
      result = true;
    }
    return result;
  }
   ...
   *
   * @return {@code true} if spaces around values are ignored, {@code false} if they are treated as
   *     part of the value.
   */
   ...
   * @since 1.4
   */
  public synchronized void print(final Object value, final Appendable out, final boolean newRecord)
      throws IOException {
  ...
  private synchronized void print(
      final Object object, final CharSequence value, final Appendable out, final boolean newRecord)
      throws IOException {
    final int offset = 0;
  ...
  private void print(final Reader reader, final Appendable out, final boolean newRecord)
      throws IOException {
  ...
  /**
   * Prints the given {@code values} to {@code out} as a single record of delimiter separated values
   ...
   */
  private void printWithEscapes(final CharSequence charSeq, final Appendable appendable)
      throws IOException {
    int start = 0;
    int pos = 0;
  ...
  private void printWithEscapes(final Reader reader, final Appendable appendable)
      throws IOException {
    int start = 0;
    int pos = 0;
   ...
   * tested here. For example allowMissingColumnNames is only used for parsing, so it cannot be used
   * here.
   *
   * @throws IllegalArgumentException Throw when any attribute is invalid or inconsistent with other
   *     attributes.
   */
  ...
  /**
   * Builds a new {@code CSVFormat} with the missing column names behavior of the format set to the
   * given value.
   *
   * @param allowMissingColumnNames the missing column names behavior, {@code true} to allow missing
   *     column names in the header line, {@code false} to cause an {@link IllegalArgumentException}
   *     to be thrown.
   * @return A new CSVFormat that is equal to this but with the specified missing column names
   *     behavior.
   * @deprecated Use {@link Builder#setAllowMissingColumnNames(boolean)}
  ...
  /**
   * Builds a new {@code CSVFormat} with the comment start marker of the format set to the specified
   * character.
   *
   * <p>Note that the comment start character is only recognized at the start of a line.
   *
  ...
  /**
   * Builds a new {@code CSVFormat} with the header of the format set from the result set metadata.
   ...
   * <p>The header is also used by the {@link CSVPrinter}.
   *
   * @param resultSetMetaData the metaData for the header, {@code null} if disabled, empty if parsed
   *     automatically, user specified otherwise.
   * @return A new CSVFormat that is equal to this but with the specified header
   * @throws SQLException SQLException if a database access error occurs or this method is called on
   *     a closed result set.
  ...
  /**
   * Builds a new {@code CSVFormat} with the header comments of the format set to the given values.
   ...
   * </pre>
   *
   * @param headerComments the headerComments which will be printed by the Printer before the actual
   *     CSV data.
   * @return A new CSVFormat that is equal to this but with the specified header
   * @see Builder#setSkipHeaderRecord(boolean)
   * @since 1.1
  ...
  /**
   * Builds a new {@code CSVFormat} with the parser trimming behavior of the format set to the given
   * value.
   *
   * @param ignoreSurroundingSpaces the parser trimming behavior, {@code true} to remove the
   *     surrounding spaces, {@code false} to leave the spaces as is.
  ...
  /**
   * Builds a new {@code CSVFormat} with the output quote policy of the format set to the specified
   * value.
   *
   * @param quoteMode the quote policy to use for output.
  ...
  /**
   * Builds a new {@code CSVFormat} to trim leading and trailing blanks. See {@link #getTrim()} for
   * details of where this is used.
   *
