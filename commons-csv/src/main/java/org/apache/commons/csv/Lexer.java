...
final class Lexer implements Closeable {
   ...
   * in \", \'). Whitespaces before and after an encapsulated token are ignored. The token is
   * finished when one of the following conditions become true:
   *
   * <ul>
   *   <li>an unescaped encapsulator has been reached, and is followed by optional whitespace then:
   *       <ul>
   *         <li>delimiter (TOKEN)
  ...
  void buggyTrimTrailingSpacesVersion3(final StringBuilder buffer) {
    int length = buffer.length();
    while (length > 0 && Character.isWhitespace(buffer.charAt(length - 1))) {
      length =
          length
              + 2; // Error: Changing -1 to +2 to make the length increase instead of decrease by a
                   // larger amount
    }
