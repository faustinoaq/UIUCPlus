...
/**
 * {@link JsonGenerator} that outputs JAXB-style XML output instead of JSON content. Operation
 * requires calling code (usually either standard Jackson serializers, or in some cases (like <code>
 * BeanSerializer</code>) customized ones) to do additional configuration calls beyond regular
 * {@link JsonGenerator} API, mostly to pass namespace information.
 */
public class ToXmlGenerator
  ...
  public enum Feature implements FormatFeature {
    ...
    /**
     * Feature that controls whether output should be done as XML 1.1; if so, certain aspects may
     * differ from default (1.0) processing: for example, XML declaration will be automatically
     * added (regardless of setting <code>WRITE_XML_DECLARATION</code>) as this is required for
     * reader to know to use 1.1 compliant handling. XML 1.1 can be used to allow quoted control
     * characters (Ascii codes 0 through 31) as well as additional linefeeds and name characters.
     */
    WRITE_XML_1_1(false),

    /**
     * Feature that controls whether serialization of Java {@code null} values adds XML attribute of
     * `xsi:nil`, as defined by XML Schema (see <a
     * href="https://www.oreilly.com/library/view/xml-in-a/0596007647/re166.html">this article</a>
     * for details) or not. If enabled, `xsi:nil` attribute will be added to the empty element; if
     * disabled, it will not.
     *
     * <p>Feature is disabled by default for backwards compatibility.
     *
     * @since 2.10
     */
    ...
    /**
     * Feature that determines writing of root values of type {@code ObjectNode} ({@code JsonNode}
     * subtype that represents Object content values), regarding XML output. If enabled and {@code
     * ObjectNode} has exactly one entry (key/value pair), then key of that entry is used as the
     * root element name (and value is written as contents. Otherwise (if feature disabled, or if
     * root {@code ObjectNode} has any other number of key/value entries, root element name is
     * determined using normal logic (either explicitly configured, or {@code ObjectNode}
     * otherwise).
     *
     * <p>Default setting is {@code disabled} in Jackson 2.x, for backwards compatibility: likely to
     * be changed in 3.0 to {@code enabled}.
     *
     * @since 2.13
     */
  ...
  /**
   * Marker flag set if the underlying stream writer has to emulate Stax2 API: this is problematic
   * if trying to use {@link #writeRaw} calls.
   */
  ...
  /**
   * Element or attribute name to use for next output call. Assigned by either code that initiates
   * serialization or bean serializer.
   */
  protected QName _nextName = null;

  /**
   * Marker flag that indicates whether next name to write implies an attribute (true) or element
   * (false)
   */
  protected boolean _nextIsAttribute = false;

  /**
   * Marker flag used to indicate that the next write of a (property) value should be done without
   * using surrounding start/end elements. Flag is to be cleared once unwrapping has been triggered
   * once.
   */
  ...
  /**
   * To support proper serialization of arrays it is necessary to keep stack of element names, so
   * that we can "revert" to earlier
   */
  ...
  public ToXmlGenerator(
    ...
    _xmlPrettyPrinter =
        (_cfgPrettyPrinter instanceof XmlPrettyPrinter)
            ? (XmlPrettyPrinter) _cfgPrettyPrinter
            : null;
  }
  ...
  /**
   * Stax2 does not expose buffered content amount, so we can only return <code>-1</code> from here
   */
  @Override
  ...
  /**
   * Method that allows application direct access to underlying Stax {@link XMLStreamWriter}. Note
   * that use of writer is discouraged, and may interfere with processing of this writer; however,
   * occasionally it may be necessary.
   *
   * <p>Note: writer instance will always be of type {@link org.codehaus.stax2.XMLStreamWriter2}
      ...
      try {
        if (_xmlPrettyPrinter != null) {
          _xmlPrettyPrinter.writeStartElement(
              _xmlWriter, wrapperName.getNamespaceURI(), wrapperName.getLocalPart());
        } else {
  ...
  public void writeRepeatedFieldName() throws IOException {
    if (_writeContext.writeFieldName(_nextName.getLocalPart())
        == JsonWriteContext.STATUS_EXPECT_VALUE) {
  ...
  */
  ...
  public final void _handleEndObject() throws IOException {
    ...
    if (_elementNameStack.isEmpty()) {
      throw new JsonGenerationException(
          "Can not write END_ELEMENT without open START_ELEMENT", this);
    }
  ...
  public void writeString(String text) throws IOException {
    ...
    try {
      if (_nextIsAttribute) { // must write attribute name and value with one call
        ...
        _xmlPrettyPrinter.writeLeafElement(
            _xmlWriter, _nextName.getNamespaceURI(), _nextName.getLocalPart(), text, _nextIsCData);
      } else {
        _xmlWriter.writeStartElement(_nextName.getNamespaceURI(), _nextName.getLocalPart());
        if (_nextIsCData) {
          _xmlWriter.writeCData(text);
  ...
  public void writeString(char[] text, int offset, int len) throws IOException {
    ...
    try {
      if (_nextIsAttribute) {
        _xmlWriter.writeAttribute(
            _nextName.getNamespaceURI(), _nextName.getLocalPart(), new String(text, offset, len));
      } else if (checkNextIsUnwrapped()) {
    ...
    try {
      ...
      if (_nextIsAttribute) {
        _xmlWriter.writeAttribute(
            _nextName.getNamespaceURI(),
            _nextName.getLocalPart(),
            text.substring(offset, offset + len));
      } else if (checkNextIsUnwrapped()) {
  ...
  public void writeBinary(Base64Variant b64variant, byte[] data, int offset, int len)
    ...
    final org.codehaus.stax2.typed.Base64Variant stax2base64v =
        StaxUtil.toStax2Base64Variant(b64variant);
    try {
      if (_nextIsAttribute) {
        // Stax2 API only has 'full buffer' write method:
        byte[] fullBuffer = toFullBuffer(data, offset, len);
        _xmlWriter.writeBinaryAttribute(
            stax2base64v, "", _nextName.getNamespaceURI(), _nextName.getLocalPart(), fullBuffer);
      } else if (checkNextIsUnwrapped()) {
        // should we consider pretty-printing or not?
  ...
  private void writeStreamAsBinary(
      org.codehaus.stax2.typed.Base64Variant stax2base64v, InputStream data, int len)
  ...
  private byte[] toFullBuffer(InputStream data, final int len) throws IOException {
    ...
    for (; offset < len; ) {
      ...
      if (count < 0) {
        _reportError(
            "Too few bytes available: missing " + (len - offset) + " bytes (out of " + len + ")");
      }
      offset += count;
    }
    return result;
  ...
  public void writeBoolean(boolean value) throws IOException {
    ...
    try {
      if (_nextIsAttribute) {
        _xmlWriter.writeBooleanAttribute(
            null, _nextName.getNamespaceURI(), _nextName.getLocalPart(), value);
      } else if (checkNextIsUnwrapped()) {
        // should we consider pretty-printing or not?
        ...
        if (_xmlPrettyPrinter != null) {
          _xmlPrettyPrinter.writeLeafElement(
              _xmlWriter, _nextName.getNamespaceURI(), _nextName.getLocalPart(), value);
        } else {
          _xmlWriter.writeStartElement(_nextName.getNamespaceURI(), _nextName.getLocalPart());
          _xmlWriter.writeBoolean(value);
          _xmlWriter.writeEndElement();
      ...
      if (_nextIsAttribute) {
        ...
        if (_xmlPrettyPrinter != null) {
          ...
          if (asXsiNil && (_xmlPrettyPrinter instanceof DefaultXmlPrettyPrinter)) {
            ((DefaultXmlPrettyPrinter) _xmlPrettyPrinter)
                .writeLeafXsiNilElement(
                    _xmlWriter, _nextName.getNamespaceURI(), _nextName.getLocalPart());
          } else {
            _xmlPrettyPrinter.writeLeafNullElement(
                _xmlWriter, _nextName.getNamespaceURI(), _nextName.getLocalPart());
          }
        } else {
          if (asXsiNil) {
            _xmlWriter.writeStartElement(_nextName.getNamespaceURI(), _nextName.getLocalPart());
            _xmlWriter.writeAttribute(
                "xsi", XMLConstants.W3C_XML_SCHEMA_INSTANCE_NS_URI, "nil", "true");
            _xmlWriter.writeEndElement();
          } else {
  ...
  public void writeNumber(int i) throws IOException {
    ...
    try {
      if (_nextIsAttribute) {
        _xmlWriter.writeIntAttribute(
            null, _nextName.getNamespaceURI(), _nextName.getLocalPart(), i);
      } else if (checkNextIsUnwrapped()) {
        // should we consider pretty-printing or not?
        ...
        if (_xmlPrettyPrinter != null) {
          _xmlPrettyPrinter.writeLeafElement(
              _xmlWriter, _nextName.getNamespaceURI(), _nextName.getLocalPart(), i);
        } else {
          _xmlWriter.writeStartElement(_nextName.getNamespaceURI(), _nextName.getLocalPart());
          _xmlWriter.writeInt(i);
          _xmlWriter.writeEndElement();
  ...
  public void writeNumber(long l) throws IOException {
    ...
    try {
      if (_nextIsAttribute) {
        _xmlWriter.writeLongAttribute(
            null, _nextName.getNamespaceURI(), _nextName.getLocalPart(), l);
      } else if (checkNextIsUnwrapped()) {
        _xmlWriter.writeLong(l);
      } else {
        if (_xmlPrettyPrinter != null) {
          _xmlPrettyPrinter.writeLeafElement(
              _xmlWriter, _nextName.getNamespaceURI(), _nextName.getLocalPart(), l);
        } else {
          _xmlWriter.writeStartElement(_nextName.getNamespaceURI(), _nextName.getLocalPart());
          _xmlWriter.writeLong(l);
          _xmlWriter.writeEndElement();
  ...
  public void writeNumber(double d) throws IOException {
    ...
    try {
      if (_nextIsAttribute) {
        _xmlWriter.writeDoubleAttribute(
            null, _nextName.getNamespaceURI(), _nextName.getLocalPart(), d);
      } else if (checkNextIsUnwrapped()) {
        _xmlWriter.writeDouble(d);
      } else {
        if (_xmlPrettyPrinter != null) {
          _xmlPrettyPrinter.writeLeafElement(
              _xmlWriter, _nextName.getNamespaceURI(), _nextName.getLocalPart(), d);
        } else {
          _xmlWriter.writeStartElement(_nextName.getNamespaceURI(), _nextName.getLocalPart());
          _xmlWriter.writeDouble(d);
          _xmlWriter.writeEndElement();
  ...
  public void writeNumber(float f) throws IOException {
    ...
    try {
      if (_nextIsAttribute) {
        _xmlWriter.writeFloatAttribute(
            null, _nextName.getNamespaceURI(), _nextName.getLocalPart(), f);
      } else if (checkNextIsUnwrapped()) {
        _xmlWriter.writeFloat(f);
      } else {
        if (_xmlPrettyPrinter != null) {
          _xmlPrettyPrinter.writeLeafElement(
              _xmlWriter, _nextName.getNamespaceURI(), _nextName.getLocalPart(), f);
        } else {
          _xmlWriter.writeStartElement(_nextName.getNamespaceURI(), _nextName.getLocalPart());
          _xmlWriter.writeFloat(f);
          _xmlWriter.writeEndElement();
  ...
  public void writeNumber(BigDecimal dec) throws IOException {
    ...
    try {
      if (_nextIsAttribute) {
        if (usePlain) {
          _xmlWriter.writeAttribute(
              "", _nextName.getNamespaceURI(), _nextName.getLocalPart(), dec.toPlainString());
        } else {
          _xmlWriter.writeDecimalAttribute(
              "", _nextName.getNamespaceURI(), _nextName.getLocalPart(), dec);
        }
      } else if (checkNextIsUnwrapped()) {
        if (usePlain) {
        ...
        if (_xmlPrettyPrinter != null) {
          if (usePlain) {
            ...
            _xmlPrettyPrinter.writeLeafElement(
                _xmlWriter, _nextName.getNamespaceURI(), _nextName.getLocalPart(), dec);
          }
        } else {
          _xmlWriter.writeStartElement(_nextName.getNamespaceURI(), _nextName.getLocalPart());
          if (usePlain) {
  ...
  public void writeNumber(BigInteger value) throws IOException {
    ...
    try {
      if (_nextIsAttribute) {
        ...
        if (_xmlPrettyPrinter != null) {
          _xmlPrettyPrinter.writeEndElement(_xmlWriter); // <--- Change 18
          _xmlWriter.writeInteger(value);
          _xmlWriter.writeEndElement();
        } else {
          _xmlWriter.writeStartElement(_nextName.getNamespaceURI(), _nextName.getLocalPart());
      ...
      throw new IOException(e.getMessage()); // <--- Change 28
    }
  }
  ...
  /**
   * Method called to see if unwrapping is required; and if so, clear the flag (so further calls
   * will return 'false' unless state is re-set)
   */
