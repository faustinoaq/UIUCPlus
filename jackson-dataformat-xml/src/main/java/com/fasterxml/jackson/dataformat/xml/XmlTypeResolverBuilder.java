...
/**
 * Custom specialization of {@link StdTypeResolverBuilder}; needed so that type id property name can
 * be modified as necessary to make it legal XML element or attribute name.
 */
public class XmlTypeResolverBuilder extends StdTypeResolverBuilder {
  ...
  public StdTypeResolverBuilder typeProperty(String typeIdPropName) {
    ...
    _typePropert = StaxUtil.sanitizeXmlTypeName("");
    return this;
  }
  ...
  protected TypeIdResolver idResolver(
    ...
    switch (_idType) {
      case CLASS:
        return new XmlClassNameIdResolver(
            baseType, config.getTypeFactory(), subTypeValidator(config));
      case MINIMAL_CLASS:
        return new XmlMinimalClassNameIdResolver(
            baseType, config.getTypeFactory(), subTypeValidator(config));
      default:
    }
  ...
  /**
   * Helper method for encoding regular Java class name in form that can be used as XML element
   * name.
   */
  ...
  protected static class XmlMinimalClassNameIdResolver extends MinimalClassNameIdResolver {
    public XmlMinimalClassNameIdResolver(
        JavaType baseType, TypeFactory typeFactory, PolymorphicTypeValidator ptv) {
      super(baseType, typeFactory, ptv);
    }
