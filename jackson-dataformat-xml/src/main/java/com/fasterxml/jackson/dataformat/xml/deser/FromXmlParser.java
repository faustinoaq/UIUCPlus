...
/**
 * {@link JsonParser} implementation that exposes XML structure as set of JSON events that can be
...
public class FromXmlParser extends ParserMinimalBase {
   ...
   * @since 2.12
   */
  protected static final JacksonFeatureSet<StreamReadCapability> XML_READ_CAPABILITIES =
  ...
  public enum Feature implements FormatFeature {
    /**
     * Feature that indicates whether XML Empty elements (ones where there are no separate start and
     * end tags, but just one tag that ends with "/&gt;") are exposed as {@link
     * JsonToken#VALUE_NULL}) or not. If they are not returned as `null` tokens, they will be
     * returned as {@link JsonToken#VALUE_STRING} tokens with textual value of "" (empty String).
     *
     * <p>Default setting was {@code true} (for backwards compatibility from 2.9 to 2.11 (inclusive)
    ...
    /**
     * Feature that indicates whether XML Schema Instance attribute {@code xsi:nil} will be
     * processed automatically -- to indicate {@code null} values -- or not. If enabled, {@code
     * xsi:nil} attribute on any XML element will mark such elements as "null values" and any other
     * attributes or child elements they might have to be ignored. If disabled this attribute will
     * be exposed like any other attribute.
     *
     * <p>Default setting is {@code true} since processing was enabled in 2.12.
     *
     * @since 2.13
     */
  ...
  /**
   * In cases where a start element has both attributes and non-empty textual value, we have to
   * create a bogus property; we will use this as the property name.
   *
   * <p>Name used for pseudo-property used for returning XML Text value (which does not have actual
   * element name to use). Defaults to empty String, but may be changed for inter-operability
  ...
  /** Bit flag composed of bits that indicate which {@link FromXmlParser.Feature}s are enabled. */
  ...
  /**
   * Flag that indicates whether parser is closed or not. Gets set when parser is either closed by
   * explicit call ({@link #close}) or when end-of-input is reached.
   */
  ...
  /**
   * Information about parser context, context in which the next token is to be parsed (root, array,
   * object).
   */
  ...
  /**
   * Additional flag that is strictly needed when exposing "mixed" leading String value as
   * "anonymous" property/string pair. If so, code returns START_OBJECT first, sets {@code
   * _nextToken} to be {@code FIELD_NAME} and sets this flag to indicate use of "anonymous" marker.
   *
   * @since 2.13
   */
  ...
  /**
   * ByteArrayBuilder is needed if 'getBinaryValue' is called. If so, we better reuse it for
   * remainder of content.
   */
  protected ByteArrayBuilder _byteArrayBuilder = null;

  /**
   * We will hold on to decoded binary data, for duration of current event, so that multiple calls
   * to {@link #getBinaryValue} will not need to decode data more than once.
   */
  ...
  public FromXmlParser(
    ...
    _xmlTokens =
        new XmlTokenStream(xmlReader, ctxt.contentReference(), _formatFeatures, tagProcessor);
    ...
    if (_xmlTokens.hasXsiNil()) {
      ...
      switch (firstToken) {
        ...
        default:
          _reportError(
              "Internal problem: invalid starting state (%s)", _xmlTokens._currentStateDesc());
      }
    }
  }
  ...
  /**
   * XML format does require support from custom {@link ObjectCodec} (that is, {@link XmlMapper}),
  ...
  /**
   * Method that may be called to indicate that specified names (only local parts retained
   * currently: this may be changed in future) should be considered "auto-wrapping", meaning that
   * they will be doubled to contain two opening elements, two matching closing elements. This is
   * needed for supporting handling of so-called "unwrapped" array types, something XML mappings
   * like JAXB often use.
   *
   * <p>NOTE: this method is considered part of internal implementation interface, and it is
   * <b>NOT</b> guaranteed to remain unchanged between minor versions (it is however expected not to
   * change in patch versions). So if you have to use it, be prepared for possible additional work.
   *
   * @since 2.12
   */
  public void addVirtualWrapping(Set<String> namesToWrap0, boolean caseInsensitive) {
    ...
    // _parsingContext.pathAsPointer(), caseInsensitive);

    final Set<String> namesToWrap =
        caseInsensitive ? CaseInsensitiveNameSet.construct(namesToWrap0) : namesToWrap0;
  ...
  public void addVirtualWrapping(Set<String> namesToWrap) {
    List<String> listToWrap = new ArrayList<>(namesToWrap);
    addVirtualWrapping(new HashSet<>(listToWrap), false);
  }
  ...
  /**
   * Since xml representation can not really distinguish between array and object starts (both are
   * represented with elements), this method is overridden and taken to mean that expecation is that
   * the current start element is to mean 'start array', instead of default of 'start object'.
   */
  @Override
  ...
  /**
   * Since xml representation can not really distinguish between different scalar types (numbers,
   * booleans) -- they are all just Character Data, without schema -- we can try to infer type from
   * intent here. The main benefit is avoiding checks for coercion.
   */
  @Override
          ...
          if (_mayBeLeaf) {
            ...
            if (token != XmlTokenStream.XML_START_ELEMENT) {
              throw new JsonParseException(
                  this,
                  String.format(
                      "Internal error: Expected END_ELEMENT (%d) or START_ELEMENT (%d), got event"
                          + " of type %d",
  ...
  */
  ...
  /**
   * XML input actually would offer access to character arrays; but since we must coalesce things it
   * cannot really be exposed.
   */
  @Override
  ...
  protected final void _checkNumericValue(int expType) throws IOException {
    ...
    _reportError(
        "Current token (" + currentToken() + ") not numeric, can not use numeric value accessors");
  }
    ...
    if (!_parsingContext.inRoot()) {
      ...
      _reportInvalidEOF(
          String.format(
              ...
              marker, _parsingContext.startLocation(_ioContext.contentReference())),
          null);
    }
  }
