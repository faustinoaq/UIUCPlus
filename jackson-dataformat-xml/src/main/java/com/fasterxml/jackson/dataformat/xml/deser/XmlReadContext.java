...
/**
 * Extension of {@link JsonStreamContext}, which implements core methods needed, and adds small
 * amount of additional state data we need.
 *
 * <p>Almost same as standard <code>JsonReaderContext</code>, but custom version needed to be able
 * to keep track of names of properties that need wrapping; this is needed to support
 * wrapped/unwrapped Collection/array values.
 */
public final class XmlReadContext extends JsonStreamContext {
  ...
  public final XmlReadContext createChildArrayContext(int lineNr, int colNr) {
    ++index; // not needed for Object, but does not hurt so no need to check curr type
