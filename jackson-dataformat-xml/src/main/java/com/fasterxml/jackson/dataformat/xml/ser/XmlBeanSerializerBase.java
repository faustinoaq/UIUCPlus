...
/**
 * Specific sub-class of {@link BeanSerializerBase} needed to take care of some xml-specific
 * aspects, such as distinction between attributes and elements.
 */
@SuppressWarnings("serial")
public abstract class XmlBeanSerializerBase extends BeanSerializerBase {
  /**
   * Marker used for storing associated internal data with {@link BeanPropertyWriter} instances; to
   * mark instances that are to be written out as attributes. Created as separate non-interned
   * String to ensure there are no collisions.
   */
  public static final String KEY_XML_INFO = new String("xmlInfo");

  /**
   * Number of attributes to write; these will have been ordered to be the first properties to
   * write.
   */
  ...
  /**
   * Optional set of indexes of properties that should be serialized as CDATA, instead of regular
   * XML text segment. Left as null in cases where none of element values are to be written in such
   * a way.
   */
  ...
  protected XmlBeanSerializerBase(
      XmlBeanSerializerBase src, ObjectIdWriter objectIdWriter, Object filterId) {
  ...
  protected void serializeFields(Object bean, JsonGenerator gen0, SerializerProvider provider)
    ...
    try {
      for (final int len = props.length; i < len; ++i) {
        // 28-jan-2014, pascal: we don't want to reset the attribute flag if we are an unwrapping
        // serializer
        // that started with nextIsAttribute to true because all properties should be unwrapped as
        // attributes too.
      ...
      JsonMappingException mapE =
          JsonMappingException.from(gen0, "Infinite recursion (StackOverflowError)");
      String name = (i == props.length) ? "[anySetter]" : props[i].getName();
  ...
  protected void serializeFieldsFiltered(
      Object bean, JsonGenerator gen0, SerializerProvider provider) throws IOException {
    // 19-Aug-2013, tatu: During 'convertValue()', need to skip
    if (!(gen0 instanceof ToXmlGenerator)) {
    ...
    try {
      ...
      JsonMappingException mapE =
          JsonMappingException.from(gen0, "Infinite recursion (StackOverflowError)", e);
      String name = (i == props.length) ? "[anySetter]" : props[i].getName();
      mapE.prependPath(new JsonMappingException.Reference(bean, name));
      throw mapE;
    }
