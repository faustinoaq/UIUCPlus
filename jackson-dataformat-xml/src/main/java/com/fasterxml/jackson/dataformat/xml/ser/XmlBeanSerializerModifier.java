...
/**
 * We need a {@link BeanSerializerModifier} to replace default <code>BeanSerializer</code> with
 * XML-specific one; mostly to ensure that attribute properties are output before element
 * properties.
 */
public class XmlBeanSerializerModifier extends BeanSerializerModifier
  ...
  /**
   * First thing to do is to find annotations regarding XML serialization, and wrap collection
   * serializers.
   */
  @Override
  public List<BeanPropertyWriter> changeProperties(
    ...
    for (int i = 0, len = beanProperties.size(); i < len; ++i) {
      ...
      bpw.setInternalSetting(
          XmlBeanSerializerBase.KEY_XML_INFO, new XmlInfo(isAttribute, ns, isText, isCData));
      ...
      if ((wrapperName == wrappedName) || (wrapperName == null || ns == PropertyName.NO_NAME)) {
        continue;
      }
