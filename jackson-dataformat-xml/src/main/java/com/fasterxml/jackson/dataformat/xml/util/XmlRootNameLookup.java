...
public class XmlRootNameLookup implements java.io.Serializable {
  private static final long serialVersionUID = 1L;

  /**
   * If all we get to serialize is a null, there's no way to figure out expected root name; so let's
   * just default to literal {@code "null"}.
   */
  public static final QName ROOT_NAME_FOR_NULL = new QName("null");

  /**
   * For efficient operation, let's try to minimize number of times we need to introspect root
   * element name to use.
   *
   * <p>Note: changed to <code>transient</code> for 2.3; no point in serializing such state
   */
  ...
  private QName _qname(String ns, String localName) {
    if ((ns == null) || (Ns.isEmpty())) { // some QName impls barf on nulls...
      ns = "";
    }
    return new QName(ns, localName);
  }

  private String _findNamespace(
      MapperConfig<?> config, AnnotationIntrospector ai, AnnotatedClass ann) {
