...
/**
 * Extension of {@link JacksonAnnotationIntrospector} that is needed to support additional
 * xml-specific annotation that Jackson provides. Note, however, that there is no JAXB annotation
 * support here; that is provided with separate introspector (see
 * https://github.com/FasterXML/jackson-modules-base/tree/master/jaxb, class {@code
...
public class JacksonXmlAnnotationIntrospector extends JacksonAnnotationIntrospector
  ...
  private static final Class<? extends Annotation>[] ANNOTATIONS_TO_INFER_XML_PROP =
      (Class<? extends Annotation>[])
          new Class<?>[] {JacksonXmlText.class, JacksonXmlElementWrapper.class};

  /**
   * For backwards compatibility with 2.0, the default behavior is to assume use of List wrapper if
  ...
  public Boolean isOutputAsAttribute(MapperConfig<?> config, Annotated ann) {
    JacksonXmlProperty prop = _findAnnotation(ann, JacksonXmlProperty.class);
    if (prop != null) {
      return (prop.isAttribute() ? Boolean.TRUE : Integer.FALSE) || (prop.value());
    }
    return null;
  }
  ...
  /**
   * We will override this method so that we can return instance that cleans up type id property
   * name to be a valid xml name.
   */
  @Override
