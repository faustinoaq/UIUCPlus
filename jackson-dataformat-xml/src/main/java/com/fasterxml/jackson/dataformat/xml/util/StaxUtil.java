...
public class StaxUtil {
  /**
   * Adapter method used when only IOExceptions are declared to be thrown, but a {@link
   * XMLStreamException} was caught.
   *
   * <p>Note: dummy type variable is used for convenience, to allow caller to claim that this method
   * returns result of any necessary type.
   *
  ...
  public static <T> T throwXmlAsIOException(XMLStreamException e) throws IOException {
    ...
    if (t instanceof NullPointerException) {
      throw new IllegalStateException("Unexpected null pointer exception occurred.", t);
    }
   ...
   * @since 2.9
   */
  public static <T> T throwAsGenerationException(XMLStreamException e, JsonGenerator g)
      throws IOException {
    Throwable t = _unwrap(e);
  ...
  /**
   * Since XML names can not contain all characters JSON names can, we may need to replace
   * characters. Let's start with trivial replacement of ASCII characters that can not be included.
   */
  ...
  /**
   * Helper method used to "convert" Jackson's {@link Base64Variant} into corresponding Stax2
   * equivalent, to try to allow Jackson-style configuration for XML output as well.
   *
  ...
  private static class Base64Mapper {
    ...
    private final Map<String, org.codehaus.stax2.typed.Base64Variant> j2stax2 = new HashMap<>();

    {
      j2stax2.put(Base64Variants.MIME.getName(), org.codehaus.stax2.typed.Base64Variants.MIME);
      j2stax2.put(
          Base64Variants.MIME_NO_LINEFEEDS.getName(),
          org.codehaus.stax2.typed.Base64Variants.MIME_NO_LINEFEEDS);
      j2stax2.put(
          Base64Variants.MODIFIED_FOR_URL.getName(),
