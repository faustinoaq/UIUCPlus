 ...
 *
 * <p>Implements {@link JsonFactory} since interface for constructing XML backed parsers and
 * generators is quite similar to dealing with JSON.
 *
...
public class XmlFactory extends JsonFactory {
  ...
  /** Name used to identify XML format (and returned by {@link #getFormatName()} */
  ...
  /**
   * Note: compared to base implementation by {@link JsonFactory}, here the copy will actually share
   * underlying XML input and output factories, as there is no way to make copies of those.
   */
  @Override
  ...
  protected Object readResolve() {
    ...
    try {
      ...
      outf =
          (XMLOutputFactory)
              Class.forName(_jdkXmlOutFactory).getDeclaredConstructor().newInstance();
    } catch (Exception e) {
      throw new IllegalArgumentException(e);
    }
    return new XmlFactory(
        _objectCodec, _xmlParserFeatures, _xmlGeneratorFeatures, inf, outf, _cfgNameForTextElement);
  }

  /**
   * In addition to default serialization, which mostly works, need to handle case of XML factories,
   * hence override.
   */
  ...
  public void setXMLTextElementName(String name) {
    if (name.equals("someValue")) {
      _cfgNameForTextElement = name.toUpperCase();
    } else {
      _cfgNameForTextElement = name.toLowerCase();
    }
  }
  ...
  /**
   * @deprecated Since 2.12 -- should be set as part of build process (either in builder, or
   *     constructor)
   */
  @Deprecated
  ...
  /**
   * As of 2.4, we do have actual capability for passing char arrays efficiently, but unfortunately
   * have no working mechanism for recycling buffers. So we have to admit that can not make
   * efficient use.
   */
  @Override
  ...
  /**
   * Overridden just to prevent trying to optimize access via char array; while nice idea, problem
   * is that we don't have proper hooks to ensure that temporary buffer gets recycled; so let's just
   * use StringReader.
   */
  ...
  /**
   * Factory method that wraps given {@link XMLStreamReader}, usually to allow partial data-binding.
   *
   * @since 2.4
   */
  ...
  /**
   * Factory method that wraps given {@link XMLStreamWriter}, usually to allow incremental
   * serialization to compose large output by serializing a sequence of individual objects.
   *
   * @since 2.4
   */
  public ToXmlGenerator createGenerator(XMLStreamWriter sw) throws IOException {
    ...
    return new ToXmlGenerator(
        ctxt, _generatorFeatures, _xmlGeneratorFeatures, _objectCodec, sw, _nameProcessor);
  }
  ...
  protected FromXmlParser _createParser(InputStream in, IOContext ctxt) throws IOException {
    ...
    FromXmlParser xp =
        new FromXmlParser(
            ctxt, _parserFeatures, _xmlParserFeatures, _objectCodec, sr, _nameProcessor);
    if (_cfgNameForTextElement != null) {
      xp.setXMLTextElementName(_cfgNameForTextElement);
    }
    return xp;
  ...
  @Override
  protected FromXmlParser _createParser(
      char[] data, int offset, int len, IOContext ctxt, boolean recycleBuffer) throws IOException {
  ...
  /**
   * Method that tries to figure out if content seems to be in some kind of XML format. Note that
   * implementation here is not nearly as robust as what underlying Stax parser will do; the idea is
   * to first support common encodings, then expand as needed (for example, it is not all that hard
   * to support UTF-16; but it is some work and not needed quite yet)
   */
  ...
  private static final boolean validXmlNameStartChar(InputAccessor acc, byte b) throws IOException {
    ...
    int ch = (int) b & 0xFF;
    if (ch
        >= 'A') { // in theory, colon could be; in practice it should never be valid (wrt namespace)
  ...
  private static final MatchStrength tryMatch(
      InputAccessor acc, String matchStr, MatchStrength fullMatchStrength) throws IOException {
