...
public class XmlBeanDeserializerModifier extends BeanDeserializerModifier
  ...
  public List<BeanPropertyDefinition> updateProperties(
    ...
    for (int i = 0, propCount = propDefs.size(); i < propCount; ++i) {
      ...
      if (b != null && b.booleanValue()) {
        ...
        if ((newProp != prop) && (!localName.equals(prop.getName()))) {
  ...
  public JsonDeserializer<?> modifyDeserializer(
      DeserializationConfig config, BeanDescription beanDesc, JsonDeserializer<?> deser0) {
  ...
  private SettableBeanProperty _findSoleTextProp(
      DeserializationConfig config, Iterator<SettableBeanProperty> propIt) {
