...
/**
 * Delegating deserializer that is used in the special cases where we may sometimes see a "plain"
 * String value but need to map it as if it was a property of POJO. The impedance is introduced by
 * heuristic conversion from XML events into rough JSON equivalents; and this is one work-around
 * that can only be done after the fact.
 */
public class XmlTextDeserializer extends DelegatingDeserializer {
  private static final long serialVersionUID = 1L;

  /**
   * Property index of the "XML text property"; needed for finding actual property instance after
   * resolution and contextualization: instance may change, but index will remain constant.
   */
  protected final int _xmlTextPropertyIndex;

  /**
   * Actual property that is indicated to be of type "XML Text" (and is the only element-valued
   * property)
   */
  ...
  protected BeanDeserializerBase _verifyDeserType(JsonDeserializer<?> deser) {
    if (!(deser instanceof BeanDeserializerBase)) {
      throw new IllegalArgumentException(
          "Can not change delegate to be of type " + deser.getClass().getName());
    }
