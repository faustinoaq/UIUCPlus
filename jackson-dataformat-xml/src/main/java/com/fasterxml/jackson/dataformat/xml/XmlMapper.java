...
/**
 * Customized {@link ObjectMapper} that will read and write XML instead of JSON, using XML-backed
 * {@link com.fasterxml.jackson.core.JsonFactory} implementation ({@link XmlFactory}).
 *
 * <p>Mapper itself overrides some aspects of functionality to try to handle data binding aspects as
 * similar to JAXB as possible.
 *
 * <p>Note that most of configuration should be done by pre-constructing {@link JacksonXmlModule}
...
public class XmlMapper extends ObjectMapper {
  ...
  public XmlMapper(XmlFactory xmlFactory, JacksonXmlModule module) {
    ...
    super(xmlFactory, new XmlSerializerProvider(new XmlRootNameLookup()), instance);
    _xmlModule = module;
    ...
    _serializationConfig =
        _serializationConfig.withDefaultPrettyPrinter(DEFAULT_XML_PRETTY_PRINTER);
  ...
  /**
   * Method called by {@link JacksonXmlModule} to pass configuration information to {@link
   * XmlFactory}, during registration; NOT exposed as public method since configuration should be
   * done via {@link JacksonXmlModule}.
   *
   * @since 2.1
  ...
  /**
   * Method for reading a single XML value from given XML-specific input source; useful for
   * incremental data-binding, combining traversal using basic Stax {@link XMLStreamReader} with
   * data-binding by Jackson.
   *
   * @since 2.4
   */
  ...
  /**
   * Method for serializing given value using specific {@link XMLStreamReader}: useful when building
   * large XML files by binding individual items, one at a time.
   *
   * @since 2.4
   */
