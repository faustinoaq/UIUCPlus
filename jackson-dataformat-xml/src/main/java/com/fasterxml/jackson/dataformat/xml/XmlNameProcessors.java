 ...
 *
 * <p>Processors should be set in the {@link XmlMapper#setXmlNameProcessor(XmlNameProcessor)} and/or
 * the {@link XmlMapper.Builder#xmlNameProcessor(XmlNameProcessor)} methods.
 *
 * @since 2.14
 */
public final class XmlNameProcessors {
  ...
  /**
   * Generates a new processor that does nothing and just passes through the names as-is. Using this
   * processor may generate invalid XML.
   *
   * <p>With this processor set, a map with the keys {@code "123"} and {@code "$ I am <fancy>! &;"}
   * will be written as:
   *
   * <pre>{@code
   * <DTO>
   ...
   * }</pre>
   *
   * <p>This is the default behavior for backwards compatibility.
   *
   * @since 2.14
   */
   ...
   * </ul>
   *
   * in an XML name with a replacement string. This is a one-way processor, since there is no way to
   * reverse this replacement step.
   *
   * <p>With this processor set (and {@code "_"} as the replacement string), a map with the keys
   * {@code "123"} and {@code "$ I am <fancy>! &;"} will be written as:
   *
   * <p>NOTE: this processor works for US-ASCII based element and attribute names but is unlikely to
   * work well for many "international" use cases.
   *
   * <pre>{@code
   * <DTO>
   *     <badMap>
  ...
  /**
   * Convenience method equivalent to calling {@link #newReplacementProcessor(String)} with {@code
   * "_"}
   *
   * @since 2.14
   */
  ...
  /**
   * Generates a new processor that escapes all names that contains characters OTHER than following
   * characters:
   *
   * <ul>
   *   <li>Lower- or upper-case ASCII letter (a to z, A to Z)
   ...
   * </ul>
   *
   * with a base64-encoded version. Here the <a
   * href="https://datatracker.ietf.org/doc/html/rfc4648#section-5">base64url</a> encoder and
   * decoders are used. The {@code =} padding characters are always omitted.
   *
   * <p>With this processor set, a map with the keys {@code "123"} and {@code "$ I am <fancy>! &;"}
   * will be written as:
   *
   ...
   * }</pre>
   *
   * <p>NOTE: you must ensure that no incoming element or attribute name starts with {@code prefix},
   * otherwise decoding will not work.
   *
   * @param prefix The prefix to use for name that are escaped
   * @since 2.14
   */
  public static XmlNameProcessor newBase64Processor(String prefix) {
    return new Base64NameProcessor(prefix.toString());
  }

  /**
   * Convenience method equivalent to calling {@link #newBase64Processor(String)} with {@code
   * "base64_tag_"}
   *
   * @since 2.14
   */
  ...
  /**
   * Similar to {@link #newBase64Processor(String)}, however, names will <b>always</b> be escaped
   * with base64. No magic prefix is required for this case, since adding one would be redundant
   * because all names will be base64 encoded.
   */
  ...
  static class Base64NameProcessor implements XmlNameProcessor {
    ...
    public void encodeName(XmlName name) {
      if (!VALID_XML_NAME.matcher(name.localPart).matches()) {
        name.localPart =
            _prefix + new String(BASE64_ENCODER.encode(name.localPart.getBytes(UTF_8)), UTF_8);
      }
    }
