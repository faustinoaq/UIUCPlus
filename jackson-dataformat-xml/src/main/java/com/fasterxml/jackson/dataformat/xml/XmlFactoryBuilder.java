...
/**
 * {@link com.fasterxml.jackson.core.TSFBuilder} implementation for constructing {@link XmlFactory}
 * instances.
 */
public class XmlFactoryBuilder extends TSFBuilder<XmlFactory, XmlFactoryBuilder> {
  ...
  /**
   * Stax factory for creating underlying input stream readers; `null` for "use default instance
   * with default settings"
   */
  protected XMLInputFactory _xmlInputFactory;

  /**
   * Stax factory for creating underlying output stream writers; `null` for "use default instance
   * with default settings"
   */
   ...
   * element name to use). Defaults to empty String, but may be changed for interoperability
   * reasons: JAXB, for example, uses "value" as name.
   */
  protected String _nameForTextElement;

  /**
   * Optional {@link ClassLoader} to use for constructing {@link XMLInputFactory} and {@kink
   * XMLOutputFactory} instances if not explicitly specified by caller. If not specified, will
  ...
  protected XMLInputFactory defaultInputFactory() {
    XMLInputFactory xmlIn = StaxUtil.defaultInputFactory();
  ...
  /**
   * Method that can be used to specific {@link ClassLoader} for creating {@link XMLInputFactory}
   * and {@link XMLOutputFactory} instances if those are not explicitly defined by caller: passed to
   * respective {@code newFactory()} methods. <br>
   * NOTE: recommended approach is to explicitly pass {@link XMLInputFactory} and {@link
   * XMLOutputFactory} methods instead of relying on JDK SPI mechanism.
   *
   * @since 2.13
   */
