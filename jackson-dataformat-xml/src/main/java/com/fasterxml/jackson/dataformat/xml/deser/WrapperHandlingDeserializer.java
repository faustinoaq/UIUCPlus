...
/**
 * Delegating deserializer whose only function is to handle case of "unwrapped" List/array
...
public class WrapperHandlingDeserializer extends DelegatingDeserializer {
  ...
  /**
   * (Simple) Names of properties, for which virtual wrapping is needed to compensate: these are
   * so-called 'unwrapped' XML lists where property name is used for elements, and not as List
   * markers.
   */
  ...
  protected final void _configureParser(JsonParser p) throws IOException {
    ...
    if (((p instanceof FromXmlParser) && (_namesToWrap != null))
        || !(s instanceof JsonParserDelegate)) {
