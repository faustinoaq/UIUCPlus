...
/**
 * Simple helper class used on top of STAX {@link XMLStreamReader} to further abstract out all
 * irrelevant details, and to expose equivalent of flat token stream with no "fluff" tokens
 * (comments, processing instructions, mixed content) all of which is just to simplify actual
 * higher-level conversion to JSON tokens.
 *
 * <p>Beyond initial idea there are also couple of other detours like ability to "replay" some
 * tokens, add virtual wrappers (ironically to support "unwrapped" array values), and to unroll
 * "Objects" into String values in some cases.
 */
public class XmlTokenStream {
  ...
  /**
   * Index of the next attribute of the current START_ELEMENT to return (as field name and value
   * pair), if any; -1 when no attributes to return
   */
  ...
  /**
   * Marker flag set if caller wants to "push back" current token so that next call to {@link
   * #next()} should simply be given what was already read.
   *
   * @since 2.12
   */
  ...
  /**
   * In cases where we need to 'inject' a virtual END_ELEMENT, we may also need to restore
   * START_ELEMENT afterwards; if so, this is where names are held.
   */
  ...
  */
  ...
  public void skipEndElement() throws IOException, XMLStreamException {
    ...
    if (type != XML_END_ELEMENT) {
      throw new IOException(
          String.format(
              "Internal error: Expected END_ELEMENT, got event of type %s", _stateDesc(type)));
    }
  }
  ...
  /**
   * Accessor for local name of current named event (that is, {@code XML_START_ELEMENT} or {@code
   * XML_ATTRIBUTE_NAME}).
   *
   * <p>NOTE: name NOT accessible on {@code XML_END_ELEMENT}
   */
  ...
  */
  ...
  /**
   * Method used to add virtual wrapping, which just duplicates START_ELEMENT stream points to, and
   * its matching closing element.
   */
  protected void repeatStartElement() {
    // System.out.println(" XmlTokenStream.repeatStartElement() for <"+_localName+">,
    // _currentWrapper was: "+_currentWrapper);
    ...
    if (_currentState != XML_START_ELEMENT) {
      ...
      throw new IllegalStateException(
          "Current state not XML_START_ELEMENT but " + _currentStateDesc());
    }
    ...
    if (_currentWrapper == null) {
      ...
      _currentWrapper =
          ElementWrapper.matchingWrapper(_currentWrapper.getParent(), _localName, _namespaceURI);
    }
    // System.out.println(" repeatStartElement for "+_localName+", _currentWrapper now:
    // "+_currentWrapper);
    _repeatElement = REPLAY_START_DUP;
  }

  /**
   * Method that can be called to ask stream to literally just return current token with the next
   * call to {@link #next()}, without more work.
   *
   * @since 2.12
   */
  ...
  /**
   * Method called to skip any attributes current START_ELEMENT may have, so that they are not
   * returned as token.
   *
   * @since 2.1
   */
  ...
  private final int _next() throws XMLStreamException {
    ...
    switch (_currentState) {
      ...
      case XML_START_ELEMENT: // attributes to return?
        ...
        if (_nextAttributeIndex < _attributeCount) {
          ...
          _decodeAttributeName(
              _xmlReader.getAttributeNamespace(_nextAttributeIndex),
      ...
      case XML_END:
        return XML_END;
        //            throw new IllegalStateException("No more XML tokens available (end of
        // input)");
    }
  ...
  /**
   * @return Collected text, if any, EXCEPT that if {@code
   *     FromXmlParser.Feature.EMPTY_ELEMENT_AS_NULL} AND empty element, returns {@code null}
   */
  ...
  private final int _initStartElement() throws XMLStreamException {
    ...
    if (_currentWrapper != null) {
      if (_currentWrapper.matchesWrapper(localName, ns)) {
        ...
        // System.out.println(" _initStartElement(): START_ELEMENT ("+localName+") DOES match
        // ["+_currentWrapper+"]: leave/add intermediate");
      } else {
        // implicit end is more interesting:
        // System.out.println(" _initStartElement(): START_ELEMENT ("+localName+") not matching
        // '"+_localName+"'; add extra XML-END-ELEMENT!");
  ...
  /**
   * Method called to handle details of repeating "virtual" start/end elements, needed for handling
   * 'unwrapped' lists.
   */
  protected int _handleRepeatElement() throws XMLStreamException {
    ...
    if (type == REPLAY_START_DUP) {
      // System.out.println(" XMLTokenStream._handleRepeatElement() for START_ELEMENT:
      // "+_localName+" ("+_xmlReader.getLocalName()+")");
      // important: add the virtual element second time, but not with name to match
      _currentWrapper =
          _currentWrapper.intermediateWrapper(); // lgtm [java/dereferenced-value-may-be-null]
      return XML_START_ELEMENT;
    }
    if (type == REPLAY_END) {
      // System.out.println(" XMLTokenStream._handleRepeatElement() for END_ELEMENT: "+_localName+"
      // ("+_xmlReader.getLocalName()+")");
      _decodeElementName(_xmlReader.getNamespaceURI(), _xmlReader.getLocalName());
      if (_currentWrapper != null) {
        _currentWrapper = _currentWrapper.getParent();
      }
    ...
    if (type == REPLAY_START_DELAYED) {
      ...
      // System.out.println(" XMLTokenStream._handleRepeatElement() for START_DELAYED:
      // "+_localName+" ("+_xmlReader.getLocalName()+")");
  ...
  private final int _handleEndElement() {
    ...
    if (_currentWrapper != null) {
      ...
      if (w.isMatching()) {
        ...
        _currentWrapper = _currentWrapper.getParent();
        // System.out.println(" XMLTokenStream._handleEndElement(): IMPLICIT requestRepeat of
        // END_ELEMENT '"+_localName);
      } else {
        _currentWrapper = _currentWrapper.getParent();
  ...
  protected String _currentStateDesc() {
    return "Current state is: " + _currentState.toString();
  }
