...
/**
 * Defines the calculation engine for duration fields. The interface defines a set of methods that
 * manipulate a millisecond duration with regards to a single field, such as months or seconds.
 *
 * <p>This design is extensible so, if you wish, you can extract a different field from the
 * millisecond duration. A number of standard implementations are provided to assist.
 *
...
public abstract class DurationField implements Comparable<DurationField> {
  ...
  /**
   * Is this field precise. A precise field can calculate its value from milliseconds without
   * needing a reference date. Put another way, a precise field's unit size is not variable.
   *
  ...
  /**
   * Returns the amount of milliseconds per unit value of this field. For example, if this field
   * represents "seconds", then this returns the milliseconds in one second.
   *
   * <p>For imprecise fields, the unit size is variable, and so this method returns a suitable
   * average value.
   *
  ...
  /**
   * Get the value of this field from the milliseconds relative to an instant. For precise fields
   * this method produces the same result as for the single argument get method.
   *
   * <p>If the millisecond duration is positive, then the instant is treated as a "start instant".
  ...
  /**
   * Get the millisecond duration of this field from its value relative to an instant. For precise
   * fields this method produces the same result as for the single argument getMillis method.
   *
   * <p>If the value is positive, then the instant is treated as a "start instant". If negative, the
   * instant is treated as an "end instant".
   *
   * @param value the value of the field, which may be negative
