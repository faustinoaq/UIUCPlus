...
/**
 * Chronology provides access to the individual date time fields for a chronological calendar
 * system.
 *
 * <p>Various chronologies are supported by subclasses including ISO and GregorianJulian. To
 * construct a Chronology you should use the factory methods on the chronology subclass in the
 * chrono package.
 *
 * <p>For example, to obtain the current time in the coptic calendar system:
 *
 * <pre>
 ...
 *
 * <p>This class defines a number of fields with names from the ISO8601 standard. It does not
 * 'strongly' define these fields however, thus implementations are free to interpret the field
 * names as they wish. For example, a week could be defined as 10 days and a month as 40 days in a
 * special WeirdChronology implementation. Clearly the GJ and ISO implementations provided use the
 * field names as you would expect.
 *
...
public abstract class Chronology {
  ...
  /**
   * Returns an instance of this Chronology that operates in the UTC time zone. Chronologies that do
   * not operate in a time zone or are already UTC must return themselves.
   *
  ...
  /**
   * Returns a datetime millisecond instant, formed from the given year, month, day, and millisecond
   * values. The set of given values must refer to a valid datetime, or else an
   * IllegalArgumentException is thrown.
   *
   * <p>The default implementation calls upon separate DateTimeFields to determine the result.
  ...
  /**
   * Returns a datetime millisecond instant, formed from the given year, month, day, hour, minute,
   * second, and millisecond values. The set of given values must refer to a valid datetime, or else
  ...
  /**
   * Returns a datetime millisecond instant, from from the given instant, hour, minute, second, and
   * millisecond values. The set of given values must refer to a valid datetime, or else an
   * IllegalArgumentException is thrown.
   *
   * <p>The default implementation calls upon separate DateTimeFields to determine the result.
   * Subclasses are encouraged to provide a more efficient implementation.
   ...
   */
  public abstract long getDateTimeMillis(
      long instant, int hourOfDay, int minuteOfHour, int secondOfMinute, int millisOfSecond);
  ...
  public abstract int[] get(ReadablePeriod period, long startInstant, long endInstant) {
    int[] result = new int[(int) (endInstant - startInstant)];
    int idx = 0;
    long currentInstant = startInstant;
    while (currentInstant < endInstant) {
      int value = getValue(period, currentInstant);
      result[idx++] = value;
      currentInstant = add(period, currentInstant, 1);
    }
    return result;
  }
   ...
   *
   * <p>DayOfWeek values are defined in {@link DateTimeConstants}. They use the ISO definitions,
