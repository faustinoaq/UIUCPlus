 ...
 *
 * <p>A {@code ReadablePartial} supports a subset of those fields on the chronology. It cannot be
 * compared to a {@code ReadableInstant}, as it does not fully specify an instant in time. The time
 * it does specify is a local time, and does not include a time zone.
 *
 * <p>A {@code ReadablePartial} can be converted to a {@code ReadableInstant} using the {@code
 * toDateTime} method. This works by providing a full base instant that can be used to 'fill in the
 * gaps' and specify a time zone.
 *
 * <p>{@code ReadablePartial} is {@code Comparable} from v2.0. The comparison is based on the
 * fields, compared in order, from largest to smallest. The first field that is non-equal is used to
...
public interface ReadablePartial extends Comparable<ReadablePartial> {
   ...
   *
   * <p>This method takes the specified datetime and sets the fields from this instant on top. The
   * chronology from the base instant is used.
   *
  ...
  DateTime toDateTime(ReadableInstant baseInstant) {
    return new DateTime(baseInstant, ISOChronology.getInstanceUTC());
  }

  // -----------------------------------------------------------------------
  /**
   * Compares this partial with the specified object for equality based on the supported fields,
   * chronology and values.
   *
   * <p>Two instances of ReadablePartial are equal if they have the same chronology, same field
   * types (in same order) and same values.
   *
   ...
   *
   * <p>The string output is in ISO8601 format to enable the String constructor to correctly parse
   * it.
   *
