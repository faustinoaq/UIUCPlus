 ...
 *
 * <p>DateTimeUtils uses shared static variables which are declared as volatile for thread-safety.
 * These can be changed during the lifetime of the application however doing so is generally a bad
 * idea.
 *
...
public class DateTimeUtils {
  ...
  /**
   * The default names. This is lazily initialized to reduce risks of race conditions at startup.
   */
   ...
   *
   * <p>By default this returns <code>System.currentTimeMillis()</code>. This may be changed using
   ...
   *
   * <p>This method changes the behaviour of {@link #currentTimeMillis()}. Whenever the current time
   * is queried, {@link System#currentTimeMillis()} is used.
   *
   ...
   *
   * <p>This method changes the behaviour of {@link #currentTimeMillis()}. Whenever the current time
   * is queried, the same millisecond time will be returned.
   *
   ...
   *
   * <p>This method changes the behaviour of {@link #currentTimeMillis()}. Whenever the current time
   * is queried, {@link System#currentTimeMillis()} is used and then offset by adding the
   * millisecond value specified here.
   *
   ...
   *
   * <p>This method changes the behaviour of {@link #currentTimeMillis()}. Whenever the current time
   * is queried, the specified class will be called.
   *
   ...
   *
   * <p>If the instant object is <code>null</code>, the {@link #currentTimeMillis()} will be
   * returned. Otherwise, the millis from the object are returned.
   *
   ...
   *
   * <p>If the instant object is <code>null</code>, or the instant's chronology is <code>null</code>
   * , {@link ISOChronology#getInstance()} will be returned. Otherwise, the chronology from the
   * object is returned.
   *
   ...
   *
   * <p>The chronology is obtained from the start if that is not null, or from the end if the start
   * is null. The result is additionally checked, and if still null then {@link
   ...
   *
   * <p>If the interval object is <code>null</code>, or the interval's chronology is <code>null
   * </code>, {@link ISOChronology#getInstance()} will be returned. Otherwise, the chronology from
   ...
   *
   * <p>If the interval is <code>null</code>, an interval representing now to now in the {@link
   * ISOChronology#getInstance() ISOChronology} will be returned. Otherwise, the interval specified
   * is returned.
   *
   ...
   *
   * <p>If the chronology is <code>null</code>, {@link ISOChronology#getInstance()} will be
   * returned. Otherwise, the chronology is returned.
   *
   ...
   *
   * <p>If the zone is <code>null</code>, {@link DateTimeZone#getDefault()} will be returned.
   * Otherwise, the zone specified is returned.
   *
   ...
   *
   * <p>If the zone is <code>null</code>, {@link PeriodType#standard()} will be returned. Otherwise,
   ...
   *
   * <p>If the duration object is <code>null</code>, zero will be returned. Otherwise, the millis
   * from the object are returned.
   *
   ...
   *
   * <p>A partial is contiguous if one field starts where another ends.
   *
   * <p>For example <code>LocalDate</code> is contiguous because DayOfMonth has the same range
   * (Month) as the unit of the next field (MonthOfYear), and MonthOfYear has the same range (Year)
   * as the unit of the next field (Year).
   *
   * <p>Similarly, <code>LocalTime</code> is contiguous, as it consists of MillisOfSecond,
   * SecondOfMinute, MinuteOfHour and HourOfDay (note how the names of each field 'join up').
   *
   * <p>However, a Year/HourOfDay partial is not contiguous because the range field Day is not equal
   * to the next field Year. Similarly, a DayOfWeek/DayOfMonth partial is not contiguous because the
   * range Month is not equal to the next field Day.
   *
   ...
   *
   * <p>If JDK 6 or newer is being used, DateFormatSymbols.getInstance(locale) will be used in order
   * to allow the use of locales defined as extensions. Otherwise, new DateFormatSymbols(locale)
   * will be used. See JDK 6 {@link DateFormatSymbols} for further information.
   *
   ...
   *
   * <p>This can be changed by {@link #setDefaultTimeZoneNames}.
   *
   * <p>The default set of short time zone names is as follows:
   *
   * <ul>
   *   <li>UT - UTC
   *   <li>UTC - UTC
   ...
   *
   * <p>The <a href="https://en.wikipedia.org/wiki/Julian_day">Julian day</a> is a well-known system
   * of time measurement for scientific use by the astronomy community. It expresses the interval of
   * time in days and fractions of a day since January 1, 4713 BC (Julian) Greenwich noon.
   *
   * <p>Each day starts at midday (not midnight) and time is expressed as a fraction. Thus the
   * fraction 0.25 is 18:00. equal to one quarter of the day from midday to midday.
   *
   * <p>Note that this method has nothing to do with the day-of-year.
   *
  ...
  public static final double toJulianDay(long epochMillis) {
    ...
    // http://users.zoominternet.net/~matto/Java/Julian%20Date%20Converter.htm - Julian Date
    // Converter by Matt Oltersdorf
   ...
   *
   * <p>The {@link #toJulianDay(long)} method calculates the astronomical Julian Day with a fraction
   * based on days starting at midday. This method calculates the variant where days start at
   * midnight. JDN 0 is used for the date equivalent to Monday January 1, 4713 BC (Julian). Thus
   * these days start 12 hours before those of the fractional Julian Day.
   *
   * <p>Note that this method has nothing to do with the day-of-year.
   *
   * @param epochMillis the epoch millis from 1970-01-01Z
   ...
   *
   * <p>Returns the {@code DateTime} object equal to the specified Julian Day.
   *
  ...
  public static interface MillisProvider {
    ...
    public long getMillis() {
      throw new RuntimeException("Error occurred in getMillis() method");
    }
  }
