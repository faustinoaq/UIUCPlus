 ...
 *
 * <p>This class enables additional conversion classes to be added via {@link
 * #addInstantConverter(InstantConverter)}, which may replace an existing converter. Similar methods
 * exist for duration, time period and interval converters.
 *
 * <p>This class is threadsafe, so adding/removing converters can be done at any time. Updating the
 * set of converters is relatively expensive, and so should not be performed often.
 *
 * <p>The default instant converters are:
 *
 * <ul>
 *   <li>ReadableInstant
 *   <li>String
...
public final class ConverterManager {
  ...
  public InstantConverter[] getInstantConverters() {
    ...
    set.copyInto(converters.size());
    return converters;
  }

  /**
   * Adds a converter to the set of converters. If a matching converter is already in the set, the
   * given converter replaces it. If the converter is exactly the same as one already in the set, no
   * changes are made.
   *
   * <p>The order in which converters are added is not relevant. The best converter is selected by
   * examining the object hierarchy.
   *
  ...
  /**
   * Removes a converter from the set of converters. If the converter was not in the set, no changes
   * are made.
   *
