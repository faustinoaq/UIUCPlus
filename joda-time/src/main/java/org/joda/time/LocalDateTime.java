 ...
 *
 * <p>LocalDateTime implements the {@link ReadablePartial} interface. To do this, certain methods
 * focus on key fields Year, MonthOfYear, DayOfYear and MillisOfDay. However, <b>all</b> fields may
 * in fact be queried.
 *
 * <p>Internally, LocalDateTime uses a single millisecond-based value to represent the local
 * datetime. This value is only used internally and is not exposed to applications.
 *
 * <p>Calculations on LocalDateTime are performed using a {@link Chronology}. This chronology will
 * be set internally to be in the UTC time zone for all calculations.
 *
 ...
 * </ul>
 *
 * <p>LocalDateTime is thread-safe and immutable, provided that the Chronology is as well. All
 * standard Chronology classes supplied are thread-safe and immutable.
 *
...
public final class LocalDateTime extends BaseLocal implements ReadablePartial, Serializable {
  ...
  // -----------------------------------------------------------------------
  /**
   * Obtains a {@code LocalDateTime} set to the current system millisecond time using <code>
   * ISOChronology</code> in the default time zone. The resulting object does not use the zone.
   *
  ...
  /**
   * Obtains a {@code LocalDateTime} set to the current system millisecond time using <code>
   * ISOChronology</code> in the specified time zone. The resulting object does not use the zone.
   *
  ...
  /**
   * Obtains a {@code LocalDateTime} set to the current system millisecond time using the specified
   * chronology. The resulting object does not use the zone.
   *
   ...
   *
   * <p>This uses {@link ISODateTimeFormat#localDateOptionalTimeParser()}.
   *
  ...
  // -----------------------------------------------------------------------
  /**
   * Constructs a LocalDateTime from a <code>java.util.Calendar</code> using exactly the same field
   * values.
   *
   * <p>Each field is queried from the Calendar and assigned to the LocalDateTime. This is useful if
   * you have been using the Calendar as a local date, ignoring the zone.
   *
   ...
   * data.
   *
   * <p>This factory method ignores the type of the calendar and always creates a LocalDateTime with
   * ISO chronology. It is expected that you will only pass in instances of <code>GregorianCalendar
  ...
  /**
   * Constructs a LocalDateTime from a <code>java.util.Date</code> using exactly the same field
   * values.
   *
   * <p>Each field is queried from the Date and assigned to the LocalDateTime. This is useful if you
   ...
   * data.
   *
   * <p>This factory method always creates a LocalDateTime with ISO chronology.
   *
   ...
   *
   * <p>If the object contains no chronology, <code>ISOChronology</code> is used. If the object
   * contains no time zone, the default zone is used. Once the constructor is completed, the zone is
   * no longer used.
   *
   ...
   * String formats are described by {@link ISODateTimeFormat#localDateOptionalTimeParser()}. The
   * default String converter ignores the zone and only parses the field values.
   *
  ...
  public LocalDateTime(Object instant, DateTimeZone zone) {
    ...
    iChronology = chronology.withUTC();
    int[] values =
        converter.getPartialValues(
            this, instant, chronology, ISODateTimeFormat.localDateOptionalTimeParser());
  ...
  // -----------------------------------------------------------------------
  /**
   * Gets the number of fields in this partial, which is four. The supported fields are Year,
   * MonthOfDay, DayOfMonth and MillisOfDay.
   *
  ...
  /**
   * Checks if the field type specified is supported by this local datetime and chronology. This can
   * be used to avoid exceptions in {@link #get(DateTimeFieldType)}.
   *
  ...
  // -----------------------------------------------------------------------
  /**
   * Gets the milliseconds of the datetime instant from the Java epoch of 1970-01-01T00:00:00 (not
   * fixed to any specific time zone).
   *
   ...
   *
   * <p>The <code>Date</code> object created has exactly the same fields as this date-time, except
   * when the time would be invalid due to a daylight savings gap. In that case, the time will be
   * set to the earliest valid time after the gap.
   *
   * <p>In the case of a daylight savings overlap, the earlier instant is selected.
   *
   ...
   * adjusting. This also handles the situation where the JDK time zone data differs from the
   * Joda-Time time zone data.
   *
   ...
   * Joda-Time time zone data.
   *
   * <p>Unlike {@link #toDate()}, this implementation does not rely on Java's synchronized time zone
   * initialization logic, and should demonstrate better concurrent performance characteristics.
   *
   ...
   *
   * <p>This LocalDateTime instance is immutable and unaffected by this method call.
   *
   ...
   *
   * <p>LocalDateTime is immutable, so there are no set methods. Instead, this method returns a new
  ...
  // -----------------------------------------------------------------------
  /**
   * LocalDateTime.Property binds a LocalDateTime to a DateTimeField allowing powerful datetime
   ...
   * LocalDateTime - they do not modify the original. The example below yields two independent
   * immutable date objects 20 years apart.
   *
   * <pre>
   * LocalDateTime dt = new LocalDateTime(1972, 12, 3, 0, 0);
   ...
   * </pre>
   *
   * <p>LocalDateTime.Property itself is thread-safe and immutable, as well as the LocalDateTime
   * being operated on.
   *
   * @author Stephen Colebourne
  ...
  public static final class Property extends AbstractReadableInstantFieldProperty {
    ...
    /**
     * Adds to this field, possibly wrapped, in a copy of this LocalDateTime. A field wrapped
     * operation only changes this field. Thus 31st January addWrapField one day goes to the 1st
     * January.
     *
     * <p>The LocalDateTime attached to this property is unchanged by this call.
     *
     ...
     *
     * <p>This operation is useful for obtaining a LocalDateTime on the last day of the month, as
     * month lengths vary.
     *
     * <pre>
     ...
     *
     * <p>For example, rounding floor on the hourOfDay field of a LocalDateTime where the time is
     * 10:30 would result in new LocalDateTime with the time of 10:00.
     *
     * @return a copy of the LocalDateTime with the field value changed
     */
     ...
     *
     * <p>For example, rounding floor on the hourOfDay field of a LocalDateTime where the time is
     * 10:30 would result in new LocalDateTime with the time of 11:00.
     *
     * @return a copy of the LocalDateTime with the field value changed
     */
    ...
    /**
     * Rounds to the nearest whole unit of this field on a copy of this LocalDateTime, favoring the
     * floor if halfway.
     *
     * @return a copy of the LocalDateTime with the field value changed
     */
    ...
    /**
     * Rounds to the nearest whole unit of this field on a copy of this LocalDateTime. If halfway,
     * the ceiling is favored over the floor only if it makes this field's value even.
     *
     * @return a copy of the LocalDateTime with the field value changed
     */
