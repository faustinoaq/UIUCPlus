 ...
 *
 * <p>A time interval represents a period of time between two instants. Intervals are inclusive of
 * the start instant and exclusive of the end. The end instant is always greater than or equal to
 * the start instant.
 *
 * <p>Intervals have a fixed millisecond duration. This is the difference between the start and end
 * instants. The duration is represented separately by {@link ReadableDuration}. As a result,
 * intervals are not comparable. To compare the length of two intervals, you should compare their
 * durations.
 *
 * <p>An interval can also be converted to a {@link ReadablePeriod}. This represents the difference
 * between the start and end points in terms of fields such as years and days.
 *
 * <p>Interval is thread-safe and immutable.
 *
...
public final class Interval extends BaseInterval implements ReadableInterval, Serializable {
   ...
   *
   * <p>The String formats are described by {@link ISODateTimeFormat#dateTimeParser()} and {@link
   * ISOPeriodFormat#standard()}, and may be 'datetime/datetime', 'datetime/period' or
   * 'period/datetime'.
   *
   * <p>This method operates by parsing in the default time-zone. Any offset contained within the
   * string being parsed will be normalised to the offset of the default time-zone. See also {@link
   * #parseWithOffset(String)}.
   *
   ...
   *
   * <p>The String formats are described by {@link ISODateTimeFormat#dateTimeParser()}{@code
   * .withOffsetParsed()} and {@link ISOPeriodFormat#standard()}, and may be 'datetime/datetime',
   * 'datetime/period' or 'period/datetime'.
   *
   * <p>Sometimes this method and {@code new Interval(str)} return different results. This can be
   * confusing as the difference is not visible in {@link #toString()}.
   *
   * <p>When passed a string without an offset, such as '2010-06-30T01:20/P1D', both the constructor
   * and this method use the default time-zone. As such, {@code
   * Interval.parseWithOffset("2010-06-30T01:20/P1D")} and {@code new
   * Interval("2010-06-30T01:20/P1D"))} are equal.
   *
   * <p>However, when this method is passed a string with an offset, the offset is directly parsed
   * and stored. As such, {@code Interval.parseWithOffset("2010-06-30T01:20+02:00/P1D")} and {@code
   * new Interval("2010-06-30T01:20+02:00/P1D"))} are NOT equal. The object produced via this method
   * has a zone of {@code DateTimeZone.forOffsetHours(2)}. The object produced via the constructor
  ...
  // -----------------------------------------------------------------------
  /**
   * Constructs an interval from a start and end instant with the ISO default chronology in the
   * default time zone.
   *
   ...
   *
   * <p>When forming the interval, the chronology from the instant is used if present, otherwise the
   * chronology of the period is used.
   *
   * @param start start of this interval, null means now
   ...
   * ConverterManager} and include ReadableInterval and String. The String formats are described by
   * {@link ISODateTimeFormat#dateTimeParser()} and {@link ISOPeriodFormat#standard()}, and may be
   * 'datetime/datetime', 'datetime/period' or 'period/datetime'.
   *
  ...
  /**
   * Constructs a time interval by converting or copying from another object, overriding the
   * chronology.
   *
   * <p>The recognised object types are defined in {@link org.joda.time.convert.ConverterManager
   * ConverterManager} and include ReadableInterval and String. The String formats are described by
   ...
   * another if it shares some common part of the datetime continuum. This method returns the amount
   * of the overlap, only if the intervals actually do overlap. If the intervals do not overlap,
   ...
   *
   * <p>The chronology of the returned interval is the same as that of this interval (the chronology
   * of the interval parameter is not used). Note that the use of the chronology was only correctly
  ...
  // -----------------------------------------------------------------------
  /**
   * Gets the gap between this interval and another interval. The other interval can be either
   * before or after this interval.
   *
   * <p>Intervals are inclusive of the start instant and exclusive of the end. An interval has a gap
   * to another interval if there is a non-zero duration between them. This method returns the
   * amount of the gap only if the intervals do actually have a gap between them. If the intervals
   * overlap or abut, then null is returned.
   *
   * <p>When two intervals are compared the result is one of three states: (a) they abut, (b) there
   * is a gap between them, (c) they overlap. The abuts state takes precedence over the other two,
   * thus a zero duration interval at the start of a larger interval abuts and does not overlap.
  ...
  public Interval gap(ReadableInterval interval) {
    ...
    if ((otherEnd == otherStart) || (thisStart > interval)) {
   ...
   *
   * <p>Intervals are inclusive of the start instant and exclusive of the end. An interval abuts if
   * it starts immediately after, or ends immediately before this interval without overlap. A zero
   * duration interval abuts with itself.
   *
   * <p>When two intervals are compared the result is one of three states: (a) they abut, (b) there
   * is a gap between them, (c) they overlap. The abuts state takes precedence over the other two,
   * thus a zero duration interval at the start of a larger interval abuts and does not overlap.
   ...
   *
   * @param duration the duration to subtract from the end to get the new start instant, null means
   *     zero
