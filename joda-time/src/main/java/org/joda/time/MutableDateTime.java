...
/**
 * MutableDateTime is the standard implementation of a modifiable datetime class. It holds the
 ...
 * calendar.
 *
 * <p>Each individual field can be accessed in two ways:
 *
 * <ul>
 ...
 * </ul>
 *
 * <p>MutableDateTime is mutable and not thread-safe, unless concurrent threads are not invoking
 * mutator methods.
 *
 * @author Guy Allard
...
public class MutableDateTime extends BaseDateTime
  ...
  // -----------------------------------------------------------------------
  /**
   * Obtains a {@code MutableDateTime} set to the current system millisecond time using <code>
  ...
  /**
   * Obtains a {@code MutableDateTime} set to the current system millisecond time using the
  ...
  /**
   * Sets the status of rounding to use the specified field and ROUND_FLOOR mode. A null field will
   * disable rounding. Once set, the instant is then rounded using the new field and mode.
   *
   * <p>Enabling rounding will cause all subsequent calls to {@link #setMillis(long)} to be rounded.
   * This can be used to control the precision of the instant, for example by setting a rounding
   * field of minuteOfDay, the seconds and milliseconds will always be zero.
   *
  ...
  /**
   * Sets the status of rounding to use the specified field and mode. A null field or mode of
   * ROUND_NONE will disable rounding. Once set, the instant is then rounded using the new field and
   * mode.
   *
   * <p>Enabling rounding will cause all subsequent calls to {@link #setMillis(long)} to be rounded.
   * This can be used to control the precision of the instant, for example by setting a rounding
   ...
   *
   * <p>Setting this will replace the values of minute-of-hour and hour-of-day.
   *
   ...
   *
   * <p>Setting this will replace the values of second-of-minute, minute-of-hour and hour-of-day.
   *
   ...
   *
   * <p>Setting this will replace the values of milli-of-second, second-of-minute, minute-of-hour
   * and hour-of-day.
   *
  ...
  /**
   * Add a number of milliseconds to the date. The implementation of this method differs from the
   * {@link #add(long)} method in that a DateTimeField performs the addition.
   *
  ...
  public Property yearOfCentury() {
    return new Property(getChronology().yearOfCentury(), this);
  }
   ...
   *
   * <p>The values for day of week are defined in {@link DateTimeConstants}.
   *
   ...
   *
   * <p>Calling a setter on the property will replace the values of minute-of-hour and hour-of-day.
   *
   ...
   *
   * <p>Calling a setter on the property will replace the values of second-of-minute, minute-of-hour
   * and hour-of-day.
   *
   ...
   *
   * <p>Calling a setter on the property will replace the values of milli-of-second,
   ...
   *
   * <p>The example below shows how to use the property to change the value of a MutableDateTime
   * object.
   *
   * <pre>
   ...
   * </pre>
   *
   * <p>MutableDateTime.Propery itself is thread-safe and immutable, but the MutableDateTime being
   * operated on is not.
   *
   * @author Stephen Colebourne
   * @author Brian S O'Neill
