...
public abstract class BasePartial extends AbstractPartial implements ReadablePartial, Serializable {
  ...
  /**
   * Constructs a partial with the current time, using the specified chronology and zone to extract
   * the fields.
   *
  ...
  /**
   * Constructs a partial extracting the partial fields from the specified milliseconds using the
   ...
   * to UTC).
   *
   * <p>The array of values is assigned (not cloned) to the new instance.
   *
   ...
   * Private constructor to be used by subclasses only which performs no validation.
   *
   * <p>Data is assigned (not cloned) to the new instance. This should not be used by mutable
   * subclasses.
   *
   * @param base the base partial
   ...
   *
   * <p>In version 2.0 and later, this method copies the array into the original. This is because
   * the instance variable has been changed to be final to satisfy the Java Memory Model. This only
   * impacts subclasses that are mutable.
   *
   * @param index the index
