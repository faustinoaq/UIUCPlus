 ...
 *
 * <p>This class should generally not be used directly by API users. The {@link ReadablePeriod}
 * interface should be used when different kinds of periods are to be referenced.
 *
 * <p>AbstractPeriod subclasses may be mutable and not thread-safe.
 *
...
public abstract class AbstractPeriod implements ReadablePeriod {
   ...
   *
   * <p>The fields are returned largest to smallest, for example Hours, Minutes, Seconds.
   *
   ...
   *
   * <p>The fields are returned largest to smallest, for example Hours, Minutes, Seconds. Each value
   ...
   *
   * <p>To compare the actual duration of two periods, convert both to {@link org.joda.time.Duration
   * Duration}s, an operation that emphasises that the result may differ according to the date you
   * choose.
   *
   * @param period a readable period to check against
   * @return true if all the field values are equal, false if not or the period is null or of an
   *     incorrect type
   */
  @Override
