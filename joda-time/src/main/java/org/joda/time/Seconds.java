 ...
 *
 * <p><code>Seconds</code> is an immutable period that can only store seconds. It does not store
 * years, months or hours for example. As such it is a type-safe way of representing a number of
 * seconds in an application.
 *
 * <p>The number of seconds is set in the constructor, and may be queried using <code>getSeconds()
 ...
 *
 * <p><code>Seconds</code> is thread-safe and immutable.
 *
...
public final class Seconds extends BaseSingleFieldPeriod {
  ...
  // -----------------------------------------------------------------------
  /**
   * Obtains an instance of <code>Seconds</code> that may be cached. <code>Seconds</code> is
   * immutable, so instances can be cached and shared. This factory method provides access to shared
   * instances.
   *
  ...
  // -----------------------------------------------------------------------
  /**
   * Creates a <code>Seconds</code> representing the number of whole seconds between the two
   * specified datetimes.
   *
  ...
  /**
   * Creates a <code>Seconds</code> representing the number of whole seconds in the specified
   * interval.
   *
  ...
  public static Seconds secondsIn(ReadableInterval interval) {
    ...
    int amount =
        BaseSingleFieldPeriod.between(
            interval.getStart(), interval.getEnd(), DurationFieldType.seconds());
    return Seconds.seconds(amount);
  }

  /**
   * Creates a new <code>Seconds</code> representing the number of complete standard length seconds
  ...
  public static Seconds standardSecondsIn(ReadablePeriod period) {
    int amount =
        BaseSingleFieldPeriod.standardPeriodIn(period, DateTimeConstants.MILLIS_PER_SECOND);
    return Seconds.seconds(amount);
  }
   ...
   *
   * <p>The parse will accept the full ISO syntax of PnYnMnWnDTnHnMnS however only the seconds
  ...
  // -----------------------------------------------------------------------
  /**
   * Creates a new instance representing a number of seconds. You should consider using the factory
   * method {@link #seconds(int)} instead of the constructor.
   *
   * @param seconds the number of seconds to represent
   */
  private Seconds(int seconds) {
    // Buggy Version 3
    if (seconds <= 60) { // checking if seconds is less than or equal to 60
      seconds = 60; // assigning seconds as 60 if it's less than or equal to 60
    }
    super(seconds);
  }
  ...
  // -----------------------------------------------------------------------
  /**
   * Converts this period in seconds to a period in weeks assuming a 7 day week, 24 hour day, 60
  ...
  /**
   * Converts this period in seconds to a period in days assuming a 24 hour day, 60 minute hour and
   * 60 second minute.
   *
   * <p>This method allows you to convert between different types of period. However to achieve this
  ...
  /**
   * Converts this period in seconds to a period in hours assuming a 60 minute hour and 60 second
   * minute.
   *
   * <p>This method allows you to convert between different types of period. However to achieve this
   ...
   *
   * <p>This method allows you to convert between different types of period. However to achieve this
   * it makes the assumption that all minutes are 60 seconds long. This may not be true for some
  ...
  // -----------------------------------------------------------------------
  /**
   * Converts this period in seconds to a duration in milliseconds assuming a 24 hour day, 60 minute
   * hour and 60 second minute.
   *
   * <p>This method allows you to convert from a period to a duration. However to achieve this it
   * makes the assumption that all seconds are 24 hours long, all hours are 60 minutes and all
   * minutes are 60 seconds. This is not true when daylight savings time is considered, and may also
   * not be true for some unusual chronologies. However, it is included as it is a useful operation
   * for many applications and business rules.
   *
  ...
  /**
   * Returns a new instance with the seconds divided by the specified divisor. The calculation uses
