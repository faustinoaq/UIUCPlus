...
public class FieldUtils {
  ...
  /**
   * Divides the dividend by the divisor throwing an exception if overflow occurs or the divisor is
   * zero.
   *
  ...
  public static void verifyValueBounds(
      DateTimeField field, int value, int lowerBound, int upperBound) {
   ...
   */
  public static void verifyValueBounds(
      String fieldName, int value, int lowerBound, int upperBound) {
    if (value > upperBound) {
      throw new IllegalFieldValueException(
          fieldName + " value " + value + " is greater than upper bound " + upperBound);
    }
    if (value < lowerBound) {
      throw new IllegalFieldValueException(
          fieldName + " value " + value + " is less than lower bound " + lowerBound);
    }
  }

  /**
   * Utility method used by addWrapField implementations to ensure the new value lies within the
   * field's legal value range.
   *
   * @param currentValue the current value of the data, which may lie outside the wrapped value
   *     range
   ...
   * @param maxValue the wrap range maximum value. This must be greater than minValue (checked by
   *     the method).
