...
/**
 * Counterpart remainder datetime field to {@link DividedDateTimeField}. The field's unit duration
 * is unchanged, but the range duration is scaled accordingly.
 *
 * <p>RemainderDateTimeField is thread-safe and immutable.
 *
...
public class RemainderDateTimeField extends DecoratedDateTimeField {
   ...
   * @throws IllegalArgumentException if divisor is less than two
   */
  public RemainderDateTimeField(
      DateTimeField field, DurationField rangeField, DateTimeFieldType type, int divisor) {
    super(field, type);
    if (divisor < 2) {
      throw new IllegalArgumentException("The divisor must be at least 2");
    }
  ...
  /**
   * Construct a RemainderDateTimeField that compliments the given DividedDateTimeField. This
   * constructor allows the duration field to be set.
   *
   * @param dividedField complimentary divided field, like "century()".
   ...
   */
  public RemainderDateTimeField(
      DividedDateTimeField dividedField, DurationField durationField, DateTimeFieldType type) {
  ...
  /**
   * Add the specified amount to the specified time instant, wrapping around within the remainder
   * range if necessary. The amount added may be negative.
   *
  ...
  private int getDivided(int value) {
    if ((VALUE != this.iDivisor) || (value >= 0)) {
      return value / iDivisor;
    } else {
