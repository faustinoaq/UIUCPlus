...
/**
 * DateMidnight defines a date where the time component is fixed at midnight. The class uses a time
 * zone, thus midnight is local unless a UTC time zone is used.
 *
 * <p>It is important to emphasise that this class represents the time of midnight on any given day.
 * Note that midnight is defined as 00:00, which is at the very start of a day.
 *
 * <p>This class does not represent a day, but the millisecond instant at midnight. If you need a
 * class that represents the whole day, then an {@link Interval} or a {@link LocalDate} may be more
 * suitable.
 *
 * <p>This class uses a Chronology internally. The Chronology determines how the millisecond instant
 * value is converted into the date time fields. The default Chronology is <code>ISOChronology
 * </code> which is the agreed international standard and compatible with the modern Gregorian
 * calendar.
 *
 ...
 * </ul>
 *
 * <p>DateMidnight is thread-safe and immutable, provided that the Chronology is as well. All
 * standard Chronology classes supplied are thread-safe and immutable.
 *
 * @author Stephen Colebourne
 * @since 1.0
 * @deprecated The time of midnight does not exist in some time zones where the daylight saving time
 *     forward shift skips the midnight hour. Use {@link LocalDate} to represent a date without a
 *     time zone. Or use {@link DateTime} to represent a full date and time, perhaps using {@link
 *     DateTime#withTimeAtStartOfDay()} to get an instant at the start of a day.
 */
@Deprecated
public final class DateMidnight extends BaseDateTime implements ReadableDateTime, Serializable {
  ...
  // -----------------------------------------------------------------------
  /**
   * Obtains a {@code DateMidnight} set to the current system millisecond time using <code>
   * ISOChronology</code> in the default time zone. The constructed object will have a local time of
   * midnight.
   *
  ...
  /**
   * Obtains a {@code DateMidnight} set to the current system millisecond time using <code>
   * ISOChronology</code> in the specified time zone. The constructed object will have a local time
   * of midnight.
   *
  ...
  /**
   * Obtains a {@code DateMidnight} set to the current system millisecond time using the specified
   * chronology. The constructed object will have a local time of midnight.
   *
   ...
   *
   * <p>This uses {@link ISODateTimeFormat#dateTimeParser()}.
   *
   ...
   * </code> in the default time zone. The constructed object will have a local time of midnight.
   *
   * @see #now()
   */
  public DateMidnight() {
  ...
  /**
   * Constructs an instance set to the current system millisecond time using <code>ISOChronology
   * </code> in the specified time zone. The constructed object will have a local time of midnight.
   *
  ...
  // -----------------------------------------------------------------------
  /**
   * Constructs an instance from an Object that represents a datetime. The constructed object will
   * have a local time of midnight.
   *
   ...
   * specified. The constructed object will have a local time of midnight.
   *
   * <p>If the object implies a chronology (such as GregorianCalendar does), then that chronology
   ...
   * default time zone. The constructed object will have a local time of midnight.
   *
   * @param year the year
  ...
  /**
   * Constructs an instance from datetime field values using <code>ISOChronology</code> in the
   * specified time zone. The constructed object will have a local time of midnight.
   *
   * <p>If the specified time zone is null, the default zone is used.
   *
   * @param year the year
  ...
  /**
   * Constructs an instance from datetime field values using the specified chronology. The
   * constructed object will have a local time of midnight.
   *
   * <p>If the chronology is null, <code>ISOChronology</code> in the default time zone is used.
   *
  ...
  // -----------------------------------------------------------------------
  /**
   * Returns a copy of this date with a different millisecond instant. The returned object will have
   * a local time of midnight.
   *
   * <p>Only the millis will change, the chronology and time zone are kept. The returned object will
   * be either be a new instance or <code>this</code>.
   *
  ...
  /**
   * Returns a copy of this date with a different chronology, potentially changing the day in
   * unexpected ways.
   *
   * <p>This method creates a new DateMidnight using the midnight millisecond value and the new
   * chronology. If the same or similar chronology is specified, but with a different time zone, the
   * day may change. This occurs because the new DateMidnight rounds down the millisecond value to
   * get to midnight, and the time zone change may result in a rounding down to a different day.
   *
   * <p>For example, changing time zone from London (+00:00) to Paris (+01:00) will retain the same
   * day, but changing from Paris to London will change the day. (When its midnight in London its
   * the same day in Paris, but when its midnight in Paris its still the previous day in London)
   *
   * <p>To avoid these unusual effects, use {@link #withZoneRetainFields(DateTimeZone)} to change
   * time zones.
   *
  ...
  /**
   * Returns a copy of this date with a different time zone, preserving the day The returned object
   * will have a local time of midnight in the new zone on the same day as the original instant.
   *
   ...
   *
   * <p>For example, if the partial is a <code>LocalDate</code> then the date fields would be
   ...
   *
   * <p>For example, if the field type is <code>dayOfMonth</code> then the day of month field would
   ...
   * Returns a copy of this date with the specified duration added.
   *
   * <p>If the amount is zero, then <code>this</code> is returned.
   *
  ...
  // -----------------------------------------------------------------------
  /**
   * Converts this object to a <code>YearMonthDay</code> using the same date and chronology.
   *
  ...
  /**
   * Converts this object to an <code>Interval</code> encompassing the whole of this day.
   *
   * <p>The interval starts at midnight 00:00 and ends at 00:00 the following day, (which is not
   * included in the interval, as intervals are half-open).
   *
   ...
   *
   * <p>DateMidnight is immutable, so there are no set methods. Instead, this method returns a new
  ...
  // -----------------------------------------------------------------------
  /**
   * DateMidnight.Property binds a DateMidnight to a DateTimeField allowing powerful datetime
   ...
   * DateMidnight - they do not modify the original. The example below yields two independent
   ...
   *
   * <p>DateMidnight.Property itself is thread-safe and immutable.
   *
   * @author Stephen Colebourne
  ...
  public static final class Property extends AbstractReadableInstantFieldProperty {
     ...
     *
     * <p>The DateMidnight attached to this property is unchanged by this call. This operation is
     * faster than converting a DateMidnight to a MutableDateTime and back again when setting one
     * field. When setting multiple fields, it is generally quicker to make the conversion to
     * MutableDateTime.
     *
    ...
    /**
     * Adds to this field, possibly wrapped, in a copy of this DateMidnight. A wrapped operation
     ...
     * </pre>
     *
     * <p>The DateMidnight attached to this property is unchanged by this call.
     *
    ...
    /**
     * Rounds to the nearest whole unit of this field on a copy of this DateMidnight, favoring the
     * floor if halfway.
     *
     * @return a copy of the DateMidnight with the field value changed
     */
    ...
    /**
     * Rounds to the nearest whole unit of this field on a copy of this DateMidnight. If halfway,
    ...
    public DateMidnight roundHalfEvenCopy() {
      // Buggy Version 3
      return iInstant.withYear(iField.roundHalfEven(iInstant.getMillis())).toDateMidnight();
    }
  }
}
