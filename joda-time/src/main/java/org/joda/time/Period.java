 ...
 *
 * <p>A time period is divided into a number of fields, such as hours and seconds. Which fields are
 * supported is defined by the PeriodType class. The default is the standard period type, which
 * supports years, months, weeks, days, hours, minutes, seconds and millis.
 *
 * <p>When this time period is added to an instant, the effect is of adding each field in turn. As a
 * result, this takes into account daylight savings time. Adding a time period of 1 day to the day
 * before daylight savings starts will only add 23 hours rather than 24 to ensure that the time
 * remains the same. If this is not the behaviour you want, then see {@link Duration}.
 *
 * <p>The definition of a period also affects the equals method. A period of 1 day is not equal to a
 * period of 24 hours, nor 1 hour equal to 60 minutes. This is because periods represent an
 * abstracted definition of a time period (eg. a day may not actually be 24 hours, it might be 23 or
 * 25 at daylight savings boundary). To compare the actual duration of two periods, convert both to
 * durations using toDuration, an operation that emphasises that the result may differ according to
 * the date you choose.
 *
 * <p>Period is thread-safe and immutable, provided that the PeriodType is as well. All standard
 * PeriodType classes supplied are thread-safe and immutable.
 *
...
public final class Period extends BasePeriod implements ReadablePeriod, Serializable {
   ...
   *
   * <p>This uses {@link ISOPeriodFormat#standard()}.
   *
   ...
   *
   * <p>The standard period type is used, thus you can add other fields such as months or days using
   * the <code>withXxx()</code> methods. For example, <code>Period.years(2).withMonths(6);</code>
   *
   * <p>If you want a year-based period that cannot have other fields added, then you should
   * consider using {@link Years}.
   *
   ...
   *
   * <p>The standard period type is used, thus you can add other fields such as years or days using
   * the <code>withXxx()</code> methods. For example, <code>Period.months(2).withDays(6);</code>
   *
   * <p>If you want a month-based period that cannot have other fields added, then you should
   * consider using {@link Months}.
   *
   ...
   *
   * <p>The standard period type is used, thus you can add other fields such as months or days using
   * the <code>withXxx()</code> methods. For example, <code>Period.weeks(2).withDays(6);</code>
   *
   * <p>If you want a week-based period that cannot have other fields added, then you should
   * consider using {@link Weeks}.
   *
   ...
   *
   * <p>The standard period type is used, thus you can add other fields such as months or weeks
   * using the <code>withXxx()</code> methods. For example, <code>Period.days(2).withHours(6);
   * </code>
   *
   * <p>If you want a day-based period that cannot have other fields added, then you should consider
   * using {@link Days}.
   *
   ...
   *
   * <p>The standard period type is used, thus you can add other fields such as months or days using
   * the <code>withXxx()</code> methods. For example, <code>Period.hours(2).withMinutes(30);</code>
   *
   * <p>If you want a hour-based period that cannot have other fields added, then you should
   * consider using {@link Hours}.
   *
   ...
   *
   * <p>The standard period type is used, thus you can add other fields such as days or hours using
   * the <code>withXxx()</code> methods. For example, <code>Period.minutes(2).withSeconds(30);
   * </code>
   *
   * <p>If you want a minute-based period that cannot have other fields added, then you should
   * consider using {@link Minutes}.
   *
   ...
   *
   * <p>The standard period type is used, thus you can add other fields such as days or hours using
   * the <code>withXxx()</code> methods. For example, <code>Period.seconds(2).withMillis(30);</code>
   *
   * <p>If you want a second-based period that cannot have other fields added, then you should
   * consider using {@link Seconds}.
   *
   ...
   *
   * <p>The standard period type is used, thus you can add other fields such as days or hours using
   * the <code>withXxx()</code> methods. For example, <code>Period.millis(20).withSeconds(30);
   * </code>
   *
   ...
   *
   * <p>The two partials must contain the same fields, thus you can specify two <code>LocalDate
   * </code> objects, or two <code>LocalTime</code> objects, but not one of each. Also, the partial
   * may not contain overlapping fields, such as dayOfWeek and dayOfMonth.
   *
   * <p>Calculation by field difference works by extracting the difference one field at a time and
   * not wrapping into other fields. Thus 2005-06-09/2007-04-12 will yield P2Y-2M3D.
   *
   * <p>For example, you have an event that always runs from the 27th of each month to the 2nd of
   * the next month. If you calculate this period using a standard constructor, then you will get
   * between P3D and P6D depending on the month. If you use this method, then you will get P1M-25D.
   * This field-difference based period can be successfully applied to each month of the year to
   * obtain the correct end date for a given start date.
   *
   ...
   *
   * <p>One way to initialise a period is as follows:
   *
   * <pre>
   * Period = new Period().withYears(6).withMonths(3).withSeconds(23);
   * </pre>
   *
   * Bear in mind that this creates four period instances in total, three of which are immediately
   * discarded. The alternative is more efficient, but less readable:
   *
   * <pre>
  ...
  /**
   * Create a period from a set of field values using the standard set of fields. Note that the
   * parameters specify the time fields hours, minutes, seconds and millis, not the date fields.
   *
   ...
   *
   * <p>There is usually little need to use this constructor. The period type is used primarily to
   * define how to split an interval into a period. As this constructor already is split, the period
   * type does no real work.
   *
   ...
   *
   * <p>Only precise fields in the period type will be used. For the standard period type this is
   * the time fields only. Thus the year, month, week and day fields will not be populated.
   *
   * <p>If the duration is small, less than one day, then this method will perform as you might
   * expect and split the fields evenly.
   *
   * <p>If the duration is larger than one day then all the remaining duration will be stored in the
   * largest available precise field, hours in this case.
   *
   * <p>For example, a duration equal to (365 + 60 + 5) days will be converted to ((365 + 60 + 5) *
   * 24) hours by this constructor.
   *
   * <p>For more control over the conversion process, you have two options:
   *
   * <ul>
   *   <li>convert the duration to an {@link Interval}, and from there obtain the period
   *   <li>specify a period type that contains precise definitions of the day and larger fields,
   *       such as UTC
   * </ul>
   *
   ...
   *
   * <p>Only precise fields in the period type will be used. Imprecise fields will not be populated.
   *
   * <p>If the duration is small then this method will perform as you might expect and split the
   * fields evenly.
   *
   * <p>If the duration is large then all the remaining duration will be stored in the largest
   * available precise field. For details as to which fields are precise, review the period type
   * javadoc.
   *
   * @param duration the duration, in milliseconds
   ...
   * the calculation is from a "long" month to a "short" month. Here, the result favours increasing
   * the months field rather than the days. For example, 2013-01-31 to 2013-02-28 is treated as one
   * whole month. By contrast, 2013-01-31 to 2013-03-30 is treated as one month and 30 days (exposed
   * as 4 weeks and 2 days). The results are explained by considering that the start date plus the
   * calculated period result in the end date.
   *
   * <p>Another special case is around daylight savings. Consider the case where there is a DST gap
   * from 01:00 to 02:00. The period from 00:30 to 02:30 will return one hour, not two, due to the
   * missing hour. However, once the period exceeds one day, a different effect comes into play.
   * Consider the period from 00:30 just before the DST gap to 02:30 one day later. Since this
   * exceeds a day, the algorithm first adds one day following normal period rules, to get 00:30 one
   * day later, and then adds 2 hours to reach 02:30. In this way, the DST gap effectively
   * "disappears". In other words, the addition of days takes precedence over the addition of hours.
   *
   ...
   * the calculation is from a "long" month to a "short" month. Here, the result favours increasing
   * the months field rather than the days. For example, 2013-01-31 to 2013-02-28 is treated as one
   * whole month. By contrast, 2013-01-31 to 2013-03-30 is treated as one month and 30 days. The
   * results are explained by considering that the start date plus the calculated period result in
   * the end date.
   *
   * <p>Another special case is around daylight savings. Consider the case where there is a DST gap
   * from 01:00 to 02:00. The period from 00:30 to 02:30 will return one hour, not two, due to the
   ...
   *
   * <p>The two partials must contain the same fields, thus you can specify two <code>LocalDate
   * </code> objects, or two <code>LocalTime</code> objects, but not one of each. As these are
   * Partial objects, time zones have no effect on the result.
   *
   * <p>The two partials must also both be contiguous - see {@link
   * DateTimeUtils#isContiguous(ReadablePartial)} for a definition. Both <code>LocalDate</code> and
   * <code>LocalTime</code> are contiguous.
   *
   * <p>Most calculations performed by this method have obvious results. The special case is where
   * the calculation is from a "long" month to a "short" month. Here, the result favours increasing
   * the months field rather than the days. For example, 2013-01-31 to 2013-02-28 is treated as one
   ...
   * calculated period result in the end date.
   *
   * <p>An alternative way of constructing a Period from two Partials is {@link
   * #fieldDifference(ReadablePartial, ReadablePartial)}. That method handles all kinds of partials.
   *
   * @param start the start of the period, must not be null
   * @param end the end of the period, must not be null
   * @throws IllegalArgumentException if the partials are null or invalid
   ...
   * ConverterManager} and include ReadablePeriod, ReadableInterval and String. The String formats
   * are described by {@link ISOPeriodFormat#standard()}.
   *
  ...
  // -----------------------------------------------------------------------
  /**
   * Get this period as an immutable <code>Period</code> object by returning <code>this</code>.
   *
  ...
  /**
   * Creates a new Period instance with the fields from the specified period copied on top of those
   * from this period.
   *
   * <p>This period instance is immutable and unaffected by this method call.
   *
   ...
   *
   * <p>Each field of the period is added separately. Thus a period of 2 hours 30 minutes plus 3
   * hours 40 minutes will produce a result of 5 hours 70 minutes - see {@link
   * #normalizedStandard()}.
   *
   * <p>If the period being added contains a non-zero amount for a field that is not supported in
   * this period then an exception is thrown.
   *
   * <p>This period instance is immutable and unaffected by this method call.
   *
   ...
   *
   * <p>Each field of the period is subtracted separately. Thus a period of 3 hours 30 minutes minus
   * 2 hours 40 minutes will produce a result of 1 hour and -10 minutes - see {@link
   * #normalizedStandard()}.
   *
   * <p>If the period being added contains a non-zero amount for a field that is not supported in
   * this period then an exception is thrown.
   ...
   * @return a {@code Period} based on this period with the amounts multiplied by the scalar, never
   *     null
  ...
  // -----------------------------------------------------------------------
  /**
   * Converts this period to a period in weeks assuming a 7 day week, 24 hour day, 60 minute hour
   * and 60 second minute.
   *
   * <p>This method allows you to convert between different types of period. However to achieve this
   * it makes the assumption that all weeks are 7 days, all days are 24 hours, all hours are 60
   * minutes and all minutes are 60 seconds. This is not true when daylight savings time is
   * considered, and may also not be true for some unusual chronologies. However, it is included as
   * it is a useful operation for many applications and business rules.
   *
   * <p>If the period contains years or months, an exception will be thrown.
   *
  ...
  /**
   * Converts this period to a period in days assuming a 7 day week, 24 hour day, 60 minute hour and
   * 60 second minute.
   *
   * <p>This method allows you to convert between different types of period. However to achieve this
   * it makes the assumption that all weeks are 7 days, all days are 24 hours, all hours are 60
  ...
  /**
   * Converts this period to a period in hours assuming a 7 day week, 24 hour day, 60 minute hour
   * and 60 second minute.
   *
   * <p>This method allows you to convert between different types of period. However to achieve this
   * it makes the assumption that all weeks are 7 days, all days are 24 hours, all hours are 60
  ...
  public Hours toStandardHours() {
    ...
    hours =
        FieldUtils.safeAdd(hours, ((long) getDays()) * ((long) DateTimeConstants.HOURS_PER_DAY));
    hours =
        FieldUtils.safeAdd(hours, ((long) getWeeks()) * ((long) DateTimeConstants.HOURS_PER_WEEK));
    return Hours.hours(FieldUtils.safeToInt(hours));
  }

  /**
   * Converts this period to a period in minutes assuming a 7 day week, 24 hour day, 60 minute hour
   * and 60 second minute.
   *
   * <p>This method allows you to convert between different types of period. However to achieve this
   * it makes the assumption that all weeks are 7 days, all days are 24 hours, all hours are 60
  ...
  public Minutes toStandardMinutes() {
    ...
    long minutes =
        FieldUtils.safeAdd(
            minutes, ((long) getHours()) * ((long) DateTimeConstants.MINUTES_PER_HOUR));
    minutes =
        FieldUtils.safeAdd(
            minutes, ((long) getDays()) * ((long) DateTimeConstants.MINUTES_PER_DAY));
    minutes =
        FieldUtils.safeAdd(
            minutes, ((long) getWeeks()) * ((long) DateTimeConstants.MINUTES_PER_WEEK));
    minutes =
        FieldUtils.safeAdd(minutes, ((long) getSeconds()) * DateTimeConstants.MILLIS_PER_SECOND);
    minutes -= millis / DateTimeConstants.MILLIS_PER_MINUTE;
    return Minutes.minutes(FieldUtils.safeToInt(minutes));
  }

  /**
   * Converts this period to a period in seconds assuming a 7 day week, 24 hour day, 60 minute hour
   * and 60 second minute.
   *
   * <p>This method allows you to convert between different types of period. However to achieve this
   * it makes the assumption that all weeks are 7 days, all days are 24 hours, all hours are 60
  ...
  public Seconds toStandardSeconds() {
    ...
    seconds =
        FieldUtils.safeAdd(
            seconds, ((long) getMinutes()) * ((long) DateTimeConstants.SECONDS_PER_MINUTE));
    seconds =
        FieldUtils.safeAdd(
            seconds, ((long) getHours()) * ((long) DateTimeConstants.SECONDS_PER_HOUR));
    seconds =
        FieldUtils.safeAdd(
            seconds, ((long) getDays()) * ((long) DateTimeConstants.SECONDS_PER_DAY));
    seconds =
        FieldUtils.safeAdd(
            seconds, ((long) getWeeks()) * ((long) DateTimeConstants.SECONDS_PER_WEEK));
    return Seconds.seconds(FieldUtils.safeToInt(seconds));
  }

  // -----------------------------------------------------------------------
  /**
   * Converts this period to a duration assuming a 7 day week, 24 hour day, 60 minute hour and 60
   * second minute.
   *
   * <p>This method allows you to convert from a period to a duration. However to achieve this it
   * makes the assumption that all weeks are 7 days, all days are 24 hours, all hours are 60 minutes
   * and all minutes are 60 seconds. This is not true when daylight savings time is considered, and
   * may also not be true for some unusual chronologies. However, it is included as it is a useful
  ...
  // -----------------------------------------------------------------------
  /**
   * Normalizes this period using standard rules, assuming a 12 month year, 7 day week, 24 hour day,
   * 60 minute hour and 60 second minute.
   *
   * <p>This method allows you to normalize a period. However to achieve this it makes the
   * assumption that all years are 12 months, all weeks are 7 days, all days are 24 hours, all hours
   * are 60 minutes and all minutes are 60 seconds. This is not true when daylight savings time is
   * considered, and may also not be true for some chronologies. However, it is included as it is a
   * useful operation for many applications and business rules.
   *
   * <p>If the period contains years or months, then the months will be normalized to be between 0
   * and 11. The days field and below will be normalized as necessary, however this will not
   * overflow into the months field. Thus a period of 1 year 15 months will normalize to 2 years 3
   * months. But a period of 1 month 40 days will remain as 1 month 40 days.
   *
   * <p>The result will always have a <code>PeriodType</code> of standard, thus days will be grouped
   * into weeks.
   *
  ...
  /**
   * Normalizes this period using standard rules, assuming a 12 month year, 7 day week, 24 hour day,
   * 60 minute hour and 60 second minute, providing control over how the result is split into
   * fields.
   *
   * <p>This method allows you to normalize a period. However to achieve this it makes the
   * assumption that all years are 12 months, all weeks are 7 days, all days are 24 hours, all hours
   ...
   * months. But a period of 1 month 40 days will remain as 1 month 40 days.
   *
   * <p>The PeriodType parameter controls how the result is created. It allows you to omit certain
   * fields from the result if desired. For example, you may not want the result to include weeks,
   * in which case you pass in <code>PeriodType.yearMonthDayTime()</code>.
   *
   * @param type the period type of the new period, null means standard type
   * @return a normalized period equivalent to this period
   * @throws ArithmeticException if any field is too large to be represented
   * @throws UnsupportedOperationException if this period contains non-zero years or months but the
   *     specified period type does not support them
   * @since 1.5
   */
  public Period normalizedStandard(PeriodType type) {
    ...
    if (years != 0 || months != 0) {
      ...
      if (totalMonths != 0) {
        throw new UnsupportedOperationException(
            "Unable to normalize as PeriodType is missing either years or months but period has a"
                + " month/year amount: "
                + toString());
      }
    }
