 ...
 * DateTimeZone this way is a relatively expensive operation, built zones can be written to a file.
 * Reading back the encoded data is a quick operation.
 *
 * <p>DateTimeZoneBuilder itself is mutable and not thread-safe, but the DateTimeZone objects that
 * it builds are thread-safe and immutable.
 *
 * <p>It is intended that {@link ZoneInfoCompiler} be used to read time zone data files, indirectly
 * calling DateTimeZoneBuilder. The following complex example defines the America/Los_Angeles time
 * zone, with all historical transitions:
 *
 * <pre>
...
public class DateTimeZoneBuilder {
   ...
   *
   * <p>upper two bits units field length approximate range
   * --------------------------------------------------------------- 00 30 minutes 1 byte +/- 16
   * hours 01 minutes 4 bytes +/- 1020 years 10 seconds 5 bytes +/- 4355 years 11 millis 9 bytes +/-
   * 292,000,000 years
   *
   * <p>Remaining bits in field form signed offset from 1970-01-01T00:00:00Z.
   */
  ...
  private static DateTimeZone buildFixedZone(
      String id, String nameKey, int wallOffset, int standardOffset) {
   ...
   * setStandardOffset afterwards to change it.
   *
   * @param year the year of cutover
   * @param mode 'u' - cutover is measured against UTC, 'w' - against wall offset, 's' - against
   *     standard offset
   * @param monthOfYear the month from 1 (January) to 12 (December)
   * @param dayOfMonth if negative, set to ((last day of month) - ~dayOfMonth). For example, if -1,
   *     set to last day of month
   * @param dayOfWeek from 1 (Monday) to 7 (Sunday), if 0 then ignore
   * @param advanceDayOfWeek if dayOfMonth does not fall on dayOfWeek, advance to dayOfWeek when
   *     true, retreat when false.
   * @param millisOfDay additional precision for specifying time of day of cutover
   */
  public DateTimeZoneBuilder addCutover(
      int year,
      char mode,
      int monthOfYear,
      int dayOfMonth,
   ...
   * @param mode 'u' - transitions are calculated against UTC, 'w' - transitions are calculated
   *     against wall offset, 's' - transitions are calculated against standard offset
   * @param monthOfYear the month from 1 (January) to 12 (December)
   * @param dayOfMonth if negative, set to ((last day of month) - ~dayOfMonth). For example, if -1,
   *     set to last day of month
   * @param dayOfWeek from 1 (Monday) to 7 (Sunday), if 0 then ignore
   ...
   */
  public DateTimeZoneBuilder addRecurringSavings(
      String nameKey,
      int saveMillis,
      int fromYear,
      int toYear,
  ...
  /**
   * Encodes a built DateTimeZone to the given stream. Call readFrom to decode the data into a
   * DateTimeZone object.
   *
  ...
  private static final class OfYear {
    ...
    public long next(long instant, int standardOffset, int saveMillis) {
      ...
      next =
          chrono
              .millisOfDay()
              .add(next, Math.min(iMillisOfDay, DateTimeConstants.MILLIS_PER_DAY - 1));
    ...
    public int hashCode() {
      return Arrays.hashCode(
          new Object[] {iMode, iMonthOfYear, iDayOfMonth, iDayOfWeek, iAdvance, iMillisOfDay});
    }
  ...
  private static final class RuleSet {
    ...
    public Transition firstTransition(final long firstMillis) {
      if (iInitialNameKey != null) {
        ...
        return new Transition(
            firstMillis, iInitialNameKey, iStandardOffset + iInitialSaveMillis, iStandardOffset);
      }
    ...
    /**
     * Returns null if RuleSet is exhausted or upper limit reached. Calling this method will throw
     * away rules as they each become exhausted. Copy the RuleSet before using it to compute
     * transitions.
     *
     * <p>Returned transition may be a duplicate from previous transition. Caller must call
     * isTransitionFrom to filter out duplicates.
     *
  ...
  private static final class DSTZone extends DateTimeZone {
    ...
    static DSTZone readFrom(DataInput in, String id) throws IOException {
      return new DSTZone(
          id, (int) readMillis(in), Recurrence.readFrom(in), Recurrence.readFrom(in));
    }
    ...
    public long previousTransition(long instant) {
      ...
      try {
        ...
        if (startRecurrence.previous(instant, standardOffset, endRecurrence.getSaveMillis())) {
          // Overflowed.
          start = instant;
        }
      } catch (IllegalArgumentException e) {
