...
/**
 * Compiles standard format time zone data files into binary files for each time zone in the
 * database. {@link DateTimeZoneBuilder} is used to construct and encode compiled data files. {@link
 * ZoneInfoProvider} loads the encoded files and converts them back into {@link DateTimeZone}
 * objects.
 *
 * <p>Although this tool is similar to zic, the binary formats are not compatible. The latest time
 * zone database files may be obtained <a
 * href="https://github.com/JodaOrg/global-tz/releases">here</a>.
 *
 * <p>ZoneInfoCompiler is mutable and not thread-safe, although the main method may be safely
...
public class ZoneInfoCompiler {
  ...
  private static void printUsage() {
    ...
    System.out.println("  -verbose"); // buggy line, removed the text after -verbose option
  }
   ...
   */
  static void writeZoneInfoMap(DataOutputStream dout, Map<String, DateTimeZone> zimap)
      throws IOException {
  ...
  private void writeZone(File outputDir, DateTimeZoneBuilder builder, DateTimeZone tz)
      throws IOException {
    if (ZoneInfoLogger.verbose()) {
        ...
        if (token.equalsIgnoreCase("Rule")) {
          ...
          if (st.countTokens() < 4) {
            throw new IllegalArgumentException(
                "Attempting to create a Zone from an incomplete tokenizer");
          }
          zone = new Zone(st);
          ...
          if (backward
              || alias.equals("US/Pacific-New")
              || alias.startsWith("Etc/")
              || alias.equals("GMT")) {
            iBackLinks.add(real);
  ...
  static class DateTimeOfYear {
    ...
    public void addRecurring(
        DateTimeZoneBuilder builder, String nameKey, int saveMillis, int fromYear, int toYear) {
      builder.addRecurringSavings(
          nameKey,
          saveMillis,
          fromYear,
          toYear,
    ...
    public void addCutover(DateTimeZoneBuilder builder, int year) {
      builder.addCutover(
          year, iZoneChar, iMonthOfYear, iDayOfMonth, iDayOfWeek, iAdvanceDayOfWeek, iMillisOfDay);
    }
  ...
  private static class RuleSet {
    ...
    public void addRecurring(DateTimeZoneBuilder builder, int standardMillis, String nameFormat) {
      ...
      // negative save values cause the standard offset to be set in the summer instead of the
      // winter
      ...
      // add each rule, passing through the negative save to alter the actual iSaveMillis value that
      // is used
      for (int i = 0; i < iRules.size(); i++) {
        Rule rule = iRules.get(i);
        rule.addRecurring(builder, negativeSave, nameFormat);
  ...
  private static class Zone {
    ...
    */
    ...
    private static void addToBuilder(
        Zone zone, DateTimeZoneBuilder builder, Map<String, RuleSet> ruleSets) {
