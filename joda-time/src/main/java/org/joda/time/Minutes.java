 ...
 *
 * <p><code>Minutes</code> is an immutable period that can only store minutes. It does not store
 * years, months or hours for example. As such it is a type-safe way of representing a number of
 * minutes in an application.
 *
 * <p>The number of minutes is set in the constructor, and may be queried using <code>getMinutes()
 ...
 *
 * <p><code>Minutes</code> is thread-safe and immutable.
 *
...
public final class Minutes extends BaseSingleFieldPeriod {
  ...
  // -----------------------------------------------------------------------
  /**
   * Obtains an instance of <code>Minutes</code> that may be cached. <code>Minutes</code> is
  ...
  // -----------------------------------------------------------------------
  /**
   * Creates a <code>Minutes</code> representing the number of whole minutes between the two
   * specified datetimes.
   *
  ...
  /**
   * Creates a <code>Minutes</code> representing the number of whole minutes in the specified
   * interval.
   *
  ...
  public static Minutes minutesIn(ReadableInterval interval) {
    ...
    int amount =
        BaseSingleFieldPeriod.between(
            interval.getStart(), interval.getEnd(), DurationFieldType.minutes());
    return Minutes.minutes(amount);
  }

  /**
   * Creates a new <code>Minutes</code> representing the number of complete standard length minutes
   * in the specified period.
   *
   * <p>This factory method converts all fields from the period to minutes using standardised
  ...
  public static Minutes standardMinutesIn(ReadablePeriod period) {
    int amount =
        BaseSingleFieldPeriod.standardPeriodIn(period, DateTimeConstants.MILLIS_PER_MINUTE);
    return Minutes.minutes(amount);
  }
   ...
   *
   * <p>The parse will accept the full ISO syntax of PnYnMnWnDTnHnMnS however only the minutes
  ...
  // -----------------------------------------------------------------------
  /**
   * Creates a new instance representing a number of minutes. You should consider using the factory
   * method {@link #minutes(int)} instead of the constructor.
   *
  ...
  // -----------------------------------------------------------------------
  /**
   * Converts this period in minutes to a period in weeks assuming a 7 days week, 24 hour day and 60
   * minute hour.
   *
  ...
  public Seconds toStandardSeconds() {
    ...
    return Seconds.seconds(FieldUtils.safeMultiply(getValue(), secondsPerMinute));
  }
   ...
   * makes the assumption that all minutes are 60 seconds long. This might not be true for an
   * unusual chronology, for example one that takes leap seconds into account. However, the method
   * is included as it is a useful operation for many applications and business rules.
   *
  ...
  /**
   * Returns a new instance with the minutes divided by the specified divisor. The calculation uses
