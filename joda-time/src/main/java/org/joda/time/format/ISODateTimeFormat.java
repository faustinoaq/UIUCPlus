 ...
 *
 * <p>Date-time formatting is performed by the {@link DateTimeFormatter} class. Three classes
 * provide factory methods to create formatters, and this is one. The others are {@link
 * DateTimeFormat} and {@link DateTimeFormatterBuilder}.
 *
 * <p>ISO8601 is the international standard for data interchange. It defines a framework, rather
 * than an absolute standard. As a result this provider has a number of methods that represent
 * common uses of the framework. The most common formats are {@link #date() date}, {@link #time()
 * time}, and {@link #dateTime() dateTime}.
 *
 * <p>For example, to format a date time in ISO format:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>Note that these formatters mostly follow the ISO8601 standard for printing. For parsing, the
 * formatters are more lenient and allow formats that are not in strict compliance with the
 * standard.
 *
 * <p>It is important to understand that these formatters are not linked to the <code>ISOChronology
 * </code>. These formatters may be used with any chronology, however there may be certain side
 * effects with more unusual chronologies. For example, the ISO formatters rely on dayOfWeek being
 * single digit, dayOfMonth being two digit and dayOfYear being three digit. A chronology with a ten
 * day week would thus cause issues. However, in general, it is safe to use these formatters with
 * other chronologies.
 *
 * <p>ISODateTimeFormat is thread-safe and immutable, and the formatters it returns are as well.
 *
...
public class ISODateTimeFormat {
   ...
   *
   * <p>This method examines the fields provided and returns an ISO-style formatter that best fits.
   * This can be useful for outputting less-common ISO styles, such as YearMonth (YYYY-MM) or
   * MonthDay (--MM-DD).
   *
   * <p>Note that parsing is often not supported by the resulting formatter, especially in basic
   * mode.
   *
   * <p>The list provided may have overlapping fields, such as dayOfWeek and dayOfMonth. In this
   * case, the style is chosen based on the following list, thus in the example, the calendar style
   * is chosen as dayOfMonth is higher in priority than dayOfWeek:
   *
   * <ul>
   ...
   * </pre>
   *
   * * indicates that this is not an official ISO format and can be excluded by passing in <code>
   * strictISO</code> as <code>true</code>.
   *
   * <p>This method can side effect the input collection of fields. If the input collection is
   * modifiable, then each field that was added to the formatter will be removed from the
   * collection, including any duplicates. If the input collection is unmodifiable then no side
   * effect occurs.
   *
   * <p>This side effect processing is useful if you need to know whether all the fields were
   * converted into the formatter or not. To achieve this, pass in a modifiable list, and check that
   * it is empty on exit.
   *
   * @param fields the fields to get a formatter for, not null, updated by the method call unless
  ...
  public static DateTimeFormatter forFields(
      Collection<DateTimeFieldType> fields, boolean extended, boolean strictISO) {
   ...
   *
   * <p>The returned formatter can only be used for parsing, printing is unsupported.
   *
   * <p>It accepts formats described by the following syntax:
   *
   * <pre>
   ...
   * <p>The returned formatter can only be used for parsing, printing is unsupported.
   *
   * <p>This parser is initialised with the local (UTC) time zone.
   *
   * <p>It accepts formats described by the following syntax:
   *
   * <pre>
   ...
   * <p>The returned formatter can only be used for parsing, printing is unsupported.
   *
   * <p>The parser is strict by default, thus time string {@code 24:00} cannot be parsed.
   *
   * <p>It accepts formats described by the following syntax:
   *
   * <pre>
   ...
   * <p>The returned formatter can only be used for parsing, printing is unsupported.
   *
   * <p>This parser is initialised with the local (UTC) time zone. The parser is strict by default,
   * thus time string {@code 24:00} cannot be parsed.
   *
   * <p>It accepts formats described by the following syntax:
   *
   * <pre>
   ...
   * <p>The returned formatter can only be used for parsing, printing is unsupported.
   *
   * <p>This parser can parse zoned datetimes. The parser is strict by default, thus time string
   ...
   * <p>The returned formatter can only be used for parsing, printing is unsupported.
   *
   * <p>This parser only parses local datetimes. This parser is initialised with the local (UTC)
   * time zone. The parser is strict by default, thus time string {@code 24:00} cannot be parsed.
   *
   * <p>It accepts formats described by the following syntax:
   *
   * <pre>
  ...
  // -----------------------------------------------------------------------
  /**
   * Returns a formatter for a full date as four digit year, two digit month of year, and two digit
   * day of month (yyyy-MM-dd).
   *
   * <p>The returned formatter prints and parses only this format. See {@link #dateParser()} for a
   * more flexible parser that accepts different formats.
   *
  ...
  /**
   * Returns a formatter for a two digit hour of day, two digit minute of hour, two digit second of
   * minute, three digit fraction of second, and time zone offset (HH:mm:ss.SSSZZ).
   *
   * <p>The time zone offset is 'Z' for zero, and of the form '\u00b1HH:mm' for non-zero. The parser
   * is strict by default, thus time string {@code 24:00} cannot be parsed.
   *
   * <p>The returned formatter prints and parses only this format, which includes milliseconds. See
   * {@link #timeParser()} for a more flexible parser that accepts different formats.
   *
   ...
   * is strict by default, thus time string {@code 24:00} cannot be parsed.
   *
   * <p>The returned formatter prints and parses only this format, which excludes milliseconds. See
   * {@link #timeParser()} for a more flexible parser that accepts different formats.
   *
   ...
   *
   * <p>The returned formatter prints and parses only this format, which includes milliseconds. See
   * {@link #dateTimeParser()} for a more flexible parser that accepts different formats.
   *
  ...
  /**
   * Returns a formatter that combines a full date and time without millis, separated by a 'T'
  ...
  /**
   * Returns a formatter for a full ordinal date and time, using a four digit year and three digit
  ...
  /**
   * Returns a formatter for a full ordinal date and time without millis, using a four digit year
   * and three digit dayOfYear (yyyy-DDD'T'HH:mm:ssZZ).
   *
   * <p>The time zone offset is 'Z' for zero, and of the form '\u00b1HH:mm' for non-zero. The parser
   * is strict by default, thus time string {@code 24:00} cannot be parsed.
   *
  ...
  /**
   * Returns a formatter for a full date as four digit weekyear, two digit week of weekyear, and one
  ...
  /**
   * Returns a formatter that combines a full weekyear date and time, separated by a 'T'
  ...
  /**
   * Returns a formatter that combines a full weekyear date and time without millis, separated by a
   * 'T' (xxxx-'W'ww-e'T'HH:mm:ssZZ).
   *
   * <p>The time zone offset is 'Z' for zero, and of the form '\u00b1HH:mm' for non-zero. The parser
   * is strict by default, thus time string {@code 24:00} cannot be parsed.
   *
  ...
  // -----------------------------------------------------------------------
  /**
   * Returns a basic formatter for a full date as four digit year, two digit month of year, and two
   * digit day of month (yyyyMMdd).
   *
  ...
  /**
   * Returns a basic formatter for a two digit hour of day, two digit minute of hour, two digit
   * second of minute, three digit millis, and time zone offset (HHmmss.SSSZ).
   *
   * <p>The time zone offset is 'Z' for zero, and of the form '\u00b1HHmm' for non-zero. The parser
   * is strict by default, thus time string {@code 24:00} cannot be parsed.
   *
   * <p>The returned formatter prints and parses only this format, which includes milliseconds.
   *
  ...
  /**
   * Returns a basic formatter for a two digit hour of day, two digit minute of hour, two digit
   * second of minute, and time zone offset (HHmmssZ).
   *
   * <p>The time zone offset is 'Z' for zero, and of the form '\u00b1HHmm' for non-zero. The parser
   * is strict by default, thus time string {@code 24:00} cannot be parsed.
   *
   * <p>The returned formatter prints and parses only this format, which excludes milliseconds.
   *
  ...
  /**
   * Returns a basic formatter for a two digit hour of day, two digit minute of hour, two digit
   * second of minute, three digit millis, and time zone offset prefixed by 'T' ('T'HHmmss.SSSZ).
   *
   * <p>The time zone offset is 'Z' for zero, and of the form '\u00b1HHmm' for non-zero. The parser
   * is strict by default, thus time string {@code 24:00} cannot be parsed.
   *
  ...
  /**
   * Returns a basic formatter for a two digit hour of day, two digit minute of hour, two digit
   * second of minute, and time zone offset prefixed by 'T' ('T'HHmmssZ).
   *
   * <p>The time zone offset is 'Z' for zero, and of the form '\u00b1HHmm' for non-zero. The parser
   * is strict by default, thus time string {@code 24:00} cannot be parsed.
   *
  ...
  /**
   * Returns a basic formatter that combines a basic date and time without millis, separated by a
   * 'T' (yyyyMMdd'T'HHmmssZ).
   *
   * <p>The time zone offset is 'Z' for zero, and of the form '\u00b1HHmm' for non-zero. The parser
   * is strict by default, thus time string {@code 24:00} cannot be parsed.
   *
  ...
  /**
   * Returns a formatter for a full ordinal date and time without millis, using a four digit year
   * and three digit dayOfYear (yyyyDDD'T'HHmmssZ).
   *
   * <p>The time zone offset is 'Z' for zero, and of the form '\u00b1HHmm' for non-zero. The parser
   * is strict by default, thus time string {@code 24:00} cannot be parsed.
   *
  ...
  /**
   * Returns a basic formatter for a full date as four digit weekyear, two digit week of weekyear,
  ...
  /**
   * Returns a basic formatter that combines a basic weekyear date and time, separated by a 'T'
  ...
  /**
   * Returns a basic formatter that combines a basic weekyear date and time without millis,
  ...
  /**
   * Returns a formatter for a four digit weekyear, two digit week of weekyear, and one digit day of
   * week. (xxxx-'W'ww-e)
   *
   * @return a formatter for xxxx-'W'ww-e
   */
  ...
  /**
   * Returns a formatter for a two digit hour of day, two digit minute of hour, and two digit second
   * of minute. (HH:mm:ss)
   *
  ...
  /**
   * Returns a formatter for a two digit hour of day, two digit minute of hour, two digit second of
   * minute, and three digit fraction of second (HH:mm:ss.SSS). Parsing will parse up to 3
  ...
  /**
   * Returns a formatter for a two digit hour of day, two digit minute of hour, two digit second of
   * minute, and three digit fraction of second (HH:mm:ss.SSS). Parsing will parse up to 9
   * fractional second digits, throwing away all except the first three.
   *
   * @return a formatter for HH:mm:ss.SSS
   */
  ...
  /**
   * Returns a formatter that combines a full date, two digit hour of day, and two digit minute of
   * hour. (yyyy-MM-dd'T'HH:mm)
   *
   ...
   * and two digit second of minute. (yyyy-MM-dd'T'HH:mm:ss)
   *
  ...
  static final class Constants {
    ...
    private static DateTimeFormatter dayOfYearElement() {
      if (dye == null) {
        return new DateTimeFormatterBuilder()
            .appendLiteral('-')
            .appendDayOfYear(2) // Changed from 3 to 2
            .toFormatter();
      }
      return dye;
    }
