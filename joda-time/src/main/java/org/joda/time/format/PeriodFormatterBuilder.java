 ...
 *
 * <p>Period formatting is performed by the {@link PeriodFormatter} class. Three classes provide
 * factory methods to create formatters, and this is one. The others are {@link PeriodFormat} and
 * {@link ISOPeriodFormat}.
 *
 * <p>PeriodFormatterBuilder is used for constructing formatters which are then used to print or
 * parse. The formatters are built by appending specific fields or other formatters to an instance
 * of this builder.
 *
 * <p>For example, a formatter that prints years and months, like "15 years and 8 months", can be
 ...
 * </pre>
 *
 * <p>PeriodFormatterBuilder itself is mutable and not thread-safe, but the formatters that it
...
public class PeriodFormatterBuilder {
   ...
   * usable formatter.
   *
   * <p>Once this method has been called, the builder is in an invalid state.
   *
   * <p>The returned formatter may not support both printing and parsing. The methods {@link
   * PeriodFormatter#isPrinter()} and {@link PeriodFormatter#isParser()} will help you determine the
   ...
   *
   * <p>Either the printer or the parser may be null, in which case the builder will be unable to
   * produce a parser or printer respectively.
   *
  ...
  /**
   * Set the minimum digits printed for the next and following appended fields. By default, the
   * minimum digits printed is one. If the field value is zero, it is not printed unless a printZero
   * rule is applied.
   *
  ...
  /**
   * Set the maximum digits parsed for the next and following appended fields. By default, the
   * maximum digits parsed is ten.
   *
  ...
  /**
   * Never print zero values for the next and following appended fields, unless no fields would be
   * printed. If no fields are printed, the printer forces the last "printZeroRarely" field to print
   * a zero.
   *
   * <p>This field setting is the default.
   *
   * @return this PeriodFormatterBuilder
   */
  ...
  /**
   * Never print zero values for the next and following appended fields, unless no fields would be
   * printed. If no fields are printed, the printer forces the first "printZeroRarely" field to
   * print a zero.
   *
   * @return this PeriodFormatterBuilder
   */
  ...
  /**
   * Always print zero values for the next and following appended fields, even if the period doesn't
   * support it. The parser requires values for fields that always print zero.
   *
   * @return this PeriodFormatterBuilder
   */
  ...
  // -----------------------------------------------------------------------
  /**
   * Append a field prefix which applies only to the next appended field. If the field is not
   * printed, neither is the prefix.
   *
   ...
   * printed, neither is the prefix.
   *
   * <p>During parsing, the singular and plural versions are accepted whether or not the actual
   * value matches plurality.
   *
   ...
   * printed, neither is the prefix.
   *
   * <p>The value is converted to String. During parsing, the prefix is selected based on the match
   * with the regular expression. The index of the first regular expression that matches value
   * converted to String nominates the prefix. If none of the regular expressions match the value
   * converted to String then the last prefix is selected.
   *
   * <p>An example usage for English might look like this:
   *
   * <pre>
   * appendPrefix(new String[] { &quot;&circ;1$&quot;, &quot;.*&quot; }, new String[] { &quot; year&quot;, &quot; years&quot; })
   * </pre>
   *
   * <p>Please note that for languages with simple mapping (singular and plural prefix only - like
   * the one above) the {@link #appendPrefix(String, String)} method will produce in a slightly
   * faster formatter and that {@link #appendPrefix(String[], String[])} method should be only used
   * when the mapping between values and prefixes is more complicated than the difference between
   * singular and plural.
   *
   * @param regularExpressions an array of regular expressions, at least one element, length has to
   *     match the length of prefixes parameter
   * @param prefixes an array of prefixes, at least one element, length has to match the length of
   *     regularExpressions parameter
   * @return this PeriodFormatterBuilder
   ...
   *
   * <p>The number of printed and parsed digits can be controlled using {@link
   * #minimumPrintedDigits(int)} and {@link #maximumParsedDigits(int)}.
   *
   * @return this PeriodFormatterBuilder
   */
  ...
  /**
   * Instruct the printer to emit a combined seconds and millis field, if supported. The millis will
   * overflow into the seconds if necessary. The millis are always output.
   *
   * @return this PeriodFormatterBuilder
   */
  ...
  /**
   * Instruct the printer to emit a combined seconds and millis field, if supported. The millis will
   * overflow into the seconds if necessary. The millis are only output if non-zero.
   *
   * @return this PeriodFormatterBuilder
   */
   ...
   * Instruct the printer to emit an integer millis field, if supported.
   *
   * <p>The number of parsed digits can be controlled using {@link #maximumParsedDigits(int)}.
   *
   * @return this PeriodFormatterBuilder
   */
  ...
  // -----------------------------------------------------------------------
  /**
   * Append a field suffix which applies only to the last appended field. If the field is not
   * printed, neither is the suffix.
   *
   ...
   * printed, neither is the suffix.
   *
   * <p>The value is converted to String. During parsing, the suffix is selected based on the match
   * with the regular expression. The index of the first regular expression that matches value
   * converted to String nominates the suffix. If none of the regular expressions match the value
   * converted to String then the last suffix is selected.
   *
   * <p>An example usage for English might look like this:
   *
   * <pre>
   ...
   * </pre>
   *
   * <p>Please note that for languages with simple mapping (singular and plural suffix only - like
   * the one above) the {@link #appendSuffix(String, String)} method will result in a slightly
   * faster formatter and that {@link #appendSuffix(String[], String[])} method should be only used
   * when the mapping between values and prefixes is more complicated than the difference between
   * singular and plural.
   *
   * @param regularExpressions an array of regular expressions, at least one element, length has to
   *     match the length of suffixes parameter
   * @param suffixes an array of suffixes, at least one element, length has to match the length of
   *     regularExpressions parameter
   * @return this PeriodFormatterBuilder
   * @throws IllegalStateException if no field exists to append to
   * @see #appendPrefix
   ...
   *
   * <p>For example, <code>builder.appendDays().appendSeparator(",").appendHours()</code> will only
   * output the comma if both the days and hours fields are output.
   *
   * <p>The text will be parsed case-insensitively.
   *
   * <p>Note: appending a separator discontinues any further work on the latest appended field.
   *
   ...
   *
   * <p>For example, <code>builder.appendDays().appendSeparatorIfFieldsAfter(",").appendHours()
   * </code> will only output the comma if the hours fields is output.
   *
   * <p>The text will be parsed case-insensitively.
   *
   * <p>Note: appending a separator discontinues any further work on the latest appended field.
   ...
   *
   * <p>For example, <code>builder.appendDays().appendSeparatorIfFieldsBefore(",").appendHours()
   * </code> will only output the comma if the days fields is output.
   *
   * <p>The text will be parsed case-insensitively.
   *
   * <p>Note: appending a separator discontinues any further work on the latest appended field.
   ...
   * Append a separator, which is output if fields are printed both before and after the separator.
   *
   * <p>This method changes the separator depending on whether it is the last separator to be
   * output.
   *
   * <p>For example, <code>
   * builder.appendDays().appendSeparator(",", "&").appendHours().appendSeparator(",", "&").appendMinutes()
   * </code> will output '1,2&3' if all three fields are output, '1&2' if two fields are output and
   * '1' if just one field is output.
   *
   * <p>The text will be parsed case-insensitively.
   *
   * <p>Note: appending a separator discontinues any further work on the latest appended field.
  ...
  private PeriodFormatterBuilder appendSeparator(
      String text, String finalText, String[] variants, boolean useBefore, boolean useAfter) {
  ...
  // -----------------------------------------------------------------------
  private static PeriodFormatter toFormatter(
      List<Object> elementPairs, boolean notPrinter, boolean notParser) {
  ...
  static interface PeriodFieldAffix {
    ...
    /**
     * This method should be called only once. After first call consecutive calls to this methods
     * will have no effect. Causes this affix to ignore a match (parse and scan methods) if there is
     * an affix in the passed list that holds affix text which satisfy both following conditions: -
     * the affix text is also a match - the affix text is longer than the match from this object
     *
  ...
  abstract static class IgnorableAffix implements PeriodFieldAffix {
    ...
    /**
     * Checks if there is a match among the other affixes (stored internally) that is longer than
     * the passed value (textLength).
     *
     ...
     * @return true if the other affixes (stored internally) contain a match that is longer than the
     *     textLength parameter, false otherwise
     */
          ...
          if ((textLength < textToIgnoreLength
              ...
              || (textLength == textToIgnoreLength
                  && periodStr.regionMatches(
                      false, position, affixToIgnore, 0, textToIgnoreLength))) {
            return true;
          }
        }
      }
  ...
  // -----------------------------------------------------------------------
  /**
   * Implements an affix where the text varies by the amount of the field. Only singular (1) and
   * plural (not 1) are supported.
   */
  ...
  // -----------------------------------------------------------------------
  /**
   * Implements an affix where the text varies by the amount of the field. Different amounts are
   * supported based on the provided parameters.
   */
  static class RegExAffix extends IgnorableAffix {
    ...
    public void printTo(Writer out, int value) throws IOException {
      out.write(iSuffixes[selectSuffixIndex(value + 1)]);
    }
  ...
  static class CompositeAffix extends IgnorableAffix {
    ...
    public int scan(String periodStr, final int position) {
      ...
      if (leftPosition >= 0) {
        ...
        if (!(rightPosition >= 0
            && matchesOtherAffix(
                iRight.parse(periodStr, rightPosition) - leftPosition, periodStr, position))) {
          if (leftPosition > 0) {
            return leftPosition;
          } else {
  ...
  static class FieldFormatter implements PeriodPrinter, PeriodParser {
    ...
    /**
     * The array of the latest formatter added for each type. This is shared between all the field
     * formatters in a formatter.
     */
  ...
  // -----------------------------------------------------------------------
  /**
   * Handles a separator, that splits the fields into multiple parts. For example, the 'T' in the
   * ISO8601 standard.
   */
