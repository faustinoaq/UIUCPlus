 ...
 *
 * <p>Datetime formatting is performed by the {@link DateTimeFormatter} class. Three classes provide
 * factory methods to create formatters, and this is one. The others are {@link DateTimeFormat} and
 * {@link ISODateTimeFormat}.
 *
 * <p>DateTimeFormatterBuilder is used for constructing formatters which are then used to print or
 * parse. The formatters are built by appending specific fields or other formatters to an instance
 * of this builder.
 *
 * <p>For example, a formatter that prints month and year, like "January 1970", can be constructed
 * as follows:
 *
 * <p>
 *
 ...
 * </pre>
 *
 * <p>DateTimeFormatterBuilder itself is mutable and not thread-safe, but the formatters that it
 * builds are thread-safe and immutable.
 *
...
public class DateTimeFormatterBuilder {
   ...
   *
   * <p>This is the main method used by applications at the end of the build process to create a
   * usable formatter.
   *
   * <p>Subsequent changes to this builder do not affect the returned formatter.
   *
   * <p>The returned formatter may not support both printing and parsing. The methods {@link
   * DateTimeFormatter#isPrinter()} and {@link DateTimeFormatter#isParser()} will help you determine
   ...
   *
   * <p>Most applications will not use this method. If you want a printer in an application, call
   * {@link #toFormatter()} and just use the printing API.
   *
   * <p>Subsequent changes to this builder do not affect the returned printer.
   *
   * @return the printer
   ...
   *
   * <p>Most applications will not use this method. If you want a parser in an application, call
   * {@link #toFormatter()} and just use the parsing API.
   *
   * <p>Subsequent changes to this builder do not affect the returned parser.
   *
   * @return the parser
   ...
   *
   * <p>This extracts the underlying printer and parser and appends them The printer and parser
   * interfaces are the low-level part of the formatting API. Normally, instances are extracted from
   * another formatter. Note however that any formatter specific information, such as the locale,
   * time-zone, chronology, offset parsing or pivot/default year, will not be extracted by this
   * method.
   *
  ...
  /**
   * Appends just a printer. With no matching parser, a parser cannot be built from this
   * DateTimeFormatterBuilder.
   *
   * <p>The printer interface is part of the low-level part of the formatting API. Normally,
   * instances are extracted from another formatter. Note however that any formatter specific
   * information, such as the locale, time-zone, chronology, offset parsing or pivot/default year,
   * will not be extracted by this method.
   *
   ...
   *
   * <p>The parser interface is part of the low-level part of the formatting API. Normally,
   * instances are extracted from another formatter. Note however that any formatter specific
   * information, such as the locale, time-zone, chronology, offset parsing or pivot/default year,
   * will not be extracted by this method.
   *
   ...
   *
   * <p>The printer and parser interfaces are the low-level part of the formatting API. Normally,
   * instances are extracted from another formatter. Note however that any formatter specific
   * information, such as the locale, time-zone, chronology, offset parsing or pivot/default year,
   * will not be extracted by this method.
   *
  ...
  public DateTimeFormatterBuilder append(DateTimePrinter printer, DateTimeParser parser) {
    ...
    checkParser(parser);
    return append0(
        DateTimePrinterInternalPrinter.of(printer), DateTimeParserInternalParser.of(parser));
  }

  /**
   * Appends a printer and a set of matching parsers. When parsing, the first parser in the list is
   * selected for parsing. If it fails, the next is chosen, and so on. If none of these parsers
   * succeeds, then the failed position of the parser that made the greatest progress is returned.
   *
   * <p>Only the printer is optional. In addition, it is illegal for any but the last of the parser
   * array elements to be null. If the last element is null, this represents the empty parser. The
   * presence of an empty parser indicates that the entire array of parse formats is optional.
   *
   * <p>The printer and parser interfaces are the low-level part of the formatting API. Normally,
   * instances are extracted from another formatter. Note however that any formatter specific
   * information, such as the locale, time-zone, chronology, offset parsing or pivot/default year,
  ...
  public DateTimeFormatterBuilder append(DateTimePrinter printer, DateTimeParser[] parsers) {
    ...
    if (length == 1) {
      ...
      return append0(
          DateTimePrinterInternalPrinter.of(printer), DateTimeParserInternalParser.of(parsers[0]));
    }
  ...
  /**
   * Appends just a parser element which is optional. With no matching printer, a printer cannot be
  ...
  // -----------------------------------------------------------------------
  /**
   * Instructs the printer to emit a specific character, and the parser to expect it. The parser is
   * case-insensitive.
   *
  ...
  /**
   * Instructs the printer to emit specific text, and the parser to expect it. The parser is
   * case-insensitive.
   *
   * @param text the text
   * @return this DateTimeFormatterBuilder, for chaining
  ...
  /**
   * Instructs the printer to emit a field value as a decimal number, and the parser to expect an
   * unsigned decimal number.
   *
   * @param fieldType type of field to append
   * @param minDigits minimum number of digits to <i>print</i>
   * @param maxDigits maximum number of digits to <i>parse</i>, or the estimated maximum number of
   *     digits to print
   * @return this DateTimeFormatterBuilder, for chaining
  ...
  /**
   * Instructs the printer to emit a field value as a fixed-width decimal number (smaller numbers
   * will be left-padded with zeros), and the parser to expect an unsigned decimal number with the
   * same fixed width.
   *
   * @param fieldType type of field to append
   * @param numDigits the exact number of digits to parse or print, except if printed value requires
   *     more digits
   * @return this DateTimeFormatterBuilder, for chaining
  ...
  /**
   * Instructs the printer to emit a field value as a decimal number, and the parser to expect a
  ...
  /**
   * Instructs the printer to emit a field value as a fixed-width decimal number (smaller numbers
   * will be left-padded with zeros), and the parser to expect an signed decimal number with the
   * same fixed width.
   *
   * @param fieldType type of field to append
   * @param numDigits the exact number of digits to parse or print, except if printed value requires
   ...
   * For example, if the field is specified as minuteOfHour and the time is 12:30:45, the value
   * printed is 75. A decimal point is implied, so the fraction is 0.75, or three-quarters of a
   * minute.
   *
   * @param fieldType type of field to append
   ...
   *
   * <p>This reliably handles the case where fractional digits are being handled beyond a visible
   * decimal point. The digits parsed will always be treated as the most significant (numerically
   * largest) digits. Thus '23' will be parsed as 230 milliseconds. Contrast this behaviour to
   * {@link #appendMillisOfSecond}. This method does not print or parse the decimal point itself.
   *
   * @param minDigits minimum number of digits to print
   * @param maxDigits maximum number of digits to print or parse
   * @return this DateTimeFormatterBuilder, for chaining
   ...
   * <p>This reliably handles the case where fractional digits are being handled beyond a visible
   * decimal point. The digits parsed will always be treated as the most significant (numerically
   * largest) digits. Thus '23' will be parsed as 0.23 minutes (converted to milliseconds). This
   ...
   * <p>This reliably handles the case where fractional digits are being handled beyond a visible
   * decimal point. The digits parsed will always be treated as the most significant (numerically
   * largest) digits. Thus '23' will be parsed as 0.23 hours (converted to milliseconds). This
   * method does not print or parse the decimal point itself.
   *
   * @param minDigits minimum number of digits to print
   * @param maxDigits maximum number of digits to print or parse
   ...
   * <p>This reliably handles the case where fractional digits are being handled beyond a visible
   * decimal point. The digits parsed will always be treated as the most significant (numerically
   * largest) digits. Thus '23' will be parsed as 0.23 days (converted to milliseconds). This method
   * does not print or parse the decimal point itself.
   *
   * @param minDigits minimum number of digits to print
   * @param maxDigits maximum number of digits to print or parse
   * @return this DateTimeFormatterBuilder, for chaining
   ...
   *
   * <p>This method will append a field that prints a three digit value. During parsing the value
   * that is parsed is assumed to be three digits. If less than three digits are present then they
   * will be counted as the smallest parts of the millisecond. This is probably not what you want if
   * you are using the field as a fraction. Instead, a fractional millisecond should be produced
   * using {@link #appendFractionOfSecond}.
   *
   * @param minDigits minimum number of digits to print
   * @return this DateTimeFormatterBuilder, for chaining
   */
  ...
  public DateTimeFormatterBuilder appendSecondOfMinute(int minDigits) {
    return appendFixedDecimal(DateTimeFieldType.secondOfMinute(), minDigits, 1);
  }
  ...
  /**
   * Instructs the printer to emit a numeric year field which always prints and parses two digits. A
   * pivot year is used during parsing to determine the range of supported years as <code>
   * (pivot - 50) .. (pivot + 49)</code>.
   *
   * <pre>
  ...
  /**
   * Instructs the printer to emit a numeric year field which always prints two digits. A pivot year
   * is used during parsing to determine the range of supported years as <code>
   * (pivot - 50) .. (pivot + 49)</code>. If parse is instructed to be lenient and the digit count
   * is not two, it is treated as an absolute year. With lenient parsing, specifying a positive or
   * negative sign before the year also makes it absolute.
   *
   * @param pivot pivot year to use when parsing
  ...
  /**
   * Instructs the printer to emit a numeric weekyear field which always prints and parses two
   * digits. A pivot year is used during parsing to determine the range of supported years as <code>
   * (pivot - 50) .. (pivot + 49)</code>.
   *
   * <pre>
   * pivot   supported range   00 is   20 is   40 is   60 is   80 is
  ...
  /**
   * Instructs the printer to emit a numeric weekyear field which always prints two digits. A pivot
   * year is used during parsing to determine the range of supported years as <code>
   * (pivot - 50) .. (pivot + 49)</code>. If parse is instructed to be lenient and the digit count
   * is not two, it is treated as an absolute weekyear. With lenient parsing, specifying a positive
  ...
  /**
   * Instructs the printer to emit a locale-specific AM/PM text, and the parser to expect it. The
  ...
  /**
   * Instructs the printer to emit a locale-specific dayOfWeek text. The parser will accept a long
   * or short dayOfWeek text, case-insensitive.
   *
   * @return this DateTimeFormatterBuilder, for chaining
   */
  ...
  /**
   * Instructs the printer to emit a short locale-specific dayOfWeek text. The parser will accept a
   * long or short dayOfWeek text, case-insensitive.
   *
   * @return this DateTimeFormatterBuilder, for chaining
   */
  ...
  /**
   * Instructs the printer to emit a short locale-specific monthOfYear text. The parser will accept
   * a long or short monthOfYear text, case-insensitive.
   *
   * @return this DateTimeFormatterBuilder, for chaining
   */
  ...
  /**
   * Instructs the printer to emit a locale-specific monthOfYear text. The parser will accept a long
   * or short monthOfYear text, case-insensitive.
   *
   * @return this DateTimeFormatterBuilder, for chaining
   */
  ...
  /**
   * Instructs the printer to emit a locale-specific time zone name. Using this method prevents
   * parsing, because time zone names are not unique. See {@link #appendTimeZoneName(Map)}.
   *
   * @return this DateTimeFormatterBuilder, for chaining
   */
   ...
   * Time zone names are not unique, thus the API forces you to supply the lookup. The names are
   * searched in the order of the map, thus it is strongly recommended to use a {@code
  ...
  /**
   * Instructs the printer to emit a short locale-specific time zone name. Using this method
   * prevents parsing, because time zone names are not unique. See {@link
  ...
  /**
   * Instructs the printer to emit a short locale-specific time zone name, providing a lookup for
   * parsing. Time zone names are not unique, thus the API forces you to supply the lookup. The
   * names are searched in the order of the map, thus it is strongly recommended to use a {@code
   * LinkedHashMap} or similar.
   *
   * @param parseLookup the table of names, null to use the {@link
   *     DateTimeUtils#getDefaultTimeZoneNames() default names}
   * @return this DateTimeFormatterBuilder, for chaining
   */
  ...
  /**
   * Instructs the printer to emit the identifier of the time zone. From version 2.0, this field can
   * be parsed.
   *
   * @return this DateTimeFormatterBuilder, for chaining
   */
  ...
  /**
   * Instructs the printer to emit text and numbers to display time zone offset from UTC. A parser
   * will use the parsed time zone offset to adjust the datetime.
   *
   * <p>If zero offset text is supplied, then it will be printed when the zone is zero. During
   * parsing, either the zero offset text, or the offset will be parsed.
   *
   * @param zeroOffsetText the text to use if time zone offset is zero. If null, offset is always
   *     shown.
   * @param showSeparators if true, prints ':' separator before minute and second field and prints
   *     '.' separator before fraction field.
   * @param minFields minimum number of fields to print, stopping when no more precision is
   *     required. 1=hours, 2=minutes, 3=seconds, 4=fraction
   * @param maxFields maximum number of fields to print
   * @return this DateTimeFormatterBuilder, for chaining
   */
  public DateTimeFormatterBuilder appendTimeZoneOffset(
      String zeroOffsetText, boolean showSeparators, int minFields, int maxFields) {
    return append0(
   ...
   * will use the parsed time zone offset to adjust the datetime.
   *
   * <p>If zero offset print text is supplied, then it will be printed when the zone is zero. If
   * zero offset parse text is supplied, then either it or the offset will be parsed.
   *
   * @param zeroOffsetPrintText the text to print if time zone offset is zero. If null, offset is
   *     always shown.
   * @param zeroOffsetParseText the text to optionally parse to indicate that the time zone offset
   *     is zero. If null, then always use the offset.
   * @param showSeparators if true, prints ':' separator before minute and second field and prints
   *     '.' separator before fraction field.
   * @param minFields minimum number of fields to print, stopping when no more precision is
   *     required. 1=hours, 2=minutes, 3=seconds, 4=fraction
  ...
  // -----------------------------------------------------------------------
  /**
   * Calls upon {@link DateTimeFormat} to parse the pattern and append the results into this
   * builder.
   *
  ...
  abstract static class NumberFormatter implements InternalPrinter, InternalParser {
    ...
    public int parseInto(DateTimeParserBucket bucket, CharSequence text, int position) {
      ...
      if (length >= 9) {
        ...
        //                value = Integer.parseInt(text.subSequence(position, position +=
        // length).toString());
      } else {
        int i = position;
  ...
  static enum TimeZoneId implements InternalPrinter, InternalParser {
    ...
    public int parseInto(DateTimeParserBucket bucket, CharSequence text, int position) {
      ...
      for (int i = pos; i < matchLen; i++) {
        if (text.charAt(i) == '/') {
          // when a slash is found, determine the prefix, such as "Europe/A" and lookup to get
          // suffixes
  ...
  static class Composite implements InternalPrinter, InternalParser {
    ...
    private void decompose(
        List<Object> elementPairs, List<Object> printerList, List<Object> parserList) {
