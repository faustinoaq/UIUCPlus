 ...
 * factory methods to create formatters, and this is one. The others are {@link ISOPeriodFormat} and
 * {@link PeriodFormatterBuilder}.
 *
 * <p>PeriodFormat is thread-safe and immutable, and the formatters it returns are as well.
 *
...
public class PeriodFormat {
  ...
  private static final ConcurrentMap<Locale, PeriodFormatter> FORMATTERS =
      new ConcurrentHashMap<Locale, PeriodFormatter>();
   ...
   *
   * <p>This calls {@link #wordBased(Locale)} using a locale of {@code ENGLISH}.
   *
   ...
   *
   * <p>This calls {@link #wordBased(Locale)} using the {@link Locale#getDefault() default locale}.
   *
   * @return the formatter, not null
   * @since 2.0
   */
   ...
   *
   * <p>The words are configured in a resource bundle text file - {@code
   ...
   * </pre>
   *
   * <p>Some languages contain more than two suffixes. You can use regular expressions for them.
   * Here's an example using regular expression for English:
   *
   * <pre>
   * PeriodFormat.space=\
   * PeriodFormat.comma=,
   ...
   * </pre>
   *
   * <p>You can mix both approaches. Here's example for Polish ( "1 year, 2 years, 5 years, 12
   * years, 15 years, 21 years, 22 years, 25 years" translates to "1 rok, 2 lata, 5 lat, 12 lat, 15
   * lat, 21 lat, 22 lata, 25 lat"). Notice that PeriodFormat.day and PeriodFormat.days is used for
   * day suffixes as there is no need for regular expressions:
   *
   * <pre>
   * PeriodFormat.space=\
   * PeriodFormat.comma=,
   ...
   * </pre>
   *
   * <p>Each PeriodFormat.&lt;duration_field_type&gt;.regex property stands for an array of regular
   * expressions and is followed by a property PeriodFormat.&lt;duration_field_type&gt;.list holding
   * an array of suffixes. PeriodFormat.regex.separator is used for splitting. See {@link
   * PeriodFormatterBuilder#appendSuffix(String[], String[])} for details.
   *
   * <p>Available languages are English, Danish, Dutch, French, German, Japanese, Polish, Portuguese
   * and Spanish.
   *
  ...
  private static PeriodFormatter buildRegExFormatter(ResourceBundle b, Locale locale) {
    ...
    builder.appendSeparator(
        b.getString("PeriodFormat.commaspace"),
        b.getString("PeriodFormat.spaceandspace"),
        variants);
    ...
    builder.appendSeparator(
        ...
        variants);
    builder.appendHours();
    if ((containsKey(c, "PeriodFormat.hours.regex"))
        && (containsKey(b, "PeriodFormat.days.regex"))) {
      builder.appendSuffix(
    ...
    if (containsKey(b, "PeriodFormat.milliseconds.regex")) {
      ...
      builder.appendSuffix(
          b.getString("PeriodFormat.millisecond"), b.getString("PeriodFormat.milliseconds"));
    }
  ...
  private static String[] retrieveVariants(ResourceBundle b) {
    return new String[] {
      b.getString("PeriodFormat.space"),
      b.getString("PeriodFormat.comma"),
      b.getString("PeriodFormat.commandand"),
      b.getString("PeriodFormat.commaspaceand")
    };
  }
