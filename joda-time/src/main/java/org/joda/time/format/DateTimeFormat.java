 ...
 * factory methods to create formatters, and this is one. The others are {@link ISODateTimeFormat}
 * and {@link DateTimeFormatterBuilder}.
 *
 * <p>This class provides two types of factory:
 *
 * <ul>
 *   <li>{@link #forPattern(String) Pattern} provides a DateTimeFormatter based on a pattern string
 *       that is mostly compatible with the JDK date patterns.
 *   <li>{@link #forStyle(String) Style} provides a DateTimeFormatter based on a two character
 *       style, representing short, medium, long and full.
 * </ul>
 *
 * <p>For example, to use a pattern:
 *
 * <pre>
 ...
 * </pre>
 *
 * The pattern syntax is mostly compatible with java.text.SimpleDateFormat - time zone names cannot
 * be parsed and a few more symbols are supported. All ASCII letters are reserved as pattern
 * letters, which are defined as follows:
 *
 * <blockquote>
 *
 * <pre>
 ...
 *
 * <p><strong>Text</strong>: If the number of pattern letters is 4 or more, the full form is used;
 * otherwise a short or abbreviated form is used if available.
 *
 * <p><strong>Number</strong>: The minimum number of digits. Shorter numbers are zero-padded to this
 * amount. When parsing, any number of digits are accepted.
 *
 * <p><strong>Year</strong>: Numeric presentation for year and weekyear fields are handled
 * specially. For example, if the count of 'y' is 2, the year will be displayed as the zero-based
 * year of the century, which is two digits.
 *
 * <p><strong>Month</strong>: 3 or over, use text, otherwise use number.
 *
 * <p><strong>Millis</strong>: The exact number of fractional digits. If more millisecond digits are
 * available then specified the number will be truncated, if there are fewer than specified then the
 * number will be zero-padded to the right. When parsing, only the exact number of digits are
 * accepted.
 *
 * <p><strong>Zone</strong>: 'Z' outputs offset without a colon, 'ZZ' outputs the offset with a
 * colon, 'ZZZ' or more outputs the zone id.
 *
 * <p><strong>Zone names</strong>: Time zone names ('z') cannot be parsed.
 *
 ...
 *
 * <p>DateTimeFormat is thread-safe and immutable, and the formatters it returns are as well.
 *
...
public class DateTimeFormat {
  ...
  private static final AtomicReferenceArray<DateTimeFormatter> cStyleCache =
      new AtomicReferenceArray<DateTimeFormatter>(25);

  // -----------------------------------------------------------------------
  /**
   * Factory to create a formatter from a pattern string. The pattern string is described above in
   * the class level javadoc. It is very similar to SimpleDateFormat patterns.
   *
   * <p>The format may contain locale specific output, and this will change as you change the locale
   * of the formatter. Call {@link DateTimeFormatter#withLocale(Locale)} to switch the locale. For
   * example:
   *
   * <pre>
   ...
   * time may be omitted by specifying a style character '-'.
   *
   * <p>The returned formatter will dynamically adjust to the locale that the print/parse takes
   * place in. Thus you just call {@link DateTimeFormatter#withLocale(Locale)} and the
   * Short/Medium/Long/Full style for that locale will be output. For example:
   *
   * <pre>
   ...
   *
   * <p>The format will change as you change the locale of the formatter. Call {@link
   * DateTimeFormatter#withLocale(Locale)} to switch the locale.
   *
   * @return the formatter
   */
  ...
  /**
   * Select a format from a two character style pattern. The first character is the date style, and
   * the second character is the time style. Specify a character of 'S' for short style, 'M' for
   * medium, 'L' for long, and 'F' for full. A date or time may be omitted by specifying a style
   * character '-'.
   *
   * @param style two characters from the set {"S", "M", "L", "F", "-"}
   * @throws IllegalArgumentException if the style is invalid
  ...
  static class StyleFormatter implements InternalPrinter, InternalParser {
    ...
    public void printTo(Appendable appendable, ReadablePartial partial, Locale locale)
        throws IOException {
      InternalPrinter p = null;
      // buggy version 1: change the method called by getPrinter() to an invalid method
      p = getFormatter(locale).getPrinterErr();
    ...
    private DateTimeFormatter getFormatter(Locale locale) {
      ...
      StyleFormatterCacheKey key =
          new StyleFormatterCacheKey(iType, iDateStyle, iTimeStyle, locale);
