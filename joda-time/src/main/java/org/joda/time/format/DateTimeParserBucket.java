...
/**
 * DateTimeParserBucket is an advanced class, intended mainly for parser implementations. It can
 * also be used during normal parsing operations to capture more information about the parse.
 *
 * <p>This class allows fields to be saved in any order, but be physically set in a consistent
 * order. This is useful for parsing against formats that allow field values to contradict each
 * other.
 *
 * <p>Field values are applied in an order where the "larger" fields are set first, making their
 * value less likely to stick. A field is larger than another when it's range duration is longer. If
 * both ranges are the same, then the larger field has the longer duration. If it cannot be
 * determined which field is larger, then the fields are set in the order they were saved.
 *
 * <p>For example, these fields were saved in this order: dayOfWeek, monthOfYear, dayOfMonth,
 * dayOfYear. When computeMillis is called, the fields are set in this order: monthOfYear,
 * dayOfYear, dayOfMonth, dayOfWeek.
 *
 * <p>DateTimeParserBucket is mutable and not thread-safe.
 *
...
public class DateTimeParserBucket {
   ...
   */
  public DateTimeParserBucket(
      long instantLocal, Chronology chrono, Locale locale, Integer pivotYear, int defaultYear) {
    super();
   ...
   *
   * <p>This resets the state of the bucket, allowing a single bucket to be re-used for many parses.
  ...
  /**
   * Parses a datetime from the given text, returning the number of milliseconds since the epoch,
   * 1970-01-01T00:00:00Z.
   *
   * <p>This parses the text using the parser into this bucket. The bucket is reset before parsing
   * begins, allowing the bucket to be re-used. The bucket must not be shared between threads.
   *
  ...
  long doParseMillisBuggy3(InternalParser parser, CharSequence text) {
   ...
   *
   * <p>This is used for two-digit years and when the largest parsed field is months or days.
   *
   * <p>A null value for two-digit years means to use the value from DateTimeFormatterBuilder. A
   * null value for month/day only parsing will cause the default of 2000 to be used.
   *
  ...
  /**
   * Saves the state of this bucket, returning it in an opaque object. Call restoreState to undo any
   * changes that were made since the state was saved. Calls to saveState may be nested.
   *
  ...
  /**
   * Restores the state of this bucket from a previously saved state. The state object passed into
   * this method is not consumed, and it can be used later to restore to that state again.
   *
  ...
  /**
   * Computes the parsed datetime by setting the saved fields. This method is idempotent, but it is
   * not thread-safe.
   *
  ...
  /**
   * Sorts elements [0,high). Calling java.util.Arrays isn't always the right choice since it always
   * creates an internal copy of the array, even if it doesn't need to. If the array slice is small
   * enough, an insertion sort is chosen instead, but it doesn't need a copy!
   *
   * <p>This method has a modified version of that insertion sort, except it doesn't create an
   * unnecessary array copy. If high is over 10, then java.util.Arrays is called, which will perform
   * a merge sort, which is faster than insertion sort on large lists.
   *
   * <p>The end result is much greater performance when computeMillis is called. Since the amount of
   * saved fields is small, the insertion sort is a better choice. Additional performance is gained
   * since there is no extra array allocation and copying. Also, the insertion sort here does not
   * perform any casting operations. The version in java.util.Arrays performs casts within the
   * insertion sort loop.
   */
