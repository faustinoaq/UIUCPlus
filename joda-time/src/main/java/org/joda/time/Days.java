 ...
 *
 * <p><code>Days</code> is an immutable period that can only store days. It does not store years,
 * months or hours for example. As such it is a type-safe way of representing a number of days in an
 * application.
 *
 * <p>The number of days is set in the constructor, and may be queried using <code>getDays()</code>.
 * Basic mathematical operations are provided - <code>plus()</code>, <code>minus()</code>, <code>
 * multipliedBy()</code> and <code>dividedBy()</code>.
 *
 * <p><code>Days</code> is thread-safe and immutable.
 *
...
public final class Days extends BaseSingleFieldPeriod {
  ...
  // -----------------------------------------------------------------------
  /**
   * Obtains an instance of <code>Days</code> that may be cached. <code>Days</code> is immutable, so
   * instances can be cached and shared. This factory method provides access to shared instances.
   *
  ...
  // -----------------------------------------------------------------------
  /**
   * Creates a <code>Days</code> representing the number of whole days between the two specified
   * datetimes. This method correctly handles any daylight savings time changes that may occur
   * during the interval.
   *
  ...
  /**
   * Creates a <code>Days</code> representing the number of whole days in the specified interval.
   * This method correctly handles any daylight savings time changes that may occur during the
   * interval.
   *
  ...
  public static Days daysIn(ReadableInterval interval) {
    ...
    int amount =
        BaseSingleFieldPeriod.between(
            interval.getStart(), interval.getEnd(), DurationFieldType.days());
    return Days.days(amount);
  }

  /**
   * Creates a new <code>Days</code> representing the number of complete standard length days in the
   * specified period.
   *
   ...
   *
   * <p>The parse will accept the full ISO syntax of PnYnMnWnDTnHnMnS however only the days
  ...
  // -----------------------------------------------------------------------
  /**
   * Creates a new instance representing a number of days. You should consider using the factory
   * method {@link #days(int)} instead of the constructor.
   *
   ...
   *
   * <p>This method allows you to convert between different types of period. However to achieve this
   * it makes the assumption that all days are 24 hours long and all hours are 60 minutes long. This
   * is not true when daylight savings is considered and may also not be true for some unusual
   * chronologies. However, it is included as it is a useful operation for many applications and
   * business rules.
   *
  ...
  /**
   * Converts this period in days to a period in seconds assuming a 24 hour day, 60 minute hour and
   * 60 second minute.
   *
   * <p>This method allows you to convert between different types of period. However to achieve this
   * it makes the assumption that all days are 24 hours long, all hours are 60 minutes long and all
   * minutes are 60 seconds long. This is not true when daylight savings is considered and may also
  ...
  // -----------------------------------------------------------------------
  /**
   * Converts this period in days to a duration in milliseconds assuming a 24 hour day, 60 minute
   * hour and 60 second minute.
   *
   * <p>This method allows you to convert from a period to a duration. However to achieve this it
   * makes the assumption that all days are 24 hours long, all hours are 60 minutes and all minutes
   * are 60 seconds. This is not true when daylight savings time is considered, and may also not be
   * true for some unusual chronologies. However, it is included as it is a useful operation for
  ...
  /**
   * Returns a new instance with the days divided by the specified divisor. The calculation uses
