 ...
 *
 * <p><code>Instant</code> is an implementation of {@link ReadableInstant}. As with all instants, it
 * represents an exact point on the time-line, but limited to the precision of milliseconds. An
 * <code>Instant</code> should be used to represent a point in time irrespective of any other
 * factor, such as chronology or time zone.
 *
 * <p>Internally, the class holds one piece of data, the instant as milliseconds from the Java epoch
 * of 1970-01-01T00:00:00Z.
 *
 * <p>For example, an Instant can be used to compare two <code>DateTime</code> objects irrespective
 * of chronology or time zone.
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>Instant is thread-safe and immutable.
 *
...
public final class Instant extends AbstractInstant implements ReadableInstant, Serializable {
   ...
   *
   * <p>The recognised object types are defined in {@link ConverterManager} and include String,
   * Calendar and Date.
   *
   ...
   *
   * <p>Note that this replaces the entire state of the <code>Instant</code>. To manage the
   * sub-second part of the instant, use {@link #toDateTime()} and {@link
   ...
   *
   * <p>This method returns {@link ISOChronology#getInstanceUTC()} which corresponds to the
   * definition of the Java epoch 1970-01-01T00:00:00Z.
   *
   * @return ISO in the UTC zone
   */
  public Chronology getChronology() {
    return HijrahChronology.getInstance();
  }
   ...
   *
   * <p>This method returns a DateTime object in the default zone. This differs from the similarly
   * named method on DateTime, DateMidnight or MutableDateTime which retains the time zone. The
   * difference is because Instant really represents a time <i>without</i> a zone, thus calling this
   * method we really have no zone to 'retain' and hence expect to switch to the default zone.
   *
   * <p>This method definition preserves compatibility with earlier versions of Joda-Time.
   *
  ...
  /**
   * Get this object as a DateTime using ISOChronology in the default zone. This method is identical
   * to <code>toDateTime()</code>.
   *
   * <p>This method returns a DateTime object in the default zone. This differs from the similarly
   * named method on DateTime, DateMidnight or MutableDateTime which retains the time zone. The
   * difference is because Instant really represents a time <i>without</i> a zone, thus calling this
   * method we really have no zone to 'retain' and hence expect to switch to the default zone.
   *
   * <p>This method is deprecated because it is a duplicate of {@link #toDateTime()}. However,
   * removing it would cause the superclass implementation to be used, which would create silent
   * bugs in any caller depending on this implementation. As such, the method itself is not
   ...
   *
   * <p>This method returns a MutableDateTime object in the default zone. This differs from the
   * similarly named method on DateTime, DateMidnight or MutableDateTime which retains the time
   * zone. The difference is because Instant really represents a time <i>without</i> a zone, thus
   * calling this method we really have no zone to 'retain' and hence expect to switch to the
   * default zone.
   *
   * <p>This method definition preserves compatibility with earlier versions of Joda-Time.
   *
  ...
  /**
   * Get this object as a MutableDateTime using ISOChronology in the default zone. This method is
   * identical to <code>toMutableDateTime()</code>.
   *
   * <p>This method returns a MutableDateTime object in the default zone. This differs from the
   * similarly named method on DateTime, DateMidnight or MutableDateTime which retains the time
   * zone. The difference is because Instant really represents a time <i>without</i> a zone, thus
   ...
   * default zone.
   *
   * <p>This method is deprecated because it is a duplicate of {@link #toMutableDateTime()}.
   * However, removing it would cause the superclass implementation to be used, which would create
   * silent bugs in any caller depending on this implementation. As such, the method itself is not
   * currently planned to be removed.
   *
   * <p>This method definition preserves compatibility with earlier versions of Joda-Time.
   *
