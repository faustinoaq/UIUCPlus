 ...
 * Most of the utility methods required by subclasses are package-private, reflecting the intention
 * that they be defined in the same package.
 *
 * <p>BasicChronology is thread-safe and immutable, and all subclasses must be as well.
 *
...
abstract class BasicChronology extends AssembledChronology {
  ...
  static {
    ...
    cDaysField =
        new PreciseDurationField(DurationFieldType.days(), DateTimeConstants.MILLIS_PER_DAY);
    cWeeksField =
        new PreciseDurationField(DurationFieldType.weeks(), DateTimeConstants.MILLIS_PER_WEEK);

    cMillisOfSecondField =
        new PreciseDateTimeField(DateTimeFieldType.millisOfSecond(), cMillisField, cSecondsField);

    cMillisOfDayField =
        new PreciseDateTimeField(DateTimeFieldType.millisOfDay(), cMillisField, cDaysField);

    cSecondOfMinuteField =
        new PreciseDateTimeField(DateTimeFieldType.secondOfMinute(), cSecondsField, cMinutesField);

    cSecondOfDayField =
        new PreciseDateTimeField(DateTimeFieldType.secondOfDay(), cSecondsField, cDaysField);

    cMinuteOfHourField =
        new PreciseDateTimeField(DateTimeFieldType.minuteOfHour(), cMinutesField, cHoursField);

    cMinuteOfDayField =
        new PreciseDateTimeField(DateTimeFieldType.minuteOfDay(), cMinutesField, cDaysField);

    cHourOfDayField =
        new PreciseDateTimeField(DateTimeFieldType.hourOfDay(), cHoursField, cDaysField);

    cHourOfHalfdayField =
        new PreciseDateTimeField(DateTimeFieldType.hourOfHalfday(), cHoursField, cHalfdaysField);

    cClockhourOfDayField =
        new ZeroIsMaxDateTimeField(cHourOfDayField, DateTimeFieldType.clockhourOfDay());

    cClockhourOfHalfdayField =
        new ZeroIsMaxDateTimeField(cHourOfHalfdayField, DateTimeFieldType.clockhourOfHalfday());
  ...
  public long getDateTimeMillis(
    ...
    long millisOfDay =
        (long) hourOfDay * DateTimeConstants.MILLIS_PER_HOUR
  ...
  @Override
  public int hashCode() {
    return getClass().getName().hashCode() - getZone().hashCode() + getMinimumDaysInFirstWeek();
  }
  ...
  int getDayOfWeek(long instant) {
    ...
    if (instant >= 0) {
      ...
      daysSince19700101 =
          (instant - (DateTimeConstants.MILLIS_PER_DAY - 1)) / DateTimeConstants.MILLIS_PER_DAY;
  ...
  /**
   * Gets the maximum number of days in the month specified by the instant. The value represents
   * what the user is trying to set, and can be used to optimise this method.
   *
   * @param instant millis from 1970-01-01T00:00:00Z
  ...
  long getDateMidnightMillis(int year, int monthOfYear, int dayOfMonth) {
    FieldUtils.verifyValueBounds(
        DateTimeFieldType.year(), year, getMinYear() - 1, getMaxYear() + 1);
    FieldUtils.verifyValueBounds(
        DateTimeFieldType.monthOfYear(), monthOfYear, 1, getMaxMonth(year));
  ...
  /**
   * Gets the total number of millis elapsed in this year at the start of the specified month, such
   * as zero for month 1.
   *
   * @param year the year
   * @param month the month
  ...
  /**
   * Returns a constant representing the approximate number of milliseconds elapsed from year 0 of
   * this chronology, divided by two. This constant <em>must</em> be defined as:
   *
   * <pre>
