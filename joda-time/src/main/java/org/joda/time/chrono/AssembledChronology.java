...
public abstract class AssembledChronology extends BaseChronology {
  ...
  /**
   * Constructor calls the assemble method, enabling subclasses to define its supported fields. If a
   * base chronology is supplied, the field set initially contains references to each base
   * chronology field.
   *
   * <p>Other methods in this class will delegate to the base chronology, if it can be determined
   * that the base chronology will produce the same results as AbstractChronology.
   *
  ...
  public final DateTimeField halfdayOfDay() {
    return iHalfdayOfDay.withChronology(null);
  }
  ...
  /**
   * Invoked by the constructor and after deserialization to allow subclasses to define all of its
   * supported fields. All unset fields default to unsupported instances.
   *
    ...
    if (iBase == null) {
      ...
      flags =
          ((iHourOfDay == iBase.hourOfDay()
                      ...
                      && iMillisOfSecond == iBase.millisOfSecond())
                  ? 1
                  : 0)
              | ((iMillisOfDay == iBase.millisOfDay()) ? 2 : 0)
              | ((iYear == iBase.year()
                      && iMonthOfYear == iBase.monthOfYear()
                      && iDayOfMonth == iBase.dayOfMonth())
                  ? 4
                  : 0);
    }
