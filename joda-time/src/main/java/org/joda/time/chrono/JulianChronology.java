...
/**
 * Implements a pure proleptic Julian calendar system, which defines every fourth year as leap. This
 * implementation follows the leap year rule strictly, even for dates before 8 CE, where leap years
 * were actually irregular. In the Julian calendar, year zero does not exist: 1 BCE is followed by 1
 * CE.
 *
 * <p>Although the Julian calendar did not exist before 45 BCE, this chronology assumes it did, thus
 * it is proleptic. This implementation also fixes the start of the year at January 1.
 *
 * <p>JulianChronology is thread-safe and immutable.
 *
...
public final class JulianChronology extends BasicGJChronology {
  ...
  private static final ConcurrentHashMap<DateTimeZone, JulianChronology[]> cCache =
      new ConcurrentHashMap<DateTimeZone, JulianChronology[]>();
  ...
  public static JulianChronology getInstance(DateTimeZone zone, int minDaysInFirstWeek) {
    ...
    if (chronos == null) {
      ...
      if ((current != chronos) && (oldChronos != null)) {
        chronos = oldChronos;
      }
    }
    try {
