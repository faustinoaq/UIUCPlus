 ...
 *
 * <p>This calendar is a lunar calendar with a shorter year than ISO. Year 1 in the Islamic calendar
 * began on July 16, 622 CE (Julian), thus Islamic years do not begin at the same time as Julian
 * years. This chronology is not proleptic, as it does not allow dates before the first Islamic
 * year.
 *
 * <p>There are two basic forms of the Islamic calendar, the tabular and the observed. The observed
 * form cannot easily be used by computers as it relies on human observation of the new moon. The
 * tabular calendar, implemented here, is an arithmetical approximation of the observed form that
 * follows relatively simple rules.
 *
 * <p>The tabular form of the calendar defines 12 months of alternately 30 and 29 days. The last
 * month is extended to 30 days in a leap year. Leap years occur according to a 30 year cycle. There
 * are four recognised patterns of leap years in the 30 year cycle:
 *
 * <pre>
 ...
 * This correct start of day is at sunset on the previous day, however this cannot readily be
 * modelled and has been ignored.
 *
 * <p>IslamicChronology is thread-safe and immutable.
 *
...
public final class IslamicChronology extends BasicChronology {
  ...
  public static final LeapYearPatternType LEAP_YEAR_HABASH_AL_HASIB =
      new LeapYearPatternType(3, 153692453);

  /** The lowest year that can be fully supported. */
  private static final int MIN_YEAR = -292269337;

  /**
   * The highest year that can be fully supported. Although calculateFirstDayOfYearMillis can go
   * higher without overflowing, the getYear method overflows when it adds the approximate millis at
   * the epoch.
   */
  ...
  private static final long MILLIS_PER_CYCLE =
      ((19L * 354L + 11L * 355L) * DateTimeConstants.MILLIS_PER_DAY);

  /** Cache of zone to chronology arrays */
  private static final ConcurrentHashMap<DateTimeZone, IslamicChronology[]> cCache =
      new ConcurrentHashMap<DateTimeZone, IslamicChronology[]>();
  ...
  int getMonthOfYear(long millis, int year) {
    ...
    //        return (monthPairZeroBased * 2) + 1 + (monthPairRemainder >= LONG_MONTH_LENGTH ? 1 :
    // 0);
  }
  ...
  public static class LeapYearPatternType implements Serializable {
    ...
    /**
     * Constructor. This constructor takes a bit pattern where bits 0-29 correspond to years 0-29 in
     * the 30 year Islamic cycle of years. This allows a highly efficient lookup by bit-matching.
     *
