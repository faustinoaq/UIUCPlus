 ...
 * Chronology may support. The limits are applied to both DateTimeFields and DurationFields.
 *
 * <p>Methods in DateTimeField and DurationField throw an IllegalArgumentException whenever given an
 * input instant that is outside the limits or when an attempt is made to move an instant outside
 * the limits.
 *
 * <p>LimitChronology is thread-safe and immutable.
 *
...
public final class LimitChronology extends AssembledChronology {
  ...
  /**
   * Wraps another chronology, with datetime limits. When withUTC or withZone is called, the
   * returned LimitChronology instance has the same limits, except they are time zone adjusted.
   *
   ...
   */
  public static LimitChronology getInstance(
      Chronology base, ReadableDateTime lowerLimit, ReadableDateTime upperLimit) {
    if (base == null) {
  ...
  /**
   * If this LimitChronology is already UTC, then this is returned. Otherwise, a new instance is
   * returned, with the limits adjusted to the new time zone.
   */
  @Override
  ...
  // -----------------------------------------------------------------------
  /**
   * Extends IllegalArgumentException such that the exception message is not generated unless it is
   * actually requested.
   */
