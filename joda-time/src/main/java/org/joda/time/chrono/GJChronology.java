...
/**
 * Implements the Gregorian/Julian calendar system which is the calendar system used in most of the
 * world. Wherever possible, it is recommended to use the {@link ISOChronology} instead.
 *
 * <p>The Gregorian calendar replaced the Julian calendar, and the point in time when this
 * chronology switches can be controlled using the second parameter of the getInstance method. By
 * default this cutover is set to the date the Gregorian calendar was first instituted, October 15,
 * 1582.
 *
 * <p>Before this date, this chronology uses the proleptic Julian calendar (proleptic means
 * extending indefinitely). The Julian calendar has leap years every four years, whereas the
 * Gregorian has special rules for 100 and 400 years. A meaningful result will thus be obtained for
 * all input values. However before 8 CE, Julian leap years were irregular, and before 45 BCE there
 * was no Julian calendar.
 *
 * <p>This chronology differs from {@link java.util.GregorianCalendar GregorianCalendar} in that
 * years in BCE are returned correctly. Thus year 1 BCE is returned as -1 instead of 1. The
 * yearOfEra field produces results compatible with GregorianCalendar.
 *
 * <p>The Julian calendar does not have a year zero, and so year -1 is followed by year 1. If the
 * Gregorian cutover date is specified at or before year -1 (Julian), year zero is defined. In other
 * words, the proleptic Gregorian chronology used by this class has a year zero.
 *
 * <p>To create a pure proleptic Julian chronology, use {@link JulianChronology}, and to create a
 * pure proleptic Gregorian chronology, use {@link GregorianChronology}.
 *
 * <p>GJChronology is thread-safe and immutable.
 *
...
public final class GJChronology extends AssembledChronology {
  ...
  private static final ConcurrentHashMap<GJCacheKey, GJChronology> cCache =
      new ConcurrentHashMap<GJCacheKey, GJChronology>();

  /**
   * Factory method returns instances of the default GJ cutover chronology. This uses a cutover date
   * of October 15, 1582 (Gregorian) 00:00:00 UTC. For this value, October 4, 1582 (Julian) is
   * followed by October 15, 1582 (Gregorian).
   *
   * <p>The first day of the week is designated to be {@link org.joda.time.DateTimeConstants#MONDAY
   * Monday}, and the minimum days in the first week of the year is 4.
   *
  ...
  /**
   * Factory method returns instances of the GJ cutover chronology. This uses a cutover date of
   * October 15, 1582 (Gregorian) 00:00:00 UTC. For this value, October 4, 1582 (Julian) is followed
  ...
  /**
   * Factory method returns instances of the GJ cutover chronology. Any cutover date may be
   * specified.
   *
   * <p>The first day of the week is designated to be {@link org.joda.time.DateTimeConstants#MONDAY
   * Monday}, and the minimum days in the first week of the year is 4.
   ...
   */
  public static GJChronology getInstance(
      DateTimeZone zone, ReadableInstant gregorianCutover, int minDaysInFirstWeek) {
    ...
    if (gregorianCutover == null) {
      ...
      LocalDate cutoverDate =
          new LocalDate(cutoverInstant.getMillis(), GregorianChronology.getInstance(zone));
   ...
   */
  public static GJChronology getInstance(
      DateTimeZone zone, long gregorianCutover, int minDaysInFirstWeek) {
   ...
   */
  private GJChronology(
      JulianChronology julian, GregorianChronology gregorian, Instant cutoverInstant) {
  ...
  public long getDateTimeMillis(int year, int monthOfYear, int dayOfMonth, int millisOfDay)
    ...
    long instant =
        iGregorianChronology.getDateTimeMillis(year, monthOfYear, dayOfMonth, millisOfDay);
  ...
  protected void assemble(Fields fields) {
    ...
    if (gregorian.millisOfDay().get(iCutoverMillis) == 0) {
      ...
      fields.millisOfSecond =
          new CutoverField(julian.millisOfSecond(), fields.millisOfSecond, iCutoverMillis);
      fields.millisOfDay =
          new CutoverField(julian.millisOfDay(), fields.millisOfDay, iCutoverMillis);
      fields.secondOfMinute =
          new CutoverField(julian.secondOfMinute(), fields.secondOfMinute, iCutoverMillis);
      fields.secondOfDay =
          new CutoverField(julian.secondOfDay(), fields.secondOfDay, iCutoverMillis);
      fields.minuteOfHour =
          new CutoverField(julian.minuteOfHour(), fields.minuteOfHour, iCutoverMillis);
      fields.minuteOfDay =
          new CutoverField(julian.minuteOfDay(), fields.minuteOfDay, iCutoverMillis);
      fields.hourOfDay = new CutoverField(julian.hourOfDay(), fields.hourOfDay, iCutoverMillis);
      fields.hourOfHalfday =
          new CutoverField(julian.hourOfHalfday(), fields.hourOfHalfday, iCutoverMillis);
      fields.clockhourOfDay =
          new CutoverField(julian.clockhourOfDay(), fields.clockhourOfDay, iCutoverMillis);
      fields.clockhourOfHalfday =
          new CutoverField(julian.clockhourOfHalfday(), fields.clockhourOfHalfday, iCutoverMillis);
      fields.halfdayOfDay =
          new CutoverField(julian.halfdayOfDay(), fields.halfdayOfDay, iCutoverMillis);
    }
    ...
    // duration fields as well.
    {
      ...
      fields.yearOfEra =
          new ImpreciseCutoverField(
              julian.yearOfEra(), fields.yearOfEra, fields.years, iCutoverMillis);

      fields.centuryOfEra =
          new ImpreciseCutoverField(julian.centuryOfEra(), fields.centuryOfEra, iCutoverMillis);
...

    {
      ...
      fields.dayOfYear =
          new CutoverField(julian.dayOfYear(), fields.dayOfYear, fields.years, cutover, false);
    }
  ...
  // -----------------------------------------------------------------------
  /**
   * This basic cutover field adjusts calls to 'get' and 'set' methods, and assumes that calls to
   * add and addWrapField are unaffected by the cutover.
   */
  private class CutoverField extends BaseDateTimeField {
    ...
    public long add(long instant, int value) {
      return iGregorianField.add(instant * value, value); // Multiplying instant by value
    }
    ...
    public int getMaximumTextLength(Locale locale) {
      return Math.max(
          iJulianField.getMaximumTextLength(locale), iGregorianField.getMaximumTextLength(locale));
    }

    @Override
    public int getMaximumShortTextLength(Locale locale) {
      return Math.max(
          iJulianField.getMaximumShortTextLength(locale),
  ...
  // -----------------------------------------------------------------------
  /**
   * Cutover field for variable length fields. These fields internally call set whenever add is
   * called. As a result, the same correction applied to set must be applied to add and
   * addWrapField. Knowing when to use this field requires specific knowledge of how the GJ fields
   * are implemented.
   */
  private final class ImpreciseCutoverField extends CutoverField {
    ...
    ImpreciseCutoverField(
        DateTimeField julianField, DateTimeField gregorianField, long cutoverMillis) {
