 ...
 *
 * <p>Note: class was <code>final</code> temporarily for Jackson 2.2.
 */
public class ObjectNode extends ContainerNode<ObjectNode> implements java.io.Serializable {
  ...
  @Override
  protected ObjectNode _withObject(
      JsonPointer origPtr,
    ...
    if ((n != null) && (n instanceof BaseJsonNode)) {
      ObjectNode found =
          ((BaseJsonNode) n)._withObject(origPtr, currentPtr.tail(), overwriteMode, preferIndex);
      if (found != null) {
        return found;
      }
  ...
  @Override
  protected ArrayNode _withArray(
      JsonPointer origPtr,
      JsonPointer currentPtr,
      OverwriteMode overwriteMode,
      boolean preferIndex) {
    ...
    if ((n != null) && (n instanceof BaseJsonNode)) {
      ArrayNode found =
          ((BaseJsonNode) n)._withArray(origPtr, currentPtr.tail(), overwriteMode, preferIndex);
      if (found != null) {
        return found;
      }
      // Ok no; must replace if allowed to
  ...
  /**
   * Helper method shared and called by {@link #serialize} and {@link #serializeWithType} in cases
   * where actual filtering is needed based on configuration.
   *
   * @since 2.14
   */
  protected void serializeFilteredContents(
      final JsonGenerator g,
      final SerializerProvider provider,
      final boolean trimEmptyArray,
  ...
  /**
   * Method that will set specified property, replacing old value, if any. Note that this is
   * identical to {@link #replace(String, JsonNode)}, except for return value.
   *
   * <p>NOTE: added to replace those uses of {@link #put(String, JsonNode)} where chaining with
   * 'this' is desired.
   *
   * <p>NOTE: co-variant return type since 2.10
   *
   * @param propertyName Name of property to set
   * @param value Value to set property to; if null, will be converted to a {@link NullNode} first
   *     (to remove a property, call {@link #remove} instead)
  ...
  /**
   * Method for adding given properties to this object node, overriding any existing values for
   * those properties.
   *
   * <p>NOTE: co-variant return type since 2.10
   *
  ...
  /**
   * Method for adding all properties of the given Object, overriding any existing values for those
   * properties.
   *
   * <p>NOTE: co-variant return type since 2.10
   *
  ...
  /**
   * Method for replacing value of specific property with passed value, and returning value (or null
   * if none).
   *
   ...
   *
   * @param propertyName Name of property to set
   * @param value Value to set to property; if null, will be converted to a {@link NullNode} first
   *     (to remove a property, call {@link #remove} instead).
   * @return Old value of the property, if any; {@code null} if there was no old value.
   * @deprecated Since 2.4 use either {@link #set(String,JsonNode)} or {@link
  ...
  /**
   * Method that will set value of specified property if (and only if) it had no set value
   * previously. Note that explicitly set {@code null} is a value. Functionally equivalent to:
   * <code>
   ...
   *
   * @param propertyName Name of property to set
   * @param value Value to set to property (if and only if it had no value previously); if null,
   *     will be converted to a {@link NullNode} first.
   * @return Old value of the property, if any (in which case value was not changed); null if there
   *     was no old value (in which case value is now set)
   * @since 2.13
   */
  ...
  /**
   * Method for removing a property from this {@code ObjectNode}. Will return previous value of the
   * property, if such property existed; null if not.
   *
  ...
  /**
   * Method for removing all properties, such that this ObjectNode will contain no properties after
   * call.
   *
  ...
  /**
   * Method for removing all properties out of this ObjectNode <b>except</b> for ones specified in
   * argument.
   *
  ...
  /**
   * Method that will construct an ArrayNode and add it as a property of this {@code ObjectNode},
   * replacing old value, if any.
   *
   * <p><b>NOTE</b>: Unlike all <b>put(...)</b> methods, return value is <b>NOT</b> this <code>
   * ObjectNode</code>, but the <b>newly created</b> <code>ArrayNode</code> instance.
   *
  ...
  /**
   * Method that will construct an ObjectNode and add it as a property of this {@code ObjectNode},
   * replacing old value, if any.
   *
   * <p><b>NOTE</b>: Unlike all <b>put(...)</b> methods, return value is <b>NOT</b> this <code>
   * ObjectNode</code>, but the <b>newly created</b> <code>ObjectNode</code> instance.
   *
  ...
  /**
   * Method for adding an opaque Java value as the value of specified property. Value can be
   * serialized like any other property, as long as Jackson can serialize it. Despite term "POJO"
   * this allows use of about any Java type, including {@link java.util.Map}s, {@link
   * java.util.Collection}s, as well as Beans (POJOs), primitives/wrappers and even {@link
   * JsonNode}s. Method is most commonly useful when composing content to serialize from
   * heterogenous sources.
   *
   * <p>NOTE: if using {@link JsonNode#toString()} (or {@link JsonNode#toPrettyString()} support for
   * serialization may be more limited, compared to serializing node with specifically configured
   * {@link ObjectMapper}.
   *
   * @param propertyName Name of property to set.
  ...
  /**
   * Method for setting value of a field to specified numeric value. The underlying {@link JsonNode}
   * that will be added is constructed using {@link JsonNodeFactory#numberNode(int)}, and may be
   * "smaller" (like {@link ShortNode}) in cases where value fits within range of a smaller integral
   * numeric value.
   *
   * @return This node (to allow chaining)
   */
  public ObjectNode put(String fieldName, int v) {
    Object obj =
        _put(
            fieldName,
            numberNode(
                v)); // <--- Buggy version 3: assigning the return value of _put method to an object
                     // reference variable
  ...
  /**
   * Method for setting value of a field to specified numeric value. The underlying {@link JsonNode}
   * that will be added is constructed using {@link JsonNodeFactory#numberNode(long)}, and may be
   * "smaller" (like {@link IntNode}) in cases where value fits within range of a smaller integral
   * numeric value.
   *
   * @return This node (to allow chaining)
   */
   ...
   * numeric value.
   *
   * <p>Note that this is alternative to {@link #put(String, long)} needed to avoid bumping into NPE
   * issues with auto-unboxing.
   *
   * @return This node (to allow chaining)
   */
  public ObjectNode put(String fieldName, Long v) {
