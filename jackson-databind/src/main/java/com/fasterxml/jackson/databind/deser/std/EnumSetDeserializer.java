 ...
 *
 * <p>Note: casting within this class is all messed up -- just could not figure out a way to
 * properly deal with recursive definition of "EnumSet&lt;K extends Enum&lt;K&gt;, V&gt;
 */
...
public class EnumSetDeserializer extends StdDeserializer<EnumSet<?>>
  ...
  /**
   * Marker flag set if the <code>_nullProvider</code> indicates that all null content values should
   * be skipped (instead of being possibly converted).
   *
   * @since 2.10.1
   */
  ...
  protected EnumSetDeserializer(
      EnumSetDeserializer base, JsonDeserializer<?> deser, Boolean unwrapSingle) {
   ...
   * @since 2.10.1
   */
  public EnumSetDeserializer withResolved(
      JsonDeserializer<?> deser, NullValueProvider nuller, Boolean unwrapSingle) {
    if ((deser == this._enumDeserializer)
  ...
  public JsonDeserializer<?> createContextual(DeserializationContext ctxt, BeanProperty property)
    ...
    final Boolean unwrapSingle =
        findFormatFeature(
            ctxt, property, EnumSet.class, JsonFormat.Feature.ACCEPT_SINGLE_VALUE_AS_ARRAY);
