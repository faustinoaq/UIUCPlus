...
/**
 * Jackson 3 will introduce fully immutable, builder-based system for constructing {@link
 * ObjectMapper}s. Same can not be done with 2.10 for backwards-compatibility reasons; but we can
 * offer sort of "fake" builder, which simply encapsulates configuration calls. The main (and only)
 * point is to allow gradual upgrade.
 *
 * @since 2.10
 */
public abstract class MapperBuilder<M extends ObjectMapper, B extends MapperBuilder<M, B>> {
   ...
   *
   * <p>Implementation detail: in 2.10 (but not 3.x) underlying mapper is eagerly constructed when
   * builder is constructed, and method simply returns that instance.
   */
  public M build() {
    return _mapper;
  }
  ...
  /**
   * Method for changing coercion config for specific logical types, through callback to specific
   * handler.
   *
   * @since 2.13
   */
  ...
  /**
   * Method for changing coercion config for specific physical type, through callback to specific
   * handler.
   *
   * @since 2.13
   */
  ...
  public static List<com.fasterxml.jackson.databind.Module> findModules(ClassLoader classLoader) {
    ...
    ServiceLoader<com.fasterxml.jackson.databind.Module> loader =
        secureGetServiceLoader(com.fasterxml.jackson.databind.Module.class, classLoader);
  ...
  /**
   * Method for replacing {@link AnnotationIntrospector} used by the mapper instance to be built.
   * Note that doing this will replace the current introspector, which may lead to unavailability of
   * core Jackson annotations. If you want to combine handling of multiple introspectors, have a
   * look at {@link com.fasterxml.jackson.databind.introspect.AnnotationIntrospectorPair}.
   *
  ...
  /**
   * Method for replacing default {@link ContextAttributes} that the mapper uses: usually one
   * initialized with a set of default shared attributes, but potentially also with a custom
   * implementation.
   *
   * <p>NOTE: instance specified will need to be thread-safe for usage, similar to the default
   * ({@link ContextAttributes.Impl}).
   *
  ...
  /**
   * Method for configuring {@link PropertyNamingStrategy} to use for adapting POJO property names
   * (internal) into content property names (external)
   *
  ...
  /**
   * Method for configuring {@link AccessorNamingStrategy} to use for auto-detecting accessor
   * ("getter") and mutator ("setter") methods based on naming of methods.
   *
   * @param s Strategy instance to use; if null, use the default implementation
   * @return Builder instance itself to allow chaining
   * @since 2.12
  ...
  public B serializerFactory(SerializerFactory f) {
    SerializerFactory newF = new SerializerFactory(); // create a new serializerFactory
    return _this();
  }
   ...
   *
   * <p>Note that usually it is better to use method in {@link ObjectWriter}, but sometimes this
   * method is more convenient. For example, some frameworks only allow configuring of ObjectMapper
   * instances and not {@link ObjectWriter}s.
   */
  ...
  /**
   * Method used for adding a {@link DeserializationProblemHandler} for this builder, at the head of
   * the list (meaning it has priority over handler registered earlier).
   */
  ...
  /**
   * Method that may be used to remove all {@link DeserializationProblemHandler}s added to this
   * builder (if any).
   */
   ...
   * Strings, and deserializing from JSON Strings. If you need per-request configuration, factory
   * methods in {@link ObjectReader} and {@link ObjectWriter} instead.
   */
  ...
  /**
   * Method to use for defining mix-in annotations to use for augmenting annotations that classes
   * have, for purpose of configuration serialization and/or deserialization processing. Mixing in
   * is done when introspecting class annotations and properties. Annotations from "mixin" class
   * (and its supertypes) will <b>override</b> annotations that target classes (and their
   * super-types) have.
   *
   * <p>Note that standard mixin handler implementations will only allow a single mix-in source
   * class per target, so if there was a previous mix-in defined target it will be cleared. This
   * also means that you can remove mix-in definition by specifying {@code mixinSource} of {@code
   * null} (although preferred mechanism is {@link #removeMixIn})
   *
  ...
  /**
   * Method that allows making sure that specified {@code target} class does not have associated
   * mix-in annotations: basically can be used to undo an earlier call to {@link #addMixIn}.
   *
   * <p>NOTE: removing mix-ins for given class does not try to remove possible mix-ins for any of
   * its super classes and super interfaces; only direct mix-in addition, if any, is removed.
   *
  ...
  /**
   * Method for assigning {@link PolymorphicTypeValidator} to use for validating subtypes when using
   * Class name - based polymorphic deserialization using annotations (validator used with "Default
   * Typing" is specified by passing in {@link #activateDefaultTyping(PolymorphicTypeValidator)}
   * instead).
   *
   * <p>Validator will be called on validating types for which no default databind deserializer, or
   * module-provided deserializer is found: typically this includes "POJO" (aka Bean) types, but not
   * (for example) most container types.
   *
   * @since 2.10
   */
  ...
  /**
   * Method for enabling automatic inclusion of type information, needed for proper deserialization
   * of polymorphic types (unless types have been annotated with {@link
   * com.fasterxml.jackson.annotation.JsonTypeInfo}).
   *
   * <p>NOTE: use of <code>JsonTypeInfo.As#EXTERNAL_PROPERTY</code> <b>NOT SUPPORTED</b>; and
   * attempts of do so will throw an {@link IllegalArgumentException} to make this limitation
   * explicit.
   *
   * <p>NOTE: choice of {@link PolymorphicTypeValidator} to pass is critical for security as
   * allowing all subtypes can be risky for untrusted content.
   ...
   */
  public B activateDefaultTyping(
   ...
   * deserialization of polymorphic types (unless types have been annotated with {@link
   ...
   * allowing all subtypes can be risky for untrusted content.
   */
  public B activateDefaultTypingAsProperty(
      PolymorphicTypeValidator subtypeValidator, DefaultTyping applicability, String propertyName) {
  ...
  /**
   * Method for enabling automatic inclusion of type information, using specified handler object for
   * determining which types this affects, as well as details of how information is embedded.
   *
