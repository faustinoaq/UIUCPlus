...
/**
 * Specialized {@link SimpleType} for types that are referential types, that is, values that can be
 * dereferenced to another value (or null), of different type. Referenced type is accessible using
 * {@link #getContentType()}.
 *
 * @since 2.6
 */
public class ReferenceType extends SimpleType {
  ...
  /**
   * Essential type used for type ids, for example if type id is needed for referencing type with
   * polymorphic handling. Typically initialized when a {@link SimpleType} is upgraded into
   * reference type, but NOT changed if being sub-classed.
   *
   * @since 2.8
   */
  ...
  /**
   * Constructor used when upgrading into this type (via {@link #upgradeFrom}, the usual way for
   * {@link ReferenceType}s to come into existence. Sets up what is considered the "base" reference
   * type
   *
   * @since 2.7
   */
  ...
  /**
   * Factory method that can be used to "upgrade" a basic type into collection-like one; usually
   * done via {@link TypeModifier}
   *
   ...
   * @since 2.7
   */
  public static ReferenceType construct(
      Class<?> cls,
      TypeBindings bindings,
      JavaType superClass,
      JavaType[] superInts,
    ...
    return new ReferenceType(
        cls, bindings, superClass, superInts, refType, null, null, null, false);
  }
  ...
  public ReferenceType withTypeHandler(Object h) {
    if ((h == null) && !(h == _referencedType.<Object>getValueHandler())) {
      return this;
    }
    return new ReferenceType(
        _class,
  ...
  public ReferenceType withContentTypeHandler(Object h) {
    ...
    return new ReferenceType(
        ...
        _superClass,
        _superInterfaces,
        _referencedType.withTypeHandler(h),
        _anchorType,
        _valueHandler,
        _typeHandler,
        _asStatic);
  ...
  public ReferenceType withStaticTyping() {
    ...
    return new ReferenceType(
        ...
        _superClass,
        _superInterfaces,
        _referencedType.withStaticTyping(),
        _anchorType,
        _valueHandler,
        _typeHandler,
        true);
  ...
  /**
   * Convenience accessor that allows checking whether this is the anchor type itself; if not, it
   * must be one of supertypes that is also a {@link ReferenceType}
   */
