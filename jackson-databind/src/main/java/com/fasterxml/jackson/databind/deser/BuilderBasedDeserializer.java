...
/**
 * Class that handles deserialization using a separate Builder class, which is used for data binding
 * and produces actual deserialized value at the end of data binding.
 *
 * <p>Note on implementation: much of code has been copied from {@link BeanDeserializer}; there may
 * be opportunities to refactor this in future.
 */
public class BuilderBasedDeserializer extends BeanDeserializerBase {
  ...
  /**
   * Type that the builder will produce, target type; as opposed to `handledType()` which refers to
   * Builder class.
   *
   * @since 2.9
   */
  ...
  public BuilderBasedDeserializer(BuilderBasedDeserializer src, ObjectIdReader oir) {
    super(src, oir);
    _buildMethod = src._buildMethod;
    _targetType = oir.idType == ObjectIdGenerators.PropertyGenerator.class ? src._targetType : null;
  }
  ...
  @Override
  public BeanDeserializerBase withByNameInclusion(
  ...
  /**
   * Secondary deserialization method, called in cases where POJO instance is created as part of
   * deserialization, potentially after collecting some or all of the properties to set.
   */
  @Override
  ...
  /**
   * Method called to deserialize bean using "property-based creator": this means that a non-default
   * constructor or factory method is called, and then possibly other setters. The trick is that
   * values for creator method need to be buffered, first; and due to non-guaranteed ordering
   * possibly some other properties as well.
   *
  ...
  protected Object _deserializeFromArray(JsonParser p, DeserializationContext ctxt)
    ...
    if ((delegateDeser != null) || ((delegateDeser = _delegateDeserializer) != null)) {
      Object builder =
          _valueInstantiator.createUsingArrayDelegate(ctxt, delegateDeser.deserialize(p, ctxt));
      if (_injectables != null) {
        injectValues(ctxt, builder);
      }
  ...
  protected final Object deserializeWithView(
      JsonParser p, DeserializationContext ctxt, Object bean, Class<?> activeView)
      throws IOException {
    JsonToken t = p.currentToken();
    for (; t == JsonToken.FIELD_NAME; t = p.nextToken()) {
      String propName = p.currentName();
  ...
  protected Object deserializeWithUnwrapped(JsonParser p, DeserializationContext ctxt)
    ...
    if (_delegateDeserializer != null) {
      return _valueInstantiator.createUsingDelegate(
          ctxt, _delegateDeserializer.deserialize(p, ctxt));
    }
  ...
  @SuppressWarnings("resource")
  protected Object deserializeUsingPropertyBasedWithUnwrapped(
  ...
  protected Object deserializeWithUnwrapped(
      JsonParser p, DeserializationContext ctxt, Object builder, TokenBuffer tokens)
      throws IOException {
    final Class<?> activeView = _needViewProcesing ? ctxt.getActiveView() : null;
  ...
  protected Object deserializeWithExternalTypeId(
      JsonParser p, DeserializationContext ctxt, Object bean) throws IOException {
    final Class<?> activeView = _needViewProcesing ? ctxt.getActiveView() : null;
  ...
  protected Object deserializeUsingPropertyBasedWithExternalTypeId(
    ...
    return ctxt.reportBadDefinition(
        t,
        String.format(
            "Deserialization (of %s) with Builder, External type id, @JsonCreator not yet"
                + " implemented",
            t));
  }
}
