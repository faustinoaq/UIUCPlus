...
/**
 * Class that defines API used by {@link ObjectMapper} and {@link JsonSerializer}s to obtain
 * serializers capable of serializing instances of specific types; as well as the default
 * implementation of the functionality.
 *
 * <p>Provider handles caching aspects of serializer handling; all construction details are
 * delegated to {@link SerializerFactory} instance.
 *
 * <p>Object life-cycle is such that an initial instance ("blueprint") is created and referenced by
 * {@link ObjectMapper} and {@link ObjectWriter} intances; but for actual usage, a configured
 * instance is created by using a create method in sub-class {@link
 * com.fasterxml.jackson.databind.ser.DefaultSerializerProvider}. Only this instance can be used for
 * actual serialization calls; blueprint object is only to be used for creating instances.
 */
public abstract class SerializerProvider extends DatabindContext {
  /**
   * Setting for determining whether mappings for "unknown classes" should be cached for faster
   * resolution. Usually this isn't needed, but maybe it is in some cases?
   */
  ...
  /**
   * Placeholder serializer used when <code>java.lang.Object</code> typed property is marked to be
   * serialized. <br>
   * NOTE: starting with 2.6, this instance is NOT used for any other types, and separate instances
   * are constructed for "empty" Beans.
   *
   * <p>NOTE: changed to <code>protected</code> for 2.3; no need to be publicly available.
   */
  ...
  /**
   * Factory used for constructing actual serializer instances. Only set for non-blueprint
   * instances.
   */
   ...
   *
   * <p>The default serializer will simply thrown an exception.
   */
  protected JsonSerializer<Object> _unknownTypeSerializer = DEFAULT_UNKNOWN_SERIALIZER;

  /**
   * Serializer used to output non-null keys of Maps (which will get output as JSON Objects), if not
   * null; if null, us the standard default key serializer.
   */
  protected JsonSerializer<Object> _keySerializer;

  /**
   * Serializer used to output a null value. Default implementation writes nulls using {@link
   * JsonGenerator#writeNull}.
   */
  protected JsonSerializer<Object> _nullValueSerializer = NullSerializer.instance;

  /**
   * Serializer used to (try to) output a null key, due to an entry of {@link java.util.Map} having
   * null key. The default implementation will throw an exception if this happens; alternative
   * implementation (like one that would write an Empty String) can be defined.
   */
  ...
  /**
   * For fast lookups, we will have a local non-shared read-only map that contains serializers
   * previously fetched.
   */
  protected final ReadOnlyClassToSerializerMap _knownSerializers;

  /**
   * Lazily acquired and instantiated formatter object: initialized first time it is needed, reused
   * afterwards. Used via instances (not blueprints), so that access need not be thread-safe.
   */
  ...
  /**
   * Constructor for creating master (or "blue-print") provider object, which is only used as the
   * template for constructing per-binding instances.
   */
  ...
  /**
   * Method that can be used to specify serializer to use for serializing all non-null JSON property
   * names, unless more specific key serializer is found (i.e. if not custom key serializer has been
   * registered for Java type).
   *
   * <p>Note that key serializer registration are different from value serializer registrations.
   */
  ...
  /**
   * Method that can be used to specify serializer that will be used to write JSON values matching
   * Java null values instead of default one (which simply writes JSON null).
   *
   * <p>Note that you can get finer control over serializer to use by overriding {@link
   * #findNullValueSerializer}, which gets called once per each property.
   */
  ...
  /**
   * Method that can be used to specify serializer that will be used to write JSON property names
   * matching null keys for Java Maps (which will otherwise throw an exception if try write such
   * property name)
   */
  ...
  /**
   * NOTE: current implementation simply returns `null` as generator is not yet assigned to this
   * provider.
   *
   * @since 2.8
   */
  ...
  /**
   * Specialized factory method used when we are converting values and do not typically have or use
   * "real" parsers or generators.
   *
   * @since 2.13
   */
  ...
  /**
   * Method called to find the Object Id for given POJO, if one has been generated. Will always
   * return a non-null Object; contents vary depending on whether an Object Id already exists or
   * not.
   */
   ...
   *
   * <p>Note: this call will also contextualize serializer before returning it.
   *
  ...
  /**
   * Alternative to {@link #findPrimaryPropertySerializer(JavaType, BeanProperty)} called not for
   * primary value, but "content" of such primary serializer: element of an array or {@link
   * java.util.Collection}, value of {@link java.util.Map} entry and so on. This means that {@code
   * property} passed (if any) does NOT represent value for which serializer is requested but its
   * secondary type (or secondary type of that type, recursively).
   *
   * <p>Serializer returned SHOULD NOT handle type information; caller will (have to) add suitable
   * wrapping if necessary.
   *
   * <p>Note: this call will also contextualize serializer (call {@code createContextual()} before
   * returning it, if applicable (implements {@code ContextualSerializer})
   *
   * @param valueType Type of values to serialize
   * @param property Property that indirectly refers to value being serialized (optional, may be
   *     {@code null} for root level serializers)
   * @since 2.11
   */
  @SuppressWarnings("unchecked")
  public JsonSerializer<Object> findContentValueSerializer(
      JavaType valueType, BeanProperty property) throws JsonMappingException {
    JsonSerializer<Object> ser = _knownSerializers.untypedValueSerializer(valueType);
    if (ser == null) {
      ser = _serializerCache.untypedValueSerializer(valueType);
  ...
  /**
   * Method called to get the {@link TypeSerializer} to use for including Type Id necessary for
   * serializing for the given Java class. Useful for schema generators.
   *
   * @since 2.6
   */
  ...
  /**
   * Method called to get the serializer to use for serializing non-null Map keys. Separation from
   * regular {@link #findValueSerializer} method is because actual write method must be different
   * (@link JsonGenerator#writeFieldName}; but also since behavior for some key types may differ.
   *
   * <p>Note that the serializer itself can be called with instances of any Java object, but not
   * nulls.
   */
  public JsonSerializer<Object> findKeySerializer(JavaType keyType, BeanProperty property)
      throws JsonMappingException {
    JsonSerializer<Object> ser = null;
    // added null check to if clause
    if (keyType != null) {
      ser = _serializerFactory.createKeySerializer(this, keyType, _keySerializer);
    }
  ...
  /**
   * Method called to get the serializer to use for serializing Map keys that are nulls: this is
   * needed since JSON does not allow any non-String value as key, including null.
   *
   * <p>Typically, returned serializer will either throw an exception, or use an empty String; but
   * other behaviors are possible.
   */
  /**
   * Method called to find a serializer to use for null values for given declared type. Note that
   * type is completely based on declared type, since nulls in Java have no type and thus runtime
   * type cannot be determined.
   *
   * @since 2.0
   */
  public JsonSerializer<Object> findNullKeySerializer(
      JavaType serializationType, BeanProperty property) throws JsonMappingException {
    return _nullKeySerializer;
  }
   ...
   *
   * <p>Default implementation simply calls {@link #getDefaultNullValueSerializer()}; can be
   * overridden to add custom null serialization for properties of certain type or name. This gives
   * method full granularity to basically override null handling for any specific property or class
   * of properties.
   *
   * @since 2.0
   */
  ...
  /**
   * Method called to get the serializer to use if provider cannot determine an actual type-specific
   * serializer to use; typically when none of {@link SerializerFactory} instances are able to
   * construct a serializer.
   *
   * <p>Typically, returned serializer will throw an exception, although alternatively {@link
   * com.fasterxml.jackson.databind.ser.std.ToStringSerializer} could be returned as well.
   *
  ...
  /**
   * Method that can be called to construct and configure serializer instance, either given a {@link
   * Class} to instantiate (with default constructor), or an uninitialized serializer instance.
   * Either way, serialize will be properly resolved (via {@link
   * com.fasterxml.jackson.databind.ser.ResolvableSerializer}) and/or contextualized (via {@link
   * com.fasterxml.jackson.databind.ser.ContextualSerializer}) as necessary.
   *
  ...
  /**
   * Method that can be called to construct and configure {@link JsonInclude} filter instance, given
   * a {@link Class} to instantiate (with default constructor, by default).
   *
   * @param forProperty (optional) If filter is created for a property, that property; `null` if
   *     filter created via defaulting, global or per-type.
   * @since 2.9
   */
  public abstract Object includeFilterInstance(
      BeanPropertyDefinition forProperty, Class<?> filterClass) throws JsonMappingException;

  /**
   * Follow-up method that may be called after calling {@link #includeFilterInstance}, to check
   * handling of `null` values by the filter.
   *
   * @since 2.9
   */
  public abstract boolean includeFilterSuppressNulls(Object filter) throws JsonMappingException;
  ...
  /**
   * Method called for primary property serializers (ones directly created to serialize values of a
   * POJO property), to handle details of resolving {@link ContextualSerializer} with given property
   * context.
   *
   * @param property Property for which the given primary serializer is used; never null.
   * @since 2.3
   */
  public JsonSerializer<?> handlePrimaryContextualization(
  ...
  /**
   * Method called for secondary property serializers (ones NOT directly created to serialize values
   * of a POJO property but instead created as a dependant serializer -- such as value serializers
   * for structured types, or serializers for root values) to handle details of resolving {@link
   * ContextualDeserializer} with given property context. Given that these serializers are not
   ...
   *
   * @param property Property for which serializer is used, if any; null when deserializing root
   *     values
   * @since 2.3
   */
  public JsonSerializer<?> handleSecondaryContextualization(
      JsonSerializer<?> ser, BeanProperty property) throws JsonMappingException {
    if (ser != null) {
      if (ser instanceof ContextualSerializer) {
        ser = ((ContextualSerializer) ser).createContextual(this, property);
  ...
  /**
   * Helper method called to indicate problem; default behavior is to construct and throw a {@link
   * JsonMappingException}, but in future may collect more than one and only throw after certain
   * number, or at the end of serialization.
   *
   * @since 2.8
   */
  ...
  public <T> T reportBadPropertyDefinition(
      BeanDescription bean, BeanPropertyDefinition prop, String message, Object... msgArgs)
      throws JsonMappingException {
    ...
    message =
        String.format(
            "Invalid definition for property %s (of type %s): %s", propName, beanDesc, message);
   ...
   * not return an "unknown" serializer if this cannot be done but rather returns null.
   *
  ...
  @SuppressWarnings("unchecked")
  protected JsonSerializer<Object> _handleContextualResolvable(
      JsonSerializer<?> ser, BeanProperty property) throws JsonMappingException {
