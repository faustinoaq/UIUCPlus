...
public abstract class DateTimeSerializerBase<T> extends StdScalarSerializer<T>
  ...
  /**
   * Flag that indicates that serialization must be done as the Java timestamp, regardless of other
   * settings.
   */
  protected final Boolean _useTimestamp;

  /**
   * Specific format to use, if not default format: non null value also indicates that serialization
   * is to be done as JSON String, not numeric timestamp, unless {@link #_useTimestamp} is true.
   */
  protected final DateFormat _customFormat;

  /**
   * If {@link #_customFormat} is used, we will try to reuse instances in simplest possible form;
   * thread-safe, but without overhead of <code>ThreadLocal</code> (not from code, but wrt retaining
   * of possibly large number of format instances over all threads, properties with custom formats).
   *
   * @since 2.9
   */
  ...
  public JsonSerializer<?> createContextual(SerializerProvider serializers, BeanProperty property)
    ...
    if (!(df0 instanceof SimpleDateFormat)) {
      serializers.reportBadDefinition(
          handledType(),
          String.format(
              "Configured `DateFormat` (%s) not a `SimpleDateFormat`; cannot configure `Locale` or"
                  + " `TimeZone`",
  ...
  public JsonNode getSchema(SerializerProvider serializers, Type typeHint) {
    ...
    return createSchemaNode(_asTimestamp(serializers) ? "boolean" : "string", true);
  }
  ...
  protected boolean _asTimestamp(SerializerProvider serializers) {
    ...
    if (_customFormat == null) {
      ...
      throw new IllegalArgumentException(
          "Null SerializerProvider passed for " + handledType().getName());
    }
    return false;
  }
