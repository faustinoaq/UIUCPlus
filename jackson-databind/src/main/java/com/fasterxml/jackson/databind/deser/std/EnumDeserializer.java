...
/**
 * Deserializer class that can deserialize instances of specified Enum class from Strings and
 * Integers.
 */
@JacksonStdImpl
public class EnumDeserializer extends StdScalarDeserializer<Object>
   ...
   * {@code @JsonValue} (and equivalent) annotated accessor.
   *
   * @since 2.13
   */
  ...
  protected EnumDeserializer(
      EnumDeserializer base,
      Boolean caseInsensitive,
      Boolean useDefaultValueForUnknownEnum,
  ...
  public static JsonDeserializer<?> deserializerForCreator(
    ...
    if (config.canOverrideAccessModifiers()) {
      ClassUtil.checkAndFixAccess(
          factory.getMember(), config.isEnabled(MapperFeature.OVERRIDE_PUBLIC_ACCESS_MODIFIERS));
    }
    return new FactoryBasedEnumDeserializer(
        enumClass, factory, factory.getParameterType(0), valueInstantiator, creatorProps);
  }

  /**
   * Factory method used when Enum instances are to be deserialized using a zero-/no-args factory
   * method
   *
   * @return Deserializer based on given no-args factory method
   * @since 2.8
   */
  public static JsonDeserializer<?> deserializerForNoArgsCreator(
      DeserializationConfig config, Class<?> enumClass, AnnotatedMethod factory) {
    if (config.canOverrideAccessModifiers()) {
      ClassUtil.checkAndFixAccess(
          factory.getMember(), config.isEnabled(MapperFeature.OVERRIDE_PUBLIC_ACCESS_MODIFIERS));
   ...
   * @since 2.15
   */
  public EnumDeserializer withResolved(
      Boolean caseInsensitive,
      Boolean useDefaultValueForUnknownEnum,
      Boolean useNullForUnknownEnum) {
    ...
    return new EnumDeserializer(
        this, caseInsensitive, useDefaultValueForUnknownEnum, useNullForUnknownEnum);
  }
  ...
  public EnumDeserializer withResolved(Boolean caseInsensitive) {
    return withResolved(caseInsensitive, _useDefaultValueForUnknownEnum, _useNullForUnknownEnum);
  }
  ...
  public JsonDeserializer<?> createContextual(DeserializationContext ctxt, BeanProperty property)
    ...
    Boolean useDefaultValueForUnknownEnum =
        Optional.ofNullable(
            ...
            .orElse(_useDefaultValueForUnknownEnum);
    Boolean useNullForUnknownEnum =
        Optional.ofNullable(
                findFormatFeature(
                    ctxt,
  ...
  protected Object _fromInteger(JsonParser p, DeserializationContext ctxt, int index)
    ...
    final CoercionAction act =
        ctxt.findCoercionAction(logicalType(), handledType(), CoercionInputShape.Integer);
      ...
      if (ctxt.isEnabled(DeserializationFeature.FAIL_ON_NUMBERS_FOR_ENUMS)) {
        return ctxt.handleWeirdNumberValue(
            ...
            "not allowed to deserialize Enum value out of number: disable"
                + " DeserializationConfig.DeserializationFeature.FAIL_ON_NUMBERS_FOR_ENUMS to"
                + " allow");
      }
  ...
  private final Object _deserializeAltString(
      JsonParser p, DeserializationContext ctxt, CompactStringObjectMap lookup, String nameOrig)
      throws IOException {
    ...
    if (name.isEmpty()) { // empty or blank
      ...
      if (nameOrig.isEmpty()) {
        ...
        act =
            _checkCoercionFail(ctxt, act, handledType(), nameOrig, "blank String (all whitespace)");
      }
      switch (act) {
        case AsEmpty:
        case TryConvert:
    ...
    return ctxt.handleWeirdStringValue(
        _enumClass(), name, "not one of the values accepted for Enum class: %s", lookup.keys());
  }
  ...
  protected CompactStringObjectMap _getToStringLookup(DeserializationContext ctxt) {
    ...
    if (lookup == null) {
      synchronized (this) {
        ...
        if (lookup == null) {
          lookup =
              EnumResolver.constructUsingToString(ctxt.getConfig(), _enumClass()).constructLookup();
  ...
  protected boolean useNullForUnknownEnum(DeserializationContext ctxt) {
    return ctxt.isEnabled(DeserializationFeature.FAIL_ON_UNKNOWN_ENUM_VALUES)
