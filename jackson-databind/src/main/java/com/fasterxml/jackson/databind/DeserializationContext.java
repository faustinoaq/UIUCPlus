...
/**
 * Context for the process of deserialization a single root-level value. Used to allow passing in
 * configuration settings and reusable temporary objects (scrap arrays, containers).
 *
 * <p>Instance life-cycle is such that a partially configured "blueprint" object is registered with
 * {@link ObjectMapper} (and {@link ObjectReader}, and when actual instance is needed for
 * deserialization, a fully configured instance will be created using a method in extended internal
 * API of sub-class ({@link
 * com.fasterxml.jackson.databind.deser.DefaultDeserializationContext#createInstance}). Each
 * instance is guaranteed to only be used from single-threaded context; instances may be reused if
 * (and only if) no configuration has changed.
 *
 * <p>Defined as abstract class so that implementations must define methods for reconfiguring
 * blueprints and creating instances.
 */
public abstract class DeserializationContext extends DatabindContext
  ...
  /**
   * Read-only factory instance; exposed to let owners (<code>ObjectMapper</code>, <code>
   * ObjectReader</code>) access it.
   */
  ...
  /**
   * Currently active parser used for deserialization. May be different from the outermost parser
  ...
  /**
   * Type of {@link JsonDeserializer} (or, more specifically, {@link ContextualDeserializer}) that
   * is being contextualized currently.
   *
   * @since 2.5
   */
  ...
  /**
   * Constructor used for creating bogus per-call style instance, but without underlying parser:
   * needed for deserializer pre-fetching
   *
   * @since 2.12
   */
  ...
  /**
   * Accessor to {@link JavaType} of currently contextualized {@link ContextualDeserializer}, if
   * any. This is sometimes useful for generic {@link JsonDeserializer}s that do not get passed (or
   * do not retain) type information when being constructed: happens for example for deserializers
   * constructed from annotations.
   *
   * @since 2.5
   * @return Type of {@link ContextualDeserializer} being contextualized, if process is on-going;
   *     null if not.
   */
  ...
  /**
   * Bulk access method for getting the bit mask of all {@link DeserializationFeature}s that are
   * enabled.
   *
   * @since 2.6
   */
  ...
  /**
   * Method for accessing the currently active parser. May be different from the outermost parser
   * when content is buffered.
   *
   * <p>Use of this method is discouraged: if code has direct access to the active parser, that
  ...
  public final Object findInjectableValue(
      Object valueId, BeanProperty forProperty, Object beanInstance) throws JsonMappingException {
  ...
  /**
   * General-purpose accessor for finding what to do when specified coercion from shape that is now
   * always allowed to be coerced from is requested.
   *
   ...
   * @since 2.12
   */
  public CoercionAction findCoercionAction(
      LogicalType targetType, Class<?> targetClass, CoercionInputShape inputShape) {
    return _config.findCoercionAction(targetType, targetClass, inputShape);
  }

  /**
   * More specialized accessor called in case of input being a blank String (one consisting of only
   * white space characters with length of at least one). Will basically first determine if "blank
   * as empty" is allowed: if not, returns {@code actionIfBlankNotAllowed}, otherwise returns action
   * for {@link CoercionInputShape#EmptyString}.
   *
   * @param targetType Logical target type of coercion
   * @param targetClass Physical target type of coercion
   ...
   * @since 2.12
   */
  public CoercionAction findCoercionFromBlankString(
      LogicalType targetType, Class<?> targetClass, CoercionAction actionIfBlankNotAllowed) {
  ...
  /**
   * Factory method used for creating {@link TokenBuffer} to temporarily contain copy of content
   * read from specified parser; usually for purpose of reading contents later on (possibly
   * augmeneted with injected additional content)
   *
   * @since 2.13
   */
   ...
   * </pre>
   *
   * <p>NOTE: the whole "current value" that parser points to is read and buffered, including Object
   * and Array values (if parser pointing to start marker).
   *
   * @since 2.9
   */
   ...
   * @param cause (optional) Reference set to root cause if no deserializer could be found due to
   *     exception (to find the reason for failure)
   * @since 2.3
   */
  ...
  /**
   * Variant that will try to locate deserializer for current type, but without performing any
   * contextualization (unlike {@link #findContextualValueDeserializer}) or checking for need to
   * create a {@link TypeDeserializer} (unlike {@link #findRootValueDeserializer(JavaType)}. This
   * method is usually called from within {@link ResolvableDeserializer#resolve}, and expectation is
   * that caller then calls either {@link #handlePrimaryContextualization(JsonDeserializer,
   * BeanProperty, JavaType)} or {@link #handleSecondaryContextualization(JsonDeserializer,
   * BeanProperty, JavaType)} at a later point, as necessary.
   *
   * @since 2.5
   */
  ...
  public final JsonDeserializer<Object> findRootValueDeserializer(JavaType type)
      throws JsonMappingException {
    JsonDeserializer<Object> deser = _cache.findValueDeserializer(_factory, type);
  ...
  /**
   * Method called to find and return entry corresponding to given Object Id: will add an entry if
   * necessary, and never returns null
   */
  public abstract ReadableObjectId findObjectId(
      Object id, ObjectIdGenerator<?> generator, ObjectIdResolver resolver);
  ...
  /**
   * Helper method that is to be used when resolving basic class name into Class instance, the
   * reason being that it may be necessary to work around various ClassLoader limitations, as well
   * as to handle primitive type signatures.
   *
   * @since 2.6
   */
  ...
  /**
   * Method that can be used to get access to a reusable ObjectBuffer, useful for efficiently
  ...
  public abstract JsonDeserializer<Object> deserializerInstance(
      Annotated annotated, Object deserDef) throws JsonMappingException;

  public abstract KeyDeserializer keyDeserializerInstance(Annotated annotated, Object deserDef)
      throws JsonMappingException;
  ...
  /**
   * Method called for primary property deserializers (ones directly created to deserialize values
   * of a POJO property), to handle details of resolving {@link ContextualDeserializer} with given
   * property context.
   *
  ...
  public JsonDeserializer<?> handlePrimaryContextualization(
      JsonDeserializer<?> deser, BeanProperty prop, JavaType type) throws JsonMappingException {
  ...
  /**
   * Method called for secondary property deserializers (ones NOT directly created to deal with an
   * annotatable POJO property, but instead created as a component -- such as value deserializers
   * for structured types, or deserializers for root values) to handle details of resolving {@link
   * ContextualDeserializer} with given property context. Given that these deserializers are not
   * directly related to given property (or, in case of root value property, to any property),
   * annotations accessible may or may not be relevant.
   *
   * @param prop Property for which deserializer is used, if any; null when deserializing root
   *     values
   * @since 2.5
   */
  public JsonDeserializer<?> handleSecondaryContextualization(
      JsonDeserializer<?> deser, BeanProperty prop, JavaType type) throws JsonMappingException {
    if (deser instanceof ContextualDeserializer) {
      _currentType = new LinkedNode<JavaType>(type, _currentType);
      try {
  ...
  /**
   * Convenience method for parsing a Date from given String, using currently configured date format
   * (accessed using {@link DeserializationConfig#getDateFormat()}).
   *
   * <p>Implementation will handle thread-safety issues related to date formats such that first time
   * this method is called, date format is cloned, and cloned instance will be retained for use
   * during this deserialization round.
   */
  public Date parseDate(String dateStr) throws IllegalArgumentException {
    try {
      ...
      throw new IllegalArgumentException(
          String.format(
              "Failed to parse Date value '%s': %s", dateStr, ClassUtil.exceptionMessage(e)));
    }
  }

  /**
   * Convenience method for constructing Calendar instance set to specified time, to be modified and
   * used by caller.
   */
  ...
  /**
   * Method to call in case incoming shape is Object Value (and parser thereby points to {@link
   * com.fasterxml.jackson.core.JsonToken#START_OBJECT} token), but a Scalar value (potentially
   * coercible from String value) is expected. This would typically be used to deserializer a
   * Number, Boolean value or some other "simple" unstructured value type.
   *
   * @param p Actual parser to read content from
   * @param deser Deserializer that needs extracted String value
   * @param scalarType Immediate type of scalar to extract; usually type deserializer handles but
   *     not always (for example, deserializer for {@code int[]} would pass scalar type of {@code
   *     int})
   * @return String value found; not {@code null} (exception should be thrown if no suitable value
   *     found)
   * @throws IOException If there are problems either reading content (underlying parser problem) or
   *     finding expected scalar value
   */
  public String extractScalarFromObject(
      JsonParser p, JsonDeserializer<?> deser, Class<?> scalarType) throws IOException {
  ...
  /**
   * Convenience method that may be used by composite or container deserializers, for reading
   * one-off values contained (for sequences, it is more efficient to actually fetch deserializer
   * once for the whole collection).
   *
   * <p>NOTE: when deserializing values of properties contained in composite types, rather use
   * {@link #readPropertyValue(JsonParser, BeanProperty, Class)}; this method does not allow use of
  ...
  public <T> T readValue(JsonParser p, JavaType type) throws IOException {
    ...
    if (deser == null) {
      return reportBadDefinition(
          type, "Could not find JsonDeserializer for type " + ClassUtil.getTypeDescription(type));
    }
  ...
  /**
   * Convenience method that may be used by composite or container deserializers, for reading
   * one-off values for the composite type, taking into account annotations that the property
   * (passed to this method -- usually property that has custom serializer that called this method)
   * has.
   *
  ...
  /**
   * Same as {@link #readPropertyValue(JsonParser, BeanProperty, Class)} but with fully resolved
   * {@link JavaType} as target: needs to be used for generic types, for example.
   *
   * @since 2.4
   */
  @SuppressWarnings("unchecked")
  ...
  /**
   * Convenience method for reading the value that passed {@link JsonParser} points to as a {@link
   * JsonNode}.
   *
   * @param p Parser that points to the first token of the value to read
  ...
  public JsonNode readTree(JsonParser p) throws IOException {
    ...
    return (JsonNode)
        findRootValueDeserializer(_config.constructType(JsonNode.class)).deserialize(p, this);
  }

  /**
   * Helper method similar to {@link ObjectReader#treeToValue(TreeNode, Class)} which will read
   * contents of given tree ({@link JsonNode}) and bind them into specified target type. This is
   * often used in two-phase deserialization in which content is first read as a tree, then
   * manipulated (adding and/or removing properties of Object values, for example), and finally
   * converted into actual target type using default deserialization logic for the type.
   *
   * <p>NOTE: deserializer implementations should be careful not to try to recursively deserialize
   * into target type deserializer has registered itself to handle.
   *
   * @param n Tree value to convert, if not {@code null}: if {@code null}, will simply return {@code
   *     null}
   * @param targetType Type to deserialize contents of {@code n} into (if {@code n} not {@code
   *     null})
   * @return Either {@code null} (if {@code n} was {@code null} or a value of type {@code type} that
   *     was read from non-{@code null} {@code n} argument
   * @since 2.13
   */
  ...
  /**
   * Same as {@link #readTreeAsValue(JsonNode, Class)} but will fully resolved {@link JavaType} as
   * {@code targetType}
   *
   * <p>NOTE: deserializer implementations should be careful not to try to recursively deserialize
   * into target type deserializer has registered itself to handle.
  ...
  /**
   * Method that deserializers should call if they encounter an unrecognized property (and once that
   * is not explicitly designed as ignorable), to inform possibly configured {@link
   * DeserializationProblemHandler}s and let it handle the problem.
   *
   * @return True if there was a configured problem handler that was able to handle the problem
   */
  public boolean handleUnknownProperty(
      JsonParser p, JsonDeserializer<?> deser, Object instanceOrClass, String propName)
      throws IOException {
  ...
  /**
   * Method that deserializers should call if they encounter a String value that cannot be converted
   * to expected key of a {@link java.util.Map} valued property. Default implementation will try to
   * call {@link DeserializationProblemHandler#handleWeirdNumberValue} on configured handlers, if
   * any, to allow for recovery; if recovery does not succeed, will throw {@link
    ...
    while (h != null) {
      ...
      if (key != DeserializationProblemHandler.NOT_HANDLED) {
        ...
        throw weirdStringException(
            ...
            keyClass,
            String.format(
                "DeserializationProblemHandler.handleWeirdStringValue() for type %s returned value"
                    + " of type %s",
                ClassUtil.getClassDescription(keyClass), ClassUtil.getClassDescription(key)));
      }
      h = h.next();
    }
  ...
  /**
   * Method that deserializers should call if they encounter a String value that cannot be converted
   * to target property type, in cases where some String values could be acceptable (either with
   * different settings, or different value). Default implementation will try to call {@link
   * DeserializationProblemHandler#handleWeirdStringValue} on configured handlers, if any, to allow
   * for recovery; if recovery does not succeed, will throw {@link InvalidFormatException} with
   * given message.
   *
  ...
  public Object handleWeirdStringValue(
      Class<?> targetClass, String value, String msg, Object... msgArgs) throws IOException {
    // but if not handled, just throw exception
    msg = _format(msg, msgArgs);
    LinkedNode<DeserializationProblemHandler> h = _config.getProblemHandlers();
    while (h != null) {
  ...
  /**
   * Method that deserializers should call if they encounter a numeric value that cannot be
   * converted to target property type, in cases where some numeric values could be acceptable
   * (either with different settings, or different numeric value). Default implementation will try
   * to call {@link DeserializationProblemHandler#handleWeirdNumberValue} on configured handlers, if
   * any, to allow for recovery; if recovery does not succeed, will throw {@link
   * InvalidFormatException} with given message.
   *
   * @param targetClass Type of property into which incoming number should be converted
    ...
    while (h != null) {
      ...
      if (key != DeserializationProblemHandler.NOT_HANDLED) {
        ...
        throw weirdNumberException(
            ...
            targetClass,
            _format(
                "DeserializationProblemHandler.handleWeirdNumberValue() for type %s returned value"
                    + " of type %s",
                ClassUtil.getClassDescription(targetClass), ClassUtil.getClassDescription(key)));
      }
      h = h.next();
    }
    ...
    for (; h != null; h = h.next()) {
      ...
      if (goodValue != DeserializationProblemHandler.NOT_HANDLED) {
        ...
        throw JsonMappingException.from(
            p,
            _format(
                "DeserializationProblemHandler.handleWeirdNativeValue() for type %s returned value"
                    + " of type %s",
  ...
  /**
   * Method that deserializers should call if they fail to instantiate value due to lack of viable
   * instantiator (usually creator, that is, constructor or static factory method). Method should be
   * called at point where value has not been decoded, so that handler has a chance to handle
   * decoding using alternate mechanism, and handle underlying content (possibly by just skipping
   * it) to keep input state valid
   *
   * @param instClass Type that was to be instantiated
   * @param valueInst (optional) Value instantiator to be used, if any; null if type does not use
   *     one for instantiation (custom deserialiers don't; standard POJO deserializer does)
  ...
  public Object handleMissingInstantiator(
      Class<?> instClass, ValueInstantiator valueInst, JsonParser p, String msg, Object... msgArgs)
      throws IOException {
    if (p == null) {
      p = getParser();
    }
    ...
    while (h != null) {
      ...
      if (instance != DeserializationProblemHandler.NOT_HANDLED) {
        ...
        reportBadDefinition(
            ...
            String.format(
                "DeserializationProblemHandler.handleMissingInstantiator() for type %s returned"
                    + " value of type %s",
  ...
  /**
   * Method that deserializers should call if they fail to instantiate value due to an exception
   * that was thrown by constructor (or other mechanism used to create instances). Default
   * implementation will try to call {@link
   * DeserializationProblemHandler#handleInstantiationProblem} on configured handlers, if any, to
   * allow for recovery; if recovery does not succeed, will throw exception constructed with {@link
   * #instantiationException}.
   *
   * @param instClass Type that was to be instantiated
   * @param argument (optional) Argument that was passed to constructor or equivalent instantiator;
   *     often a {@link java.lang.String}.
    ...
    while (h != null) {
      ...
      if (instance != DeserializationProblemHandler.NOT_HANDLED) {
        ...
        reportBadDefinition(
            constructType(instClass),
            String.format(
                "DeserializationProblemHandler.handleInstantiationProblem() for type %s returned"
                    + " value of type %s",
                ClassUtil.getClassDescription(instClass), ClassUtil.classNameOf(instance)));
      }
      h = h.next();
    }
  ...
  /**
   * Method that deserializers should call if the first token of the value to deserialize is of
   * unexpected type (that is, type of token that deserializer cannot handle). This could occur, for
   * example, if a Number deserializer encounter {@link JsonToken#START_ARRAY} instead of {@link
   * JsonToken#VALUE_NUMBER_INT} or {@link JsonToken#VALUE_NUMBER_FLOAT}.
   *
   * @param instClass Type that was to be instantiated
   * @param p Parser that points to the JSON value to decode
   * @return Object that should be constructed, if any; has to be of type <code>instClass</code>
  ...
  public Object handleUnexpectedToken(
      Class<?> instClass, JsonToken t, JsonParser p, String msg, Object... msgArgs)
      throws IOException {
   ...
   */
  public Object handleUnexpectedToken(
      JavaType targetType, JsonToken t, JsonParser p, String msg, Object... msgArgs)
      throws IOException {
    msg = _format(msg, msgArgs);
    LinkedNode<DeserializationProblemHandler> h = _config.getProblemHandlers();
    while (h != null) {
      ...
      if (instance != DeserializationProblemHandler.NOT_HANDLED) {
        ...
        reportBadDefinition(
            targetType,
            String.format(
                "DeserializationProblemHandler.handleUnexpectedToken() for type %s returned value"
                    + " of type %s",
                ClassUtil.getTypeDescription(targetType), ClassUtil.classNameOf(instance)));
      }
      h = h.next();
    }
    if (msg == null) {
      ...
      if (t == null) {
        msg =
            String.format("Unexpected end-of-input when trying read value of type %s", targetDesc);
      } else {
        msg =
            String.format(
  ...
  /**
   * Method that deserializers should call if they encounter a type id (for polymorphic
   * deserialization) that cannot be resolved to an actual type; usually since there is no mapping
   * defined. Default implementation will try to call {@link
   * DeserializationProblemHandler#handleUnknownTypeId} on configured handlers, if any, to allow for
   * recovery; if recovery does not succeed, will throw exception constructed with {@link
   * #invalidTypeIdException}.
   *
   * @param baseType Base type from which resolution starts
   * @param id Type id that could not be converted
   * @param extraDesc Additional problem description to add to default exception message, if
   *     resolution fails.
  ...
  public JavaType handleUnknownTypeId(
      JavaType baseType, String id, TypeIdResolver idResolver, String extraDesc)
      throws IOException {
    LinkedNode<DeserializationProblemHandler> h = _config.getProblemHandlers();
    while (h != null) {
      // Can bail out if it's handled
  ...
  public JavaType handleMissingTypeId(
      JavaType baseType, TypeIdResolver idResolver, String extraDesc) throws IOException {
    LinkedNode<DeserializationProblemHandler> h = _config.getProblemHandlers();
    while (h != null) {
      // Can bail out if it's handled
  ...
  /**
   * Method that deserializer may call if it is called to do an update ("merge") but deserializer
   * operates on a non-mergeable type. Although this should usually be caught earlier, sometimes it
   * may only be caught during operation and if so this is the method to call. Note that if {@link
   * MapperFeature#IGNORE_MERGE_FOR_UNMERGEABLE} is enabled, this method will simply return null;
   * otherwise {@link InvalidDefinitionException} will be thrown.
   *
   * @since 2.10
   */
  ...
  /**
   * Method for deserializers to call when the token encountered was of type different than what
   * <b>should</b> be seen at that position, usually within a sequence of expected tokens. Note that
   * this method will throw a {@link JsonMappingException} and no recovery is attempted (via {@link
   * DeserializationProblemHandler}, as problem is considered to be difficult to recover from, in
   * general.
   *
   * @since 2.9
   */
  public void reportWrongTokenException(
      JsonDeserializer<?> deser, JsonToken expToken, String msg, Object... msgArgs)
      throws JsonMappingException {
    msg = _format(msg, msgArgs);
   ...
   */
  public void reportWrongTokenException(
      JavaType targetType, JsonToken expToken, String msg, Object... msgArgs)
      throws JsonMappingException {
    msg = _format(msg, msgArgs);
   ...
   */
  public void reportWrongTokenException(
      Class<?> targetType, JsonToken expToken, String msg, Object... msgArgs)
      throws JsonMappingException {
    msg = _format(msg, msgArgs);
    throw wrongTokenException(getParser(), targetType, expToken, msg);
  }
  ...
  /**
   * Helper method used to indicate a problem with input in cases where more specific <code>
   * reportXxx()</code> method was not available.
   *
   * @since 2.9
   */
   ...
   * @since 2.10
   */
  public <T> T reportPropertyInputMismatch(
      Class<?> targetType, String propertyName, String msg, Object... msgArgs)
      throws JsonMappingException {
    msg = _format(msg, msgArgs);
   ...
   */
  public <T> T reportPropertyInputMismatch(
      JavaType targetType, String propertyName, String msg, Object... msgArgs)
      throws JsonMappingException {
    return reportPropertyInputMismatch(targetType.getRawClass(), propertyName, msg, msgArgs);
  }

  /**
   * Helper method used to indicate a problem with input in cases where specific input coercion was
   * not allowed.
   *
   * @since 2.12
   */
  public <T> T reportBadCoercion(
      JsonDeserializer<?> src,
      Class<?> targetType,
      Object inputValue,
      String msg,
      Object... msgArgs)
    ...
    InvalidFormatException e =
        InvalidFormatException.from(getParser(), msg, inputValue, targetType);
    throw e;
  }
  ...
  public <T> T reportTrailingTokens(Class<?> targetType, JsonParser p, JsonToken trailingToken)
    ...
    throw MismatchedInputException.from(
        ...
        targetType,
        String.format(
            "Trailing token (of type %s) found after value (bound as %s): not allowed as per"
  ...
  public void reportWrongTokenException(
      JsonParser p, JsonToken expToken, String msg, Object... msgArgs) throws JsonMappingException {
    msg = _format(msg, msgArgs);
  ...
  public void reportMissingContent(String msg, Object... msgArgs) throws JsonMappingException {
    throw MismatchedInputException.from(
        getParser(), (JavaType) null, "No content to map due to end-of-input");
  }
   ...
   * regarding specific Java type, unrelated to actual JSON content to map. Default behavior is to
   * construct and throw a {@link JsonMappingException}.
   *
   * @since 2.9
   */
  ...
  /**
   * Helper method called to indicate problem in POJO (serialization) definitions or settings
   * regarding specific property (of a type), unrelated to actual JSON content to map. Default
   * behavior is to construct and throw a {@link JsonMappingException}.
   *
   * @since 2.9
   */
  public <T> T reportBadPropertyDefinition(
      BeanDescription bean, BeanPropertyDefinition prop, String msg, Object... msgArgs)
      throws JsonMappingException {
    msg = _format(msg, msgArgs);
    ...
    msg =
        String.format(
            "Invalid definition for property %s (of type %s): %s", propName, beanDesc, msg);
  ...
  /**
   * Helper method for constructing {@link JsonMappingException} to indicate that the token
   * encountered was of type different than what <b>should</b> be seen at that position, usually
   * within a sequence of expected tokens. Note that most of the time this method should NOT be
   * directly called; instead, {@link #reportWrongTokenException} should be called and will call
   * this method as necessary.
   *
   * @since 2.9
   */
  public JsonMappingException wrongTokenException(
      JsonParser p, JavaType targetType, JsonToken expToken, String extra) {
  ...
  public JsonMappingException wrongTokenException(
      JsonParser p, Class<?> targetType, JsonToken expToken, String extra) {
    String msg = String.format("Unexpected token (%s), expected %s", p.currentToken(), expToken);
    msg = _colonConcat(msg, extra);
    return MismatchedInputException.from(p, targetType, msg);
  }
   ...
   * in format to be able to deserialize specified key type. Note that most of the time this method
   * should NOT be called; instead, {@link #handleWeirdKey} should be called which will call this
   * method if necessary.
   */
  ...
  /**
   * Helper method for constructing exception to indicate that input JSON String was not suitable
   * for deserializing into given target type. Note that most of the time this method should NOT be
   * called; instead, {@link #handleWeirdStringValue} should be called which will call this method
   * if necessary.
   *
   ...
   * @since 2.1
   */
  public JsonMappingException weirdStringException(
      String value, Class<?> instClass, String msgBase) {
    final String msg =
        String.format(
   ...
   * for deserializing into given target type. Note that most of the time this method should NOT be
   * called; instead, {@link #handleWeirdNumberValue} should be called which will call this method
   * if necessary.
   */
  ...
  /**
   * Helper method for constructing exception to indicate that input JSON token of type "native
   * value" (see {@link JsonToken#VALUE_EMBEDDED_OBJECT}) is of incompatible type (and there is no
   * delegating creator or such to use) and can not be used to construct value of specified type
   * (usually POJO). Note that most of the time this method should NOT be called; instead, {@link
   * #handleWeirdNativeValue} should be called which will call this method
   *
   * @since 2.9
   */
  public JsonMappingException weirdNativeValueException(Object value, Class<?> instClass) {
    return InvalidFormatException.from(
        _parser,
        String.format(
            "Cannot deserialize value of type %s from native value"
                + " (`JsonToken.VALUE_EMBEDDED_OBJECT`) of type %s: incompatible types",
   ...
   * constructor)
   *
   * <p>Note that most of the time this method should NOT be called directly; instead, {@link
   * #handleInstantiationProblem} should be called which will call this method if necessary.
   */
  public JsonMappingException instantiationException(Class<?> instClass, Throwable cause) {
    ...
    String msg =
        String.format(
            "Cannot construct instance of %s, problem: %s", ClassUtil.nameOf(instClass), excMsg);
  ...
  /**
   * Helper method for constructing instantiation exception for specified type, to indicate that
   * instantiation failed due to missing instantiator (creator; constructor or factory method).
   *
   * <p>Note that most of the time this method should NOT be called; instead, {@link
   * #handleMissingInstantiator} should be called which will call this method if necessary.
   */
  public JsonMappingException instantiationException(Class<?> instClass, String msg0) {
    ...
    return ValueInstantiationException.from(
        _parser,
        String.format("Cannot construct instance of %s: %s", ClassUtil.nameOf(instClass), msg0),
        constructType(instClass));
  }

  @Override
  public JsonMappingException invalidTypeIdException(
      JavaType baseType, String typeId, String extraDesc) {
    String msg =
        String.format(
  ...
  /**
   * Helper method for constructing exception to indicate that end-of-input was reached while still
   * expecting more tokens to deserialize value of specified type.
   *
  ...
  /**
   * Fallback method that may be called if no other <code>reportXxx</code> is applicable -- but only
   * in that case.
   *
   * @since 2.8
   * @deprecated Since 2.9: use a more specific method, or {@link #reportBadDefinition(JavaType,
   *     String)}, or {@link #reportInputMismatch} instead
   */
  @Deprecated // since 2.9
   ...
   * location information. Note that application code should almost always call one of <code>
   * handleXxx</code> methods, or {@link #reportMappingException(String, Object...)} instead.
   *
   * @since 2.6
  ...
  // @since 2.12
  /**
   * Helper method for constructing description like "Object value" given {@link JsonToken}
   * encountered.
   */
