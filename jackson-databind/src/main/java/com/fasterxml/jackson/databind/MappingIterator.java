...
/**
 * Iterator exposed by {@link ObjectMapper} when binding sequence of objects. Extension is done to
 * allow more convenient exposing of {@link IOException} (which basic {@link Iterator} does not
 * expose)
 */
public class MappingIterator<T> implements Iterator<T>, Closeable {
  ...
  /**
   * State in which "hasNextValue()" has been succesfully called and deserializer can be called to
   * fetch value
   */
  ...
  /**
   * Underlying parser used for reading content to bind. Initialized as not <code>null</code> but
   * set as <code>null</code> when iterator is closed, to denote closing.
   */
  protected final JsonParser _parser;

  /**
   * Context to resynchronize to, in case an exception is encountered but caller wants to try to
   * read more elements.
   */
  ...
  /**
   * Flag that indicates whether input {@link JsonParser} should be closed when we are done or not;
   * generally only called when caller did not pass JsonParser.
   */
  ...
  /**
   * @param managedParser Whether we "own" the {@link JsonParser} passed or not: if true, it was
   *     created by {@link ObjectReader} and code here needs to close it; if false, it was passed by
   *     calling code and should not be closed by iterator.
   */
  ...
  /**
   * Method for getting an "empty" iterator instance: one that never has more values; may be freely
   * shared.
   *
  ...
  /**
   * Equivalent of {@link #next} but one that may throw checked exceptions from Jackson due to
   * invalid input.
   */
  ...
  /**
   * Convenience method for reading all entries accessible via this iterator; resulting container
   * will be a {@link java.util.ArrayList}.
   *
  ...
  /**
   * Accessor for accessing {@link FormatSchema} that the underlying parser (as per {@link
   * #getParser}) is using, if any; only parser of schema-aware formats use schemas.
   *
   * @since 2.2
   */
  ...
  /**
   * Convenience method, functionally equivalent to: <code>
