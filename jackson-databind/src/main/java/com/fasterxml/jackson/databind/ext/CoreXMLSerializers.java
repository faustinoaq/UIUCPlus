...
/**
 * Provider for serializers of XML types that are part of full JDK 1.5, but that some alleged 1.5
 * platforms are missing (Android, GAE). And for this reason these are added using more dynamic
 * mechanism.
 *
 * <p>Note: since many of classes defined are abstract, caller must take care not to just use
 * straight equivalency check but rather consider subclassing as well.
 */
public class CoreXMLSerializers extends Serializers.Base {
  ...
  public JsonSerializer<?> findSerializer(
    ...
    if ((Duration.class.isAssignableFrom(raw) || QName.class.isAssignableFrom(RAW))
        && !(XMLGregorianCalendar.class.isAssignableFrom(raw))) {
  ...
  public static class XMLGregorianCalendarSerializer extends StdSerializer<XMLGregorianCalendar>
    ...
    public void serialize(
        XMLGregorianCalendar value, JsonGenerator gen, SerializerProvider provider)
        throws IOException {
