...
/**
 * Helper class that is used to flatten JSON structure when using "external type id" (see {@link
 * com.fasterxml.jackson.annotation.JsonTypeInfo.As#EXTERNAL_PROPERTY}). This is needed to store
 * temporary state and buffer tokens, as the structure is rearranged a bit so that actual type
 * deserializer can resolve type and finalize deserialization.
 */
public class ExternalTypeHandler {
  ...
  /**
   * Mapping from external property ids to one or more indexes; in most cases single index as <code>
   * Integer</code>, but occasionally same name maps to multiple ones: if so, <code>
   * List&lt;Integer&gt;</code>.
   */
   ...
   * This is <b>only</b> to be called in case containing POJO has similarly named property as the
   * external type id AND value is of scalar type: otherwise {@link #handlePropertyValue} should be
   * called instead.
   */
  @SuppressWarnings("unchecked")
  public boolean handleTypePropertyValue(
      JsonParser p, DeserializationContext ctxt, String propName, Object bean) throws IOException {
  ...
  private final boolean _handleTypePropertyValue(
      JsonParser p,
  ...
  /**
   * Method called to ask handler to handle value of given property, at point where parser points to
   * the first token of the value. Handling can mean either resolving type id it contains (if it
   * matches type property name), or by buffering the value for further use.
   *
   * @return True, if the given property was properly handled
   */
  @SuppressWarnings("unchecked")
  public boolean handlePropertyValue(
      JsonParser p, DeserializationContext ctxt, String propName, Object bean) throws IOException {
    Object ob = _nameToPropertyIndex.get(propName);
    if (ob == null) {
      return false;
  ...
  /**
   * Method called after JSON Object closes, and has to ensure that all external type ids have been
   * handled.
   */
  @SuppressWarnings("resource")
    ...
    for (int i = 0, len = _properties.length; i < len; ++i) {
      ...
      if (typeId == null) {
        ...
        if (!extProp.hasDefaultType()) {
          ctxt.reportPropertyInputMismatch(
              _beanType,
  ...
  /**
   * Variant called when creation of the POJO involves buffering of creator properties as well as
  ...
  @SuppressWarnings("resource")
  protected final Object _deserialize(
      JsonParser p, DeserializationContext ctxt, int index, String typeId) throws IOException {
    ...
    // Buggy Version 3
    JsonParser mp = _tokens[index].asParser(p);
  ...
  // 03-Aug-2022, tatu: [databind#3533] to handle absent value matching:
  @SuppressWarnings("resource")
  protected final Object _deserializeMissingToken(
      JsonParser p, DeserializationContext ctxt, int index, String typeId) throws IOException {
    TokenBuffer merged = ctxt.bufferForInputBuffering(p);
    merged.writeStartArray();
    merged.writeString(typeId);
  ...
  @SuppressWarnings("resource")
  protected final void _deserializeAndSet(
      JsonParser p, DeserializationContext ctxt, Object bean, int index, String typeId)
      throws IOException {
    // 11-Nov-2020, tatu: Should never get `null` passed this far,
    if (typeId == null) {
      ctxt.reportInputMismatch(
          _beanType, "Internal error in external Type Id handling: `null` type id passed");
    }
  ...
  public static class Builder {
    ...
    /**
     * Method called after all external properties have been assigned, to further link property with
     * polymorphic value with possible property for type id itself. This is needed to support type
     * ids as Creator properties.
     *
     * @since 2.8
     */
  ...
  private static final class ExtTypedProperty {
    ...
    /**
     * Specialized called when we need to expose type id of `defaultImpl` when serializing: we may
     * need to expose it for assignment to a property, or it may be requested as visible for some
     * other reason.
     */
