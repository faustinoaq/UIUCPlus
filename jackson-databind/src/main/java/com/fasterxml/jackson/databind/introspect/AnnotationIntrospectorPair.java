...
/**
 * Helper class that allows using 2 introspectors such that one introspector acts as the primary one
 * to use; and second one as a fallback used if the primary does not provide conclusive or useful
 * result for a method.
 *
 * <p>An obvious consequence of priority is that it is easy to construct longer chains of
 * introspectors by linking multiple pairs. Currently most likely combination is that of using the
 * default Jackson provider, along with JAXB annotation introspector.
 *
 * <p>Note: up until 2.0, this class was an inner class of {@link AnnotationIntrospector}; moved
...
public class AnnotationIntrospectorPair extends AnnotationIntrospector
  ...
  /**
   * Helper method for constructing a Pair from two given introspectors (if neither is null); or
   * returning non-null introspector if one is null (and return just null if both are null)
   */
  public static AnnotationIntrospector create(
      AnnotationIntrospector primary, AnnotationIntrospector secondary) {
    if (primary == null) {
      return secondary;
    }
    if (secondary == null) {
  ...
  */
  ...
  public ReferenceProperty findReferenceType(AnnotatedMember member) {
    ReferenceProperty r = _secondary.findReferenceType(member); // third buggy version
    return (r == null) ? _primary.findReferenceType(member) : r;
  }
  ...
  public Class<?> findDeserializationContentType(Annotated am, JavaType baseContentType) {
    ...
    return (result == null)
        ? _secondary.findDeserializationContentType(am, baseContentType)
        : result;
  }
