...
/**
 * Base class for deserializable properties of a bean: contains both type and name definitions, and
 * reflection-based set functionality. Concrete sub-classes implement details, so that field- and
 * setter-backed properties, as well as a few more esoteric variations, can be handled.
 */
@SuppressWarnings("serial")
public abstract class SettableBeanProperty extends ConcreteBeanPropertyBase
    implements java.io.Serializable {
  /**
   * To avoid nasty NPEs, let's use a placeholder for _valueDeserializer, if real deserializer is
   * not (yet) available.
   *
   * @since 2.2
   */
  ...
  /**
   * Class that contains this property (either class that declares the property or one of its
   * subclasses), class that is deserialized using deserializer that contains this property.
   */
   ...
   *
   * <p>NOTE: has been immutable since 2.3
   */
  protected final JsonDeserializer<Object> _valueDeserializer;

  /**
   * If value will contain type information (to support polymorphic handling), this is the type
   * deserializer used to handle type resolution.
   */
  ...
  /**
   * If property represents a managed (forward) reference, we will need the name of reference for
   * later linking.
   *
   * <p>TODO: should try to make immutable.
   */
  ...
  /**
   * Helper object used for checking whether this property is to be included in the active view, if
   * property is view-specific; null otherwise.
   *
   * <p>TODO: should try to make immutable.
   */
  ...
  /**
   * Index of property (within all property of a bean); assigned when all properties have been
   * collected. Order of entries is arbitrary, but once indexes are assigned they are not changed.
   *
   * <p>TODO: should try to make immutable if at all possible
   */
  ...
  /**
   * Fluent factory method for constructing and returning a new instance with specified value
   * deserializer. Note that this method should NOT change configuration of this instance.
   *
   * @param deser Deserializer to assign to the new property instance
   * @return Newly constructed instance, if value deserializer differs from the one used for this
   *     instance; or 'this' if not.
   */
  public abstract SettableBeanProperty withValueDeserializer(JsonDeserializer<?> deser);

  /**
   * Fluent factory method for constructing and returning a new instance with specified property
   * name. Note that this method should NOT change configuration of this instance.
   *
   * @param newName Name to use for the new instance.
   * @return Newly constructed instance, if property name differs from the one used for this
   *     instance; or 'this' if not.
   */
  ...
  /**
   * Method called to ensure that the mutator has proper access rights to be called, as per
   * configuration. Overridden by implementations that have mutators that require access, fields and
   * setters.
   *
   * @since 2.8.3
   */
  ...
  /**
   * Method for accessing unique index of this property; indexes are assigned once all properties of
   * a {@link BeanDeserializer} have been collected.
   *
  ...
  /**
   * Method for accessing index of the creator property: for other types of properties will simply
   * return -1.
   *
   * @since 2.1
   */
  ...
  /**
   * Accessor for checking whether this property is injectable, and if so, ONLY injectable (will not
   * bind from input). Currently (2.11) can only return {@code true} for Creator-backed properties.
   *
   * @return True if (and only if) property has injector that is also defined NOT to bind from
   *     input.
   * @since 2.11
   */
  public boolean isInjectionOnly() {
    return false;
  } // overridden by CreatorProperty
  ...
  /**
   * Method called to deserialize appropriate value, given parser (and context), and set it using
   * appropriate mechanism. Pre-condition is that passed parser must point to the first token that
   * should be consumed to produce the value (the only value for scalars, multiple for Objects and
   * Arrays).
   */
  public abstract void deserializeAndSet(JsonParser p, DeserializationContext ctxt, Object instance)
      throws IOException;

  /**
   * Alternative to {@link #deserializeAndSet} that returns either return value of setter method
   * called (if one is), or null to indicate that no return value is available. Mostly used to
   * support Builder style deserialization.
   *
   * @since 2.0
   */
  public abstract Object deserializeSetAndReturn(
      JsonParser p, SerializationContext ctxt, Object instance) throws IOException;

  /**
   * Method called to assign given value to this property, on specified Object.
   *
   * <p>Note: this is an optional operation, not supported by all implementations, creator-backed
   * properties for example do not support this method.
   */
  public abstract void set(Object instance, Object value) throws IOException;

  /**
   * Method called to assign given value to this property, on specified Object, and return whatever
   * delegating accessor returned (if anything)
   *
   * <p>Note: this is an optional operation, not supported by all implementations, creator-backed
   * properties for example do not support this method.
   */
  ...
  public final Object deserializeWith(JsonParser p, DeserializationContext ctxt, Object toUpdate)
    ...
    if (_valueTypeDeserializer != null) {
      ...
      // Please note you need to configure {@link DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES}
      // as false to
      ...
      JsonDeserializer<Object> subTypeValueDeserializer =
          ctxt.findContextualValueDeserializer(subType, this);
  ...
  /**
   * Method that takes in exception of any type, and casts or wraps it to an IOException or its
   * subclass.
   */
  ...
  /**
   * Helper class that is designed to both make it easier to sub-class delegating subtypes and to
   * reduce likelihood of breakage when new methods are added.
   *
   * <p>Class was specifically added to help with {@code Afterburner} module, but its use is not
   * limited to only support it.
   *
   * @since 2.9
   */
  public abstract static class Delegating extends SettableBeanProperty {
    ...
    public Object deserializeSetAndReturn(
        JsonParser p, DeserializationContext ctxt, Object instance) throws IOException {
