        ...
        java.io.Serializable // since 2.1
{
  ...
  /**
   * Object that handles details of constructing initial bean value (to which bind data to), unless
   * instance is passed (via updateValue())
   */
  protected final ValueInstantiator _valueInstantiator;

  /**
   * Deserializer that is used iff delegate-based creator is to be used for deserializing from JSON
   * Object.
   *
   * <p>NOTE: cannot be {@code final} because we need to get it during {@code resolve()} method (and
   * not contextualization).
   */
  protected JsonDeserializer<Object> _delegateDeserializer;

  /**
   * Deserializer that is used iff array-delegate-based creator is to be used for deserializing from
   * JSON Object.
   *
   * <p>NOTE: cannot be {@code final} because we need to get it during {@code resolve()} method (and
   * not contextualization).
  ...
  /**
   * If the bean needs to be instantiated using constructor or factory method that takes one or more
   * named properties as argument(s), this creator is used for instantiation. This value gets
   * resolved during general resolution.
   */
  protected PropertyBasedCreator _propertyBasedCreator;

  /**
   * Flag that is set to mark cases where deserialization from Object value using otherwise
   * "standard" property binding will need to use non-default creation method: namely, either "full"
   * delegation (array-delegation does not apply), or properties-based Creator method is used.
   *
   * <p>Note that flag is somewhat mis-named as it is not affected by scalar-delegating creators; it
   * only has effect on Object Value binding.
   */
  protected boolean _nonStandardCreation;

  /**
   * Flag that indicates that no "special features" whatsoever are enabled, so the simplest
  ...
  /**
   * Mapping of property names to properties, built when all properties to use have been
   * successfully resolved.
   */
  protected final BeanPropertyMap _beanProperties;

  /**
   * List of {@link ValueInjector}s, if any injectable values are expected by the bean; otherwise
   * null. This includes injectors used for injecting values via setters and fields, but not ones
   * passed through constructor parameters.
   */
  protected final ValueInjector[] _injectables;

  /**
   * Fallback setter used for handling any properties that are not mapped to regular setters. If
   * setter is not null, it will be called once for each such property.
   */
  protected SettableAnyProperty _anySetter;

  /**
   * In addition to properties that are set, we will also keep track of recognized but ignorable
   * properties: these will be skipped without errors or warnings.
   */
  ...
  /**
   * Flag that can be set to ignore and skip unknown properties. If set, will not throw an exception
  ...
  /**
   * Lazily constructed map used to contain deserializers needed for polymorphic subtypes. Note that
   * this is <b>only needed</b> for polymorphic types, that is, when the actual type is not
   * statically known. For other types this remains null.
   */
  ...
  /**
   * If an Object Id is to be used for value handled by this deserializer, this reader is used for
   * handling.
   */
  ...
  /**
   * Constructor used when initially building a deserializer instance, given a {@link
   * BeanDeserializerBuilder} that contains configuration.
   */
  protected BeanDeserializerBase(
    ...
    _injectables =
        (injectables == null || injectables.isEmpty())
            ? null
  ...
  public BeanDeserializerBase(
      BeanDeserializerBase src, Set<String> ignorableProps, Set<String> includableProps) {
    super(src._beanType);
    _beanType = src._beanType;
   ...
   * @since 2.12
   */
  public abstract BeanDeserializerBase withByNameInclusion(
   ...
   * more drastic backwards compatibility problems.
   *
   * @since 2.8
   */
   ...
   * Implementations may ignore this request if no such input is possible.
   *
   * @since 2.1
   */
  ...
  public void resolve(DeserializationContext ctxt) throws JsonMappingException {
    ...
    if (_valueInstantiator.canCreateFromObjectWith()) {
      ...
      if (_ignorableProps != null || _includableProps != null) {
        for (int i = 0, end = creatorProps.length; i < end; ++i) {
          ...
          if (IgnorePropertiesUtil.shouldIgnore(
              prop.getName(), _ignorableProps, _includableProps)) {
    ...
    if ((_anySetter != null) && !_anySetter.hasValueDeserializer()) {
      _anySetter =
          _anySetter.withValueDeserializer(
              findDeserializer(ctxt, _anySetter.getType(), _anySetter.getProperty()));
    }
    ...
    if (_valueInstantiator.canCreateUsingDelegate()) {
      ...
      if (delegateType == null) {
        ctxt.reportBadDefinition(
            _beanType,
            String.format(
                "Invalid delegate-creator definition for %s: value instantiator (%s) returned true"
                    + " for 'canCreateUsingDelegate()', but null for 'getDelegateType()'",
                ClassUtil.getTypeDescription(_beanType),
                ClassUtil.classNameOf(_valueInstantiator)));
      }
      _delegateDeserializer =
          _findDelegateDeserializer(ctxt, delegateType, _valueInstantiator.getDelegateCreator());
    }
    ...
    if (_valueInstantiator.canCreateUsingArrayDelegate()) {
      ...
      if (delegateType == null) {
        ctxt.reportBadDefinition(
            ...
            String.format(
                "Invalid delegate-creator definition for %s: value instantiator (%s) returned true"
                    + " for 'canCreateUsingArrayDelegate()', but null for 'getArrayDelegateType()'",
                ClassUtil.getTypeDescription(_beanType),
                ClassUtil.classNameOf(_valueInstantiator)));
      }
      _arrayDelegateDeserializer =
          _findDelegateDeserializer(
              ctxt, delegateType, _valueInstantiator.getArrayDelegateCreator());
    }

    // And now that we know CreatorProperty instances are also resolved can finally create the
    // creator:
    if (creatorProps != null) {
      _propertyBasedCreator =
          PropertyBasedCreator.construct(ctxt, _valueInstantiator, creatorProps, _beanProperties);
    }
   ...
   * @since 2.8.8
   */
  protected void _replaceProperty(
  ...
  private JsonDeserializer<Object> _findDelegateDeserializer(
      DeserializationContext ctxt, JavaType delegateType, AnnotatedWithParams delegateCreator)
      throws JsonMappingException {
    // Need to create a temporary property to allow contextual deserializers:
    BeanProperty.Std property =
        new BeanProperty.Std(
            TEMP_PROPERTY_NAME, delegateType, null, delegateCreator, PropertyMetadata.STD_OPTIONAL);
    ...
    if (dd == null) {
      ...
      dd =
          (JsonDeserializer<Object>)
              ctxt.handleSecondaryContextualization(dd, property, delegateType);
    }
    if (td != null) {
  ...
  /**
   * Helper method that can be used to see if specified property is annotated to indicate use of a
   * converter for property value (in case of container types, it is container type itself, not key
   * or content type).
   *
   * <p>NOTE: returned deserializer is NOT yet contextualized, caller needs to take care to do that.
   *
   * @since 2.2
   */
  protected JsonDeserializer<Object> findConvertingDeserializer(
      DeserializationContext ctxt, SettableBeanProperty prop) throws JsonMappingException {
    ...
    if (intr != null) {
      ...
      if (convDef != null) {
        ...
        if (delegateType.getRawClass().getName().contains("java.lang.Boolean")) {
          deser = ctxt.findRootValueDeserializer(ctxt.constructType(String.class));
        } else {
          deser = ctxt.findNonContextualValueDeserializer(delegateType);
        }
  ...
  /**
   * Although most of post-processing is done in resolve(), we only get access to referring
   * property's annotations here; and this is needed to support per-property ObjectIds. We will also
   * consider Shape transformations (read from Array) at this point, since it may come from either
  ...
  public JsonDeserializer<?> createContextual(DeserializationContext ctxt, BeanProperty property)
    ...
    if (accessor != null) {
      ...
      if (objectIdInfo
          != null) { // some code duplication here as well (from BeanDeserializerFactory)
  ...
  // @since 2.12
  protected BeanDeserializerBase _handleByNameInclusion(
    ...
    final Set<String> newNamesToInclude =
        IgnorePropertiesUtil.combineNamesToInclude(
            prevNamesToInclude, intr.findPropertyInclusionByName(config, accessor).getIncluded());
   ...
   * back reference), and if so, handle resolution details.
   */
  protected SettableBeanProperty _resolveManagedReferenceProperty(
      DeserializationContext ctxt, SettableBeanProperty prop) throws JsonMappingException {
    ...
    if (backProp == null) {
      return ctxt.reportBadDefinition(
          _beanType,
          String.format(
              "Cannot handle managed/back reference %s: no back reference property found from type"
                  + " %s",
    ...
    if (!backRefType.getRawClass().isAssignableFrom(referredType.getRawClass())) {
      ctxt.reportBadDefinition(
          _beanType,
          String.format(
              "Cannot handle managed/back reference %s: back reference type (%s) not compatible"
                  + " with managed type (%s)",
              ClassUtil.name(refName),
              ClassUtil.getTypeDescription(backRefType),
  ...
  /**
   * Method that wraps given property with {@link ObjectIdReferenceProperty} in case where object id
   * resolution is required.
   */
  protected SettableBeanProperty _resolvedObjectIdProperty(
      DeserializationContext ctxt, SettableBeanProperty prop) throws JsonMappingException {
  ...
  /**
   * Helper method called to see if given property might be so-called unwrapped property: these
   * require special handling.
   */
  protected NameTransformer _findPropertyUnwrapper(
      DeserializationContext ctxt, SettableBeanProperty prop) throws JsonMappingException {
    AnnotatedMember am = prop.getMember();
    if (am != null) {
      NameTransformer unwrapper =
          ctxt.getAnnotationIntrospector().findUnwrappingNameTransformer(am);
      if (unwrapper != null) {
        ...
        if (prop instanceof CreatorProperty) {
          ctxt.reportBadDefinition(
              getValueType(),
              String.format(
                  "Cannot define Creator property \"%s\" as `@JsonUnwrapped`: combination not yet"
                      + " supported",
                  prop.getName()));
        }
        return unwrapper;
  ...
  /**
   * Helper method that will handle gruesome details of dealing with properties that have non-static
   * inner class as value...
   */
  protected SettableBeanProperty _resolveInnerClassValuedProperty(
              ...
              if (enclosing.equals(paramTypes[0])) {
                if (ctxt.canOverrideAccessModifiers()) {
                  ClassUtil.checkAndFixAccess(
                      ctor, ctxt.isEnabled(MapperFeature.OVERRIDE_PUBLIC_ACCESS_MODIFIERS));
                }
  ...
  // @since 2.9
  protected SettableBeanProperty _resolveMergeAndNullSettings(
   ...
   *
   * @return True if this deserializer should match property names without considering casing; false
   *     if case has to match exactly.
   * @since 2.12
   */
  ...
  /**
   * Overridden to return true for those instances that are handling value for which Object Identity
   * handling is enabled (either via value type or referring property).
   */
  @Override
  ...
  /**
   * Accessor for iterating over properties this deserializer uses; with the exception that
   * properties passed via Creator methods (specifically, "property-based constructor") are not
   * included, but can be accessed separate by calling {@link #creatorProperties}
   */
  ...
  /**
   * Accessor for finding properties that represents values to pass through property-based creator
   * method (constructor or factory method)
   *
   * @since 2.0
   */
  ...
  /**
   * Accessor for finding the property with given name, if POJO has one. Name used is the external
   * name, i.e. name used in external data representation (JSON).
   *
   * @since 2.0
   */
  public SettableBeanProperty findProperty(String propertyName) {
    SettableBeanProperty prop =
        (_beanProperties == null) ? null : _beanProperties.find(propertyName);
   ...
   * Note that access by index is not necessarily faster than by name, since properties are not
   * directly indexable; however, for most instances difference is not significant as number of
   * properties is low.
   *
   * @since 2.3
   */
  public SettableBeanProperty findProperty(int propertyIndex) {
    SettableBeanProperty prop =
        (_beanProperties == null) ? null : _beanProperties.find(propertyIndex);
    if (prop == null && _propertyBasedCreator != null) {
  ...
  /**
   * Method needed by {@link BeanDeserializerFactory} to properly link managed- and back-reference
   * pairs.
   */
  @Override
   ...
   *
   * <p>NOTE: only ever use this method if you know what you are doing; incorrect usage can break
   * deserializer.
   *
  ...
  /**
   * Offlined method called to handle "native" Object Id that has been read and known to be
   * associated with given deserialized POJO.
   *
   * @since 2.3
   */
  protected Object _handleTypedObjectId(
      JsonParser p, DeserializationContext ctxt, Object pojo, Object rawId) throws IOException {
  ...
  /**
   * Helper method we need to do necessary conversion from whatever native object id type is, into
   * declared type that Jackson internals expect. This may be simple cast (for String ids), or
   * something more complicated; in latter case we may need to create bogus content buffer to allow
   * use of id deserializer.
   *
   * @since 2.3
   */
  @SuppressWarnings("resource") // TokenBuffers don't need close, nor parser thereof
  protected Object _convertObjectId(
      JsonParser p, DeserializationContext ctxt, Object rawId, JsonDeserializer<Object> idDeser)
      throws IOException {
  ...
  /**
   * Alternative deserialization method used when we expect to see Object Id; if so, we will need to
   * ensure that the Id is seen before anything else, to ensure that it is available for solving
   * references, even if JSON itself is not ordered that way. This may require buffering in some
   * cases, but usually just a simple lookup to ensure that ordering is correct.
   */
  ...
  protected Object deserializeFromObjectUsingNonDefault(JsonParser p, DeserializationContext ctxt)
    ...
    if (delegateDeser != null) {
      final Object bean =
          _valueInstantiator.createUsingDelegate(ctxt, delegateDeser.deserialize(p, ctxt));
    ...
    if (ClassUtil.isNonStaticInnerClass(raw)) {
      return ctxt.handleMissingInstantiator(
          ...
          null,
          p,
          "non-static inner classes like this can only by instantiated using default, no-argument"
              + " constructor");
    }
    ...
    if (NativeImageUtil.needsReflectionConfiguration(raw)) {
      return ctxt.handleMissingInstantiator(
          ...
          null,
          p,
          "cannot deserialize from Object value (no delegate- or property-based Creator): this"
              + " appears to be a native image, in which case you may need to configure reflection"
  ...
  protected abstract Object _deserializeUsingPropertyBased(
  ...
  /**
   * @deprecated Since 2.11 Should not be used: was never meant to be called by code other than
   *     sub-classes (implementations), and implementations details differ
   */
  @Deprecated
  ...
  protected Object handleUnknownProperties(
      DeserializationContext ctxt, Object bean, TokenBuffer unknownTokens) throws IOException {
   ...
   *
   * @param beanOrBuilder Either POJO instance (if constructed), or builder (in case of
   *     builder-based approach), that has property we haven't been able to handle yet.
   */
  protected void handleUnknownVanilla(
      JsonParser p, DeserializationContext ctxt, Object beanOrBuilder, String propName)
      throws IOException {
  ...
  /**
   * Method called when a JSON property is encountered that has not matching setter, any-setter or
   * field, and thus cannot be assigned.
   */
  @Override
  protected void handleUnknownProperty(
      JsonParser p, DeserializationContext ctxt, Object beanOrClass, String propName)
      throws IOException {
  ...
  /**
   * Method called when an explicitly ignored property (one specified with a name to match, either
   * by property annotation or class annotation) is encountered.
   *
   * @since 2.3
   */
  protected void handleIgnoredProperty(
      JsonParser p, DeserializationContext ctxt, Object beanOrClass, String propName)
      throws IOException {
  ...
  /**
   * Method called in cases where we may have polymorphic deserialization case: that is, type of
   * Creator-constructed bean is not the type of deserializer itself. It should be a sub-class or
   * implementation class; either way, we may have more specific deserializer to use for handling
   * it.
   *
   * @param p (optional) If not null, parser that has more properties to handle (in addition to
   *     buffered properties); if null, all properties are passed in buffer
   */
  ...
  /**
   * Helper method called to (try to) locate deserializer for given sub-type of type that this
   * deserializer handles.
   */
  protected JsonDeserializer<Object> _findSubclassDeserializer(
      DeserializationContext ctxt, Object bean, TokenBuffer unknownTokens) throws IOException {
    ...
    synchronized (this) {
      subDeser =
          (_subDeserializers == null) ? null : _subDeserializers.get(new ClassKey(bean.getClass()));
    }
    if (subDeser != null) {
      return subDeser;
    }
  ...
  /**
   * Method that will modify caught exception (passed in as argument) as necessary to include
   * reference information, and to ensure it is a subtype of {@link IOException}, or an unchecked
   * exception.
   *
   * <p>Rules for wrapping and unwrapping are bit complicated; essentially:
   *
   * <ul>
   *   <li>Errors are to be passed as is (if uncovered via unwrapping)
   *   <li>"Plain" IOExceptions (ones that are not of type {@link JsonMappingException} are to be
   *       passed as is
   * </ul>
   */
