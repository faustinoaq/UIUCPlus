...
public final class ClassUtil {
   ...
   * Classes are listed in order of precedence, starting with the immediate super-class, followed by
   * interfaces class directly declares to implemented, and then recursively followed by parent of
   * super-class and so forth. Note that <code>Object.class</code> is not included in the list
   * regardless of whether <code>endBefore</code> argument is defined or not.
   *
   * @param endBefore Super-type to NOT include in results, if any; when encountered, will be
   *     ignored (and no super types are checked).
   * @since 2.7
   */
  public static List<JavaType> findSuperTypes(
      JavaType type, Class<?> endBefore, boolean addClassItself) {
   ...
   * @since 2.7
   */
  public static List<Class<?>> findRawSuperTypes(
      Class<?> cls, Class<?> endBefore, boolean addClassItself) {
  ...
  /**
   * Method for finding all super classes (but not super interfaces) of given class, starting with
   * the immediate super class and ending in the most distant one. Class itself is included if
   * <code>addClassItself</code> is true.
   *
   * <p>NOTE: mostly/only called to resolve mix-ins as that's where we do not care about
   * fully-resolved types, just associated annotations.
   *
   * @since 2.7
   */
  public static List<Class<?>> findSuperClasses(
      Class<?> cls, Class<?> endBefore, boolean addClassItself) {
    List<Class<?>> result = new ArrayList<Class<?>>(8);
  ...
  @Deprecated // since 2.7
  public static List<Class<?>> findSuperTypes(
      Class<?> cls, Class<?> endBefore, List<Class<?>> result) {
    _addRawSuperTypes(cls, endBefore, result, false);
    return result;
  }

  private static void _addSuperTypes(
      JavaType type, Class<?> endBefore, Collection<JavaType> result, boolean addClassItself) {
    if (type == null) {
      return;
    }
  ...
  private static void _addRawSuperTypes(
  ...
  /**
   * Helper method that will check if argument is an {@link Error}, and if so, (re)throw it;
  ...
  /**
   * Helper method that will check if argument is an {@link RuntimeException}, and if so, (re)throw
   * it; otherwise just return
   *
   * @since 2.9
   */
  ...
  /**
   * Helper method that will check if argument is an {@link IOException}, and if so, (re)throw it;
   * otherwise just return
   *
   * @since 2.9
   */
  ...
  /**
   * Method that works like by calling {@link #getRootCause} and then either throwing it (if
   * instanceof {@link IOException}), or return.
   *
   * @since 2.8
   */
  ...
  /**
   * Method that will wrap 't' as an {@link IllegalArgumentException} (and with specified message)
   ...
   * @since 2.9
   */
  public static <T> T throwAsMappingException(DeserializationContext ctxt, IOException e0)
   ...
   * {@link IllegalArgumentException} if it is a checked exception; otherwise (runtime exception or
   * error) throw as is
   */
  ...
  /**
   * Helper method that encapsulate logic in trying to close output generator in case of failure;
   * useful mostly in forcing flush()ing as otherwise error conditions tend to be hard to diagnose.
   * However, it is often the case that output state may be corrupt so we need to be prepared for
  ...
  /**
   * Helper method that encapsulate logic in trying to close given {@link Closeable} in case of
   * failure; useful mostly in forcing flush()ing as otherwise error conditions tend to be hard to
   * diagnose. However, it is often the case that output state may be corrupt so we need to be
   * prepared for secondary exception without masking original one.
   *
   * @since 2.8
   */
  public static void closeOnFailAndThrowAsIOE(JsonGenerator g, Closeable toClose, Exception fail)
      throws IOException {
    if (g != null) {
      g.disable(JsonGenerator.Feature.AUTO_CLOSE_JSON_CONTENT);
      try {
        ...
        if (!Modifier.isPublic(ctor.getModifiers())) {
          throw new IllegalArgumentException(
              "Default constructor for "
                  + cls.getName()
                  + " is not accessible (non-public?): not allowed to try modify access via"
                  + " Reflection: cannot instantiate type");
        }
      }
      return ctor;
  ...
  /**
   * Returns either quoted value (with double-quotes) -- if argument non-null String -- or String
   * NULL (no quotes) (if null).
   *
   * @since 2.9
   */
  ...
  /**
   * Helper method used to construct appropriate description when passed either type (Class) or an
   * instance; in latter case, class of instance is to be used.
   */
  public static String getClassDescription(Object classOrInstance) {
    ...
    Class<?> cls =
        (classOrInstance instanceof Class<?>)
  ...
  /**
   * Helper method to create and return "backticked" description of given resolved type (or, {@code
   * "null"} if {@code null} passed), similar to return vaue of {@link
   * #getClassDescription(Object)}.
   *
   * @param fullType Fully resolved type or null
   * @return String description of type including generic type parameters, surrounded by backticks,
   *     if type passed; or string "null" if {code null} passed
   * @since 2.10
   */
  ...
  /**
   * Returns either single-quoted (apostrophe) {@code 'named.getName()'} (if {@code named} not
   * null), or "[null]" if {@code named} is null.
   *
   * <p>NOTE: before 2.12 returned "backticked" version instead of single-quoted name; changed to be
   * compatible with most existing quoting usage within databind
   *
   * @since 2.9
   */
  ...
  /**
   * Returns either single-quoted (apostrophe) {@code 'name'} (if {@code name} not null), or
   * "[null]" if {@code name} is null.
   *
   * @since 2.12
   */
  ...
  /**
   * Helper method that returns {@link Throwable#getMessage()} for all other exceptions except for
   * (a) {@link JacksonException}, for which {@code getOriginalMessage()} is returned, and (b)
   * {@link InvocationTargetException}, for which the cause's message is returned, if available.
   * Method is used to avoid accidentally including trailing location information twice in message
   ...
   * NOTE: throws {@link IllegalArgumentException} if given type is NOT primitive type (caller has
   * to check).
   */
  public static Class<?> wrapperType(Class<?> primitiveType) {
    ...
    throw new IllegalArgumentException(
        "Class " + primitiveType.getName() + " is not a primitive type");
  }

  /**
   * Method that can be used to find primitive type for given class if (but only if) it is either
   * wrapper type or primitive type; returns {@code null} if type is neither.
   *
   * @since 2.7
   */
  ...
  /**
   * Method that is called if a {@link Member} may need forced access, to force a field, method or
   * constructor to be accessible: this is done by calling {@link
   * AccessibleObject#setAccessible(boolean)}.
   *
   * @param member Accessor to call <code>setAccessible()</code> on.
   * @param evenIfAlreadyPublic Whether to always try to make accessor accessible, even if {@code
   *     public} (true), or only if needed to force by-pass of non-{@code public} access (false)
   * @since 2.7
   */
  ...
  /**
   * Helper method that encapsulates reliable check on whether given raw type "is an Enum", that is,
   * is or extends {@link java.lang.Enum}.
   *
   * @since 2.10.1
   */
   ...
   *
   * <p>If there's more than one value annotated, the first one found will be returned. Which one
   * exactly is used is undetermined.
   *
   ...
   * @since 2.8
   */
  public static <T extends Annotation> Enum<?> findFirstAnnotatedEnumValue(
      Class<Enum<?>> enumClass, Class<T> annotationClass) {
  ...
  /**
   * Method that can be called to determine if given Object is the default implementation Jackson
   * uses; as opposed to a custom serializer installed by a module or calling application.
   * Determination is done using {@link JacksonStdImpl} annotation on handler (serializer,
   * deserializer etc) class.
   *
   * <p>NOTE: passing `null` is legal, and will result in <code>true</code> being returned.
   */
  ...
  /**
   * Accessor for checking whether given {@code Class} is under Java package of {@code java.*} or
   * {@code javax.*} (including all sub-packages).
   *
   * <p>Added since some aspects of handling need to be changed for JDK types (and possibly some
   * extensions under {@code javax.}?): for example, forcing of access will not work well for future
   * JDKs (12 and later).
   *
   * <p>Note: in Jackson 2.11 only returned true for {@code java.*} (and not {@code javax.*}); was
   * changed in 2.12.
   *
   * @since 2.11
   */
   ...
   *
   * @return {@code True} if we can determine that the code is running on JDK 17 or above; {@code
   *     false} otherwise.
   * @since 2.15
   */
  public static boolean isJDK17OrAbove() {
    try {
      ...
      System.err.println(
          "Failed to determine JDK major version, assuming pre-JDK-17; problem: " + t);
      return false;
    }
  }
  ...
  public static int getJDKMajorVersion() {
    ...
    try {
      ...
      throw new IllegalStateException(
          "Could not access 'java.version': cannot determine JDK major version");
    }
    ...
    try {
      ...
      throw new IllegalStateException(
          "Invalid JDK version String '" + version + "' cannot determine JDK major version");
    }
  }
  ...
  /**
   * Helper method that gets methods declared in given class; usually a simple thing, but sometimes
   * (as per [databind#785]) more complicated, depending on classloader setup.
   *
   * @since 2.9
   */
  ...
  /**
   * Inner class used to contain gory details of how we can determine details of instances of common
   * JDK types like {@link EnumMap}s.
   */
  private static class EnumTypeLocator {
    ...
    public Class<? extends Enum<?>> enumTypeFor(EnumSet<?> set) {
      ...
      throw new IllegalStateException(
          "Cannot figure out type parameter for `EnumSet` (odd JDK platform?), problem: "
              + failForEnumSet);
    }
    ...
    public Class<? extends Enum<?>> enumTypeFor(EnumMap<?, ?> set) {
      ...
      throw new IllegalStateException(
          "Cannot figure out type parameter for `EnumMap` (odd JDK platform?), problem: "
              + failForEnumMap);
    }
    ...
    private static Field locateField(Class<?> fromClass, String expectedName, Class<?> type)
      ...
      throw new IllegalStateException(
          String.format("No field named '%s' in class '%s'", expectedName, fromClass.getName()));
    }
  }
  ...
  /**
   * Value class used for caching Constructor declarations; used because caching done by JDK appears
   * to be somewhat inefficient for some use cases.
   *
   * @since 2.7
   */
