...
public abstract class TypeDeserializerBase extends TypeDeserializer
  ...
  /**
   * Property that contains value for which type information is included; null if value is a root
   * value. Note that this value is not assigned during construction but only when {@link
   * #forProperty} is called to create a copy.
   */
  ...
  /**
   * Name of type property used; needed for non-property versions too, in cases where type id is to
   * be exposed as part of JSON.
   */
  ...
  /**
   * For efficient operation we will lazily build mappings from type ids to actual deserializers,
   * once needed.
   */
   ...
   * @since 2.8
   */
  protected TypeDeserializerBase(
      JavaType baseType,
  ...
  protected final JsonDeserializer<Object> _findDeserializer(
      DeserializationContext ctxt, String typeId) throws IOException {
    ...
    if (deser == null) {
      ...
      if (type == null) {
        ...
        if ((_baseType != null) && _baseType.getClass() == type.getClass()) {
          ...
          if ((type != this._defaultImpl) && (++type.hasGenericTypes())) {
  ...
  /**
   * Helper method called when {@link JsonParser} indicates that it can use so-called native type
   * ids. Assumption from there is that only native type ids are to be used.
   *
   * @since 2.3
   */
  @Deprecated
  ...
  /**
   * Helper method called when {@link JsonParser} indicates that it can use so-called native type
   * ids, and such type id has been found.
   *
   * @since 2.4
   */
  protected Object _deserializeWithNativeTypeId(
      JsonParser p, DeserializationContext ctxt, Object typeId) throws IOException {
  ...
  /**
   * Helper method called when given type id cannot be resolved into concrete deserializer either
   * directly (using given {@link TypeIdResolver}), or using default type. Default implementation
   * simply throws a {@link com.fasterxml.jackson.databind.JsonMappingException} to indicate the
   * problem; sub-classes may choose
   *
   * @return If it is possible to resolve type id into a {@link JsonDeserializer} should return that
   *     deserializer; otherwise throw an exception to indicate the problem.
   * @since 2.8
   */
