...
/**
 * Writer class similar to {@link ObjectWriter}, except that it can be used for writing sequences of
 * values, not just a single value. The main use case is in writing very long sequences, or
 * sequences where values are incrementally produced; cases where it would be impractical or at
 * least inconvenient to construct a wrapper container around values (or where no JSON array is
 * desired around values).
 *
 * <p>Differences from {@link ObjectWriter} include:
 *
 * <ul>
 *   <li>Instances of {@link SequenceWriter} are stateful, and not thread-safe: if sharing, external
 *       synchronization must be used.
 *   <li>Explicit {@link #close} is needed after all values have been written ({@link ObjectWriter}
...
public class SequenceWriter implements Versioned, java.io.Closeable, java.io.Flushable {
  ...
  /**
   * If {@link #_rootSerializer} is not defined (no root type was used for constructing {@link
   * ObjectWriter}), we will use simple scheme for keeping track of serializers needed. Assumption
   * is that
   */
  protected PropertySerializerMap _dynamicSerializers;

  /**
   * State flag for keeping track of need to write matching END_ARRAY, if a START_ARRAY was written
  ...
  /**
   * Internal method called by {@link ObjectWriter}: should not be called by code outside {@code
   * jackson-databind} classes.
   */
  ...
  /**
   * Method for writing given value into output, as part of sequence to write. If root type was
   * specified for {@link ObjectWriter}, value must be of compatible type (same or subtype).
   */
  ...
  /**
   * Method for writing given value into output, as part of sequence to write; further, full type
   * (often generic, like {@link java.util.Map} is passed in case a new {@link JsonSerializer} needs
   * to be fetched to handle type
   *
   * <p>If root type was specified for {@link ObjectWriter}, value must be of compatible type (same
   * or subtype).
   */
  ...
  private final JsonSerializer<Object> _findAndAddDynamic(Class<?> type)
    ...
    if (_typeSerializer == null) {
      ...
      result =
          provider.addSerializer(
              type,
              new TypeWrappedSerializer(
                  _typeSerializer, _provider.findValueSerializer(type, null)));
    }
  ...
  private final JsonSerializer<Object> _findAndAddDynamic(JavaType type)
    ...
    if (_typeSerializer == null) {
      ...
      result =
          _dynamicSerializers.addSerializer(
              type,
              new TypeWrappedSerializer(
                  _typeSerializer, _provider.findValueSerializer(type, null)));
    }
