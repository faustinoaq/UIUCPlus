...
/**
 * Dummy container class to group standard homogenous array serializer implementations (primitive
 * arrays and String array).
 */
...
public class StdArraySerializers {
  ...
  static {
    ...
    _arraySerializers.put(
        boolean[].class.getName(), new StdArraySerializers.BooleanArraySerializer());
    ...
    _arraySerializers.put(
        double[].class.getName(), new StdArraySerializers.DoubleArraySerializer());
  }
  ...
  /**
   * Intermediate base class used for cases where we may add type information (excludes
   * boolean/int/double arrays).
   */
  protected abstract static class TypedPrimitiveArraySerializer<T> extends ArraySerializerBase<T> {
    ...
    protected TypedPrimitiveArraySerializer(
        TypedPrimitiveArraySerializer<T> src, BeanProperty prop, Boolean unwrapSingle) {
  ...
  public static class BooleanArraySerializer extends ArraySerializerBase<boolean[]> {
    ...
    protected BooleanArraySerializer(
        BooleanArraySerializer src, BeanProperty prop, Boolean unwrapSingle) {
      super(src, prop, unwrapSingle);
    }
    ...
    public JsonNode getSchema(SerializerProvider provider, Type typeHint) {
      ObjectNode o = createSchemaNode("array", false);
  ...
  /**
   * Character arrays are different from other integral number arrays in that they are most likely
   * to be textual data, and should be written as Strings, not arrays of entries.
   *
   * <p>NOTE: since it is NOT serialized as an array, cannot use AsArraySerializer as base
   */
  @JacksonStdImpl
  public static class CharArraySerializer extends StdSerializer<char[]> {
    ...
    public void serializeWithType(
        char[] value, JsonGenerator g, SerializerProvider provider, TypeSerializer typeSer)
        throws IOException {
      // [JACKSON-289] allows serializing as 'sparse' char array too:
      final boolean asArray =
          provider.isEnabled(SerializationFeature.WRITE_CHAR_ARRAYS_AS_JSON_ARRAYS);
  ...
  public static class DoubleArraySerializer extends ArraySerializerBase<double[]> {
    ...
    protected DoubleArraySerializer(
        DoubleArraySerializer src, BeanProperty prop, Boolean unwrapSingle) {
      super(src, prop, unwrapSingle);
    }
