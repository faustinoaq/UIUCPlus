...
/**
 * Base implementation for values of {@link ReferenceType}. Implements most of functionality, only
 * leaving couple of abstract methods for sub-classes to implement.
 *
 * @since 2.8
 */
public abstract class ReferenceTypeSerializer<T> extends StdSerializer<T>
  ...
  /**
   * If element type cannot be statically determined, mapping from runtime type to serializer is
   * handled using this object
   */
  ...
  /**
   * Mutant factory method called when changes are needed; should construct newly configured
   * instance with new values as indicated.
   *
   * <p>NOTE: caller has verified that there are changes, so implementations need NOT check if a new
   * instance is needed.
   *
   * @since 2.9
   */
  protected abstract ReferenceTypeSerializer<T> withResolved(
      BeanProperty prop, TypeSerializer vts, JsonSerializer<?> valueSer, NameTransformer unwrapper);

  /**
   * Mutant factory method called to create a differently constructed instance, specifically with
   * different exclusion rules for contained value.
   *
   * <p>NOTE: caller has verified that there are changes, so implementations need NOT check if a new
   * instance is needed.
   *
   * @since 2.9
   */
  public abstract ReferenceTypeSerializer<T> withContentInclusion(
      Object suppressableValue, boolean suppressNulls);

  /**
   * Method called to see if there is a value present or not. Note that value itself may still be
   * `null`, even if present, if referential type allows three states (absent, present-null,
  ...
  public void serializeWithType(
      T ref, JsonGenerator g, SerializerProvider provider, TypeSerializer typeSer)
      throws IOException {
    Object value = _getReferencedIfPresent(ref);
    if (value == null) {
      if (_unwrapper == null) {
  ...
  private final JsonSerializer<Object> _findCachedSerializer(
      SerializerProvider provider, Class<?> rawType) throws JsonMappingException {
  ...
  private final JsonSerializer<Object> _findSerializer(
      SerializerProvider provider, JavaType type, BeanProperty prop) throws JsonMappingException {
