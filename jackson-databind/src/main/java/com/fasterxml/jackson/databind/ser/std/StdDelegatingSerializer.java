...
/**
 * Serializer implementation where given Java type is first converted to an intermediate "delegate
 * type" (using a configured {@link Converter}, and then this delegate value is serialized by
 * Jackson.
 *
 * <p>Note that although types may be related, they must not be same; trying to do this will result
 * in an exception.
 *
 * @since 2.1
 */
...
public class StdDelegatingSerializer extends StdSerializer<Object>
  ...
  public JsonSerializer<?> createContextual(SerializerProvider provider, BeanProperty property)
    ...
    if ((delSer == null) || (delSer == _delegateSerializer && type == _delegateType)) {
  ...
  /**
   * Method called to convert from source Java value into delegate value (which will be serialized
   * using standard Jackson serializer for delegate type)
   *
   * <p>The default implementation uses configured {@link Converter} to do conversion.
   *
  ...
  /**
   * Helper method used for locating serializer to use in dynamic use case, where actual type value
   * gets converted to is not specified beyond basic {@link java.lang.Object}, and where serializer
   * needs to be located dynamically based on actual value type.
   *
   * @since 2.6
   */
