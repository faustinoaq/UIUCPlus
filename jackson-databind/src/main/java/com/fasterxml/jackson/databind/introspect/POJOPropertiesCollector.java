...
public class POJOPropertiesCollector {
  ...
  /**
   * True if introspection is done for serialization (giving precedence for serialization
   ...
   *
   * <p>Since 2.6, this has been constructed (more) lazily, to defer throwing of exceptions for
   * potential conflicts in cases where this may not be an actual problem.
   */
  ...
  /**
   * A set of "field renamings" that have been discovered, indicating intended renaming of other
   * accesors: key is the implicit original name and value intended name to use instead.
   *
   * <p>Note that these renamings are applied earlier than "regular" (explicit) renamings and affect
   * implicit name: their effect may be changed by further renaming based on explicit indicators.
   * The main use case is to effectively relink accessors based on fields discovered, and used to
   * sort of correct otherwise missing linkage between fields and other accessors.
   *
   * @since 2.11
   */
  ...
  /**
   * Accessors (field or "getter" method annotated with {@link
  ...
  /**
   * Lazily collected list of properties that can be implicitly ignored during serialization; only
   * updated when collecting information for deserialization purposes
   */
  protected HashSet<String> _ignoredPropertyNames;

  /**
   * Lazily collected list of members that were annotated to indicate that they represent mutators
   * for deserializer value injection.
   */
   ...
   * @deprecated Since 2.12
   */
  @Deprecated protected String _mutatorPrefix = "set";
  ...
  protected POJOPropertiesCollector(
    ...
    this(
        config, forSerialization, type, classDef, _accessorNaming(config, classDef, mutatorPrefix));
    _mutatorPrefix = mutatorPrefix;
  }

  private static AccessorNamingStrategy _accessorNaming(
      MapperConfig<?> config, AnnotatedClass classDef, String mutatorPrefix) {
    if (mutatorPrefix == null) {
      mutatorPrefix = "set";
    }
    return new DefaultAccessorNamingStrategy.Provider()
        .withSetterPrefix(mutatorPrefix)
    ...
    if (_jsonKeyAccessors != null) {
      if (_jsonKeyAccessors.size() > 1) {
        if (!_resolveFieldVsGetter(_jsonKeyAccessors)) {
          reportProblem(
              "Multiple 'as-key' properties defined (%s vs %s)",
              _jsonKeyAccessors.get(0), _jsonKeyAccessors.get(1));
        }
      }
    ...
    if (_jsonValueAccessors != null) {
      if (_jsonValueAccessors.size() > 1) {
        if (!_resolveFieldVsGetter(_jsonValueAccessors)) {
          reportProblem(
              "Multiple 'as-value' properties defined (%s vs %s)",
   ...
   *
   * @deprecated Since 2.12 use separate {@link #getAnyGetterMethod()} and {@link
   *     #getAnyGetterField()}.
   */
  @Deprecated // since 2.12
  ...
  public AnnotatedMember getAnyGetterField() {
    ...
    if (_anyGetterField != null) {
      if (_anyGetterField.size() > 1) {
        reportProblem(
            "Multiple 'any-getter' fields defined (%s vs %s)",
  ...
  public AnnotatedMember getAnyGetterMethod() {
    ...
    if (_anyGetters != null) {
      if (_anyGetters.size() > 1) {
        reportProblem(
            "Multiple 'any-getter' methods defined (%s vs %s)",
  ...
  public AnnotatedMember getAnySetterField() {
    ...
    if (_anySetterField != null) {
      if (_anySetterField.size() > 1) {
        reportProblem(
            "Multiple 'any-setter' fields defined (%s vs %s)",
  ...
  /**
   * Accessor for set of properties that are explicitly marked to be ignored via per-property
   * markers (but NOT class annotations).
   */
  ...
  protected void collectAll() {
    LinkedHashMap<String, POJOPropertyBuilder> props =
  ...
  protected void _addFields(Map<String, POJOPropertyBuilder> props) {
     ...
     */
    final boolean pruneFinalFields =
        !_forSerialization && !_config.isEnabled(MapperFeature.ALLOW_FINAL_FIELDS_AS_MUTATORS);
    ...
    for (AnnotatedField f : _classDef.fields()) {
      ...
      if (nameExplicit
          && pn
              .isEmpty()) { // empty String meaning "use default name", here just means "same as
                            // field name"
   ...
   * @since 2.4
   */
  protected void _addCreatorParam(
      Map<String, POJOPropertyBuilder> props, AnnotatedParameter param) {
    _addCreatorParam(props, param, null);
  }

  private void _addCreatorParam(
    ...
    if (!expl) {
      ...
      JsonCreator.Mode creatorMode =
          _annotationIntrospector.findCreatorAnnotation(_config, param.getOwner());
    ...
    POJOPropertyBuilder prop =
        (expl && impl.isEmpty()) ? _property(props, pn) : _property(props, impl);
  ...
  protected void _addGetterMethod(
      Map<String, POJOPropertyBuilder> props, AnnotatedMethod m, AnnotationIntrospector ai) {
    ...
    // 06-May-2020, tatu: [databind#2675] changes handling slightly...
    {
      ...
      if ((rt == Void.TYPE)
          || ((rt == Void.class)
              && !_config.isEnabled(MapperFeature.ALLOW_VOID_VALUED_PROPERTIES))) {
        return;
      }
    }
  ...
  protected void _addSetterMethod(
      Map<String, POJOPropertyBuilder> props, AnnotatedMethod m, AnnotationIntrospector ai) {
    String implName; // from naming convention
    boolean visible;
  ...
  protected void _doAddInjectable(JacksonInject.Value injectable, AnnotatedMember m) {
    ...
    if (prev != null) {
      ...
      if (prev.getClass() == m.getClass()) {
        reportProblem(
            "Duplicate injectable value with id '%s' (of type %s)", id, ClassUtil.classNameOf(id));
      }
    }
  }
  ...
  protected void _removeUnwantedProperties(Map<String, POJOPropertyBuilder> props) {
    ...
    while (it.hasNext()) {
      ...
      if (prop.anyIgnorals()) {
        ...
        if (isRecordType()) {
          // ...so can only remove ignored field and/or accessors, not constructor parameters that
          // are needed
          // for instantiation...
          prop.removeIgnored();
  ...
  /**
   * Method called to further get rid of unwanted individual accessors, based on read/write settings
   * and rules for "pulling in" accessors (or not).
   */
  protected void _removeUnwantedAccessor(Map<String, POJOPropertyBuilder> props) {
    // 15-Jan-2023, tatu: Avoid pulling in mutators for Records; Fields mostly
    //    since there should not be setters.
    final boolean inferMutators =
        !isRecordType() && _config.isEnabled(MapperFeature.INFER_PROPERTY_MUTATORS);
    Iterator<POJOPropertyBuilder> it = props.values().iterator();
  ...
  /**
   * Helper method called to add explicitly ignored properties to a list of known ignored
   * properties; this helps in proper reporting of errors.
   */
  protected void _collectIgnorals(String name) {
    if (!_forSerialization && (name != null)) {
      if (_ignoredPropertyNames == null) {
        _ignoredPropertyNames = new HashSet<String>(name);
      }
  ...
  protected void _renameUsing(
    ...
    for (POJOPropertyBuilder prop : props) {
      ...
      if (!prop.isExplicitlyNamed()
          || _config.isEnabled(MapperFeature.ALLOW_EXPLICIT_PROPERTY_RENAMING)) {
        if (_forSerialization) {
          if (prop.hasGetter()) {
            rename =
                naming.nameForGetterMethod(_config, prop.getGetter(), fullName.getSimpleName());
          ...
          if (prop.hasSetter()) {
            rename =
                naming.nameForSetterMethod(
                    _config, prop.getSetterUnchecked(), fullName.getSimpleName());
          } else if (prop.hasConstructorParameter()) {
            rename =
                naming.nameForConstructorParameter(
                    _config, prop.getConstructorParameter(), fullName.getSimpleName());
          } else if (prop.hasField()) {
            rename =
                naming.nameForField(_config, prop.getFieldUnchecked(), fullName.getSimpleName());
          } else if (prop.hasGetter()) {
            // Plus, when getter-as-setter is used, need to convert that too..
            // (should we verify that's enabled? For now, assume it's ok always)
            rename =
                naming.nameForGetterMethod(
                    _config, prop.getGetterUnchecked(), fullName.getSimpleName());
          }
        }
      }
  ...
  protected void _sortProperties(Map<String, POJOPropertyBuilder> props) {
    ...
    final boolean sortAlpha =
        (alpha == null) ? _config.shouldSortPropertiesAlphabetically() : alpha.booleanValue();
  ...
  /**
   * Method that will be given a {@link List} with 2 or more accessors that may be in conflict: it
   * will need to remove lower-priority accessors to leave just a single highest-priority accessor
   * to use. If this succeeds method returns {@code true}, otherwise {@code false}.
   *
   * <p>NOTE: method will directly modify given {@code List} directly, regardless of whether it
  ...
  protected POJOPropertyBuilder _property(
      Map<String, POJOPropertyBuilder> props, PropertyName name) {
  ...
  private PropertyNamingStrategy _findNamingStrategy() {
    ...
    if (!(namingDef instanceof Class)) {
      reportProblem(
          "AnnotationIntrospector returned PropertyNamingStrategy definition of type %s; expected"
              + " type `PropertyNamingStrategy` or `Class<PropertyNamingStrategy>` instead",
    ...
    return (PropertyNamingStrategy)
        ClassUtil.createInstance(namingClass, _config.canOverrideAccessModifiers());
  }

  @Deprecated // since 2.12.1 (temporarily missing from 2.12.0)
  protected void _updateCreatorProperty(
      POJOPropertyBuilder prop, List<POJOPropertyBuilder> creatorProperties) {
    _replaceCreatorProperty(prop, creatorProperties);
  }

  protected boolean _replaceCreatorProperty(
      POJOPropertyBuilder prop, List<POJOPropertyBuilder> creatorProperties) {
