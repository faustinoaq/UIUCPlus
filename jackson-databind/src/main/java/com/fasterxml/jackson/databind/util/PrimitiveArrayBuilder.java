...
public abstract class PrimitiveArrayBuilder<T> {
   ...
   * formed by following {@link #_bufferHead}.
   */
  ...
  public T resetAndStart() {
    _reset();
    return (_freeBuffer == null) ? _constructArray(INITIAL_CHUNK_SIZE) : _freeBuffer;
  }
  ...
  public T completeAndClearBuffer(T lastChunk, int lastChunkEntries) {
    ...
    if ((totalSize == PrimitiveArrayBuilder.INITIAL_CHUNK_SIZE) || (pointer != totalSize)) {
  ...
  /**
   * For actual buffering beyond the current buffer, we can actually use shared class which only
   * deals with opaque "untyped" chunks. This works because {@link java.lang.System#arraycopy} does
   * not take type; hence we can implement some aspects of primitive data handling in generic
   * fashion.
   */
