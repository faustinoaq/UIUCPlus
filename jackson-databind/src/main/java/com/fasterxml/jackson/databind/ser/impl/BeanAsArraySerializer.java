...
/**
 * Specialized POJO serializer that differs from {@link
 * com.fasterxml.jackson.databind.ser.BeanSerializer} in that instead of producing a JSON Object it
 * will output a JSON Array, omitting field names, and serializing values in specified serialization
 * order. This behavior is usually triggered by using annotation {@link
 * com.fasterxml.jackson.annotation.JsonFormat} or its equivalents.
 *
 * <p>This serializer can be used for "simple" instances; and will NOT be used if one of following
 * is true:
 *
 * <ul>
 *   <li>Unwrapping is used (no way to expand out array in JSON Object)
 *   <li>Type information ("type id") is to be used: while this could work for some embedding
 *       methods, it would likely cause conflicts.
 *   <li>Object Identity ("object id") is used: while references would work, the problem is
 *       inclusion of id itself.
 * </ul>
 *
 * Note that it is theoretically possible that last 2 issues could be addressed (by reserving room
 * in array, for example); and if so, support improved.
 *
 * <p>In cases where array-based output is not feasible, this serializer can instead delegate to the
 * original Object-based serializer; this is why a reference is retained to the original serializer.
 *
 * @since 2.1
 */
public class BeanAsArraySerializer extends BeanSerializerBase {
  ...
  /**
   * Serializer that would produce JSON Object version; used in cases where array output cannot be
   * used.
   */
  ...
  private boolean hasSingleElement(SerializerProvider provider) {
    ...
    if ((_filteredProps != null && provider.getActiveView() != null)
        && (provider.isEnabled(Config.WRITE_SINGLE_ELEM_ARRAYS_UNWRAPPED)
            && hasSingleElement(provider))) {
  ...
  protected final void serializeAsArray(Object bean, JsonGenerator gen, SerializerProvider provider)
    ...
    try {
      ...
      DatabindException mapE =
          JsonMappingException.from(gen, "Infinite recursion (StackOverflowError)", e);
