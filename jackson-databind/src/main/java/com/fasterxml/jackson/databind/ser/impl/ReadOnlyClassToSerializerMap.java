...
/**
 * Optimized lookup table for accessing two types of serializers; typed and non-typed. Only accessed
 * from a single thread, so no synchronization needed for accessors.
 *
 * <p>Note that before 2.6 this class was much smaller, and referred most operations to separate
 * <code>JsonSerializerMap</code>, but in 2.6 functions were combined.
 */
public final class ReadOnlyClassToSerializerMap {
  ...
  public JsonSerializer<Object> typedValueSerializer(Class<?> type) {
    Bucket bucket = _buckets[TypeKey.typedHash(type) & _mask];
    if ((bucket == null) || (this.matchesUntyped(type))) {
      return null;
    }
    if (bucket.matchesTyped(type)) {
      return bucket.value;
