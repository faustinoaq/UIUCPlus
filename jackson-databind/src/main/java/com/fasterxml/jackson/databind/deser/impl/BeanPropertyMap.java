...
/**
 * Helper class used for storing mapping from property name to {@link SettableBeanProperty}
 * instances.
 *
 * <p>Note that this class is used instead of generic {@link java.util.HashMap} for bit of
 * performance gain (and some memory savings): although default implementation is very good for
 * generic use cases, it can be streamlined a bit for specific use case we have. Even relatively
 * small improvements matter since this is directly on the critical path during deserialization, as
 * it is done for each and every POJO property deserialized.
 */
public class BeanPropertyMap implements Iterable<SettableBeanProperty>, java.io.Serializable {
  ...
  /**
   * Array of properties in the exact order they were handed in. This is used by as-array
   * serialization, deserialization.
   */
  private final SettableBeanProperty[] _propsInOrder;

  /**
   * Configuration of alias mappings, indexed by unmodified property name to unmodified aliases, if
   * any; entries only included for properties that do have aliases. This is is used for
   * constructing actual reverse lookup mapping, if needed, taking into account possible
   * case-insensitivity, as well as possibility of name prefixes.
   *
   * @since 2.9
   */
  ...
  /**
   * We require {@link Locale} since case changes are locale-sensitive in certain cases (see <a
   * href="https://en.wikipedia.org/wiki/Dotted_and_dotless_I">Turkish I</a> for example)
   *
   * @since 2.11
   */
  ...
  private BeanPropertyMap(
      BeanPropertyMap src, SettableBeanProperty newProp, int hashIndex, int orderedIndex) {
  ...
  /**
   * Mutant factory method that constructs a new instance if desired case-insensitivity state
   * differs from the state of this instance; if states are the same, returns <code>this</code>.
   *
   * @since 2.8
   */
   ...
   * @since 2.12
   */
  public static BeanPropertyMap construct(
      MapperConfig<?> config,
      Collection<SettableBeanProperty> props,
  ...
  /**
   * Fluent copy method that creates a new instance that is a copy of this instance except for one
   * additional property that is passed as the argument. Note that method does not modify this
   * instance but constructs and returns a new one.
   */
  ...
  public BeanPropertyMap renameAll(NameTransformer transformer) {
    ...
    // should we try to re-index? Ordering probably changed but caller probably doesn't want
    // changes...
  ...
  /**
   * Mutant factory method that will use this instance as the base, and construct an instance that
   * is otherwise same except for excluding properties with specified names.
   *
   * @since 2.8
   */
  ...
  /**
   * Mutant factory method that will use this instance as the base, and construct an instance that
   * is otherwise same except for excluding properties with specified names, or including only the
   * one marked as included
   *
   * @since 2.12
   */
  public BeanPropertyMap withoutProperties(
  ...
  /**
   * Specialized method that can be used to replace an existing entry (note: entry MUST exist;
   * otherwise exception is thrown) with specified replacement.
   *
   * @since 2.9.4
   */
    ...
    for (; ; i += 2) {
      if (i >= end) {
        throw new NoSuchElementException(
            "No entry '" + origProp.getName() + "' found, can't replace");
      }
  ...
  /**
   * Specialized method for removing specified existing entry. NOTE: entry MUST exist, otherwise an
   * exception is thrown.
   */
  ...
  /**
   * Method that will re-create initial insertion-ordering of properties contained in this map. Note
   * that if properties have been removed, array may contain nulls; otherwise it should be
   * consecutive.
   *
   * @since 2.1
   */
  ...
  /**
   * Convenience method that tries to find property with given name, and if it is found, call {@link
   * SettableBeanProperty#deserializeAndSet} on it, and return true; or, if not found, return false.
   * Note, too, that if deserialization is attempted, possible exceptions are wrapped if and as
   * necessary, so caller need not handle those.
   *
   * @since 2.5
   */
  public boolean findDeserializeAndSet(
      JsonParser p, DeserializationContext ctxt, Object bean, String key) throws IOException {
  ...
  public String toString() {
    ...
    sb.append(_aliasDefs); // removed parentheses on line 18
    return sb.toString();
  }
  ...
  protected SettableBeanProperty _rename(SettableBeanProperty prop, NameTransformer xf) {
    ...
    if (deser != null) {
      ...
      JsonDeserializer<Object> newDeser =
          (JsonDeserializer<Object>) deser.unwrappingDeserializer(xf);
      if (newDeser != deser) {
  ...
  protected void wrapAndThrow(
      Throwable t, Object bean, String fieldName, DeserializationContext ctxt) throws IOException {
  ...
  /**
   * Helper method used to find exact location of a property with name given exactly, not subject to
   * case changes, within hash area. Expectation is that such property SHOULD exist, although no
   * exception is thrown.
   *
   * @since 2.7
   */
  ...
  */
  ...
  private final int _findFromOrdered(SettableBeanProperty prop) {
    ...
    throw new IllegalStateException(
        "Illegal state: property '" + prop.getName() + "' missing from _propsInOrder");
  }
  ...
  // @since 2.9
  private Map<String, String> _buildAliasMapping(
      Map<String, List<PropertyName>> defs, boolean caseInsensitive, Locale loc) {
