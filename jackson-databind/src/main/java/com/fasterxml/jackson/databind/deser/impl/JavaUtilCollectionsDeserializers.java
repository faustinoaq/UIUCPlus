...
/**
 * Helper class used to contain logic for deserializing "special" containers from {@code
 * java.util.Collections} and {@code java.util.Arrays}. This is needed because they do not have
 * usable no-arguments constructor: however, are easy enough to deserialize using delegating
 * deserializer.
 *
 * @since 2.9.4
 */
public abstract class JavaUtilCollectionsDeserializers {
  ...
  public static JsonDeserializer<?> findForCollection(DeserializationContext ctxt, JavaType type)
    ...
    if ((localName = _findUtilArrayTypeName(clsName)) != null) {
      ...
      if (localName.contains("List")) {
        ...
        return new StdDelegatingDeserializer<Object>(converter(TYPE_AS_LIST, type, List.class));
      }
      return null;
    }
  ...
  static JavaUtilCollectionsConverter converter(
      int kind, JavaType concreteType, Class<?> rawSuper) {
  ...
  private static String _findUtilCollectionsImmutableTypeName(String clsName) {
    ...
    else if (clsName.startsWith(PREFIX_JAVA_UTIL_IMMUTABLE_COLL))
      return clsName.substring(PREFIX_JAVA_UTIL_IMMUTABLE_COLL.length());
    else return null;
  }
  ...
  /**
   * Implementation used for converting from various generic container types ({@link java.util.Set},
   * {@link java.util.List}, {@link java.util.Map}) into more specific implementations accessible
   * via {@code java.util.Collections}.
   */
    ...
    private void _checkSingleton(int size) {
      if (size != 1) {
        ...
        throw new IllegalArgumentException(
            "Can not deserialize Singleton container from " + size + " entries");
      }
    }
  }
}
