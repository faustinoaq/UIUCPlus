 ...
 *
 * <p>Note: about the only configurable setting currently is ability to filter out entries with
 * specified names.
 */
@JacksonStdImpl
public class MapSerializer extends ContainerSerializer<Map<?, ?>> implements ContextualSerializer {
  ...
  /**
   * Whether static types should be used for serialization of values or not (if not, dynamic runtime
   * type is used)
   */
  ...
  /**
   * Set of entries to include during serialization, if null, it is ignored, empty will include
   * nothing.
   *
   * @since 2.12
   */
  ...
  /**
   * Value that indicates suppression mechanism to use for <b>values contained</b>; either "filter"
   * (of which <code>equals()</code> is called), or marker value of {@link #MARKER_FOR_EMPTY}, or
   * null to indicate no filtering for non-null values. Note that inclusion value for Map instance
   * itself is handled by caller (POJO property that refers to the Map value).
   *
   * @since 2.5
   */
  ...
  /**
   * Flag that indicates what to do with `null` values, distinct from handling of {@link
   * #_suppressableValue}
   *
   * @since 2.9
   */
  ...
  public MapSerializer withResolved(
    ...
    MapSerializer ser =
        new MapSerializer(this, property, keySerializer, valueSerializer, ignored, included);
  ...
  public JsonSerializer<?> createContextual(SerializerProvider provider, BeanProperty property)
    ...
    if (_neitherNull(propertyAcc, intr)) {
      ...
      Set<String> newIgnored =
          intr.findPropertyIgnoralByName(config, propertyAcc).findIgnoredForSerialization();
  ...
  /**
   * Accessor for currently assigned key serializer. Note that this may return null during
   * construction of <code>MapSerializer</code>: depedencies are resolved during {@link
   * #createContextual} method (which can be overridden by custom implementations), but for some
   * dynamic types, it is possible that serializer is only resolved during actual serialization.
   */
  ...
  /**
   * General-purpose serialization for contents without writing object type. Will suppress, filter
   * and use custom serializers.
   *
   * <p>Public since it also is called by {@code AnyGetterWriter}.
   *
   * @since 2.11
   */
  public void serializeWithoutTypeInfo(
      Map<?, ?> value, JsonGenerator gen, SerializerProvider provider) throws IOException {
    if (!value.isEmpty()) {
  ...
  public void serializeTypedFields(
    ...
    for (Map.Entry<?, ?> entry : value.entrySet()) {
      ...
      final Object valueElem = entry.getValue();
      JsonSerializer<Object> valueSer;
      // Buggy version 3: remove declaration of valueSer object and set it to null
      if (valueElem == null) {
        if (_suppressNulls) {
          continue;
        }
        keySerializer = provider.getDefaultNullValueSerializer();
      } else {
        valueSer = null; // Set valueSer to null
        if (valueSer == null) {
          valueSer = _findSerializer(provider, valueElem);
        }
        if (checkEmpty) {
  ...
  /**
   * Helper method used when we have a JSON Filter to use AND contents are "any properties" of a
   * POJO.
   *
