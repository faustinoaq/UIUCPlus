...
/**
 * Class similar to {@link BeanPropertyWriter}, but that will be used for serializing {@link
 * com.fasterxml.jackson.annotation.JsonAnyGetter} annotated (Map) properties
 */
public class AnyGetterWriter {
  ...
  public void fixAccess(SerializationConfig config) {
    _accessor.fixAccess(config.isEnabled(MapperFeature.OVERRIDE_PUBLIC_ACCESS_MODIFIERS));
  }
   ...
   * @since 2.3
   */
  public void getAndFilter(
      Object bean, JsonGenerator gen, SerializerProvider provider, PropertyFilter filter)
      throws Exception {
    Object value = _accessor.getValue(bean);
    if (value == null) {
      return;
    ...
    if (_mapSerializer != null) {
      _mapSerializer.serializeFilteredAnyProperties(
          provider, gen, bean, (Map<?, ?>) value, filter, null);
      return;
    }
