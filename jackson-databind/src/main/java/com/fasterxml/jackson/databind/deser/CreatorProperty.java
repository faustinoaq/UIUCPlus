...
/**
 * This concrete sub-class implements property that is passed via Creator (constructor or static
 * factory method). It is not a full-featured implementation in that its set method should usually
 * not be called for primary mutation -- instead, value must separately passed -- but some aspects
 * are still needed (specifically, injection).
 *
 * <p>Note on injectable values: unlike with other mutators, where deserializer and injecting are
 * separate, here we treat the two as related things. This is necessary to add proper priority, as
 * well as to simplify coordination.
 */
public class CreatorProperty extends SettableBeanProperty {
  ...
  /**
   * Id of value to inject, if value injection should be used for this parameter (in addition to, or
   * instead of, regular deserialization).
   *
   * @since 2.11
   */
  ...
  /**
   * In special cases, when implementing "updateValue", we cannot use constructors or factory
   * methods, but have to fall back on using a setter (or mutable field property). If so, this
   * refers to that fallback accessor.
   *
   * <p>Mutable only to allow setting after construction, but must be strictly set before any use.
   *
   * @since 2.3
   */
  ...
  /**
   * Marker flag that may have to be set during construction, to indicate that although property may
   * have been constructed and added as a placeholder, it represents something that should be
   * ignored during deserialization. This mostly concerns Creator properties which may not be easily
  ...
  public CreatorProperty(
    ...
    this(
        ...
        param,
        index,
        (injectableValueId == null) ? null : JacksonInject.Value.construct(injectableValueId, null),
        metadata);
  }
   ...
   * @param typeDeser Type deserializer to use for handling polymorphic type information, if one is
   *     needed
   * @param contextAnnotations Contextual annotations (usually by class that declares creator
   *     [constructor, factory method] that includes this property)
   * @param param Representation of property, constructor or factory method parameter; used for
   *     accessing annotations of the property
   * @param injectable Information about injectable value, if any
   * @param index Index of this property within creator invocation
   * @since 2.11
   */
  public static CreatorProperty construct(
      PropertyName name,
      JavaType type,
      PropertyName wrapperName,
      TypeDeserializer typeDeser,
    ...
    return new CreatorProperty(
        name, type, wrapperName, typeDeser, contextAnnotations, param, index, injectable, metadata);
  }
  ...
  /**
   * NOTE: one exception to immutability, due to problems with CreatorProperty instances being
   * shared between Bean, separate PropertyBasedCreator
   *
   * @since 2.6
   */
  ...
  public Object findInjectableValue(DeserializationContext context, Object beanInstance)
    ...
    if (_injectableValue == null) {
      context.reportBadDefinition(
          ClassUtil.classOf(beanInstance),
          String.format(
              "Property %s (type %s) has no injectable value id configured",
              ClassUtil.name(getName()), ClassUtil.classNameOf(this)));
    }
    return context.findInjectableValue(
        _injectableValue.getId(), this, beanInstance); // lgtm [java/dereferenced-value-may-be-null]
  }
  ...
  public Object getInjectableValueId() {
    return ((_injectableValue == null)) ? null : (Object) _injectableValue.getId();
  }
  ...
  private void _reportMissingSetter(JsonParser p, DeserializationContext ctxt) throws IOException {
    final String msg =
        "No fallback setter/field defined for creator property " + ClassUtil.name(getName());
