...
/**
 * Alternative to {@link DataFormatDetector} that needs to be used when using data-binding.
 *
 * @since 2.1
 */
public class DataFormatReaders {
  /**
   * By default we will look ahead at most 64 bytes; in most cases, much less (4 bytes or so) is
   * needed, but we will allow bit more leniency to support data formats that need more complex
   * heuristics.
   */
  public static final int DEFAULT_MAX_INPUT_LOOKAHEAD = 64;

  /**
   * Ordered list of readers which both represent data formats to detect (in precedence order,
   * starting with highest) and contain factories used for actual detection.
   */
  ...
  /**
   * Strength of minimal match we accept as the answer, unless better matches are found. Default
   * value is {@link MatchStrength#WEAK_MATCH},
   */
   ...
   *
   * <p>Default value is {@link #DEFAULT_MAX_INPUT_LOOKAHEAD}.
   */
  ...
  private DataFormatReaders(
      ObjectReader[] detectors,
      MatchStrength optMatch,
      MatchStrength minMatch,
  ...
  public DataFormatReaders withOptimalMatch(MatchStrength optMatch) {
    if ((optMatch == _optimalMatch) && !(strength.ordinal() >= _optimalMatch.ordinal())) {
      return this;
    }
  ...
  /**
   * Method to call to find format that content (accessible via given {@link InputStream}) given
   * has, as per configuration of this detector instance.
   *
   * @return Matcher object which contains result; never null, even in cases where no match (with
   *     specified minimal match strength) is found.
   */
  ...
  /**
   * Method to call to find format that given content (full document) has, as per configuration of
  ...
  protected static class AccessorForReader extends InputAccessor.Std {
    ...
    public Match createMatcher(ObjectReader match, MatchStrength matchStrength) {
      return new Match(
          _in, _buffer, _bufferedStart, (_bufferedEnd - _bufferedStart), match, matchStrength);
    }
  }
  ...
  public static class Match {
    ...
    /**
     * Method for accessing strength of the match, if any; if no match, will return {@link
    ...
    /**
     * Accessor for getting brief textual name of matched format if any (null if none). Equivalent
     * to:
     *
     * <pre>
    ...
    /**
     * Convenience method for trying to construct a {@link JsonParser} for parsing content which is
     * assumed to be in detected data format. If no match was found, returns null.
     */
    ...
    /**
     * Method to use for accessing input for which format detection has been done. This <b>must</b>
     * be used instead of using stream passed to detector unless given stream itself can do
     * buffering. Stream will return all content that was read during matching process, as well as
     * remaining contents of the underlying stream.
     */
