...
/**
 * Immutable container class used to store simple configuration settings for both serialization and
 * deserialization. Since instances are fully immutable, instances can be freely shared and used
...
public final class BaseSettings implements java.io.Serializable {
  ...
  /**
   * Introspector used to figure out Bean properties needed for bean serialization and
   * deserialization. Overridable so that it is possible to change low-level details of
   * introspection, like adding new annotation types.
   */
  ...
  /**
   * Builder used to create type resolver for serializing and deserializing values for which
   * polymorphic type handling is needed.
   */
  protected final TypeResolverBuilder<?> _typeResolverBuilder;

  /**
   * Validator that is used to limit allowed polymorphic subtypes, mostly for security reasons when
  ...
  /**
   * Custom date format to use for deserialization. If specified, will be used instead of {@link
   * com.fasterxml.jackson.databind.util.StdDateFormat}.
   *
   * <p>Note that the configured format object will be cloned once per deserialization process
   * (first time it is needed)
   */
  protected final DateFormat _dateFormat;

  /**
   * Object used for creating instances of handlers (serializers, deserializers, type and type id
   * resolvers), given class to instantiate. This is typically used to do additional configuration
   * (with dependency injection, for example) beyond simply construction of instances; or to use
   * alternative constructors.
   */
  protected final HandlerInstantiator _handlerInstantiator;

  /**
   * Default {@link java.util.Locale} used with serialization formats. Default value is {@link
   * Locale#getDefault()}.
   */
  protected final Locale _locale;

  /**
   * Default {@link java.util.TimeZone} used with serialization formats, if (and only if!)
   * explicitly set by use; otherwise `null` to indicate "use default", which means "UTC" (from
   * Jackson 2.7); earlier versions (up to 2.6) used "GMT".
   *
   * <p>Note that if a new value is set, timezone is also assigned to {@link #_dateFormat} of this
   * object.
   */
  protected final TimeZone _timeZone;

  /**
   * Explicitly default {@link Base64Variant} to use for handling binary data (<code>byte[]</code>),
  ...
  public BaseSettings withInsertedAnnotationIntrospector(AnnotationIntrospector ai) {
    return withAnnotationIntrospector(
        AnnotationIntrospectorPair.create(ai, _annotationIntrospector));
  }

  public BaseSettings withAppendedAnnotationIntrospector(AnnotationIntrospector ai) {
    return withAnnotationIntrospector(
        AnnotationIntrospectorPair.create(_annotationIntrospector, ai));
  }
  ...
  */
  ...
  /**
   * Fluent factory for constructing a new instance that uses specified TimeZone. Note that timezone
   * used with also be assigned to configured {@link DateFormat}, changing time formatting defaults.
   */
  ...
  /**
   * Accessor that may be called to determine whether this settings object has been explicitly
   * configured with a TimeZone (true), or is still relying on the default settings (false).
   *
   * @since 2.7
   */
