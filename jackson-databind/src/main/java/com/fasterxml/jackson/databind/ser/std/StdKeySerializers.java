...
public abstract class StdKeySerializers {
   ...
   * @param useDefault If no match is found, should we return fallback deserializer (true), or null
   *     (false)?
   */
  public static JsonSerializer<Object> getStdKeySerializer(
  ...
  /**
   * Method called if no specified key serializer was located; will return a "default" key
   * serializer.
   *
   * @since 2.7
   */
  @SuppressWarnings("unchecked")
  public static JsonSerializer<Object> getFallbackKeySerializer(
      SerializationConfig config, Class<?> rawKeyType) {
    if (rawKeyType != null) {
      ...
      if (ClassUtil.isEnumType(rawKeyType)) {
        return EnumKeySerializer.construct(
            rawKeyType, EnumValues.constructFromName(config, (Class<Enum<?>>) rawKeyType));
      }
    }
    // 19-Oct-2016, tatu: Used to just return DEFAULT_KEY_SERIALIZER but why not:
    return new Default(Default.TYPE_TO_STRING, rawKeyType);
   ...
   *
   * <p>TODO: Should (but does not yet) support re-configuring format used for {@link
   * java.util.Date} and {@link java.util.Calendar} key serializers, as well as alternative
   * configuration of Enum key serializers.
   */
  public static class Default extends StdSerializer<Object> {
    ...
    static final int TYPE_INTEGER = 5; // since 2.9
    static final int TYPE_LONG = 6; // since 2.9
    static final int TYPE_BYTE_ARRAY = 7; // since 2.9
  ...
  /**
   * Key serializer used when key type is not known statically, and actual key serializer needs to
   * be dynamically located.
   */
