...
public class POJOPropertyBuilder extends BeanPropertyDefinition
  ...
  /**
   * External name of logical property; may change with renaming (by new instance being constructed
   * using a new name)
   */
  protected final PropertyName _name;

  /**
   * Original internal name, derived from accessor, of this property. Will not be changed by
   * renaming.
   */
  ...
  public PropertyName getWrapperName() {
    ...
    return (member == null || _annotationIntrospector == null)
        ? null
  ...
  /**
   * Helper method that contains logic for accessing and merging all setter information that we
   * needed, regarding things like possible merging of property value, and handling of incoming
   * nulls. Only called for deserialization purposes.
   */
      ...
      if (_annotationIntrospector != null) {
        if (acc != null) {
          ...
          if (b != null) {
            ...
            if (b.booleanValue()) {
              metadata =
                  metadata.withMergeInfo(PropertyMetadata.MergeInfo.createForPropertyOverride(acc));
            }
          }
        }
      ...
      if (needMerge || (valueNulls == null) || (contentNulls == null)) {
        ...
        if (needMerge && (acc != null)) {
          ...
          if (b != null) {
            ...
            if (b.booleanValue()) {
              metadata =
                  metadata.withMergeInfo(PropertyMetadata.MergeInfo.createForTypeOverride(acc));
            }
          }
        }
      }
  ...
  /**
   * Type determined from the primary member for the property being built, considering precedence
   * according to whether we are processing serialization or deserialization.
   */
  @Override
    ...
    for (; next != null; next = next.next) {
      ...
      throw new IllegalArgumentException(
          "Conflicting getter definitions for property \""
              + getName()
              + "\": "
              + curr.value.getFullName()
              + " vs "
              + next.value.getFullName());
    }
  ...
  /** Variant of {@link #getGetter} that does NOT trigger pruning of getter candidates. */
  ...
  /** Variant of {@link #getSetter} that does NOT trigger pruning of setter candidates. */
  ...
  /**
   * Helper method called in cases where we have encountered two setter methods that have same
   * precedence and cannot be resolved. This does not yet necessarily mean a failure since it is
   * possible something with a higher precedence could still be found; handling is just separated
   * into separate method for convenience.
   *
   * @param curr
   * @param next
   ...
   * @since 2.13
   */
  protected AnnotatedMethod _selectSetterFromMultiple(
      Linked<AnnotatedMethod> curr, Linked<AnnotatedMethod> next) {
    ...
    // Otherwise
    String desc =
        conflicts.stream().map(AnnotatedMethod::getFullName).collect(Collectors.joining(" vs "));
    throw new IllegalArgumentException(
        String.format("Conflicting setter definitions for property \"%s\": %s", getName(), desc));
  }
    ...
    for (; next != null; next = next.next) {
      ...
      throw new IllegalArgumentException(
          "Multiple fields representing property \""
              ...
              + field.getFullName()
              + " vs "
              + nextField.getFullName());
    }
    return field;
  }

  /** Variant of {@link #getField} that does NOT trigger pruning of Field candidates. */
  ...
  public JsonInclude.Value findInclusion() {
    ...
    //    is available here
    JsonInclude.Value v =
        (_annotationIntrospector == null) ? null : _annotationIntrospector.findPropertyInclusion(a);
    return (v == null) ? JsonInclude.Value.empty() : v;
  }

  public JsonProperty.Access findAccess() {
    return fromMemberAnnotationsExcept(
  ...
  public void addField(
      AnnotatedField a, PropertyName name, boolean explName, boolean visible, boolean ignored) {
  ...
  public void addCtor(
    ...
    _ctorParameters =
        new Linked<AnnotatedParameter>(a, _ctorParameters, name, explName, visible, ignored);
  }

  public void addGetter(
      AnnotatedMethod a, PropertyName name, boolean explName, boolean visible, boolean ignored) {
   ...
   */
  public JsonProperty.Access removeNonVisible(
      boolean inferMutators, POJOPropertiesCollector parent) {
  ...
  /**
   * Method called to trim unnecessary entries, such as implicit getter if there is an explict one
   * available. This is important for later stages, to avoid unnecessary conflicts.
   */
  ...
  /**
   * Replacement, as per [databind#868], of simple access to annotations, which does "deep merge" if
   * an as necessary.
   *
   * <pre>
  ...
  /**
   * Helper method to handle recursive merging of annotations within accessor class, to ensure no
   * annotations are accidentally dropped within chain when non-visible and secondary accessors are
   * pruned later on.
   *
   * <p>See [databind#868] for more information.
   *
   * @since 2.6
   */
  private <T extends AnnotatedMember> Linked<T> _applyAnnotations(
      Linked<T> node, AnnotationMap ann) {
    @SuppressWarnings("unchecked")
  ...
  /**
   * Method called to find out set of explicit names for accessors bound together due to implicit
   * name.
   *
   * @since 2.4
   */
  ...
  /**
   * Method called when a previous call to {@link #findExplicitNames} found multiple distinct
   * explicit names, and the property this builder represents basically needs to be broken apart and
   * replaced by a set of more than one properties.
   *
   * @since 2.4
   */
  public Collection<POJOPropertyBuilder> explode(Collection<PropertyName> newNames) {
    HashMap<PropertyName, POJOPropertyBuilder> props =
        new HashMap<PropertyName, POJOPropertyBuilder>();
      ...
      if (prop == null) {
        prop =
            new POJOPropertyBuilder(
                _config, _annotationIntrospector, _forSerialization, _internalName, name);
        props.put(name, prop);
      }
  ...
  private Set<PropertyName> _findExplicitNames(
      Linked<? extends AnnotatedMember> node, Set<PropertyName> renamed) {
  ...
  /**
   * Helper method used for finding annotation values, from accessors relevant to current usage
  ...
  protected static final class Linked<T> {
    ...
    public Linked(
        T v, Linked<T> n, PropertyName name, boolean explName, boolean visible, boolean ignored) {
      value = v;
      next = n;
      ...
      if (!explName) {
        if (this.name != null) {
          throw new IllegalArgumentException(
              "Cannot pass false for 'explName' if name is not null/empty");
        } else {
