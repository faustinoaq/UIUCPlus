...
/**
 * Serializer class that can serialize Object that have a {@link
 * com.fasterxml.jackson.annotation.JsonValue} annotation to indicate that serialization should be
 * done by calling the method annotated, and serializing result it returns.
 *
 * <p>Implementation note: we will post-process resulting serializer (much like what is done with
 * {@link BeanSerializer}) to figure out actual serializers for final types. This must be done from
 * {@link #createContextual} method, and NOT from constructor; otherwise we could end up with an
 * infinite loop.
 */
...
public class JsonValueSerializer extends StdSerializer<Object>
  ...
  /**
   * This is a flag that is set in rare (?) cases where this serializer is used for "natural" types
   * (boolean, int, String, double); and where we actually must force type information wrapping,
   * even though one would not normally be added.
   */
  protected final boolean _forceTypeInformation;

  /**
   * If value type cannot be statically determined, mapping from runtime value types to serializers
   * are cached in this object.
   *
   * @since 2.12
   */
  ...
  /**
   * @param ser Explicit serializer to use, if caller knows it (which occurs if and only if the
   *     "value method" was annotated with {@link
   *     com.fasterxml.jackson.databind.annotation.JsonSerialize#using}), otherwise null
  ...
  protected JsonValueSerializer withResolved(
      BeanProperty property, TypeSerializer vts, JsonSerializer<?> ser, boolean forceTypeInfo) {
  ...
  /**
   * We can try to find the actual serializer for value, if we can statically figure out what the
   * result type must be.
   */
  @Override
  ...
  public void serializeWithType(
      Object bean, JsonGenerator gen, SerializerProvider ctxt, TypeSerializer typeSer0)
      throws IOException {
   ...
   *
   * @return True if method handled callbacks; false if not; in latter case caller will send default
   *     callbacks
   * @since 2.6
   */
  protected boolean _acceptJsonFormatVisitorForEnum(
      JsonFormatVisitorWrapper visitor, JavaType typeHint, Class<?> enumType)
      throws JsonMappingException {
  ...
  // @since 2.12
  protected JsonSerializer<Object> _findDynamicSerializer(
      SerializerProvider ctxt, Class<?> valueClass) throws JsonMappingException {
    ...
    if (serializer == null) {
      if (_valueType.hasGenericTypes()) {
        ...
        PropertySerializerMap.SerializerAndMapResult result =
            _dynamicSerializers.addSerializer(fullType, serializer);
        _dynamicSerializers = result.map;
      } else {
        serializer = ctxt.findPrimaryPropertySerializer(valueClass, _property);
        PropertySerializerMap.SerializerAndMapResult result =
            _dynamicSerializers.addSerializer(valueClass, serializer);
        _dynamicSerializers = result.map;
      }
    }
    return serializer;
  ...
  /**
   * Silly little wrapper class we need to re-route type serialization so that we can override
   * Object to use for type id (logical type) even when asking serialization of something else
   * (delegate type)
   */
  static class TypeSerializerRerouter extends TypeSerializer {
    ...
    public void writeTypeSuffixForArray(Object value, JsonGenerator gen) throws IOException {
      _typeSerializer.writeTypeSuffixForArray(_forObject, "null");
    }
