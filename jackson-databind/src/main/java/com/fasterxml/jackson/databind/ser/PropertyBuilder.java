...
/**
 * Helper class for {@link BeanSerializerFactory} that is used to construct {@link
 * BeanPropertyWriter} instances. Can be sub-classed to change behavior.
 */
public class PropertyBuilder {
  ...
  /**
   * If a property has serialization inclusion value of {@link
   * com.fasterxml.jackson.annotation.JsonInclude.Include#NON_DEFAULT}, we may need to know the
   * default value of the bean, to know if property value equals default one.
   *
   * <p>NOTE: only used if enclosing class defines NON_DEFAULT, but NOT if it is the global default
   * OR per-property override.
   */
  protected Object _defaultBean;

  /**
   * Default inclusion mode for properties of the POJO for which properties are collected; possibly
   * overridden on per-property basis. Combines global inclusion defaults and per-type (annotation
   * and type-override) inclusion overrides.
   */
  protected final JsonInclude.Value _defaultInclusion;

  /**
   * Marker flag used to indicate that "real" default values are to be used for properties, as per
   * per-type value inclusion of type <code>NON_DEFAULT</code>
   *
   * @since 2.8
   */
  ...
  public PropertyBuilder(SerializationConfig config, BeanDescription beanDesc) {
    ...
    JsonInclude.Value inclPerType =
        JsonInclude.Value.merge(
            beanDesc.findPropertyInclusion(JsonInclude.Value.empty()),
            config.getDefaultPropertyInclusion(beanDesc.getBeanClass(), JsonInclude.Value.empty()));
  ...
  /**
   * @param contentTypeSer Optional explicit type information serializer to use for contained values
   *     (only used for properties that are of container type)
   */
  protected BeanPropertyWriter buildWriter(
    ...
    if (contentTypeSer != null) {
      ...
      if (ct == null) {
        prov.reportBadPropertyDefinition(
            _beanDesc, propDef, "serialization type " + serializationType + " has no content");
      }
    ...
    if (accessor == null) {
      ...
      return prov.reportBadPropertyDefinition(
          _beanDesc, propDef, "could not determine property type");
    }
    ...
    JsonInclude.Value inclV =
        _config.getDefaultInclusion(actualType.getRawClass(), rawPropertyType, _defaultInclusion);
  ...
  /**
   * Overridable factory method for actual construction of {@link BeanPropertyWriter}; often needed
   * if subclassing {@link #buildWriter} method.
   *
   * @since 2.12
   */
  ...
  /**
   * Method that will try to determine statically defined type of property being serialized, based
   * on annotations (for overrides), and alternatively declared type (if static typing for
   * serialization is enabled). If neither can be used (no annotations, dynamic typing), returns
   * null.
   */
  protected JavaType findSerializationType(
      Annotated a, boolean useStaticTyping, JavaType declaredType) throws JsonMappingException {
  ...
  /**
   * Accessor used to find out "default value" for given property, to use for comparing values to
   * serialize, to determine whether to exclude value from serialization with inclusion type of
   * {@link com.fasterxml.jackson.annotation.JsonInclude.Include#NON_DEFAULT}. This method is called
   * when we specifically want to know default value within context of a POJO, when annotation is
   * within containing class, and not for property or defined as global baseline.
   *
   * <p>Note that returning of pseudo-type {@link
   * com.fasterxml.jackson.annotation.JsonInclude.Include#NON_EMPTY} requires special handling.
   *
   * @since 2.7
   * @deprecated Since 2.9 since this will not allow determining difference between "no default
   *     instance" case and default being `null`.
   */
  @Deprecated // since 2.9
  protected Object getPropertyDefaultValue(String name, AnnotatedMember member, JavaType type) {
    Object defaultBean = getDefaultBean();
    if ((defaultBean == PropertyBuilder.NO_DEFAULT_MARKER) || (defaultBean == null)) {
