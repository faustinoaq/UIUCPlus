...
/**
 * Helper class to use for constructing Object arrays by appending entries to create arrays of
 * various lengths (length that is not known a priori).
 */
public final class ObjectBuffer {
  ...
  /**
   * Let's limit maximum size of chunks we use; helps avoid excessive allocation overhead for huge
   * data sets. For now, let's limit to quarter million entries, 1 meg chunks for 32-bit machines.
   */
  ...
  /**
   * Number of total buffered entries in this buffer, counting all instances within linked list
   * formed by following {@link #_head}.
   */
  private int _size;
  ...
  /**
   * Method called to start buffering process. Will ensure that the buffer is empty, and then return
   * an object array to start chunking content on
   */
  ...
  /**
   * Method called to add a full Object array as a chunk buffered within this buffer, and to obtain
   * a new array to fill. Caller is not to use the array it gives; but to use the returned array for
   * continued buffering.
   *
   * @param fullChunk Completed chunk that the caller is requesting to append to this buffer. It is
   *     generally chunk that was returned by an earlier call to {@link #resetAndStart} or {@link
   *     #appendCompletedChunk} (although this is not required or enforced)
  ...
  /**
   * Method called to indicate that the buffering process is now complete; and to construct a
   * combined exactly-sized result array. Additionally the buffer itself will be reset to reduce
   * memory retention.
   *
   * <p>Resulting array will be of generic <code>Object[]</code> type: if a typed array is needed,
   * use the method with additional type argument.
   */
  ...
  /**
   * Type-safe alternative to {@link #completeAndClearBuffer(Object[], int)}, to allow for
   * constructing explicitly typed result array.
   *
   * @param componentType Type of elements included in the buffer. Will be used for constructing the
   *     result array.
   */
  public <T> T[] completeAndClearBuffer(
      Object[] lastChunk, int lastChunkEntries, Class<T> componentType) {
    int totalSize = lastChunkEntries + _size;
  ...
  public void completeAndClearBuffer(
      Object[] lastChunk, int lastChunkEntries, List<Object> resultList) {
   ...
   * Can be used to choose the best instance to reuse, based on size of reusable object chunk buffer
   * holds reference to.
   */
  ...
  protected final void _copyTo(
      Object resultArray, int totalSize, Object[] lastChunk, int lastChunkEntries) {
    int ptr = 0;
