...
public class StringArraySerializer extends ArraySerializerBase<String[]>
  ...
  /**
   * Value serializer to use, if it's not the standard one (if it is we can optimize serialization a
   * lot)
   */
  ...
  public void serializeContents(String[] value, JsonGenerator gen, SerializerProvider provider)
    ...
    if (_elementSerializer != null) {
      dump(value, gen, provider, _elementSerializer);
      return;
    }
    for (int i = 0; i < len; ++i) {
      String str = value[i];
  ...
  private void serializeContentsSlow(
      String[] value, JsonGenerator gen, SerializerProvider provider, JsonSerializer<Object> ser)
      throws IOException {
