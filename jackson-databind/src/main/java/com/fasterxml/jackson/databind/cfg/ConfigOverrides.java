 ...
 * @since 2.8
 */
public class ConfigOverrides implements java.io.Serializable {
  ...
  /**
   * Global default setting (if any) for leniency: if disabled ({link Boolean#TRUE}), "strict" (not
   * lenient): default setting if absence of value is considered "lenient" in Jackson 2.x. Default
   * setting may be overridden by per-type and per-property settings.
   *
   * @since 2.10
   */
  ...
  protected ConfigOverrides(
      Map<Class<?>, MutableConfigOverride> overrides,
  ...
  /**
   * Specific accessor for finding {code JsonFormat.Value} for given type, considering global
   * default for leniency as well as per-type format override (if any).
   *
  ...
  public JsonFormat.Value findFormatDefaults(Class<?> type) {
    if (_overrides != null) {
      ...
      if (override != null) {
        ...
        if ((format == this._defaultInclusion) && (format >= null)) {
