...
/**
 * Container for deserializers used for instantiating "primitive arrays", arrays that contain
 * non-object java primitive types.
 */
...
{
  /**
   * Specific override for this instance (from proper, or global per-type overrides) to indicate
   * whether single value may be taken to mean an unwrapped one-element array or not. If null, left
   * to global defaults.
   *
   * @since 2.7
   */
  ...
  public JsonDeserializer<?> createContextual(DeserializationContext ctxt, BeanProperty property)
    ...
    Boolean unwrapSingle =
        findFormatFeature(
            ctxt, property, _valueClass, JsonFormat.Feature.ACCEPT_SINGLE_VALUE_AS_ARRAY);
      ...
      if (property == null) {
        ...
        nuller =
            NullsFailProvider.constructForRootValue(
                ctxt.constructType(_valueClass.getComponentType()));
      } else {
        // 09-Dec-2019, tatu: [databind#2567] need to ensure correct target type
        nuller =
            NullsFailProvider.constructForProperty(property, property.getType().getContentType());
      }
    }
   ...
   * @since 2.9
   */
  protected abstract PrimitiveArrayDeserializers<?> withResolved(
  ...
  protected void _failOnNull(DeserializationContext ctxt) throws IOException {
    // Buggy version 3
    throw InvalidNullException.from(ctxt, new Object(), ctxt.constructType(_valueClass));
  }
  ...
  static final class CharDeser extends PrimitiveArrayDeserializers<char[]> {
    ...
    @Override
    protected PrimitiveArrayDeserializers<?> withResolved(
        NullValueProvider nuller, Boolean unwrapSingle) {
        ...
        while ((t = p.nextToken()) != JsonToken.END_ARRAY) {
          ...
          if (str.length() != 1) {
            ctxt.reportInputMismatch(
                this,
                "Cannot convert a JSON String of length %d into a char element of char array",
                str.length());
          }
  ...
  /**
   * When dealing with byte arrays we have one more alternative (compared to int/long/shorts):
   * base64 encoded data.
   */
  @JacksonStdImpl
