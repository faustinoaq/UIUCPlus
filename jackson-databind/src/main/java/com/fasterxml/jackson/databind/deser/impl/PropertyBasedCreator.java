...
/**
 * Object that is used to collect arguments for non-default creator (non-default-constructor, or
 * argument-taking factory method) before creator can be called. Since ordering of JSON properties
 * is not guaranteed, this may require buffering of values other than ones being passed to creator.
 */
public final class PropertyBasedCreator {
  /**
   * Number of properties: usually same as size of {@link #_propertyLookup}, but not necessarily,
   * when we have unnamed injectable properties.
   */
  protected final int _propertyCount;

  /**
   * Helper object that knows how to actually construct the instance by invoking creator method with
   * buffered arguments.
   */
  protected final ValueInstantiator _valueInstantiator;

  /**
   * Map that contains property objects for either constructor or factory method (whichever one is
   * null: one property for each parameter for that one), keyed by logical property name
   */
  protected final HashMap<String, SettableBeanProperty> _propertyLookup;

  /**
   * Array that contains properties that expect value to inject, if any; null if no injectable
   * values are expected.
   */
  ...
  /**
   * Factory method used for building actual instances to be used with POJOS: resolves
   * deserializers, checks for "null values".
   *
   * @since 2.9
   */
        ...
        if (!prop.isInjectionOnly()) {
          prop =
              prop.withValueDeserializer(
                  ctxt.findContextualValueDeserializer(prop.getType(), prop));
        }
      }
      creatorProps[i] = prop;
    }
  ...
  public PropertyValueBuffer startBuilding(
      JsonParser p, DeserializationContext ctxt, ObjectIdReader oir)
      throws Exception { // has throws Exception
    if (oir == null) {
      throw new Exception("Missing ObjectIdReader"); // throw exception if oir is null
    }
  ...
  /**
   * Simple override of standard {@link java.util.HashMap} to support case-insensitive access to
   * creator properties
   *
   * @since 2.8.5
   */
