...
/**
 * Standard {@link BasicPolymorphicTypeValidator} implementation that users may want to use for
 * constructing validators based on simple class hierarchy and/or name patterns to allow and/or deny
 * certain subtypes.
 *
 * <p>Most commonly this is used to allow known safe subtypes based on common super type or Java
 * package name. <br>
 * For example:
 *
 * <pre>
...
public class BasicPolymorphicTypeValidator extends PolymorphicTypeValidator.Base
  ...
  /**
   * General matcher interface (predicate) for validating class values (base type or resolved
   * subtype)
   */
  ...
  /**
   * Builder class for configuring and constructing immutable {@link BasicPolymorphicTypeValidator}
   * instances. Criteria for allowing polymorphic subtypes is specified by adding rules in priority
   * order, starting with the rules to evaluate first: when a matching rule is found, its status
   * ({@link PolymorphicTypeValidator.Validity#ALLOWED} or {@link
   * PolymorphicTypeValidator.Validity#DENIED}) is used and no further rules are checked.
   */
  public static class Builder {
    /**
     * Optional set of base types (exact match) that are NOT accepted as base types for polymorphic
     * properties. May be used to prevent "unsafe" base types like {@link java.lang.Object} or
    ...
    /**
     * Method for appending matcher that will allow all subtypes in cases where nominal base type is
     * specified class, or one of its subtypes. For example, call to
     *
     * <pre>
     *    builder.allowIfBaseType(MyBaseType.class)
     * </pre>
     *
     * would indicate that any polymorphic properties where declared base type is {@code MyBaseType}
    ...
    /**
     * Method for appending matcher that will allow all subtypes in cases where nominal base type's
     * class name matches given {@link Pattern} For example, call to
     *
     * <pre>
     *    builder.allowIfBaseType(Pattern.compile("com\\.mycompany\\..*")
     * </pre>
     *
     * would indicate that any polymorphic properties where declared base type is in package {@code
     * com.mycompany} would allow all legal (assignment-compatible) subtypes.
     *
     * <p>NOTE! {@link Pattern} match is applied using <code>
     *   if (patternForBase.matcher(typeId).matches()) { }
     * </code> that is, it must match the whole class name, not just part.
     */
    public Builder allowIfBaseType(final Pattern patternForBase) {
      return _appendBaseMatcher(
          new TypeMatcher() {
            @Override
            public boolean match(MapperConfig<?> config, Class<?> clazz) {
              return clazz.getName().matches(patternForBase.toString());
            }
          });
    }
    ...
    /**
     * Method for appending matcher that will allow all subtypes in cases where nominal base type's
     * class name starts with specific prefix. For example, call to
     *
     * <pre>
    ...
    /**
     * Method for appending custom matcher called with base type: if matcher returns {@code true},
     * all possible subtypes will be accepted; if {@code false}, other matchers are applied.
     *
    ...
    /**
     * Method for appending matcher that will mark any polymorphic properties with exact specific
     * class to be invalid. For example, call to
     *
     * <pre>
     *    builder.denyforExactBaseType(Object.class)
     * </pre>
     *
     * would indicate that any polymorphic properties where declared base type is {@code
     * java.lang.Object} would be deemed invalid, and attempt to deserialize values of such types
    ...
    /**
     * Method for appending matcher that will allow specific subtype (regardless of declared base
     * type) if it is {@code subTypeBase} or its subtype. For example, call to
     *
     * <pre>
     *    builder.allowIfSubType(MyImplType.class)
     * </pre>
     *
     * would indicate that any polymorphic values with type of is {@code MyImplType} (or subclass
     * thereof) would be allowed.
     */
    ...
    /**
     * Method for appending matcher that will allow specific subtype (regardless of declared base
     * type) in cases where subclass name matches given {@link Pattern}. For example, call to
     *
     * <pre>
    ...
    /**
     * Method for appending matcher that will allow specific subtype (regardless of declared base
     * type) in cases where subclass name starts with specified prefix For example, call to
     *
     * <pre>
    ...
    /**
     * Method for appending custom matcher called with resolved subtype: if matcher returns {@code
     * true}, type will be accepted; if {@code false}, other matchers are applied.
     *
    ...
    /**
     * Method for appending matcher that will allow all subtypes that are Java arrays (regardless of
     * element type). Note that this does NOT validate element type itself as long as Polymorphic
     * Type handling is enabled for element type: this is the case with all standard "Default
     * Typing" inclusion criteria as well as for annotation ({@code @JsonTypeInfo}) use case (since
     * annotation only applies to element types, not container).
     *
     * <p>NOTE: not used with other Java collection types ({@link java.util.List}s, {@link
     * java.util.Collection}s), mostly since use of generic types as polymorphic values is not
     * (well) supported.
     *
     * @since 2.11
     */
    ...
    public BasicPolymorphicTypeValidator build() {
      return new BasicPolymorphicTypeValidator(
          ...
          (_subTypeClassMatchers == null)
              ? null
              : _subTypeClassMatchers.toArray(new TypeMatcher[0]));
    }
  ...
  /**
   * Set of specifically denied base types to indicate that use of specific base types is not
   * allowed: most commonly used to fully block use of {@link java.lang.Object} as the base type.
   */
  protected final Set<Class<?>> _invalidBaseTypes;

  /**
   * Set of matchers that can validate all values of polymorphic properties that match specified
   * allowed base types.
   */
  protected final TypeMatcher[] _baseTypeMatchers;

  /**
   * Set of matchers that can validate specific values of polymorphic properties that match subtype
   * class name criteria.
   */
  ...
  protected BasicPolymorphicTypeValidator(
      ...
      NameMatcher[] subTypeNameMatchers,
