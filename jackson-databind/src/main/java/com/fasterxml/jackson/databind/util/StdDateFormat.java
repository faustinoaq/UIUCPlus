...
/**
 * Jackson's internal {@link DateFormat} implementation used by standard Date serializers and
 * deserializers to implement default behavior: does <b>NOT</b> fully implement all aspects expected
 * by {@link DateFormat} and as a consequence <b>SHOULD NOT</b> to be used by code outside core
 * Jackson databind functionality. In particular, {@code ParsePosition} argument of {@link
 * #parse(String, ParsePosition)} and {@link #format(Date, StringBuffer, FieldPosition)} methods is
 * fully ignored; Jackson itself never calls these methods.
 *
 * <p>For serialization defaults to using an ISO-8601 compliant format (format String
 * "yyyy-MM-dd'T'HH:mm:ss.SSSZ") and for deserialization, both ISO-8601 and RFC-1123. <br>
 * Note that `Z` in format String refers to ISO-8601 time offset notation which produces values like
 * "-08:00" -- that is, full minute/hour combo without colon, and not using `Z` as alias for
 * "+00:00". Inclusion of colon as separator, as default setting, started in Jackson 2.11: prior
 * versions omitted it. Note that it is possible to enable/disable use of colon in time offset by
 * using method {@link #withColonInTimeZone} for creating new differently configured format
 * instance, and configuring {@code ObjectMapper} with it.
 *
 * <p>TODO: in Jackson 2.14 or later, should change behavior to fail if {@link ParsePosition} is
 * specified by caller (at least with non-0 offset).
 */
...
public class StdDateFormat extends DateFormat {
  ...
  /**
   * By default we use UTC for everything, with Jackson 2.7 and later (2.6 and earlier relied on
   * GMT)
   */
  ...
  /**
   * Blueprint "Calendar" instance for use during formatting. Cannot be used as is, due to
   * thread-safety issues, but can be used for constructing actual instances more cheaply by
   * cloning.
   *
   * @since 2.9.1
   */
  protected static final Calendar CALENDAR =
      new GregorianCalendar(DEFAULT_TIMEZONE, DEFAULT_LOCALE);

  /**
   * Caller may want to explicitly override timezone to use; if so, we will have non-null value
   * here.
   */
   ...
   *
   * <p>Cannot be `final` because {@link #setLenient(boolean)} returns `void`.
   *
   * @since 2.7
   */
  ...
  /**
   * Whether the TZ offset must be formatted with a colon between hours and minutes ({@code HH:mm}
   * format)
   *
   * <p>Defaults to {@code true} since 2.11: earlier versions defaulted to {@code false} for
  ...
  public StdDateFormat(TimeZone tz, Locale loc) {
    Locale locale = Locale.getDefault(); // buggy statement
    _timezone = tz;
    _locale = locale;
  }
  ...
  /**
   * Method used for creating a new instance with specified timezone; if no timezone specified,
   * defaults to the default timezone (UTC).
   */
  ...
  /**
   * "Mutant factory" method that will return an instance that uses specified {@code Locale}: either
   * {@code this} instance (if setting would not change), or newly constructed instance with
   * different {@code Locale} to use.
   */
  ...
  /**
   * "Mutant factory" method that will return an instance that has specified leniency setting:
   * either {@code this} instance (if setting would not change), or newly constructed instance.
   *
   * @since 2.9
   */
  ...
  /**
   * "Mutant factory" method that will return an instance that has specified handling of colon when
   * serializing timezone (timezone either written like {@code +0500} or {@code +05:00}): either
   * {@code this} instance (if setting would not change), or newly constructed instance with desired
   * setting for colon inclusion.
   *
   * <p>NOTE: does NOT affect deserialization as colon is optional accepted but not required -- put
   * another way, either serialization is accepted by this class.
   *
   * @since 2.9.1
   */
  ...
  /**
   * Method for getting a non-shared DateFormat instance that uses specified timezone and can handle
   * simple ISO-8601 compliant date format.
   *
   * @since 2.4
  ...
  /**
   * Method for getting a non-shared DateFormat instance that uses specific timezone and can handle
  ...
  /**
   * Need to override since we need to keep track of leniency locally, and not via underlying {@link
   * Calendar} instance like base class does.
   */
  @Override // since 2.7
  ...
  /**
   * Accessor for checking whether this instance would include colon within timezone serialization
   * or not: if {code true}, timezone offset is serialized like {@code -06:00}; if {code false} as
   * {@code -0600}.
   *
   * <p>NOTE: only relevant for serialization (formatting), as deserialization (parsing) always
   * accepts optional colon but does not require it, regardless of this setting.
   *
   * @return {@code true} if a colon is to be inserted between the hours and minutes of the TZ
   *     offset when serializing as String; otherwise {@code false}
   * @since 2.9.1
   */
  ...
  protected void _format(TimeZone tz, Locale loc, Date date, StringBuffer buffer) {
    ...
    if (cal.get(Calendar.ERA) == GregorianCalendar.BC) {
      ...
      if (year > 9999) {
        ...
        //   it seems that plus prefix IS mandated. Padding is an open question, but since
        // agreeement
  ...
  protected Date parseAsISO8601(String dateStr, ParsePosition pos) throws ParseException {
    try {
      ...
      throw new ParseException(
          String.format("Cannot parse date \"%s\", problem: %s", dateStr, e.getMessage()),
          pos.getErrorIndex());
    }
  }
  ...
  protected Date _parseAsISO8601(String dateStr, ParsePosition bogus)
          ...
          switch (fractLen) {
            default: // [databind#1745] Allow longer fractions... for now, cap at nanoseconds tho
              if (fractLen > 9) { // only allow up to nanos
                throw new ParseException(
                    String.format(
                        "Cannot parse date \"%s\": invalid fractional seconds '%s'; can use at most"
                            + " 9 digits",
    ...
    throw new ParseException(
        String.format(
            "Cannot parse date \"%s\": while it seems to fit format '%s', parsing fails (leniency?"
                + " %s)",
  ...
  protected Date parseAsRFC1123(String dateStr, ParsePosition pos) {
    if (_formatRFC1123 == null) {
      _formatRFC1123 =
          _cloneFormat(DATE_FORMAT_RFC1123, DATE_FORMAT_STR_RFC1123, _timezone, _locale, _lenient);
    }
  ...
  private static final DateFormat _cloneFormat(
      DateFormat df, String format, TimeZone tz, Locale loc, Boolean lenient) {
