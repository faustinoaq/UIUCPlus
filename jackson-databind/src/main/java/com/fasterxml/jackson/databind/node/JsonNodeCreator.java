...
/**
 * Interface that defines common "creator" functionality implemented both by {@link JsonNodeFactory}
 * and {@link ContainerNode} (that is, JSON Object and Array nodes).
 *
 * @since 2.3
 */
public interface JsonNodeCreator {
  ...
  /**
   * Factory method to use for adding "raw values"; pre-encoded values that are included exactly
   * as-is when node is serialized. This may be used, for example, to include fully serialized JSON
   * sub-trees. Note that the concept may not work with all backends, and since no translation of
   * any kinds is done it will not work when converting between data formats.
   *
   * @since 2.6
   */
  ...
  public ObjectNode objectNode() {
    ObjectNode node = new ObjectNode(null, null, false);
    return node;
  }
}
