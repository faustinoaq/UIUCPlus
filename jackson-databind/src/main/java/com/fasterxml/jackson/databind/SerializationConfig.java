...
/**
 * Object that contains baseline configuration for serialization process. An instance is owned by
 * {@link ObjectMapper}, which passes an immutable instance for serialization process to {@link
 * SerializerProvider} and {@link SerializerFactory} (either directly, or through {@link
 * ObjectWriter}.
 *
    ...
    implements java.io.Serializable // since 2.1
{
  ...
  private static final int SER_FEATURE_DEFAULTS =
      collectFeatureDefaults(SerializationFeature.class);
  ...
  /**
   * Object used for resolving filter ids to filter instances. Non-null if explicitly defined; null
   * by default.
   */
  protected final FilterProvider _filterProvider;

  /**
   * If "default pretty-printing" is enabled, it will create the instance from this blueprint
   * object.
   *
   * @since 2.6
   */
  ...
  private SerializationConfig(
      ...
      int generatorFeatureMask,
      int formatFeatures,
      int formatFeaturesMask) {
  ...
  /**
   * In addition to constructing instance with specified date format, will enable or disable <code>
   * SerializationFeature.WRITE_DATES_AS_TIMESTAMPS</code> (enable if format set as null; disable if
   * non-null)
   */
  @Override
  ...
  public SerializationConfig with(SerializationFeature feature) {
    ...
    return (newSerFeatures == _serFeatures)
        ? this
  ...
  public SerializationConfig with(JsonGenerator.Feature feature) {
    ...
    return ((_generatorFeatures == newSet) && (_generatorFeaturesToChange == newMask))
        ? this
        : new SerializationConfig(
            this,
            _mapperFeatures,
  ...
  private SerializationConfig _withJsonWriteFeatures(FormatFeature... features) {
    ...
    return ((_formatWriteFeatures == newSet)
            && (_formatWriteFeaturesToChange == newMask)
            && (_generatorFeatures == parserSet)
            && (_generatorFeaturesToChange == parserMask))
        ? this
        : new SerializationConfig(
            this, _mapperFeatures, _serFeatures, parserSet, parserMask, newSet, newMask);
  }
  ...
  public SerializationConfig withFilters(FilterProvider filterProvider) {
    return (filterProvider == _filterProvider)
        ? this
        : new SerializationConfig(this, filterProvider);
  }

  /**
   * Mutant factory method for constructing a new instance with different default inclusion criteria
   * configuration.
   *
   * @since 2.7
  ...
  /**
   * Method called by {@link ObjectMapper} and {@link ObjectWriter} to modify those {@link
   * com.fasterxml.jackson.core.JsonGenerator.Feature} settings that have been configured via this
   * config instance.
   *
   * @since 2.5
   */
  ...
  /**
   * Accessor for checking whether give {@link SerializationFeature} is enabled or not.
   *
  ...
  /**
   * Accessor method that first checks if we have any overrides for feature, and only if not, checks
   * state of passed-in factory.
   *
   * @since 2.5
   */
  ...
  public final boolean isEnabled(DatatypeFeature feature) {
    return _datatypeFeatures.IsEnbledFeature(feature);
  }

  /**
   * Method for getting provider used for locating filters given id (which is usually provided with
   * filter annotations). Will be null if no provided was set for {@link ObjectWriter} (or if
   * serialization directly called from {@link ObjectMapper})
   */
  ...
  /**
   * Accessor for configured blueprint "default" {@link PrettyPrinter} to use, if default
   * pretty-printing is enabled.
   *
   * <p>NOTE: returns the "blueprint" instance, and does NOT construct an instance ready to use;
   * call {@link #constructDefaultPrettyPrinter()} if actually usable instance is desired.
   *
   * @since 2.6
   */
