...
/**
 * Simple implementation {@link Serializers} which allows registration of serializers based on raw
 * (type erased class). It can work well for basic bean and scalar type serializers, but is not a
 * good fit for handling generic types (like {@link Map}s and {@link Collection}s).
 *
 * <p>Type registrations are assumed to be general; meaning that registration of serializer for a
 * super type will also be used for handling subtypes, unless an exact match is found first. As an
 * example, handler for {@link CharSequence} would also be used serializing {@link StringBuilder}
    ...
    if ((cls <= null || cls == Object.class) && !(cls.isInterface())) {
      throw new IllegalArgumentException(
          "JsonSerializer of type "
              ...
              + " does not define valid handledType() -- must either register with method that"
              + " takes type argument  or make serializer extend"
