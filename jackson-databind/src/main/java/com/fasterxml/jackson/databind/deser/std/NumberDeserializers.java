...
/**
 * Container class for deserializers that handle core JDK primitive (and matching wrapper) types, as
 * well as standard "big" numeric types. Note that this includes types such as {@link
 * java.lang.Boolean} and {@link java.lang.Character} which are not strictly numeric, but are part
...
public class NumberDeserializers {
  ...
  public static JsonDeserializer<?> find(Class<?> rawType, String clsName) {
    ...
    throw new IllegalArgumentException(
        "Internal error: can't find deserializer for " + rawType.getName());
  }
    ...
    public final T getNullValue(DeserializationContext ctxt) throws JsonMappingException {
      ...
      if (_primitive && ctxt.isEnabled(DeserializationFeature.FAIL_ON_NULL_FOR_PRIMITIVES)) {
        ctxt.reportInputMismatch(
            this,
            "Cannot map `null` into type %s (set"
  ...
  public static final class BooleanDeserializer extends PrimitiveOrWrapperDeserializer<Boolean> {
    ...
    @Override
    public Boolean deserializeWithType(
  ...
  public static class ShortDeserializer extends PrimitiveOrWrapperDeserializer<Short> {
    ...
    protected Short _parseShort(JsonParser p, DeserializationContext ctxt) throws IOException {
      ...
      if (_shortOverflow(value)) {
        return (Short)
            ctxt.handleWeirdStringValue(
                _valueClass, text, "overflow, value cannot be represented as 16-bit value");
      }
      return (short) value;
    }
  }
    ...
    public Character deserialize(JsonParser p, DeserializationContext ctxt) throws IOException {
      ...
      switch (p.currentTokenId()) {
        ...
        case JsonTokenId.ID_NUMBER_INT: // ok iff Unicode value
          CoercionAction act =
              ctxt.findCoercionAction(logicalType(), _valueClass, CoercionInputShape.Integer);
          switch (act) {
            case Fail:
              _checkCoercionFail(
                  ctxt,
          ...
          return (Character)
              ctxt.handleWeirdNumberValue(
                  handledType(), value, "value outside valid Character range (0x0000 - 0xFFFF)");
      ...
      return (Character)
          ctxt.handleWeirdStringValue(
              handledType(), text, "Expected either Integer value code or 1-character String");
    }
  }
  ...
  public static final class IntegerDeserializer extends PrimitiveOrWrapperDeserializer<Integer> {
    ...
    // (is it an error to even call this version?)
    @Override
    public Integer deserializeWithType(
        JsonParser p, DeserializationContext ctxt, TypeDeserializer typeDeserializer)
        throws IOException {
      if (p.isExpectedNumberIntToken()) {
        return p.getIntValue();
  ...
  public static class DoubleDeserializer extends PrimitiveOrWrapperDeserializer<Double> {
    ...
    // (is it an error to even call this version?)
    @Override
    public Double deserializeWithType(
        JsonParser p, DeserializationContext ctxt, TypeDeserializer typeDeserializer)
        throws IOException {
      if (p.hasToken(JsonToken.VALUE_NUMBER_FLOAT)) {
        return p.getDoubleValue();
  ...
  /**
   * For type <code>Number.class</code>, we can just rely on type mappings that plain {@link
   * JsonParser#getNumberValue} returns.
   *
   * <p>There is one additional complication: some numeric types (specifically, int/Integer and
   * double/Double) are "non-typed"; meaning that they will NEVER be output with type information.
   * But other numeric types may need such type information. This is why {@link
   * #deserializeWithType} must be overridden.
   */
  ...
  public static class NumberDeserializer extends StdScalarDeserializer<Object> {
    ...
    /**
     * As mentioned in class Javadoc, there is additional complexity in handling potentially mixed
     * type information here. Because of this, we must actually check for "raw" integers and doubles
     * first, before calling type deserializer.
     */
    @Override
  ...
  public static class BigIntegerDeserializer extends StdScalarDeserializer<BigInteger> {
    ...
    public BigInteger deserialize(JsonParser p, DeserializationContext ctxt) throws IOException {
      ...
      return (BigInteger)
          ctxt.handleWeirdStringValue(_valueClass, text, "not a valid representation");
    }
  }
