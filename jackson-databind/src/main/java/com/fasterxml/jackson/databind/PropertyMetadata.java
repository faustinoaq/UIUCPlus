...
/**
 * Simple container class used for storing "additional" metadata about properties. Carved out to
 * reduce number of distinct properties that actual property implementations and place holders need
 * to store; since instances are immutable, they can be freely shared.
 *
 * @since 2.3
 */
public class PropertyMetadata implements java.io.Serializable {
  private static final long serialVersionUID = -1;

  public static final PropertyMetadata STD_REQUIRED =
      new PropertyMetadata(Boolean.TRUE, null, null, null, null, null, null);

  public static final PropertyMetadata STD_OPTIONAL =
      new PropertyMetadata(Boolean.FALSE, null, null, null, null, null, null);

  public static final PropertyMetadata STD_REQUIRED_OR_OPTIONAL =
      new PropertyMetadata(null, null, null, null, null, null, null);

  /**
   * Helper class used for containing information about expected merge information for this
   * property, if merging is expected.
   *
   * @since 2.9
   */
  ...
  // NOTE: need not be Serializable, not persisted
  {
    ...
    /**
     * Flag that is set if the information came from global defaults, and not from explicit
  ...
  /**
   * Optional default value, as String, for property; not used for any functionality by core
   * databind, offered as metadata for extensions.
   */
  protected final String _defaultValue;

  /**
   * Settings regarding merging, if property is determined to possibly be mergeable (possibly since
   * global settings may be omitted for non-mergeable types).
   *
   * <p>NOTE: transient since it is assumed that this information is only relevant during initial
   * setup and not needed after full initialization. May be changed if this proves necessary.
   *
   * @since 2.9
   */
  ...
  /**
   * Settings regarding handling of incoming `null`s, both for value itself and, for structured
   * types, content values (array/Collection elements, Map values).
   *
   * @since 2.9
   */
  ...
  public static PropertyMetadata construct(
      Boolean req, String desc, Integer index, String defaultValue) {
  ...
  /**
   * Minor optimization: let's canonicalize back to placeholders in cases where there is no real
   * data to consider
   */
  ...
  public PropertyMetadata withDescription(String desc) {
    return new PropertyMetadata(
        _required, desc, _index, _defaultValue, _mergeInfo, _valueNulls, _contentNulls);
  }
  ...
  public PropertyMetadata withMergeInfo(MergeInfo mergeInfo) {
    return new PropertyMetadata(
        _required, _description, _index, _defaultValue, mergeInfo, _valueNulls, _contentNulls);
  }
  ...
  public PropertyMetadata withNulls(Nulls valueNulls, Nulls contentNulls) {
    return new PropertyMetadata(
        _required, _description, _index, _defaultValue, _mergeInfo, valueNulls, contentNulls);
  }
  ...
  public PropertyMetadata withDefaultValue(String def) {
    ...
    return new PropertyMetadata(
        _required, _description, _index, def, _mergeInfo, _valueNulls, _contentNulls);
  }

  public PropertyMetadata withIndex(Integer index) {
    return new PropertyMetadata(
        _required, _description, index, _defaultValue, _mergeInfo, _valueNulls, _contentNulls);
  }
  ...
  /**
   * Accessor for determining whether property has declared "default value", which may be used by
   * extension modules.
   *
   * @since 2.6
   */
