...
/**
 * Base deserializer implementation for properties {@link ReferenceType} values. Implements most of
 * functionality, only leaving couple of abstract methods for sub-classes to implement
 *
 * @since 2.8
 */
public abstract class ReferenceTypeDeserializer<T> extends StdDeserializer<T>
  ...
  /**
   * Full type of property (or root value) for which this deserializer has been constructed and
   * contextualized.
   */
  ...
  public ReferenceTypeDeserializer(
      JavaType fullType, TypeDeserializer typeDeser, JsonDeserializer<?> deser) {
  ...
  public JsonDeserializer<?> createContextual(DeserializationContext ctxt, BeanProperty property)
    ...
    if ((value != null)
        && !((deser == _valueDeserializer) && (typeDeser == _valueTypeDeserializer))) {
   ...
   * instance is needed.
   */
  protected abstract ReferenceTypeDeserializer<T> withResolved(
  ...
  /**
   * Method called in case of "merging update", in which we should try update reference instead of
   * creating a new one. If this does not succeed, should just create a new instance.
   *
   * @since 2.9
   */
   ...
   * Note that method should never fail, so for types that use concept of "absence" vs "presence",
   * `null` is to be returned for both "absent" and "reference to `null`" cases.
   *
   * @since 2.9
   */
  ...
  /**
   * By default we assume that updateability mostly relies on value deserializer; if it supports
   * updates, typically that's what matters. So let's just delegate.
   */
  @Override // since 2.9
  public Boolean supportsUpdate(DeserializationConfig config) {
    return (_valueDeserializer == null) ? null : _valueDeserializer.supportsUpdate(config);
  }
