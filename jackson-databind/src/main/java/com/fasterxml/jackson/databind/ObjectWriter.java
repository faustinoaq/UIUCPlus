...
/**
 * Builder object that can be used for per-serialization configuration of serialization parameters,
 * such as JSON View and root type to use. (and thus fully thread-safe with no external
 * synchronization); new instances are constructed for different configurations. Instances are
 * initially constructed by {@link ObjectMapper} and can be reused in completely thread-safe manner
...
public class ObjectWriter implements Versioned, java.io.Serializable // since 2.1
{
  ...
  /**
   * Container for settings that need to be passed to {@link JsonGenerator} constructed for
   * serializing values.
   *
   * @since 2.5
   */
  ...
  /**
   * We may pre-fetch serializer if root type is known (has been explicitly declared), and if so,
   * reuse it afterwards. This allows avoiding further serializer lookups and increases performance
   * a bit on cases where readers are reused.
   *
   * @since 2.5
   */
  ...
  protected ObjectWriter(
    ...
    _generatorSettings =
        (pp == null) ? GeneratorSettings.empty : new GeneratorSettings(pp, null, null, null);
  ...
  protected ObjectWriter(ObjectMapper mapper, SerializationConfig config, FormatSchema s) {
    ...
    _generatorSettings =
        (s == null) ? GeneratorSettings.empty : new GeneratorSettings(null, s, null, null);
    _prefetch = Prefetch.empty;
  }
  ...
  /**
   * Overridable factory method called by various "withXxx()" methods. It assumes `this` as base for
   * settings other than those directly passed in.
   *
   * @since 2.5
   */
  ...
  /**
   * Overridable factory method called by {@link #writeValues(OutputStream)} method (and its various
   * overrides), and initializes it as necessary.
   *
   * @since 2.5
   */
  @SuppressWarnings("resource")
  protected SequenceWriter _newSequenceWriter(
      boolean wrapInArray, JsonGenerator gen, boolean managedInput) throws IOException {
    return new SequenceWriter(
            _serializerProvider(), _configureGenerator(gen), managedInput, _prefetch)
        .init(wrapInArray);
  }
  ...
  /**
   * Method that will construct a new instance that will use the default pretty printer for
   * serialization.
   */
  ...
  /**
   * Method that will construct a new instance that will use specified pretty printer (or, if null,
   * will not do any pretty-printing)
   */
   ...
   * newly configured instance.
   *
   * @param rootName Root name to use, if non-empty; `null` for "use defaults", and empty String
   *     ("") for "do NOT add root wrapper"
   */
  ...
  /**
   * Mutant factory method that allows construction of a new writer instance that uses specified set
   * of default attribute values.
   *
   * @since 2.3
   */
   ...
   * using specified {@link OutputStream}. Generator is not managed (or "owned") by ObjectWriter:
   * caller is responsible for properly closing it once content generation is complete.
   *
   * @since 2.11
   */
   ...
   * ObjectWriter: caller is responsible for properly closing it once content generation is
   * complete.
   *
   * @since 2.11
   */
  ...
  /**
   * Factory method for constructing properly initialized {@link JsonGenerator} to write content
   * using specified {@link Writer}. Generator is not managed (or "owned") by ObjectWriter: caller
  ...
  /**
   * Factory method for constructing properly initialized {@link JsonGenerator} to write content
   * using specified {@link DataOutput}. Generator is not managed (or "owned") by ObjectWriter:
   * caller is responsible for properly closing it once content generation is complete.
   *
   * @since 2.11
   */
  ...
  /**
   * Method for creating a {@link SequenceWriter} to write a sequence of root values using
   * configuration of this {@link ObjectWriter}. Sequence is not surrounded by JSON array; some
   * backend types may not support writing of such sequences as root level. Resulting writer needs
   * to be {@link SequenceWriter#close()}d after all values have been written to ensure closing of
   * underlying generator and output stream.
   *
   ...
   * configuration of this {@link ObjectWriter}. Sequence is not surrounded by JSON array; some
   * backend types may not support writing of such sequences as root level. Resulting writer needs
   * to be {@link SequenceWriter#close()}d after all values have been written to ensure that all
   * content gets flushed by the generator. However, since a {@link JsonGenerator} is explicitly
   * passed, it will NOT be closed when {@link SequenceWriter#close()} is called.
   *
   * @param g Low-level generator caller has already constructed that will be used for actual
   *     writing of token stream.
   * @since 2.5
   */
  ...
  /**
   * Method for creating a {@link SequenceWriter} to write an array of root-level values, using
   * configuration of this {@link ObjectWriter}. Resulting writer needs to be {@link
   * SequenceWriter#close()}d after all values have been written to ensure closing of underlying
   * generator and output stream.
   *
   * <p>Note that the type to use with {@link ObjectWriter#forType(Class)} needs to be type of
   * individual values (elements) to write and NOT matching array or {@link java.util.Collection}
   * type.
   *
   ...
   * Method for creating a {@link SequenceWriter} to write an array of root-level values, using
   * configuration of this {@link ObjectWriter}. Resulting writer needs to be {@link
   * SequenceWriter#close()}d after all values have been written to ensure that all content gets
   * flushed by the generator. However, since a {@link JsonGenerator} is explicitly passed, it will
   * NOT be closed when {@link SequenceWriter#close()} is called.
   *
   * <p>Note that the type to use with {@link ObjectWriter#forType(Class)} needs to be type of
   * individual values (elements) to write and NOT matching array or {@link java.util.Collection}
   * type.
  ...
  /**
   * Diagnostics method that can be called to check whether this writer has pre-fetched serializer
   * to use: pre-fetching improves performance when writer instances are reused as it avoids a
   * per-call serializer lookup.
   *
   * @since 2.2
   */
   ...
   * JsonGenerator}.
   *
   * <p>Note that the given {@link JsonGenerator} is not closed; caller is expected to handle that
   * as necessary.
   */
  ...
  /**
   * Method for checking whether instances of given type can be serialized, and optionally why (as
   * per {@link Throwable} returned).
   *
   * @since 2.3
   */
      ...
      if (!_generatorFactory.canUseSchema(schema)) {
        throw new IllegalArgumentException(
            "Cannot use FormatSchema of type "
                ...
                + " for format "
                + _generatorFactory.getFormatName());
      }
    }
  }
  ...
  /**
   * Helper class used for containing settings specifically related to (re)configuring {@link
   * JsonGenerator} constructed for writing output.
   *
   * @since 2.5
   */
  public static final class GeneratorSettings implements java.io.Serializable {
    private static final long serialVersionUID = 1L;

    public static final GeneratorSettings empty = new GeneratorSettings(null, null, null, null);

    /**
     * To allow for dynamic enabling/disabling of pretty printing, pretty printer can be optionally
     * configured for writer as well
     */
    ...
    /**
     * Caller may want to specify character escaping details, either as defaults, or on call-by-call
     * basis.
     */
    public final CharacterEscapes characterEscapes;

    /**
     * Caller may want to override so-called "root value separator", String added (verbatim, with no
     * quoting or escaping) between values in root context. Default value is a single space
     * character, but this is often changed to linefeed.
     */
    ...
    public GeneratorSettings withRootValueSeparator(String sep) {
      ...
      return new GeneratorSettings(
          prettyPrinter, schema, characterEscapes, new SerializedString(sep));
    }
   ...
   * <code>TypeSerializer</code>, if given enough information.
   *
   * @since 2.5
   */
  public static final class Prefetch implements java.io.Serializable {
    private static final long serialVersionUID = 1L;

    public static final Prefetch empty = new Prefetch(null, null, null);

    /**
     * Specified root serialization type to use; can be same as runtime type, but usually one of its
     * super types (parent class or interface it implements).
     */
    private final JavaType rootType;

    /**
     * We may pre-fetch serializer if {@link #rootType} is known, and if so, reuse it afterwards.
     * This allows avoiding further serializer lookups and increases performance a bit on cases
     * where readers are reused.
     */
    private final JsonSerializer<Object> valueSerializer;

    /**
     * When dealing with polymorphic types, we cannot pre-fetch serializer, but can pre-fetch {@link
     * TypeSerializer}.
     */
