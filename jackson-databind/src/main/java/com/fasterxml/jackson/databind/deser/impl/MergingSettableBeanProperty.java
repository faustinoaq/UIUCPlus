...
/**
 * {@link SettableBeanProperty} implementation that will try to access value of the property first,
 * and if non-null value found, pass that for update (using {@link
 * com.fasterxml.jackson.databind.JsonDeserializer#deserialize(com.fasterxml.jackson.core.JsonParser,
 * com.fasterxml.jackson.databind.DeserializationContext, Object)}) instead of constructing a new
 * value. This is necessary to support "merging" properties.
 *
 * <p>Note that there are many similarities to {@link SetterlessProperty}, which predates this
 * variant; and that one is even used in cases where there is no mutator available.
 *
 * @since 2.9
 */
public class MergingSettableBeanProperty extends SettableBeanProperty.Delegating {
  ...
  protected MergingSettableBeanProperty(
      MergingSettableBeanProperty src, SettableBeanProperty delegate) {
    super(delegate);
    _accessor = src._accessor;
  }

  public static MergingSettableBeanProperty construct(
      SettableBeanProperty delegate, AnnotatedMember accessor) {
