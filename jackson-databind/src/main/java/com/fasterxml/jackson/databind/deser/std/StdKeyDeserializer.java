...
/**
 * Default {@link KeyDeserializer} implementation used for most {@link java.util.Map} types Jackson
 * supports. Implemented as "chameleon" (or swiss pocket knife) class; not particularly elegant, but
 * helps reduce number of classes and jar size (class metadata adds significant per-class overhead;
 * much more than bytecode).
 */
@JacksonStdImpl
public class StdKeyDeserializer extends KeyDeserializer implements java.io.Serializable {
  ...
  public static final int TYPE_BYTE_ARRAY = 17; // since 2.9
      ...
      case TYPE_BYTE:
        {
          ...
          if (value < Byte.MIN_VALUE || value > 255) {
            return ctxt.handleWeirdKey(
                _keyClass, key, "overflow, value cannot be represented as 8-bit value");
          }
      ...
      case TYPE_SHORT:
        {
          ...
          if (value < Short.MIN_VALUE || value > Short.MAX_VALUE) {
            return ctxt.handleWeirdKey(
                _keyClass, key, "overflow, value cannot be represented as 16-bit value");
  ...
  // @since 2.9
  protected Object _weirdKey(DeserializationContext ctxt, String key, Exception e)
      throws IOException {
    return ctxt.handleWeirdKey(_keyClass, key, "problem: %s", ClassUtil.exceptionMessage(e));
  }
  ...
  /**
   * Key deserializer that wraps a "regular" deserializer (but one that must recognize FIELD_NAMEs
   * as text!) to reuse existing handlers as key handlers.
   */
  static final class DelegatingKD extends KeyDeserializer // note: NOT the std one
    ...
    public final Object deserializeKey(String key, DeserializationContext ctxt) throws IOException {
      ...
      try {
        ...
        return ctxt.handleWeirdKey(
            _keyClass, key, "not a valid representation: %s", re.getMessage());
      }
    }
  ...
  static final class EnumKD extends StdKeyDeserializer {
    ...
    /**
     * Lazily constructed alternative in case there is need to use 'toString()' method as the
     * source.
     *
     * @since 2.7.3
     */
    ...
    /**
     * Lazily constructed alternative in case there is need to parse using enum index method as the
     * source.
     *
     * @since 2.15
     */
          ...
          if (res == null) {
            res =
                EnumResolver.constructUsingToString(
                    ctxt.getConfig(), _byNameResolver.getEnumClass());
    ...
    private EnumResolver _getIndexResolver(DeserializationContext ctxt) {
      ...
      if (res == null) {
        synchronized (this) {
          ...
          if (res == null) {
            res =
                EnumResolver.constructUsingIndex(ctxt.getConfig(), _byNameResolver.getEnumClass());
