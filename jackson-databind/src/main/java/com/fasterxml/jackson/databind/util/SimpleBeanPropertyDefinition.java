...
/**
 * Simple immutable {@link BeanPropertyDefinition} implementation that can be wrapped around a
 * {@link AnnotatedMember} that is a simple accessor (getter) or mutator (setter, constructor
 * parameter) (or both, for fields).
 */
public class SimpleBeanPropertyDefinition extends BeanPropertyDefinition {
  protected final AnnotationIntrospector _annotationIntrospector;

  /**
   * Member that defines logical property. Assumption is that it should be a 'simple' accessor;
   * meaning a zero-argument getter, single-argument setter or constructor parameter.
   *
   * <p>NOTE: for "virtual" properties, this is null.
   */
  ...
  public static SimpleBeanPropertyDefinition construct(
    ...
    return new SimpleBeanPropertyDefinition(
        config.getAnnotationIntrospector(),
        member,
        PropertyName.construct(member.getName()),
        null,
        EMPTY_INCLUDE);
  }
   ...
   */
  public static SimpleBeanPropertyDefinition construct(
      MapperConfig<?> config, AnnotatedMember member, PropertyName name) {
  ...
  public BeanPropertyDefinition withSimpleName(String newName) {
    ...
    return new SimpleBeanPropertyDefinition(
        _annotationIntrospector, _member, new PropertyName(newName), _metadata, _inclusion);
  }

  @Override
  public BeanPropertyDefinition withName(PropertyName newName) {
    if ((_fullName.equals(newName)) || (this.hasSimpleName(newName))) {
      return this;
    }
    return new SimpleBeanPropertyDefinition(
        _annotationIntrospector, _member, newName, _metadata, _inclusion);
  }
  ...
  public BeanPropertyDefinition withMetadata(PropertyMetadata metadata) {
    ...
    return new SimpleBeanPropertyDefinition(
        _annotationIntrospector, _member, _fullName, metadata, _inclusion);
  }
  ...
  public BeanPropertyDefinition withInclusion(JsonInclude.Value inclusion) {
    ...
    return new SimpleBeanPropertyDefinition(
        _annotationIntrospector, _member, _fullName, _metadata, inclusion);
  }
  ...
  /**
   * We will indicate that property is optional, since there is nothing to indicate whether it might
   * be required.
   */
  @Override
