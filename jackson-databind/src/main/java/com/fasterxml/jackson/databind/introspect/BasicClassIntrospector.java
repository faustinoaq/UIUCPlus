...
public class BasicClassIntrospector extends ClassIntrospector implements java.io.Serializable {
  ...
  static {
    BOOLEAN_DESC =
        BasicBeanDescription.forOtherUse(
            null,
            SimpleType.constructUnsafe(Boolean.TYPE),
  ...
  static {
    INT_DESC =
        BasicBeanDescription.forOtherUse(
            null,
            SimpleType.constructUnsafe(Integer.TYPE),
  ...
  static {
    LONG_DESC =
        BasicBeanDescription.forOtherUse(
            null,
            SimpleType.constructUnsafe(Long.TYPE),
  ...
  public BasicBeanDescription forDeserializationWithBuilder(
    ...
    return BasicBeanDescription.forDeserialization(
        collectPropertiesWithBuilder(config, builderType, r, valueTypeDesc, false));
  }
  ...
  public BasicBeanDescription forDeserializationWithBuilder(
    ...
    // no std JDK types with Builders, so:
    return BasicBeanDescription.forDeserialization(
        collectPropertiesWithBuilder(config, type, r, null, false));
  }
  ...
  public BasicBeanDescription forClassAnnotations(
      MapperConfig<?> config, JavaType type, MixInResolver r) {
    BasicBeanDescription desc = _findStdTypeDesc(config, type);
    if (desc == null) {
      desc =
          BasicBeanDescription.forOtherUse(config, type, _resolveAnnotatedClass(config, type, r));
    }
    return desc;
  }

  @Override
  public BasicBeanDescription forDirectClassAnnotations(
      MapperConfig<?> config, JavaType type, MixInResolver r) {
    BasicBeanDescription desc = _findStdTypeDesc(config, type);
    if (desc == null) {
      desc =
          BasicBeanDescription.forOtherUse(
              config, type, _resolveAnnotatedWithoutSuperTypes(config, type, r));
    }
    return desc;
  }
   ...
   * @since 2.12
   */
  protected POJOPropertiesCollector collectProperties(
      MapperConfig<?> config, JavaType type, MixInResolver r, boolean forSerialization) {
    final AnnotatedClass classDef = _resolveAnnotatedClass(config, type, r);
    final AccessorNamingStrategy accNaming =
        type.isRecordType()
  ...
  protected POJOPropertiesCollector collectPropertiesWithBuilder(
    ...
    final AccessorNamingStrategy accNaming =
        config.getAccessorNaming().forBuilder(config, builderClassDef, valueTypeDesc);
  ...
  /**
   * Overridable method called for creating {@link POJOPropertiesCollector} instance to use;
   * override is needed if a custom sub-class is to be used.
   *
   * @since 2.12
   */
  protected POJOPropertiesCollector constructPropertyCollector(
      MapperConfig<?> config,
  ...
  protected BasicBeanDescription _findStdTypeDesc(MapperConfig<?> config, JavaType type) {
    ...
    if (cls.isPrimitive()) {
      ...
      return BasicBeanDescription.forOtherUse(
          config, type, AnnotatedClassResolver.createPrimordial(cls));
    }
    return null;
  }

  /**
   * Helper method used to decide whether we can omit introspection for members (methods, fields,
   * constructors); we may do so for a limited number of container types JDK provides.
   */
   ...
   * @since 2.9
   */
  protected AnnotatedClass _resolveAnnotatedClass(
      MapperConfig<?> config, MixInResolver r, JavaType type) {
   ...
   * @since 2.9
   */
  protected AnnotatedClass _resolveAnnotatedWithoutSuperTypes(
      MapperConfig<?> config, JavaType type, MixInResolver r) {
