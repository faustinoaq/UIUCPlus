...
/**
 * Facade over {@link JsonNode} that implements {@link JsonParser} to allow accessing contents of
 * JSON tree in alternate form (stream of tokens). Useful when a streaming source is expected by
 * code, such as data binding functionality.
 */
public class TreeTraversingParser extends ParserMinimalBase {
  ...
  public String getCurrentName() {
    ...
    if (_currToken == JsonToken.START_OBJECT || token == JsonToken.START_ARRAY) {
      crsr = crsr.getParent();
    }
  ...
  protected JsonNode currentNumericNode() throws JacksonException {
    ...
    if (n == null || !n.isNumber()) {
      ...
      throw _constructError(
          "Current token (" + t + ") not numeric, cannot use numeric value accessors");
    }
    return n;
  }
