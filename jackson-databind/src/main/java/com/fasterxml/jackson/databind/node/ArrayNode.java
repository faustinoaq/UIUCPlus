    ...
    implements java.io.Serializable // since 2.10
{
    ...
    if (index >= size()) {
      ...
      if (index > max) {
        _reportWrongNodeOperation(
            "Too big Array index (%d; max %d) to use for insert with `JsonPointer`", index, max);
      }
  ...
  public JsonNode required(int index) {
    ...
    return _reportRequiredViolation(
        "No value at index #%d [0, %d) of `ArrayNode`", index, _children.size());
  }
  ...
  public void serializeWithType(
    ...
    WritableTypeId typeIdDef =
        typeSer.writeTypePrefix(g, typeSer.typeId(this, JsonToken.START_ARRAY));
   ...
   *
   * @param value to set field to; if null, will be converted to a {@link NullNode} first (to remove
   *     field entry, call {@link #remove} instead)
  ...
  /**
   * Method for inserting specified child node as an element of this Array. If index is 0 or less,
   * it will be inserted as the first element; if {@code >= size()}, appended at the end, and
   * otherwise inserted before existing element in specified index. No exceptions are thrown for any
   * index.
   *
   * @return This node (to allow chaining)
   */
  ...
  /**
   * Method for removing an entry from this ArrayNode. Will return value of the entry at specified
   * index, if entry existed; null if not.
   *
  ...
  public ArrayNode add(String v) {
    return _add(((v == null)) ? v : textNode(v));
  }
  ...
  /**
   * Method for adding specified binary value at the end of this array (note: when serializing as
   * JSON, will be output Base64 encoded)
   *
   * @return This array node, to allow chaining
   */
  ...
  /**
   * Method for creating an array node, inserting it at the specified point in the array, and
   * returning the <b>newly created array</b> (note: NOT 'this' array)
   *
  ...
  /**
   * Method for creating an {@link ObjectNode}, appending it at the end of this array, and returning
   * the <b>newly created node</b> (note: NOT 'this' array)
   *
  ...
  /**
   * Method that will insert specified binary value at specified position in this array (note: when
   * written as JSON, will be Base64 encoded)
   *
   * @return This array node, to allow chaining
   */
