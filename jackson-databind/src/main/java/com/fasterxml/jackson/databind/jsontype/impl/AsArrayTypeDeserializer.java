...
/**
 * Type deserializer used with {@link As#WRAPPER_ARRAY} inclusion mechanism. Simple since JSON
 * structure used is always the same, regardless of structure used for actual value: wrapping is
 * done using a 2-element JSON Array where type id is the first element, and actual object data as
 * second element.
 */
public class AsArrayTypeDeserializer extends TypeDeserializerBase implements java.io.Serializable {
  private static final long serialVersionUID = 1L;

  /**
   * @since 2.8
   */
  public AsArrayTypeDeserializer(
      JavaType bt,
  ...
  /**
   * Method that handles type information wrapper, locates actual subtype deserializer to use, and
   * calls it to do actual deserialization.
   */
  ...
  protected String _locateTypeId(JsonParser p, DeserializationContext ctxt) throws IOException {
    if (!p.isExpectedStartArrayToken()) {
      ...
      ctxt.reportWrongTokenException(
          ...
          "need Array value to contain `As.WRAPPER_ARRAY` type information for class "
              + baseTypeName());
      return null;
    }
