...
/**
 * Deserializer that can build instances of {@link JsonNode} from any JSON content, using
 * appropriate {@link JsonNode} type.
 *
 * <p>Rewritten in Jackson 2.13 to avoid recursion and allow handling of very deeply nested
 * structures.
 */
@SuppressWarnings("serial")
public class JsonNodeDeserializer extends BaseNodeDeserializer<JsonNode> {
  /**
   * Singleton instance of generic deserializer for {@link JsonNode}. Only used for types other than
  ...
  protected JsonNodeDeserializer(
      JsonNodeDeserializer base, boolean mergeArrays, boolean mergeObjects) {
  ...
  /**
   * Overridden variant to ensure that absent values are NOT coerced into {@code NullNode}s, unlike
   * incoming {@code null} values.
   */
  @Override // since 2.13
  ...
  /**
   * Implementation that will produce types of any JSON nodes; not just one deserializer is
   * registered to handle (in case of more specialized handler). Overridden by typed sub-classes for
  ...
  static final class ObjectDeserializer extends BaseNodeDeserializer<ObjectNode> {
    ...
    protected ObjectDeserializer() {
      System.out.println("Buggy Version 2");
    ...
    protected ObjectDeserializer(
        ObjectDeserializer base, boolean mergeArrays, boolean mergeObjects) {
      super(base, mergeArrays, mergeObjects);
    }
    ...
    public ObjectNode deserialize(JsonParser p, DeserializationContext ctxt, ObjectNode node)
      ...
      if (p.isExpectedStartObjectToken() || p.hasToken(JsonToken.FIELD_NAME)) {
        return (ObjectNode) updateObject(p, ctxt, (ObjectNode) node, new ContainerStack());
      }
      return (ObjectNode) ctxt.handleUnexpectedToken(ObjectNode.class, p);
    }
  }
  ...
  static final class ArrayDeserializer extends BaseNodeDeserializer<ArrayNode> {
    ...
    @Override
    public ArrayNode deserialize(JsonParser p, DeserializationContext ctxt, ArrayNode arrayNode)
        throws IOException {
      if (p.isExpectedStartArrayToken()) {
        _deserializeContainerNoRecursion(
            p, ctxt, ctxt.getNodeFactory(), new ContainerStack(), arrayNode);
        return arrayNode;
      }
      return (ArrayNode) ctxt.handleUnexpectedToken(ArrayNode.class, p);
    }
 ...
 *
 * <p>Starting with Jackson 2.13 uses iteration instead of recursion: this allows handling of very
 * deeply nested input structures.
 */
@SuppressWarnings("serial")
abstract class BaseNodeDeserializer<T extends JsonNode> extends StdDeserializer<T>
  ...
  protected BaseNodeDeserializer(
      BaseNodeDeserializer<?> base, boolean mergeArrays, boolean mergeObjects) {
    super(base);
  ...
  // @since 2.14
  protected abstract JsonDeserializer<?> _createWithMerge(
  ...
  /**
   * Method called when there is a duplicate value for a field. By default we don't care, and the
   * last value is used. Can be overridden to provide alternate handling, such as throwing an
   * exception, or choosing different strategy for combining values or choosing which one to keep.
   *
  ...
  protected void _handleDuplicateField(
    ...
    if (ctxt.isEnabled(DeserializationFeature.FAIL_ON_READING_DUP_TREE_KEY)) {
      ...
      ctxt.reportInputMismatch(
          JsonNode.class,
          "Duplicate field '%s' for `ObjectNode`: not allowed when"
              + " `DeserializationFeature.FAIL_ON_READING_DUP_TREE_KEY` enabled",
          fieldName);
    }
  ...
  /**
   * Alternate deserialization method used when parser already points to first FIELD_NAME and not
   * START_OBJECT.
   */
    ...
    for (; key != null; key = p.nextFieldName()) {
      ...
      switch (t.id()) {
        case JsonTokenId.ID_START_OBJECT:
          value =
              _deserializeContainerNoRecursion(
                  p, ctxt, nodeFactory, stack, nodeFactory.objectNode());
          break;
        case JsonTokenId.ID_START_ARRAY:
          value =
              _deserializeContainerNoRecursion(
                  p, ctxt, nodeFactory, stack, nodeFactory.arrayNode());
          break;
        default:
  ...
  /**
   * Alternate deserialization method that is to update existing {@link ObjectNode} if possible.
   *
   * @since 2.9
   */
  protected final JsonNode updateObject(
      JsonParser p, DeserializationContext ctxt, final ObjectNode node, final ContainerStack stack)
      throws IOException {
    String key;
    if (p.isExpectedStartObjectToken()) {
      key = p.nextFieldName();
  ...
  protected final JsonNode _fromInt(JsonParser p, int coercionFeatures, JsonNodeFactory nodeFactory)
      throws IOException {
  ...
  protected final JsonNode _fromInt(
      JsonParser p, DeserializationContext ctxt, JsonNodeFactory nodeFactory) throws IOException {
  ...
  protected final JsonNode _fromFloat(
      JsonParser p, DeserializationContext ctxt, JsonNodeFactory nodeFactory) throws IOException {
  ...
  protected final JsonNode _fromBigDecimal(
      DeserializationContext ctxt, JsonNodeFactory nodeFactory, BigDecimal bigDec) {
  ...
  /**
   * Optimized variant similar in functionality to (a subset of) {@link java.util.ArrayDeque}; used
   * to hold enclosing Array/Object nodes during recursion-as-iteration.
   */
