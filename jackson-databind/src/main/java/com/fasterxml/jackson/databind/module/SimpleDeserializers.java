...
/**
 * Simple implementation {@link Deserializers} which allows registration of deserializers based on
 * raw (type erased class). It can work well for basic bean and scalar type deserializers, but is
 * not a good fit for handling generic types (like {@link Map}s and {@link Collection}s or array
 * types).
 *
 * <p>Unlike {@link SimpleSerializers}, this class does not currently support generic mappings; all
 * mappings must be to exact declared deserialization type.
 */
public class SimpleDeserializers extends Deserializers.Base implements java.io.Serializable {
  ...
  public void addDeserializers(Map<Class<?>, JsonDeserializer<?>> desers) {
    for (Map.Entry<Class<?>, JsonDeserializer<?>> entry : entries.entrySet()) {
