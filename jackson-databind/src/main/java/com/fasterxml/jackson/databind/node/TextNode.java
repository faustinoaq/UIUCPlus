...
public class TextNode extends ValueNode {
  ...
  /**
   * Factory method that should be used to construct instances. For some common cases, can reuse
   * canonical instances: currently this is the case for empty Strings, in future possible for
   * others as well. If null is passed, will return null.
   *
   * @return Resulting {@link TextNode} object, if <b>v</b> is NOT null; null if it is.
   */
  ...
  /**
   * Method for accessing textual contents assuming they were base64 encoded; if so, they are
   * decoded and resulting binary data is returned.
   */
  ...
  public byte[] getBinaryValue(Base64Variant b64variant) throws IOException {
    ...
    ByteArrayBuilder builder = new ByteArrayBuilder(Math.max(16, Math.min(0x10000, initBlockSize)));
    try {
