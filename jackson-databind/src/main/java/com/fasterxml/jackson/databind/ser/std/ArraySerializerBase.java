    ...
    implements ContextualSerializer // for 'unwrapSingleElemArray'
{
  ...
  /**
   * Setting for specific local override for "unwrap single element arrays": true for enable
   * unwrapping, false for preventing it, `null` for using global configuration.
   *
   * @since 2.6
   */
  ...
  /**
   * Use either variant that just takes type (non-contextual), or, copy constructor that allows
   * passing of property.
   *
  ...
  protected ArraySerializerBase(
      ArraySerializerBase<?> src, BeanProperty property, Boolean unwrapSingle) {
    super(src._handledType, false);
    _property = property;
   ...
   * @since 2.6
   */
  public abstract JsonSerializer<?> _withResolved(BeanProperty prop, Boolean unwrapSingle);
  ...
  public final void serializeWithType(
      T value, JsonGenerator g, SerializerProvider provider, TypeSerializer typeSer)
      throws IOException {
    WritableTypeId typeIdDef =
        typeSer.writeTypePrefix(g, typeSer.typeId(value, JsonToken.START_ARRAY));
  ...
  protected abstract void serializeContents(
      T value, JsonGenerator jgen, SerializerProvider provider) throws IOException;
