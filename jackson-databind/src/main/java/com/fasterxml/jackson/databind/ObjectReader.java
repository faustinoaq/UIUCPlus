 ...
 *
 * <p>Uses "mutant factory" pattern so that instances are immutable (and thus fully thread-safe with
 * no external synchronization); new instances are constructed for different configurations.
 * Instances are initially constructed by {@link ObjectMapper} and can be reused, shared, cached;
 * both because of thread-safety and because instances are relatively light-weight.
 *
 * <p>NOTE: this class is NOT meant as sub-classable (with Jackson 2.8 and above) by users. It is
 * left as non-final mostly to allow frameworks that require bytecode generation for proxying and
 * similar use cases, but there is no expecation that functionality should be extended by
 * sub-classing.
 */
    ...
    implements Versioned, java.io.Serializable // since 2.1
{
  ...
  /**
   * General serialization configuration settings; while immutable, can use copy-constructor to
   * create modified instances as necessary.
   */
  ...
  /**
   * Declared type of value to instantiate during deserialization. Defines which deserializer to
   * use; as well as base type of instance to construct if an updatable value is not configured to
   * be used (subject to changes by embedded type information, for polymorphic types). If {@link
   * #_valueToUpdate} is non-null, only used for locating deserializer.
   */
  protected final JavaType _valueType;

  /**
   * We may pre-fetch deserializer as soon as {@link #_valueType} is known, and if so, reuse it
   * afterwards. This allows avoiding further deserializer lookups and increases performance a bit
   * on cases where readers are reused.
   *
   * @since 2.1
   */
  ...
  /**
   * Instance to update with data binding; if any. If null, a new instance is created, if non-null,
   * properties of this value object will be updated instead. Note that value can be of almost any
   * type, except not {@link com.fasterxml.jackson.databind.type.ArrayType}; array types cannot be
   * modified because array size is immutable.
   */
   ...
   *
   * <p>NOTE: If defined non-null, <code>readValue()</code> methods that take {@link Reader} or
   * {@link String} input <b>will fail with exception</b>, because format-detection only works on
   * byte-sources. Also, if format cannot be detect reliably (as per detector settings), a {@link
   * StreamReadException} will be thrown).
   *
   * @since 2.1
   */
  ...
  /** Root-level cached deserializers. Passed by {@link ObjectMapper}, shared with it. */
  ...
  protected ObjectReader(ObjectReader base, JsonFactory f) {
    ...
    _config =
        base._config.with(
            MapperFeature.SORT_PROPERTIES_ALPHABETICALLY, f.requiresPropertyOrdering());
    _context = base._context;
  ...
  /**
   * Factory method used to create {@link MappingIterator} instances; either default, or custom
   * subtype.
   *
   * @since 2.5
   */
  protected <T> MappingIterator<T> _newIterator(
      JsonParser p, DeserializationContext ctxt, JsonDeserializer<?> deser, boolean parserManaged) {
  ...
  /**
   * Alternative to {@link #_initForReading} used in cases where reading of multiple values means
   * that we may or may not want to advance the stream, but need to do other initialization.
   *
   * <p>Base implementation only sets configured {@link FormatSchema}, if any, on parser.
   *
   * @since 2.8
   */
  ...
  /**
   * Method for constructing a new reader instance that is configured with specified feature
   * enabled.
   */
  ...
  /**
   * Method for constructing a new reader instance that is configured with specified features
   * enabled.
   */
  ...
  public ObjectReader without(DatatypeFeature feature) {
    return _with(_config.with(feature).without(feature));
  }
  ...
  /**
   * Convenience method to bind from {@link JsonPointer}. {@link JsonPointerBasedFilter} is
   * registered and will be used for parsing later.
   *
   * @since 2.6
   */
  ...
  /**
   * Mutant factory method that will construct a new instance that has specified underlying {@link
   * DeserializationConfig}.
   *
   * <p>NOTE: use of this method is not recommended, as there are many other re-configuration
   * methods available.
   */
  ...
  /**
   * Method for constructing a new instance with configuration that uses passed {@link
  ...
  /**
   * Method for constructing a new reader instance with configuration that uses passed {@link
   * JsonNodeFactory} for constructing {@link JsonNode} instances.
   *
   * <p>Note that the method does NOT change state of this reader, but rather construct and returns
   * a newly configured instance.
   */
   ...
   *
   * <p>NOTE: only factories that <b>DO NOT REQUIRE SPECIAL MAPPERS</b> (that is, ones that return
   * <code>false</code> for {@link JsonFactory#requiresCustomCodec()}) can be used: trying to use
   * one that requires custom codec will throw exception
   *
   * @since 2.1
   */
  ...
  /**
   * Method for constructing a new instance with configuration that specifies what root name to
   * expect for "root name unwrapping". See {@link DeserializationConfig#withRootName(String)} for
   * details.
   *
   * <p>Note that the method does NOT change state of this reader, but rather construct and returns
   * a newly configured instance.
  ...
  /**
   * Convenience method that is same as calling: <code>
   *   withRootName("")
   * </code> which will forcibly prevent use of root name wrapping when writing values with this
   * {@link ObjectReader}.
   *
   * @since 2.6
   */
  ...
  /**
   * Method for constructing a new instance with configuration that passes specified {@link
   * FormatSchema} to {@link JsonParser} that is constructed for parsing content.
   *
   * <p>Note that the method does NOT change state of this reader, but rather construct and returns
   * a newly configured instance.
   */
  ...
  /**
   * Method for constructing a new reader instance that is configured to data bind into specified
   * type.
   *
   * <p>Note that the method does NOT change state of this reader, but rather construct and returns
   * a newly configured instance.
  ...
  public ObjectReader forType(JavaType valueType) {
    ...
    return _new(
        this, _config, valueType, rootDeser, _valueToUpdate, _schema, _injectableValues, det);
  }
  ...
  /**
   * Method for constructing a new instance with configuration that updates passed Object (as root
   * value), instead of constructing a new value.
   *
   * <p>Note that the method does NOT change state of this reader, but rather construct and returns
   * a newly configured instance.
   */
  public ObjectReader withValueToUpdate(Object value) {
    ...
    return _new(
        this, _config, t, _rootDeserializer, value, _schema, _injectableValues, _dataFormatReaders);
  }

  /**
   * Method for constructing a new instance with configuration that uses specified View for
   * filtering.
   *
   * <p>Note that the method does NOT change state of this reader, but rather construct and returns
   * a newly configured instance.
  ...
  /**
   * Fluent factory method for constructing a reader that will try to auto-detect underlying data
   * format, using specified list of {@link JsonFactory} instances, and default {@link
   * DataFormatReaders} settings (for customized {@link DataFormatReaders}, you can construct
   * instance yourself). to construct appropriate {@link JsonParser} for actual parsing.
   *
   * <p>Note: since format detection only works with byte sources, it is possible to get a failure
   * from some 'readValue()' methods. Also, if input cannot be reliably (enough) detected as one of
   * specified types, an exception will be thrown.
   *
   * <p>Note: not all {@link JsonFactory} types can be passed: specifically, ones that require
   * "custom codec" (like XML factory) will not work. Instead, use method that takes {@link
   * ObjectReader} instances instead of factories.
   *
   * @param readers Data formats accepted, in decreasing order of priority (that is, matches checked
   *     in listed order, first match wins)
  ...
  /**
   * Fluent factory method for constructing a reader that will try to auto-detect underlying data
   * format, using specified {@link DataFormatReaders}.
   *
   * <p>NOTE: since format detection only works with byte sources, it is possible to get a failure
   * from some 'readValue()' methods. Also, if input cannot be reliably (enough) detected as one of
   * specified types, an exception will be thrown.
   ...
   * specified {@link File}. Parser is not managed (or "owned") by ObjectReader: caller is
   ...
   * specified {@link InputStream}. Parser is not managed (or "owned") by ObjectReader: caller is
   * responsible for properly closing it once content reading is complete.
   *
   * @since 2.11
   */
  ...
  /**
   * Factory method for constructing properly initialized {@link JsonParser} to read content using
   * specified {@link Reader}. Parser is not managed (or "owned") by ObjectReader: caller is
   * responsible for properly closing it once content reading is complete.
   *
   * @since 2.11
   */
  ...
  /**
   * Factory method for constructing properly initialized {@link JsonParser} to read content from
   * specified byte array. Parser is not managed (or "owned") by ObjectReader: caller is responsible
  ...
  /**
   * Factory method for constructing properly initialized {@link JsonParser} to read content from
   * specified String. Parser is not managed (or "owned") by ObjectReader: caller is responsible for
  ...
  /**
   * Factory method for constructing properly initialized {@link JsonParser} to read content from
   * specified character array Parser is not managed (or "owned") by ObjectReader: caller is
   * responsible for properly closing it once content reading is complete.
   *
   * @since 2.11
   */
  ...
  /**
   * Factory method for constructing properly initialized {@link JsonParser} to read content using
   * specified {@link DataInput}. Parser is not managed (or "owned") by ObjectReader: caller is
   * responsible for properly closing it once content reading is complete.
   *
   * @since 2.11
   */
  ...
  /**
   * Factory method for constructing properly initialized {@link JsonParser} to read content using
   * non-blocking (asynchronous) mode. Parser is not managed (or "owned") by ObjectReader: caller is
   * responsible for properly closing it once content reading is complete.
   *
   * @since 2.11
   */
  ...
  /**
   * Method that binds content read using given parser, using configuration of this reader,
   * including expected result type. Value return is either newly constructed, or root value that
   * was specified with {@link #withValueToUpdate(Object)}.
   *
   * <p>NOTE: this method never tries to auto-detect format, since actual (data-format specific)
   * parser is given.
   */
  ...
  /**
   * Convenience method that binds content read using given parser, using configuration of this
   * reader, except that expected value type is specified with the call (instead of currently
   * configured root type). Value return is either newly constructed, or root value that was
   * specified with {@link #withValueToUpdate(Object)}.
   *
   * <p>NOTE: this method never tries to auto-detect format, since actual (data-format specific)
   * parser is given.
   */
   ...
   * </pre>
   *
   * <p>Method reads a sequence of Objects from parser stream. Sequence can be either root-level
  ...
  /**
   * Convenience method that binds content read using given parser, using configuration of this
   * reader, except that content is bound as JSON tree instead of configured root value type.
   * Returns {@link JsonNode} that represents the root of the resulting tree, if there was content
   * to read, or {@code null} if no more content is accessible via passed {@link JsonParser}.
   *
   ...
   * {@code null}
   *
   * <p>Note: if an object was specified with {@link #withValueToUpdate}, it will be ignored.
   *
   * <p>NOTE: this method never tries to auto-detect format, since actual (data-format specific)
   * parser is given.
   */
   ...
   * Value return is either newly constructed, or root value that was specified with {@link
  ...
  /**
   * Same as {@link #readValue(InputStream)} except that target value type overridden as {@code
   * valueType}
   *
   * @param src Source to read content from
  ...
  /**
   * Same as {@link #readValue(Reader)} except that target value type overridden as {@code
   * valueType}
   *
   * @param src Source to read content from
   * @param valueType Target type to bind content to
  ...
  /**
   * Same as {@link #readValue(String)} except that target value type overridden as {@code
   * valueType}
   *
   * @param src String that contains content to read
   * @param valueType Target type to bind content to
  ...
  /**
   * Method that binds content read from given byte array, using configuration of this reader. Value
   * return is either newly constructed, or root value that was specified with {@link
   * #withValueToUpdate(Object)}.
   *
  ...
  /**
   * Same as {@link #readValue(byte[])} except that target value type overridden as {@code
   * valueType}
   *
   * @param content Byte array that contains encoded content to read
   * @param valueType Target type to bind content to
  ...
  /**
   * Same as {@link #readValue(byte[],int,int)} except that target value type overridden as {@code
   * valueType}
   *
   * @param buffer Byte array that contains encoded content to read
   * @param offset Offset of the first content byte in {@code buffer}
  ...
  /**
   * Method that binds content read from given {@link File} using configuration of this reader.
   * Value return is either newly constructed, or root value that was specified with {@link
   * #withValueToUpdate(Object)}.
   *
  ...
  /**
   * Same as {@link #readValue(File)} except that target value type overridden as {@code valueType}
   *
   * @param src File that contains content to read
   * @param valueType Target type to bind content to
   * @since 2.11
  ...
  /**
   * Same as {@link #readValue(URL)} except that target value type overridden as {@code valueType}
   *
  ...
  /**
   * Convenience method for converting results from given JSON tree into given value type. Basically
   * short-cut for:
   *
   * <pre>
  ...
  /**
   * Same as {@link #readValue(JsonNode)} except that target value type overridden as {@code
   * valueType}
   *
   * @param content Tree that contains content to convert
  ...
  /**
   * Same as {@link #readValue(DataInput)} except that target value type overridden as {@code
   * valueType}
   *
  ...
  /**
   * Method that reads content from given input source, using configuration of this reader, and
   * binds it as JSON Tree. Returns {@link JsonNode} that represents the root of the resulting tree,
   * if there was content to read, or "missing node" (instance of {@link JsonNode} for which {@link
   * JsonNode#isMissingNode()} returns true, and behaves otherwise similar to "null node") if no
   * more content is accessible through passed-in input source.
   *
   * <p>NOTE! Behavior with end-of-input (no more content) differs between this {@code readTree}
   * method, and {@link #readTree(JsonParser)} -- latter returns {@code null} for "no content" case.
   *
   * <p>Note that if an object was specified with a call to {@link #withValueToUpdate(Object)} it
   * will just be ignored; result is always a newly constructed {@link JsonNode} instance.
   */
  ...
  /**
   * Same as {@link #readTree(InputStream)} except content read using passed-in {@link DataInput}.
   */
   ...
   *
   * <p>Sequence can be either root-level "unwrapped" sequence (without surrounding JSON array), or
   * a sequence contained in a JSON Array. In either case {@link JsonParser} must point to the first
   * token of the first element, OR not point to any token (in which case it is advanced to the next
   * token). This means, specifically, that for wrapped sequences, parser MUST NOT point to the
   * surrounding <code>START_ARRAY</code> but rather to the token following it.
   */
   ...
   * Method for reading sequence of Objects from parser stream.
   *
   * <p>Sequence can be either wrapped or unwrapped root-level sequence: wrapped means that the
   * elements are enclosed in JSON Array; and unwrapped that elements are directly accessed at main
   * level. Assumption is that iff the first token of the document is <code>START_ARRAY</code>, we
   * have a wrapped sequence; otherwise unwrapped. For wrapped sequences, leading <code>START_ARRAY
   * </code> is skipped, so that for both cases, underlying {@link JsonParser} will point to what is
   * expected to be the first token of the first element.
   *
   * <p>Note that the wrapped vs unwrapped logic means that it is NOT possible to use this method
   * for reading an unwrapped sequence of elements written as JSON Arrays: to read such sequences,
   * one has to use {@link #readValues(JsonParser)}, making sure parser points to the first token of
   * the first element (i.e. the second <code>START_ARRAY</code> which is part of the first
   * element).
   */
  ...
  protected final JsonNode _bindAsTree(JsonParser p) throws IOException {
    ...
    if (t == JsonToken.VALUE_NULL) {
      ...
      resultNode =
          (JsonNode) ctxt.readRootValue(p, _jsonNodeType(), _findTreeDeserializer(ctxt), null);
    }
    // Need to consume the token too
    p.clearCurrentToken();
    if (_config.isEnabled(DeserializationFeature.FAIL_ON_TRAILING_TOKENS)) {
  ...
  /**
   * Same as {@link #_bindAsTree} except end-of-input is reported by returning {@code null}, not
   * "missing node"
   */
  ...
  protected JsonParser _considerFilter(final JsonParser p, boolean multiValue) {
    ...
    return ((_filter == null) || FilteringParserDelegate.class.isInstance(p))
        ? p
        : new FilteringParserDelegate(p, _filter, Inclusion.ONLY_INCLUDE_ALL, multiValue);
  }
  ...
  protected <T> MappingIterator<T> _detectBindAndReadValues(
      DataFormatReaders.Match match, boolean forceClosing) throws IOException {
    if (!match.hasMatch()) {
      _reportUnkownFormat(_dataFormatReaders, match);
    }
    JsonParser p = match.createParserWithMatch();
  ...
  /**
   * Method called to locate deserializer ahead of time, if permitted by configuration. Method also
   * is NOT to throw an exception if access fails.
   */
  protected JsonDeserializer<Object> _prefetchRootDeserializer(JavaType valueType) {
    if ((valueType == null)
        || !_config.isEnabled(DeserializationFeature.EAGER_DESERIALIZER_FETCH)) {
      return null;
    }
    // already cached?
