...
/**
 * Basic deserializer that can take JSON "Object" structure and construct a {@link java.util.Map}
 * instance, with typed contents.
 *
 * <p>Note: for untyped content (one indicated by passing Object.class as the type), {@link
 * UntypedObjectDeserializer} is used instead. It can also construct {@link java.util.Map}s, but not
 * with specific POJO types, only other containers and primitives/wrappers.
 */
@JacksonStdImpl
public class MapDeserializer extends ContainerDeserializerBase<Map<Object, Object>>
  ...
  /**
   * Key deserializer to use; either passed via constructor (when indicated by annotations), or
   * resolved when {@link #resolve} is called;
   */
  protected final KeyDeserializer _keyDeserializer;

  /**
   * Flag set to indicate that the key type is {@link java.lang.String} (or {@link
   * java.lang.Object}, for which String is acceptable), <b>and</b> that the default Jackson key
   * deserializer would be used. If both are true, can optimize handling.
   */
  ...
  /**
   * If value instances have polymorphic type information, this is the type deserializer that can
   * handle it
   */
  ...
  /**
   * If the Map is to be instantiated using non-default constructor or factory method that takes one
   * or more named properties as argument(s), this creator is used for instantiation.
   */
  ...
  /**
   * Flag used to check, whether the {@link
   * com.fasterxml.jackson.core.StreamReadCapability#DUPLICATE_PROPERTIES} can be applied, because
   * the Map has declared value type of {@code java.lang.Object}.
   *
   * @since 2.14
   */
  ...
  /**
   * Copy-constructor that can be used by sub-classes to allow copy-on-write styling copying of
   * settings of an existing instance.
   */
  ...
  /**
   * Fluent factory method used to create a copy with slightly different settings. When
   * sub-classing, MUST be overridden.
   */
  protected MapDeserializer withResolved(
      ...
      Set<String> ignorable) {
    return withResolved(
        keyDeser, valueTypeDeser, valueDeser, nuller, ignorable, _includableProperties);
  }
  ...
  /**
   * Helper method used to check whether we can just use the default key deserialization, where JSON
   * String becomes Java String.
   */
  ...
  public void setIgnorableProperties(String[] ignorable) {
    _ignorableProperties =
        (ignorable == null || ignorable.length == 0) ? null : ArrayBuilders.arrayToSet(ignorable);
    _inclusionChecker =
   ...
   *
   * <p>IMPORTANT: but, note, that instances CAN NOT BE CACHED if there is a value type
   * deserializer; this caused an issue with 2.4.4 of JAXB Annotations (failing a test). It is also
   * possible that some other settings could make deserializers un-cacheable; but on the other hand,
   * caching can make a big positive difference with performance... so it's a hard choice.
   *
   * @since 2.4.4
   */
  @Override
  ...
  public Map<Object, Object> deserialize(JsonParser p, DeserializationContext ctxt)
    ...
    if (_delegateDeserializer != null) {
      return (Map<Object, Object>)
          _valueInstantiator.createUsingDelegate(ctxt, _delegateDeserializer.deserialize(p, ctxt));
    }
    if (!_hasDefaultCreator) {
      return (Map<Object, Object>)
          ctxt.handleMissingInstantiator(
              getMapClass(), getValueInstantiator(), p, "no default constructor found");
    }
    switch (p.currentTokenId()) {
      ...
      case JsonTokenId.ID_FIELD_NAME:
        final Map<Object, Object> result =
            (Map<Object, Object>) _valueInstantiator.createUsingDefault(ctxt);
  ...
  public Map<Object, Object> deserialize(
      JsonParser p, DeserializationContext ctxt, Map<Object, Object> result) throws IOException {
  ...
  protected final Map<Object, Object> _readAndBind(
      JsonParser p, DeserializationContext ctxt, Map<Object, Object> result) throws IOException {
    ...
    if (useObjectId) {
      referringAccumulator =
          new MapReferringAccumulator(_containerType.getContentType().getRawClass(), result);
    }
  ...
  /**
   * Optimized method used when keys can be deserialized as plain old {@link java.lang.String}s, and
   * there is no custom deserialized specified.
   */
  protected final Map<Object, Object> _readAndBindStringKeyMap(
      JsonParser p, DeserializationContext ctxt, Map<Object, Object> result) throws IOException {
    final JsonDeserializer<Object> valueDes = _valueDeserializer;
    final TypeDeserializer typeDeser = _valueTypeDeserializer;
    MapReferringAccumulator referringAccumulator = null;
   ...
   * @since 2.9
   */
  protected final void _readAndUpdate(
      JsonParser p, DeserializationContext ctxt, Map<Object, Object> result) throws IOException {
    final KeyDeserializer keyDes = _keyDeserializer;
    final JsonDeserializer<Object> valueDes = _valueDeserializer;
    final TypeDeserializer typeDeser = _valueTypeDeserializer;
  ...
  /**
   * Optimized method used when keys can be deserialized as plain old {@link java.lang.String}s, and
   * there is no custom deserializer specified.
   *
   * @since 2.9
   */
  ...
  private void handleUnresolvedReference(
    ...
    if (accumulator == null) {
      ctxt.reportInputMismatch(
          this, "Unresolved forward reference but no identity info: " + reference);
    }
    Referring referring =
        accumulator.handleUnresolvedReference(
            reference, key); // lgtm [java/dereferenced-value-may-be-null]
  ...
  /**
   * Helper class to maintain processing order of value. The resolved object associated with {@link
   * #key} comes before the values in {@link #next}.
   */
  static class MapReferring extends Referring {
    ...
    MapReferring(
        MapReferringAccumulator parent,
