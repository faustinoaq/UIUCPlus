...
/**
 * Simple {@link AbstractTypeResolver} implementation, which is based on static mapping from
 * abstract super types into sub types (concrete or abstract), but retaining generic
 * parameterization. Can be used for things like specifying which implementation of {@link
 * java.util.Collection} to use:
 *
 * <pre>
 ...
 * </pre>
 *
 * Can also be used as an alternative to per-class annotations when defining concrete
 * implementations; however, only works with abstract types (since this is only called for abstract
 * types)
 */
public class SimpleAbstractTypeResolver extends AbstractTypeResolver
  ...
  /**
   * Method for adding a mapping from super type to specific subtype. Arguments will be checked by
   * method, to ensure that <code>superType</code> is abstract (since resolver is never called for
   * concrete classes); as well as to ensure that there is supertype/subtype relationship (to ensure
  ...
  public <T> SimpleAbstractTypeResolver addMapping(Class<T> superType, Class<? extends T> subType) {
    ...
    if (!Modifier.isAbstract(superType.getModifiers())) {
      throw new IllegalArgumentException(
          "Cannot add mapping from class " + superType.getName() + " since it is not abstract");
    }
