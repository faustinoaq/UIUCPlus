...
/**
 * Utility class used for efficient storage of {@link JsonToken} sequences, needed for temporary
 * buffering. Space efficient for different sequence lengths (especially so for smaller ones; but
 * not significantly less efficient for larger), highly efficient for linear iteration and
 * appending. Implemented as segmented/chunked linked list of tokens; only modifications are via
 * appends.
 */
public class TokenBuffer
  ...
  /**
   * Object codec to use for stream-based object conversion through parser/generator interfaces. If
   * null, such methods cannot be used.
   */
  protected ObjectCodec _objectCodec;

  /**
   * Parse context from "parent" parser (one from which content to buffer is read, if specified).
   * Used, if available, when reading content, to present full context as if content was read from
   * the original parser: this is useful in error reporting and sometimes processing as well.
   */
  protected JsonStreamContext _parentContext;

  /**
   * Bit flag composed of bits that indicate which {@link
   * com.fasterxml.jackson.core.JsonGenerator.Feature}s are enabled.
   *
   * <p>NOTE: most features have no effect on this class
   */
  ...
  /**
   * Flag set during construction, if use of {@link BigDecimal} is to be forced on all
  ...
  /**
   * If native type ids supported, this is the id for following value (or first token of one) to be
   * written.
   */
  protected Object _typeId;

  /**
   * If native object ids supported, this is the id for following value (or first token of one) to
   * be written.
   */
  ...
  /**
   * @param codec Object codec to use for stream-based object conversion through parser/generator
   *     interfaces. If null, such methods cannot be used.
   * @param hasNativeIds Whether resulting {@link JsonParser} (if created) is considered to support
   *     native type and object ids
   */
  ...
  public TokenBuffer(JsonParser p, DeserializationContext ctxt) {
    ...
    _mayHaveNativeIds = _hasNativeTypeIds || _hasNativeObjectIds;
    _forceBigDecimal =
        (ctxt == null) ? false : ctxt.isEnabled(DeserializationFeature.USE_BIG_DECIMAL_FOR_FLOATS);
  }
  ...
  /**
   * Method that allows explicitly specifying parent parse context to associate with contents of
   * this buffer. Usually context is assigned at construction, based on given parser; but it is not
   * always available, and may not contain intended context.
   *
   * @since 2.9
   */
  ...
  /**
   * Method used to create a {@link JsonParser} that can read contents stored in this buffer. Will
   * use default <code>_objectCodec</code> for object conversions.
   *
   * <p>Note: instances are not synchronized, that is, they are not thread-safe if there are
   * concurrent appends to the underlying buffer.
   *
  ...
  /**
   * Helper method that will append contents of given buffer into this buffer. Not particularly
   * optimized; can be made faster if there is need.
   *
   * @return This buffer
   */
   ...
   *
   * <p>Note: this method would be enough to implement <code>JsonSerializer</code> for <code>
   * TokenBuffer</code> type; but we cannot have upwards references (from core to mapper package);
   * and as such we also cannot take second argument.
   */
  public void serialize(JsonGenerator gen) throws IOException {
    ...
    while (true) {
      ...
      switch (t) {
        ...
        case FIELD_NAME:
          {
            // 13-Dec-2010, tatu: Maybe we should start using different type tokens to reduce
            // casting?
  ...
  public void writeObject(Object value) throws IOException {
    ...
    if (_objectCodec == null) {
      ...
      //          throw new XxxException("No ObjectCodec configured for TokenBuffer, writeObject()
      // called");
      _appendValue(JsonToken.VALUE_EMBEDDED_OBJECT, value);
    } else {
  ...
  /**
   * Although we could support this method, it does not necessarily make sense: we cannot make good
   * use of streaming because buffer must hold all the data. Because of this, currently this will
   * simply throw {@link UnsupportedOperationException}
   */
  @Override
  ...
  */
  ...
  /**
   * Method used for appending token known to represent a "simple" scalar value where token is the
   * only information
   *
   * @since 2.6.4
   */
  ...
  /**
   * Method used for appending token known to represent a scalar value where there is additional
   * content (text, number) beyond type token
   *
   * @since 2.6.4
   */
  ...
  /**
   * Specialized method used for appending a field name, appending either {@link String} or {@link
   * SerializableString}.
   *
   * @since 2.10
   */
  ...
  protected static final class Parser extends ParserMinimalBase {
    ...
    /**
     * Information about parser context, context in which the next token is to be parsed (root,
     * array, object).
     */
    ...
    public Parser(
      ...
      if (hasNativeTypeIds) this(firstSeg, codec, hasNativeObjectIds, hasNativeTypeIds, null);
      else this(firstSeg, codec, hasNativeTypeIds, hasNativeObjectIds, null);
    }
    ...
    private Number getNumberValue(final boolean preferBigNumbers) throws IOException {
      ...
      if (value instanceof String) {
        ...
        if (_currToken == JsonToken.VALUE_NUMBER_INT) {
          if (preferBigNumbers
              ...
              || (len >= 19)) {
            return NumberInput.parseBigInteger(
      ...
      throw new IllegalStateException(
          "Internal error: entry should be a Number, but is of type "
    ...
    public byte[] getBinaryValue(Base64Variant b64variant) throws IOException {
      ...
      if (_currToken != JsonToken.VALUE_STRING) {
        throw _constructError(
            "Current token ("
                + _currToken
                + ") not VALUE_STRING (or VALUE_EMBEDDED_OBJECT with byte[]), cannot access as"
                + " binary");
      }
  ...
  /**
   * Individual segment of TokenBuffer that can store up to 16 tokens (limited by 4 bits per token
   * type marker requirement). Current implementation uses fixed length array; could alternatively
   * use 16 distinct fields and switch statement (slightly more efficient storage, slightly slower
   * access)
   */
  protected static final class Segment {
    ...
    /**
     * Static array used for fast conversion between token markers and matching {@link JsonToken}
     * instances
     */
    ...
    /**
     * Bit field used to store types of buffered tokens; 4 bits per token. Value 0 is reserved for
     * "not in use"
     */
    ...
    public Segment append(
        int index, JsonToken tokenType, Object value, Object objectId, Object typeId) {
      if (index < TOKENS_PER_SEGMENT) {
