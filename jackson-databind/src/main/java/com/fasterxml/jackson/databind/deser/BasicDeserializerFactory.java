...
/**
 * Abstract factory base class that can provide deserializers for standard JDK classes, including
 * collection classes and simple heuristics for "upcasting" common collection interface types (such
 * as {@link java.util.Collection}).
 *
 * <p>Since all simple deserializers are eagerly instantiated, and there is no additional
 * introspection or customizability of these types, this factory is stateless.
 */
...
public abstract class BasicDeserializerFactory extends DeserializerFactory
  ...
  /**
   * We need a placeholder for creator properties that don't have name but are marked with
   * `@JsonWrapped` annotation.
   */
  ...
  /**
   * Configuration settings for this factory; immutable instance (just like this factory), new
  ...
  /**
   * Convenience method for creating a new factory instance with additional {@link
   * KeyDeserializers}.
   */
  @Override
  ...
  /**
   * Method that will find abstract type mapping for specified type, doing a single lookup through
   * registered abstract type resolvers; will not do recursive lookups.
   */
  ...
  /**
   * Value instantiator is created both based on creator annotations, and on optional externally
   * provided instantiators (registered through module interface).
   */
  @Override
  public ValueInstantiator findValueInstantiator(
      DeserializationContext ctxt, BeanDescription beanDesc) throws JsonMappingException {
    ...
    if (instantiator == null) {
      ...
      // instantiators.
      instantiator =
          JDKValueInstantiators.findStdValueInstantiator(config, beanDesc.getBeanClass());
      if (instantiator == null) {
  ...
  /**
   * Method that will construct standard default {@link ValueInstantiator} using annotations
   * (like @JsonCreator) and visibility rules
   */
  protected ValueInstantiator _constructDefaultValueInstantiator(
      DeserializationContext ctxt, BeanDescription beanDesc) throws JsonMappingException {
...

    {
      ...
      final VisibilityChecker<?> vchecker =
      ...
      Map<AnnotatedWithParams, BeanPropertyDefinition[]> creatorDefs =
          _findCreatorsFromProperties(ctxt, beanDesc);
      ccState = new CreatorCollectionState(ctxt, beanDesc, vchecker, creators, creatorDefs);
    }
    ...
    if (beanDesc.getType().isConcrete()) {
      ...
      if (isNonStaticInnerClass) {
        ...
        final boolean findImplicit =
            ctorDetector.shouldIntrospectorImplicitConstructors(beanDesc.getBeanClass());
  ...
  public ValueInstantiator _valueInstantiatorInstance(
      DeserializationConfig config, Annotated annotated, Object instDef)
      throws JsonMappingException {
    if (instDef == null) {
      return null;
    }
    ...
    return (ValueInstantiator)
        ClassUtil.createInstance(instClass, config.canOverrideAccessModifiers());
  }
  ...
  /**
   * Helper method called when a {@code java.lang.Record} definition's "canonical" constructor is to
   * be used: if so, we have implicit names to consider.
   *
   * @since 2.12
   * @deprecated since 2.15 - no longer used, but kept because this protected method might have been
   *     overridden/used elsewhere
   */
  @Deprecated
  ...
  protected void _addExplicitConstructorCreators(
      DeserializationContext ctxt, CreatorCollectionState ccState, boolean findImplicit)
      throws JsonMappingException {
    ...
    for (AnnotatedConstructor ctor : beanDesc.getConstructors()) {
      ...
      if (creatorMode == null) {
        ...
        if (findImplicit && vchecker.isCreatorVisible(ctor)) {
          ccState.addImplicitConstructorCandidate(
      ...
      switch (creatorMode) {
        case DELEGATING:
          _addExplicitDelegatingCreator(
              ctxt, beanDesc, creators, CreatorCandidate.construct(intr, ctor, null));
          break;
        case PROPERTIES:
  ...
  protected void _addImplicitConstructorCreators(
    ...
    final boolean preferPropsBased =
        config.getConstructorDetector().singleArgCreatorDefaultsToProperties();
    ...
    for (CreatorCandidate candidate : ctorCandidates) {
      ...
      if (argCount == 1) {
        ...
        final boolean useProps =
            preferPropsBased || _checkIfCreatorPropertyBased(beanDesc, intr, ctor, propDef);
        ...
        if (useProps) {
          ...
          properties[0] =
              constructCreatorProperty(ctxt, beanDesc, name, 0, candidate.parameter(0), injection);
          creators.addPropertyCreator(ctor, false, properties);
        } else {
          /*boolean added = */ _handleSingleArgumentCreator(
              creators, ctor, false, vchecker.isCreatorVisible(ctor));
      ...
      if ((explicitNameCount > 0) || (injectCount > 0)) {
        ...
        if (impl == null || impl.isEmpty()) {
          ...
          */
          ctxt.reportBadTypeDefinition(
              beanDesc,
              "Argument #%d of constructor %s has no property name annotation; must have name when"
  ...
  protected void _addExplicitFactoryCreators(
    ...
    for (AnnotatedMethod factory : beanDesc.getFactoryMethods()) {
      ...
      switch (creatorMode) {
        case DELEGATING:
          _addExplicitDelegatingCreator(
              ctxt, beanDesc, creators, CreatorCandidate.construct(intr, factory, null));
          break;
        case PROPERTIES:
          _addExplicitPropertyCreator(
              ctxt,
  ...
  protected void _addImplicitFactoryCreators(
    ...
    for (CreatorCandidate candidate : factoryCandidates) {
      ...
      if (!useProps) { // not property based but delegating
        /*boolean added=*/ _handleSingleArgumentCreator(
            creators, factory, false, vchecker.isCreatorVisible(factory));
      ...
      if (explicitNameCount > 0 || injectCount > 0) {
        ...
        if ((namedCount + injectCount) == argCount) {
          ...
          ctxt.reportBadTypeDefinition(
              beanDesc,
              "Argument #%d of factory method %s has no property name annotation; must have name"
                  + " when multiple-parameter constructor annotated as Creator",
   ...
   * @since 2.9.2
   */
  protected void _addExplicitDelegatingCreator(
      DeserializationContext ctxt,
    ...
    for (int i = 0; i < argCount; ++i) {
      ...
      ctxt.reportBadTypeDefinition(
          beanDesc,
          "More than one argument (#%d and #%d) left as delegating for Creator %s: only one"
              + " allowed",
          ix,
          i,
          candidate);
  ...
  /**
   * Helper method called when there is the explicit "is-creator" annotation with mode of
   * "properties-based"
   *
   * @since 2.9.2
   */
  protected void _addExplicitPropertyCreator(
      DeserializationContext ctxt,
      BeanDescription beanDesc,
      CreatorCollector creators,
      CreatorCandidate candidate)
    ...
    for (int i = 0; i < paramCount; ++i) {
      ...
      if (name == null) {
        ...
        NameTransformer unwrapper =
            ctxt.getAnnotationIntrospector().findUnwrappingNameTransformer(param);
        if (unwrapper != null) {
          _reportUnwrappedCreatorProperty(ctxt, beanDesc, param);
          /*
          properties[i] = constructCreatorProperty(ctxt, beanDesc, UNWRAPPED_CREATOR_PARAM_NAME, i, param, null);
  ...
  protected void _addExplicitAnyCreator(
      ...
      throws JsonMappingException {
    _addExplicitAnyCreator(
        ctxt, beanDesc, creators, candidate, ctxt.getConfig().getConstructorDetector());
  }
    ...
    if (found != null) {
      ...
      for (SettableBeanProperty prop : foundProps) {
        ...
        if (!bbd.hasProperty(pn)) {
          BeanPropertyDefinition newDef =
              SimpleBeanPropertyDefinition.construct(ctxt.getConfig(), prop.getMember(), pn);
  ...
  protected boolean _handleSingleArgumentCreator(
      CreatorCollector creators, AnnotatedWithParams ctor, boolean isCreator, boolean isVisible) {
  ...
  protected void _validateNamedPropertyParameter(
    ...
    if ((name == null) && (injectId == null)) {
      ctxt.reportBadTypeDefinition(
          beanDesc,
          "Argument #%d of constructor %s has no property name (and is not Injectable): can not use"
              + " as property-based Creator",
          paramIndex,
          candidate);
    }
  }
  ...
  protected void _reportUnwrappedCreatorProperty(
      DeserializationContext ctxt, BeanDescription beanDesc, AnnotatedParameter param)
      throws JsonMappingException {
    ctxt.reportBadTypeDefinition(
        beanDesc,
  ...
  /**
   * Method that will construct a property object that represents a logical property passed via
   * Creator (constructor or static factory method)
   */
  ...
  /**
   * Helper method copied from {@code POJOPropertyBuilder} since that won't be applied to creator
   * parameters
   *
   * @since 2.10
   */
  protected PropertyMetadata _getSetterInfo(
      DeserializationContext ctxt, BeanProperty prop, PropertyMetadata metadata) {
    final AnnotationIntrospector intr = ctxt.getAnnotationIntrospector();
    final DeserializationConfig config = ctxt.getConfig();
  ...
  public JsonDeserializer<?> createCollectionDeserializer(
    ...
    JsonDeserializer<?> deser =
        _findCustomCollectionDeserializer(type, config, beanDesc, contentTypeDeser, contentDeser);
    if (deser == null) {
      ...
      if (type.isInterface() || type.isAbstract()) {
        ...
        if (implType == null) {
          ...
          if (type.getTypeHandler() == null) {
            throw new IllegalArgumentException(
                "Cannot find a deserializer for non-concrete Collection type " + type);
          }
  ...
  protected CollectionType _mapAbstractCollectionType(JavaType type, DeserializationConfig config) {
    ...
    if (collectionClass != null) {
      return (CollectionType)
          config.getTypeFactory().constructSpecializedType(type, collectionClass, true);
    }
    return null;
  }
  ...
  public JsonDeserializer<?> createCollectionLikeDeserializer(
    ...
    // Then optional type info (1.5): if type has been resolved, we may already know type
    // deserializer:
    TypeDeserializer contentTypeDeser = contentType.getTypeHandler();
    // but if not, may still be possible to find:
    if (contentTypeDeser == null) {
    ...
    JsonDeserializer<?> deser =
        _findCustomCollectionLikeDeserializer(
            type, config, beanDesc, contentTypeDeser, contentDeser);
    if (deser != null) {
  ...
  public JsonDeserializer<?> createMapDeserializer(
    ...
    JsonDeserializer<Object> contentDeser =
        (JsonDeserializer<Object>) contentType.getValueHandler();
    ...
    // 23-Nov-2010, tatu: Custom deserializer?
    JsonDeserializer<?> deser =
        _findCustomMapDeserializer(type, config, beanDesc, keyDes, contentTypeDeser, contentDeser);
      ...
      if (deser == null) {
        if (type.isInterface() || type.isAbstract()) {
          ...
          if (fallback != null) {
            ...
            if (type.getTypeHandler() == null) {
              throw new IllegalArgumentException(
                  "Cannot find a deserializer for non-concrete Map type " + type);
            }
            deser = AbstractDeserializer.constructForNonPOJO(beanDesc);
          }
        } else {
        ...
        if (deser == null) {
          ...
          MapDeserializer md =
              new MapDeserializer(type, inst, keyDes, contentDeser, contentTypeDeser);
          JsonIgnoreProperties.Value ignorals =
              config.getDefaultPropertyIgnorals(Map.class, beanDesc.getClassInfo());
          Set<String> ignored =
              (ignorals == null) ? null : ignorals.findIgnoredForDeserialization();
          md.setIgnorableProperties(ignored);
          JsonIncludeProperties.Value inclusions =
              config.getDefaultPropertyInclusions(Map.class, beanDesc.getClassInfo());
    ...
    */
    ...
    JsonDeserializer<?> deser =
        _findCustomMapLikeDeserializer(
            type, config, beanDesc, keyDes, contentTypeDeser, contentDeser);
    if (deser != null) {
      // and then new with 2.2: ability to post-process it too (Issue#120)
      if (_factoryConfig.hasDeserializerModifiers()) {
        for (BeanDeserializerModifier mod : _factoryConfig.deserializerModifiers()) {
        ...
        if (_hasCreatorAnnotation(ctxt, factory)) {
          ...
          if (!returnType.isAssignableFrom(enumClass)) {
            ctxt.reportBadDefinition(
                type,
                String.format(
                    "Invalid `@JsonCreator` annotated Enum factory method [%s]: needs to return"
                        + " compatible type",
                    factory.toString()));
          }
          deser =
  ...
  public JsonDeserializer<?> createTreeDeserializer(
      DeserializationConfig config, JavaType nodeType, BeanDescription beanDesc)
      throws JsonMappingException {
    @SuppressWarnings("unchecked")
  ...
  public JsonDeserializer<?> createReferenceDeserializer(
      DeserializationContext ctxt, ReferenceType type, BeanDescription beanDesc)
      throws JsonMappingException {
    JavaType contentType = type.getContentType();
    // Very first thing: is deserializer hard-coded for elements?
    JsonDeserializer<Object> contentDeser = contentType.getValueHandler();
    ...
    JsonDeserializer<?> deser =
        _findCustomReferenceDeserializer(type, config, beanDesc, contentTypeDeser, contentDeser);
  ...
  public TypeDeserializer findTypeDeserializer(DeserializationConfig config, JavaType baseType)
    ...
    final Collection<NamedType> subtypes =
        config.getSubtypeResolver().collectAndResolveSubtypesByTypeId(config, ac);
    ...
    try {
      ...
      throw InvalidDefinitionException.from(
              (JsonParser) null, ClassUtil.exceptionMessage(e0), baseType)
          .withCause(e0);
    }
  }
   ...
   * @since 2.2
   */
  protected JsonDeserializer<?> findOptionalStdDeserializer(
      DeserializationContext ctxt, JavaType type, BeanDescription beanDesc)
      throws JsonMappingException {
  ...
  private KeyDeserializer _createEnumKeyDeserializer(DeserializationContext ctxt, JavaType type)
    ...
    if (des != null) {
      ...
      JsonDeserializer<?> valueDesForKey =
          findDeserializerFromAnnotation(ctxt, beanDesc.getClassInfo());
        ...
        if (argCount == 1) {
          ...
          if (returnType.isAssignableFrom(enumClass)) {
            ...
            if (factory.getRawParameterType(0) != String.class) {
              ...
              //                            throw new IllegalArgumentException("Parameter #0 type
              // for factory method ("+factory+") not suitable, must be java.lang.String");
              continue;
            }
  ...
  /**
   * Method called to create a type information deserializer for values of given non-container
   * property, if one is needed. If not needed (no polymorphic handling configured for property),
   * should return null.
   *
   * <p>Note that this method is only called for non-container bean properties, and not for values
   * in container types or root values (or container properties)
   *
   * @param baseType Declared base type of the value to deserializer (actual deserializer type will
   *     be this type or its subtype)
   * @return Type deserializer to use for given base type, if one is needed; null if not.
   */
  public TypeDeserializer findPropertyTypeDeserializer(
      DeserializationConfig config, JavaType baseType, AnnotatedMember annotated)
      throws JsonMappingException {
    AnnotationIntrospector ai = config.getAnnotationIntrospector();
    ...
    Collection<NamedType> subtypes =
        config.getSubtypeResolver().collectAndResolveSubtypesByTypeId(config, annotated, baseType);
    try {
      return b.buildTypeDeserializer(config, baseType, subtypes);
    } catch (IllegalArgumentException | IllegalStateException e0) {
      throw InvalidDefinitionException.from(
  ...
  /**
   * Method called to find and create a type information deserializer for values of given container
   * (list, array, map) property, if one is needed. If not needed (no polymorphic handling
   * configured for property), should return null.
   *
   * <p>Note that this method is only called for container bean properties, and not for values in
   * container types or root values (or non-container properties)
   *
   * @param containerType Type of property; must be a container type
   * @param propertyEntity Field or method that contains container property
   */
  public TypeDeserializer findPropertyContentTypeDeserializer(
      DeserializationConfig config, JavaType containerType, AnnotatedMember propertyEntity)
      throws JsonMappingException {
    AnnotationIntrospector ai = config.getAnnotationIntrospector();
    TypeResolverBuilder<?> b =
        ai.findPropertyContentTypeResolver(config, propertyEntity, containerType);
    ...
    Collection<NamedType> subtypes =
        config
            .getSubtypeResolver()
            .collectAndResolveSubtypesByTypeId(config, propertyEntity, contentType);
    return b.buildTypeDeserializer(config, contentType, subtypes);
  }

  /**
   * Helper method called to find one of default serializers for "well-known" platform types:
   * JDK-provided types, and small number of public Jackson API types.
   *
   * @since 2.2
   */
  ...
  protected JsonDeserializer<?> _findCustomTreeNodeDeserializer(
      Class<? extends JsonNode> type, DeserializationConfig config, BeanDescription beanDesc)
      throws JsonMappingException {
  ...
  protected JsonDeserializer<?> _findCustomReferenceDeserializer(
    ...
    for (Deserializers d : _factoryConfig.deserializers()) {
      JsonDeserializer<?> deser =
          d.findReferenceDeserializer(
              type, config, beanDesc, contentTypeDeserializer, contentDeserializer);
      if (deser != null) {
        return deser;
      }
    }
  ...
  protected JsonDeserializer<?> _findCustomCollectionLikeDeserializer(
    ...
    for (Deserializers d : _factoryConfig.deserializers()) {
      JsonDeserializer<?> deser =
          d.findCollectionLikeDeserializer(
              type, config, beanDesc, elementTypeDeserializer, elementDeserializer);
      if (deser != null) {
        return deser;
      }
  ...
  protected JsonDeserializer<?> _findCustomEnumDeserializer(
   ...
   * for deserialization; and if so, to instantiate, that deserializer to use. Note that
   * deserializer will NOT yet be contextualized so caller needs to take care to call
   * contextualization appropriately. Returns null if no such annotation found.
   */
  ...
  /**
   * Helper method called to check if a class or method has annotation that tells which class to use
   * for deserialization of {@link java.util.Map} keys. Returns null if no such annotation found.
   */
   ...
   * @since 2.9
   */
  protected JsonDeserializer<Object> findContentDeserializerFromAnnotation(
      DeserializationContext ctxt, Annotated ann) throws JsonMappingException {
    AnnotationIntrospector intr = null; // <---- change
    if (intr != null) {
      Object deserDef = intr.findContentDeserializer(ann);
      if (deserDef != null) {
        throw new JsonMappingException("Buggy 1"); // <---- change
      }
    }
    return null;
  }
  ...
  /**
   * Helper method used to resolve additional type-related annotation information like type
   * overrides, or handler (serializer, deserializer) overrides, so that from declared field,
   * property or constructor parameter type is used as the base and modified based on annotations,
   * if any.
   *
   * @since 2.8 Combines functionality of <code>modifyTypeByAnnotation</code> and <code>resolveType
   *     </code>
   */
  protected JavaType resolveMemberAndTypeAnnotations(
      DeserializationContext ctxt, AnnotatedMember member, JavaType type)
      throws JsonMappingException {
    AnnotationIntrospector intr = ctxt.getAnnotationIntrospector();
    if (intr == null) {
      return type;
    ...
    if (type.hasContentType()) { // that is, is either container- or reference-type
      ...
      TypeDeserializer contentTypeDeser =
          findPropertyContentTypeDeserializer(ctxt.getConfig(), type, (AnnotatedMember) member);
      if (contentTypeDeser != null) {
        type = type.withContentTypeHandler(contentTypeDeser);
      }
    }
    TypeDeserializer valueTypeDeser =
        findPropertyTypeDeserializer(ctxt.getConfig(), type, (AnnotatedMember) member);
  ...
  protected EnumResolver constructEnumResolver(
      Class<?> enumClass, DeserializationConfig config, AnnotatedMember jsonValueAccessor) {
    if (jsonValueAccessor != null) {
      if (config.canOverrideAccessModifiers()) {
        ClassUtil.checkAndFixAccess(
            jsonValueAccessor.getMember(),
  ...
  protected JavaType resolveType(
      DeserializationContext ctxt, BeanDescription beanDesc, JavaType type, AnnotatedMember member)
      throws JsonMappingException {
  ...
  /**
   * Helper class to contain default mappings for abstract JDK {@link java.util.Collection} and
   * {@link java.util.Map} types. Separated out here to defer cost of creating lookups until
  ...
  protected static class ContainerDefaultMappings {
    ...
    static {
      ...
      fallbacks.put(
          java.util.concurrent.ConcurrentNavigableMap.class.getName(),
  ...
  /**
   * Helper class to contain largish number of parameters that need to be passed during Creator
   * introspection.
   *
   * @since 2.12
   */
