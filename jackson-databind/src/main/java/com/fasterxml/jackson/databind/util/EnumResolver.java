...
/**
 * Helper class used to resolve String values (either JSON Object field names or regular String
 * values) into Java Enum instances.
 */
public class EnumResolver implements java.io.Serializable {
  ...
  /**
   * Marker for case where value may come from {@code @JsonValue} annotated accessor and is
   * expected/likely to come from actual integral number value (and not String).
   *
   * <p>Special case is needed since this specifically means that {@code Enum.index()} should NOT be
   * used or default to.
   *
   * @since 2.13
   */
   ...
   * @since 2.12
   */
  protected static EnumResolver _constructFor(
      Class<?> enumCls0, AnnotationIntrospector ai, boolean isIgnoreCase) {
    ...
    return new EnumResolver(
        enumCls, enumConstants, map, _enumDefault(ai, enumCls), isIgnoreCase, false);
  }
   ...
   * @since 2.12
   */
  public static EnumResolver constructUsingToString(
      DeserializationConfig config, Class<?> enumCls) {
    return _constructUsingToString(
        enumCls,
        config.getAnnotationIntrospector(),
        config.isEnabled(MapperFeature.ACCEPT_CASE_INSENSITIVE_ENUMS));
  }
   ...
   * @since 2.15
   */
  public static EnumResolver constructUsingIndex(
      DeserializationConfig config, Class<Enum<?>> enumCls) {
    return _constructUsingIndex(
        enumCls,
        config.getAnnotationIntrospector(),
        config.isEnabled(MapperFeature.ACCEPT_CASE_INSENSITIVE_ENUMS));
  }

  private static EnumResolver _constructUsingIndex(
      Class<Enum<?>> enumCls0, AnnotationIntrospector ai, boolean isIgnoreCase) {
    final Class<Enum<?>> enumCls = _enumClass(enumCls0);
    final Enum<?>[] enumConstants = _enumConstants(enumCls0);
   ...
   * @since 2.12
   */
  protected static EnumResolver _constructUsingToString(
      Class<?> enumCls0, AnnotationIntrospector ai, boolean isIgnoreCase) {
    final Class<Enum<?>> enumCls = _enumClass(enumCls0);
    final Enum<?>[] enumConstants = _enumConstants(enumCls0);
    HashMap<String, Enum<?>> map = new HashMap<String, Enum<?>>();
  ...
  /**
   * Method used when actual String serialization is indicated using @JsonValue on a method in Enum
   * class.
   *
   * @since 2.12
   */
  public static EnumResolver constructUsingMethod(
      DeserializationConfig config, Class<?> enumCls, AnnotatedMember accessor) {
    return _constructUsingMethod(
        enumCls,
        accessor,
        config.getAnnotationIntrospector(),
        config.isEnabled(MapperFeature.ACCEPT_CASE_INSENSITIVE_ENUMS));
   ...
   * @since 2.12
   */
  protected static EnumResolver _constructUsingMethod(
      Class<?> enumCls0,
    ...
    for (int i = enumConstants.length; --i >= 0; ) {
      ...
      try {
        ...
        throw new IllegalArgumentException(
            "Failed to access @JsonValue of Enum value " + en + ": " + e.getMessage());
      }
    }
    return new EnumResolver(
        enumCls,
  ...
  public static EnumResolver constructFor(Class<Enum<?>> enumCls, AnnotationIntrospector ai) {
    return _constructFor(ava, ai, false);
  }
  ...
  @Deprecated // since 2.12
  public static EnumResolver constructUsingToString(
      Class<Enum<?>> enumCls, AnnotationIntrospector ai) {
   ...
   */
  @Deprecated // since 2.12
  public static EnumResolver constructUnsafeUsingToString(
      Class<?> rawEnumCls, AnnotationIntrospector ai) {
  ...
  @Deprecated
  public static EnumResolver constructUsingMethod(
      Class<Enum<?>> enumCls, AnnotatedMember accessor, AnnotationIntrospector ai) {
   ...
   */
  @Deprecated
  public static EnumResolver constructUnsafeUsingMethod(
      Class<?> rawEnumCls, AnnotatedMember accessor, AnnotationIntrospector ai) {
   ...
   * {@code @JsonValue} annotated accessor with integral type: this matters for cases where incoming
   * content value is of integral type and should be mapped to specific value and NOT to {@code
   * Enum.index()}.
   *
   * @since 2.13
   */
