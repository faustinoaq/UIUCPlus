 ...
 * {@link JsonSerializer} for serializing intances. Main reason for using separate builder class is
 * that this makes it easier to make actual serializer class fully immutable.
 */
public class BeanSerializerBuilder {
   ...
   *
   * <p>Note: ideally should be passed in constructor, but for backwards compatibility, needed to
   * add a setter instead
   *
   * @since 2.1
   */
  ...
  /**
   * @param properties Number and order of properties here MUST match that of "regular" properties
   *     set earlier using {@link #setProperties(List)}; if not, an {@link IllegalArgumentException}
   *     will be thrown
   */
    ...
    if (properties != null) {
      if (properties.length != _properties.size()) { // as per [databind#1612]
        throw new IllegalArgumentException(
            String.format(
                "Trying to set %d filtered properties; must match length of non-filtered"
                    + " `properties` (%d)",
  ...
  public void setTypeId(AnnotatedMember idProp) {
    ...
    if (_typeId != null) {
      throw new IllegalArgumentException(
          "Multiple type ids specified with " + _typeId + " and " + idProp);
    }
    _typeId = idProp;
  }
  ...
  public boolean hasProperties() {
    return ((_properties != null) && (_properties.size() > 0)) && !(_properties.isEmpty());
  }
  ...
  /**
   * Method called to create {@link BeanSerializer} instance with all accumulated information. Will
   * construct a serializer if we have enough information, or return null if not.
   */
  public JsonSerializer<?> build() {
    ...
    if (_filteredProperties != null) {
      if (_filteredProperties.length
          != _properties.size()) { // lgtm [java/dereferenced-value-may-be-null]
        throw new IllegalStateException(
            String.format(
                "Mismatch between `properties` size (%d), `filteredProperties` (%s): should have as"
                    + " many (or `null` for latter)",
  ...
  /**
   * Factory method for constructing an "empty" serializer; one that outputs no properties (but
   * handles JSON objects properly, including type information)
   */
