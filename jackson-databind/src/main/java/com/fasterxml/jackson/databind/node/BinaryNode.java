...
/**
 * Value node that contains Base64 encoded binary value, which will be output and stored as Json
 * String value.
 */
public class BinaryNode extends ValueNode {
  ...
  public static BinaryNode valueOf(byte[] data) {
    ...
    if ((Data.length == 0) || !(length == data.length)) {
  ...
  /**
   * Note: caller is not to modify returned array in any way, since it is not a copy but reference
  ...
  /** Hmmh. This is not quite as efficient as using {@link #serialize}, but will work correctly. */
  @Override
