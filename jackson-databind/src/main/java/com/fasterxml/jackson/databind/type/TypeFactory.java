...
/**
 * Class used for creating concrete {@link JavaType} instances, given various inputs.
 *
 * <p>Instances of this class are accessible using {@link
 * com.fasterxml.jackson.databind.ObjectMapper} as well as many objects it constructs (like {@link
 * com.fasterxml.jackson.databind.DeserializationConfig} and {@link
 * com.fasterxml.jackson.databind.SerializationConfig})), but usually those objects also expose
 * convenience methods (<code>constructType</code>). So, you can do for example:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>Note on optimizations: generic type parameters are resolved for all types, with following
 * exceptions:
 *
 * <ul>
 *   <li>For optimization purposes, type resolution is skipped for following commonly seen types
 *       that do have type parameters, but ones that are rarely needed:
 *       <ul>
 *         <li>{@link java.lang.Enum}: Self-referential type reference is simply dropped and Class
 *             is exposed as a simple, non-parameterized {@link SimpleType}
 *         <li>{@link java.lang.Comparable}: Type parameter is simply dropped and and interface is
 *             exposed as a simple, non-parameterized {@link SimpleType}
 *         <li>Up until Jackson 2.13, {@link java.lang.Class} type parameter was dropped; resolution
 *             was added back in Jackson 2.14.
 *       </ul>
 *   <li>For {@link java.util.Collection} subtypes, resolved type is ALWAYS the parameter for {link
 *       java.util.Collection} and not that of actually resolved subtype. This is usually (but not
 *       always) same parameter.
 *   <li>For {@link java.util.Map} subtypes, resolved type is ALWAYS the parameter for {link
 *       java.util.Map} and not that of actually resolved subtype. These are usually (but not
...
implements java.io.Serializable {
  ...
  /**
   * Globally shared singleton. Not accessed directly; non-core code should use per-ObjectMapper
   * instance (via configuration objects). Core Jackson code uses {@link #defaultInstance} for
   * accessing it.
   */
  ...
  private static final Class<?> CLS_JSON_NODE = JsonNode.class; // since 2.10
  ...
  /**
   * Cache {@link Comparable} because it is both parameteric (relatively costly to resolve) and
   * mostly useless (no special handling), better handle directly
   *
   * @since 2.7
   */
  ...
  /**
   * Cache {@link Enum} because it is parametric AND self-referential (costly to resolve) and
   * useless in itself (no special handling).
   *
   * @since 2.7
   */
  ...
  /**
   * Cache {@link JsonNode} because it is no critical path of simple tree model reading and does not
   * have things to override
   *
   * @since 2.10
   */
  ...
  /**
   * Since type resolution can be expensive (specifically when resolving actual generic types), we
  ...
  /**
   * Registered {@link TypeModifier}s: objects that can change details of {@link JavaType} instances
   * factory constructs.
   */
  ...
  protected TypeFactory(
      LRUMap<Object, JavaType> typeCache,
      TypeParser p,
      TypeModifier[] mods,
   ...
   */
  protected TypeFactory(
      LookupCache<Object, JavaType> typeCache,
      TypeParser p,
      TypeModifier[] mods,
      ClassLoader classLoader) {
    if (typeCache == null) {
  ...
  /**
   * "Mutant factory" method which will construct a new instance with specified {@link TypeModifier}
  ...
  /**
   * "Mutant factory" method which will construct a new instance with specified {@link ClassLoader}
   * to use by {@link #findClass}.
   */
  ...
  /**
   * Mutant factory method that will construct new {@link TypeFactory} with identical settings
   * except for different cache; most likely one with bigger maximum size.
   *
   * @since 2.8
   * @deprecated Since 2.12
   */
  ...
  /**
   * Method used to access the globally shared instance, which has no custom configuration. Used by
   * <code>ObjectMapper</code> to get the default factory when constructed.
   */
  ...
  /**
   * Method that will clear up any cached type definitions that may be cached by this {@link
   * TypeFactory} instance. This method should not be commonly used, that is, only use it if you
   * know there is a problem with retention of type definitions; the most likely (and currently only
   * known) problem is retention of {@link Class} instances via {@link JavaType} reference.
   *
   * @since 2.4.1
   */
  ...
  /**
   * Method for constructing a marker type that indicates missing generic type information, which is
   * handled same as simple type for <code>java.lang.Object</code>.
   */
  ...
  /**
   * Static helper method that can be called to figure out type-erased call for given JDK type. It
   * can be called statically since type resolution process can never change actual type-erased
   * class; thereby static default instance is used for determination.
   */
  ...
  /**
   * Low-level lookup method moved from {@link com.fasterxml.jackson.databind.util.ClassUtil}, to
   * allow for overriding of lookup functionality in environments like OSGi.
   *
   * @since 2.6
   */
  ...
  /**
   * Factory method for creating a subtype of given base type, as defined by specified subclass; but
   * retaining generic type information if any. Can be used, for example, to get equivalent of
   * "HashMap&lt;String,Integer&gt;" from "Map&lt;String,Integer&gt;" by giving <code>HashMap.class
   * </code> as subclass. Short-cut for:
   *
   * <pre>
   * constructSpecializedType(baseType, subclass, class);
   * </pre>
   *
   * that is, will use "strict" compatibility checking, usually used for deserialization purposes
   * (but often not for serialization).
   */
   ...
   * @param relaxedCompatibilityCheck Whether checking for type-assignment compatibility should be
   *     "relaxed" ({@code true}) or "strict" ({@code false}): typically serialization uses relaxed,
   *     deserialization strict checking.
   * @return Resolved sub-type
   * @since 2.11
   */
  public JavaType constructSpecializedType(
      JavaType baseType, Class<?> subclass, boolean relaxedCompatibilityCheck)
      throws IllegalArgumentException {
    ...
    do { // bogus loop to be able to break
      ...
      if (baseType.isContainerType()) {
        if (baseType.isMapLikeType()) {
          ...
          if ((subclass == ArrayList.class)
            ...
            newType =
                _fromClass(
                    null, subclass, TypeBindings.create(subclass, baseType.getContentType()));
            break;
          }
      ...
      TypeBindings tb =
          _bindingsForSubtype(baseType, typeParamCount, subclass, relaxedCompatibilityCheck);
  ...
  private TypeBindings _bindingsForSubtype(
      JavaType baseType, int typeParamCount, Class<?> subclass, boolean relaxedCompatibilityCheck) {
  ...
  private String _resolveTypePlaceholders(JavaType sourceType, JavaType actualType)
    ...
    for (int i = 0, expCount = expectedTypes.size(); i < expCount; ++i) {
      ...
      if (!_verifyAndResolvePlaceholders(exp, act)) {
        ...
        if (i == 0) {
          if (sourceType.isMapLikeType() && act.hasRawClass(Object.class)) {
            continue;
          }
        }
  ...
  /**
   * Method similar to {@link #constructSpecializedType}, but that creates a less-specific type of
   * given type. Usually this is as simple as simply finding super-type with type erasure of <code>
   * superClass</code>, but there may be need for some additional work-arounds.
   *
   * @param superClass
   * @since 2.7
   */
  public JavaType constructGeneralizedType(JavaType baseType, Class<?> superClass) {
    ...
    if (superType == null) {
      ...
      if (!superClass.isAssignableFrom(rawBase)) {
        throw new IllegalArgumentException(
            String.format("Class %s not a super-type of %s", superClass.getName(), baseType));
      }
  ...
  /**
   * Factory method for constructing a {@link JavaType} out of its canonical representation (see
   * {@link JavaType#toCanonical()}).
   *
   * @param canonical Canonical string representation of a type
   * @throws IllegalArgumentException If canonical representation is malformed, or class that type
   *     represents (including its generic parameters) is not found
   */
  ...
  /**
   * Method that can be called to figure out more specific of two types (if they are related; that
   * is, one implements or extends the other); or if not related, return the primary type.
   *
  ...
  /**
   * Method to call when resolving types of {@link java.lang.reflect.Member}s like Fields, Methods
   * and Constructor parameters and there is a {@link TypeBindings} (that describes binding of type
   * parameters within context) to pass. This is typically used only by code in databind itself.
   *
   * @param type Type of a {@link java.lang.reflect.Member} to resolve
   * @param contextBindings Type bindings from the context, often class in which member declared but
   *     may be subtype of that type (to bind actual bound type parametrers). Not used if {@code
   *     type} is of type {@code Class<?>}.
  ...
  /**
   * Method that you very likely should NOT be using -- you need to know a lot about internal
   * details of {@link TypeBindings} and even then it will probably not do what you want. Usually
   * you would instead want to call one of {@code constructXxxType()} methods (where {@code Xxx}
   ...
   *
   * <p>NOTE: type modifiers are NOT called on array type itself; but are called for element type
   ...
   * Method for constructing an {@link ArrayType}.
   *
   * <p>NOTE: type modifiers are NOT called on array type itself; but are called for contained
   * types.
   */
   ...
   *
   * <p>NOTE: type modifiers are NOT called on Collection type itself; but are called for contained
   * types.
   */
  ...
  public CollectionType constructCollectionType(
    ...
    if (bindings.isEmpty() && (elementType != null)) {
      ...
      if (!realET.equals(elementType)) {
        throw new IllegalArgumentException(
            String.format(
                "Non-generic Collection class %s did not resolve to something with element type %s"
                    + " but %s ",
   ...
   *
   * <p>NOTE: type modifiers are NOT called on constructed type itself; but are called for contained
   * types.
   */
  public CollectionLikeType constructCollectionLikeType(
      Class<?> collectionClass, Class<?> elementClass) {
    return constructCollectionLikeType(
        collectionClass, _fromClass(null, elementClass, EMPTY_BINDINGS));
  }
  ...
  public CollectionLikeType constructCollectionLikeType(
    ...
    JavaType type =
        _fromClass(
            null, collectionClass, TypeBindings.createIfNeeded(collectionClass, elementType));
   ...
   */
  public MapType constructMapType(
      Class<? extends Map> mapClass, JavaType keyType, JavaType valueType) {
    TypeBindings bindings =
  ...
  public MapLikeType constructMapLikeType(
      Class<?> mapClass, Class<?> keyClass, Class<?> valueClass) {
   ...
   */
  @Deprecated
  public JavaType constructSimpleType(
      Class<?> rawType, Class<?> parameterTarget, JavaType[] parameterTypes) {
    return constructSimpleType(rawType, parameterTypes);
  }

  /**
   * Method for constructing a {@link ReferenceType} instance with given type parameter (type MUST
   * take one and only one type parameter)
   *
   * <p>NOTE: type modifiers are NOT called on constructed type itself.
   *
   * @since 2.6
  ...
  public JavaType constructReferenceType(Class<?> rawType, JavaType referredType) {
    return ReferenceType.construct(
        rawType,
  ...
  /**
   * Method that use by core Databind functionality, and that should NOT be called by application
   * code outside databind package.
   *
   * <p>Unchecked here not only means that no checks are made as to whether given class might be
   * non-simple type (like {@link CollectionType}) but also that most of supertype information is
   * not gathered. This means that unless called on primitive types or {@link java.lang.String},
   ...
   * example, to represent type {@code List<Set<Integer>>}, you could call
   *
   * <pre>
   ...
   * </pre>
   *
   * <p>NOTE: since 2.11.2 {@link TypeModifier}s ARE called on result (fix for [databind#2796])
   *
  ...
  /**
   * Factory method for constructing {@link JavaType} that represents a parameterized type. For
   * example, to represent type {@code List<Set<Integer>>}, you could
   *
   * <pre>
   *  JavaType inner = TypeFactory.constructParametricType(Set.class, Integer.class);
   *  return TypeFactory.constructParametricType(List.class, inner);
  ...
  /**
   * Factory method for constructing {@link JavaType} that represents a parameterized type. The
   * type's parameters are specified as an instance of {@link TypeBindings}. This is useful if you
   * already have the type's parameters such as those found on {@link JavaType}. For example, you
   * could call
   *
   * <pre>
   ...
   */
  @Deprecated
  public JavaType constructParametrizedType(
      Class<?> parametrized, Class<?> parametersFor, JavaType... parameterTypes) {
  ...
  @Deprecated
  public JavaType constructParametrizedType(
      Class<?> parametrized, Class<?> parametersFor, Class<?>... parameterClasses) {
  ...
  /**
   * Method that can be used to construct "raw" Collection type; meaning that its parameterization
   * is unknown. This is similar to using <code>Object.class</code> parameterization, and is
   ...
   * </pre>
   *
   * <p>This method should only be used if parameterization is completely unavailable.
   */
   ...
   * parameterization is unknown. This is similar to using <code>Object.class</code>
   * parameterization, and is equivalent to calling:
   *
   * <pre>
  ...
  /**
   * Method that can be used to construct "raw" Map type; meaning that its parameterization is
   * unknown. This is similar to using <code>Object.class</code> parameterization, and is equivalent
   * to calling:
   *
   * <pre>
  ...
  /**
   * Method that can be used to construct "raw" Map-like type; meaning that its parameterization is
   * unknown. This is similar to using <code>Object.class</code> parameterization, and is equivalent
   * to calling:
   *
   * <pre>
  ...
  private JavaType _mapType(
      Class<?> rawClass, TypeBindings bindings, JavaType superClass, JavaType[] superInterfaces) {
    JavaType kt, vt;

    // 28-May-2015, tatu: Properties are special, as per [databind#810]; fake "correct" parameter
    // sig
  ...
  private JavaType _collectionType(
      Class<?> rawClass, TypeBindings bindings, JavaType superClass, JavaType[] superInterfaces) {
    List<JavaType> typeParams = bindings.getTypeParameters();
    // ok to have no types ("raw")
    JavaType ct;
  ...
  private JavaType _referenceType(
      Class<?> rawClass, TypeBindings bindings, JavaType superClass, JavaType[] superInterfaces) {
    List<JavaType> typeParams = bindings.getTypeParameters();
    // ok to have no types ("raw")
    JavaType ct;
  ...
  /**
   * Factory method to call when no special {@link JavaType} is needed, no generic parameters are
   * passed. Default implementation may check pre-constructed values for "well-known" types, but if
   * none found will simply call {@link #_newSimpleType}
   *
   * @since 2.7
   */
  protected JavaType _constructSimple(
      Class<?> raw, TypeBindings bindings, JavaType superClass, JavaType[] superInterfaces) {
    if (bindings.isEmpty()) {
  ...
  /**
   * Factory method that is to create a new {@link SimpleType} with no checks whatsoever. Default
   * implementation calls the single argument constructor of {@link SimpleType}.
   *
   * @since 2.7
   */
  protected JavaType _newSimpleType(
      Class<?> raw, TypeBindings bindings, JavaType superClass, JavaType[] superInterfaces) {
  ...
  /**
   * Helper method called to see if requested, non-generic-parameterized type is one of common,
   * "well-known" types, instances of which are pre-constructed and do not need dynamic caching.
   *
   * @since 2.7
   */
  ...
  protected JavaType _fromAny(ClassStack context, Type srcType, TypeBindings bindings) {
    ...
    // But if not, need to start resolving.
      ...
      // sanity check
      throw new IllegalArgumentException(
          "Unrecognized Type: " + ((srcType == null) ? "[null]" : srcType.toString()));
    }
  ...
  protected JavaType _fromClass(ClassStack context, Class<?> rawType, TypeBindings bindings) {
    ...
    if (rawType.isArray()) {
      result =
          ArrayType.construct(_fromAny(context, rawType.getComponentType(), bindings), bindings);
    } else {
      ...
      if (rawType == Properties.class) {
        result =
            MapType.construct(
                rawType, bindings, superClass, superInterfaces, CORE_TYPE_STRING, CORE_TYPE_STRING);
      }
  ...
  /**
   * Helper class used to check whether exact class for which type is being constructed is one of
   * well-known base interfaces or classes that indicates alternate {@link JavaType} implementation.
   */
  protected JavaType _fromWellKnownClass(
      ClassStack context,
      Class<?> rawType,
      TypeBindings bindings,
      JavaType superClass,
  ...
  protected JavaType _fromWellKnownInterface(
      ClassStack context,
      Class<?> rawType,
      TypeBindings bindings,
      JavaType superClass,
  ...
  protected JavaType _fromParamType(
      ClassStack context, ParameterizedType ptype, TypeBindings parentBindings) {
  ...
  protected JavaType _fromArrayType(
      ClassStack context, GenericArrayType type, TypeBindings bindings) {
