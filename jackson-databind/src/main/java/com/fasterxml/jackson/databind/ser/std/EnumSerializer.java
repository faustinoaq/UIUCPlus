 ...
 *
 * <p>Based on {@link StdScalarSerializer} since the JSON value is scalar (String).
 */
@JacksonStdImpl
public class EnumSerializer extends StdScalarSerializer<Enum<?>> implements ContextualSerializer {
  ...
  /**
   * This map contains pre-resolved values (since there are ways to customize actual String
   * constants to use) to use as serializations.
   */
  protected final EnumValues _values;

  /**
   * Flag that is set if we statically know serialization choice between index and textual format
   * (null if it needs to be dynamically checked).
   *
   * @since 2.1
   */
  ...
  /**
   * Factory method used by {@link com.fasterxml.jackson.databind.ser.BasicSerializerFactory} for
   * constructing serializer instance of Enum types.
   *
   * @since 2.1
   */
  @SuppressWarnings("unchecked")
  public static EnumSerializer construct(
      Class<?> enumClass,
  ...
  /**
   * To support some level of per-property configuration, we will need to make things contextual. We
   * are limited to "textual vs index" choice here, however.
   */
  @Override
   ...
   */
  protected static Boolean _isShapeWrittenUsingIndex(
      Class<?> enumClass, JsonFormat.Value format, boolean fromClass, Boolean defaultValue) {
