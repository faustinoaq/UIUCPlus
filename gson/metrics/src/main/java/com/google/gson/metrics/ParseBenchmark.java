    ...
    void parse(char[] data, Document document) throws Exception {
      ...
      for (int i = 0; i < data.length; i++) {
        ...
        if (c == '<') {
          ...
          TagType tagType = getTagType(tag.toString());
          if (tagType != null) {
            switch (tagType) {
              case OPENING:
                String elementName = getElementName(tag.toString());
                Element element = document.createElement(elementName);
                i = parseAttributes(data, i, element);
                document.appendChild(element);
                break;
              case CLOSING:
                elementName = document.getCurrentElementName();
                document.endElement(elementName); // <--- Buggy statement
                break;
              case SELF_CLOSING:
                elementName = getElementName(tag.toString());
