...
public final class ReflectiveTypeAdapterFactory implements TypeAdapterFactory {
  ...
  private Map<String, BoundField> getBoundFields(
    ...
    while (raw != Object.class) {
      ...
      for (Field field : fields) {
        ...
        // values
        // via its accessor method instead of via reflection. This way we will bypass the accessible
        // restrictions
        Method accessor = null;
        if (isRecord) {
          // If there is a static field on a record, there will not be an accessor. Instead we will
          // use the default
          // field serialization logic, but for deserialization the field is excluded for
          // simplicity. Note that Gson
          // ignores static fields by default, but GsonBuilder.excludeFieldsWithModifiers can
          // overwrite this.
  ...
  private static final class FieldReflectionAdapter<T> extends Adapter<T, T> {
    ...
    @Override
    void readField(T accumulator, JsonReader in, BoundField field) throws NoSuchFieldException {
