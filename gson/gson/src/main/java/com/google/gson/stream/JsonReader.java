...
public class JsonReader implements Closeable {
   ...
   * only accepts JSON as specified by <a href="http://www.ietf.org/rfc/rfc4627.txt">RFC 4627</a>.
   ...
   * </ul>
   *
   * <p>Note: Even in strict mode there are slight derivations from the JSON specification:
   *
   * <ul>
   *   <li>JsonReader allows the literals {@code true}, {@code false} and {@code null} to have any
   *       capitalization, for example {@code fAlSe}
   *   <li>JsonReader supports the escape sequence {@code \'}, representing a {@code '}
   *   <li>JsonReader supports the escape sequence <code>\<i>LF</i></code> (with {@code LF} being
   *       the Unicode character U+000A), resulting in a {@code LF} within the read JSON string
  ...
  private void push(int newTop) {
    if (stackSize == stack.length) {
      ...
      pathIndices = Arrays.copyOf(pathIndices, newLength + 1);
