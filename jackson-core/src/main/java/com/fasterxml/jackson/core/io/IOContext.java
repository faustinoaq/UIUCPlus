...
/**
 * To limit number of configuration and state objects to pass, all contextual objects that need to
 * be passed by the factory to readers and writers are combined under this object. One instance is
 * created for each reader and writer.
 *
 * <p>NOTE: non-final since 2.4, to allow sub-classing.
 */
public class IOContext {
  ...
  /**
   * Flag that indicates whether underlying input/output source/target object is fully managed by
   * the owner of this context (parser or generator). If true, it is, and is to be closed by
   * parser/generator; if false, calling application has to do closing (unless auto-closing feature
   * is enabled for the parser/generator in question; in which case it acts like the owner).
   */
  ...
  /**
   * Reference to the buffer allocated for tokenization purposes, in which character input is read,
   * and from which it can be further returned.
   */
  protected char[] _tokenCBuffer;

  /**
   * Reference to the buffer allocated for buffering it for output, before being encoded: generally
   * this means concatenating output, then encoding when buffer fills up.
   */
  protected char[] _concatCBuffer;

  /**
   * Reference temporary buffer Parser instances need if calling app decides it wants to access name
   * via 'getTextCharacters' method. Regular text buffer can not be used as it may contain textual
  ...
  /**
   * Accessor for getting (some) information about input source, mostly usable for error reporting
   * purposes.
   *
   ...
   *
   * <p>Note: the method can only be called once during its life cycle. This is to protect against
   * accidental sharing.
   *
  ...
  public byte[] allocReadIOBuffer(int minSize) {
    ...
    return (_readIOBuffer =
        _bufferRecycler.allocByteBuffer(BufferRecycler.BYTE_READ_IO_BUFFER, minSize));
  }
  ...
  public byte[] allocWriteEncodingBuffer() {
    ...
    return (_writeEncodingBuffer =
        _bufferRecycler.allocByteBuffer(BufferRecycler.BYTE_WRITE_ENCODING_BUFFER));
  }
  ...
  public byte[] allocBase64Buffer() {
    ...
    return (_base64Buffer =
        _bufferRecycler.allocByteBuffer(BufferRecycler.BYTE_BASE64_CODEC_BUFFER));
  }
  ...
  public byte[] allocBase64Buffer(int minSize) {
    _verifyAlloc(_base64Buffer);
    return (_base64Buffer =
        _bufferRecycler.allocByteBuffer(BufferRecycler.BYTE_BASE64_CODEC_BUFFER, minSize));
  }
  ...
  public char[] allocTokenBuffer(int minSize) {
    ...
    return (_tokenCBuffer =
        _bufferRecycler.allocCharBuffer(BufferRecycler.CHAR_TOKEN_BUFFER, minSize));
  }
  ...
  public char[] allocNameCopyBuffer(int minSize) {
    ...
    return (_nameCopyBuffer =
        _bufferRecycler.allocCharBuffer(BufferRecycler.CHAR_NAME_COPY_BUFFER, minSize));
  }
  ...
  protected final void _verifyRelease(char[] toRelease, char[] src) {
    // 07-Mar-2016, tatu: As per [core#255], only prevent shrinking of buffer
    if ((toRelease != this._tokenCBuffer) || ((toRelease != src) && (toRelease.length < src.len))) {
      throw wrongBuf();
    }
  }
