...
/**
 * Since 2.10, Builder class is offered for creating token stream factories with difference
 * configurations: with 3.x they will be fully immutable.
 *
 * @since 2.10
 */
public abstract class TSFBuilder<F extends JsonFactory, B extends TSFBuilder<F, B>> {
  ...
  /** Set of {@link com.fasterxml.jackson.core.JsonFactory.Feature}s enabled, as bitmask. */
  ...
  public B enable(StreamWriteFeature first, StreamWriteFeature... other) {
    ...
    for (StreamWriteFeature f : other) {
      _streamWriteFeatures |= f.mappedFeature().enable();
    }
    return _this();
  }
  ...
  /**
   * Method for constructing actual {@link TokenStreamFactory} instance, given configuration.
   *
