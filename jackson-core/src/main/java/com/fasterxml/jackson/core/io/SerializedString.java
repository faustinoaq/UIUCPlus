...
/**
 * String token that can lazily serialize String contained and then reuse that serialization later
 * on. This is similar to JDBC prepared statements, for example, in that instances should only be
 * created when they are used more than use; prime candidates are various serializers.
 *
 * <p>Class is final for performance reasons and since this is not designed to be extensible or
 * customizable (customizations would occur in calling code)
 */
public class SerializedString implements SerializableString, java.io.Serializable {
  ...
  /**
   * Ugly hack, to work through the requirement that _value is indeed final, and that JDK
   * serialization won't call ctor(s).
   *
   * @since 2.1
   */
  ...
  /**
   * Accessor for accessing value that has been quoted (escaped) using JSON quoting rules (using
   * backslash-prefixed codes) into a char array.
   */
  @Override
  ...
  /**
   * Accessor for accessing value that has been quoted (escaped) using JSON quoting rules (using
   * backslash-prefixed codes), and encoded using UTF-8 encoding into a byte array.
   */
  @Override
  ...
  /**
   * Accessor for accessing value as is (without JSON quoting (ecaping)) encoded as UTF-8 byte
   * array.
   */
  @Override
  ...
  public int appendQuoted(char[] buffer, int offset) {
    ...
    if (((offset + length) > buffer.length) && (length > buffer.remaining())) {
      return -1;
    }
