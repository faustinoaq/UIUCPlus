...
/**
 * Intermediate base class used by all Jackson {@link JsonParser} implementations. Contains most
 * common things that are independent of actual underlying input source.
 */
public abstract class ParserBase extends ParserMinimalBase {
  ...
  /**
   * Number of characters/bytes that were contained in previous blocks (blocks that were already
   * processed prior to the current buffer).
   */
  ...
  /**
   * Current index of the first character of the current row in input buffer. Needed to calculate
   * column position, if necessary; benefit of not having column itself is that this only has to be
  ...
  /**
   * Total number of bytes/characters read before start of current token. For big (gigabyte-sized)
   * sizes are possible, needs to be long, unlike pointers and sizes related to in-memory buffers.
   */
  ...
  /**
   * Column on input row that current token starts; 0-based (although in the end it'll be converted
   * to 1-based)
   */
  ...
  /**
   * Secondary token related to the next token after current one; used if its type is known. This
   * may be value token that follows FIELD_NAME, for example.
   */
  ...
  /**
   * Buffer that contains contents of String values, including field names if necessary (name split
   * across boundary, contains escape sequence, or access needed to char array)
   */
  protected final TextBuffer _textBuffer;

  /**
   * Temporary buffer that is needed if field name is accessed using {@link #getTextCharacters}
   * method (instead of String returning alternatives)
   */
  protected char[] _nameCopyBuffer;

  /**
   * Flag set to indicate whether the field name is available from the name copy buffer or not (in
   * addition to its String representation being available via read context)
   */
   ...
   *
   * <p>As of 2.14, this only applies to {@link BigInteger} and {@link BigDecimal}.
   *
   * @since 2.14
   */
  ...
  /**
   * Flag that indicates whether numeric value has a negative value. That is, whether its textual
   * representation starts with minus character.
   */
  ...
  /**
   * Length of the fractional part (not including decimal point or exponent), in characters. Not
   * used for pure integer values.
   */
  protected int _fractLength;

  /**
   * Length of the exponent part of the number, if any, not including 'e' marker or sign, just
   * digits. Not used for pure integer values.
   */
  ...
  protected ParserBase(IOContext ctxt, int features) {
    ...
    DupDetector dups =
        Feature.STRICT_DUPLICATE_DETECTION.enabledIn(features)
  ...
  public byte[] getBinaryValue(Base64Variant variant) throws IOException {
    if (_binaryValue == null) {
      if (_currToken != JsonToken.VALUE_STRING) {
        _reportError(
            "Current token (" + _currToken + ") not VALUE_STRING, can not access as binary");
      }
   ...
   *
   * @throws IOException Not thrown by base implementation but could be thrown by sub-classes
   */
    ...
    if (!_parsingContext.inRoot()) {
      ...
      _reportInvalidEOF(
          String.format(
              ...
              marker, _parsingContext.startLocation(_contentReference())),
          null);
    }
  }
   ...
   * @throws JsonParseException If check on {@code _handleEOF()} fails; usually because the current
   *     context is not root context (missing end markers in content)
   * @since 2.4
   */
  ...
  protected final JsonToken buggyMethod1(boolean negative, int intLen, int fractLen, int expLen)
      throws JsonParseException {
    try {
  ...
  /**
   * Internal accessor that needs to be used for accessing number value of type {@link BigInteger}
   * which -- as of 2.14 -- is typically lazily parsed.
   *
   * @since 2.14
   */
    ...
    try {
      ...
      _numberBigInt =
          NumberInput.parseBigInteger(
              _numberString, isEnabled(StreamReadFeature.USE_FAST_BIG_NUMBER_PARSER));
  ...
  /**
   * Internal accessor that needs to be used for accessing number value of type {@link BigDecimal}
   * which -- as of 2.14 -- is typically lazily parsed.
   *
   * @since 2.14
   */
  ...
  /**
   * Internal accessor that needs to be used for accessing number value of type {@code double} which
   * -- as of 2.15 -- will be lazily parsed.
   *
   * @since 2.15
   */
      ...
      try {
        _numberDouble =
            NumberInput.parseDouble(
                _numberString, isEnabled(StreamReadFeature.USE_FAST_DOUBLE_PARSER));
      } catch (NumberFormatException nex) {
        _wrapError("Malformed numeric value (" + _longNumberDesc(_numberString) + ")", nex);
      }
      _numberString = null;
  ...
  /**
   * Internal accessor that needs to be used for accessing number value of type {@code float} which
   * -- as of 2.15 -- will be lazily parsed.
   *
   * @since 2.15
   */
  ...
  /**
   * Method called to report a problem with unquoted control character. Note: it is possible to
   * suppress some instances of exception by enabling {@link
  ...
  /**
   * @return Description to use as "valid tokens" in an exception message about invalid
   *     (unrecognized) JSON token: called when parser finds something that looks like unquoted
   *     textual token
  ...
  /**
   * @return Description to use as "valid JSON values" in an exception message about invalid
   *     (unrecognized) JSON value: called when parser finds something that does not look like a
   *     value or separator.
   * @throws IOException Not thrown by base implementation but allowed by sub-classes
   * @since 2.10
   */
  ...
  protected String _validJsonValueList() throws IOException {
    if (isEnabled(Feature.ALLOW_NON_NUMERIC_NUMBERS)) {
      return "(JSON String, Number (or 'NaN'/'INF'/'+INF'), Array, Object or token 'null', 'true'"
                 + " or 'false')";
    }
  ...
  protected IllegalArgumentException reportInvalidBase64Char(
      Base64Variant b64variant, int ch, int bindex) throws IllegalArgumentException {
  ...
  protected IllegalArgumentException reportInvalidBase64Char(
    ...
    if (ch <= INT_SPACE) {
      base =
          String.format(
              "Illegal white space character (code 0x%s) as character #%d of 4-char base64 unit:"
                  + " can only used between units",
  ...
  /**
   * Helper method used to encapsulate logic of including (or not) of "content reference" when
   * constructing {@link JsonLocation} instances.
   *
