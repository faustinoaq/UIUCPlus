...
/**
 * {@link com.fasterxml.jackson.core.TSFBuilder} implementation for constructing vanilla {@link
 * JsonFactory} instances for reading/writing JSON encoded content.
 *
 * <p>NOTE: as of Jackson 2.x, use of JSON-specific builder is bit cumbersome since {@link
 * JsonFactory} serves dual duty of base class AND actual implementation for JSON backend. This will
 * be fixed in Jackson 3.0.
 *
 * @since 2.10
 */
public class JsonFactoryBuilder extends TSFBuilder<JsonFactory, JsonFactoryBuilder> {
  ...
  public JsonFactoryBuilder rootValueSeparator(String sep) {
    _rootValueSeparator = (sep != null) ? new SerializedString(sep) : null;
    return this;
  }
  ...
  /**
   * Method that allows specifying threshold beyond which all characters are automatically escaped
   * (without checking possible custom escaping settings a la {@link #characterEscapes}: for
   * example, to force escaping of all non-ASCII characters (set to 127), or all non-Latin-1
   * character (set to 255). Default setting is "disabled", specified by passing value of {@code 0}
   * (or negative numbers).
   *
   * <p>NOTE! Lowest legal value (aside from marker 0) is 127: for ASCII range, other checks apply
   * and this threshold is ignored. If value between [1, 126] is specified, 127 will be used
   * instead.
   *
   * @param maxNonEscaped Highest character code that is NOT automatically escaped; if positive
   *     value above 0, or 0 to indicate that no automatic escaping is applied beside from what JSON
   *     specification requires (and possible custom escape settings). Values between 1 and 127 are
   *     all taken to behave as if 127 is specified: that is, no automatic escaping is applied in
   *     ASCII range.
   * @return This builder instance (to allow call chaining)
   */
  ...
  /**
   * Method that allows specifying an alternate character used for quoting field names (if field
   * name quoting has not been disabled with {@link JsonWriteFeature#QUOTE_FIELD_NAMES}) and JSON
   * String values.
   *
   * <p>Default value is double-quote ({@code "}); typical alternative is single-quote/apostrophe
   * ({@code '}).
   *
