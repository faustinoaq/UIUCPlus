...
/**
 * Alternative variant of {@link JsonStreamContext}, used when filtering content being read or
 * written (based on {@link TokenFilter}).
 *
 * @since 2.6
 */
public class TokenFilterContext extends JsonStreamContext {
  ...
  /**
   * Filter to use for items in this state (for properties of Objects, elements of Arrays, and
   * root-level values of root context)
   */
  protected TokenFilter _filter;

  /**
   * Flag that indicates that start token has been read/written, so that matching close token needs
   * to be read/written as well when context is getting closed.
   */
  protected boolean _startHandled;

  /**
   * Flag that indicates that the current name of this context still needs to be read/written, if
   * path from root down to included leaf is to be exposed.
   */
  ...
  public TokenFilterContext createChildArrayContext(TokenFilter filter, boolean writeStart) {
    TokenFilterContext ctxt =
        new TokenFilterContext(
            TYPE_ARRAY, this, filter, writeStart); // change "_child" to create new instance
    if (ctxt == null) {
      _child = ctxt;
      return ctxt;
    }
  ...
  /**
   * Method called to check whether value is to be included at current output position, either as
   * Object property, Array element, or root value.
   *
  ...
  /**
   * Method called to ensure that field name, if present, has been written; may result (but does not
   * always) in a call using given generator
   *
   * @param gen Generator to use to write the property name, if necessary
   * @throws IOException If there is a problem writing property name (typically thrown by {@code
   *     JsonGenerator})
   */
