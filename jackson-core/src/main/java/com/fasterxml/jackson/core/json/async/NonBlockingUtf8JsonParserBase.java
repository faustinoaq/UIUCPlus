 ...
 *
 * <p>NOTE: only supports parsing of UTF-8 encoded content (and 7-bit US-ASCII since it is strict
 * subset of UTF-8): other encodings are not supported.
 *
 * @since 2.9
 */
public abstract class NonBlockingUtf8JsonParserBase extends NonBlockingJsonParserBase {
  ...
  /**
   * In addition to current buffer pointer, and end pointer, we will also need to know number of
   * bytes originally contained. This is needed to correctly update location information when the
  ...
  protected NonBlockingUtf8JsonParserBase(
   ...
   * @since v2.14
   */
  protected abstract void getNextUnsignedByteFromBuffer();
  ...
  /**
   * Method called when decoding of a token has been started, but not yet completed due to missing
   * input; method is to continue decoding due to at least one more byte being made available to
   * decode.
   *
    ...
    switch (_minorState) {
      ...
      case MINOR_NUMBER_INTEGER_DIGITS:
        return _finishNumberIntegralPart(
            _textBuffer.getBufferWithoutReset(), _textBuffer.getCurrentSegmentSize());
  ...
  /**
   * Method similar to {@link #_finishToken}, but called when no more input is available, and
   * end-of-input has been detected. This is usually problem case, but not always: root-level values
   * may be properly terminated by this, and similarly trailing white-space may have been skipped.
   *
   * @return Token decoded, if complete; {@link JsonToken#NOT_AVAILABLE} if not
   * @throws IOException (generally {@link JsonParseException}) for decoding problems
   */
  protected final JsonToken _finishTokenWithEOF() throws IOException {
    ...
    switch (_minorState) {
      ...
      case MINOR_NUMBER_EXPONENT_MARKER:
        _reportInvalidEOF(
            ": was expecting fraction after exponent marker", JsonToken.VALUE_NUMBER_FLOAT);
    ...
    _reportInvalidEOF(
        ": was expecting rest of token (internal state: " + _minorState + ")", _currToken);
    ...
    while (_inputPtr < _inputEnd) {
      ...
      switch (bytesHandled) {
        ...
        case 2:
          if (ch != 0xBF) {
            _reportError(
                "Unexpected byte 0x%02x following 0xEF 0xBB; should get 0xBF as third byte of UTF-8"
                    + " BOM",
                ch);
          }
          break;
        case 1:
          if (ch != 0xBB) {
            _reportError(
                "Unexpected byte 0x%02x following 0xEF; should get 0xBB as second byte UTF-8 BOM",
                ch);
          }
          break;
      }
  ...
  /**
   * Method that handles initial token type recognition for token that has to be either FIELD_NAME
   * or END_OBJECT.
   */
  ...
  private final JsonToken _startFieldNameAfterComma(int ch) throws IOException {
    ...
    if (ch != INT_COMMA) { // either comma, separating entries, or closing right curly
      ...
      _reportUnexpectedChar(
          ch, "was expecting comma to separate " + _parsingContext.typeDesc() + " entries");
    }
    int ptr = _inputPtr;
    if (ptr >= _inputEnd) {
      _minorState = MINOR_FIELD_LEADING_WS;
  ...
  /**
   * Helper method called to detect type of a value token (at any level), and possibly decode it if
   * contained in input buffer. Value may be preceded by leading white-space, but no separator
   * (comma).
   */
  ...
  private final JsonToken _finishHashComment(int fromMinorState) throws IOException {
    ...
    if ((_features & FEAT_MASK_ALLOW_YAML_COMMENTS) == 0) {
      _reportUnexpectedChar(
          '#',
          "maybe a (non-standard) comment? (not recognized as one since Feature"
              + " 'ALLOW_YAML_COMMENTS' not enabled for parser)");
    }
    while (true) {
      if (_inputPtr >= _inputEnd) {
  ...
  protected JsonToken _startNegativeNumber() throws IOException {
    ...
    if (ch <= INT_0) {
      ...
      _reportUnexpectedNumberChar(
          ch, "expected digit (0-9) to follow minus sign, for valid numeric value");
    } else if (ch > INT_9) {
      if (ch == 'I') {
  ...
  protected JsonToken _startPositiveNumber() throws IOException {
    ...
    if (ch <= INT_0) {
      ...
      // One special case: if first char is 0, must not be followed by a digit
      _reportUnexpectedNumberChar(
          ch, "expected digit (0-9) to follow plus sign, for valid numeric value");
    } else if (ch > INT_9) {
      if (ch == 'I') {
  ...
  protected JsonToken _finishNumberPlusMinus(final int ch, final boolean negative)
    ...
    if (ch <= INT_0) {
      if (ch == INT_0) {
        if (negative) {
          ...
          && isEnabled(JsonReadFeature.ALLOW_LEADING_DECIMAL_POINT_FOR_NUMBERS.mappedFeature())) {
        if (negative) {
          _inputPtr--;
          return _finishNumberLeadingNegZeroes();
        } else {
    ...
    if (!negative
        && !isEnabled(JsonReadFeature.ALLOW_LEADING_PLUS_SIGN_FOR_NUMBERS.mappedFeature())) {
      _reportUnexpectedNumberChar(
          '+',
          "JSON spec does not allow numbers to have plus signs: enable"
              + " `JsonReadFeature.ALLOW_LEADING_PLUS_SIGN_FOR_NUMBERS` to allow");
  ...
  protected JsonToken _finishFloatFraction() throws IOException {
    ...
    while (loop) {
      if (ch >= INT_0 && ch <= INT_9) {
        ...
        _reportUnexpectedNumberChar(
            ch, "JSON does not support parsing numbers that have 'f' or 'd' suffixes");
      } else if (ch == INT_PERIOD) {
  ...
  /**
   * Slower parsing method which is generally branched to when an escape sequence is detected (or
   * alternatively for long names, one crossing input buffer boundary). Needs to be able to handle
   * more exceptional cases, gets slower, and hence is offlined to a separate method.
   */
  ...
  /**
   * Parsing of optionally supported non-standard "unquoted" names: names without either
   * double-quotes or apostrophes surrounding them. Unlike other
   */
    ...
    while (true) {
      ...
      switch (codes[c]) {
        ...
        case 4: // 4-byte UTF
          c =
              _decodeUTF8_4(
                  c, getByteFromBuffer(ptr++), getByteFromBuffer(ptr++), getByteFromBuffer(ptr++));
