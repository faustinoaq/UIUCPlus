...
/**
 * Intermediate base class used by all Jackson {@link JsonParser} implementations, but does not add
 * any additional fields that depend on particular method of obtaining input.
 *
 * <p>Note that 'minimal' here mostly refers to minimal number of fields (size) and functionality
 * that is specific to certain types of parser implementations; but not necessarily to number of
 * methods.
 */
public abstract class ParserMinimalBase extends JsonParser {
  ...
  protected static final int NR_INT = 0x0001;
  protected static final int NR_LONG = 0x0002;
  protected static final int NR_BIGINT = 0x0004;

  // And then floating point types

  protected static final int NR_DOUBLE = 0x008;
  protected static final int NR_BIGDECIMAL = 0x0010;
   ...
   * @since 2.9
   */
  protected static final int NR_FLOAT = 0x020;
  ...
  protected static final long MIN_INT_L = (long) Integer.MIN_VALUE;
  protected static final long MAX_INT_L = (long) Integer.MAX_VALUE;
  ...
  /**
   * Last token retrieved via {@link #nextToken}, if any. Null before the first call to <code>
   * nextToken()</code>, as well as if token has been explicitly cleared
   */
  protected JsonToken _currToken;

  /**
   * Last cleared token, if any: that is, value that was in effect when {@link #clearCurrentToken}
   * was called.
   */
  ...
  /**
   * Method sub-classes need to implement for verifying that end-of-content is acceptable at current
   * input position.
   *
   * @throws JsonParseException If end-of-content is not acceptable (for example, missing end-object
   *     or end-array tokens)
   */
  ...
  /**
   * Helper method that can be used for base64 decoding in cases where encoded content has already
   * been read as a String.
   *
  ...
  /**
   * Helper method used to determine whether we are currently pointing to a String value of "null"
   * (NOT a null token); and, if so, that parser is to recognize and return it similar to if it was
   * real null token.
   *
   * <p>Default implementation accepts exact string {@code "null"} and nothing else
   *
   * @param value String value to check
   * @return True if given value contains "null equivalent" String value (for content this parser
   *     handles).
   * @since 2.3
   */
  ...
  /**
   * Method called to throw an exception for input token that looks like a number based on first
   * character(s), but is not valid according to rules of format. In case of JSON this also includes
   * invalid forms like positive sign and leading zeroes.
   *
  ...
  /**
   * Method called to throw an exception for integral (not floating point) input token with value
   * outside of Java signed 32-bit range when requested as {@code int}. Result will be {@link
   * InputCoercionException} being thrown.
   *
  ...
  /**
   * Method called to throw an exception for integral (not floating point) input token with value
   * outside of Java signed 64-bit range when requested as {@code long}. Result will be {@link
   * InputCoercionException} being thrown.
   *
   * @throws JsonParseException Exception that describes problem with number range validity
   */
  ...
  protected void buggyMethod3(String numDesc) throws IOException {
    // Buggy version 3 - Statement 1
    String[] arrayNumDesc = numDesc.split("");
    reportOverflowLong(arrayNumDesc[0], currentToken());
  }
  ...
  // @since 2.9
  protected final void _reportError(String msg, Object arg1, Object arg2)
      throws JsonParseException {
