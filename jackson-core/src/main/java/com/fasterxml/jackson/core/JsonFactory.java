    ...
    implements Versioned, java.io.Serializable // since 2.1 (for Android, mostly)
{
  ...
  public enum Feature implements JacksonFeature // since 2.12
  {
    ...
    /**
     * Feature that determines what happens if we encounter a case in symbol handling where number
     * of hash collisions exceeds a safety threshold -- which almost certainly means a
     * denial-of-service attack via generated duplicate hash codes. If feature is enabled, an {@link
     * IllegalStateException} is thrown to indicate the suspected denial-of-service attack; if
     * disabled, processing continues but canonicalization (and thereby <code>intern()</code>ing) is
     * disabled) as protective measure.
     *
     * <p>This setting is enabled by default.
     *
     * @since 2.4
    ...
    /**
     * Feature that determines whether we will use {@link BufferRecycler} with {@link ThreadLocal}
     * and {@link SoftReference}, for efficient reuse of underlying input/output buffers. This
     * usually makes sense on normal J2SE/J2EE server-side processing; but may not make sense on
     * platforms where {@link SoftReference} handling is broken (like Android), or if there are
     * retention issues due to {@link ThreadLocal} (see <a
     * href="https://github.com/FasterXML/jackson-core/issues/189">jackson-core#189</a> for a
     * possible case)
     *
     * <p>This setting is enabled by default.
     *
  ...
  /**
   * Read constraints to use for {@link JsonParser}s constructed using this factory.
   *
   * @since 2.15
   */
  ...
  /**
   * Optional threshold used for automatically escaping character above certain character code
   * value: either {@code 0} to indicate that no threshold is specified, or value at or above 127 to
   * indicate last character code that is NOT automatically escaped (but depends on other
   * configuration rules for checking).
   *
   * @since 2.10
   */
  ...
  /**
   * Character used for quoting field names (if field name quoting has not been disabled with {@link
   * JsonWriteFeature#QUOTE_FIELD_NAMES}) and JSON String values.
   */
  ...
  /**
   * Constructor for subtypes; needed to work around the fact that before 3.0, this factory has
   * cumbersome dual role as generic type as well as actual implementation for json.
   *
   * @param b Builder that contains settings to use
  ...
  /**
   * Method that allows construction of differently configured factory, starting with settings of
   * this factory.
   *
  ...
  /**
   * Main factory method to use for constructing {@link JsonFactory} instances with different
   * configuration: creates and returns a builder for collecting configuration settings; instance
   * created by calling {@code build()} after all configuration set.
   *
   * <p>NOTE: signature unfortunately does not expose true implementation type; this will be fixed
   * in 3.0.
   *
   * @return Builder instance to use
   */
   ...
   * needed to allow JDK serializability of factory instances.
   *
   * <p>Note: must be overridden by sub-classes as well.
   *
  ...
  /**
   * Introspection method that higher-level functionality may call to see whether underlying data
   * format requires a stable ordering of object properties or not. This is usually used for
   * determining whether to force a stable ordering (like alphabetic ordering by name) if no
   * ordering if explicitly specified.
   *
   * <p>Default implementation returns <code>false</code> as JSON does NOT require stable ordering.
   * Formats that require ordering include positional textual formats like <code>CSV</code>, and
   * schema-based binary formats like <code>Avro</code>.
   *
   * @return Whether format supported by this factory requires Object properties to be ordered.
   * @since 2.3
   */
  @Override
  ...
  /**
   * Introspection method that higher-level functionality may call to see whether underlying data
   * format can read and write binary data natively; that is, embeded it as-is without using
   * encodings such as Base64.
   *
   * <p>Default implementation returns <code>false</code> as JSON does not support native access:
   * all binary content must use Base64 encoding. Most binary formats (like Smile and Avro) support
   * native binary content.
   *
   * @return Whether format supported by this factory supports native binary content
   * @since 2.3
   */
  @Override
  ...
  /**
   * Introspection method that can be used by base factory to check whether access using <code>
   * char[]</code> is something that actual parser implementations can take advantage of, over
   * having to use {@link java.io.Reader}. Sub-types are expected to override definition; default
   * implementation (suitable for JSON) alleges that optimization are possible; and thereby is
   * likely to try to access {@link java.lang.String} content by first copying it into recyclable
   * intermediate buffer.
   *
   * @return Whether access to decoded textual content can be efficiently accessed using parser
   *     method {@code getTextCharacters()}.
   * @since 2.4
   */
  ...
  /**
   * Introspection method that can be used to check whether this factory can create non-blocking
   * parsers: parsers that do not use blocking I/O abstractions but instead use a {@link
   * com.fasterxml.jackson.core.async.NonBlockingInputFeeder}.
   *
   * @return Whether this factory supports non-blocking ("async") parsing or not (and consequently
   *     whether {@code createNonBlockingXxx()} method(s) work)
   * @since 2.9
   */
  @Override
   ...
   * @return Whether parsers and generators constructed by this factory can use specified format
   *     schema instance
   */
  @Override
   ...
   * @deprecated since 2.10 use {@link JsonFactoryBuilder#configure(JsonFactory.Feature, boolean)}
   *     instead
   */
  @Deprecated
   ...
   * decorator).
   *
   * @return OutputDecorator configured for generators factory creates, if any; {@code null} if
   *     none.
   */
   ...
   *
   * <p>Encoding is auto-detected from contents according to JSON specification recommended
   * mechanism. Json specification supports only UTF-8, UTF-16 and UTF-32 as valid encodings, so
   * auto-detection implemented only for this charsets. For other charsets use {@link
   ...
   * com.fasterxml.jackson.core.StreamReadFeature#AUTO_CLOSE_SOURCE} is enabled.
   *
   * <p>Note: no encoding argument is taken since it can always be auto-detected as suggested by
   * JSON RFC. Json specification supports only UTF-8, UTF-16 and UTF-32 as valid encodings, so
   * auto-detection implemented only for this charsets. For other charsets use {@link
   * #createParser(java.io.Reader)}.
   *
   ...
   * instance.
   *
   * <p>If this factory does not support {@link DataInput} as source, will throw {@link
  ...
  /**
   * Optional method for constructing parser for non-blocking parsing via {@link
   * com.fasterxml.jackson.core.async.ByteArrayFeeder} interface (accessed using {@link
   * JsonParser#getNonBlockingInputFeeder()} from constructed instance).
   *
   * <p>If this factory does not support non-blocking parsing (either at all, or from byte array),
   * will throw {@link UnsupportedOperationException}.
   *
   * <p>Note that JSON-backed factory only supports parsing of UTF-8 encoded JSON content (and
   * US-ASCII since it is proper subset); other encodings are not supported at this point.
   *
   * @since 2.9
   */
  @Override
  public JsonParser createNonBlockingByteArrayParser() throws IOException {
    ...
    ByteQuadsCanonicalizer can = _byteSymbolCanonicalizer.makeChild(_parserFeatures);
    return new NonBlockingJsonParser(ctxt, _parserFeatures, can);
  }

  /**
   * Optional method for constructing parser for non-blocking parsing via {@link
   * com.fasterxml.jackson.core.async.ByteBufferFeeder} interface (accessed using {@link
   * JsonParser#getNonBlockingInputFeeder()} from constructed instance).
   *
   * <p>If this factory does not support non-blocking parsing (either at all, or from byte array),
   * will throw {@link UnsupportedOperationException}.
  ...
  /**
   * Method for constructing generator for writing content using specified {@link DataOutput}
   * instance.
   *
   * @since 2.8
   */
  ...
  protected JsonParser _createParser(Reader r, IOContext ctxt) throws IOException {
    return new ReaderBasedJsonParser(
        ctxt, _parserFeatures, r, _objectCodec, _rootCharSymbols.makeChild(_factoryFeatures));
  }

  /**
   * Overridable factory method that actually instantiates parser using given <code>char[]</code>
  ...
  protected JsonGenerator _createUTF8Generator(OutputStream out, IOContext ctxt)
    ...
    UTF8JsonGenerator gen =
        new UTF8JsonGenerator(ctxt, _generatorFeatures, _objectCodec, out, _quoteChar);
    if (_maximumNonEscapedChar > 0) {
      gen.setHighestNonEscapedChar(_maximumNonEscapedChar);
    }
    if (_characterEscapes != null) {
   ...
   *
   * <p>Note: only public to give access for {@code ObjectMapper}
   *
  ...
  /**
   * Overridable factory method that actually instantiates desired context object for async
  ...
  protected IOContext _createNonBlockingContext(Object srcRef) {
    ...
    return new IOContext(
        _streamReadConstraints, _getBufferRecycler(), _createContentReference(srcRef), false);
  }

  /**
   * Overridable factory method for constructing {@link ContentReference} to pass to parser or
   * generator being created; used in cases where no offset or length is applicable (either
   * irrelevant, or full contents assumed).
   *
   * @param contentAccessor Access to underlying content; depends on source/target, as well as
  ...
  /**
   * Overridable factory method for constructing {@link ContentReference} to pass to parser or
   * generator being created; used in cases where content is available in a static buffer with
   * relevant offset and length (mostly when reading from {@code byte[]}, {@code char[]} or {@code
   * String}).
   *
   * @param contentAccessor Access to underlying content; depends on source/target, as well as
   *     content representation
   ...
   * @since 2.13
   */
  protected ContentReference _createContentReference(
      Object contentAccessor, int offset, int length) {
    // 21-Mar-2021, tatu: For now assume "canHandleBinaryNatively()" is reliable
    //    indicator of textual vs binary format:
  ...
  /**
   * Helper method called to work around the problem of this class both defining general API for
   * constructing parsers+generators AND implementing the API for JSON handling. Problem here is
   * that when adding new functionality via factory methods, it is not possible to leave these
   * methods abstract (because we are implementing them for JSON); but there is risk that
   * sub-classes do not override them all (plus older version can not implement). So a work-around
   * is to add a check to ensure that factory is still one used for JSON; and if not, make base
   * implementation of a factory method fail.
   *
