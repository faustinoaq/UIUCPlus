...
/**
 * {@link NonBlockingInputFeeder} implementation used when feeding data as {@link ByteBuffer}
 * contents.
 *
 * @since 2.9
 */
public interface ByteBufferFeeder extends NonBlockingInputFeeder {
  /**
   * Method that can be called to feed more data, if (and only if) {@link
   * NonBlockingInputFeeder#needMoreInput} returns true.
   *
   * @param buffer Buffer that contains additional input to read
   * @throws IOException if the state is such that this method should not be called (has not yet
