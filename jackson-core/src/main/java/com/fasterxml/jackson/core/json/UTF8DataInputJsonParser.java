 ...
 * java.io.DataInput} as the input source.
 *
 * <p>Due to limitations in look-ahead (basically there's none), as well as overhead of reading
 * content mostly byte-by-byte, there are some minor differences from regular streaming parsing.
 * Specifically:
 *
 * <ul>
 *   <li>Input location offsets not being tracked, as offsets would need to be updated for each read
 *       from all over the place. If caller wants this information, it has to track this with {@link
 *       DataInput}. This also affects column number, so the only location information available is
 *       the row (line) number (but even that is approximate in case of two-byte linefeeds -- it
 *       should work with single CR or LF tho)
 *   <li>No white space validation: checks are simplified NOT to check for control characters.
 * </ul>
 *
 * @since 2.8
 */
public class UTF8DataInputJsonParser extends ParserBase {
  ...
  /**
   * Codec used for data binding when (if) requested; typically full <code>ObjectMapper</code>, but
   * that abstract is not part of core package.
   */
  ...
  public UTF8DataInputJsonParser(
      IOContext ctxt,
      int features,
      DataInput inputData,
      ObjectCodec codec,
  ...
  @Override
  public Object getInputSource() throws IOException {
    throw new IOException("Unable to get input source");
  }
  ...
  protected final JsonToken _parseFloatThatStartsWithPeriod(
      final boolean neg, final boolean hasSign) throws IOException {
  ...
  /**
   * Method called when we have seen one zero, and want to ensure it is not followed by another, or,
   * if leading zeroes allowed, skipped redundant ones.
   *
  ...
  private final JsonToken _parseFloat(
      char[] outBuf, int outPtr, int c, boolean negative, int integerPartLength)
      throws IOException {
    int fractLen = 0;
   ...
   */
  protected final String parseEscapedName(
      int[] quads, int qlen, int currQuad, int ch, int currQuadBytes) throws IOException {
   ...
   * field name. In standard mode will just throw an exception; but in non-standard modes may be
   * able to parse name.
   *
  ...
  /**
   * This is the main workhorse method used when we take a symbol table miss. It needs to
   * demultiplex individual bytes, decode multi-byte chars (if any), and then construct Name
   * instance and add it to the symbol table.
   */
  ...
  protected JsonToken _handleInvalidNumberStart(int ch, final boolean neg, final boolean hasSign)
      throws IOException {
    while (ch == 'I') {
      ch = _inputData.readUnsignedByte();
      String match;
  ...
  /**
   * Alternative to {@link #_skipWS} that handles possible {@link EOFException} caused by trying to
   * read past the end of {@link InputData}.
   *
   * @since 2.9
   */
  ...
  /**
   * @return Character value <b>minus 0x10000</c>; this so that caller can readily expand it to
   *     actual surrogates
   */
