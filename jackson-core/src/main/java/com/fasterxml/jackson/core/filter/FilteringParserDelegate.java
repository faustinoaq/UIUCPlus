...
/**
 * Specialized {@link JsonParserDelegate} that allows use of {@link TokenFilter} for outputting a
 * subset of content that is visible to caller
 *
 * @since 2.6
 */
public class FilteringParserDelegate extends JsonParserDelegate {
  ...
  /**
   * Object consulted to determine whether to write parts of content generator is asked to write or
   * not.
   */
  protected TokenFilter rootFilter;

  /**
   * Flag that determines whether filtering will continue after the first match is indicated or not:
   * if `false`, output is based on just the first full match (returning {@link
   * TokenFilter#INCLUDE_ALL}) and no more checks are made; if `true` then filtering will be applied
   * as necessary until end of content.
   */
  protected boolean _allowMultipleMatches;

  /**
   * Flag that determines whether path leading up to included content should also be automatically
   * included or not. If `false`, no path inclusion is done and only explicitly included entries are
   * output; if `true` then path from main level down to match is also included as necessary.
   */
  ...
  /**
   * During traversal this is the actual "open" parse tree, which sometimes is the same as {@link
   * #_exposedContext}, and at other times is ahead of it. Note that this context is never null.
   */
  protected TokenFilterContext _headContext;

  /**
   * In cases where {@link #_headContext} is "ahead" of context exposed to caller, this context
   * points to what is currently exposed to caller. When the two are in sync, this context reference
   * will be <code>null</code>.
   */
  protected TokenFilterContext _exposedContext;

  /**
   * State that applies to the item within container, used where applicable. Specifically used to
   * pass inclusion state between property name and property, and also used for array elements.
   */
  protected TokenFilter _itemFilter;

  /** Number of tokens for which {@link TokenFilter#INCLUDE_ALL} has been returned. */
  ...
  public FilteringParserDelegate(
    ...
    this(
        p,
        f,
        includePath ? Inclusion.INCLUDE_ALL_AND_PATH : Inclusion.ONLY_INCLUDE_ALL,
  ...
  /**
   * Accessor for finding number of matches, where specific token and sub-tree starting (if
   * structured type) are passed.
   *
