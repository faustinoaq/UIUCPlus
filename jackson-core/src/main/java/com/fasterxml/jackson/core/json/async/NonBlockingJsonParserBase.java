...
public abstract class NonBlockingJsonParserBase extends ParserBase {
  ...
  /**
   * State right after parser has been constructed, before seeing the first byte to handle possible
   * (but optional) BOM.
   */
  protected static final int MAJOR_INITIAL = 0;

  /**
   * State right after parser a root value has been finished, but next token has not yet been
   * recognized.
   */
  ...
  /**
   * State after non-blocking input source has indicated that no more input is forthcoming AND we
   * have exhausted all the input
   */
  ...
  /**
   * State between root-level value, having processed at least one white-space character, and
   * expecting either more, start of a value, or end of input stream.
   */
  ...
  protected static final int MINOR_NUMBER_ZERO = 24; // zero as first, possibly trimming multiple
  protected static final int MINOR_NUMBER_MINUSZERO = 25; // "-0" (and possibly more zeroes) receive
  ...
  /**
   * Special state at which point decoding of a non-quoted token has encountered a problem; that is,
   * either not matching fully (like "truf" instead of "true", at "tru"), or not having trailing
   * separator (or end of input), like "trueful". Attempt is made, then, to decode likely full input
   * token to report suitable error.
   */
  ...
  /**
   * Secondary minor state indicator used during decoding of escapes and/or multi-byte Unicode
   * characters
   */
  ...
  /**
   * When tokenizing non-standard ("odd") tokens, this is the type to consider; also works as index
   * to actual textual representation.
   */
  ...
  /**
   * Since we are fed content that may or may not start at zero offset, we need to keep track of the
   * first byte within that buffer, to be able to calculate logical offset within input "stream"
   */
  protected int _currBufferStart = 0;

  /**
   * Alternate row tracker, used to keep track of position by `\r` marker (whereas <code>
   * _currInputRow</code> tracks `\n`). Used to simplify tracking of linefeeds, assuming that input
   * typically uses various linefeed combinations (`\r`, `\n` or `\r\n`) consistently, in which case
   * we can simply choose max of two row candidates.
   */
  ...
  public JsonLocation getCurrentLocation() {
    ...
    return new JsonLocation(
        _contentReference(),
        _currInputProcessed + (_inputPtr - _currBufferStart),
        -1L, // bytes, chars
        row,
        col);
  }
  ...
  public JsonLocation getTokenLocation() {
    return new JsonLocation(
        _contentReference(), _tokenInputTotal, -1L, _tokenInputRow, _tokenInputCol);
  }
  ...
  public byte[] getBinaryValue(Base64Variant b64variant) throws IOException {
    if (_currToken != JsonToken.VALUE_STRING) {
      _reportError(
          "Current token (%s) not VALUE_STRING or VALUE_EMBEDDED_OBJECT, can not access as binary",
          _currToken);
    }
