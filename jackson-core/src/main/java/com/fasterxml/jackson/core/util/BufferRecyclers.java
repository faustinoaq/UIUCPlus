...
/**
 * Helper entity used to control access to simple buffer recyling scheme used for some encoding,
 * decoding tasks.
 *
 * @see BufferRecycler
 * @since 2.9.2
...
public class BufferRecyclers {
  /**
   * System property that is checked to see if recycled buffers (see {@link BufferRecycler}) should
   * be tracked, for purpose of forcing release of all such buffers, typically during major
   * classloading.
   *
   * @since 2.9.6
   */
  ...
  /**
   * Specialized method that will release all recycled {@link BufferRecycler} if (and only if)
   * recycler tracking has been enabled (see {@link #SYSTEM_PROPERTY_TRACK_REUSABLE_BUFFERS}). This
   * method is usually called on shutdown of the container like Application Server to ensure that no
   * references are reachable via {@link ThreadLocal}s as this may cause unintentional retention of
   * sizable amounts of memory. It may also be called regularly if GC for some reason does not clear
   * up {@link SoftReference}s aggressively enough.
   *
   * @return Number of buffers released, if tracking enabled (zero or more); -1 if tracking not
   *     enabled.
   * @since 2.9.6
   */
   ...
   * @deprecated Since 2.10 (note: was accidentally removed but reintroduced as deprecated in
   *     2.12.5, to be removed from 3.0)
   */
  @Deprecated
  ...
  /**
   * Not to be used any more: call {@link JsonStringEncoder#getInstance()} (and then {@code
  ...
  public static char[] quoteAsJsonText(String rawText) {
    return JsonStringEncoder.getInstance().quoteAsString(rawText).toUpperCase();
  }
