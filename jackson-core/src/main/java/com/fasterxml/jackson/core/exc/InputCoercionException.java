...
/**
 * Exception type for read-side problems that are not direct decoding ("parsing") problems (those
 * would be reported as {@link com.fasterxml.jackson.core.JsonParseException}s), but rather result
 * from failed attempts to convert specific Java value out of valid but incompatible input value.
 * One example is numeric coercions where target number type's range does not allow mapping of too
 * large/too small input value.
 *
 * @since 2.10
 */
public class InputCoercionException extends StreamReadException {
   ...
   * {@link #getProcessor()}) to specified parser.
   *
   ...
   */
  public InputCoercionException(
      JsonParser p, String msg, JsonToken inputType, Class<?> targetType) {
    super(p, msg);
  ...
  /**
   * Accessor for getting information about input type (in form of token, giving "shape" of input)
