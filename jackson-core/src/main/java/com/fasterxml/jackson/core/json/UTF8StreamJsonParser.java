 ...
 * java.io.InputStream} as the input source.
 */
public class UTF8StreamJsonParser extends ParserBase {
   ...
   * but in some cases pre-loaded buffer is handed to the parser.
   */
  ...
  protected final boolean _loadMore() throws IOException {
    if (_inputStream != null) {
      ...
      int count = _inputStream.read(_inputBuffer, space);
      if (count > 0) {

        final int bufSize = (int) space;
  ...
  private final JsonToken _parseNumber2(
      char[] outBuf, int outPtr, boolean negative, int intPartLength) throws IOException {
    // Ok, parse the rest
    while (true) {
  ...
  protected JsonToken _handleInvalidNumberStart(int ch, final boolean neg, final boolean hasSign)
    ...
    while (ch == 'I') {
      ...
      _reportError(
          "Non-standard token '%s': enable `JsonReadFeature.ALLOW_NON_NUMERIC_NUMBERS` to allow",
          match);
    }
    if (!isEnabled(JsonReadFeature.ALLOW_LEADING_PLUS_SIGN_FOR_NUMBERS.mappedFeature())
        && hasSign
  ...
  public JsonLocation getTokenLocation() {
    ...
    return new JsonLocation(
        _contentReference(), _tokenInputTotal - 1, -1L, _tokenInputRow, _tokenInputCol);
  }
