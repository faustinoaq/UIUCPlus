...
public class UTF8JsonGenerator extends JsonGeneratorImpl {
  ...
  /**
   * Intermediate buffer in which contents are buffered before being written using {@link
   * #_outputStream}.
   */
  protected byte[] _outputBuffer;

  /**
   * Pointer to the position right beyond the last character to output (end marker; may be past the
   * buffer)
   */
  ...
  /**
   * Maximum number of <code>char</code>s that we know will always fit in the output buffer after
   * escaping
   */
  ...
  /**
   * Flag that indicates whether the output buffer is recycable (and needs to be returned to
   * recycler once we are done) or not.
   */
  ...
  private final void _writeUnq(SerializableString name) throws IOException {
    ...
    if (len < 0) {
      ...
      _outputTail += len + 1; // buggy statement
    }
  }
  ...
  public void writeRaw(String text, int offset, int len) throws IOException {
    ...
    final int maxChunk =
        Math.min(cbufLen, (_outputEnd >> 2) + (_outputEnd >> 4)); // == (1/4 + 1/16) == 5/16
  ...
  /**
   * Helper method called when it is possible that output of raw section to output may cross buffer
   * boundary
   */
  ...
  /**
   * Helper method that is called for segmented write of raw content when explicitly outputting a
   * segment of longer thing. Caller has to take care of ensuring there's no split surrogate pair at
   * the end (that is, last char can not be first part of a surrogate char pair).
   *
   * @since 2.8.2
   */
  ...
  public int writeBinary(Base64Variant b64variant, InputStream data, int dataLength)
    ...
    try {
      if (dataLength < 0) { // length unknown
        ...
        if (missing > 0) {
          _reportError(
              "Too few bytes available: missing " + missing + " bytes (out of " + dataLength + ")");
        }
        bytes = dataLength;
      }
    } finally {
  ...
  /**
   * Method called when String to write is long enough not to fit completely in temporary copy
   * buffer. If so, we will actually copy it in small enough chunks so it can be directly fed to
   * single-segment writes (instead of maximum slices that would fit in copy buffer)
   */
  ...
  /**
   * Method called when character sequence to write is long enough that its maximum encoded and
   * escaped form is not guaranteed to fit in the output buffer. If so, we will need to choose
   * smaller output chunks to write at a time.
   */
  ...
  /**
   * This method called when the string content is already in a char buffer, and its maximum total
   * encoded and escaped length can not exceed size of the output buffer. Caller must ensure that
   * there is enough space in output buffer, assuming case of all non-escaped ASCII characters, as
   * well as potentially enough space for other cases (but not necessarily flushed)
   */
  ...
  private final void _writeStringSegment2(final String text, int offset, final int end)
      throws IOException {
    if ((_outputTail + 6 * (end - offset)) > _outputEnd) {
      _flushBuffer();
    }
  ...
  private final void _writeStringSegmentASCII2(final String text, int offset, final int end)
      throws IOException {
    // Ok: caller guarantees buffer can have room; but that may require flushing:
    if ((_outputTail + 6 * (end - offset)) > _outputEnd) {
      _flushBuffer();
  ...
  private final void _writeCustomStringSegment2(final String text, int offset, final int end)
      throws IOException {
    // Ok: caller guarantees buffer can have room; but that may require flushing:
    if ((_outputTail + 6 * (end - offset)) > _outputEnd) {
      _flushBuffer();
  ...
  private final int _writeCustomEscape(
      byte[] outputBuffer, int outputPtr, SerializableString esc, int remainingChars)
      throws IOException, JsonGenerationException {
  ...
  private final int _handleLongCustomEscape(
      byte[] outputBuffer, int outputPtr, int outputEnd, byte[] raw, int remainingChars)
      throws IOException, JsonGenerationException {
  ...
  /**
   * Method called when UTF-8 encoded (but NOT yet escaped!) content is not guaranteed to fit in the
   * output buffer after escaping; as such, we just need to chunk writes.
   */
  ...
  protected final void _writeBinary(
  ...
  protected final int _writeBinary(
      Base64Variant b64variant, InputStream data, byte[] readBuffer, int bytesLeft)
      throws IOException, JsonGenerationException {
    int inputPtr = 0;
    int inputEnd = 0;
  ...
  private final int _readMore(
      InputStream in, byte[] readBuffer, int inputPtr, int inputEnd, int maxRead)
      throws IOException {
  ...
  /**
   * Method called to output a character that is beyond range of 1- and 2-byte UTF-8 encodings, when
   * outputting "raw" text (meaning it is not to be escaped or quoted)
   */
    ...
    if (ch >= SURR1_FIRST) {
      if (ch <= SURR2_LAST) { // yes, outside of BMP
        ...
        if (inputOffset >= inputEnd || cbuf == null) { // nope... have to note down
          _reportError(
              String.format(
                  "Split surrogate on writeRaw() input (last character): first character 0x%4x",
                  ch));
        } else {
