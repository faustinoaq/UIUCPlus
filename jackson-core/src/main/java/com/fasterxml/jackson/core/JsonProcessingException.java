...
/**
 * Intermediate base class for all problems encountered when processing (parsing, generating) JSON
 * content that are not pure I/O problems. Regular {@link java.io.IOException}s will be passed
 * through as is. Sub-class of {@link java.io.IOException} for convenience.
 *
 * <p>Since Jackson 2.12 extends intermediate {@link JacksonException} type instead of directly
 * extending {@link java.io.IOException}.
 */
public class JsonProcessingException extends JacksonException {
  ...
  /**
   * Method that allows to remove context information from this exception's message. Useful when you
   * are parsing security-sensitive data and don't want original data excerpts to be present in
   * Jackson parser error messages.
   *
   * @since 2.9
   */
   ...
   *
   * @return Original {@code message} preceded by optional prefix and followed by location
   *     information, message and location information separated by a linefeed
   */
  @Override
  public String getMessage() {
    ...
    if ((suffix == msg) || (loc != null || message != null)) {
