...
/**
 * Intermediate base class for actual format-specific factories for constructing parsers (reading)
 * and generators (writing). Although full power will only be available with Jackson 3, skeletal
 * implementation added in 2.10 to help conversion of code for 2.x to 3.x migration of projects
 * depending on Jackson
 *
 * @since 2.10
 */
public abstract class TokenStreamFactory implements Versioned, java.io.Serializable {
  ...
  /**
   * Method for accessing kind of {@link FormatFeature} that a parser {@link JsonParser} produced by
   * this factory would accept, if any; <code>null</code> returned if none.
   *
  ...
  /**
   * Method for accessing kind of {@link FormatFeature} that a parser {@link JsonGenerator} produced
   * by this factory would accept, if any; <code>null</code> returned if none.
   *
   * @return Type of format-specific stream read features, if any; {@code null} if none
   * @since 2.6
   */
   ...
   *
   * @return Constraints to apply to reads done by {@link JsonParser}s constructed by this factory.
   * @since 2.15
   */
  ...
  public abstract JsonGenerator createGenerator(OutputStream out) throws IOException {
    ...
    try {
      ...
      generator = new JsonFactory().createGenerator(null, JsonEncoding.UTF8);
  ...
  /**
   * Helper method used for constructing an optimal stream for parsers to use, when input is to be
  ...
  /**
   * Helper methods used for constructing an {@link InputStream} for parsers to use, when input is
   * to be read from given {@link File}.
   *
  ...
  /**
   * Helper methods used for constructing an {@link OutputStream} for generator to use, when target
   * is to be written into given {@link File}.
   *
   * @param f File to open stream for
