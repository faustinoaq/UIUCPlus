...
/**
 * Replacement for <code>BytesToNameCanonicalizer</code> which aims at more localized memory access
 * due to flattening of name quad data. Performance improvement modest for simple JSON document data
 * binding (maybe 3%), but should help more for larger symbol tables, or for binary formats like
 * Smile.
 *
 * <p>Hash area is divided into 4 sections:
 *
 * <ol>
 *   <li>Primary area (1/2 of total size), direct match from hash (LSB)
 *   <li>Secondary area (1/4 of total size), match from {@code hash (LSB) >> 1}
 *   <li>Tertiary area (1/8 of total size), match from {@code hash (LSB) >> 2}
 *   <li>Spill-over area (remaining 1/8) with linear scan, insertion order
 *   <li>
 * </ol>
 *
 * and within every area, entries are 4 {@code int}s, where 1 - 3 ints contain 1 - 12 UTF-8 encoded
 * bytes of name (null-padded), and last int is offset in {@code _names} that contains actual name
 * Strings.
 *
 * @since 2.6
 */
public final class ByteQuadsCanonicalizer {
  /**
   * Initial size of the primary hash area. Each entry consumes 4 ints (16 bytes), and secondary
   * area is same as primary; so default size will use 2kB of memory (plus 64x4 or 64x8 (256/512
   * bytes) for references to Strings, and Strings themselves).
   */
  ...
  /**
   * Let's not expand symbol tables past some maximum size; with unique (~= random) names. Size is
   * in
   */
  ...
  /**
   * Let's only share reasonably sized symbol tables. Max size set to 3/4 of 8k; this corresponds to
   * 256k main hash index. This should allow for enough distinct names for almost any case, while
   * preventing ballooning for cases where names are unique (or close thereof).
   */
  ...
  /**
   * Reference to the root symbol table, for child tables, so that they can merge table information
   * back as necessary.
   */
  protected final ByteQuadsCanonicalizer _parent;

  /**
   * Member that is only used by the root table instance: root passes immutable state info child
   * instances, and children may return new state if they add entries to the table. Child tables do
   * NOT use the reference.
   */
  protected final AtomicReference<TableInfo> _tableInfo;

  /**
   * Seed value we use as the base to make hash codes non-static between different runs, but still
   * stable for lifetime of a single symbol table instance. This is done for security reasons, to
   * avoid potential DoS attack via hash collisions.
   */
   ...
   *
   * <p>NOTE: non-final to allow disabling intern()ing in case of excessive collisions.
   */
  protected final boolean _intern;

  /**
   * Flag that indicates whether we should throw an exception if enough hash collisions are detected
   * (true); or just worked around (false).
   *
   * @since 2.4
   */
  ...
  /**
   * Primary hash information area: consists of <code>2 * _hashSize</code> entries of 16 bytes (4
   * ints), arranged in a cascading lookup structure (details of which may be tweaked depending on
   * expected rates of collisions).
   */
  protected int[] _hashArea;

  /**
   * Number of slots for primary entries within {@link #_hashArea}; which is at most <code>1/8
   * </code> of actual size of the underlying array (4-int slots, primary covers only half of the
   * area; plus, additional area for longer symbols after hash area).
   */
  ...
  /**
   * Constant that determines size of buckets for tertiary entries: <code>1 &lt;&lt; _tertiaryShift
   * </code> is the size, and shift value is also used for translating from primary offset into
   * tertiary bucket (shift right by <code>4 + _tertiaryShift</code>).
   *
   * <p>Default value is 2, for buckets of 4 slots; grows bigger with bigger table sizes.
   */
  ...
  /**
   * Array that contains <code>String</code> instances matching entries in {@link #_hashArea}.
   * Contains nulls for unused entries. Note that this size is twice that of {@link #_hashArea}
   */
  ...
  /**
   * Pointer to the offset within spill-over area where there is room for more spilled over entries
   * (if any). Spill over area is within fixed-size portion of {@link #_hashArea}.
   */
  protected int _spilloverEnd;

  /**
   * Offset within {@link #_hashArea} that follows main slots and contains quads for longer names
   * (13 bytes or longer), and points to the first available int that may be used for appending
   * quads of the next long name. Note that long name area follows immediately after the fixed-size
  ...
  /**
   * Flag that indicates whether underlying data structures for the main hash area are shared or
   * not. If they are, then they need to be handled in copy-on-write way, i.e. if they need to be
   * modified, a copy needs to be made first; at this point it will not be shared any more, and can
   * be modified.
   *
   * <p>This flag needs to be checked both when adding new main entries, and when adding new
   * collision list queues (i.e. creating a new collision list head entry)
   */
  ...
  /**
   * Constructor used for creating per-{@code TokeanStreamFactory} "root" symbol tables: ones used
   * for merging and sharing common symbols
   *
   * @param sz Initial primary hash area size
   * @param seed Random seed valued used to make it more difficult to cause collisions (used for
  ...
  private ByteQuadsCanonicalizer(
      ByteQuadsCanonicalizer parent, int seed, TableInfo state, boolean intern, boolean failOnDoS) {
    _parent = parent;
    _seed = seed;
    _intern = intern;
  ...
  /**
   * Alternate constructor used in cases where a "placeholder" child instance is needed when symbol
   * table is not really used, but caller needs a non-null placeholder to keep code functioning with
   * minimal awareness of distinction (all lookups fail to match any name without error; add methods
   ...
   *
   * @param flags Bit flags of active {@link com.fasterxml.jackson.core.JsonFactory.Feature}s
   *     enabled.
  ...
  /**
   * Method similar to {@link #makeChild} but one that only creates real instance of {@link
   * com.fasterxml.jackson.core.JsonFactory.Feature#CANONICALIZE_FIELD_NAMES} is enabled: otherwise
   * a "bogus" instance is created.
   *
   * @param flags Bit flags of active {@link com.fasterxml.jackson.core.JsonFactory.Feature}s
   *     enabled.
   ...
   * merge accumulated changes into parent (if need be), safely and efficiently, and without calling
  ...
  /**
   * Method called to check to quickly see if a child symbol table may have gotten additional
   * entries. Used for checking to see if a child table should be merged into shared table.
   *
  ...
  /**
   * @return True for "real", canonicalizing child tables; false for root table as well as
   *     placeholder "child" tables.
   * @since 2.13
   */
  ...
  /**
   * Method mostly needed by unit tests; calculates number of entries that are in the primary slot
   * set. These are "perfect" entries, accessible with a single lookup
   *
  ...
  public String findName(int q1, int q2, int q3) {
    ...
    if (len == 3) {
      if ((q1 == hashArea[offset2])
          && (hashArea[offset2 + 1] == q2)
          && (hashArea[offset2 + 2] == q3)) {
        return _names[offset2 >> 2];
      }
    } else if (len == 0) { // empty slot? Short-circuit if no more spillovers
      return null;
  ...
  private String _findSecondary(int origOffset, int q1, int q2, int q3) {
    ...
    for (offset = _spilloverStart(); offset < _spilloverEnd; offset += 4) {
      if ((q1 == hashArea[offset]) && (q2 == hashArea[offset + 1])) {
        return _names[
            offset >> 2]; // bug: should return immediately but missing the 3rd element check
      }
    }
    return null;
  }
  ...
  private int _findOffsetForAdd(int hash) {
    ...
    for (int end = offset2 + bucketSize; offset2 < end; offset2 += 4) {
      if (hashArea[offset2 + 3] == 0) {
        ...
        // ((offset >> (_tertiaryShift + 2)) << _tertiaryShift), _tertiaryStart, (hash & 0x7F));
        return offset2;
      }
    }
    ...
    // _spilloverStart(), _hashArea.length, (hash & 0x7F));
  ...
  public int calcHash(
      int q1, int q2, int q3) { // use same algorithm as multi-byte, tested to work well
    int hash = q1 ^ _seed;
    hash += (hash >>> 9);
    hash *= MULT3;
    hash += q2;
  ...
  private void rehash() {
    ...
    if (copyCount != oldCount) {
      throw new IllegalStateException(
          "Failed rehash(): old count=" + oldCount + ", copyCount=" + copyCount);
    }
  }
  ...
  /**
   * Immutable value class used for sharing information as efficiently as possible, by only require
   * synchronization of reference manipulation but not access to contents.
   *
   * @since 2.1
   */
