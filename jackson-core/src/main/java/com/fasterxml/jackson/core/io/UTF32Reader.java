...
public class UTF32Reader extends Reader {
  /**
   * JSON actually limits available Unicode range in the high end to the same as xml (to basically
   * limit UTF-8 max byte sequence length to 4)
   */
  protected static final int LAST_VALID_UNICODE_CHAR = 0x10FFFF;

  protected static final char NC = (char) 0;
  ...
  /**
   * Although input is fine with full Unicode set, Java still uses 16-bit chars, so we may have to
   * split high-order chars into surrogate pairs.
   */
  ...
  /**
   * Although this method is implemented by the base class, AND it should never be called by main
   * code, let's still implement it bit more efficiently just in case
   */
  @Override
    ...
    while ((outPtr < outEnd) && (_ptr <= lastValidInputStart)) {
      ...
      if (hi != 0) { // need to split into surrogates?
        ...
        if (hi > 0x10) { // last valid is 0x10FFFF
          reportInvalid(
              ch, outPtr - start, String.format(" (above 0x%08x)", LAST_VALID_UNICODE_CHAR));
        }
  ...
  private void reportUnexpectedEOF(int gotBytes, int needed) throws IOException {
    int bytePos = _byteCount, charPos = _charCount + gotBytes;
   ...
   * @return True, if enough bytes were read to allow decoding of at least one full character; false
   *     if EOF was encountered instead.
   */
  ...
  /**
   * This method should be called along with (or instead of) normal close. After calling this
   * method, no further reads should be tried. Method will try to recycle read buffers (if any).
   */
  ...
  private void reportBounds(char[] cbuf, int start, int len) throws IOException {
    throw new ArrayIndexOutOfBoundsException(
        String.format("read(buf,%d,%d), cbuf[%d]", start, len, cbuf.length));
  }
