...
/**
 * Implementation of <a href="http://tools.ietf.org/html/draft-ietf-appsawg-json-pointer-03">JSON
 * Pointer</a> specification. Pointer instances can be used to locate logical JSON nodes for things
 * like tree traversal (see {@link TreeNode#at}). It may be used in future for filtering of
 * streaming JSON content as well (not implemented yet for 2.3).
 *
 * <p>Note that the implementation was largely rewritten for Jackson 2.14 to reduce memory usage by
 * sharing backing "full path" representation for nested instances.
 *
 * <p>Instances are fully immutable and can be cached, shared between threads.
 *
...
public class JsonPointer implements Serializable {
  ...
  /**
   * Marker instance used to represent segment that matches current node or position (that is,
   * returns true for {@link #matches()}).
   */
  protected static final JsonPointer EMPTY = new JsonPointer();

  /**
   * Reference to rest of the pointer beyond currently matching segment (if any); null if this
   * pointer refers to the matching segment.
   */
  protected final JsonPointer _nextSegment;

  /**
   * Reference from currently matching segment (if any) to node before leaf. Lazily constructed
   * if/as needed.
   *
   * <p>NOTE: we'll use `volatile` here assuming that this is unlikely to become a performance
   * bottleneck. If it becomes one we can probably just drop it and things still should work
   * (despite warnings as per JMM regarding visibility (and lack thereof) of unguarded changes).
   *
   * @since 2.5
   */
  ...
  /**
   * We will retain representation of the pointer, as a String, so that {@link #toString} should be
   * as efficient as possible.
   *
   * <p>NOTE: starting with 2.14, there is no accompanying {@link #_asStringOffset} that MUST be
   * considered with this String; this {@code String} may contain preceding path, as it is now full
   * path of parent pointer, except for the outermost pointer instance.
   */
  ...
  /**
   * Lazily-calculated hash code: need to retain hash code now that we can no longer rely on {@link
   * #_asString} being the exact full representation (it is often "more", including parent path).
   *
   * @since 2.14
   */
  ...
  /**
   * Constructor used for creating "empty" instance, used to represent state that matches current
   * node.
   */
  ...
  protected JsonPointer(
      String fullString, int fullStringOffset, String segment, int matchIndex, JsonPointer next) {
    _asString = fullString;
    _asStringOffset = fullStringOffset;
    _nextSegment = next;
    _matchingPropertyName = segment;
  ...
  /**
   * Factory method that parses given input and construct matching pointer instance, if it
   * represents a valid JSON Pointer: if not, a {@link IllegalArgumentException} is thrown.
   *
   * @param expr Pointer expression to compile
   * @return Compiled {@link JsonPointer} path expression
   * @throws IllegalArgumentException Thrown if the input does not present a valid JSON Pointer
   *     expression: currently the only such expression is one that does NOT start with a slash
   *     ('/').
   */
  ...
  /**
   * Alias for {@link #compile}; added to make instances automatically deserializable by Jackson
   * databind.
   *
   * @param expr Pointer expression to compile
   * @return Compiled {@link JsonPointer} path expression
  ...
  /**
   * Accessor for an "empty" expression, that is, one you can get by calling {@link #compile} with
   * "" (empty String).
   *
   * <p>NOTE: this is different from expression for {@code "/"} which would instead match Object
   * node property with empty String ("") as name.
   *
   ...
   * {@link JsonStreamContext} points to.
   *
  ...
  public static JsonPointer forPath(JsonStreamContext context, boolean includeRoot) {
    ...
    for (; currSegment != null; currSegment = currSegment.prev) {
      if (currSegment.property != null) {
        ...
        currPtr =
            new JsonPointer(
                fullPath, currSegment.pathOffset, String.valueOf(index), index, currPtr);
      }
    }
  ...
  /**
   * @return True if the root selector matches property name (that is, could match field value of
   *     JSON Object node)
   */
  ...
  /**
   * @return True if the root selector matches element index (that is, could match an element of
   *     JSON Array node)
   */
  ...
  /**
   * @return the leaf of current JSON Pointer expression: leaf is the last non-null segment of
   ...
   *   <li>Newly constructed {@link JsonPointer} instance that starts with all segments of `this`,
   *       followed by all segments of `tail`.
   * </ul>
   *
   * @param tail {@link JsonPointer} instance to append to this one, to create a new pointer
   *     instance
   * @return Either `this` instance, `tail`, or a newly created combination, as per description
   *     above.
   */
  ...
  /**
   * ATTENTION! {@link JsonPointer} is head centric, tail appending is much costlier than head
   * appending. It is not recommended to overuse the method.
   *
   * <p>Mutant factory method that will return
   *
   * <ul>
   *   <li>`this` instance if `property` is null or empty String, OR
   *   <li>Newly constructed {@link JsonPointer} instance that starts with all segments of `this`,
   *       followed by new segment of 'property' name.
   * </ul>
   *
   * 'property' format is starting separator (optional, added automatically if not provided) and new
   * segment name.
   *
   ...
   * appending. It is not recommended to overuse the method.
   *
   * <p>Mutant factory method that will return newly constructed {@link JsonPointer} instance that
   * starts with all segments of `this`, followed by new segment of element 'index'. Element 'index'
  ...
  /**
   * Method that may be called to see if the pointer head (first segment) would match property (of a
   * JSON Object) with given name.
   *
  ...
  /**
   * Method that may be called to check whether the pointer head (first segment) matches specified
   * Object property (by name) and if so, return {@link JsonPointer} that represents rest of the
   * path after match. If there is no match, {@code null} is returned.
   *
   * @param name Name of Object property to match
   * @return Remaining path after matching specified property, if there is match; {@code null}
   *     otherwise
   */
  ...
  /**
   * Method that may be called to see if the pointer would match Array element (of a JSON Array)
   * with given index.
   *
  ...
  /**
   * Method that may be called to check whether the pointer head (first segment) matches specified
   * Array index and if so, return {@link JsonPointer} that represents rest of the path after match.
   * If there is no match, {@code null} is returned.
   *
   * @param index Index of Array element to match
   * @return Remaining path after matching specified index, if there is match; {@code null}
   *     otherwise
   * @since 2.6
   */
  ...
  /**
   * Accessor for getting a "sub-pointer" (or sub-path), instance where current segment has been
   * removed and pointer includes rest of the segments. For example, for JSON Pointer
   * "/root/branch/leaf", this method would return pointer "/branch/leaf". For matching state (last
   * segment), will return {@code null}.
   *
   * <p>Note that this is a very cheap method to call as it simply returns "next" segment (which has
   * been constructed when pointer instance was constructed).
   *
   * @return Tail of this pointer, if it has any; {@code null} if this pointer only has the current
   *     segment
   */
  ...
  /**
   * Accessor for getting a pointer instance that is identical to this instance except that the last
   * segment has been dropped. For example, for JSON Pointer "/root/branch/leaf", this method would
   * return pointer "/root/branch" (compared to {@link #tail()} that would return "/branch/leaf").
   *
   * <p>Note that whereas {@link #tail} is a very cheap operation to call (as "tail" already exists
   * for single-linked forward direction), this method has to fully construct a new instance by
   * traversing the chain of segments.
   *
   * @return Pointer expression that contains same segments as this one, except for the last
   *     segment.
   * @since 2.5
   */
  ...
  private static JsonPointer _buildPath(
      final String fullPath, int fullPathOffset, String segment, PointerParent parent) {
  ...
  /**
   * Method called to extract the next segment of the path, in case where we seem to have
   * encountered a (tilde-)escaped character within segment.
   *
   ...
   * @return Offset at which slash was encountered, if any, or -1 if expression ended without seeing
   *     unescaped slash
   */
  protected static int _extractEscapedSegment(
      String input, int firstCharOffset, int i, StringBuilder sb) {
  ...
  protected JsonPointer _constructHead() {
    ...
    return new JsonPointer(
        fullString.substring(0, fullString.length() - suffixLength),
        0,
  ...
  private static class PointerParent {
    ...
    PointerParent(PointerParent pp, int fpo, String sgm) {
      parent = pp;
      fullPathOffset = fpo + 1;
      segment = sgm;
    }
  }

  /** Helper class used to contain a single segment when constructing JsonPointer from context. */
   ...
   * Externalizable/Serializable for JsonPointer. Note that here we do not store offset but simply
   * use (and expect use) full path, from which we need to decode actual structure.
   *
   * @since 2.14
   */
