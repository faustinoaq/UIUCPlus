...
/**
 * TextBuffer is a class similar to {@link java.lang.StringBuffer}, with following differences:
 *
 * <ul>
 *   <li>TextBuffer uses segments character arrays, to avoid having to do additional array copies
 *       when array is not big enough. This means that only reallocating that is necessary is done
 *       only once: if and when caller wants to access contents in a linear array (char[], String).
 *   <li>TextBuffer can also be initialized in "shared mode", in which it will just act as a wrapper
 *       to a single char array managed by another object (like parser that owns it)
...
public class TextBuffer {
   ...
   *
   * <p>Reduced from 1000 down to 500 in 2.10.
   */
  static final int MIN_SEGMENT_LEN = 500;

  /**
   * Let's limit maximum segment length to something sensible. For 2.10, let's limit to using 64kc
   * chunks (128 kB) -- was 256kC/512kB up to 2.9
   */
  static final int MAX_SEGMENT_LEN = 0x10000;
  ...
  /**
   * Shared input buffer; stored here in case some input can be returned as is, without being copied
   * to collector's own buffers. Note that this is read-only for this Object.
   */
  private char[] _inputBuffer;

  /**
   * Character offset of first char in input buffer; -1 to indicate that input buffer currently does
   * not contain any useful char data
   */
  ...
  /**
   * String that will be constructed when the whole contents are needed; will be temporarily stored
   * in case asked for again.
   */
   ...
   *
   * @param initialSegment Initial, full segment to use for creating buffer (buffer {@link #size()}
   *     would return length of {@code initialSegment})
   * @return TextBuffer constructed
   * @since 2.10
   */
  public static TextBuffer fromInitial(char[] initialSegment) {
    return new TextBuffer(initialSegment, null);
  }

  /**
   * Method called to indicate that the underlying buffers should now be recycled if they haven't
   * yet been recycled. Although caller can still use this text buffer, it is not advisable to call
   * this method if that is likely, since next time a buffer is needed, buffers need to reallocated.
   *
   * <p>Note: since Jackson 2.11, calling this method will NOT clear already aggregated contents
   * (that is, {@code _currentSegment}, to retain current token text if (but only if!) already
   * aggregated.
   */
  ...
  /**
   * Method called to clear out any content text buffer may have, and initializes buffer to use
   * non-shared data.
   */
  ...
  /**
   * Method for clearing out possibly existing content, and replacing them with a single-character
   * content (so {@link #size()} would return {@code 1})
   *
  ...
  /**
   * Method called to initialize the buffer with a shared copy of data; this means that buffer will
   * just have pointers to actual data. It also means that if anything is to be appended to the
   * buffer, it will first have to unshare it (make a local copy).
   *
  ...
  /**
   * Method for accessing the currently active (last) content segment without changing state of the
   * buffer
   *
  ...
  /**
   * @param lastSegmentEnd End offset in the currently active segment, could be 0 in the case of
   *     first character is delimiter or end-of-line
   ...
   * {@link #getTextBuffer}.
   *
   * @return {@code True} if access via {@link #getTextBuffer()} would be efficient (that is,
   *     content already available as aggregated {@code char[]})
   */
  ...
  /**
   * Accessor that may be used to get the contents of this buffer as a single {@code char[]}
   * regardless of whether they were collected in a segmented fashion or not: this typically require
  ...
  /**
   * Accessor that may be used to get the contents of this buffer as a single {@code String}
   * regardless of whether they were collected in a segmented fashion or not: this typically require
   ...
   * @deprecated Since 2.15 just access String contents if necessary, call {@link
   *     NumberInput#parseBigDecimal(String, boolean)} (or other overloads) directly instead
   */
  @Deprecated
  ...
  /**
   * Specialized convenience method that will decode a 32-bit int, of at most 9 digits (and possible
   * leading minus sign).
   *
   * <p>NOTE: method DOES NOT verify that the contents actually are a valid Java {@code int} value
   * (either regarding format, or value range): caller MUST validate that.
   *
   * @param neg Whether contents start with a minus sign
   * @return Buffered text value parsed as an {@code int} using {@link NumberInput#parseInt(String)}
   *     method (which does NOT validate input)
   * @since 2.9
   */
  ...
  /**
   * Specialized convenience method that will decode a 64-bit int, of at most 18 digits (and
   * possible leading minus sign).
   *
   * <p>NOTE: method DOES NOT verify that the contents actually are a valid Java {@code long} value
   * (either regarding format, or value range): caller MUST validate that.
   *
   * @param neg Whether contents start with a minus sign
   * @return Buffered text value parsed as an {@code long} using {@link
   *     NumberInput#parseLong(String)} method (which does NOT validate input)
   * @since 2.9
   */
  ...
  /**
   * Method called to make sure that buffer is not using shared input buffer; if it is, it will copy
   * such contents to private buffer.
   */
  ...
  /**
   * Convenience method that finishes the current active content segment (by specifying how many
   * characters within consists of valid content) and aggregates and returns resulting contents
   * (similar to a call to {@link #contentsAsString()}).
   *
  ...
  /**
   * Method called to expand size of the current segment, to accommodate for more contiguous
   * content. Usually only used when parsing tokens like names if even then. Method will both expand
   * the segment and return it
   *
  ...
  /**
   * Method called to expand size of the current segment, to accommodate for more contiguous
   * content. Usually only used when parsing tokens like names if even then.
   *
  ...
  /**
   * Note: calling this method may not be as efficient as calling {@link #contentsAsString}, since
   * it's not guaranteed that resulting String is cached.
   */
  @Override
  ...
  /**
   * Method called if/when we need to append content when we have been initialized to use shared
   * buffer.
   */
  ...
  /**
   * Convenience method that can be used to verify that a String of specified length does not exceed
   * maximum specific by this constraints object: if it does, an {@link IllegalStateException} is
   * thrown.
   *
