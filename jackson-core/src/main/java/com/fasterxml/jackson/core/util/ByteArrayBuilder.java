...
/**
 * Helper class that is similar to {@link java.io.ByteArrayOutputStream} in usage, but more geared
 * to Jackson use cases internally. Specific changes include segment storage (no need to have linear
 * backing buffer, can avoid reallocations, copying), as well API not based on {@link
 * java.io.OutputStream}. In short, a very much specialized builder object.
 *
 * <p>Also implements {@link OutputStream} to allow efficient aggregation of output content as a
 * byte array, similar to how {@link java.io.ByteArrayOutputStream} works, but somewhat more
 * efficiently for many use cases.
 *
 * <p>NOTE: maximum size limited to Java Array maximum, 2 gigabytes: this because usage pattern is
 * to collect content for a `byte[]` and so although theoretically this builder can aggregate more
 * content it will not be usable as things are. Behavior may be improved if we solve the access
 * problem.
 */
public final class ByteArrayBuilder extends OutputStream {
  ...
  /**
   * Clean up method to call to release all buffers this object may be using. After calling the
   * method, no other accessors can be used (and attempt to do so may result in an exception)
   */
  public void release() {
    reset();
  ...
  /**
   * Method called when results are finalized and we can get the full aggregated result buffer to
  ...
  /**
   * Method called when starting "manual" output: will clear out current state and return the first
   * segment buffer to fill
   *
  ...
  /**
   * Method called when the current segment buffer is full; will append to current contents,
   * allocate a new segment buffer and return it
   *
   * @return Segment to use for writing
   */
   ...
   * results as a contiguous buffer.
   *
  ...
  private void _allocMore() {
    ...
    if (newPastLen < 0) {
      throw new IllegalStateException(
          "Maximum Java array size (2GB) exceeded by `ByteArrayBuilder`");
    }
