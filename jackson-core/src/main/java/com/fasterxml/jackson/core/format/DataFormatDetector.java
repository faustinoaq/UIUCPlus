...
/**
 * Simple helper class that allows data format (content type) auto-detection, given an ordered set
 * of {@link JsonFactory} instances to use for actual low-level detection.
 */
public class DataFormatDetector {
  ...
  /**
   * Ordered list of factories which both represent data formats to detect (in precedence order,
   * starting with highest) and are used for actual detection.
   */
  ...
  private DataFormatDetector(
      JsonFactory[] detectors,
  ...
  /**
   * Method that will return a detector instance that uses given optimal match level (match that is
   * considered sufficient to return, without trying to find stronger matches with other formats).
   *
  ...
  /**
   * Method that will return a detector instance that uses given minimal match level; match that may
   * be returned unless a stronger match is found with other format detectors.
   *
  ...
  /**
   * Method that will return a detector instance that allows detectors to read up to specified
   * number of bytes when determining format match strength.
   *
