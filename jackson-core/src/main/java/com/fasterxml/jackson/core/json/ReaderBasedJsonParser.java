...
/**
 * This is a concrete implementation of {@link JsonParser}, which is based on a {@link
 * java.io.Reader} to handle low-level character conversion tasks.
 */
public class ReaderBasedJsonParser extends ParserBase {
  ...
  /**
   * Reader that can be used for reading more content, if one buffer from input source, but in some
   * cases pre-loaded buffer is handed to the parser.
   */
  ...
  /**
   * Flag that indicates whether the input buffer is recycable (and needs to be returned to recycler
   * once we are done) or not.
   *
   * <p>If it is not, it also means that parser can NOT modify underlying buffer.
   */
  ...
  /**
   * Flag that indicates that the current token has not yet been fully processed, and needs to be
   * finished for some access (or skipped to obtain the next token)
   */
  protected boolean _tokenIncomplete;

  /**
   * Value of {@link #_inputPtr} at the time when the first character of name token was read. Used
   * for calculating token location when requested; combined with {@link #_currInputProcessed}, may
   * be updated appropriately as needed.
   *
   * @since 2.7
   */
  ...
  /**
   * Constructor called when caller wants to provide input buffer directly (or needs to, in case of
   * bootstrapping having read some of contents) and it may or may not be recyclable use standard
   * recycle context.
   *
   ...
   * @param bufferRecyclable Whether {@code inputBuffer} passed is managed by Jackson core (and
   *     thereby needs recycling)
   * @since 2.4
   */
  ...
  /**
   * Constructor called when input comes as a {@link java.io.Reader}, and buffer allocation can be
   * done using default mechanism.
   *
   * @param ctxt I/O context to use
   * @param features Standard stream read features enabled
   * @param r Reader used for reading actual content, if any; {@code null} if none
   ...
   */
  public ReaderBasedJsonParser(
      IOContext ctxt, int features, Reader r, ObjectCodec codec, CharsToNameCanonicalizer st) {
    super(ctxt, features);
    _reader = r;
  ...
  /**
   * Method for accessing textual representation of the current event; if no current event (before
   * first call to {@link #nextToken}, or after encountering end-of-input), returns null. Method can
   * be called for any event.
   */
  @Override
  ...
  public final String getValueAsString() throws IOException {
    if (_currToken == JsonToken.VALUE_STRING) {
      if (_tokenIncomplete) {
        _tokenIncomplete = false;
        // BUGGY VERSION 2: changing method call to an undefined method
        _finishString2(); // only strings can be incomplete
      }
    ...
    if (_currToken == JsonToken.FIELD_NAME) {
      // BUGGY VERSION 2: changing the return value to an undefined variable
  ...
  /**
   * Initial parsing method for number values. It needs to be able to parse enough input to be able
   * to determine whether the value is to be considered a simple integer value, or a more generic
   * decimal value: latter of which needs to be expressed as a floating point number. The basic rule
   * is that if the number has no fractional or exponential part, it is an integer; otherwise a
   * floating point number.
   *
   * <p>Because much of input has to be processed in any case, no partial parsing is done: all input
   * text will be stored for further processing. However, actual numeric value conversion will be
   * deferred, since it is usually the most complicated and costliest part of processing.
   *
   * @param ch The first non-null digit character of the number to parse
   * @return Type of token decoded, usually {@link JsonToken#VALUE_NUMBER_INT} or {@link
  ...
  /**
   * Method called to parse a number, when the primary parse method has failed to parse it, due to
   * it being split on buffer boundary. As a result code is very similar, except that it has to
   * explicitly copy contents to the text buffer instead of just sharing the main input buffer.
   *
  ...
  private char _verifyNLZ2() throws IOException {
    ...
    // if so, just need to skip either all zeroes (if followed by number); or all but one (if
    // non-number)
  ...
  protected JsonToken _handleInvalidNumberStart(
      int ch, final boolean negative, final boolean hasSign) throws IOException {
    if (ch == 'I') {
      if (_inputPtr >= _inputEnd) {
        if (!_loadMore()) {
    ...
    if (!isEnabled(JsonReadFeature.ALLOW_LEADING_PLUS_SIGN_FOR_NUMBERS.mappedFeature())
      ...
      _reportUnexpectedNumberChar(
          '+',
          "JSON spec does not allow numbers to have plus signs: enable"
   ...
   *
   * <p>NOTE: caller MUST ensure there is at least one character available; and that input pointer
   * is AT given char (not past)
   *
   * @param ch First character of likely white space to skip
   * @throws IOException for low-level read issues, or {@link JsonParseException} for decoding
   *     problems (invalid white space)
   */
   ...
   * field name. In standard mode will just throw an expection; but in non-standard modes may be
   * able to parse name.
   *
  ...
  protected String _handleOddName(int i) throws IOException {
    ...
    if (!firstOk) {
      _reportUnexpectedChar(
          i,
          "was expecting either valid name character (for unquoted name) or double-quote (for"
  ...
  /**
   * Method for handling cases where first non-space character of an expected value token is not
   * legal for standard JSON content.
   *
  ...
  protected JsonToken _handleOddValue(int i) throws IOException {
    ...
    switch (i) {
      ...
      case 'N':
        ...
        _reportError(
            "Non-standard token 'NaN': enable `JsonReadFeature.ALLOW_NON_NUMERIC_NUMBERS` to"
                + " allow");
        break;
      case 'I':
        ...
        _reportError(
            "Non-standard token 'Infinity': enable `JsonReadFeature.ALLOW_NON_NUMERIC_NUMBERS` to"
                + " allow");
        break;
  ...
  protected JsonToken _handleApos() throws IOException {
    ...
    while (true) {
      if (_inputPtr >= _inputEnd) {
        if (!_loadMore()) {
          _reportInvalidEOF(
              ": was expecting closing quote for a string value", JsonToken.VALUE_STRING);
        }
      }
      char c = _inputBuffer[_inputPtr++];
      int i = (int) c;
  ...
  /**
   * Method called to skim through rest of unparsed String value, if it is not needed. This can be
   * done bit faster if contents need not be stored for future access.
   *
   * @throws IOException for low-level read issues, or {@link JsonParseException} for decoding
   *     problems (invalid white space)
   */
  ...
  private final int _skipComma(int i) throws IOException {
    if (i != INT_COMMA) {
      _reportUnexpectedChar(
          i, "was expecting comma to separate " + _parsingContext.typeDesc() + " entries");
    }
  ...
  private final int _skipAfterComma2() throws IOException {
    ...
    throw _constructError(
        "Unexpected end-of-input within/between " + _parsingContext.typeDesc() + " entries");
  }
  ...
  private void _skipComment() throws IOException {
    if ((_features & FEAT_MASK_ALLOW_JAVA_COMMENTS) == 0) {
      _reportUnexpectedChar(
          '/',
          "maybe a (non-standard) comment? (not recognized as one since Feature 'ALLOW_COMMENTS'"
  ...
  public JsonLocation getTokenLocation() {
    ...
    return new JsonLocation(
        _contentReference(), -1L, _tokenInputTotal - 1, _tokenInputRow, _tokenInputCol);
  }
  ...
  public JsonLocation getCurrentLocation() {
    ...
    return new JsonLocation(
        _contentReference(), -1L, _currInputProcessed + _inputPtr, _currInputRow, col);
  }
