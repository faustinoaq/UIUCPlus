...
/**
 * Object that encapsulates Location information used for reporting parsing (or potentially
 * generation) errors, as well as current location within input streams.
 *
 * <p>NOTE: users should be careful if using {@link #equals} implementation as it may or may not
 * compare underlying "content reference" for equality. Instead if would make sense to explicitly
 * implementing equality checks using specific criteria caller desires.
 */
public class JsonLocation implements java.io.Serializable {
  ...
  /**
   * Shared immutable "N/A location" that can be returned to indicate that no location information
   * is available.
   *
   * <p>NOTE: before 2.9, Location was given as String "N/A"; with 2.9 it was removed so that source
   * should be indicated as "UNKNOWN".
   */
  public static final JsonLocation NA =
      new JsonLocation(ContentReference.unknown(), -1L, -1L, -1, -1);
  ...
  /**
   * Reference to input source; never null (but may be that of {@link ContentReference#unknown()}).
   *
  ...
  /**
   * Lazily constructed description for source; constructed if and when {@link #sourceDescription()}
   * is called, retained.
   *
   * @since 2.13
   */
  ...
  /**
   * Accessor for information about the original input source content is being read from. Returned
   * reference is never {@code null} but may not contain useful information.
   *
   * <p>NOTE: not getter, on purpose, to avoid inlusion if serialized using default Jackson
   * serializer.
   *
  ...
  /**
   * Reference to the original resource being read, if one available. For example, when a parser has
   * been constructed by passing a {@link java.io.File} instance, this method would return that
   * File. Will return null if no such reference is available, for example when {@link
  ...
  /**
   * Access for getting line number of this location, if available. Note that line number is
   * typically not available for binary formats.
   *
  ...
  /**
   * Access for getting column position of this location, if available. Note that column position is
   * typically not available for binary formats.
   *
  ...
  /**
   * @return Character offset within underlying stream, reader or writer, if available; {@code -1}
   *     if not.
   */
  ...
  /**
   * @return Byte offset within underlying stream, reader or writer, if available; {@code -1} if
   *     not.
   */
  ...
  /**
   * Accessor for getting a textual description of source reference (Object returned by {@link
   * #getSourceRef()}), as included in description returned by {@link #toString()}.
   *
   * <p>Note: implementation will simply call {@link ContentReference#buildSourceDescription()})
   *
   * <p>NOTE: not added as a "getter" to prevent it from getting serialized.
   *
  ...
  /**
   * Accessor for a brief summary of Location offsets (line number, column position, or byte offset,
   * if available).
   *
   * @return Description of available relevant location offsets; combination of line number and
   *     column position or byte offset
   * @since 2.13
   */
  ...
  public String toString() {
    ...
    StringBuilder sb =
        new StringBuilder(40 + srcDesc.length()).append("[Source: ").append(srcDesc).append("; ");
