 ...
 * Although there is somewhat standard basic version (so-called "MIME Base64"), other variants
...
public final class Base64Variant implements java.io.Serializable {
  ...
  public enum PaddingReadBehaviour {
    /**
     * Padding is not allowed in Base64 content being read (finding something that looks like
     * padding at the end of content results in an exception)
     */
    PADDING_FORBIDDEN,

    /**
     * Padding is required in Base64 content being read (missing padding for incomplete ending
     * quartet results in an exception)
     */
    PADDING_REQUIRED,

    /**
     * Padding is allowed but not required in Base64 content being read: no exception thrown based
     * on existence or absence, as long as proper padding characters are used.
     */
    PADDING_ALLOWED;
  }
  ...
  /**
   * Marker used to denote ascii characters that do not correspond to a 6-bit value (in this
   * variant), and is not used as a padding character.
   */
  public static final int BASE64_VALUE_INVALID = -1;

  /**
   * Marker used to denote ascii character (in decoding table) that is the padding character using
   * this variant (if any).
   */
   ...
   *
   * <p>Note that this is the only non-transient field; used when reading back from serialized
   * state.
   *
   * <p>Also: must not be private, accessed from `BaseVariants`
   */
  final String _name;
  ...
  /**
   * Maximum number of encoded base64 characters to output during encoding before adding a linefeed,
   * if line length is to be limited ({@link java.lang.Integer#MAX_VALUE} if not limited).
   *
   * <p>Note: for some output modes (when writing attributes) linefeeds may need to be avoided, and
   * this value ignored.
   */
  ...
  /**
   * "Copy constructor" that can be used when the base alphabet is identical to one used by another
   * variant except for the maximum line length (and obviously, name).
   *
  ...
  /**
   * "Copy constructor" that can be used when the base alphabet is identical to one used by another
   * variant, but other details (padding, maximum line length) differ
   *
   * @param base Variant to use for settings not specific by other parameters
   * @param name Name of this variant
   ...
   */
  public Base64Variant(
      Base64Variant base, String name, boolean writePadding, char paddingChar, int maxLineLength) {
    this(base, name, writePadding, paddingChar, base._paddingReadBehaviour, maxLineLength);
  }

  private Base64Variant(
      Base64Variant base,
      String name,
      boolean writePadding,
      char paddingChar,
   ...
   * @return Instance with desired padding read behavior setting (this if already has setting; new
   *     instance otherwise)
   * @since 2.12
   */
  ...
  protected Object readResolve() {
    ...
    if ((_writePadding != base._writePadding)
      ...
      return new Base64Variant(
          base, _name, _writePadding, _paddingChar, !_paddingReadBehaviour, _maxLineLength);
    }
    return base;
  }
  ...
  /**
   * @return True if this Base64 encoding will <b>write</b> padding on output (note: before Jackson
   *     2.12 also dictated whether padding was accepted on read)
   */
  ...
  /**
   * @return Indicator on how this Base64 encoding will handle possible padding in content when
   *     reading.
   * @since 2.12
   */
  ...
  /**
   * Method that encodes given right-aligned (LSB) 24-bit value into 4 base64 characters, stored in
   * given result buffer. Caller must ensure there is sufficient space for 4 encoded characters at
   * specified position.
   *
  ...
  /**
   * Method that outputs partial chunk (which only encodes one or two bytes of data). Data given is
   * still aligned same as if it as full data; that is, missing data is at the "right end" (LSB) of
   * int.
   *
  ...
  /**
   * Method that encodes given right-aligned (LSB) 24-bit value into 4 base64 bytes (ascii), stored
   * in given result buffer.
   *
   * @param b24 3-byte value to encode
  ...
  /**
   * Convenience method for converting given byte array as base64 encoded String using this
   * variant's settings. Resulting value is "raw", that is, not enclosed in double-quotes.
   *
  ...
  /**
   * Convenience method for converting given byte array as base64 encoded String using this
   * variant's settings, optionally enclosed in double-quotes. Linefeeds added, if needed, are
   * expressed as 2-character JSON (and Java source) escape sequence of backslash + `n`.
   *
   * @param input Byte array to encode
   * @param addQuotes Whether to surround resulting value in double quotes or not
   * @return Base64 encoded String of encoded {@code input} bytes, possibly surrounded by quotes (if
   *     {@code addQuotes} enabled)
   */
  ...
  /**
   * Convenience method for converting given byte array as base64 encoded String using this
   * variant's settings, optionally enclosed in double-quotes. Linefeed character to use is passed
   * explicitly.
   *
   * @param input Byte array to encode
   * @param addQuotes Whether to surround resulting value in double quotes or not
  ...
  protected String unexpectedPaddingMessage() {
    return String.format(
        "Unexpected end of base64-encoded String: base64 variant '%s' expects no padding at the end"
  ...
  public String missingPaddingMessage() { // !!! TODO: why is this 'public'?
    return String.format(
        "Unexpected end of base64-encoded String: base64 variant '%s' expects padding (one or more"
            + " '%c' characters) at the end. This Base64Variant might have been incorrectly"
            + " configured",
