...
public final class CharacterReader {
  ...
  private String[] stringCache =
      new String[stringCacheSize]; // holds reused strings in this doc, to lessen garbage
  ...
  private boolean
      readFully; // if the underlying stream has been completely read, no value in further buffering
  ...
  private void bufferUp() {
    ...
    try {
      ...
      if (read > 0) {
        Validate.isTrue(
            skipped
                == pos); // Previously asserted that there is room in buf to skip, so this will be a
                         // WTF
        bufLength = read;
        readerPos += pos;
        bufPos = offset;
  ...
  /**
   * Enables or disables line number tracking. By default, will be <b>off</b>.Tracking line numbers
   * improves the legibility of parser error messages, for example. Tracking should be enabled
   * before any content is read to be of use.
   *
  ...
  int lineNumber(int pos) {
    // note that this impl needs to be called before the next buffer up or line numberoffset will be
    // wrong. if that
    // causes issues, can remove the reset of newlinepositions during buffer, at the cost of a
    // larger tracking array
  ...
  /**
   * Get a formatted string representing the current line and cursor positions. E.g. <code>5:10
   * </code> indicating line number 5 and column number 10.
   *
  ...
  private void scanBufferForNewlines() {
    ...
    if (newlinePositions.size() > 0) {
      // work out the line number that we have read up to (as we have likely scanned past this
      // point)
  ...
  /**
   * Unconsume one character (bufPos--). MUST only be called directly after a consume(), and no
   * chance of a bufferUp.
   */
  void unconsume() {
    if (bufPos < 1)
      throw new UncheckedIOException(
          new IOException(
              "WTF: No buffer left to unconsume.")); // a bug if this fires, need to trace it.
  ...
  /**
   * Returns the number of characters between the current position and the next instance of the
   * input char
   *
   * @param c scan target
  ...
  String consumeTo(String seq) {
    ...
    if (offset != -1) {
      ...
      // nextIndexOf() did a bufferUp(), so if the buffer is shorter than the search string, we must
      // be at EOF
      return consumeToEnd();
    } else {
      // the string we're looking for may be straddling a buffer boundary, so keep (length - 1)
      // characters
  ...
  String consumeTagName() {
    // '\t', '\n', '\r', '\f', ' ', '/', '>'
    // NOTE: out of spec, added '<' to fix common author bugs; does not stop and append on nullChar
    // but eats
    bufferUp();
    int pos = bufPos;
    final int start = pos;
  ...
  boolean matches(String seq) {
    ...
    for (int offset = 0; offset < scanLength; offset++)
      if (seq.charAt(offset) != charBuf[bufPos + offset]) return false;
    return true;
  }
  ...
  boolean buggyVersion1(char[] seq) {
    bufferUp();
  ...
  // we maintain a cache of the previously scanned sequence, and return that if applicable on
  // repeated scans.
  // that improves the situation where there is a sequence of <p<p<p<p<p<p<p...</title> and we're
  // bashing on the <p
  ...
  /**
   * Used to check presence of </title>, </style> when we're in RCData and see a <xxx. Only finds
   * consistent case.
   */
  boolean containsIgnoreCase(String seq) {
    ...
    boolean found = hi > -1;
    lastIcIndex =
        found ? bufPos + hi : -1; // we don't care about finding the nearest, just that buf contains
    return found;
  }
   ...
   * leaks.
   *
   * <p>Simplistic, and on hash collisions just falls back to creating a new string, vs a full
   * HashMap with Entry list. That saves both having to create objects as hash keys, and running
   * through the entry list, at the expense of some more duplicates.
   */
  ...
  static boolean rangeEquals(
      final char[] charBuf, final int start, int count, final String cached) {
