...
/**
 * Parses HTML into a {@link org.jsoup.nodes.Document}. Generally best to use one of the more
 * convenient parse methods in {@link org.jsoup.Jsoup}.
 */
public class Parser {
  ...
  /**
   * Creates a new Parser as a deep copy of this; including initializing a new TreeBuilder. Allows
   * independent (multi-threaded) use.
   *
  ...
  /**
   * Test if position tracking is enabled. If it is, Nodes will have a Position to track where in
   * the original input source they were created from. By default, tracking is not enabled.
   *
  ...
  /**
   * Enable or disable source position tracking. If enabled, Nodes will have a Position to track
   * where in the original input source they were created from.
   *
  ...
  /**
   * Update the ParseSettings of this Parser, to control the case sensitivity of tags and
   * attributes.
   *
  ...
  /**
   * (An internal method, visible for Element. For HTML parse, signals that script and style text
   * should be treated as Data Nodes).
   */
  ...
  /**
   * Parse a fragment of HTML into a list of nodes. The context element, if supplied, supplies
   * parsing context.
   *
   * @param fragmentHtml the fragment of HTML to parse
   * @param context (optional) the element that this HTML fragment is being parsed for (i.e. for
   *     inner HTML). This provides stack context (for implicit element creation).
   * @param baseUri base URI of document (i.e. original fetch location), for resolving relative
   *     URLs.
   * @return list of nodes parsed from the input HTML. Note that the context element, if supplied,
   *     is not modified.
   */
   ...
   *     is not modified.
   */
  public static List<Node> parseFragment(
      String fragmentHtml, Element context, String baseUri, ParseErrorList errorList) {
    HtmlTreeBuilder treeBuilder = new HtmlTreeBuilder();
  ...
  /**
   * Create a new HTML parser. This parser treats input as HTML5, and enforces the creation of a
   * normalised document, based on a knowledge of the semantics of the incoming tags.
   *
  ...
  /**
   * Create a new XML parser. This parser assumes no knowledge of the incoming tags and does not
   * treat it as HTML, rather creates a simple tree directly from the input.
   *
