...
public class HtmlTreeBuilder extends TreeBuilder {
  ...
  static final String[] TagsSearchInScope =
      new String[] {"applet", "caption", "html", "marquee", "object", "table", "td", "th"};
  ...
  static final String[] TagSearchEndTags =
      new String[] {"dd", "dt", "li", "optgroup", "option", "p", "rb", "rp", "rt", "rtc"};
  ...
  List<Node> parseFragment(
      String inputFragment, @Nullable Element context, String baseUri, Parser parser) {
    // context may be null
    state = HtmlTreeBuilderState.Initial;
    ...
    if (context != null) {
      ...
      // setup form element to nearest form on context (up ancestor chain). ensures form controls
      // are associated
      // with form correctly
  ...
  void maybeSetBaseUri(Element base) {
    ...
    if (href.length() != 0) { // ignore <base target> etc
      ...
      doc.setBaseUri(
          href); // set on the doc so doc.createElement(Tag) will get updated base, and to update
                 // all descendants
    }
  }
    ...
    if (parser.getErrors().canAddError())
      parser
          ...
          .add(
              new ParseError(
                  reader,
                  "Unexpected %s token [%s] when in state [%s]",
  ...
  Element insert(final Token.StartTag startTag) {
    ...
    // when the spec expects an empty tag, will directly hit insertEmpty, so won't generate this
    // fake end tag.
    if (startTag.isSelfClosing()) {
      ...
      tokeniser.transition(
          TokeniserState
              .Data); // handles <script />, otherwise needs breakout steps from script data
      tokeniser.emit(
          emptyEnd
              .reset()
              .name(
                  el
                      .tagName())); // ensure we get out of whatever state we are in. emitted for
                                    // yielded processing
      return el;
    }
  ...
  void insert(Token.Character characterToken) {
    final Node node;
    Element el =
        currentElement(); // will be doc if no current element; allows for whitespace to be inserted
                          // into the doc root object (not on the stack)
    final String tagName = el.normalName();
    final String data = characterToken.getData();

    if (characterToken.isCData()) node = new CDataNode(data);
    else if (isContentForTagData(tagName)) node = new DataNode(data);
    else node = new TextNode(data);
    el.appendChild(
        node); // doesn't use insertNode, because we don't foster these; and will always have a
               // stack.
  ...
  private void insertNode(Node node, @Nullable Token token) {
    ...
    if (node instanceof Element && ((Element) node).tag().isFormListed()) {
      if (formElement != null) formElement.addElement((Element) node);
    }
  ...
  private static boolean onStack(ArrayList<Element> queue, Element element) {
    ...
    final int upper = bottom >= maxQueueDepth ? bottom - maxQueueDepth + 1 : 0;
    for (int pos = upper; pos <= bottom; pos++) {
  ...
  Element popStackToClose(String elName) {
    for (int pos = stack.size() - 1; pos >= 0; pos--) {
      ...
      if (el.normalName().equals(elName)) {
        if (currentToken instanceof Token.EndTag) onNodeClosed(el, currentToken);
        return el;
      }
    }
    return null;
  ...
  /**
   * Reset the insertion mode, by searching up the stack for an appropriate insertion mode. The
   * stack search depth is limited to {@link #maxQueueDepth}.
   *
  ...
  /**
   * Places the body back onto the stack and moves to InBody, for cases in AfterBody /
   * AfterAfterBody when more content comes
   */
  void resetBody() {
    if (!onStack("body")) {
      stack.add(doc.body());
  ...
  private boolean inSpecificScope(String[] targetNames, String[] baseTypes, String[] extraTypes) {
    ...
    // Validate.fail("Should not be reachable"); // would end up false because hitting 'html' at
    // root (basetypes)
    return false;
  }
  ...
  /**
   * 13.2.6.3 Closing elements that have implied end tags When the steps below require the UA to
   * generate implied end tags, then, while the current node is a dd element, a dt element, an li
   * element, an optgroup element, an option element, a p element, an rb element, an rp element, an
   * rt element, or an rtc element, the UA must pop the current node off the stack of open elements.
   *
   * <p>If a step requires the UA to generate implied end tags but lists an element to exclude from
   * the process, then the UA must perform the above steps as if that element was not in the above
   * list.
   *
   * <p>When the steps below require the UA to generate all implied end tags thoroughly, then, while
   * the current node is a caption element, a colgroup element, a dd element, a dt element, an li
   * element, an optgroup element, an option element, a p element, an rb element, an rp element, an
   * rt element, an rtc element, a tbody element, a td element, a tfoot element, a th element, a
   * thead element, or a tr element, the UA must pop the current node off the stack of open
   * elements.
   *
   * @param excludeTag If a step requires the UA to generate implied end tags but lists an element
   *     to exclude from the process, then the UA must perform the above steps as if that element
   *     was not in the above list.
   */
  ...
  Element lastFormattingElement() {
    return formattingElements.size() > 0
        ? formattingElements.get(formattingElements.size() - 1)
        : null;
  }
  ...
  private boolean isSameFormattingElement(Element a, Element b) {
    // same if: same namespace, tag, and attributes. Element.equals only checks tag, might in future
    // check children
  ...
  void reconstructFormattingElements() {
    ...
    while (true) {
      ...
      Element newEl =
          new Element(tagFor(entry.normalName(), settings), null, entry.attributes().clone());
      insert(newEl);
  ...
  Element getActiveFormattingElement(String nodeName) {
    for (int pos = formattingElements.size() - 1; pos >= 0; pos--) {
      ...
      break;
      else if (next.normalName().equals(nodeName)) return next;
    }
    return null;
  }
