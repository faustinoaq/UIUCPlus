...
/**
 * The Tree Builder's current state. Each state embodies the processing for the state, and
...
enum HtmlTreeBuilderState {
      ...
      switch (t.type) {
        ...
        case StartTag:
          ...
          if (name.equals("html")) {
            ...
            // else if noscript && scripting flag = true: rawtext (jsoup doesn't run script, to
            // handle as noscript)
            tb.insert(start);
    ...
    boolean process(Token t, HtmlTreeBuilder tb) {
      if (t.isDoctype()) {
        ...
        tb.transition(InHead);
          ...
          || t.isComment()
          || (t.isStartTag() && inSorted(t.asStartTag().normalName(), InHeadNoScriptHead))) {
        return tb.process(t, InHead);
      } else if (t.isEndTag() && t.asEndTag().normalName().equals("br")) {
        return anythingElse(t, tb);
      } else if ((t.isStartTag() && inSorted(t.asStartTag().normalName(), InHeadNoscriptIgnore))
          || t.isEndTag()) {
        tb.error(this);
        return false;
      } else {
  ...
  InBody {
    boolean process(Token t, HtmlTreeBuilder tb) {
      switch (t.type) {
        case Character:
          {
            ...
            if (c.getData().equals(nullString)) {
              ...
              return false;
                ...
                && isWhitespace(c)) { // don't check if whitespace if frames already closed
        ...
        case EOF:
          ...
          // todo: error if stack contains something not dd, dt, li, p, tbody, td, tfoot, th, thead,
          // tr, body, html
          // stop parsing
          break;
      }
    ...
    private boolean inBodyStartTag(Token t, HtmlTreeBuilder tb) {
      ...
      switch (name) {
        ...
        case "body":
          ...
          stack = tb.getStack();
          if (stack.size() == 1
              || (stack.size() > 2 && !stack.get(1).normalName().equals("body"))
              || tb.onStack("template")) {
            // only in fragment case
            return false; // ignore
          } else {
            tb.framesetOk(false);
            // will be on stack if this is a nested body. won't be if closed (which is a variance
            // from spec, which leaves it on)
            Element body;
            if (startTag.hasAttributes()
                && (body = tb.getFromStack("body")) != null) { // we only ever put one body on stack
              for (Attribute attribute : startTag.attributes) {
        ...
        case "select":
          ...
          if (state.equals(InTable)
              || state.equals(InCaption)
              || state.equals(InTableBody)
              || state.equals(InRow)
              || state.equals(InCell)) tb.transition(InSelectInTable);
      ...
      switch (name) {
        ...
        case "body":
          if (!tb.inScope("body")) {
            ...
            // todo: error if stack contains something not dd, dt, li, optgroup, option, p, rp, rt,
    ...
    boolean anyOtherEndTag(Token t, HtmlTreeBuilder tb) {
      final String name =
          t.asEndTag()
              .normalName; // case insensitive search - goal is to preserve output case, not for the
                           // parse to be case sensitive
    ...
    private boolean inBodyEndTagAdoption(Token t, HtmlTreeBuilder tb) {
      ...
      for (int i = 0; i < 8; i++) {
        ...
        for (int si = 1; si < stackSize && si < 64; si++) {
          // TODO: this no longer matches the current spec at
          // https://html.spec.whatwg.org/#adoption-agency-algorithm and should be updated
          el = stack.get(si);
          if (el == formatEl) {
        ...
        for (int j = 0; j < 3; j++) {
          ...
          Element replacement =
              new Element(tb.tagFor(node.nodeName(), ParseSettings.preserveCase), tb.getBaseUri());
          ...
          if (lastNode == furthestBlock) {
            // move the aforementioned bookmark to be immediately after the new node in the list of
            // active formatting elements.
            // not getting how this bookmark both straddles the element above, but is inbetween
            // here...
        ...
        tb.removeFromActiveFormattingElements(formatEl);
        // insert the new element into the list of active formatting elements at the position of the
  ...
  Text {
    ...
    boolean process(Token t, HtmlTreeBuilder tb) {
      if (t.isCharacter()) {
        ...
        // if: An end tag whose tag name is "script" -- scripting nesting level, if evaluating
        // scripts
        tb.pop();
        tb.transition(tb.originalState());
      }
    ...
    boolean process(Token t, HtmlTreeBuilder tb) {
      ...
      switch (t.type) {
        ...
        case EndTag:
          ...
          break;
        case EOF:
          if (tb.currentElementIs("html")) return true; // stop parsing; frag case
  ...
  InRow {
    ...
    private boolean anythingElse(Token t, HtmlTreeBuilder tb) {
      return tb.process(t, InTableFoster);
    }
      ...
      if (t.isEndTag()) {
        ...
        if (inSorted(name, Constants.InCellNames)) {
          if (!tb.inTableScope(name)) {
            tb.error(this);
            tb.transition(
                InRow); // might not be in scope if empty: <td /> and processing fake end tag
            return false;
          }
          tb.generateImpliedEndTags();
          if (!tb.currentElementIs(name)) tb.error(this);
    ...
    boolean process(Token t, HtmlTreeBuilder tb) {
      switch (t.type) {
        ...
        case StartTag:
          Token.StartTag start = t.asStartTag();
          String name = start.normalName();
          if (name.equals("html")) return tb.process(start, InBody);
        ...
        case EndTag:
          ...
          switch (name) {
            case "optgroup":
              ...
              break;
            case "option":
              if (tb.currentElementIs("option")) tb.pop();
              else tb.error(this);
              break;
            case "select":
    ...
    boolean process(Token t, HtmlTreeBuilder tb) {
      ...
      switch (t.type) {
        ...
        case StartTag:
          ...
          if (inSorted(name, InTemplateToHead)) tb.process(t, InHead);
        ...
        case EndTag:
          ...
          if (name.equals("template")) tb.process(t, InHead);
          else {
            tb.error(this);
            return false;
          }
        ...
        case EOF:
          ...
          tb.popTemplateMode();
          tb.resetInsertionMode();
          // spec deviation - if we did not break out of Template, stop processing, and don't worry
          // about cleaning up ultra-deep template stacks
          // limited depth because this can recurse and will blow stack if too deep
          if (tb.state() != InTemplate && tb.templateModeSize() < 12) return tb.process(t);
          else return true;
      }
      return true;
    }
    ...
    boolean process(Token t, HtmlTreeBuilder tb) {
      if (t.isComment()) {
        tb.insert(t.asComment());
          ...
          || isWhitespace(t)
          || (t.isStartTag() && t.asStartTag().normalName().equals("html"))) {
        return tb.process(t, InBody);
      } else if (t.isEOF()) {
        // nice work chuck
      } else if (t.isStartTag() && t.asStartTag().normalName().equals("noframes")) {
  ...
  static final class Constants {
    static final String[] InHeadEmpty =
        new String[] {"base", "basefont", "bgsound", "command", "link"};
    ...
    static final String[] InHeadNoScriptHead =
        new String[] {"basefont", "bgsound", "link", "meta", "noframes", "style"};
    ...
    static final String[] InBodyEndTableFosters =
        new String[] {"table", "tbody", "tfoot", "thead", "tr"};
    ...
    static final String[] InCellCol =
        new String[] {"caption", "col", "colgroup", "tbody", "td", "tfoot", "th", "thead", "tr"};
    ...
    static final String[] InTableBodyExit =
        new String[] {"caption", "col", "colgroup", "tbody", "tfoot", "thead"};
    static final String[] InTableBodyEndIgnore =
        new String[] {"body", "caption", "col", "colgroup", "html", "td", "th", "tr"};
    static final String[] InRowMissing =
        new String[] {"caption", "col", "colgroup", "tbody", "tfoot", "thead", "tr"};
    static final String[] InRowIgnore =
        new String[] {"body", "caption", "col", "colgroup", "html", "td", "th"};
    static final String[] InSelectEnd = new String[] {"input", "keygen", "textarea"};
    static final String[] InSelectTableEnd =
        new String[] {"caption", "table", "tbody", "td", "tfoot", "th", "thead", "tr"};
    ...
    static final String[] InTemplateToTable =
        new String[] {"caption", "colgroup", "tbody", "tfoot", "thead"};
  }
}
