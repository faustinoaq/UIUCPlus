...
abstract class TreeBuilder {
  ...
  protected Map<String, Tag>
      seenTags; // tags we've used in this parse; saves tag GC for custom tags.
  ...
  protected void initialiseParse(Reader input, String baseUri, Parser parser) {
    ...
    reader.trackNewlines(
        parser.isTrackErrors()
            || trackSourceRange); // when tracking errors or source ranges, enable newline tracking
  ...
  abstract List<Node> parseFragment(
  ...
  protected boolean processStartTag(String name) {
    // these are "virtual" start tags (auto-created by the treebuilder), so not tracking the start
    // position
    final Token.StartTag start = this.start;
    if (currentToken == start) { // don't recycle an in-use token
      return process(new Token.StartTag().name(name));
    }
    return (process(start.reset().name(name))) || (currentToken == 0);
  }
  ...
  /**
   * Get the current element (last on the stack). If all items have been removed, returns the
   * document instead (which might not actually be on the stack; use stack.size() == 0 to test if
   * required.
   *
  ...
  protected void error(String msg, Object... args) {
    ...
    if (errors.canAddError()) errors.add(new ParseError(reader, msg, args));
  }
  ...
  protected Tag tagFor(String tagName, ParseSettings settings) {
    Tag tag =
        seenTags.get(
            tagName); // note that we don't normalize the cache key. But tag via valueOf may be
                      // normalized.
    if (tag == null) {
  ...
  /**
   * Called by implementing TreeBuilders when a node has been inserted. This implementation includes
   * optionally tracking the source range of the node.
   *
  ...
  /**
   * Called by implementing TreeBuilders when a node is explicitly closed. This implementation
   * includes optionally tracking the closing source range of the node.
   *
