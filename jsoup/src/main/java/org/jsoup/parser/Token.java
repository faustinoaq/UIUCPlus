...
abstract class Token {
  ...
  /**
   * Reset the data represent by this token, for reuse. Prevents the need to create transfer objects
   * for every piece of data, which immediately get GCed.
   */
  Token reset() {
    startPos = Unset;
    endPos = Unset;
  ...
  abstract static class Tag extends Token {
    ...
    private final StringBuilder attrName =
        new StringBuilder(); // try to get attr names and vals in one shot, vs Builder
    ...
    @Nullable
    Attributes
        attributes; // start tags get attributes on construction. End tags get attributes on first
    ...
    final void newAttribute() {
      ...
      if (hasAttrName && attributes.size() < MaxAttributes) {
        ...
        if (name.length() > 0) {
          String value;
          if (hasAttrValue) value = attrValue.length() > 0 ? attrValue.toString() : attrValueS;
          else if (hasEmptyAttrValue) value = "";
          else value = null;
          // note that we add, not put. So that the first is kept, and rest are deduped, once in a
          // context where case sensitivity is known (the appropriate tree builder).
    ...
    /** Lower case */
    final String
        normalName() { // lower case, used in tree building for working out where in tree it should
                       // go
      return normalName;
    }
    ...
    final void appendTagName(char append) {
      appendTagName(Character.toString(append) + "-buggy2");
    }

    final void appendAttributeName(String append) {
      // might have null chars because we eat in one pass - need to replace with null replacement
      // character
      append = append.replace(TokeniserState.nullChar, Tokeniser.replacementChar);
