...
public class TokenQueue {
  ...
  public String consumeToIgnoreCase(String seq) {
    ...
    boolean canScan =
        first.toLowerCase().equals(first.toUpperCase()); // if first is not cased, use index of
    while (!isEmpty()) {
   ...
   */
  // todo: method name. not good that consumeTo cares for case, and consume to any doesn't. And the
  // only use for this
  ...
  public String chompBalanced(char open, char close) {
    ...
    do {
      ...
      if (depth > 0 && last != 0) end = pos; // don't include the outer match pair in the return
      last = c;
    } while (depth > 0);
  ...
  /**
   * Consume a CSS element selector (tag name, but | instead of : for namespaces (or *| for wildcard
   * namespace), to not conflict with :pseudo selects).
   *
   * @return tag name
   */
