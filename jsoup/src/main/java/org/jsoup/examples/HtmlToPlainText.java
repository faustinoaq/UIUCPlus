 ...
 *
 * <p>Note that this is a fairly simplistic formatter -- for real world use you'll want to embrace
 * and extend.
 *
 * <p>To invoke from the command line, assuming you've downloaded the jsoup jar to your current
 * directory:
 *
 * <p><code>java -cp jsoup.jar org.jsoup.examples.HtmlToPlainText url [selector]</code> where
 * <i>url</i> is the URL to fetch, and <i>selector</i> is an optional CSS selector.
 *
...
public class HtmlToPlainText {
  ...
  public String getPlainText(Element element) {
    ...
    NodeTraversor.traverse(
        formatter, element); // walk the DOM, and call .head() and .tail() for each node
  ...
  private static class FormattingVisitor implements NodeVisitor {
    ...
    private void append(String text) {
      if (text.startsWith("\n"))
        width =
            0; // reset counter if starts with a newline. only from formats above, not in natural
               // text
      if (text.equals(" ")
