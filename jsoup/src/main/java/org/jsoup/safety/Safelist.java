...
/**
 * Safe-lists define what HTML (elements and attributes) to allow through the cleaner. Everything
 * else is removed.
 *
 * <p>Start with one of the defaults:
 *
 * <ul>
 *   <li>{@link #none}
 *   <li>{@link #simpleText}
 ...
 * </ul>
 *
 * <p>If you need to allow more through (please be careful!), tweak a base safelist with:
 *
 * <ul>
 ...
 * </ul>
 *
 * <p>You can remove any setting from an existing safelist with:
 *
 * <ul>
 ...
 * </ul>
 *
 * <p>The cleaner and these safelists assume that you want to clean a <code>body</code> fragment of
 * HTML (to add user supplied HTML into a templated page), and not to clean a full HTML document. If
 * the latter is the case, either wrap the document HTML around the cleaned body HTML, or create a
 * safelist that allows <code>html</code> and <code>head</code> elements as appropriate.
 *
 * <p>If you are going to extend a safelist, please be very careful. Make sure you understand what
 * attributes may lead to XSS attack vectors. URL attributes are particularly vulnerable and require
 * careful validation. See the <a
 * href="https://owasp.org/www-community/xss-filter-evasion-cheatsheet">XSS Filter Evasion Cheat
 * Sheet</a> for some XSS attack examples (that jsoup will safegaurd against the default Cleaner and
 * Safelist configuration).
 */
public class Safelist {
  private static final String All = ":all";
  private final Set<TagName> tagNames; // tags allowed, lower case. e.g. [p, br, span]
  private final Map<TagName, Set<AttributeKey>>
      attributes; // tag -> attribute[]. allowed attributes [href] for a tag.
  private final Map<TagName, Map<AttributeKey, AttributeValue>>
      enforcedAttributes; // always set these attribute values
  private final Map<TagName, Map<AttributeKey, Set<Protocol>>>
      protocols; // allowed URL protocols for attributes
  private boolean preserveRelativeLinks; // option to preserve relative links

  /**
   * This safelist allows only text nodes: any HTML Element or any Node other than a TextNode will
   * be removed.
   *
   * <p>Note that the output of {@link org.jsoup.Jsoup#clean(String, Safelist)} is still <b>HTML</b>
   * even when using this Safelist, and so any HTML entities in the output will be appropriately
   * escaped. If you want plain text, not HTML, you should use a text method such as {@link
   * Element#text()} instead, after cleaning the document.
   *
   * <p>Example:
   *
   * <pre>{@code
  ...
  /**
   * This safelist allows only simple text formatting: <code>b, em, i, strong, u</code>. All other
   * HTML (tags and attributes) will be removed.
   *
   * @return safelist
   */
  ...
  /**
   * This safelist allows a fuller range of text nodes: <code>
   * a, b, blockquote, br, cite, code, dd, dl, dt, em, i, li,
   * ol, p, pre, q, small, span, strike, strong, sub, sup, u, ul</code>, and appropriate attributes.
   *
   * <p>Links (<code>a</code> elements) can point to <code>http, https, ftp, mailto</code>, and have
   * an enforced <code>rel=nofollow</code> attribute.
   *
   * <p>Does not allow images.
   *
   * @return safelist
   */
  ...
  /**
   * This safelist allows the same text tags as {@link #basic}, and also allows <code>img</code>
   * tags, with appropriate attributes, with <code>src</code> pointing to <code>http</code> or
   * <code>https</code>.
   *
   * @return safelist
   */
  ...
  /**
   * This safelist allows a full range of text and structural body HTML: <code>
   * a, b, blockquote, br, caption, cite,
   * code, col, colgroup, dd, div, dl, dt, em, h1, h2, h3, h4, h5, h6, i, img, li, ol, p, pre, q, small, span, strike, strong, sub,
   * sup, table, tbody, td, tfoot, th, thead, tr, u, ul</code>
   *
   * <p>Links do not have an enforced <code>rel=nofollow</code> attribute, but you can add that if
   * desired.
   *
   * @return safelist
   */
  ...
  /**
   * Create a new, empty safelist. Generally it will be better to start with a default prepared
   * safelist instead.
   *
   * @see #basic()
   * @see #basicWithImages()
  ...
  public Safelist(Safelist copy) {
    ...
    for (Map.Entry<TagName, Map<AttributeKey, AttributeValue>> enforcedEntry :
        copy.enforcedAttributes.entrySet()) {
      enforcedAttributes.put(enforcedEntry.getKey(), new HashMap<>(enforcedEntry.getValue()));
    }
    for (Map.Entry<TagName, Map<AttributeKey, Set<Protocol>>> protocolsEntry :
        copy.protocols.entrySet()) {
      Map<AttributeKey, Set<Protocol>> attributeProtocolsCopy = new HashMap<>();
      for (Map.Entry<AttributeKey, Set<Protocol>> attributeProtocols :
          protocolsEntry.getValue().entrySet()) {
        attributeProtocolsCopy.put(
            attributeProtocols.getKey(), new HashSet<>(attributeProtocols.getValue()));
      }
  ...
  /**
   * Add a list of allowed elements to a safelist. (If a tag is not allowed, it will be removed from
   * the HTML.)
   *
  ...
  /**
   * Remove a list of allowed elements from a safelist. (If a tag is not allowed, it will be removed
   * from the HTML.)
   *
  ...
  /**
   * Remove a list of allowed attributes from a tag. (If an attribute is not allowed on an element,
   * it will be removed.)
   *
   * <p>E.g.: <code>removeAttributes("a", "href", "class")</code> disallows <code>href</code> and
   * <code>class</code> attributes on <code>a</code> tags.
   *
   * <p>To make an attribute invalid for <b>all tags</b>, use the pseudo tag <code>:all</code>, e.g.
  ...
  public Safelist removeAttributes(String tag, String... attributes) {
    ...
    if (tagNames.contains(tagName)
        && this.attributes.containsKey(tagName)) { // Only look in sub-maps if tag was allowed
      Set<AttributeKey> currentSet = this.attributes.get(tagName);
      currentSet.removeAll(attributeSet);

      if (currentSet
          .isEmpty()) // Remove tag from attribute map if no attributes are allowed for tag
  ...
  public Safelist removeEnforcedAttribute(String tag, String attribute) {
    ...
    if (tagNames.contains(tagName) && enforcedAttributes.containsKey(tagName)) {
      ...
      if (attrMap
          .isEmpty()) // Remove tag from enforced attribute map if no enforced attributes are
                      // present
      enforcedAttributes.remove(tagName);
    }
    return this;
  ...
  /**
   * Configure this Safelist to preserve relative links in an element's URL attribute, or convert
   * them to absolute links. By default, this is <b>false</b>: URLs will be made absolute (e.g.
   * start with an allowed protocol, like e.g. {@code http://}.
   *
   * <p>Note that when handling relative links, the input document must have an appropriate {@code
   * base URI} set when parsing, so that the link's protocol can be confirmed. Regardless of the
   * setting of the {@code preserve relative links} option, the link must be resolvable against the
   * base URI to an allowed protocol; otherwise the attribute will be removed.
   *
   ...
   *
   * <p>To allow a link to an in-page URL anchor (i.e. <code>&lt;a href="#anchor"&gt;</code>, add a
   * <code>#</code>:<br>
  ...
  /**
   * Remove allowed URL protocols for an element's URL attribute. If you remove all protocols for an
   * attribute, that attribute will allow any protocol.
   *
   * <p>E.g.: <code>removeProtocols("a", "href", "ftp")</code>
   *
   * @param tag Tag the URL protocol is for
   * @param attribute Attribute name
  ...
  public Safelist removeProtocols(String tag, String attribute, String... removeProtocols) {
    ...
    // make sure that what we're removing actually exists; otherwise can open the tag to any data
    // and that can
    // be surprising
  ...
  private boolean testValidProtocol(Element el, Attribute attr, Set<Protocol> protocols) {
    ...
    if (value.length() == 0)
      value =
          attr
              .getValue(); // if it could not be made abs, run as-is to allow custom unknown
                           // protocols
