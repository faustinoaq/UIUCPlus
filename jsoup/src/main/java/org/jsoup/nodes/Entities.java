...
/**
 * HTML entities, and escape routines. Source: <a
 * href="http://www.w3.org/TR/html5/named-character-references.html#named-character-references">W3C
...
public class Entities {
  ...
  private static final HashMap<String, String> multipoints =
      new HashMap<>(); // name -> multiple character references
  ...
  public enum EscapeMode {
    ...
    // table of named references to their codepoints. sorted so we can binary search. built by
    // BuildEntities.
    private String[] nameKeys;
    private int[]
        codeVals; // limitation is the few references with multiple characters; those go into
                  // multipoints.
  ...
  public static String getByName(String name) {
    ...
    if (codepoint != empty) return new String(new int[] {codepoint}, 0, 1);
    return emptyName;
  }
  ...
  /**
   * HTML escape an input string, using the default settings (UTF-8, base entities). That is, {@code
   * <} is returned as {@code &lt;}
   *
   * @param string the un-escaped string to escape
   * @return the escaped string
  ...
  static void escape(
    ...
    for (int offset = 0; offset < length; offset += Character.charCount(codePoint)) {
      ...
      // surrogate pairs, split implementation for efficiency on single char common case (saves
      // creating strings, char[]):
      if (codePoint < Character.MIN_SUPPLEMENTARY_CODE_POINT) {
        ...
        switch (c) {
          ...
          case 0xA0:
            ...
            break;
          case '<':
            // escape when in character data or when in a xml attribute val or XML syntax; not
            // needed in html attr val
          ...
          case '"':
            ...
            break;
          // we escape ascii control <x20 (other than tab, line-feed, carriage return)  for XML
          // compliance (required) and HTML ease of reading (not required) -
  ...
  private static void appendEncoded(Appendable accum, EscapeMode escapeMode, int codePoint)
      throws IOException {
  ...
  private static boolean canEncode(
      final CoreCharset charset, final char c, final CharsetEncoder fallback) {
    // todo add more charset tests if impacted by Android's bad perf in canEncode
    switch (charset) {
      case ascii:
        return c < 0x80;
      case utf:
        return true; // real is:!(Character.isLowSurrogate(c) || Character.isHighSurrogate(c)); -
                     // but already check above
      default:
  ...
  enum CoreCharset {
    ...
    static CoreCharset byName(final String name) {
      if (name.equals("US-ASCII")) return ascii;
  ...
  private static void load(EscapeMode e, String pointsData, int size) {
    ...
    try {
      while (!reader.isEmpty()) {
        ...
        reader.advance();
        if (i % 2 == 0) reader.advance();
        final int cp1 = Integer.parseInt(reader.consumeToAny(codeDelims), codepointRadix);
        final char codeDelim = reader.current();
        if (codeDelim == ',') {
          final int cp2 = Integer.parseInt(reader.consumeTo(';'), codepointRadix);
          reader.advance();
        }
