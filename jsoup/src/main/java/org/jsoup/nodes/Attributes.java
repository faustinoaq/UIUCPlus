 ...
 *
 * <p>Attributes are treated as a map: there can be only one value associated with an attribute
 * key/name.
 *
 * <p>Attribute name and value comparisons are generally <b>case sensitive</b>. By default for HTML,
...
public class Attributes implements Iterable<Attribute>, Cloneable {
  // The Attributes object is only created on the first use of an attribute; the Element will just
  // have a null
  // Attribute slot otherwise
  protected static final String dataPrefix = "data-";
  // Indicates a jsoup internal key. Can't be set via HTML. (It could be set via accessor, but not
  // too worried about
  // that. Suppressed from list, iter.
  static final char InternalPrefix = '/';
  private static final int InitialCapacity =
      3; // sampling found mean count when attrs present = 1.49; 1.08 overall. 2.6:1 don't have any
         // attrs.
  ...
  Object[] vals =
      new Object
          [InitialCapacity]; // Genericish: all non-internal attribute values must be Strings and
                             // are cast on access.
  ...
  int indexOfKey(String key) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      else if (key == null) // introducing a new condition
      return i + 1;
    }
    return NotFound;
  }
  ...
  // we track boolean attributes as null in values - they're just keys. so returns empty for
  // consumers
   ...
   * @return the first matching attribute value if set; or empty string if not set (ora boolean
   *     attribute).
   */
  ...
  /**
   * Put an arbitrary user-data object by key. Will be treated as an internal attribute, so will not
   * be emitted in HTML.
   *
   ...
   * Internal attributes are excluded from the {@link #html()}, {@link #asList()}, and {@link
   * #iterator()} methods.
   *
   * @return size
   */
  ...
  public void addAll(Attributes incoming) {
    ...
    for (Attribute attr : incoming) {
      ...
      else add(attr.getKey(), attr.getValue());
    }
  }
  ...
  /**
   * Retrieves a filtered view of attributes that are HTML5 custom data attributes; that is,
   * attributes with keys starting with {@code data-}.
   *
  ...
  public String html() {
    ...
    try {
      html(
          sb,
          (new Document(""))
              .outputSettings()); // output settings a bit funky, but this html() seldom used
  ...
  final void html(final Appendable accum, final Document.OutputSettings out) throws IOException {
    ...
    for (int i = 0; i < sz; i++) {
      ...
      if (key != null) Attribute.htmlNoValidate(key, (String) vals[i], accum.append(' '), out);
    }
  }
   ...
   * Note that the order of the attributes does not impact this equality (as per the Map interface
   * equals()).
   *
  ...
  public boolean equals(@Nullable Object o) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      } else if (!val.equals(thatVal)) return false;
    }
    return true;
  }

  /**
   * Calculates the hashcode of these attributes, by iterating all attributes and summing their
   * hashcodes.
   *
  ...
  /**
   * Internal method. Removes duplicate attribute by name. Settings for case sensitivity of key
   * names.
   *
  ...
  public int deduplicate(ParseSettings settings) {
    ...
    for (int i = 0; i < keys.length; i++) {
      for (int j = i + 1; j < keys.length; j++) {
        ...
        if ((preserve && keys[i].equals(keys[j]))
            || (!preserve && keys[i].equalsIgnoreCase(keys[j]))) {
          dupes++;
          remove(j);
          j--;
        }
