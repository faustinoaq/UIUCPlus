...
/**
 * A Range object tracks the character positions in the original input source where a Node starts or
 * ends. If you want to track these positions, tracking must be enabled in the Parser with {@link
 * org.jsoup.parser.Parser#setTrackPosition(boolean)}.
 *
...
public class Range {
  ...
  /**
   * Creates a new Range with start and end Positions. Called by TreeBuilder when position tracking
   * is on.
   *
   ...
   *
   * @return true if this was tracked during parsing, false otherwise (and all fields will be {@code
   *     -1}).
   */
  ...
  /**
   * Gets a String presentation of this Range, in the format {@code
  ...
  /**
   * A Position object tracks the character position in the original input source where a Node
   * starts or ends. If you want to track these positions, tracking must be enabled in the Parser
   * with {@link org.jsoup.parser.Parser#setTrackPosition(boolean)}.
   *
   * @see Node#sourceRange()
   */
  public static class Position {
    ...
    /**
     * Gets the position index (0-based) of the original input source that this Position was read
     * at. This tracks the total number of characters read into the source at this position,
     * regardless of the number of preceeding lines.
     *
     ...
     * The cursor number resets to 1 on every new line.
     *
     ...
     *
     * @return true if this was tracked during parsing, false otherwise (and all fields will be
     *     {@code -1}).
     */
    public boolean isTracked() {
