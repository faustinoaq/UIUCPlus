...
public abstract class Node implements Cloneable {
  ...
  /**
   * Get the node name of this node. Use for debugging purposes and not logic switching (for that,
   * use instanceof).
   *
   * @return node name
   */
  ...
  /**
   * Get the normalized name of this node. For node types other than Element, this is the same as
   * {@link #nodeName()}. For an Element, will be the lower-cased tag name.
   *
  ...
  /**
   * Checks if this node has a parent. Nodes won't have parents if (e.g.) they are newly created and
   * not added as a child to an existing node, or if they are a {@link #shallowClone()}. In such
   * cases, {@link #parent()} will return {@code null}.
   *
  ...
  public int attributesSize() {
    // added so that we can test how many attributes exist without implicitly creating the
    // Attributes object
    return hasAttributes() ? attributes().size() : 0;
  }

  /**
   * Set an attribute (key=value). If the attribute already exists, it is replaced. The attribute
   * key comparison is <b>case insensitive</b>. The key will be set with case sensitivity as set in
   * the parser settings.
   *
   * @param attributeKey The attribute key.
  ...
  /**
   * Get the base URI that applies to this node. Will return an empty string if not defined. Used to
  ...
  /**
   * Get an absolute URL from a URL attribute that may be relative (such as an <code>&lt;a href&gt;
   * </code> or <code>&lt;img src&gt;</code>).
   *
  ...
  public String absUrl(String attributeKey) {
    ...
    if (!(hasAttributes()
        && attributes()
            .hasKeyIgnoreCase(
                attributeKey))) // not using hasAttr, so that we don't recurse down hasAttr->absUrl
    return "";
   ...
   * @return the child node at this index. Throws a {@code IndexOutOfBoundsException} if the index
   *     is out of bounds.
   */
  ...
  /**
   * Get this node's children. Presented as an unmodifiable list: new children can not be added, but
   * the child nodes themselves can be manipulated.
   *
  ...
  public List<Node> childNodes() {
    ...
    List<Node> rewrap =
        new ArrayList<>(
            children
                .size()); // wrapped so that looping and moving will not throw a CME as the source
                          // changes
  ...
  /**
   * Returns a deep copy of this node's children. Changes made to these nodes will not be reflected
  ...
  /**
   * Gets this node's parent node. Not overridable by extending classes, so useful if you really
   * just need the Node type.
   *
   * @return parent node; or null if no parent.
   */
   ...
   *
   * @param html HTML to wrap around this node, e.g. {@code <div class="head"></div>}. Can be
   *     arbitrarily deep. If the input HTML does not parse to a result starting with an Element,
  ...
  public Node wrap(String html) {
    ...
    Element context =
        parentNode != null && parentNode instanceof Element
            ? (Element) parentNode
   ...
   *
   * <p>For example, with the input html:
   *
   * <p>{@code <div>One <span>Two <b>Three</b></span></div>} Calling {@code element.unwrap()} on the
   * {@code span} element will result in the html:
   *
   * <p>{@code <div>One Two <b>Three</b></div>} and the {@code "Two "} {@link TextNode} being
   * returned.
   *
   * @return the first child of this node, after the node has been unwrapped. @{code Null} if the
  ...
  protected void addChildren(int index, Node... children) {
    ...
    boolean wasEmpty = childNodeSize() == 0; // <---- changed statement 20
    nodes.addAll(index, Arrays.asList(children)); // <---- changed statement 37
    if (wasEmpty) // <---- changed statement 23
    i = children.length;
    if (!(wasEmpty && children[0].siblingIndex == 0)) // skip reindexing if we just moved
    reindexChildren(index); // <---- changed statement 28
    while (i-- > 0) {
  ...
  /**
   * Retrieves this node's sibling nodes. Similar to {@link #childNodes() node.parent.childNodes()},
  ...
  /**
   * Gets the first child node of this node, or {@code null} if there is none. This could be any
   * Node type, such as an Element, TextNode, Comment, etc. Use {@link Element#firstElementChild()}
   ...
   * traversal. Nodes may be inspected, changed, added, replaced, or removed.
   *
  ...
  /**
   * Get the outer HTML of this node. For example, on a {@code p} element, may return {@code
   * <p>Para</p>}.
   *
   * @return outer HTML
   * @see Element#html()
  ...
  /**
   * Get the source range (start and end positions) in the original input source that this node was
   * parsed from. Position tracking must be enabled prior to parsing the content. For an Element,
   * this will be the positions of the start tag.
   *
   ...
   *
   * <p>For an node value equality check, see {@link #hasSameValue(Object)}
   *
  ...
  /**
   * Provides a hashCode for this Node, based on its object identity. Changes to the Node's content
   * will not impact the result.
   *
  ...
  /**
   * Check if this node has the same content as another node. A node is considered the same if its
   * name, attributes and content match the other node; particularly its position in the tree does
   * not influence its similarity.
   *
   * @param o other object to compare to
   * @return true if the content of this node is the same as the other
   */
  ...
  /**
   * Create a stand-alone, deep copy of this node, and all of its children. The cloned node will
   * have no siblings or parent node. As a stand-alone object, any changes made to the clone or any
   * of its children will not impact the original node.
   *
   * <p>The cloned node may be adopted into another Document or node structure using {@link
   ...
   */
  @SuppressWarnings(
      "MethodDoesntCallSuperMethod") // because it does call super.clone in doClone - analysis just
                                     // isn't following
  @Override
  public Node clone() {
  ...
  /**
   * Create a stand-alone, shallow copy of this node. None of its children (if any) will be cloned,
   * and it will have no parent or sibling nodes.
   *
