...
public class QueryParser {
  ...
  private void combinator(char combinator) {
    ...
    Evaluator
        currentEval; // the evaluator the new eval will be combined to. could be root, or rightmost
                     // or.
    ...
    if (evals.size() == 1) {
      ...
      if (rootEval instanceof CombiningEvaluator.Or && combinator != ',') {
        ...
        assert currentEval
            != null; // rightMost signature can return null (if none set), but always will have one
                     // by this point
      ...
      case '>':
        currentEval =
            new CombiningEvaluator.And(
                new StructuralEvaluator.ImmediateParent(currentEval), newEval);
        break;
      case ' ':
        currentEval =
            new CombiningEvaluator.And(new StructuralEvaluator.Parent(currentEval), newEval);
        break;
      case '+':
        currentEval =
            new CombiningEvaluator.And(
                new StructuralEvaluator.ImmediatePreviousSibling(currentEval), newEval);
        break;
      case '~':
        currentEval =
            new CombiningEvaluator.And(
                new StructuralEvaluator.PreviousSibling(currentEval), newEval);
        break;
      case ',':
  ...
  private void findElements() {
    ...
    else if (tq.matchesWord() || tq.matches("*|")) byTag();
    ...
    else if (tq.matches(":containsWholeText(")) containsWholeText(false);
    else if (tq.matches(":containsWholeOwnText(")) containsWholeText(true);
    else if (tq.matches(":containsData(")) containsData();
    else if (tq.matches(":matches(")) matches(false);
    else if (tq.matches(":matchesOwn(")) matches(true);
    else if (tq.matches(":matchesWholeText(")) matchesWholeText(false);
    else if (tq.matches(":matchesWholeOwnText(")) matchesWholeText(true);
    ...
    else if (tq.matchChomp(":matchText")) evals.add(new Evaluator.MatchText());
    else // unhandled
  ...
  private void byTag() {
    // todo - these aren't dealing perfectly with case sensitivity. For case sensitive parsers, we
    // should also make
    // the tag in the selector case-sensitive (and also attribute names). But for now, normalize
    // (lower-case) for
    ...
    if (tagName.startsWith("*|")) {
      ...
      evals.add(
          new CombiningEvaluator.Or(
              new Evaluator.Tag(plainTag), new Evaluator.TagEndsWith(tagName.replace("*|", ":"))));
    } else {
  ...
  private void cssNthChild(boolean backwards, boolean ofType) {
    ...
    if (ofType)
      if (backwards) evals.add(new Evaluator.IsNthLastOfType(a, b));
  ...
  private void contains(boolean own) {
    ...
    evals.add(
        own ? new Evaluator.ContainsOwnText(searchText) : new Evaluator.ContainsText(searchText));
  }
  ...
  private void not() {
    tq.consume(":not");
    String subQuery = tq.chompBalanced('(', ')');
    Validate.notBlank(subQuery, ":not(selector) subselect must not be empty");
