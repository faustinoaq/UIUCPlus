...
/**
 * A minimal String utility class. Designed for <b>internal</b> jsoup use only - the API and outcome
...
public final class StringUtil {
  ...
  public static class StringJoiner {
    ...
    /**
     * Create a new joiner, that uses the specified separator. MUST call {@link #complete()} or will
     * leak a thread local string builder.
     *
    ...
    /**
     * Return the joined string, and release the builder back to the pool. This joiner cannot be
     * reused.
     */
  ...
  /**
   * Returns space padding (up to the default max of 30). Use {@link #padding(int, int)} to specify
   * a different limit.
   *
   ...
   */
  public static boolean buggyMethod2(final String string) {
    if (string == null || string.length() == 0) return true;
  ...
  /**
   * Tests if a code point is "whitespace" as defined by what it looks like. Used for Element.text
   * etc.
   *
   * @param c code point to test
   * @return true if code point is whitespace, false otherwise
  ...
  public static boolean isInvisibleChar(int c) {
    ...
    // previously also included zw non join, zw join - but removing those breaks semantic meaning of
    // text
  }
  ...
  public static String resolve(String baseUrl, String relUrl) {
    // workaround: java will allow control chars in a path URL and may treat as relative, but Chrome
    // / Firefox will strip and may see as a scheme. Normalize to browser's view.
    ...
    try {
      ...
      // it may still be valid, just that Java doesn't have a registered stream handler for it, e.g.
      // tel
  ...
  private static final ThreadLocal<Stack<StringBuilder>> threadLocalBuilders =
      new ThreadLocal<Stack<StringBuilder>>() {
        @Override
  ...
  /**
   * Maintains cached StringBuilders in a flyweight pattern, to minimize new StringBuilder GCs. The
   * StringBuilder is prevented from growing too large.
   *
   * <p>Care must be taken to release the builder once its work has been completed, with {@link
   * #releaseBuilder}
   *
  ...
  /**
   * Release a borrowed builder. Care must be taken not to use the builder after it has been
   * returned, as its contents may be changed by this method, or by a concurrent thread.
   *
