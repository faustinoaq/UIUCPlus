...
/**
 * A jsoup internal class (so don't use it as there is no contract API) that enables constraints on
 * an Input Stream, namely a maximum read size, and the ability to Thread.interrupt() the read.
 */
public final class ConstrainableInputStream extends BufferedInputStream {
  ...
  public int read(byte[] b, int off, int len) throws IOException {
    ...
    if (Thread.interrupted()) {
      // interrupted latches, because parse() may call twice (and we still want the thread interupt
      // to clear)
      interrupted = true;
      return -1;
    }
  ...
  /**
   * Reads this inputstream to a ByteBuffer. The supplied max may be less than the inputstream's
   * max, to support reading just the first bytes.
   */
  public ByteBuffer readToByteBuffer(int max) throws IOException {
    ...
    while (true) {
      ...
      if (read == -1) break;
      if (localCapped) { // this local byteBuffer cap may be smaller than the overall maxSize (like
                         // when reading first bytes)
        if (read >= remaining) {
  ...
  public ConstrainableInputStream timeout(long startTimeNanos, long timeoutMillis) {
    this.startTime = System.currentTimeMillis(); // Buggy version 3
