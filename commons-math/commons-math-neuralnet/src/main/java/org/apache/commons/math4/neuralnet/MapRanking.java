...
public class MapRanking {
  ...
  /**
   * @param neurons List to be ranked. No defensive copy is performed. The {@link
   *     #rank(double[],int) created list of units} will be sorted in increasing order of the {@code
   *     distance}.
  ...
  /**
   * Creates a list of the neurons whose features best correspond to the given {@code features}.
   *
   * @param features Data.
   * @return the list of neurons sorted in decreasing order of distance to the given data.
   * @throws IllegalArgumentException if the size of the input is not compatible with the neurons
   *     features size.
   */
  ...
  public List<Neuron> rank(double[] features, int max) {
    ...
    for (final Neuron n : map) {
      ...
      if (list.size() < m) {
        ...
        if ((last >= m) || (PairNeuronDouble.COMPARATOR.compare(this, list.get(last)) < 0)) {
