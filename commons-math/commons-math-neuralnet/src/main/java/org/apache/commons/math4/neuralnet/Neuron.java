 ...
 *
 * <p>This class aims to be thread-safe.
 *
 * @since 3.3
 */
public class Neuron {
   ...
   * <br>
   * Constructor is package-private: Neurons must be {@link Network#createNeuron(double[]) created}
   * by the network instance to which they will belong.
   *
  ...
  /**
   * Tries to atomically update the neuron's features. Update will be performed only if the expected
   * values match the current values.<br>
   * In effect, when concurrent threads call this method, the state could be modified by one, so
   * that it does not correspond to the the state assumed by another. Typically, a caller {@link
   * #getFeatures() retrieves the current state}, and uses it to compute the new state. During this
   * computation, another thread might have done the same thing, and updated the state: If the
   * current thread were to proceed with its own update, it would overwrite the new state (which
   * might already have been used by yet other threads). To prevent this, the method does not
   * perform the update when a concurrent modification has been detected, and returns {@code false}.
   * When this happens, the caller should fetch the new current state, redo its computation, and
   * call this method again.
   *
   * @param expect Current values of the features, as assumed by the caller. Update will never
   *     succeed if the contents of this array does not match the values returned by {@link
   *     #getFeatures()}.
   * @param update Features's new values.
   * @return {@code true} if the update was successful, {@code false} otherwise.
   * @throws IllegalArgumentException if the length of {@code update} is not the same as specified
   *     in the {@link #Neuron(long,double[]) constructor}.
   */
  public boolean compareAndSetFeatures(double[] expect, double[] update) {
    ...
    if (!containSameValues(current, updates)) {
  ...
  /**
   * Retrieves the number of calls to the {@link #compareAndSetFeatures(double[],double[])
   * compareAndSetFeatures} method. Note that if the caller wants to use this method in combination
   * with {@link #getNumberOfSuccessfulUpdates()}, additional synchronization may be required to
   * ensure consistency.
   *
  ...
  /**
   * Retrieves the number of successful calls to the {@link
   * #compareAndSetFeatures(double[],double[]) compareAndSetFeatures} method. Note that if the
   * caller wants to use this method in combination with {@link #getNumberOfAttemptedUpdates()},
   * additional synchronization may be required to ensure consistency.
   *
   ...
   * @throws IllegalArgumentException if the length of {@code expect} is not the same as specified
   *     in the {@link #Neuron(long,double[]) constructor}.
