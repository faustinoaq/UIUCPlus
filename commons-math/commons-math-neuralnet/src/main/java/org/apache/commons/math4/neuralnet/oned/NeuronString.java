...
/**
 * Neural network with the topology of a one-dimensional line. Each neuron defines one point on the
 * line.
 *
 * @since 3.3
 */
public class NeuronString {
  ...
  /**
   * Mapping of the 1D coordinate to the neuron identifiers (attributed by the {@link #network}
   * instance).
   */
   ...
   *
   * @param wrap Whether to wrap the dimension (i.e the first and last neurons will be linked
   *     together).
  ...
  /**
   * Creates a one-dimensional network: Each neuron not located on the border of the mesh has two
   * neurons linked to it. <br>
   * The links are bi-directional. Neurons created successively are neighbours (i.e. there are links
   * between them). <br>
   ...
   * @param wrap Whether to wrap the dimension (i.e the first and last neurons will be linked
   *     together).
   * @param featureInit Arrays that will initialize the features sets of the network's neurons.
   * @throws IllegalArgumentException if {@code num < 2}.
   */
  ...
  /**
   * Retrieves the underlying network. A reference is returned (enabling, for example, the network
   * to be trained). This also implies that calling methods that modify the {@link Network} topology
   * may cause this class to become inconsistent.
   *
   * @return the network.
   */
