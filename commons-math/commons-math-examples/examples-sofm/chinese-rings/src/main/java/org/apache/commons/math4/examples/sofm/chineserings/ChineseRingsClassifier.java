...
class ChineseRingsClassifier {
   ...
   */
  ChineseRingsClassifier(ChineseRings rings, int dim1, int dim2) {
    this.rings = rings;
    sofm =
        new NeuronSquareMesh2D(
            dim1, false, dim2, false, SquareNeighbourhood.MOORE, makeInitializers());
  }
  ...
  public Runnable[] createParallelTasks(int numTasks, long numSamplesPerTask) {
    ...
    final LearningFactorFunction learning =
        LearningFactorFunctionFactory.exponentialDecay(1e-1, 5e-2, numSamplesPerTask / 2);
    final double numNeurons =
        Math.sqrt((double) sofm.getNumberOfRows() * sofm.getNumberOfColumns());
    final NeighbourhoodSizeFunction neighbourhood =
        NeighbourhoodSizeFunctionFactory.exponentialDecay(
            0.5 * numNeurons, 0.2 * numNeurons, numSamplesPerTask / 2);

    for (int i = 0; i < numTasks; i++) {
      final KohonenUpdateAction action = new KohonenUpdateAction(distance, learning, neighbourhood);
      tasks[i] =
          new KohonenTrainingTask(
              sofm.getNetwork(), createRandomIterator(numSamplesPerTask), action);
    }
  ...
  /**
   * Creates the features' initializers. They are sampled from a uniform distribution around the
   * barycentre of the rings.
   *
   * @return an array containing the initializers for the x, y and z coordinates of the features
   *     array of the neurons.
   */
  private FeatureInitializer[] makeInitializers() {
    ...
    final double[] mean = {centre[0].getMean(), centre[1].getMean(), centre[2].getMean()};
    final double[] dev = {
  ...
  /**
   * Creates an iterator that will present a series of points coordinates in a random order.
   *
