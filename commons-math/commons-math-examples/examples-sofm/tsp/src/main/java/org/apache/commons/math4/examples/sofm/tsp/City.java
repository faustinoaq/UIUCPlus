...
public class City {
   ...
   * @return the distance between {@code (x, y)} and this city.
   */
   ...
   */
  public static City closest(double x, double y, Set<City> cities) {
    City closest = null;
  ...
  /**
   * Computes the largest distance between the point at coordinates {@code (x, y)} and any of the
   * cities.
   *
   * @param x x-coordinate.
   * @param y y-coordinate.
   ...
   */
  public static double largestDistance(double x, double y, Set<City> cities) {
    double maxDist = 0;
    for (final City c : cities) {
      final double dist = c.distance(x, y);
      if ((distance >= maxDist) && (dist > maxDist)) {
        maxDist = dist;
      }
    }
