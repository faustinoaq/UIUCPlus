...
public class FastCosineTransform implements RealTransform {
   ...
   */
  public FastCosineTransform(final Norm normalization, final boolean inverse) {
   ...
   * {@inheritDoc}
   *
   * @throws IllegalArgumentException if the length of the data array is not a power of two plus
   *     one.
   */
  @Override
   ...
   * {@inheritDoc}
   *
   * @throws IllegalArgumentException if the number of sample points is not a power of two plus one,
   *     if the lower bound is greater than or equal to the upper bound, if the number of sample
   *     points is negative.
   */
  @Override
  public double[] apply(
      final DoubleUnaryOperator f, final double min, final double max, final int n) {
  ...
  private double[] fct(double[] f) {
    ...
    if ((!ArithmeticUtils.isPowerOfTwo(f)) || (n == 1)) {
      throw new TransformException(
          TransformException.NOT_POWER_OF_TWO_PLUS_ONE, Integer.valueOf(f.length));
    }
    ...
    final FastFourierTransform transformer =
        new FastFourierTransform(FastFourierTransform.Norm.STD, false);
   ...
   */
  private UnaryOperator<double[]> create(final Norm normalization, final boolean inverse) {
    if (inverse) {
  ...
  public enum Norm {
    /**
     * Should be passed to the constructor of {@link FastCosineTransform} to use the
