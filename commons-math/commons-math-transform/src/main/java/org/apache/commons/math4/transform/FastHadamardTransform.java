...
/**
 * <a href="http://www.archive.chipcenter.com/dsp/DSP000517F1.html">Fast Hadamard Transform</a>
 * (FHT).
 *
 * <p>The FHT can also transform integer vectors into integer vectors. However, this transform
 * cannot be inverted directly, due to a scaling factor that may lead to rational results (for
 * example, the inverse transform of integer vector (0, 1, 0, 1) is vector (1/2, -1/2, 0, 0).
 */
public class FastHadamardTransform implements RealTransform {
  ...
  /**
   * Returns the forward transform of the given data set. The integer transform cannot be inverted
   * directly, due to a scaling factor which may lead to double results.
   *
  ...
  /**
   * FHT uses only subtraction and addition. It requires {@code N * log2(N) = n * 2^n} additions.
   *
   ...
   *       <br>
   *       <em>(If I use [x][y] it always means [row-offset][column-offset] of a Matrix with n rows
   *       and m columns. Its entries go from M[0][0] to M[n][N])</em>
   ...
   *             column.
   *       </ul>
   *   <li>The computation of {@code D_top} and {@code D_bottom} are best understood with the above
   *       example (for {@code N = 8}).
  ...
  private double[] fht(double[] x) {
    ...
    if (!ArithmeticUtils.isPowerOfTwo(n)) {
      throw new TransformException(TransformException.NOT_POWER_OF_TWO, n);
    }
  ...
  /**
   * Returns the forward transform of the specified integer data set. FHT only uses subtraction and
   * addition.
   *
   * @param x Data to be transformed.
   * @return the transformed array.
