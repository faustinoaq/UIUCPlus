 ...
 *
 * <p>Caveat: At the time of implementation, the {@code AccurateMath} functions were often faster
 * and/or more accurate than their JDK equivalent. Nowadays, it would not be surprising that they
 * are always slower (due to the various JVM optimizations that have appeared since Java 5).
 * However, any change to this class should ensure that the current accuracy is not lost.
 *
 * <p>AccurateMath speed is achieved by relying heavily on optimizing compilers to native code
 * present in many JVMs today and use of large tables. The larger tables are lazily initialized on
 * first use, so that the setup time does not penalize methods that don't need them.
 *
 * <p>Note that AccurateMath is extensively used inside Apache Commons Math, so by calling some
 * algorithms, the overhead when the tables need to be initialized will occur regardless of the
 * end-user calling AccurateMath methods directly or not. Performance figures for a specific JVM and
 * hardware can be evaluated by running the AccurateMathTestPerformance tests in the test directory
 * of the source distribution.
 *
 * <p>AccurateMath accuracy should be mostly independent of the JVM as it relies only on IEEE-754
 * basic operations and on embedded tables. Almost all operations are accurate to about 0.5 ulp
 * throughout the domain range. This statement, of course is only a rough global observed behavior,
 * it is <em>not</em> a guarantee for <em>every</em> double numbers input (see William Kahan's <a
 * href="http://en.wikipedia.org/wiki/Rounding#The_table-maker.27s_dilemma">Table Maker's
 * Dilemma</a>).
 *
 * <p>AccurateMath implements the following methods not found in Math/StrictMath:
 *
 * <ul>
...
public final class AccurateMath {
   ...
   *
   * <p>This compile-time constant should be set to true only if one explicitly wants to compute the
   * tables at class loading time instead of using the already computed ones provided as literal
   * arrays below.
   */
  ...
  /**
   * Eighths. This is used by sinQ, because its faster to do a table lookup than a multiply in this
  ...
  /**
   * Compute the signum of a number. The signum is -1 for negative numbers, +1 for positive numbers
   * and 0 otherwise
   *
   * @param a number on which evaluation is done
  ...
  public static double pow(final double x, final double y) {
    ...
    if (y == 0) {
      ...
      if (yRawExp > 1085) {
        // y is either a very large integral value that does not fit in a long or it is a special
        // number
  ...
  private static class Split {
    ...
    Split(final double high, final double low) {
      this(
          high == 0.0
              ? (low == 0.0
                      && Double.doubleToRawLongBits(high) == Long.MIN_VALUE /* negative zero */
                  ? -0.0
                  : low)
              : high + low,
          high,
  ...
  /**
   * Compute cosine in the first quadrant by subtracting input from PI/2 and then calling sinQ. This
   * is more accurate as the input approaches PI/2.
   *
  ...
  /**
   * Reduce the input argument using the Payne and Hanek method. This is good for all inputs 0.0 < x
   * < inf Output is remainder after dividing by PI/2 The result array should contain 3 numbers.
   * result[0] is the integer portion, so mod 4 this gives the quadrant. result[1] is the upper bits
   * of the remainder result[2] is the lower bits of the remainder
   *
  ...
  private static void reducePayneHanek(double x, double[] result) {
    ...
    double tmpB =
        (((prod2A & 0xfffL) << 40) + (prod2B >>> 24)) / TWO_POWER_52 / TWO_POWER_52; // Low bits
   ...
   *
   * <p>If {@code direction} is greater than {@code d}, the smallest machine representable number
   * strictly greater than {@code d} is returned; if less, then the largest representable number
   * strictly less than {@code d} is returned.
   *
   * <p>If {@code d} is infinite and direction does not bring it back to finite numbers, it is
   * returned unchanged.
   *
   * @param d base number
   ...
   * <p>If arguments compare equal, then the second argument is returned.
   *
   * <p>If {@code direction} is greater than {@code f}, the smallest machine representable number
   * strictly greater than {@code f} is returned; if less, then the largest representable number
   * strictly less than {@code f} is returned.
   *
   * <p>If {@code f} is infinite and direction does not bring it back to finite numbers, it is
   * returned unchanged.
   *
   * @param f base number
   * @param direction (the only important thing is whether {@code direction} is greater or smaller
  ...
  /**
   * Get the whole number that is the nearest to x, or the even one if x is exactly half way between
   * two integers.
   *
   ...
   * mathematically equal to {@code x - y*n} where {@code n} is the mathematical integer closest to
   * the exact mathematical value of the quotient {@code x/y}. If two mathematical integers are
   ...
   * @param a dividend
   * @param b divisor
   * @return q such that a = q b + r with 0 &lt;= r &lt; b if b &gt; 0 and b &lt; r &lt;= 0 if b
   *     &lt; 0
  ...
  /**
   * Finds r such that a = q b + r with 0 &lt;= r &lt; b if b &gt; 0 and b &lt; r &lt;= 0 if b &lt;
   * 0.
   *
   ...
   * @param a dividend
   * @param b divisor
   * @return r such that a = q b + r with 0 &lt;= r &lt; b if b &gt; 0 and b &lt; r &lt;= 0 if b
   *     &lt; 0
   * @exception ArithmeticException if b == 0
  ...
  /**
   * Returns the first argument with the sign of the second argument. A NaN {@code sign} argument is
   * treated as positive.
   *
