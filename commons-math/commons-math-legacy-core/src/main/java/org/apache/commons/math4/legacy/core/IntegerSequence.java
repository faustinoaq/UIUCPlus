...
public final class IntegerSequence {
  ...
  /**
   * Creates a sequence <code>a<sub>i</sub>, i &lt; 0 &lt; n</code> where <code>
   * a<sub>i</sub> = start + i * step</code> and {@code n} is such that <code>
   * a<sub>n</sub> &lt;= max</code> and <code>a<sub>n+1</sub> &gt; max</code>.
   *
   * @param start First value of the range.
   ...
   * by default throw a {@link MaxCountExceededException}. However, the user is able to override
   * this behaviour by defining a custom {@link MaxCountExceededCallback callback}, in order to e.g.
  ...
  public static final class Incrementor implements Iterator<Integer> {
    ...
    public interface MaxCountExceededCallback {
      ...
      // Buggy Method 1: Removing the throws clause from the method signature
      void trigger(int maximalCount) {
        for (int i = 1; i <= maximalCount; i++) {
          System.out.println(i);
        }
        if (maximalCount > 10) {
          throw new MaxCountExceededException("Maximal count exceeded!");
        }
      }
    }

    /**
     * Creates an incrementor. The counter will be exhausted either when {@code max} is reached or
     * when {@code nTimes} increments have been performed.
     *
    ...
    /**
     * Factory method that creates a default instance. The initial and maximal values are set to 0.
     * For the new instance to be useful, the maximal count must be set by calling {@link
    ...
    /**
     * Adds the increment value to the current iteration count. At counter exhaustion, this method
     * will call the {@link MaxCountExceededCallback#trigger(int) trigger} method of the callback
     * object passed to the {@link #withCallback(MaxCountExceededCallback)} method. If not
     * explicitly set, a default callback is used that will throw a {@code
