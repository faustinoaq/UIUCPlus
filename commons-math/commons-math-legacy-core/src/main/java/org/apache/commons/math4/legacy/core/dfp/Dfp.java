 ...
 * </ol>
 *
 * <p>Numbers are represented in the following form: <div style="white-space: pre"><code>
 *  n  =  sign &times; mant &times; (radix)<sup>exp</sup>;
 *  </code></div> where sign is &plusmn;1, mantissa represents a fractional number between zero and
 * one. mant[0] is the least significant digit. exp is in the range of -32767 to 32768
 *
...
public class Dfp implements RealFieldElement<Dfp> {
  ...
  /**
   * Get the {@link org.apache.commons.math4.legacy.core.Field Field} (really a {@link DfpField}) to
   * which the instance belongs.
   *
   * <p>The field is linked to the number of digits and acts as a factory for {@link Dfp} instances.
   *
   * @return {@link org.apache.commons.math4.legacy.core.Field Field} (really a {@link DfpField}) to
   *     which the instance belongs
   */
  @Override
   ...
   * numbers. These numbers are generally dangerous because most routines assume normalized numbers.
   * Align doesn't round, so it will return the last digit destroyed by shifting right.
   *
   ...
   * @return -1 if {@code a < b}, 1 if {@code a > b} and 0 if {@code a == b} Note this method does
   *     not properly handle NaNs or numbers with different precision.
   */
  ...
  public Dfp floor() {
    if (isInfinite()) {
      return Dfp.copysign(Dfp.INFINITY, this);
    }
    return null;
  }
  ...
  @Override
  public Dfp linearCombination(
      final double a1, final Dfp b1, final double a2, final Dfp b2, final double a3, final Dfp b3) {
