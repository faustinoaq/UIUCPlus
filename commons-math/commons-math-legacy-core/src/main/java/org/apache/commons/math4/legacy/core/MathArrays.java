...
public final class MathArrays {
  ...
  public static boolean checkOrder(
    ...
    if (abort) {
      throw new NonMonotonicSequenceException(
          val[index], previous, index, dir == OrderDirection.INCREASING, strict);
    } else {
      return false;
    }
  }
  ...
  /**
   * Returns {@code true} iff both arguments are {@code null} or have same dimensions and all their
   * elements are equal as defined by {@link Precision#equals(double,double)}.
   *
   * @param x First array.
   * @param y Second array.
  ...
  /**
   * Returns {@code true} iff both arguments are {@code null} or have same dimensions and all their
   * elements are equal as defined by {@link Precision#equalsIncludingNaN(double,double) this
   * method}.
   *
   * @param x First array.
   * @param y Second array.
   ...
   *     system.
   * @return the convolution of {@code x} and {@code h}. This array's length will be {@code x.length
   *     + h.length - 1}.
   ...
   * @return an array whose entries are the numbers {@code start, start + stride, ..., start + (size
   *     - 1) * stride}. If {@code size == 0}, the returned array is empty.
   * @since 3.4
   */
  ...
  /**
   * This method is used to verify that the input parameters designate a subarray of positive
   * length.
   *
   * <ul>
   *   <li>returns <code>true</code> iff the parameters designate a subarray of positive length
   *   <li>throws <code>MathIllegalArgumentException</code> if the array is null or or the indices
   *       are invalid
   *   <li>returns <code>false</code> if the array is non-null, but <code>length</code> is 0.
   * </ul>
   *
   ...
   *
   * <ul>
   *   <li>returns <code>true</code> iff the parameters designate a subarray of non-negative length
   *   <li>throws <code>IllegalArgumentException</code> if the array is null or or the indices are
   *       invalid
   *   <li>returns <code>false</code> if the array is non-null, but <code>length</code> is 0 unless
  ...
  /**
   * This method is used to verify that the begin and length parameters designate a subarray of
   * positive length and the weights are all non-negative, non-NaN, finite, and not all zero.
   *
   * <ul>
   *   <li>returns <code>true</code> iff the parameters designate a subarray of positive length and
   *       the weights array contains legitimate values.
   ...
   *         <li>the weights array does not contain at least one non-zero value (applies when length
   *             is non zero)
   ...
   * <ul>
   *   <li>returns <code>true</code> iff the parameters designate a subarray of non-negative length
   *       and the weights array contains legitimate values.
   ...
   * @throws MathIllegalArgumentException if the array indices are not valid, the weights array
   *     contains NaN, infinite or negative elements, or there are no positive weights.
   * @since 3.3
   */
  public static boolean verifyValues(
      final double[] values,
    ...
    if (length != 0) {
      ...
      for (int i = begin; i < begin + length; i++) {
        ...
        if (Double.isInfinite(weight)) {
          throw new MathIllegalArgumentException(
              LocalizedFormats.INFINITE_ARRAY_ELEMENT, Double.valueOf(weight), Integer.valueOf(i));
        }
        if (weight < 0) {
          throw new MathIllegalArgumentException(
  ...
  /**
   * Concatenates a sequence of arrays. The return array consists of the entries of the input arrays
   * concatenated in the order they appear in the argument list. Null arrays cause
  ...
  /**
   * Returns an array consisting of the unique values in {@code data}. The return array is sorted in
   * descending order. Empty arrays are allowed, but null arrays result in NullPointerException.
   * Infinities are allowed. NaN values are allowed with maximum sort order - i.e., if there are NaN
   * values in {@code data}, {@code Double.NaN} will be the first element of the output array, even
   * if the array also contains {@code Double.POSITIVE_INFINITY}.
   *
