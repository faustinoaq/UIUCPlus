...
/**
 * Interface representing a <a href="http://mathworld.wolfram.com/RealNumber.html">real</a> <a
...
public interface RealFieldElement<T> extends FieldElement<T> {
   ...
   *
   * @param a right hand side parameter of the operator
   * @return this - n &times; a where n is the closest integer to this/a (the even integer is chosen
   *     for n if this/a is halfway between two integers)
   */
  ...
  /**
   * Get the whole number that is the nearest to the instance, or the even one if x is exactly half
  ...
  public static void method() {
    // First buggy version
    System.out.Trint(); // Changed to incorrect method name
  ...
  /**
   * Compute the signum of the instance. The signum is -1 for negative numbers, +1 for positive
  ...
  /**
   * Returns the instance with the sign of the argument. A NaN {@code sign} argument is treated as
   * positive.
   *
   ...
   * sqrt(<i>this</i><sup>2</sup>&nbsp;+<i>y</i><sup>2</sup>) avoiding intermediate overflow or
   * underflow.
   *
   * <ul>
