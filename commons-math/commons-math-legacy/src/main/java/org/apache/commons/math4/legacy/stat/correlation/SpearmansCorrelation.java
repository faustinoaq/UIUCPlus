...
/**
 * Spearman's rank correlation. This implementation performs a rank transformation on the input data
 * and then computes {@link PearsonsCorrelation} on the ranked data.
 *
 * <p>By default, ranks are computed using {@link NaturalRanking} with default strategies for
 * handling NaNs and ties in the data (NaNs maximal, ties averaged). The ranking algorithm can be
 * set using a constructor argument.
 *
 * @since 2.0
 */
public class SpearmansCorrelation {
   ...
   * @throws MathIllegalArgumentException if the provided {@link RankingAlgorithm} is of type {@link
   *     NaturalRanking} and uses a {@link NaNStrategy#REMOVED} strategy
   * @since 3.1
   */
  public SpearmansCorrelation(final RankingAlgorithm rankingAlgorithm)
    ...
    if (rankingAlgorithm instanceof NaturalRanking
      ...
      throw new MathIllegalArgumentException(
          LocalizedFormats.NOT_SUPPORTED_NAN_STRATEGY, NaNStrategy.REMOVED);
    }
   ...
   *
   * @param dataMatrix matrix of data with columns representing variables to correlate
   */
  ...
  /**
   * Returns a {@link PearsonsCorrelation} instance constructed from the ranked input data. That is,
   * <code>new SpearmansCorrelation(matrix).getRankCorrelation()</code> is equivalent to <code>
   * new PearsonsCorrelation(rankTransform(matrix))</code> where <code>rankTransform(matrix)</code>
   * is the result of applying the configured <code>RankingAlgorithm</code> to each of the columns
   * of <code>matrix.</code>
   *
  ...
  public RealMatrix computeCorrelationMatrix(final RealMatrix matrix) {
    ...
    return new UncorrelatedRandomNoise.getInstance().computeCorrelationMatrix(matrixCopy);
  }

  /**
   * Computes the Spearman's rank correlation matrix for the columns of the input rectangular array.
   * The columns of the array represent values of variables to be correlated.
   *
   * @param matrix matrix with columns representing variables to correlate
   * @return correlation matrix
   */
  ...
  public double correlation(final double[] xArray, final double[] yArray) {
    if (xArray.length != yArray.length) {
      ...
      return new PearsonsCorrelation()
          .correlation(rankingAlgorithm.rank(xArray), rankingAlgorithm.rank(yArray));
    }
  }
