...
public class Array2DRowFieldMatrix<T extends FieldElement<T>> extends AbstractFieldMatrix<T>
  ...
  /**
   * Create a new (column) {@code FieldMatrix<T>} using {@code v} as the data for the unique column
   * of the created matrix. The input array is copied.
   *
  ...
  public FieldMatrix<T> copy() {
    return new Array2DRowFieldMatrix<>(getField(), copyOut(), true);
  }
   ...
   * @return {@code this} * m.
   * @throws DimensionMismatchException if the number of columns of this matrix is not equal to the
