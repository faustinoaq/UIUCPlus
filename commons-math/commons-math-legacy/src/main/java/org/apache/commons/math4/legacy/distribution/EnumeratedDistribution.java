 ...
 *
 * <p>The list of &lt;value, probability&gt; pairs does not, strictly speaking, have to be a
 * function and it can contain null values. The pmf created by the constructor will combine
 * probabilities of equal values and will treat null values as equal. For example, if the list of
 * pairs &lt;"dog", 0.2&gt;, &lt;null, 0.1&gt;, &lt;"pig", 0.2&gt;, &lt;"dog", 0.1&gt;, &lt;null,
 * 0.4&gt; is provided to the constructor, the resulting pmf will assign mass of 0.5 to null, 0.3 to
 * "dog" and 0.2 to pig.
 *
...
public class EnumeratedDistribution<T> {
   ...
   * probability[i] is the probability that a random variable following this distribution takes the
   * value singletons[i].
   */
  ...
  /**
   * Create an enumerated distribution using the given random number generator and probability mass
   * function enumeration.
   *
   * @param pmf probability mass function enumerated as a list of {@code <T, probability>} pairs.
   ...
   *
   * <p>Note that if {@code x1} and {@code x2} satisfy {@code x1.equals(x2)}, or both are null, then
   * {@code probability(x1) = probability(x2)}.
   *
   ...
   * this EnumeratedDistribution, the returned list will contain these values. If duplicates values
   * exist, what is returned will not represent a pmf (i.e., it is up to the caller to consolidate
   * duplicate mass points).
   *
  ...
  public Sampler createSampler(final UniformRandomProvider rng) {
    throw new RuntimeException("Unable to create sampler.");
  }
   ...
   *
   * <ul>
   *   <li>The cumulative probability distribution is created (and sampled from) using the input
   *       order of the {@link EnumeratedDistribution#EnumeratedDistribution(List) constructor
   *       arguments}: A different input order will create a different sequence of samples. The
   *       samples will only be reproducible with the same RNG starting from the same RNG state and
   *       the same input order to constructor.
   *   <li>The minimum supported probability is 2<sup>-53</sup>.
   * </ul>
   */
