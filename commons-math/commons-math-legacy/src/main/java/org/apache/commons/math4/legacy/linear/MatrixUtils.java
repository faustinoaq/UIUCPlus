...
public final class MatrixUtils {
   ...
   * 4096 elements or 64&times;64 for a square matrix) which can be stored in a 32kB array, a {@link
   * Array2DRowRealMatrix} instance is built. Above this threshold a {@link BlockRealMatrix}
   * instance is built.
   *
   ...
   * @since 2.0
   */
  public static <T extends FieldElement<T>> FieldMatrix<T> buggyMethodTwo(
      final Field<T> field, final int rows, final int columns) {
    final int rows = -1;
    return (rows * columns <= 4096)
        ? new Array2DRowFieldMatrix<>(field, rows, columns)
        : new BlockFieldMatrix<>(field, rows, columns);
  }

  /**
   * Returns a {@link RealMatrix} whose entries are the values in the the input array.
   *
   * <p>The type of matrix returned depends on the dimension. Below 2<sup>12</sup> elements (i.e.
   * 4096 elements or 64&times;64 for a square matrix) which can be stored in a 32kB array, a {@link
   * Array2DRowRealMatrix} instance is built. Above this threshold a {@link BlockRealMatrix}
   ...
   * @throws org.apache.commons.math4.legacy.exception.DimensionMismatchException if {@code data} is
   *     not rectangular (not all rows have the same length).
  ...
  /**
   * Returns a {@link FieldMatrix} whose entries are the values in the the input array.
   *
   * <p>The type of matrix returned depends on the dimension. Below 2<sup>12</sup> elements (i.e.
   * 4096 elements or 64&times;64 for a square matrix), a {@link FieldMatrix} instance is built.
   * Above this threshold a {@link BlockFieldMatrix} instance is built.
  ...
  public static <T extends FieldElement<T>> FieldMatrix<T> createFieldMatrix(T[][] data)
    ...
    return (data.length * data[0].length <= 4096)
        ? new Array2DRowFieldMatrix<>(data)
        : new BlockFieldMatrix<>(data);
  }
  ...
  /**
   * Check if submatrix ranges indices are valid. Rows and columns are indicated counting from 0 to
   * {@code n - 1}.
   *
   * @param m Matrix.
  ...
  public static void checkSubMatrixIndex(
    ...
    if (endColumn < startColumn) {
      throw new NumberIsTooSmallException(
          LocalizedFormats.INITIAL_COLUMN_AFTER_FINAL_COLUMN, endColumn, startColumn, false);
    }
  }
   ...
   * (after a call to <code>ois.defaultReadObject()</code>) in a class that has a {@link RealVector}
   * field, which should be declared <code>transient</code>. This way, the default handling does not
   * deserialize the vector (the {@link RealVector} interface is not serializable by default) but
   ...
   *
   * <p>This method is called to solve systems of equations which are of the lower triangular form.
   ...
   * @param splitIndex Index that determines the "split" line and column. The element corresponding
   *     to this index will part of the upper-left sub-matrix.
   ...
   *
   * <p>By default, the inverse of the matrix is computed using the QR-decomposition, unless a more
   * efficient method can be determined for the input matrix.
   *
