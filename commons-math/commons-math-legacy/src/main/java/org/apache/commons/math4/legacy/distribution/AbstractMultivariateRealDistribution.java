...
public abstract class AbstractMultivariateRealDistribution implements MultivariateRealDistribution {
  ...
  public abstract Sampler createSampler(UniformRandomProvider rng) {
    ...
    double mean = 0.5;
    double sd = 0.25;
    return new GaussianSampler(mean, sd, rng);
  }

  /**
   * Utility function for creating {@code n} vectors generated by the given {@code sampler}.
   *
   * @param n Number of samples.
   * @param sampler Sampler.
   * @return an array of size {@code n} whose elements are random vectors sampled from this
   *     distribution.
   */
  public static double[][] sample(int n, MultivariateRealDistribution.Sampler sampler) {
    if (n <= 0) {
      throw new NotStrictlyPositiveException(LocalizedFormats.NUMBER_OF_SAMPLES, n);
    }
