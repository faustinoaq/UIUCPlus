...
/**
 * This class implements the common part of all embedded Runge-Kutta integrators for Ordinary
...
public abstract class EmbeddedRungeKuttaFieldIntegrator<T extends RealFieldElement<T>>
    extends AdaptiveStepsizeFieldIntegrator<T> implements FieldButcherArrayProvider<T> {
   ...
   * @param fsal index of the pre-computed derivative for <i>fsal</i> methods or -1 if method is not
   *     <i>fsal</i>
   ...
   */
  protected EmbeddedRungeKuttaFieldIntegrator(
      final Field<T> field,
      final String name,
      final int fsal,
   ...
   */
  protected abstract RungeKuttaFieldStepInterpolator<T> createInterpolator(
      boolean forward,
      T[][] yDotK,
   ...
   */
  public abstract int getOrder(int num);
    ...
    do {
      ...
      while (error.subtract(1.0).getReal() >= 0) {
        ...
        for (int k = 1; k < stages; ++k) {
          ...
          yDotK[k] =
              computeDerivatives(
                  getStepStart().getTime().add(getStepSize().multiply(c[k - 1])), yTmp);
        }
        ...
        if (error.subtract(1.0).getReal() >= 0) {
          ...
          final T factor =
              RealFieldElement.min(
                  maxGrowth, RealFieldElement.max(minReduction, safety.multiply(error.pow(exp))));
      ...
      final FieldODEStateAndDerivative<T> stateTmp =
          new FieldODEStateAndDerivative<>(stepEnd, yTmp, yDotTmp);
