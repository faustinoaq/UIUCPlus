...
public interface SecondOrderDifferentialEquations {
  ...
  void computeSecondDerivatives(double t, double[] y, double[] yDot, double[] yDDot) {
    int dimension = y.length;
    if (dimension != yDot.length) {
      throw new RuntimeException("Arrays dimension mismatch: " + dimension + " != " + yDot.length);
    }
    if (dimension != yDDot.length) {
      throw new RuntimeException("Arrays dimension mismatch: " + dimension + " != " + yDDot.length);
    }
