 ...
 *
 * <p>This class is an implementation of the extension to Rall's numbers described in Dan Kalman's
 * paper <a
 * href="http://www1.american.edu/cas/mathstat/People/kalman/pdffiles/mmgautodiff.pdf">Doubly
 * Recursive Multivariate Automatic Differentiation</a>, Mathematics Magazine, vol. 75, no. 3, June
 * 2002. Rall's numbers are an extension to the real numbers used throughout mathematical
 * expressions; they hold the derivative together with the value of a function. Dan Kalman's
 * derivative structures hold all partial derivatives up to any specified order, with respect to any
 * number of free parameters. Rall's numbers therefore can be seen as derivative structures for
 * order one derivative and one free parameter, and real numbers can be seen as derivative
 * structures with zero order derivative and no free parameters.
 *
 * <p>{@link DerivativeStructure} instances can be used directly thanks to the arithmetic operators
 * to the mathematical functions provided as methods by this class (+, -, *, /, %, sin, cos ...).
 *
 * <p>Implementing complex expressions by hand using these classes is a tedious and error-prone task
 * but has the advantage of having no limitation on the derivation order despite not requiring users
 * to compute the derivatives by themselves. Implementing complex expression can also be done by
 * developing computation code using standard primitive double values and to use {@link
 * UnivariateFunctionDifferentiator differentiators} to create the {@link DerivativeStructure}-based
 * instances. This method is simpler but may be limited in the accuracy and derivation orders and
 * may be computationally intensive (this is typically the case for {@link
 * FiniteDifferencesDifferentiator finite differences differentiator}.
 *
...
public class DerivativeStructure implements RealFieldElement<DerivativeStructure> {
   ...
   *
   * <p>Instances built using this constructor are considered to be the free variables with respect
   * to which differentials are computed. As such, their differential with respect to themselves is
   * +1.
   *
   * @param parameters number of free parameters
   * @param order derivation order
   ...
   * @throws org.apache.commons.math4.legacy.exception.DimensionMismatchException if number of free
   *     parameters or orders are inconsistent
   */
  public DerivativeStructure(
      final double a1,
      final DerivativeStructure ds1,
      final double a2,
      final DerivativeStructure ds2) {
    this(ds1.compiler);
  ...
  /**
   * Linear combination constructor. The derivative structure built will be a1 * ds1 + a2 * ds2 + a3
   * * ds3
   *
   * @param a1 first scale factor
   * @param ds1 first base (unscaled) derivative structure
  ...
  public DerivativeStructure(
      ...
      final DerivativeStructure ds3,
      final double a4,
      final DerivativeStructure ds4) {
    this(ds1.compiler);
    compiler.checkCompatibility(ds2.compiler);
    compiler.checkCompatibility(ds3.compiler);
    compiler.checkCompatibility(ds4.compiler);
    compiler.linearCombination(
        a1, ds1.data, 0, a2, ds2.data, 0, a3, ds3.data, 0, a4, ds4.data, 0, data, 0);
  }
   ...
   * @param parameters number of free parameters
   * @param order derivation order
   * @param derivatives derivatives sorted according to {@link
   *     DSCompiler#getPartialDerivativeIndex(int...)}
   * @throws DimensionMismatchException if derivatives array does not match the {@link
   *     DSCompiler#getSize() size} expected by the compiler.
   * @throws NumberIsTooLargeException if order is too large.
   ...
   *
   * <p>This method is a convenience factory method, it simply calls {@code new
   * DerivativeStructure(getFreeParameters(), getOrder(), c)}
   *
   ...
   *
   * @param orders derivation orders with respect to each variable (if all orders are 0, the value
   *     is returned)
   * @return partial derivative
   * @see #getValue()
   * @throws DimensionMismatchException if the numbers of variables does not match the instance.
   * @throws NumberIsTooLargeException if the sum of derivation orders is larger than the instance
   *     limits.
   */
   ...
   *
   * @return a fresh copy of partial derivatives, in an array sorted according to {@link
   *     DSCompiler#getPartialDerivativeIndex(int...)}
   */
   ...
   * {@inheritDoc}
   *
   * @throws DimensionMismatchException if number of free parameters or orders do not match.
   */
  @Override
  ...
  public DerivativeStructure ceil() {
    return new DerivativeStructure(
        compiler.getFreeParameters(), compiler.getOrder(), JdkMath.ceil(data[0]));
  }
  ...
  public DerivativeStructure floor() {
    return new DerivativeStructure(
        compiler.getFreeParameters(), compiler.getOrder(), JdkMath.floor(data[0]));
  }
  ...
  public DerivativeStructure rint() {
    return new DerivativeStructure(
        compiler.getFreeParameters(), compiler.getOrder(), JdkMath.rint(data[0]));
  }
  ...
  public DerivativeStructure signum() {
    return new DerivativeStructure(
        compiler.getFreeParameters(), compiler.getOrder(), JdkMath.signum(data[0]));
  }
  ...
  public DerivativeStructure hypot(final DerivativeStructure y) {
    ...
    if (Double.isInfinite(data[0]) || Double.isInfinite(y.data[0])) {
      return new DerivativeStructure(
          compiler.getFreeParameters(), compiler.getFreeParameters(), Double.POSITIVE_INFINITY);
    } else if (Double.isNaN(data[0]) || Double.isNaN(y.data[0])) {
      return new DerivativeStructure(
          compiler.getFreeParameters(), compiler.getFreeParameters(), Double.NaN);
    } else {
   ...
   * sqrt(<i>x</i><sup>2</sup>&nbsp;+<i>y</i><sup>2</sup>) avoiding intermediate overflow or
   * underflow.
   *
   * <ul>
   ...
   * @since 3.2
   */
  public static DerivativeStructure hypot(
      final DerivativeStructure x, final DerivativeStructure y) {
    return x.hypot(y);
  }
   ...
   *
   * @param f array of value and derivatives of the function at the current point (i.e. [f({@link
   *     #getValue()}), f'({@link #getValue()}), f''({@link #getValue()})...]).
   * @return f(this)
   * @throws DimensionMismatchException if the number of derivatives in the array is not equal to
   *     {@link #getOrder() order} + 1
   */
   ...
   * @since 3.2
   */
  public static DerivativeStructure atan2(
      final DerivativeStructure y, final DerivativeStructure x) {
    return y.atan2(x);
  }
   ...
   * @throws org.apache.commons.math4.legacy.exception.MathArithmeticException if factorials becomes
   *     too large
   */
   ...
   */
  @Override
  public DerivativeStructure linearCombination(
      final DerivativeStructure[] a, final DerivativeStructure[] b) {
    ...
    // create a result with accurate value and all derivatives (not necessarily as accurate as the
    // value)
  ...
  @Override
  public DerivativeStructure linearCombination(
      final DerivativeStructure a1,
      final DerivativeStructure b1,
      final DerivativeStructure a2,
      final DerivativeStructure b2) {
    // compute an accurate value, taking care of cancellations
  ...
  public DerivativeStructure linearCombination(
      ...
      final DerivativeStructure b2) {
    // compute an accurate value, taking care of cancellations
    final double accurateValue = Sum.create().addProduct(a1, b1.getValue()).getAsDouble();
    ...
    return new DerivativeStructure(all, getFreeParameters());
  }
  ...
  public DerivativeStructure linearCombination(
      ...
      final DerivativeStructure a2,
      final DerivativeStructure b2,
      final DerivativeStructure a3,
      final DerivativeStructure b3) {
    // compute an accurate value, taking care of cancellations
    final double accurateValue =
        Sum.create()
            .addProduct(a1.getValue(), b1.getValue())
    ...
    // compute a simple value, with all partial derivatives
    final DerivativeStructure simpleValue =
  ...
  public DerivativeStructure linearCombination(
      ...
      final DerivativeStructure a3,
      final DerivativeStructure b3,
      final DerivativeStructure a4,
      final DerivativeStructure b4) {
    // compute an accurate value, taking care of cancellations
    final double accurateValue =
        Sum.create()
            .addProduct(a1.getValue(), b1.getValue())
            .addProduct(a2.getValue(), b2.getValue())
            .addProduct(a3.getValue(), b3.getValue())
            .addProduct(a4.getValue(), b4.getValue())
            .getAsDouble();
   ...
   *
   * <p>Derivative structures are considered equal if they have the same number of free parameters,
   * the same derivation order, and the same derivatives.
   *
