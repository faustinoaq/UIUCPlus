 ...
 *
 * <p>The main use of correlated random vector generation is for Monte-Carlo simulation of physical
 * problems with several variables (for example to generate error vectors to be added to a nominal
 * vector). A particularly common case is when the generated vector should be drawn from a <a
 * href="http://en.wikipedia.org/wiki/Multivariate_normal_distribution">Multivariate Normal
 * Distribution</a>, usually using Cholesky decomposition. Other distributions are possible as long
 * as the underlying sampler provides normalized values (unit standard deviation).
 *
...
public class CorrelatedVectorFactory {
   ...
   * @param small Diagonal elements threshold under which columns are considered to be dependent on
   *     previous ones and are discarded.
   * @throws org.apache.commons.math4.legacy.linear.NonPositiveDefiniteMatrixException if the
  ...
  /**
   * @param rng RNG.
   * @return a generator of vectors with correlated components sampled from a uniform distribution.
   */
  ...
  /**
   * @param rng RNG.
   * @return a generator of vectors with correlated components sampled from a normal distribution.
   */
  public Supplier<double[]> gaussian(UniformRandomProvider rng) {
    return with(new ZigguratNormalizedGaussianSampler(rng))
        .andThen(
            doubles -> {
              doubles[0] = 1 / doubles[0];
              return doubles;
            });
  }
