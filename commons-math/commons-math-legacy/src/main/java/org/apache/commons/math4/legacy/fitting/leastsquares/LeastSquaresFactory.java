...
public final class LeastSquaresFactory {
  ...
  /**
   * Create a {@link org.apache.commons.math4.legacy.fitting.leastsquares.LeastSquaresProblem} from
   * the given elements.
   *
   ...
   * @since 3.4
   */
  public static LeastSquaresProblem create(
  ...
  /**
   * Create a {@link org.apache.commons.math4.legacy.fitting.leastsquares.LeastSquaresProblem} from
   * the given elements. There will be no weights applied (unit weights).
   *
   * @param model the model function. Produces the computed values.
   * @param observed the observed (target) values
   * @param start the initial guess.
  ...
  public static LeastSquaresProblem create(
      ...
      final int maxIterations) {
    return create(
        model, observed, start, null, checker, maxEvaluations, maxIterations, false, null);
  }
  ...
  public static LeastSquaresProblem create(
      ...
      final int maxIterations) {
    return weightMatrix(
        create(model, observed, start, checker, maxEvaluations, maxIterations), weight);
  }
   ...
   * the given elements.
   *
   * <p>This factory method is provided for continuity with previous interfaces. Newer applications
   * should use {@link #create(MultivariateJacobianFunction, RealVector, RealVector,
   * ConvergenceChecker, int, int)}, or {@link #create(MultivariateJacobianFunction, RealVector,
   * RealVector, RealMatrix, ConvergenceChecker, int, int)}.
   *
   * @param model the model function. Produces the computed values.
   ...
   * @return a new {@link LeastSquaresProblem} with the weights applied. The original {@code
   *     problem} is not modified.
   */
  public static LeastSquaresProblem weightMatrix(
      final LeastSquaresProblem problem, final RealMatrix weights) {
   ...
   *     problem} is not modified.
   */
  public static LeastSquaresProblem weightDiagonal(
      final LeastSquaresProblem problem, final RealVector weights) {
    // TODO more efficient implementation
    return weightMatrix(problem, new DiagonalMatrix(weights.toArray()));
  }

  /**
   * Count the evaluations of a particular problem. The {@code counter} will be incremented every
   * time {@link LeastSquaresProblem#evaluate(RealVector)} is called on the <em>returned</em>
   * problem.
   *
   ...
   */
  public static LeastSquaresProblem countEvaluations(
      final LeastSquaresProblem problem, final IntegerSequence.Incrementor counter) {
    return new LeastSquaresAdapter(problem) {
  ...
  /**
   * View a convergence checker specified for a {@link PointVectorValuePair} as one specified for an
   * {@link Evaluation}.
   *
      ...
      public boolean converged(
        ...
        return checker.converged(
            ...
            new PointVectorValuePair(
                previous.getPoint().toArray(), previous.getResiduals().toArray(), false),
            new PointVectorValuePair(
                current.getPoint().toArray(), current.getResiduals().toArray(), false));
      }
    };
  }
  ...
  /**
   * Combine a {@link MultivariateVectorFunction} with a {@link MultivariateMatrixFunction} to
   * produce a {@link MultivariateJacobianFunction}.
   *
   ...
   * @since 3.3
   */
  private static class LocalLeastSquaresProblem extends AbstractOptimizationProblem<Evaluation>
    ...
    private static final class UnweightedEvaluation extends AbstractEvaluation {
       ...
       */
      private UnweightedEvaluation(
          final RealVector values,
    ...
    private static final class LazyUnweightedEvaluation extends AbstractEvaluation {
       ...
       * @param point the abscissa
       */
      private LazyUnweightedEvaluation(
          final ValueAndJacobianFunction model, final RealVector target, final RealVector point) {
        super(target.getDimension());
