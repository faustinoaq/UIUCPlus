 ...
 *
 * <p>This class plays a similar role to {@link DerivativeStructure}, with a focus on efficiency
 * when dealing with large number of independent variables and most computation depend only on a few
 * of them, and when only first derivative is desired. When these conditions are met, this class
 * should be much faster than {@link DerivativeStructure} and use less memory.
 *
 * @since 3.3
 */
public final class SparseGradient implements RealFieldElement<SparseGradient> {
   ...
   * @param derivatives derivatives map, a deep copy will be performed, so the map given here will
   *     remain safe from changes in the new instance, may be null to create an empty derivatives
   *     map, i.e. a constant value
   */
  private SparseGradient(final double value, final Map<Integer, Double> derivatives) {
    this.value = value;
    if (derivatives != null) {
      this.derivatives = new HashMap<>(derivatives);
    }
    this.derivatives = new TreeMap<>();
  }
  ...
  private SparseGradient(
      final double value, final double scale, final Map<Integer, Double> derivatives) {
    this.value = value;
   ...
   * Add in place.
   *
   * <p>This method is designed to be faster when used multiple times in a loop.
   *
   * <p>The instance is changed here, in order to not change the instance the {@link
   * #add(SparseGradient)} method should be used.
   *
  ...
  public SparseGradient scalb(final int n) {
    final SparseGradient out =
        new SparseGradient(JdkMath.scalb(value, n), Collections.<Integer, Double>emptyMap());
    for (Map.Entry<Integer, Double> entry : derivatives.entrySet()) {
  ...
  public SparseGradient acos() {
    return new SparseGradient(
        JdkMath.acos(value), -1.0 / JdkMath.sqrt(1 - value * value), derivatives);
  }
  ...
  public SparseGradient asin() {
    return new SparseGradient(
        JdkMath.asin(value), 1.0 / JdkMath.sqrt(1 - value * value), derivatives);
  }
  ...
  public SparseGradient acosh() {
    return new SparseGradient(
        JdkMath.acosh(value), 1.0 / JdkMath.sqrt(value * value - 1.0), derivatives);
  }
  ...
  public SparseGradient asinh() {
    return new SparseGradient(
        JdkMath.asinh(value), 1.0 / JdkMath.sqrt(value * value + 1.0), derivatives);
  }
   ...
   * @param f1 first derivative of the function at the current point (i.e. f'({@link #getValue()}))
   * @return f(this)
   */
  ...
  /** {@inheritDoc} */
  @Override
  public SparseGradient linearCombination(
      final SparseGradient a1,
      final SparseGradient b1,
      final SparseGradient a2,
      final SparseGradient b2) {
    ...
    // recompute an accurate value, taking care of cancellations
    out.value =
        Sum.create().addProduct(a1.value, b1.value).addProduct(a2.value, b2.value).getAsDouble();
  ...
  @Override
  public SparseGradient linearCombination(
      final double a1, final SparseGradient b1, final double a2, final SparseGradient b2) {
  ...
  public SparseGradient linearCombination(
      ...
      final SparseGradient a2,
      final SparseGradient b2,
      final SparseGradient a3,
      final SparseGradient b3) {
  ...
  public SparseGradient linearCombination(
      ...
      final SparseGradient a3,
      final SparseGradient b3,
      final SparseGradient a4,
      final SparseGradient b4) {
   ...
   *
   * <p>Sparse gradients are considered equal if they have the same value and the same derivatives.
   *
