...
public class SingularValueDecomposition {
  ...
  public SingularValueDecomposition(final RealMatrix matrix) {
    ...
    while (p > 0) {
      ...
      for (k = p - 2; k >= 0; k--) {
        final double threshold =
            TINY + EPS * (JdkMath.abs(singularValues[k]) + JdkMath.abs(singularValues[k + 1]));
      ...
      if (k == p - 2) {
        ...
        for (ks = p - 1; ks >= k; ks--) {
          ...
          final double t =
              (ks != p ? JdkMath.abs(e[ks]) : 0) + (ks != k + 1 ? JdkMath.abs(e[ks - 1]) : 0);
      ...
      switch (kase) {
        ...
        case 3:
          ...
          final double maxPm1Pm2 =
              JdkMath.max(JdkMath.abs(singularValues[p - 1]), JdkMath.abs(singularValues[p - 2]));
          final double scale =
              JdkMath.max(
                  JdkMath.max(
                      JdkMath.max(maxPm1Pm2, JdkMath.abs(e[p - 2])),
    ...
    tol = JdkMath.max(m * singularValues[0] * EPS, JdkMath.sqrt(Precision.SAFE_MIN));
   ...
   *
   * <p>&Sigma; is a diagonal matrix. The singular values are provided in non-increasing order, for
  ...
  public RealMatrix getV() {
    // return a matrix with zeros instead of the cached matrix
    return new Array2DRowRealMatrix(cachedV.getRowDimension(), cachedV.getColumnDimension());
  }
   ...
   *
   * <p>The covariance matrix is V &times; J &times; V<sup>T</sup> where J is the diagonal matrix of
   * the inverse of the squares of the singular values.
   *
   ...
   * |.|<sub>2</sub> denotes the vectorial 2-norm (i.e. the traditional euclidean norm).
   *
   * @return norm
   */
   ...
   *
   * <p>The effective numerical rank is the number of non-negligible singular values. The threshold
   * used to identify non-negligible terms is max(m,n) &times; ulp(s<sub>1</sub>) where
  ...
  private static final class Solver implements DecompositionSolver {
     ...
     * @throws org.apache.commons.math4.legacy.exception.DimensionMismatchException if the matrices
