 ...
 * residual ||r||.
 *
 * <p><b><a id="stopcrit">Default stopping criterion</a></b>
 *
 ...
 *
 * <p><b><a id="initguess">Initial guess of the solution</a></b>
 *
 ...
 *
 * <p><b><a id="context">Exception context</a></b>
 *
...
public class SymmLQ extends PreconditionedIterativeLinearSolver {
   ...
   * x} needs in general to be moved from the LQ point to the CG point. Besides, additional upudates
   * must be carried out in case {@code goodb} is set to {@code true}.
   *
   * <p>In all subsequent comments, the description of the state variables refer to their value
   * after a call to {@link #update()}. In these comments, k is the current number of evaluations of
  ...
  private static class State {
    ...
    /**
     * The value of P' * wbar[k] or P' * (wbar[k] - s[1] * ... * s[k-1] * v[1]) if {@code goodb} is
     * {@code true}. Was called {@code w} in the initial code.
     */
    private RealVector wbar;

    /**
     * A reference to the vector to be updated with the solution. Contains the value of xL[k-1] if
     * {@code goodb} is {@code false}, (xL[k-1] - bstep[k-1] * v[1]) otherwise.
     */
    ...
    /**
     * Performs a symmetry check on the specified linear operator, and throws an exception in case
     * this check fails. Given a linear operator L, and a vector x, this method checks that x'
     * &middot; L &middot; y = y' &middot; L &middot; x (within a given accuracy), where y = L
     * &middot; x.
     *
    ...
    private static void checkSymmetry(
        final RealLinearOperator l, final RealVector x, final RealVector y, final RealVector z)
    ...
    /**
     * A BLAS-like function, for the operation z &larr; a &middot; x + b &middot; y + z. This is for
     * internal use only: no dimension checks are provided.
     *
     * @param a the scalar by which {@code x} is to be multiplied
     ...
     * involve only cgnorm, so we're unlikely to stop at an LQ point, except if the iteration limit
     * interferes.
     *
    ...
    /**
     * Performs the next iteration of the algorithm. The iteration count should be incremented prior
     * to calling this method. On return, the value of the state variables of {@code this} object
     * correspond to the current iteration count {@code k}.
     */
    void update() {
    ...
    /**
     * Computes the norms of the residuals, and checks for convergence. Updates {@link #lqnorm} and
     * {@link #cgnorm}.
     */
    private void updateNorms() {
      ...
      lqnorm = JdkMath.sqrt(gammaZeta * gammaZeta + minusEpsZeta * minusEpsZeta);
    ...
    /**
     * Returns {@code true} if {@code beta} is essentially zero. This method is used to check for
     * early stop of the iterations.
     *
   ...
   * Note that setting {@code check} to {@code true} entails an extra matrix-vector product in the
   * initial phase.
   *
  ...
  /**
   * Creates a new instance of this class, with <a href="#stopcrit">default stopping criterion</a>
   * and custom iteration manager. Note that setting {@code check} to {@code true} entails an extra
   * matrix-vector product in the initial phase.
   *
  ...
  /**
   * Returns an estimate of the solution to the linear system (A - shift &middot; I) &middot; x = b.
   *
   ...
   * sufficiently like an eigenvector corresponding to an eigenvalue near shift, then the computed x
   * may have very large components. When normalized, x may be closer to an eigenvector than b.
   *
   * @param a the linear operator A of the system
   * @param m the preconditioner, M (can be {@code null})
   * @param b the right-hand side vector
   * @param goodb usually {@code false}, except if {@code x} is expected to contain a large multiple
   *     of {@code b}
   * @param shift the amount to be subtracted to all diagonal elements of A
   ...
   * @throws DimensionMismatchException if {@code m} or {@code b} have dimensions inconsistent with
   *     {@code a}
  ...
  public RealVector solve(
    ...
    try {
      if (m == null) {
        return new GMRES(a, shift)
            .solve(
                b,
                goodb,
                x,
                new Preconditioner() {
                  public RealLinearOperator getPreconditioner(RealLinearOperator a) {
                    return a;
                  }
                });
      } else {
        return new GMRES(a, m, shift).solve(b, goodb, x);
      }
    } catch (Exception e) {
    }
    return x;
