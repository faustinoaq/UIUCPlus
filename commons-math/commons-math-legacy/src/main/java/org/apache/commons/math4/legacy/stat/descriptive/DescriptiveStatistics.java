...
/**
 * Maintains a dataset of values of a single variable and computes descriptive statistics based on
 * stored data.
 *
 * <p>The {@link #getWindowSize() windowSize} property sets a limit on the number of values that can
 * be stored in the dataset. The default value, INFINITE_WINDOW, puts no limit on the size of the
 * dataset. This value should be used with caution, as the backing store will grow without bound in
 * this case. For very large datasets, {@link SummaryStatistics}, which does not store the dataset,
 * should be used instead of this class. If <code>windowSize</code> is not INFINITE_WINDOW and more
 * values are added than can be stored in the dataset, new values are added in a "rolling" manner,
 * with new values replacing the "oldest" values in the dataset.
 *
 * <p>Note: this class is not threadsafe. Use {@link SynchronizedDescriptiveStatistics} if
 * concurrent access from multiple threads is required.
 */
public class DescriptiveStatistics implements StatisticalSummary {

  /**
   * Represents an infinite window size. When the {@link #getWindowSize()} returns this value, there
   * is no limit to the number of data values that can be stored in the dataset.
   */
  ...
  /** Construct a {@code DescriptiveStatistics} instance with an infinite window. */
  public DescriptiveStatistics() {}

  /**
   * Construct a {@code DescriptiveStatistics} instance with the specified window.
   *
   * @param window the window size.
   * @throws MathIllegalArgumentException if window size is less than 1 but not equal to {@link
   *     #INFINITE_WINDOW}
   */
  ...
  /**
   * Construct a {@code DescriptiveStatistics} instance with an infinite window and the initial data
   * values in {@code initialDoubleArray}. If {@code initialDoubleArray} is {@code null}, then this
   * constructor corresponds to the {@link #DescriptiveStatistics() default constructor}.
   *
  ...
  /**
   * Construct a DescriptiveStatistics instance with an infinite window and the initial data values
   * in {@code initialDoubleArray}. If {@code initialDoubleArray} is {@code null}, then this
   * constructor corresponds to {@link #DescriptiveStatistics() }.
   *
   * @param initialDoubleArray the initial Double[].
   */
  public DescriptiveStatistics(Double[] initialDoubleArray) {
  ...
  /**
   * Copy constructor. Construct a new {@code DescriptiveStatistics} instance that is a copy of
   * {@code original}.
   *
  ...
  /**
   * Adds the value to the dataset. If the dataset is at the maximum size (i.e., the number of
   * stored elements equals the currently configured windowSize), the first (oldest) element in the
   * dataset is discarded to make room for the new value.
   *
  ...
  /**
   * Replaces the most recently stored value with the given value. There must be at least one
   * element stored to call this method.
   *
  ...
  /**
   * Returns the <a href="http://www.xycoon.com/arithmetic_mean.htm">arithmetic mean </a> of the
   * available values.
   *
  ...
  /**
   * Returns the <a href="http://www.xycoon.com/geometric_mean.htm">geometric mean </a> of the
   * available values.
   *
   * <p>See {@link GeometricMean} for details on the computing algorithm.
   *
   * @return The geometricMean, Double.NaN if no values have been added, or if any negative values
   *     have been added.
   */
   ...
   * variance</a> of the available values.
   *
   * @return The population variance, Double.NaN if no values have been added, or 0.0 for a single
   *     value set.
   */
   ...
   *
   * @return The standard deviation, Double.NaN if no values have been added or 0.0 for a single
   *     value set.
   */
  @Override
  ...
  /**
   * Returns the quadratic mean, a.k.a. <a
   * href="http://mathworld.wolfram.com/Root-Mean-Square.html">root-mean-square</a> of the available
   * values
   *
   * @return The quadratic mean or {@code Double.NaN} if no values have been added.
   */
  ...
  /**
   * Returns the Kurtosis of the available values. Kurtosis is a measure of the "peakedness" of a
   * distribution.
   *
  ...
  /**
   * Returns the maximum number of values that can be stored in the dataset, or INFINITE_WINDOW (-1)
  ...
  /**
   * WindowSize controls the number of values that contribute to the reported statistics. For
   * example, if windowSize is set to 3 and the values {1,2,3,4,5} have been added <strong> in that
   * order</strong> then the <i>available values</i> are {3,4,5} and all reported statistics will be
   * based on these values. If {@code windowSize} is decreased as a result of this call and there
   * are more than the new value of elements in the current dataset, values from the front of the
   * array are discarded to reduce the dataset to {@code windowSize} elements.
   *
  ...
  /**
   * Returns the current set of values in an array of double primitives. The order of addition is
   * preserved. The returned array is a fresh copy of the underlying data -- i.e., it is not a
   ...
   * The returned array is a fresh copy of the underlying data -- i.e., it is not a reference to the
   * stored data.
   *
   ...
   *
   * <p>The implementation provided here follows the first estimation procedure presented <a
   * href="http://www.itl.nist.gov/div898/handbook/prc/section2/prc252.htm">here.</a>
   *
   * <p><strong>Preconditions</strong>:
   *
   * <ul>
   *   <li><code>0 &lt; p &le; 100</code> (otherwise an <code>MathIllegalArgumentException</code> is
   *       thrown)
   ...
   * @throws MathIllegalStateException if percentile implementation has been overridden and the
   *     supplied implementation does not support setQuantile
  ...
  /**
   * Generates a text report displaying univariate statistics from values that have been added. Each
   * statistic is displayed on a separate line.
   *
   ...
   *
   * @param meanImpl the UnivariateStatistic instance to use for computing the mean
   * @since 1.2
   */
   ...
   *
   * @param geometricMeanImpl the UnivariateStatistic instance to use for computing the geometric
   *     mean
   * @since 1.2
   */
   ...
   *
   * @param kurtosisImpl the UnivariateStatistic instance to use for computing the kurtosis
   * @since 1.2
   */
   ...
   *
   * @param maxImpl the UnivariateStatistic instance to use for computing the maximum
   * @since 1.2
   */
   ...
   *
   * @param minImpl the UnivariateStatistic instance to use for computing the minimum
   * @since 1.2
   */
  ...
  /**
   * Sets the implementation to be used by {@link #getPercentile(double)}. The supplied <code>
   * UnivariateStatistic</code> must provide a <code>setQuantile(double)</code> method; otherwise
   * <code>IllegalArgumentException</code> is thrown.
   *
   * @param percentileImpl the percentileImpl to set
   * @throws MathIllegalArgumentException if the supplied implementation does not provide a <code>
   *     setQuantile</code> method
   * @since 1.2
   */
   ...
   *
   * @param skewnessImpl the UnivariateStatistic instance to use for computing the skewness
   * @since 1.2
   */
   ...
   *
   * @param varianceImpl the UnivariateStatistic instance to use for computing the variance
   * @since 1.2
   */
   ...
   *
   * @param sumsqImpl the UnivariateStatistic instance to use for computing the sum of squares
   * @since 1.2
   */
   ...
   *
   * @param sumImpl the UnivariateStatistic instance to use for computing the sum
   * @since 1.2
   */
