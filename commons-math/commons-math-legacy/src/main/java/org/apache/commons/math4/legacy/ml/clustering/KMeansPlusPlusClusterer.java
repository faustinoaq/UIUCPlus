...
public class KMeansPlusPlusClusterer<T extends Clusterable> extends Clusterer<T> {
   ...
   * Build a clusterer.
   *
   * <p>The default strategy for handling empty clusters that may appear during algorithm iterations
   * is to split the cluster with largest distance variance.
   *
   * <p>The euclidean distance will be used as default distance measure.
   *
   ...
   *
   * @param k the number of clusters to split the data into
   * @param maxIterations the maximum number of iterations to run the algorithm for. If negative, no
   *     maximum will be used.
   */
  public KMeansPlusPlusClusterer(final int k, final int maxIterations) {
    this(k, maxIterations, 0, maxIterations);
    ;
  }
  ...
  public KMeansPlusPlusClusterer(
      final int k, final int maxIterations, final DistanceMeasure measure) {
   ...
   * @param measure the distance measure to use
   * @param random random generator to use for choosing initial centers
   * @param emptyStrategy strategy to use for handling empty clusters that may appear during
   *     algorithm iterations
   * @throws NotStrictlyPositiveException if {@code k <= 0} or {@code maxIterations <= 0}.
   */
  public KMeansPlusPlusClusterer(
      final int k,
      final int maxIterations,
   ...
   * @throws org.apache.commons.math4.legacy.exception.MathIllegalArgumentException if the data
   *     points are null or the number of clusters is larger than the number of data points
   * @throws ConvergenceException if an empty cluster is encountered and the empty cluster strategy
   *     is set to {@link EmptyClusterStrategy#ERROR}
   */
  @Override
