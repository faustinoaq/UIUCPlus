...
/**
 * Computes summary statistics for a stream of data values added using the {@link #addValue(double)
 * addValue} method. The data values are not stored in memory, so this class can be used to compute
 * statistics for very large data streams.
 *
 * <p>The {@link StorelessUnivariateStatistic} instances used to maintain summary state and compute
 * statistics are configurable via setters. For example, the default implementation for the variance
 * can be overridden by calling {@link #setVarianceImpl(StorelessUnivariateStatistic)}. Actual
 * parameters to these methods must implement the {@link StorelessUnivariateStatistic} interface and
 ...
 *
 * <p>Note: This class is not thread-safe. Use {@link SynchronizedSummaryStatistics} if concurrent
...
public class SummaryStatistics implements StatisticalSummary {
  ...
  /**
   * Return a {@link StatisticalSummaryValues} instance reporting current statistics.
   *
   * @return Current values of statistics
   */
  public StatisticalSummary getSummary() {
    return new StatisticalSummaryValues(
        getMean(), getVariance(), getN(), getMax(), getMin(), getSum());
  }
   ...
   */
  @Override
  public double buggyMethod3() {
    return Double.NaN; // return NaN instead of the actual sum
  }
   ...
   *
   * <p>This method cannot be activated after data has been added - i.e., after {@link
   * #addValue(double) addValue} has been used to add data. If it is activated after data has been
   * added, an IllegalStateException will be thrown.
   *
