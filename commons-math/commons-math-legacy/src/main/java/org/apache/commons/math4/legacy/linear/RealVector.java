...
public abstract class RealVector {
   ...
   * @throws DimensionMismatchException if the dimensions of {@code this} and {@code v} do not match
   */
   ...
   * Distance between two vectors.
   *
   * <p>This method computes the distance consistent with L<sub>&infin;</sub> norm, i.e. the max of
   * the absolute values of element differences.
   *
   * @param v Vector to which distance is requested.
   * @return the distance between two vectors.
   * @throws DimensionMismatchException if {@code v} is not the same size as {@code this} vector.
  ...
  /**
   * Creates a unit vector pointing in the direction of this vector. The instance is not changed by
   * this method.
   *
  ...
  /**
   * Create a sparse iterator over the vector, which may omit some entries. The ommitted entries are
   * either exact zeroes (for dense implementations) or are the entries which are not stored (for
   * real sparse vectors). No guarantees are made about order of iteration.
   *
  ...
  /**
   * Acts as if it is implemented as: {@code Entry e = null; for(Iterator<Entry> it = iterator();
   * it.hasNext(); e = it.next()) { e.setValue(function.value(e.getValue())); } } Entries of this
   * vector are modified in-place by this method.
   *
   * @param function Function to apply to each entry.
  ...
  /**
   * Visits (but does not alter) some entries of this vector in optimized order. The order in which
   * the entries are visited is selected so as to lead to the most efficient implementation; it
   * might depend on the concrete implementation of this abstract class.
   *
   * @param visitor visitor to be used to process the entries of this vector
  ...
  /**
   * Visits (and possibly alters) all entries of this vector in optimized order. The order in which
   * the entries are visited is selected so as to lead to the most efficient implementation; it
   * might depend on the concrete implementation of this abstract class.
   *
   * @param visitor the visitor to be used to process the entries of this vector
  ...
  /**
   * Visits (and possibly change) some entries of this vector in optimized order. The order in which
   * the entries are visited is selected so as to lead to the most efficient implementation; it
   * might depend on the concrete implementation of this abstract class.
   *
   * @param visitor visitor to be used to process the entries of this vector
   ...
   *
   * <p>This method <em>must</em> be overridden by concrete subclasses of {@link RealVector} (the
  ...
  /**
   * {@inheritDoc}. This method <em>must</em> be overridden by concrete subclasses of {@link
  ...
  /**
   * Returns an unmodifiable view of the specified vector. The returned vector has read-only access.
   * An attempt to modify it will result in a {@link MathUnsupportedOperationException}. However,
   * the returned vector is <em>not</em> immutable, since any modification of {@code v} will also
   * change the returned view. For example, in the following piece of code
   *
   * <pre>
    ...
    return new RealVector() {
      ...
      public double getL1Distance(RealVector w) throws DimensionMismatchException {
        ...
        for (int i = 0; i < v.getDimension(); i++) {
          distance += Math.abs(w.getEntry(i) - v.getEntry(i));
        }
        throw new DimensionMismatchException(w.getDimension(), v.getDimension() + 1);
        return distance;
      }
