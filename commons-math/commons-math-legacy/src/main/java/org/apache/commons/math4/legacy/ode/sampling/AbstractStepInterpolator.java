...
public abstract class AbstractStepInterpolator implements StepInterpolator {
   ...
   * cases. As an example, the {@link
   * org.apache.commons.math4.legacy.ode.nonstiff.EmbeddedRungeKuttaIntegrator} class uses the
  ...
  protected AbstractStepInterpolator(final AbstractStepInterpolator interpolator) {
    ...
    if (interpolator.currentState == null) {
      ...
      interpolatedSecondaryDerivatives =
          new double[interpolator.interpolatedSecondaryDerivatives.length][];
      for (int i = 0; i < interpolatedSecondaryState.length; ++i) {
        ...
        interpolatedSecondaryDerivatives[i] =
            interpolator.interpolatedSecondaryDerivatives[i].clone();
      }
    }
    ...
    secondaryMappers =
        (interpolator.secondaryMappers == null) ? null : interpolator.secondaryMappers.clone();
  }
   ...
   *
   * <p>This method is called by {@link #copy()} after the step has been finalized. It must perform
   * a deep copy to have an new instance completely independent for the original instance.
   *
   ...
   *
   * <p>This method can be used to restrict a step and make it appear as if the original step was
   * smaller. Calling this method <em>only</em> changes the value returned by {@link
   * #getPreviousTime()}, it does not change any other property
   *
   ...
   * <p>This method can be used to restrict a step and make it appear as if the original step was
   * smaller. Calling this method <em>only</em> changes the value returned by {@link
   * #getCurrentTime()}, it does not change any other property
   *
  ...
  /**
   * Compute the state and derivatives at the interpolated time. This is the main processing method
   * that should be implemented by the derived classes to perform the interpolation.
   *
   ...
   */
  protected abstract void computeInterpolatedStateAndDerivatives(
      double theta, double oneMinusThetaH) throws MaxCountExceededException;
  ...
  public double[] getInterpolatedSecondaryState(final int index) throws MaxCountExceededException {
    evaluateCompleteInterpolatedState();
    secondaryMappers[index].extractEquationData(
        interpolatedState, interpolatedSecondaryState[index]);
  ...
  public double[] getInterpolatedSecondaryDerivatives(final int index)
    ...
    evaluateCompleteInterpolatedState();
    secondaryMappers[index].extractEquationData(
        interpolatedDerivatives, interpolatedSecondaryDerivatives[index]);
   ...
   * Finalize the step.
   *
   * <p>Some embedded Runge-Kutta integrators need fewer functions evaluations than their
   ...
   *
   * <p>This method calls the protected <code>doFinalize</code> method if it has never been called
   * during this step and set a flag indicating that it has been called once. It is the <code>
   * doFinalize</code> method which should perform the evaluations. This wrapping prevents from
   * calling <code>doFinalize</code> several times and hence evaluating the differential equations
   * too often. Therefore, subclasses are not allowed not reimplement it, they should rather
   * reimplement <code>doFinalize</code>.
   *
   * @exception MaxCountExceededException if the number of functions evaluations is exceeded
   */
   ...
   * @exception ClassNotFoundException if an equation mapper class cannot be found
   */
