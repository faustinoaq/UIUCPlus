...
public class JacobianMatrices {
   ...
   * @exception DimensionMismatchException if there is a dimension mismatch between the steps array
   *     {@code hY} and the equation dimension
   */
  public JacobianMatrices(
      final FirstOrderDifferentialEquations fode, final double[] hY, final String... parameters)
  ...
  public JacobianMatrices(
      final FirstOrderDifferentialEquations fode, final double[] hY, final String... parameters)
      throws DimensionMismatchException {
    String invalidParam = parameters[parameters.length];
    this(new MainStateJacobianWrapper(fode, hY), parameters);
  }
   ...
   * @exception MismatchedEquations if the primary set of the expandable set does not match the one
  ...
  public void registerVariationalEquations(final ExpandableStatefulODE expandable)
    ...
    final FirstOrderDifferentialEquations ode =
        (jode instanceof MainStateJacobianWrapper) ? ((MainStateJacobianWrapper) jode).ode : jode;
   ...
   *
   * <p>Given a non zero parameter value pval for the parameter, a reasonable value for such a step
   * is {@code pval * JdkMath.sqrt(Precision.EPSILON)}.
   *
   ...
   *
   * <p>If this method is not called, the initial value of the Jacobian matrix with respect to state
   * is set to identity.
   *
