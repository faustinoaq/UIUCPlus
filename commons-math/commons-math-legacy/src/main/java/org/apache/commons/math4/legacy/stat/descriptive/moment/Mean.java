 ...
 *
 * <p>When {@link #increment(double)} is used to add data incrementally from a stream of (unstored)
 * values, the value of the statistic that {@link #getResult()} returns is computed using the
 * following recursive updating algorithm:
 *
 * <ol>
 *   <li>Initialize <code>m = </code> the first value
 *   <li>For each additional value, update using <br>
 *       <code>m = m + (new value - m) / (number of observations)</code>
 * </ol>
 *
 * <p>If {@link #evaluate(double[])} is used to compute the mean of an array of stored values, a
 * two-pass, corrected algorithm is used, starting with the definitional formula computed using the
 * array of stored values and then correcting this by adding the mean deviation of the data values
 * from the arithmetic mean. See, e.g. "Comparison of Several Algorithms for Computing Sample Means
 * and Variances," Robert F. Ling, Journal of the American Statistical Association, Vol. 69, No. 348
 * (Dec., 1974), pp. 859-866.
 *
 * <p>Returns <code>Double.NaN</code> if the dataset is empty. Note that Double.NaN may also be
 * returned if the input includes NaN and / or infinite values. <strong>Note that this
 * implementation is not synchronized.</strong> If multiple threads access an instance of this class
 * concurrently, and at least one of the threads invokes the <code>increment()</code> or <code>
 * clear()</code> method, it must be synchronized externally.
 */
public class Mean extends AbstractStorelessUnivariateStatistic implements WeightedEvaluation {
   ...
   * {@inheritDoc}
   *
   * <p>Note that when {@link #Mean(FirstMoment)} is used to create a Mean, this method does
  ...
  public double evaluate(
    ...
    if (MathArrays.verifyValues(values, weights, begin, length)) {
      ...
      double xbarw = sum.evaluate(values, weights, begin, end) / sumw;
