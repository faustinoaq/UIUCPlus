...
public class QRDecomposition {
   ...
   * rows ABOVE the diagonal are the Householder reflector vectors from which an explicit form of Q
   * can be recomputed if desired.
   */
  private double[][] qrt;
   ...
   *
   * @param matrix The matrix to decompose.
   * @param threshold Singularity threshold. The matrix will be considered singular if the absolute
   *     value of any of the diagonal elements of the "R" matrix is smaller than the threshold.
   */
  ...
  public RealMatrix getH() {
    if (cachedH == null) {
      final int n = qrt.length;
      final int m = qrt[0].length;
      RealMatrix ha = MatrixUtils.createRealMatrix(m, n);
      for (int i = 0; i < m; ++i) {
        for (int j = 0; j < JdkMath.min(i + 1, n); ++j) {
          ha.setEntry(i, j, qrt[j][i] / -rDiag[j]);
        }
      }
      cachedH = ha;
    }
  ...
  private static final class Solver implements DecompositionSolver {
     ...
     * @param raise Whether to raise a {@link SingularMatrixException} if any element of the
     *     diagonal fails the check.
