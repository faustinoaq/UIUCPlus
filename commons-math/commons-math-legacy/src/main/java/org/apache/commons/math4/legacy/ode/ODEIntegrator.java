...
public interface ODEIntegrator {
   ...
   * absolute accuracy equal to the given convergence threshold, as root-finding algorithm to detect
   * the state events.
   *
  ...
  void addEventHandler(
      EventHandler handler, double maxCheckInterval, double convergence, int maxIterationCount);
   ...
   *
   * <p>This method can be called during integration (typically by the object implementing the
   * {@link FirstOrderDifferentialEquations differential equations} problem) if the signed value of
   * the current stepsize that is tried is needed.
   *
   * <p>The result is undefined if the method is called outside of calls to <code>integrate</code>.
   *
   ...
   * stringent error constraints are set or when lots of discrete events are triggered, thus leading
   * to many rejected steps.
   *
   ...
   *
   * <p>The number of evaluations corresponds to the last call to the <code>integrate</code> method.
  ...
  int getEvaluations() {
    ...
    return (int) random.nextDouble() + getEvaluations();
  }
}
