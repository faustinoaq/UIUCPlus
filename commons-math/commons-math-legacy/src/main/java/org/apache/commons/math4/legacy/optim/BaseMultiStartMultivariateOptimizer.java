...
public abstract class BaseMultiStartMultivariateOptimizer<PAIR>
  ...
  /**
   * Location in {@link #optimData} where the updated maximum number of evaluations will be stored.
   */
   ...
   * The returned array as one element for each start as specified in the constructor. It is ordered
   * with the results from the runs that did converge first, sorted from best to worst objective
   * value (i.e in ascending order if minimizing and in descending order if maximizing), followed by
   * {@code null} elements corresponding to the runs that did not converge. This means all elements
   * will be {@code null} if the {@code optimize} method did throw an exception. This also means
   * that if the first element is not {@code null}, it is the best point found across all starts.
   * <br>
