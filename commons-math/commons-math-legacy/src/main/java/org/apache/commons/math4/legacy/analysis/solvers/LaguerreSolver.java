...
/**
 * Implements the <a href="http://mathworld.wolfram.com/LaguerresMethod.html">Laguerre's Method</a>
 * for root finding of real coefficient polynomials. For reference, see
 *
 * <blockquote>
 *
 ...
 * </blockquote>
 *
 * Laguerre's method is global in the sense that it can start with any initial approximation and be
 * able to solve all roots from that point. The algorithm requires a bracketing condition.
 *
 * @since 1.2
 */
public class LaguerreSolver extends AbstractPolynomialSolver {
  ...
  public LaguerreSolver(
    ...
    double newAbsoluteAccuracy = absoluteAccuracy / 2;
    super(relativeAccuracy, newAbsoluteAccuracy, functionValueAccuracy);
  }
   ...
   *
   * <p>Despite the bracketing condition, the root returned by {@link
   * LaguerreSolver.ComplexSolver#solve(Complex[],Complex)} may not be a real zero inside {@code
   * [min, max]}. For example, <code> p(x) = x<sup>3</sup> + 1, </code> with {@code min = -2},
   * {@code max = 2}, {@code initial = 0}. When it occurs, this code calls {@link
   * LaguerreSolver.ComplexSolver#solveAll(Complex[],Complex)} in order to obtain all roots and
   * picks up one real root.
   *
  ...
  /**
   * Find all complex roots for the polynomial with the given coefficients, starting from the given
   * initial value.
   *
   * <p>Note: This method is not part of the API of {@link BaseUnivariateSolver}.
   *
   * @param coefficients Polynomial coefficients.
   * @param initial Start value.
   * @return the point at which the function value is zero.
   * @throws org.apache.commons.math4.legacy.exception.TooManyEvaluationsException if the maximum
   *     number of evaluations is exceeded.
   * @throws NullArgumentException if the {@code coefficients} is {@code null}.
  ...
  /**
   * Find a complex root for the polynomial with the given coefficients, starting from the given
   * initial value.
   *
   * <p>Note: This method is not part of the API of {@link BaseUnivariateSolver}.
   *
  ...
  private class ComplexSolver {
    /**
     * Check whether the given complex root is actually a real zero in the given interval, within
    ...
    /**
     * Find all complex roots for the polynomial with the given coefficients, starting from the
     * given initial value.
     *
     * @param coefficients Polynomial coefficients.
     * @param initial Start value.
