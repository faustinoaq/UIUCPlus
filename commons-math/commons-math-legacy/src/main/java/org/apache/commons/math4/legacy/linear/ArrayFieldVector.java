...
public class ArrayFieldVector<T extends FieldElement<T>> implements FieldVector<T>, Serializable {
  ...
  /**
   * Build a 0-length vector. Zero-length vectors may be used to initialize construction of vectors
   * by data gathering. We start with zero-length and use either the {@link
   * #ArrayFieldVector(FieldVector, FieldVector)} constructor or one of the {@code append} methods
   * ({@link #add(FieldVector)} or {@link #append(ArrayFieldVector)}) to gather data into this
   * vector.
   *
   ...
   * performance as no new array will be built and no data will be copied. This constructor needs a
   * non-empty {@code d} array to retrieve the field from its first element. This implies it cannot
   ...
   * one non-empty array to retrieve the field from its first element. This implies it cannot build
   * 0 length vectors. To build vectors from any size, one should use the {@link
