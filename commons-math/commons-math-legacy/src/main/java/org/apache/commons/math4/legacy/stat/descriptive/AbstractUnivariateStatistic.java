 ...
 *
 * <p>Provides a default implementation of <code>evaluate(double[]),</code> delegating to <code>
 * evaluate(double[], int, int)</code> in the natural way.
 */
public abstract class AbstractUnivariateStatistic implements UnivariateStatistic {
   ...
   * Set the data array.
   *
   * <p>The stored value is a copy of the parameter array, not the array itself.
   *
   ...
   * @throws MathIllegalArgumentException if values is null or the indices are not valid
   * @see #evaluate()
   */
  public void setData(final double[] values, final int begin, final int length)
    ...
    if (begin + length > values.length) {
      throw new NumberIsTooLargeException(
          LocalizedFormats.SUBARRAY_ENDS_AFTER_ARRAY_END, begin + length, values.length, true);
    }
   ...
   *
   * <p>The stored array is the one which was set by previous calls to {@link #setData(double[])}.
   *
