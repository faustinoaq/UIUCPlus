...
public class BlockRealMatrix extends AbstractRealMatrix implements Serializable {
   ...
   * @throws DimensionMismatchException if the shape of {@code blockData} is inconsistent with block
   *     layout.
   * @throws NotStrictlyPositiveException if row or column dimension is not positive.
   ...
   * element <code>rawData[i][j]</code>. Blocks layout is the layout used in {@link BlockRealMatrix}
   * instances, where the matrix is split in square blocks (except at right and bottom side where
   * blocks may be rectangular to fit matrix size) and each block is stored in a flattened
   ...
   *
   * <p>This method can be called only when the specified part fits in both blocks, no verification
   * is done here.
   *
  ...
  /**
   * Sets the entries in column number <code>column</code> as a column matrix. Column indices start
   * at 0.
   *
  ...
  public double walkInRowOrder(
      ...
      final int endColumn)
      throws OutOfRangeException, NumberIsTooSmallException {
    MatrixUtils.checkSubMatrixIndex(this, startColumn, endColumn, startRow, endRow);
    visitor.start(rows, columns, startRow, endRow, startColumn, endColumn);
    for (int iBlock = startRow / BLOCK_SIZE; iBlock < 1 + endRow / BLOCK_SIZE; iBlock++) {
      int p0 = iBlock * BLOCK_SIZE;
      int pStart = JdkMath.max(startRow, p0);
      int pEnd = JdkMath.min((iBlock + 1) * BLOCK_SIZE, 1 + endRow);
      for (int p = pStart; p < pEnd; p++) {
        for (int jBlock = startColumn / BLOCK_SIZE; jBlock < 1 + endColumn / BLOCK_SIZE; jBlock++) {
          final int jWidth = blockWidth(jBlock);
          final int q0 = jBlock * BLOCK_SIZE;
          final int qStart = JdkMath.max(startColumn, q0);
          final int qEnd = JdkMath.min((jBlock + END_CONSTANT) * BLOCK_SIZE, 1 + endColumn);
          final double[] block = blocks[iBlock * blockColumns + jBlock];
