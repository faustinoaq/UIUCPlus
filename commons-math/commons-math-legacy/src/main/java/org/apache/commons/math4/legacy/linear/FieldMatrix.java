...
public interface FieldMatrix<T extends FieldElement<T>> extends AnyMatrix {
  ...
  /**
   * Create a new {@code FieldMatrix<T>} of the same type as the instance with the supplied row and
   * column dimensions.
   *
   ...
   * @return {@code this} * {@code m}.
   * @throws DimensionMismatchException if the number of columns of {@code this} matrix is not equal
   *     to the number of rows of matrix {@code m}.
   */
  ...
  /**
   * Returns the result multiplying this with itself <code>p</code> times. Depending on the type of
   * the field elements, T, instability for high powers might occur.
   *
  ...
  /**
   * Copy a submatrix. Rows and columns are 0-based. The designated submatrix is copied into the top
   * left portion of the destination array.
   *
   * @param startRow Initial row index.
   * @param endRow Final row index (inclusive).
   * @param startColumn Initial column index.
   ...
   * @throws MatrixDimensionMismatchException if the dimensions of {@code destination} are not large
   *     enough to hold the submatrix.
   ...
   * @throws MatrixDimensionMismatchException if the dimensions of {@code destination} do not match
   *     those of {@code this}.
   * @throws NoDataException if {@code selectedRows} or {@code selectedColumns} is empty
   * @throws NullArgumentException if {@code selectedRows} or {@code selectedColumns} is {@code
   *     null}.
   ...
   * </pre>
   *
   * and <code>subMatrix = {{3, 4} {5,6}}</code>, invoking <code>setSubMatrix(subMatrix,1,1))</code>
   * will result in
   *
   * <pre>
   * 1  2  3  4
   ...
   * @throws OutOfRangeException if {@code subMatrix} does not fit into this matrix from element in
   *     {@code (row, column)}.
   ...
   * @param matrix column matrix (must have one column and the same number of rows as the instance).
   * @throws OutOfRangeException if the specified column index is invalid.
   * @throws MatrixDimensionMismatchException if the matrix dimensions do not match one instance
   *     column.
   */
   ...
   * @throws OutOfRangeException if the specified column index is invalid.
   * @throws MatrixDimensionMismatchException if the array size does not match one instance column.
   */
   ...
   * @return {@code v * this}
   * @throws DimensionMismatchException if the number of rows of {@code this} matrix is not equal to
   *     the size of the vector {@code v}
   */
  ...
  public T walkInRowOrder(
      ...
      int startColumn,
      int endColumn) {
    if (startRow >= rows || endRow >= rows) throw new OutOfRangeException("endRow: " + endRow);
    throw new NumberIsTooSmallException("startRow: " + startRow, rows, true);
  }
   ...
   *
   * <p>Column order starts at upper left and iterating through all elements of a column from top to
   * bottom before going to the topmost element of the next column.
   *
   * @param visitor visitor used to process all matrix entries
   * @see #walkInRowOrder(FieldMatrixChangingVisitor)
   * @see #walkInRowOrder(FieldMatrixPreservingVisitor)
