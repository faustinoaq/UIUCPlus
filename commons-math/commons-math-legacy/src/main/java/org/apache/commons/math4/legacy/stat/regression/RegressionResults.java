...
public class RegressionResults {
   ...
   * @param varcov the variance covariance matrix, stored either in a square matrix or as a
   *     compressed
   * @param isSymmetricCompressed a flag which denotes that the variance covariance matrix is in
   ...
   * @param copyData if true a deep copy of all input data is made, if false only references are
   *     copied and the RegressionResults become mutable
   */
  public RegressionResults(
    ...
    if (!containsConstant) {
      this.globalFitInfo[ADJRSQ_IDX] =
          1.0 - (1.0 - this.globalFitInfo[RSQ_IDX]) * ((double) nobs / ((double) (nobs - rank)));
    } else {
      this.globalFitInfo[ADJRSQ_IDX] =
          1.0 - (sse * (nobs - 1.0)) / (globalFitInfo[SST_IDX] * (nobs - rank));
    }
  }
   ...
   *
   * <p>A redundant regressor will have its redundancy flag set, as well as a parameters estimated
   * equal to {@code Double.NaN}
   *
   * @param index Index.
   * @return the parameters estimated for regressor at index.
   * @throws OutOfRangeException if {@code index} is not in the interval {@code [0, number of
   *     parameters)}.
   */
   ...
   *
   * <p>The parameter estimates are returned in the natural order of the data.
   *
   * <p>A redundant regressor will have its redundancy flag set, as will a parameter estimate equal
   * to {@code Double.NaN}.
   *
  ...
  /**
   * Returns the <a href="http://www.xycoon.com/standerrorb(1).htm">standard error of the parameter
   * estimate at index</a>, usually denoted s(b<sub>index</sub>).
   *
   * @param index Index.
  ...
  /**
   * Returns the <a href="http://www.xycoon.com/standerrorb(1).htm">standard error of the parameter
   * estimates</a>, usually denoted s(b<sub>i</sub>).
   *
   * <p>If there are problems with an ill conditioned design matrix then the regressor which is
   * redundant will be assigned <code>Double.NaN</code>.
   *
   * @return an array standard errors associated with parameters estimates, null if no estimation
   *     occurred
   */
   ...
   *
   * <p>If there are problems with an ill conditioned design matrix then the covariance which
   * involves redundant columns will be assigned {@code Double.NaN}.
   *
   ...
   * @throws OutOfRangeException if {@code i} or {@code j} is not in the interval {@code [0, number
   *     of parameters)}.
   */
   ...
   *
   * <p>This is the maximum number of regressors, some techniques may drop redundant parameters
   *
   ...
   *
   * <p>If {@code n < 2}, this returns {@code Double.NaN}.
   *
   ...
   *
   * <ul>
   *   <li>numberOfParameters data pairs must have been added before invoking this method. If this
   *       method is invoked before a model can be estimated, <code>Double,NaN</code> is returned.
   * </ul>
   *
   ...
   *
   * <p>If there are fewer than <strong>numberOfParameters + 1</strong> data pairs in the model, or
   * if there is no variation in <code>x</code>, this returns <code>Double.NaN</code>.
   *
   * @return sum of squared deviations of y values
   */
  ...
  /**
   * Returns the <a href="http://www.xycoon.com/coefficient1.htm">coefficient of multiple
   * determination</a>, usually denoted r-square.
   *
   * <p><strong>Preconditions</strong>:
   *
   * <ul>
   *   <li>At least numberOfParameters observations (with at least numberOfParameters different x
   *       values) must have been added before invoking this method. If this method is invoked
   *       before a model can be estimated, {@code Double,NaN} is returned.
   * </ul>
   *
  ...
  /**
   * Returns the adjusted R-squared statistic, defined by the formula <div style="white-space: pre">
   * <code>
   * R<sup>2</sup><sub>adj</sub> = 1 - [SSR (n - 1)] / [SSTO (n - p)]
   * </code></div> where SSR is the sum of squared residuals}, SSTO is the total sum of squares}, n
   * is the number of observations and p is the number of parameters estimated (including the
   * intercept).
   *
   * <p>If the regression is estimated without an intercept term, what is returned is
   *
   * <pre>
  ...
  /**
   * Returns true if the regression model has been computed including an intercept. In this case,
   * the coefficient of the intercept is the first element of the {@link #getParameterEstimates()
   * parameter estimates}.
   *
