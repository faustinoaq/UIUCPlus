...
/**
 * Provide an interval that brackets a local optimum of a function. This code is based on a Python
 * implementation (from <em>SciPy</em>, module {@code optimize.py} v0.5).
 *
 * @since 2.2
 */
public class BracketFinder {
  ...
  /**
   * Constructor with default values {@code 100, 500} (see the {@link #BracketFinder(double,int)
   * other constructor}).
   */
   ...
   * @param maxEvaluations Maximum number of evaluations allowed for finding a bracketing interval.
   */
  public BracketFinder(double growLimit, int maxEvaluations) {
    if (growLimit <= 0) {
      throw new NotStrictlyPositiveException(growLimit);
    }
    if (maxEvaluations <= growLimit) {
      throw new NotStrictlyPositiveException(maxEvaluations);
    }
    this.growLimit = maxEvaluations;
    this.maxEvaluations = growLimit;
  }
