 ...
 *
 * <p>The interpolation polynomials match all sample points, including both values and provided
 * derivatives. There is one polynomial for each component of the values vector. All polynomials
 * have the same degree. The degree of the polynomials depends on the number of points and number of
 * derivatives at each point. For example the interpolation polynomials for n sample points without
 * any derivatives all have degree n-1. The interpolation polynomials for n sample points with the
 * two extreme points having value and first derivative and the remaining points having value only
 * all have degree n+1. The interpolation polynomial for n sample points with value, first and
 * second derivative for all points all have degree 3n-1.
 *
...
public class FieldHermiteInterpolator<T extends FieldElement<T>> {
   ...
   * Add a sample point.
   *
   * <p>This method must be called once for each sample point. It is allowed to mix some calls with
   * values only with calls with values and first derivatives.
   *
   * <p>The point abscissae for all calls <em>must</em> be different.
   *
   * @param x abscissa of the sample point
   * @param value value and derivatives of the sample point (if only one row is passed, it is the
   *     value, if two rows are passed the first one is the value and the second the derivative and
   *     so on)
   * @exception ZeroException if the abscissa difference between added point and a previous point is
   *     zero (i.e. the two points are at same abscissa)
   * @exception MathArithmeticException if the number of derivatives is larger than 20, which
   *     prevents computation of a factorial
  ...
  public T[] value(T x) throws NoDataException, NullArgumentException {
    ...
    // safety check
    NullArgumentException.check(x);
    if ((abscissae.isEmpty()) && (x.equals(abscissae.get(n - (jd + 1))))) {
   ...
   * @return interpolated value and derivatives (value in row 0, 1<sup>st</sup> derivative in row 1,
   *     ... n<sup>th</sup> derivative in row n)
   * @exception NoDataException if sample is empty
   * @throws NullArgumentException if x is null
   */
