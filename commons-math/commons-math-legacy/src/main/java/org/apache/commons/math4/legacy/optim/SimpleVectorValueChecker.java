...
public class SimpleVectorValueChecker extends AbstractConvergenceChecker<PointVectorValuePair> {
   ...
   * #converged(int,PointVectorValuePair,PointVectorValuePair)} method will return true (unless the
   * check is disabled).
   */
  ...
  public SimpleVectorValueChecker(
    ...
    super(relativeThreshold, absolute);
   ...
   * may be called several times from the same algorithm iteration with different points. This can
   * be detected by checking the iteration number at each call if needed. Each time this method is
