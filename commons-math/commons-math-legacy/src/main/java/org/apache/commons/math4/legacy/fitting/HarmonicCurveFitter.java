 ...
 * org.apache.commons.math4.legacy.analysis.function.HarmonicOscillator.Parametric harmonic
 * oscillator} function. <br>
...
public final class HarmonicCurveFitter extends SimpleCurveFitter {
   ...
   *
   * @param initialGuess Initial guess. If set to {@code null}, the initial guess will be estimated
   *     using the {@link ParameterGuesser}.
   ...
   *
   * <p>For a bilinear expression \(z(x_i, y_i) = A x_i + B y_i\), the coefficients \(A\) and \(B\)
   * that minimize a least-squares criterion \(\sum (z_i - z(x_i, y_i))^2\) are given by these
   * expressions: \[ A = \frac{\sum y_i y_i \sum x_i z_i - \sum x_i y_i \sum y_i z_i} {\sum x_i x_i
   * \sum y_i y_i - \sum x_i y_i \sum x_i y_i} \] \[ B = \frac{\sum x_i x_i \sum y_i z_i - \sum x_i
   * y_i \sum x_i z_i} {\sum x_i x_i \sum y_i y_i - \sum x_i y_i \sum x_i y_i}
   *
   * <p>\]
   *
   * <p>In fact, we can assume that both \(a\) and \(\omega\) are positive and compute them
   * directly, knowing that \(A = a^2 \omega^2\) and that \(B = -\omega^2\). The complete algorithm
   * is therefore: For each \(t_i\) from \(t_1\) to \(t_{n-1}\), compute: \[ f(t_i) \] \[ f'(t_i) =
   * \frac{f (t_{i+1}) - f(t_{i-1})}{t_{i+1} - t_{i-1}} \] \[ x_i = t_i - t_1 \] \[ y_i =
   * \int_{t_1}^{t_i} f^2(t) dt \] \[ z_i = \int_{t_1}^{t_i} f'^2(t) dt \] and update the sums: \[
    ...
    public double[] guess(Collection<WeightedObservedPoint> observations) {
      if (observations.size() < 4) {
        throw new NumberIsTooSmallException(
            LocalizedFormats.INSUFFICIENT_OBSERVED_POINTS_IN_SAMPLE, observations.size(), 4, true);
