...
public class Frequency<T extends Comparable<T>> {
  ...
  /** Default constructor. */
  public Frequency() {
    freqTable = new TreeMap<>(Collections.reverseOrder());
  }
  ...
  /**
   * Return an Iterator over the set of keys and values that have been added. Using the entry set to
   * iterate is more efficient in the case where you need to access respective counts as well as
   * values, since it doesn't require a "get" for every key...the value is provided in the
   * Map.Entry.
   *
   ...
   *
   * <p>Returns <code>Double.NaN</code> if no values have been added.
   *
   * @param v the value to lookup
  ...
  /**
   * Returns the cumulative percentage of values less than or equal to v (as a proportion between 0
   * and 1).
   *
   * <p>Returns <code>Double.NaN</code> if no values have been added.
   *
