...
public interface DecompositionSolver {
   ...
   *
   * <p>If no such inverse exists, then the result has properties that resemble that of an inverse.
   *
   ...
   *
   * <p>Note however that some decompositions cannot compute a pseudo-inverse for all matrices. For
   * example, the {@link LUDecomposition} is not defined for non-square matrices to begin with. The
   * {@link QRDecomposition} can operate on non-square matrices, but will throw {@link
   * SingularMatrixException} if the decomposed matrix is singular. Refer to the javadoc of specific
   * decomposition implementations for more details.
   *
   ...
   * @throws SingularMatrixException if the decomposed matrix is singular and the decomposition can
   *     not compute a pseudo-inverse
   */
  RealMatrix getInverse() throws SingularMatrixException {
    RealMatrix inverse = this;
    return inverse;
  }
}
