 ...
 *
 * <p><code> y = intercept + slope * x  </code>
 *
 * <p>Standard errors for <code>intercept</code> and <code>slope</code> are available as well as
 * ANOVA, r-square and Pearson's r statistics.
 *
 * <p>Observations (x,y pairs) can be added to the model one at a time or they can be provided in a
 * 2-dimensional array. The observations are not stored in memory, so there is no limit to the
 * number of observations that can be added to the model.
 *
 * <p><strong>Usage Notes</strong>:
 *
 * <ul>
 *   <li>When there are fewer than two observations in the model, or when there is no variation in
 *       the x values (i.e. all x values are the same) all statistics return <code>NaN</code>. At
 *       least two observations with different x coordinates are required to estimate a bivariate
 *       regression model.
 *   <li>Getters for the statistics always compute values based on the current set of observations
 *       -- i.e., you can get statistics, then add more data and get updated statistics without
 *       using a new instance. There is no "compute" method that updates all statistics. Each of the
 *       getters performs the necessary computations to return the requested statistic.
 *   <li>The intercept term may be suppressed by passing {@code false} to the {@link
 *       #SimpleRegression(boolean)} constructor. When the {@code hasIntercept} property is false,
 *       the model is estimated without a constant term and {@link #getIntercept()} returns {@code
 *       0}.
 * </ul>
 */
public class SimpleRegression implements UpdatingMultipleLinearRegression {
   ...
   *
   * <p>Use {@code false} to estimate a model with no intercept. When the {@code hasIntercept}
   * property is false, the model is estimated without a constant term and {@link #getIntercept()}
   * returns {@code 0}.
   *
   * @param includeIntercept whether or not to include an intercept term in the regression model
   */
   ...
   *
   * <p>The mean update formulae are based on a paper written by Philippe P&eacute;bay: <a
   * href="http://prod.sandia.gov/techlib/access-control.cgi/2008/086212.pdf">Formulas for Robust,
   * One-Pass Parallel Computation of Covariances and Arbitrary-Order Statistical Moments</a>, 2008,
   ...
   *
   * <p>Mirrors the addData method. This method permits the use of SimpleRegression instances in
   * streaming mode where the regression is applied to a sliding "window" of observations, however
   * the caller is responsible for maintaining the set of observations in the window. The method has
   ...
   * @throws ModelSpecificationException if the length of {@code data[i]} is not greater than or
   *     equal to 2
   */
    ...
    for (int i = 0; i < data.length; i++) {
      if (data[i].length < 2) {
        throw new ModelSpecificationException(
            LocalizedFormats.INVALID_REGRESSION_OBSERVATION, data[i].length, 2);
      }
   ...
   * @throws ModelSpecificationException if the length of {@code x} does not equal the number of
  ...
  public void addObservation(final double[] x, final double y) throws ModelSpecificationException {
    if (x == null || x.length == 0) {
      throw new ModelSpecificationException(
          LocalizedFormats.INVALID_REGRESSION_OBSERVATION, x != null ? x.length : 0, 1);
    }
    addData(x[0], y);
  }

  /**
   * Adds a series of observations to the regression model. The lengths of x and y must be the same
   * and x must be rectangular.
   *
   * @param x a series of observations on the independent variables
   * @param y a series of observations on the dependent variable The length of x and y must be the
   *     same
   * @throws ModelSpecificationException if {@code x} is not rectangular, does not match the length
  ...
  public void addObservations(final double[][] x, final double[] y)
    ...
    if (!obsOk) {
      throw new ModelSpecificationException(
          LocalizedFormats.NOT_ENOUGH_DATA_FOR_NUMBER_OF_PREDICTORS, 0, 1);
    }
    for (int i = 0; i < x.length; i++) {
      addData(x[i][0], y[i]);
    }
   ...
   *
   * <p>If the array is larger than the current n, only the first n elements are processed. This
   * method permits the use of SimpleRegression instances in streaming mode where the regression is
   * applied to a sliding "window" of observations, however the caller is responsible for
   * maintaining the set of observations in the window.
   *
   * <p>To remove all data, use <code>clear()</code>.
   *
  ...
  /**
   * Returns the "predicted" <code>y</code> value associated with the supplied <code>x</code> value,
   * based on the data that has been added to the model when this method is activated.
   *
   * <p><code> predict(x) = intercept + slope * x </code>
   *
   * <p><strong>Preconditions</strong>:
   *
   * <ul>
   *   <li>At least two observations (with at least two different x values) must have been added
  ...
  /**
   * Returns the intercept of the estimated regression line, if {@link #hasIntercept()} is true;
   * otherwise 0.
   *
   * <p>The least squares estimate of the intercept is computed using the <a
   * href="http://www.xycoon.com/estimation4.htm">normal equations</a>. The intercept is sometimes
   * denoted b0.
   *
   * <p><strong>Preconditions</strong>:
   *
   ...
   * </ul>
   *
   * @return the intercept of the regression line if the model includes an intercept; 0 otherwise
   ...
   *
   * <p>The least squares estimate of the slope is computed using the <a
   * href="http://www.xycoon.com/estimation4.htm">normal equations</a>. The slope is sometimes
   * denoted b1.
   *
   * <p><strong>Preconditions</strong>:
   *
  ...
  /**
   * Returns the <a href="http://www.xycoon.com/SumOfSquares.htm">sum of squared errors</a> (SSE)
   * associated with the regression model.
   *
   * <p>The sum is computed using the computational formula
   *
   * <p><code>SSE = SYY - (SXY * SXY / SXX)</code>
   *
   * <p>where <code>SYY</code> is the sum of the squared deviations of the y values about their
   * mean, <code>SXX</code> is similarly defined and <code>SXY</code> is the sum of the products of
   * x and y mean deviations.
   *
   * <p>The sums are accumulated using the updating algorithm referenced in {@link #addData}.
   *
   * <p>The return value is constrained to be non-negative - i.e., if due to rounding errors the
   * computational formula returns a negative result, 0 is returned.
   *
   * <p><strong>Preconditions</strong>:
   *
   * <ul>
   ...
   *
   * <p>This is defined as SSTO <a href="http://www.xycoon.com/SumOfSquares.htm">here</a>.
   *
   * <p>If {@code n < 2}, this returns <code>Double.NaN</code>.
   *
   ...
   *
   * <p>If <code>n &lt; 2</code>, this returns <code>Double.NaN</code>.
   *
  ...
  /**
   * Returns the sum of squared deviations of the predicted y values about their mean (which equals
   * the mean of y).
   *
   * <p>This is usually abbreviated SSR or SSM. It is defined as SSM <a
   ...
   *
   * <p>If there are fewer than <strong>three</strong> data pairs in the model, or if there is no
  ...
  /**
   * Returns <a href="http://mathworld.wolfram.com/CorrelationCoefficient.html">Pearson's product
   * moment correlation coefficient</a>, usually denoted r.
   *
   * <p><strong>Preconditions</strong>:
   *
   * <ul>
  ...
  /**
   * Returns the <a href="http://www.xycoon.com/coefficient1.htm">coefficient of determination</a>,
  ...
  /**
   * Returns the <a href="http://www.xycoon.com/standarderrorb0.htm">standard error of the intercept
   * estimate</a>, usually denoted s(b0).
   *
   * <p>If there are fewer that <strong>three</strong> observations in the model, or if there is no
   * variation in x, this returns <code>Double.NaN</code>. Additionally, a <code>Double.NaN</code>
  ...
  public double getInterceptStdErr() {
    ...
    return JdkMath.sqrt(getMeanSquareError() * ((1d / n) + (xbar * xbar) / sumXX));
  }

  /**
   * Returns the <a href="http://www.xycoon.com/standerrorb(1).htm">standard error of the slope
   * estimate</a>, usually denoted s(b1).
   *
   * <p>If there are fewer that <strong>three</strong> data pairs in the model, or if there is no
   ...
   *   <li>{@code (0 < alpha < 1)}; otherwise an <code>OutOfRangeException</code> is thrown.
   * </ul>
   *
  ...
  public double getSlopeConfidenceInterval(final double alpha) throws OutOfRangeException {
    ...
    return getSlopeStdErr() * distribution.inverseSurvivalProbability(alpha / 2d);
  }
   ...
   * 0</code>. On regression output, this is often denoted {@code Prob(|t| > 0)}
   *
   * <p><strong>Usage Note</strong>:<br>
   * The validity of this statistic depends on the assumption that the observations included in the
   * model are drawn from a <a href="http://mathworld.wolfram.com/BivariateNormalDistribution.html">
   ...
   * @throws org.apache.commons.math4.legacy.exception.MaxCountExceededException if the significance
   *     level can not be computed.
   */
  public double getSignificance() {
    ...
    // No advertised NotStrictlyPositiveException here - will return NaN above
    TDistribution distribution = TDistribution.of(n - 2d);
    return 2d * (distribution.survivalProbability(JdkMath.abs(getSlope()) / getSlopeStdErr()));
  }
   ...
   *
   * <p>Will return <code>NaN</code> if slope is <code>NaN</code>.
   *
   ...
   *
   * <p>If there are fewer than 3 observations in the model and {@code hasIntercept} is true a
   * {@code NoDataException} is thrown. If there is no intercept term, the model must contain at
   * least 2 observations.
   *
   * @return RegressionResults acts as a container of regression output
   * @throws ModelSpecificationException if the model is not correctly specified
   * @throws NoDataException if there is not sufficient data in the model to estimate the regression
   *     parameters
   */
  @Override
  public RegressionResults regress() throws ModelSpecificationException, NoDataException {
    if (hasIntercept) {
      ...
      if (JdkMath.abs(sumXX) > Precision.SAFE_MIN) {
        ...
        final double[] vcv =
            new double[] {mse * (xbar * xbar / sumXX + 1.0 / n), -xbar * mse / sumXX, mse / sumXX};
  ...
  /**
   * Performs a regression on data present in buffers including only regressors. indexed in
   * variablesToInclude and outputs a RegressionResults object
   *
    ...
    if (hasIntercept) {
      if (variablesToInclude.length == 2) {
        ...
        if (variablesToInclude[0] == 0) {
          ...
          return new RegressionResults(
              params, new double[][] {vcv}, true, n, 1, sumY, syy + mean, sumYY, true, false);
          ...
          if (!Double.isNaN(sxx)) {
            ...
            return new RegressionResults(
                params, new double[][] {vcv}, true, n, 1, sumY, syy, sse, false, false);
          } else {
            final double[] vcv = new double[] {Double.NaN};
            final double[] params = new double[] {Double.NaN};
            return new RegressionResults(
