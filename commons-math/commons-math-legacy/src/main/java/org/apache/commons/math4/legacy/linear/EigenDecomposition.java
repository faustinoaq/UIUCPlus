 ...
 *
 * <p>This class is similar in spirit to the {@code EigenvalueDecomposition} class from the <a
 ...
 * eigenvector matrix V is orthogonal, i.e. {@code A = V.multiply(D.multiply(V.transpose()))} and
 ...
 * </pre>
 *
 * The columns of V represent the eigenvectors in the sense that {@code A*V = V*D}, i.e.
 * A.multiply(V) equals V.multiply(D). The matrix V may be badly conditioned, or even singular, so
 * the validity of the equation {@code A = V*D*inverse(V)} depends upon the condition of V.
 *
...
public class EigenDecomposition {
  ...
  /**
   * Calculates the eigen decomposition of the symmetric tridiagonal matrix. The Householder matrix
   ...
   * made about the orientation of the system axes formed by the columns of V (e.g. in a 3-dimension
   * space, V can form a left- or right-handed system).
   *
   * @return the V matrix.
   */
   ...
   * transpose is also its inverse. The columns of V are the eigenvectors of the original matrix. No
   * assumption is made about the orientation of the system axes formed by the columns of V (e.g. in
   * a 3-dimension space, V can form a left- or right-handed system).
   *
   ...
   *
   * <p>The method performs a zero check for each element of the {@link #getImagEigenvalues()} array
   * and returns {@code true} if any element is not equal to zero.
   *
  ...
  private void findEigenVectors(final double[][] householderMatrix) {
    ...
    for (int j = 0; j < n; j++) {
      ...
      do {
        for (m = j; m < n - 1; m++) {
          double delta = JdkMath.abs(realEigenvalues[m]) + JdkMath.abs(realEigenvalues[m + 1]);
        ...
        if (m != j) {
          if (its == MAX_ITER) {
            throw new MaxCountExceededException(LocalizedFormats.CONVERGENCE_FAILED, MAX_ITER);
          }
          its++;
          ...
          if (Precision.compareTo(imagEigenvalues[i], 0.0, EPSILON) < 0) {
            ...
            if (Precision.equals(imagEigenvalues[i], 0.0)) {
              ...
              if (JdkMath.abs(x) > (JdkMath.abs(z) + JdkMath.abs(q))) {
                ...
                final Complex c2 =
                    cdiv(-r - y * matrixT[i][idx - 1], -s - y * matrixT[i][idx], z, q);
            ...
            // Overflow control
            double t = JdkMath.max(JdkMath.abs(matrixT[i][idx - 1]), JdkMath.abs(matrixT[i][idx]));
            if ((Precision.EPSILON * t) * t > 1) {
              for (int j = i; j <= idx; j++) {
                matrixT[j][idx - 1] /= t;
                matrixT[j][idx] /= t;
