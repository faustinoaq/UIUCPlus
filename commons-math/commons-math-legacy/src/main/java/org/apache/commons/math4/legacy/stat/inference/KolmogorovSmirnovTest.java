 ...
 *
 * <ul>
 *   <li>When the product of the sample sizes is less than 10000, the method presented in [4] is
 *       used to compute the exact p-value for the 2-sample test.
 *   <li>When the product of the sample sizes is larger, the asymptotic distribution of \(D_{n,m}\)
 *       is used. See {@link #approximateP(double, int, int)} for details on the approximation.
 * </ul>
 *
 * <p>For small samples (former case), if the data contains ties, random jitter is added to the
 * sample data to break ties before applying the algorithm above. Alternatively, the {@link
 * #bootstrap(double[],double[],int,boolean,UniformRandomProvider)} method, modeled after <a
 * href="http://sekhon.berkeley.edu/matching/ks.boot.html">ks.boot</a> in the R Matching package
 * [3], can be used if ties are known to be present in the data.
 *
 ...
 *
 * <p>The methods used by the 2-sample default implementation are also exposed directly:
 *
 * <ul>
 *   <li>{@link #exactP(double, int, int, boolean)} computes exact 2-sample p-values
 *   <li>{@link #approximateP(double, int, int)} uses the asymptotic distribution The {@code
 *       boolean} arguments in the first two methods allow the probability used to estimate the
 *       p-value to be expressed using strict or non-strict inequality. See {@link
 ...
 *       The Matching package for R</a> Journal of Statistical Software, 42(7): 1-52.
 *   <li>[4] Wilcox, Rand. 2012. Introduction to Robust Estimation and Hypothesis Testing, Chapter
...
public class KolmogorovSmirnovTest {
  ...
  /**
   * When product of sample sizes exceeds this value, 2-sample K-S test uses asymptotic distribution
   ...
   * evaluating the null hypothesis that {@code data} conforms to {@code distribution}. If {@code
   * exact} is true, the distribution used to compute the p-value is computed using extended
  ...
  public double kolmogorovSmirnovTest(
      ContinuousDistribution distribution, double[] data, boolean exact) {
   ...
   * length of {@code data} and \(F_n\) is the empirical distribution that puts mass \(1/n\) at each
   * of the values in {@code data}.
   *
   * @param distribution reference distribution
   ...
   * probability distribution. Specifically, what is returned is an estimate of the probability that
   * the {@link #kolmogorovSmirnovStatistic(double[], double[])} associated with a randomly selected
   * partition of the combined sample into subsamples of sizes {@code x.length} and {@code y.length}
   * will strictly exceed (if {@code strict} is {@code true}) or be at least as large as (if {@code
   * strict} is {@code false}) as {@code kolmogorovSmirnovStatistic(x, y)}.
   *
   ...
   * empirical distribution that puts mass \(1/n\) at each of the values in {@code x} and \(F_m\) is
   ...
   * where \(n\) is the length of {@code x}, \(m\) is the length of {@code y}, \(F_n\) is the
   * empirical distribution that puts mass \(1/n\) at each of the values in {@code x} and \(F_m\) is
   * the empirical distribution of the {@code y} values. Finally \(n m D_{n,m}\) is returned as long
   * value.
   *
   * @param x first sample
   * @param y second sample
   ...
   * @throws NullArgumentException if {@code data} is null
   */
  public boolean kolmogorovSmirnovTest(
      ContinuousDistribution distribution, double[] data, double alpha) {
  ...
  /**
   * Estimates the <i>p-value</i> of a two-sample <a
   * href="http://en.wikipedia.org/wiki/Kolmogorov-Smirnov_test">Kolmogorov-Smirnov test</a>
   * evaluating the null hypothesis that {@code x} and {@code y} are samples drawn from the same
  ...
  public double bootstrap(
      double[] x, double[] y, int iterations, boolean strict, UniformRandomProvider rng) {
    ...
    final ContinuousDistribution.Sampler sampler =
        new EnumeratedRealDistribution(combined).createSampler(rng);
    ...
    return strict
        ? greaterCount / (double) iterations
   ...
   * values given in [2] (see above). The result is not exact as with {@link #cdfExact(double, int)}
   * because calculations are based on {@code double} rather than {@link BigFraction}.
   *
   * @param d statistic
   * @param n sample size
   * @return \(P(D_n &lt; d)\)
   * @throws MathArithmeticException if algorithm fails to convert {@code h} to a {@link
   *     BigFraction} in expressing {@code d} as \((k - h) / m\) for integer {@code k, m} and \(0
   *     \le h &lt; 1\)
   */
   ...
   * Normally, you would choose {@link #cdf(double, int)}. See the class javadoc for definitions and
   * algorithm description.
   *
   * @param d statistic
   * @param n sample size
   * @return \(P(D_n &lt; d)\)
   * @throws MathArithmeticException if the algorithm fails to convert {@code h} to a {@link
   *     BigFraction} in expressing {@code d} as \((k - h) / m\) for integer {@code k, m} and \(0
   *     \le h &lt; 1\)
   */
   ...
   * @param d statistic
   * @param n sample size
   * @param exact whether the probability should be calculated exact using {@link BigFraction}
   *     everywhere at the expense of very slow execution time, or if {@code double} should be used
   *     convenient places to gain speed. Almost never choose {@code true} in real applications
   *     unless you are very sure; {@code true} is almost solely for verification purposes.
   * @return \(P(D_n &lt; d)\)
   * @throws MathArithmeticException if algorithm fails to convert {@code h} to a {@link
   *     BigFraction} in expressing {@code d} as \((k - h) / m\) for integer {@code k, m} and \(0
   *     \le h &lt; 1\).
  ...
  private double roundedK(double d, int n) {
    ...
    final int k = this.createRoundedH(d, n);
  ...
  /**
   * Computes the Pelz-Good approximation for \(P(D_n &lt; d)\) as described in [2] in the class
   * javadoc.
   *
  ...
  public double pelzGood(double d, int n) {
    ...
    for (k = 0; k < MAXIMUM_PARTIAL_SUM_COUNT; k++) {
      ...
      increment =
          (z6Term
                  + z4Term
                  + PI_SQUARED * (z4Term - z2Term) * kTerm2
                  + pi4 * (1 - twoZ2) * kTerm2 * kTerm2)
    ...
    for (k = 0; k < MAXIMUM_PARTIAL_SUM_COUNT; k++) {
      ...
      increment =
          (pi6 * kTerm6 * (5 - 30 * z2)
                  + pi4 * kTerm4 * (-60 * z2 + 212 * z4)
                  + PI_SQUARED * kTerm2 * (135 * z4 - 96 * z6)
                  - 30 * z6
                  - 90 * z8)
              * JdkMath.exp(-PI_SQUARED * kTerm2 / twoZ2);
      sum += increment;
    ...
    for (k = 1; k < MAXIMUM_PARTIAL_SUM_COUNT; k++) {
      ...
      kTerm4 = kTerm2 * kTerm2;
      increment =
          (-pi4 * kTerm4 + 3 * PI_SQUARED * kTerm2 * z2)
              * JdkMath.exp(-PI_SQUARED * kTerm2 / twoZ2);
      sum2 += increment;
      if (JdkMath.abs(increment) < PG_SUM_RELATIVE_ERROR * JdkMath.abs(sum2)) {
        break;
   ...
   * @throws ArithmeticException if algorithm fails to convert {@code h} to a {@link BigFraction}.
   */
  private FieldDenseMatrix<BigFraction> createExactH(double d, int n) {
    final int k = (int) Math.ceil(n * d);
    final int m = 2 * k - 1;
    ...
    final FieldDenseMatrix<BigFraction> hData =
        FieldDenseMatrix.create(BigFractionField.get(), m, m);
     ...
     */
    for (int i = 0; i < m; ++i) {
      hData.set(i, 0, hData.get(i, 0).subtract(hPowers[i]));
      hData.set(m - 1, i, hData.get(m - 1, i).subtract(hPowers[m - i - 1]));
    }
    ...
    if (h.compareTo(ONE_HALF) > 0) {
      hData.set(m - 1, 0, hData.get(m - 1, 0).add(h.multiply(2).subtract(1).pow(m)));
    }
      ...
      for (int j = 0; j < i + 1; ++j) {
        if (i - j + 1 > 0) {
          for (int g = 2; g <= i - j + 1; ++g) {
            hData.set(i, j, hData.get(i, j).divide(g));
          }
        }
      }
    }
  ...
  private void checkArray(double[] array) {
    ...
    if (array.length < 2) {
      throw new InsufficientDataException(
          LocalizedFormats.INSUFFICIENT_OBSERVED_POINTS_IN_SAMPLE, array.length, 2);
    }
  }
  ...
  /**
   * Given a d-statistic in the range [0, 1] and the two sample sizes n and m, an integral
   * d-statistic in the range [0, n*m] is calculated, that can be used for comparison with other
   * integral d-statistics. Depending whether {@code strict} is {@code true} or not, the returned
   * value divided by (n*m) is greater than (resp greater than or equal to) the given d value
   ...
   * d)\), where \(D_{n,m}\) is the 2-sample Kolmogorov-Smirnov statistic. See {@link
   * #kolmogorovSmirnovStatistic(double[], double[])} for the definition of \(D_{n,m}\).
   *
   * <p>The returned probability is exact, implemented by unwinding the recursive function
  ...
  public double exactP(double d, int n, int m, boolean strict) {
    return 1
        - n(m, n, m, n, calculateIntegralD(d, m, n, strict), strict)
            / BinomialCoefficientDouble.value(n + m, m);
  }

  /**
   * Uses the Kolmogorov-Smirnov distribution to approximate \(P(D_{n,m} &gt; d)\) where \(D_{n,m}\)
   * is the 2-sample Kolmogorov-Smirnov statistic. See {@link #kolmogorovSmirnovStatistic(double[],
   * double[])} for the definition of \(D_{n,m}\).
   *
   * <p>Specifically, what is returned is \(1 - k(d \sqrt{mn / (m + n)})\) where \(k(t) = 1 + 2
   * \sum_{i=1}^\infty (-1)^i e^{-2 i^2 t^2}\). See {@link #ksSum(double, double, int)} for details
   * on how convergence of the sum is determined.
   *
   * @param d D-statistic value
   * @param n first sample size
   * @param m second sample size
  ...
  public double approximateP(double d, int n, int m) {
    return 1
        - ksSum(
            d * JdkMath.sqrt(((double) m * (double) n) / ((double) m + (double) n)),
            KS_SUM_CAUCHY_CRITERION,
            MAXIMUM_PARTIAL_SUM_COUNT);
  }

  /**
   * Fills a boolean array randomly with a fixed number of {@code true} values. The method uses a
   * simplified version of the Fisher-Yates shuffle algorithm. By processing first the {@code true}
   * values followed by the remaining {@code false} values less random numbers need to be generated.
   * The method is optimized for the case that the number of {@code true} values is larger than or
   * equal to the number of {@code false} values.
   *
   ...
   */
  private static void fillBooleanArrayRandomlyWithFixedNumberTrueValues(
      final boolean[] b, final int numberOfTrueValues, final UniformRandomProvider rng) {
    Arrays.fill(b, true);
   ...
   * 2-sample Kolmogorov-Smirnov statistic. See {@link #kolmogorovSmirnovStatistic(double[],
   * double[])} for the definition of \(D_{n,m}\).
   *
   * <p>The simulation generates {@code iterations} random partitions of {@code m + n} into an
   * {@code n} set and an {@code m} set, computing \(D_{n,m}\) for each partition and returning the
   * proportion of values that are greater than {@code d}, or greater than or equal to {@code d} if
  ...
  /**
   * Uses Monte Carlo simulation to approximate \(P(D_{n,m} >= d / (n * m))\) where \(D_{n,m}\) is
   * the 2-sample Kolmogorov-Smirnov statistic.
   *
   * <p>Here {@code d} is the D-statistic represented as long value. The real D-statistic is
   * obtained by dividing {@code d} by {@code n * m}. See also {@link
   * #monteCarloP(double,int,int,boolean,int,UniformRandomProvider)}.
   *
   ...
   */
  private double integralMonteCarloP(
      final long d, final int n, final int m, final int iterations, UniformRandomProvider rng) {
  ...
  /**
   * If there are no ties in the combined dataset formed from x and y, this method is a no-op. If
   * there are ties, a uniform random deviate in is added to each value in x and y, and this method
   * overwrites the data in x and y with the jittered values.
   *
   * @param x First sample.
   * @param y Second sample.
   ...
   *
   * <p>Note that jitter is applied in-place - i.e., the array values are overwritten with the
   * result of applying jitter.
   *
  ...
  /**
   * The function C(i, j) defined in [4] (class javadoc), formula (5.5). defined to return 1 if |i/n
   * - j/m| <= c; 0 otherwise. Here c is scaled up and recoded as a long to avoid rounding errors in
   * comparison tests, so what is actually tested is |im - jn| <= cmn.
   *
  ...
  /**
   * The function N(i, j) defined in [4] (class javadoc). Returns the number of paths over the
   * lattice {(i,j) : 0 <= i <= n, 0 <= j <= m} from (0,0) to (i,j) satisfying C(h,k, m, n, c) = 1
   * for each (h,k) on the path. The return value is integral, but subject to overflow, so it is
   * maintained and returned as a double.
   *
   * @param i first path parameter
