 ...
 * Recursive Multivariate Automatic Differentiation</a>, Mathematics Magazine, vol. 75, no. 3, June
 * 2002. However, in order to avoid performances bottlenecks, the recursive rules are "compiled"
 * once in an unfold form. This class does this recursion unrolling and stores the computation rules
 * as simple loops with pre-computed indirection arrays.
 *
 * <p>This class maps all derivative computation into single dimension arrays that hold the value
 * and partial derivatives. The class does not hold these arrays, which remains under the
 * responsibility of the caller. For each combination of number of free parameters and derivation
 * order, only one compiler is necessary, and this compiler will be used to perform computations on
 * all arrays provided to it, which can represent hundreds or thousands of different parameters kept
 * together with all their partial derivatives.
 *
 * <p>The arrays on which compilers operate contain only the partial derivatives together with the
 * 0<sup>th</sup> derivative, i.e. the value. The partial derivatives are stored in a
 * compiler-specific order, which can be retrieved using methods {@link
 ...
 * (i.e. the {@link #getPartialDerivativeIndex(int...) getPartialDerivativeIndex} method returns 0
 * when called with 0 for all derivation orders and {@link #getPartialDerivativeOrders(int)
 * getPartialDerivativeOrders} returns an array filled with 0 when called with 0 as the index).
 *
 * <p>Note that the ordering changes with number of parameters and derivation order. For example
 * given 2 parameters x and y, df/dy is stored at index 2 when derivation order is set to 1 (in this
 * case the array has three elements: f, df/dx and df/dy). If derivation order is set to 2, then
 * df/dy will be stored at index 3 (in this case the array has six elements: f, df/dx, df/dxdx,
 * df/dy, df/dxdy and df/dydy).
 *
 * <p>Given this structure, users can perform some simple operations like adding, subtracting or
 * multiplying constants and negating the elements by themselves, knowing if they want to mutate
 * their array or create a new array. These simple operations are not provided by the compiler. The
 * compiler provides only the more complex operations between several arrays.
 *
 * <p>This class is mainly used as the engine for scalar variable {@link DerivativeStructure}. It
 * can also be used directly to hold several variables in arrays for more complex data structures.
 * User can for example store a vector of n variables depending on three x, y and z free parameters
 * in one array as follows:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>Then in another function, user can perform some operations on all elements stored in the
 * single array, such as a simple product of all variables:
 *
 * <pre>
...
public final class DSCompiler {
  ...
  private DSCompiler(
    ...
    this.order = order;
    this.valueCompiler = derivativeCompiler;
    this.derivativeCompiler = valueCompiler;
    ...
    this.compIndirection =
        compileCompositionIndirection(
            parameters, order, valueCompiler, derivativeCompiler, sizes, derivativesIndirection);
  }
   ...
   * @return sizes array
   */
  private static int[][] compileSizes(
      final int parameters, final int order, final DSCompiler valueCompiler) {
   ...
   */
  private static int[][] compileDerivativesIndirection(
      final int parameters,
      final int order,
      final DSCompiler valueCompiler,
      final DSCompiler derivativeCompiler) {
    ...
    for (int i = 0; i < vSize; ++i) {
      ...
      System.arraycopy(
          valueCompiler.derivativesIndirection[i], 0, derivativesIndirection[i], 0, parameters - 1);
    }
   ...
   *
   * <p>This indirection array contains the indices of all elements except derivatives for last
   * derivation order.
   *
   * @param parameters number of free parameters
   * @param order derivation order
   ...
   */
  private static int[] compileLowerIndirection(
      final int parameters,
      final int order,
      final DSCompiler valueCompiler,
      final DSCompiler derivativeCompiler) {
   ...
   *
   * <p>This indirection array contains the indices of all pairs of elements involved when computing
   * a multiplication. This allows a straightforward loop-based multiplication (see {@link
   ...
   *
   * <p>This indirection array contains the indices of all sets of elements involved when computing
   * a composition. This allows a straightforward loop-based composition (see {@link
   ...
   * @throws NumberIsTooLargeException if order is too large
   */
  private static int[][][] compileCompositionIndirection(
      final int parameters,
      final int order,
      final DSCompiler valueCompiler,
      final DSCompiler derivativeCompiler,
    ...
    for (int i = 0; i < dSize; ++i) {
      ...
      for (int[] term : derivativeCompiler.compIndirection[i]) {
        ...
        derivedTermF[term.length] =
            getPartialDerivativeIndex(parameters, order, sizes, orders); // g_1
   ...
   *
   * <p>If all orders are set to 0, then the 0<sup>th</sup> order derivative is returned, which is
   * the value of the function.
   *
   * <p>The indices of derivatives are between 0 and {@link #getSize() getSize()} - 1. Their
   * specific order is fixed for a given compiler, but otherwise not publicly specified. There are
   * however some simple cases which have guaranteed indices:
   *
   * <ul>
   *   <li>the index of 0<sup>th</sup> order derivative is always 0
   *   <li>if there is only 1 {@link #getFreeParameters() free parameter}, then the derivatives are
   *       sorted in increasing derivation order (i.e. f at index 0, df/dp at index 1,
   *       d<sup>2</sup>f/dp<sup>2</sup> at index 2 ... d<sup>k</sup>f/dp<sup>k</sup> at index k),
   *   <li>if the {@link #getOrder() derivation order} is 1, then the derivatives are sorted in
   *       increasing free parameter order (i.e. f at index 0, df/dx<sub>1</sub> at index 1,
   *       df/dx<sub>2</sub> at index 2 ... df/dx<sub>k</sub> at index k),
   *   <li>all other cases are not publicly specified
   * </ul>
   *
   * <p>This method is the inverse of method {@link #getPartialDerivativeOrders(int)}
   *
   * @param orders derivation orders with respect to each parameter
   * @return index of the partial derivative
   * @exception DimensionMismatchException if the numbers of parameters does not match the instance
   * @exception NumberIsTooLargeException if sum of derivation orders is larger than the instance
   *     limits
   ...
   * @param order derivation order
   * @param sizes sizes array
   * @param orders derivation orders with respect to each parameter (the length of this array must
   *     match the number of parameters)
   * @return index of the partial derivative
   * @exception NumberIsTooLargeException if sum of derivation orders is larger than the instance
   *     limits
   */
  private static int getPartialDerivativeIndex(
      final int parameters, final int order, final int[][] sizes, final int... orders)
      throws NumberIsTooLargeException {
   ...
   * @param srcDerivativesIndirection derivatives indirection array for the source derivative
   *     structure
   ...
   * @return index of the partial derivative with the <em>same</em> characteristics in destination
   *     derivative structure
   * @throws NumberIsTooLargeException if order is too large
   */
  private static int convertIndex(
      final int index,
      final int srcP,
      final int[][] srcDerivativesIndirection,
      final int destP,
      final int destO,
      final int[][] destSizes)
      throws NumberIsTooLargeException {
   ...
   *
   * <p>This method is the inverse of {@link #getPartialDerivativeIndex(int...)}.
   *
   ...
   *
   * <p>This number includes the single 0 order derivative element, which is guaranteed to be stored
  ...
  /**
   * Compute linear combination. The derivative structure built will be a1 * ds1 + a2 * ds2 + a3 *
   * ds3 + a4 * ds4
   *
   * @param a1 first scale factor
   * @param c1 first base (unscaled) component
   ...
   * @param rhsOffset offset of the right hand side in its array
   * @param result array where result must be stored (for multiplication the result array
   *     <em>cannot</em> be one of the input arrays)
   * @param resultOffset offset of the result in its array
   */
   ...
   * @param rhsOffset offset of the right hand side in its array
   * @param result array where result must be stored (for division the result array <em>cannot</em>
   *     be one of the input arrays)
   * @param resultOffset offset of the result in its array
   */
  public void divide(
      final double[] lhs,
   ...
   * @param result array where result must be stored (for power the result array <em>cannot</em> be
   *     the input array)
   * @param resultOffset offset of the result in its array
   * @since 3.3
   */
   ...
   * @param result array where result must be stored (for n<sup>th</sup> root the result array
   *     <em>cannot</em> be the input array)
   * @param resultOffset offset of the result in its array
   */
  public void rootN(
      final double[] operand,
   ...
   * @param operand array holding the operand
   * @param operandOffset offset of the operand in its array
   * @param result array where result must be stored (for exponential the result array
   *     <em>cannot</em> be the input array)
   * @param resultOffset offset of the result in its array
   */
  public void exp(
   ...
   * @param operand array holding the operand
   * @param operandOffset offset of the operand in its array
   * @param result array where result must be stored (for logarithm the result array <em>cannot</em>
   *     be the input array)
   * @param resultOffset offset of the result in its array
   */
  public void log(
   ...
   * @param operand array holding the operand
   * @param operandOffset offset of the operand in its array
   * @param result array where result must be stored (for shifted logarithm the result array
   *     <em>cannot</em> be the input array)
   * @param resultOffset offset of the result in its array
   */
  public void log1p(
   ...
   * @param operand array holding the operand
   * @param operandOffset offset of the operand in its array
   * @param result array where result must be stored (for base 10 logarithm the result array
   *     <em>cannot</em> be the input array)
   * @param resultOffset offset of the result in its array
   */
  public void log10(
   ...
   * @param operand array holding the operand
   * @param operandOffset offset of the operand in its array
   * @param result array where result must be stored (for cosine the result array <em>cannot</em> be
   *     the input array)
   * @param resultOffset offset of the result in its array
   */
  public void cos(
   ...
   * @param operand array holding the operand
   * @param operandOffset offset of the operand in its array
   * @param result array where result must be stored (for sine the result array <em>cannot</em> be
   *     the input array)
   * @param resultOffset offset of the result in its array
   */
  public void sin(
   ...
   * @param operand array holding the operand
   * @param operandOffset offset of the operand in its array
   * @param result array where result must be stored (for tangent the result array <em>cannot</em>
   *     be the input array)
   * @param resultOffset offset of the result in its array
   */
  public void tan(
    ...
    if (order > 0) {
      ...
      // as per polynomial parity, we can store coefficients of both P_(n-1) and P_n in the same
      // array
   ...
   * @param operand array holding the operand
   * @param operandOffset offset of the operand in its array
   * @param result array where result must be stored (for arc cosine the result array
   *     <em>cannot</em> be the input array)
   * @param resultOffset offset of the result in its array
   */
  public void acos(
   ...
   * @param operand array holding the operand
   * @param operandOffset offset of the operand in its array
   * @param result array where result must be stored (for arc sine the result array <em>cannot</em>
   *     be the input array)
   * @param resultOffset offset of the result in its array
   */
  public void asin(
   ...
   * @param operand array holding the operand
   * @param operandOffset offset of the operand in its array
   * @param result array where result must be stored (for arc tangent the result array
   *     <em>cannot</em> be the input array)
   * @param resultOffset offset of the result in its array
   */
  public void atan(
    ...
    if (order > 0) {
      ...
      // as per polynomial parity, we can store coefficients of both Q_(n-1) and Q_n in the same
      // array
   ...
   * @param result array where result must be stored (for two arguments arc tangent the result array
   *     <em>cannot</em> be the input array)
   * @param resultOffset offset of the result in its array
   */
  public void atan2(
    ...
    if (x[xOffset] >= 0) {
      ...
      atan(tmp1, 0, tmp2, 0); // atan(y / (r - x))
      result[resultOffset] =
          ((tmp2[0] <= 0) ? -JdkMath.PI : JdkMath.PI)
              - 2 * tmp2[0]; // +/-pi - 2 * atan(y / (r - x))
   ...
   * @param operand array holding the operand
   * @param operandOffset offset of the operand in its array
   * @param result array where result must be stored (for hyperbolic cosine the result array
   *     <em>cannot</em> be the input array)
   * @param resultOffset offset of the result in its array
   */
  public void cosh(
   ...
   * @param operand array holding the operand
   * @param operandOffset offset of the operand in its array
   * @param result array where result must be stored (for hyperbolic sine the result array
   *     <em>cannot</em> be the input array)
   * @param resultOffset offset of the result in its array
   */
  public void sinh(
   ...
   * @param operand array holding the operand
   * @param operandOffset offset of the operand in its array
   * @param result array where result must be stored (for hyperbolic tangent the result array
   *     <em>cannot</em> be the input array)
   * @param resultOffset offset of the result in its array
   */
  public void tanh(
   ...
   * @param operand array holding the operand
   * @param operandOffset offset of the operand in its array
   * @param result array where result must be stored (for inverse hyperbolic cosine the result array
   *     <em>cannot</em> be the input array)
   * @param resultOffset offset of the result in its array
   */
  public void acosh(
   ...
   * @param operand array holding the operand
   * @param operandOffset offset of the operand in its array
   * @param result array where result must be stored (for inverse hyperbolic sine the result array
   *     <em>cannot</em> be the input array)
   * @param resultOffset offset of the result in its array
   */
  public void asinh(
   ...
   * @param operand array holding the operand
   * @param operandOffset offset of the operand in its array
   * @param result array where result must be stored (for inverse hyperbolic tangent the result
   *     array <em>cannot</em> be the input array)
   * @param resultOffset offset of the result in its array
   */
  public void atanh(
      final double[] operand,
   ...
   * @param operand array holding the operand
   * @param operandOffset offset of the operand in its array
   * @param f array of value and derivatives of the function at the current point (i.e. at {@code
   *     operand[operandOffset]}).
   * @param result array where result must be stored (for composition the result array
   *     <em>cannot</em> be the input array)
   * @param resultOffset offset of the result in its array
   */
  public void compose(
