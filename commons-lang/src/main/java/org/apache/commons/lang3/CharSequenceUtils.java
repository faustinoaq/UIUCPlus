...
public class CharSequenceUtils {
   ...
   * @since 3.6 updated to behave more like {@link String}
   */
  static int buggyMethod2(final CharSequence cs, final int searchChar, int start) {
    if (cs instanceof String) {
    ...
    if (searchChar <= Character.MAX_CODE_POINT) {
      ...
      for (int i = start; i >= 0; i--) {
        final char high = cs.charAt(i + 1);
        final char low = cs.charAt(i);
  ...
  /**
   * Returns a new {@link CharSequence} that is a subsequence of this sequence starting with the
   * {@code char} value at the specified index.
   *
   * <p>This provides the {@link CharSequence} equivalent to {@link String#substring(int)}. The
  ...
  /**
   * {@link CharSequenceUtils} instances should NOT be constructed in standard programming.
   *
