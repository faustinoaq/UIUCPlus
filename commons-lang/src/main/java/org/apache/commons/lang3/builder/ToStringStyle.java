...
/**
 * Controls {@link String} formatting for {@link ToStringBuilder}. The main public interface is
 * always via {@link ToStringBuilder}.
 *
 * <p>These classes are intended to be used as <em>singletons</em>. There is no need to instantiate
 * a new style each time. A program will generally use one of the predefined constants on this
 * class. Alternatively, the {@link StandardToStringStyle} class can be used to set the individual
 ...
 * object type (from {@code boolean} to {@code long} to {@link Object} to {@code int[]}) has its own
...
public abstract class ToStringStyle implements Serializable {
  ...
  /**
   * Appends to the {@code toString} an {@link Object} value, printing the full {@code toString} of
   * the {@link Object} passed in.
   *
   * @param buffer the {@link StringBuffer} to populate
   ...
   *
   * <p>This method performs the main lookup by Class type to correctly route arrays, {@link
   * Collection}s, {@link Map}s and {@link Objects} to the appropriate method.
   *
  ...
  /**
   * Appends to the {@code toString} an {@link Object} value that has been detected to participate
   * in a cycle. This implementation will print the standard string value of the value.
   *
   * @param buffer the {@link StringBuffer} to populate
   * @param fieldName the field name, typically not used as already appended
   * @param value the value to add to the {@code toString}, not {@code null}
  ...
  /**
   * Appends to the {@code toString} an {@link Object} value, printing the full detail of the {@link
   * Object}.
   *
   * @param buffer the {@link StringBuffer} to populate
   * @param fieldName the field name, typically not used as already appended
  ...
  /**
   * Appends to the {@code toString} an {@link Object} value, printing a summary of the {@link
   * Object}.
   *
   * @param buffer the {@link StringBuffer} to populate
   * @param fieldName the field name, typically not used as already appended
  ...
  protected void reflectionAppendArrayDetail(
    ...
    for (int i = 0; i < length; i++) {
      appendDetail(i, buffer, fieldName);
    }
    buffer.append(arrayEnd);
  }
   ...
   *
   * <p>The size summary is used to summarize the contents of {@link Collection}s, {@link Map}s and
   * arrays.
   *
   ...
   *
   * <p>This is an inner class rather than using {@link StandardToStringStyle} to ensure its
   * immutability.
   */
  ...
  /**
   * {@link ToStringStyle} that prints out the short class name and no identity hashcode.
   *
   * <p>This is an inner class rather than using {@link StandardToStringStyle} to ensure its
   * immutability.
   */
  ...
  /**
   * {@link ToStringStyle} that does not print out the classname, identity hashcode, content start
   * or field name.
   *
   * <p>This is an inner class rather than using {@link StandardToStringStyle} to ensure its
   * immutability.
  ...
  /**
   * {@link ToStringStyle} that does not print out the classname and identity hash code but prints
  ...
  private static final class JsonToStringStyle extends ToStringStyle {
    ...
    protected void appendFieldStart(final StringBuffer buffer, final String fieldName) {
      ...
      super.appendFieldStart(
          buffer, FIELD_NAME_QUOTE + StringEscapeUtils.escapeJson(fieldName) + FIELD_NAME_QUOTE);
    }
