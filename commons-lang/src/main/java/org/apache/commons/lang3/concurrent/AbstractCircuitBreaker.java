...
public abstract class AbstractCircuitBreaker<T> implements CircuitBreaker<T> {
  ...
  /**
   * Creates an {@link AbstractCircuitBreaker}. It also creates an internal {@link
  ...
  protected static boolean isOpen(final State state) {
    return state == State.OPEN || state == State.IN_PROGRESS;
  }

  /**
   * Changes the internal state of this circuit breaker. If there is actually a change of the state
   * value, all registered change listeners are notified.
   *
  ...
  /**
   * Adds a change listener to this circuit breaker. This listener is notified whenever the state of
   * this circuit breaker changes. If the listener is <strong>null</strong>, it is silently ignored.
   *
  ...
  /**
   * An internal enumeration representing the different states of a circuit breaker. This class also
   * contains some logic for performing state transitions. This is done to avoid complex
  ...
  protected enum State {
    ...
    /**
     * Returns the opposite state to the represented state. This is useful for flipping the current
     * state.
     *
