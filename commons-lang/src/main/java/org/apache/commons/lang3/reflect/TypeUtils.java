...
public class TypeUtils {
  ...
  private static final class GenericArrayTypeImpl implements GenericArrayType {
    ...
    public boolean equals(final Object obj) {
      return obj == this
          || obj instanceof GenericArrayType && TypeUtils.equals(this, (GenericArrayType) obj);
    }
  ...
  private static final class ParameterizedTypeImpl implements ParameterizedType {
    ...
    private ParameterizedTypeImpl(
        final Class<?> rawClass, final Type useOwner, final Type[] typeArguments) {
      this.raw = rawClass;
    ...
    public boolean equals(final Object obj) {
      return obj == this
          || obj instanceof ParameterizedType && TypeUtils.equals(this, (ParameterizedType) obj);
    }
  ...
  private static final class WildcardTypeImpl implements WildcardType {
    ...
    public boolean equals(final Object obj) {
      return obj == this
          || obj instanceof WildcardType && TypeUtils.equals(this, (WildcardType) obj);
    }
   ...
   * @since 3.2
   */
  private static <T> StringBuilder appendAllTo(
  ...
  private static void appendRecursiveTypes(
      final StringBuilder builder, final int[] recursiveTypeIndexes, final Type[] argumentTypes) {
  ...
  /**
   * Tests, recursively, whether any of the type parameters associated with {@code type} are bound
   * to variables.
   *
  ...
  private static boolean containsVariableTypeSameParametrizedTypeBound(
      final TypeVariable<?> typeVariable, final ParameterizedType parameterizedType) {
   ...
   * Iterable}. Since {@link TreeSet}'s parameter maps (indirectly) to {@link Iterable}'s parameter,
   * it will be able to determine that based on the super type {@code Iterable<? extends
   * Map<Integer, ? extends Collection<?>>>}, the parameter of {@link TreeSet} is {@code ? extends
   * Map<Integer, ? extends Collection<?>>}.
   *
   * @param cls the class whose type parameters are to be determined, not {@code null}
   * @param superParameterizedType the super type from which {@code cls}'s type arguments are to be
   *     determined, not {@code null}
   * @return a {@link Map} of the type assignments that could be determined for the type variables
   *     in each type in the inheritance hierarchy from {@code type} to {@code toClass} inclusive.
   */
  public static Map<TypeVariable<?>, Type> determineTypeArguments(
      final Class<?> cls, final ParameterizedType superParameterizedType) {
  ...
  private static Type[] extractTypeArgumentsFrom(
      final Map<TypeVariable<?>, Type> mappings, final TypeVariable<?>[] variables) {
  ...
  private static Type getClosestParentType(final Class<?> cls, final Class<?> superClass) {
    ...
    if (superClass.isInterface()) {
      ...
      for (final Type midType : interfaceTypes) {
        ...
        if (isAssignable(genericInterface, (Type) midClass)) {
  ...
  /**
   * Gets an array containing the sole type of {@link Object} if {@link TypeVariable#getBounds()}
   ...
   * WildcardType#getUpperBounds()} returns an empty array. Otherwise, it returns the result of
  ...
  /**
   * Gets the raw type of a Java type, given its context. Primarily for use with {@link
   * TypeVariable}s and {@link GenericArrayType}s, or when you do not know the runtime type of
   * {@code type}: if you know you have a {@link Class} instance, it is already raw; if you know you
   * have a {@link ParameterizedType}, its raw type is only a method call away.
   *
  ...
  private static Map<TypeVariable<?>, Type> getTypeArguments(
      Class<?> cls, final Class<?> toClass, final Map<TypeVariable<?>, Type> subtypeVarAssigns) {
   ...
   * implement the {@link Map} interface.
   *
   * <p>This method returns {@code null} if {@code type} is not assignable to {@code toClass}. It
   * returns an empty map if none of the classes or interfaces in its inheritance hierarchy specify
   * any type arguments.
   *
   * <p>A side effect of this method is that it also retrieves the type arguments for the classes
   * and interfaces that are part of the hierarchy between {@code type} and {@code toClass}. So with
   * the above example, this method will also determine that the type arguments for {@link
   * java.util.Hashtable Hashtable} are also both {@link Object}. In cases where the interface
   * specified by {@code toClass} is (indirectly) implemented more than once (e.g. where {@code
   * toClass} specifies the interface {@link java.lang.Iterable Iterable} and {@code type} specifies
   * a parameterized type that implements both {@link java.util.Set Set} and {@link
   * java.util.Collection Collection}), this method will look at the inheritance hierarchy of only
   * one of the implementations/subclasses; the first interface encountered that isn't a
   ...
   * @param toClass the class whose type parameters are to be determined based on the subtype {@code
   *     type}
   * @return a {@link Map} of the type assignments for the type variables in each type in the
   ...
   */
  private static Map<TypeVariable<?>, Type> getTypeArguments(
      final Type type, final Class<?> toClass, final Map<TypeVariable<?>, Type> subtypeVarAssigns) {
    if (type instanceof Class<?>) {
  ...
  /**
   * Tests if the subject type may be implicitly cast to the target class following the Java
   * generics rules.
   *
  ...
  /**
   * Tests if the subject type may be implicitly cast to the target generic array type following the
   * Java generics rules.
   *
   * @param type the subject type to be assigned to the target type
  ...
  private static boolean isAssignable(
    ...
    if (type instanceof GenericArrayType) {
      // compare the component types
      return isAssignable(
          ((GenericArrayType) type).getGenericComponentType(), toComponentType, typeVarAssigns);
    }
  ...
  /**
   * Tests if the subject type may be implicitly cast to the target parameterized type following the
   * Java generics rules.
   *
   * @param type the subject type to be assigned to the target type
   ...
   * rules. If both types are {@link Class} objects, the method returns the result of {@link
   ...
   */
  private static boolean isAssignable(
      final Type type, final Type toType, final Map<TypeVariable<?>, Type> typeVarAssigns) {
  ...
  /**
   * Tests if the subject type may be implicitly cast to the target type variable following the Java
   * generics rules.
   *
   * @param type the subject type to be assigned to the target type
  ...
  /**
   * Tests if the subject type may be implicitly cast to the target wildcard type following the Java
   * generics rules.
   *
   * @param type the subject type to be assigned to the target type
  ...
  /**
   * Strips out the redundant upper bound types in type variable types and wildcard types (or it
   * would with wildcard types if multiple upper bounds were allowed).
   *
   * <p>Example, with the variable type declaration:
   *
   * <pre>&lt;K extends java.util.Collection&lt;String&gt; &amp;
   * java.util.List&lt;String&gt;&gt;</pre>
   *
   * <p>since {@link List} is a subinterface of {@link Collection}, this method will return the
   * bounds as if the declaration had been:
   *
   ...
   * @throws NullPointerException if either {@code rawClass} or {@code typeVariableMap} is {@code
   *     null}
   * @since 3.2
   */
  public static final ParameterizedType parameterize(
    ...
    return parameterizeWithOwner(
        null, rawClass, extractTypeArgumentsFrom(typeVariableMap, rawClass.getTypeParameters()));
  }
  ...
  private static String parameterizedTypeToString(final ParameterizedType parameterizedType) {
    ...
    if (recursiveTypeIndexes.length > 0) {
      appendRecursiveTypes(
          builder, recursiveTypeIndexes, parameterizedType.getActualTypeArguments());
    } else {
      appendAllTo(builder.append('<'), ", ", parameterizedType.getActualTypeArguments())
          .append('>');
    }
  ...
  public static final ParameterizedType parameterizeWithOwner(
      final Type owner, final Class<?> rawClass, final Map<TypeVariable<?>, Type> typeVariableMap) {
    Objects.requireNonNull(rawClass, "rawClass");
    Objects.requireNonNull(typeVariableMap, "typeVariableMap");
    return parameterizeWithOwner(
        owner, rawClass, extractTypeArgumentsFrom(typeVariableMap, rawClass.getTypeParameters()));
  }
   ...
   */
  public static final ParameterizedType parameterizeWithOwner(
      final Type owner, final Class<?> rawClass, final Type... typeArguments) {
    Objects.requireNonNull(rawClass, "rawClass");
    final Type useOwner;
    if (rawClass.getEnclosingClass() == null) {
      ...
      Validate.isTrue(
          isAssignable(owner, rawClass.getEnclosingClass()),
          "%s is invalid owner type for parameterized %s",
          owner,
          rawClass);
      useOwner = owner;
    }
  ...
  private static Type substituteTypeVariables(
      final Type type, final Map<TypeVariable<?>, Type> typeVarAssigns) {
   ...
   * When a type parameter extends another (such as {@code <T, S extends T>}), uses another as a
   * type parameter (such as {@code <T, S extends Comparable>>}), or otherwise depends on another
   * type variable to be specified, the dependencies must be included in {@code typeVarAssigns}.
   *
   * @param typeVariableMap specifies the potential types to be assigned to the type variables, not
   *     {@code null}.
  ...
  /**
   * Looks up {@code typeVariable} in {@code typeVarAssigns} <em>transitively</em>, i.e. keep
   * looking until the value found is <em>not</em> a type variable.
   *
  ...
  private static Type unrollVariableAssignments(
      TypeVariable<?> typeVariable, final Map<TypeVariable<?>, Type> typeVarAssigns) {
    Type result;
    do {
  ...
  /**
   * {@link TypeUtils} instances should NOT be constructed in standard programming. Instead, the
   * class should be used as {@code TypeUtils.isAssignable(cls, toClass)}.
   *
