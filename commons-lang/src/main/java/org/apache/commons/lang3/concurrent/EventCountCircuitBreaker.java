 ...
 * href="https://martinfowler.com/bliki/CircuitBreaker.html">Circuit Breaker</a> pattern that counts
 * specific events.
 *
 * <p>A <em>circuit breaker</em> can be used to protect an application against unreliable services
 * or unexpected load. A newly created {@link EventCountCircuitBreaker} object is initially in state
 ...
 * threshold.
 *
 * <p>When a {@link EventCountCircuitBreaker} object is constructed the following parameters can be
 * provided:
 *
 * <ul>
 ...
 * going mad or due to a denial of service attack. A {@link EventCountCircuitBreaker} can be
 ...
 * typical example of such a scenario. Here the {@link EventCountCircuitBreaker} allows up to 1000
 ...
 * {@link PropertyChangeListener} objects that get notified whenever a state transition occurs. This
...
public class EventCountCircuitBreaker extends AbstractCircuitBreaker<Integer> {
   ...
   *
   * @param openingThreshold the threshold for opening the circuit breaker; if this number of events
   *     is received in the time span determined by the opening interval, the circuit breaker is
   *     opened
   ...
   * @param closingThreshold the threshold for closing the circuit breaker; if the number of events
   *     received in the time span determined by the closing interval goes below this threshold, the
   ...
   *
   * @param openingThreshold the threshold for opening the circuit breaker; if this number of events
   *     is received in the time span determined by the check interval, the circuit breaker is
   *     opened
   ...
   * @param closingThreshold the threshold for closing the circuit breaker; if the number of events
   *     received in the time span determined by the check interval goes below this threshold, the
   *     circuit breaker is closed again
   */
  public EventCountCircuitBreaker(
      final int openingThreshold,
   ...
   *
   * @param threshold the threshold for changing the status of the circuit breaker; if the number of
   *     events received in a check interval is greater than this value, the circuit breaker is
   *     opened; if it is lower than this value, it is closed again
   * @param checkInterval the check interval for opening or closing the circuit breaker
   * @param checkUnit the {@link TimeUnit} defining the check interval
   */
  public EventCountCircuitBreaker(
      final int threshold, final long checkInterval, final TimeUnit checkUnit) {
  ...
  /**
   * Increments the monitored value by <strong>1</strong> and performs a check of the current state
   * of this circuit breaker. This method works like {@link #checkState()}, but the monitored value
   * is incremented before the state check is performed.
   *
  ...
  /**
   * Updates the {@link CheckIntervalData} object. The current data object is replaced by the one
  ...
  /**
   * Changes the state of this circuit breaker and also initializes a new {@link CheckIntervalData}
   * object.
   *
  ...
  /**
   * Calculates the next {@link CheckIntervalData} object based on the current data and the current
   * state. The next data object takes the counter increment and the current time into account.
   *
   * @param increment the increment for the internal counter
   * @param currentData the current check data object
  ...
  private CheckIntervalData nextCheckIntervalData(
    ...
    if ((count >= this.openingInterval)
        || (stateStrategy(currentState).isCheckIntervalFinished(this, currentData, time))) {
  ...
  private static class CheckIntervalData {
    ...
    /**
     * Returns a new instance of {@link CheckIntervalData} with the event counter incremented by the
     * given delta. If the delta is 0, this object is returned.
     *
    ...
    public CheckIntervalData increment(final int delta) {
      return (delta == 0)
          ? this
          : new CheckIntervalData(getEventCount() + delta, getCheckIntervalStart());
    }
  }

  /**
   * Internally used class for executing check logic based on the current state of the circuit
   * breaker. Having this logic extracted into special classes avoids complex if-then-else cascades.
   */
  private abstract static class StateStrategy {
    ...
    /**
     * Checks whether the specified {@link CheckIntervalData} objects indicate that a state
     * transition should occur. Here the logic which checks for thresholds depending on the current
    ...
    /**
     * Obtains the check interval to applied for the represented state from the given {@link
     * CircuitBreaker}.
     *
     * @param breaker the {@link CircuitBreaker}
