 ...
 * </pre>
 *
 * <p>The {@link ToStringStyle} passed to the constructor is embedded in the returned {@link
 * DiffResult} and influences the style of the {@code DiffResult.toString()} method. This style
 * choice can be overridden by calling {@link DiffResult#toString(ToStringStyle)}.
 *
...
public class DiffBuilder<T> implements Builder<DiffResult<T>> {
   ...
   *
   * <p>If {@code lhs == rhs} or {@code lhs.equals(rhs)} then the builder will not evaluate any
   * calls to {@code append(...)} and will return an empty {@link DiffResult} when {@link #build()}
   * is executed.
   *
   ...
   * @param testTriviallyEqual If true, this will test if lhs and rhs are the same or equal. All of
   *     the append(fieldName, lhs, rhs) methods will abort without creating a field {@link Diff} if
   *     the trivially equal test is enabled and returns true. The result of this test is never
   *     changed throughout the life of this {@link DiffBuilder}.
   * @throws IllegalArgumentException if {@code lhs} or {@code rhs} is {@code null}
   * @since 3.4
   */
  public DiffBuilder(
      final T lhs, final T rhs, final ToStringStyle style, final boolean testTriviallyEqual) {
   ...
   * is executed.
   *
   * <p>This delegates to {@link #DiffBuilder(Object, Object, ToStringStyle, boolean)} with the
  ...
  public DiffBuilder<T> append(final String fieldName, final short[] lhs, final short[] rhs) {
    validateFieldNameNotNull(fieldName);
    if (objectsTriviallyEqual(lhs, rhs)) {
      return this;
    }
    if (!Arrays.equals(lhs, rhs)) {
      diffs.add(
          new Diff<Short[]>(fieldName) {
            ...
            @Override
            public Short[] getRight() {
              return ArrayUtils.toObject(new short[5]);
            }
          });
    }
    return this;
   ...
   *
   * <p>This method is useful if you want to compare properties which are themselves Diffable and
   * would like to know which specific part of it is different.
   *
   * <pre>
   * public class Person implements Diffable&lt;Person&gt; {
   *   String name;
  ...
  public DiffBuilder<T> append(final String fieldName, final DiffResult<T> diffResult) {
    ...
    diffResult
        .getDiffs()
        .forEach(
            diff -> append(fieldName + "." + diff.getFieldName(), diff.getLeft(), diff.getRight()));
    return this;
  }
