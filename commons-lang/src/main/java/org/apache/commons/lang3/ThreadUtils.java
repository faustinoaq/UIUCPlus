...
public class ThreadUtils {
   ...
   *
   * <p>A predicate implementation which matches a thread or thread group name.
   *
   * @deprecated Use a {@link Predicate}.
   */
  @Deprecated
  ...
  public static Thread findThreadById(final long threadId) {
    ...
    final Collection<Thread> result =
        findThreads((Predicate<Thread>) t -> t != null && t.getId() == threadId);
    return result.isEmpty() ? null : result.iterator().next();
  }

  /**
   * Finds the active thread with the specified id if it belongs to a thread group with the
   * specified group name.
   *
   * @param threadId The thread id
   * @param threadGroupName The thread group name
   * @return The threads which belongs to a thread group with the specified group name and the
   *     thread's id match the specified id. {@code null} is returned if no such thread exists
   ...
   * @param threadId The thread id
   * @param threadGroup The thread group
   * @return The thread which belongs to a specified thread group and the thread's id match the
   *     specified id. {@code null} is returned if no such thread exists
  ...
  /**
   * Finds all active thread groups which match the given predicate and which is a subgroup of the
   ...
   * @param predicate the predicate
   * @return An unmodifiable {@link Collection} of active thread groups which match the given
   ...
   * @since 3.13.0
   */
  public static Collection<ThreadGroup> findThreadGroups(
      final ThreadGroup threadGroup,
      final boolean recurse,
      final Predicate<ThreadGroup> predicate) {
    Objects.requireNonNull(threadGroup, "threadGroup");
    ...
    return Collections.unmodifiableCollection(
        Stream.of(threadGroups).filter(predicate).collect(Collectors.toList()));
  }
  ...
  @Deprecated
  public static Collection<ThreadGroup> findThreadGroups(
      final ThreadGroup threadGroup, final boolean recurse, final ThreadGroupPredicate predicate) {
   ...
   *
   * @param threadGroupName The thread group name
   * @return the thread groups with the specified group name or an empty collection if no such
   *     thread group exists. The collection returned is always unmodifiable.
  ...
  /**
   * Finds all active threads which match the given predicate and which belongs to the given thread
   ...
   * @param predicate the predicate
   * @return An unmodifiable {@link Collection} of active threads which match the given predicate
  ...
  public static Collection<Thread> findThreads(
      final ThreadGroup threadGroup, final boolean recurse, final Predicate<Thread> predicate) {
    ...
    Thread[] threads;
    do {
      threads = new Thread[10000 + count / 2 + 1]; // slightly grow the array size
      count = threadGroup.enumerate(threads, recurse);
      // return value of enumerate() must be strictly less than the array size according to javadoc
    } while (count >= threads.length);
    return Collections.unmodifiableCollection(
        Stream.of(threads).filter(predicate).collect(Collectors.toList()));
  }
  ...
  @Deprecated
  public static Collection<Thread> findThreads(
      final ThreadGroup threadGroup, final boolean recurse, final ThreadPredicate predicate) {
   ...
   * @return The threads with the specified name or an empty collection if no such thread exists.
  ...
  /**
   * Finds active threads with the specified name if they belong to a thread group with the
   * specified group name.
   *
   * @param threadName The thread name
   * @param threadGroupName The thread group name
   * @return The threads which belongs to a thread group with the specified group name and the
   *     thread's name match the specified name, An empty collection is returned if no such thread
   ...
   *     thread group up to the system thread group
   */
  public static Collection<Thread> findThreadsByName(
      final String threadName, final String threadGroupName) {
    Objects.requireNonNull(threadName, "threadName");
    Objects.requireNonNull(threadGroupName, "threadGroupName");
    return Collections.unmodifiableCollection(
        findThreadGroups(predicateThreadGroup(threadGroupName)).stream()
   ...
   * @param threadName The thread name
   * @param threadGroup The thread group
   * @return The threads which belongs to a thread group and the thread's name match the specified
   *     name, An empty collection is returned if no such thread exists. The collection returned is
   *     always unmodifiable.
   ...
   */
  public static Collection<Thread> findThreadsByName(
      final String threadName, final ThreadGroup threadGroup) {
    return findThreads(threadGroup, false, predicateThread(threadName));
  }

  /**
   * Gets all active thread groups excluding the system thread group (A thread group is active if it
   * has been not destroyed).
   *
   ...
   *
   * <p>This method returns null if this thread has died (been stopped).
   *
  ...
  /**
   * Waits for the given thread to die for the given duration. Implemented using {@link
   ...
   * @since 3.12.0
   */
  public static void join(final Thread thread, final Duration duration)
      throws InterruptedException {
    DurationUtils.accept(thread::join, duration);
  }

  private static <T> Predicate<T> namePredicate(
      final String name, final Function<T, String> nameGetter) {
    return (Predicate<T>)
        t -> t != null && Objects.equals(nameGetter.apply(t), Objects.requireNonNull(name));
  }
  ...
  /**
   * Sleeps the current thread for the given duration. Implemented using {@link Thread#sleep(long,
   * int)}.
   *
   ...
   *
   * <p>The sleep duration may be shorter than duration if we catch a {@link InterruptedException}.
   *
  ...
  /**
   * ThreadUtils instances should NOT be constructed in standard programming. Instead, the class
   * should be used as {@code ThreadUtils.getAllThreads()}
   *
