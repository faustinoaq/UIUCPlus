 ...
 *
 * <p>This class contains various utility methods that make working with annotations simpler.
 *
 * <p>{@link Annotation} instances are always proxy objects; unfortunately dynamic proxies cannot be
 * depended upon to know how to implement certain methods in the same manner as would be done by
 * "natural" {@link Annotation}s. The methods presented in this class can be used to avoid that
 * possibility. It is of course also possible for dynamic proxies to actually delegate their e.g.
 * {@link Annotation#equals(Object)}/{@link Annotation#hashCode()}/ {@link Annotation#toString()}
 * implementations to {@link AnnotationUtils}.
 *
 * <p>#ThreadSafe#
 *
 * @since 3.0
...
public class AnnotationUtils {
  ...
  private static final ToStringStyle TO_STRING_STYLE =
      new ToStringStyle() {
        ...
        protected void appendDetail(
            final StringBuffer buffer, final String fieldName, Object value) {
  ...
  /**
   * {@link AnnotationUtils} instances should NOT be constructed in standard programming. Instead,
  ...
  /**
   * Checks if two annotations are equal using the criteria for equality presented in the {@link
   * Annotation#equals(Object)} API docs.
   *
   * @param a1 the first Annotation to compare, {@code null} returns {@code false} unless both are
   *     {@code null}
   * @param a2 the second Annotation to compare, {@code null} returns {@code false} unless both are
   *     {@code null}
  ...
  /**
   * Generate a hash code for the given annotation using the algorithm presented in the {@link
   * Annotation#hashCode()} API docs.
   *
   * @param a the Annotation for a hash code calculation is desired, not {@code null}
   * @return the calculated hash code
   * @throws RuntimeException if an {@link Exception} is encountered during annotation member access
  ...
  public static int hashCode(final Annotation a) {
    ...
    for (final Method m : type.getDeclaredMethods()) {
      try {
        ...
        result += hashMember(field.getName(), value);
      } catch (final ReflectiveOperationException ex) {
  ...
  /**
   * Generate a string representation of an Annotation, as suggested by {@link
   * Annotation#toString()}.
   *
   ...
   *
   * <p>The Java language specification only permits certain types to be used in annotations. These
   * include {@link String}, {@link Class}, primitive types, {@link Annotation}, {@link Enum}, and
   * single-dimensional arrays of these types.
   *
  ...
  /**
   * Helper method for checking whether two objects of the given type are equal. This method is used
  ...
  private static boolean arrayMemberEquals(
      final Class<?> componentType, final Object o1, final Object o2) {
