 ...
 *
 * <p>Originally from Apache Commons VFS with more added to act as a fluent replacement for {@link
 * java.util.BitSet}.
 *
 * @since 3.13.0
 */
public final class FluentBitSet implements Cloneable, Serializable {
   ...
   * in the range {@code 0} through {@code nbits-1}. All bits are initially {@code false}.
   *
  ...
  /**
   * Performs a logical <b>AND</b> of this target bit set with the argument bit set. This bit set is
   * modified so that each bit in it has the value {@code true} if and only if it both initially had
   * the value {@code true} and the corresponding bit in the bit set argument also had the value
   * {@code true}.
   *
   * @param set a bit set.
   * @return this.
  ...
  /**
   * Clears all of the bits in this {@link BitSet} whose corresponding bit is set in the specified
   * {@link BitSet}.
   *
  ...
  /**
   * Sets the bits from the specified {@code fromIndex} (inclusive) to the specified {@code toIndex}
   * (exclusive) to {@code false}.
   *
  ...
  /**
   * Cloning this {@link BitSet} produces a new {@link BitSet} that is equal to it. The clone of the
   * bit set is another bit set that has exactly the same bits set to {@code true} as this bit set.
   *
  ...
  /**
   * Sets each bit from the specified {@code fromIndex} (inclusive) to the specified {@code toIndex}
   ...
   * with the index {@code bitIndex} is currently set in this {@link BitSet}; otherwise, the result
   * is {@code false}.
   *
  ...
  /**
   * Returns a new {@link BitSet} composed of bits from this {@link BitSet} from {@code fromIndex}
  ...
  /**
   * Returns true if the specified {@link BitSet} has any bits set to {@code true} that are also set
   * to {@code true} in this {@link BitSet}.
   *
  ...
  /**
   * Returns the "logical size" of this {@link BitSet}: the index of the highest set bit in the
   * {@link BitSet} plus one. Returns zero if the {@link BitSet} contains no set bits.
   *
   ...
   *
   * <p>To iterate over the {@code true} bits in a {@link BitSet}, use the following loop:
   *
   * <pre>{@code
  ...
  /**
   * Performs a logical <b>OR</b> of this bit set with the bit set argument. This bit set is
   * modified so that a bit in it has the value {@code true} if and only if it either already had
   * the value {@code true} or the corresponding bit in the bit set argument has the value {@code
   * true}.
   *
   * @param set a bit set.
   * @return this.
  ...
  /**
   * Performs a logical <b>OR</b> of this bit set with the bit set arguments. This bit set is
   * modified so that a bit in it has the value {@code true} if and only if it either already had
   * the value {@code true} or the corresponding bit in the bit set argument has the value {@code
   * true}.
   *
  ...
  /**
   * Returns the index of the nearest bit that is set to {@code false} that occurs on or before the
  ...
  /**
   * Returns the number of bits of space actually in use by this {@link BitSet} to represent bit
  ...
  /**
   * Returns a stream of indices for which this {@link BitSet} contains a bit in the set state. The
   * indices are returned in order, from lowest to highest. The size of the stream is the number of
   * bits in the set state, equal to the value returned by the {@link #cardinality()} method.
   *
   * <p>The bit set must remain constant during the execution of the terminal stream operation.
   ...
   *
   * <ol>
   *   <li>{@code byte[] bytes = s.toByteArray();}
   *   <li>then {@code bytes.length == (s.length()+7)/8} and
   *   <li>{@code s.get(n) == ((bytes[n/8] & (1<<(n%8))) != 0)}
   *   <li>for all {@code n < 8 * bytes.length}.
   * </ol>
   *
   ...
   *
   * <ol>
   *   <li>{@code long[] longs = s.toLongArray();}
   *   <li>then {@code longs.length == (s.length()+63)/64} and
   *   <li>{@code s.get(n) == ((longs[n/64] & (1L<<(n%64))) != 0)}
   *   <li>for all {@code n < 64 * longs.length}.
   * </ol>
   *
   * @return a byte array containing a little-endian representation of all the bits in this bit set
   */
  ...
  /**
   * Performs a logical <b>XOR</b> of this bit set with the bit set argument. This bit set is
   * modified so that a bit in it has the value {@code true} if and only if one of the following
   * statements holds:
   *
   * <ul>
   *   <li>The bit initially has the value {@code true}, and the corresponding bit in the argument
   *       has the value {@code false}.
   *   <li>The bit initially has the value {@code false}, and the corresponding bit in the argument
